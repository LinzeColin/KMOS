...
No GPU at all? `--soft-gl` also switches on flat mode (`--flat`, or `PROJECT.flat`): watercolour fills are painted as equivalent flat washes, which takes a frame from tens of seconds to about 0.1 s in software WebGL while keeping the ink, boil and paper grain. `--page=<file>` renders another studio page (a production folder that loads this kit's `src/`). Fonts load from `assets/fonts/`, so rendering works offline.
