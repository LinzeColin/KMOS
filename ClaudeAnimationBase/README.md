...
No GPU at all? `--soft-gl` also switches on flat mode (`--flat`, or `PROJECT.flat`): watercolour fills are painted as equivalent flat washes, which takes a 1080p frame from 40 s or more to about 3 s in software WebGL (SwiftShader, 4 cores) while keeping the ink, boil and paper grain. The ms/frame that sheets print counts only queuing the draw calls; the real cost lands when the frame is read back. `--density=0.6667` paints at two-thirds resolution and scales up (lettering stays sharp), about 1.7 s per frame. `--page=<file>` renders another studio page (a production folder that loads this kit's `src/`). Fonts load from `assets/fonts/`, so rendering works offline.
