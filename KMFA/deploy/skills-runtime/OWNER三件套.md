...
- ~~一条 DNS~~ skills 无入站、无需域名；App（P2）上线时才需一条 Cloudflare 记录。根域名按公开软件合同匿名开放，但现有财务/运维 `/api*`、`/ops*` 必须使用路径级 Cloudflare Access，并由源站校验 Access JWT；具体四类路径与回滚顺序以 `../coolify/README.md` 为准。
