...
16. Each scheduled trigger executes exactly one corresponding window command once, with explicit `--input-zip`; never execute both windows in one task.
17. Scheduled commands must not include `--cleanup` or `--apply`. Cleanup is manual maintenance only.
18. Stream required ZIP members without copying or extracting them. Do not automatically evict the ZIP after each run.
