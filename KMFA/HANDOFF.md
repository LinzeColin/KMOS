...
- 最近完成的唯一执行单元：**S04 / P4.4 / T-S04-04 anonymous abuse control 本地 phase candidate**，见 `machine/runs/S04_P44_ABUSE_CONTROL.md`。新增版本化 `p44-v1` admission policy：按 identity/recovery/mutation/upload/export/read/unknown 分层，使用 10 秒+1 小时、edge IP/device/workspace/global 四作用域预算，upload/export 全局并发预算 `2/4`；actor 局部超限触发 90 秒一次性、actor/workspace/operation 绑定的 SHA-256 proof，global/并发不可被挑战绕过。控制 SQLite 与业务库隔离，只存 HMAC tag、限时 counter 和 16 桶聚合事件；invalid config/控制面故障对昂贵 API fail closed，根页和浅状态保持可浏览，紧急模式只放宽低成本 read/mutation。业务侧硬上限覆盖 workspace/session/总 artifact bytes/磁盘保留/audit rows，拒绝事务不删除既有资产。最终镜像 `sha256:63e37f89…e8e65` 的正常夹具误伤 `0/100`、暴力/上传/导出/6 路并发/602 actor 低速分布式攻击绕过 `0`，并发 admitted/blocked=`2/4`、第 601 个 sustained 请求全局阻断且下一窗口恢复，根页/状态始终 `200`；浏览器自动挑战、P4.2/P4.3 恢复/secret/hash/Flag、公共壳、三浏览器 a11y/index 与私有 `11/11` 全 PASS。backend `156/156`，focused 连续三次 `10/10`，phase review findings `10/10` 已修复、open `0`。S03 legacy ID、schema v1、verifier、卷、对象与 v1.5 恢复资产未迁移、重写或删除。本 phase 未做 S04 overall review、GitHub push、Cloudflare WAF mutation 或生产部署。
- P4.3 secret hygiene 继续有效：浏览器 API 不返回 access token 明文，改用 host-only `__Secure-kmfa_session`（Secure/HttpOnly/SameSite=Strict/API Path/1h），携 Cookie 写操作强制 scheme/host 同源；服务端显式撤销与恢复 secret 轮换原子撤销旧 session。全局边界拒绝 raw/percent-encoded capability URL/Referer，服务端与浏览器 console 脱敏、raw access log 关闭、CSP `connect-src 'self'` 且无第三方分析/错误 SDK。P4.4 最终镜像复跑 request URL `43`、performance URL `38`、error sample `7`、walking no-store response `18`、日志/状态/cache/screenshot，全部 canary 命中 `0`、foreign request `0`；轮换与显式撤销旧 session 重放均 `404`。
...
- 最近已发布 Stage 是 **S03 全 Stage Review + guarded rollout**：远端 `main=ce9c27da2c6518fec2c740897c788bd63069678f`，同 SHA deploy Gate run `29955179158` PASS，根域 Everyone Bypass、`/api*`/`/ops*` owner-only Access、源站 403 fail-closed、Walking Skeleton `1→0→1` 与旧数据恢复/hash Oracle 全部闭合；published Stage=`4/14`。S03 仍只证明 early skeleton，不把完整匿名旅程、长期持久化、G2 或 GA 提前标绿。
...
- 当前生产唯一身份由 read-only query run `29996883514` 回显：deployment source `ce9c27da2c6518fec2c740897c788bd63069678f`、image `sha256:bafa6a96547ed9bed3f62e0ea956a27963be5078b91bf68d24f77c857a44e60c`、Coolify deployment `amffqczj7p2o22yz3en1or13`、completed `2026-07-23T09:42:56.000000Z`、status `finished`。P4.1-P4.4 是未发布本地 candidate，不得冒充该生产身份。
...
- P0.4/S00 的失败边界已局部刷新：生产现已挂载 `kmfa-app-state`，并以合成项目/文件证明单节点跨部署重启和 Flag 回滚保留；但生产关系数据库、S3-compatible object adapter 与 backup/restore 均未实现，所以有限 RPO/RTO 仍不可证明，继续按 `unbounded/not recoverable` 处理。Named volume 不得被伪报为 S05 durable PASS。
- 当前总进度：本地完成 Task `20/56`；S04 phase `4/4`；published Stage `4/14`。P4.4 phase review findings `10/10` 已最小修复、open `0`；它只可本地提交，不上传、不部署。完整匿名使用、耐久 DB/object、备份恢复、明确删除和 `G2` 仍未完成。
- 下一个新 run 只能执行 **S04 whole-stage review / fix / upload**：整体复核 P4.1-P4.4 的 source→image→evidence 身份、匿名身份/恢复/secret/abuse 跨相位不变量、Flag/策略回滚、CI/发布 Gate 与 v1.5 恢复资产保持；先解决全部 Stage finding，再整体上传 GitHub。不得顺带进入 S05 或把本地 `sha256:63e37f89…e8e65` 冒充生产制品；若 Stage review 未全绿，禁止 push/deploy，`kmfa-app-state`、既有 verifier/session/object 与 v1.5 恢复资产不得删除。
