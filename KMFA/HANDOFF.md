...
- 最近完成的唯一执行单元：**S07 / P7.2 / T-S07-02 Range 与批量 ZIP 下载**，见 `machine/runs/S07_P72_RANGE_BATCH_DOWNLOAD.md`。默认关闭的 `KMFA_RANGE_BATCH_DOWNLOAD_ENABLED` 在 P7.1 精确 selector 上增加单 Range/稳定 SHA-256 ETag/并行续传，以及最多 `500` 项、`512 MiB` 原始字节预算的 deterministic ZIP32 stream。归档只保留有界中央目录元数据、一次物化并复核一个对象、每 chunk 不超过 `64 KiB`；ordinal 独立目录阻止重名覆盖和 zip-slip，canonical `manifest.json` 提供逐项来源/size/hash。最终本地 image `sha256:3dfe4a4d…a63ab5` 上并行 Range=`2`、重组 hash match=`true`，ZIP missing/overwrite/traversal=`0/0/0`，server whole-archive buffered=`false`、archive temp=`0`；Chromium 即时注入一次 connection reset 后只重试当前 chunk 一次并完成，批量 ZIP 已实际打开复核。重启、P7.2 rollback 与 re-enable 均保留资产，P7.1 单文件继续可用。没有真实时间 soak、观察期、等待窗口、全量测试前置、上传、部署或灰度。
- P7.1 单文件下载继续有效，见 `machine/runs/S07_P71_SINGLE_FILE_DOWNLOAD.md`：默认关闭的 `KMFA_SINGLE_FILE_DOWNLOAD_ENABLED` 为已授权 workspace 提供 JSON body opaque selector，可精确下载历史原件版本、registry-validated 派生物及已进入 workspace artifact registry 的报告文件；asset ID 不进入 URL，响应强制 attachment + nosniff，并返回/由浏览器复核类型、大小、checksum、源版本与上传 operation/processor。旧全局报告中心未接入；export job 与 published snapshot 仍分别留给 P7.3/S08。
...
- 当前总进度：Task `30/56`；S07 phase `2/4`，S07 整体复审未开始；P7.1/P7.2 + abuse/walking/public-entry focused regression `54 passed`，final-image API + Chromium Oracle PASS，并行 Range/hash=`2/match`，ZIP missing/overwrite/traversal=`0/0/0`，server whole-archive buffered/archive-temp/secret-hit=`false/0/0`。此前 full-backend 结果仅作历史诊断，不是当前或后续前置。published Stage 仍为 `6/14`，因为中间 phase/Stage 均不上传，须待全部 `56/56` Task 与最终整包复审完成后一次性上传。生产 P6.1/P6.2/P6.3/P7.1/P7.2 Flags 保持 `0`，G2 与 GA 均未晋级。
- 下一个新 run 只可执行 **S07 / P7.3 / T-S07-03**，且最多一个 phase；不得提前进入 P7.4、S07 整体复审、上传、部署、灰度、启用生产 Flags，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 assessment/event/intent/chunk/original/derivative/backup、既有 verifier/session 或 v1.5 恢复资产。时间逻辑只能使用 Fake Clock、历史回放、Fixture、模拟消息和即时故障注入；禁止真实时间 soak、观察期、等待窗口、后台空转、重复审批、形式化 Gate、无限重试和把全量测试作为前置。P7.2 快速回滚置 `KMFA_RANGE_BATCH_DOWNLOAD_ENABLED=0`，继续保留 P7.1 精确单文件下载及 schema `6` 全部状态；禁止 binary/schema downgrade。仅在整个 v1.5.2 Taskpack 完成并通过最终整包复审后一次性上传 GitHub。
