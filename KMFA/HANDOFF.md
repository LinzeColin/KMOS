...
- 最近完成的唯一执行单元：**S06 / P6.2 / T-S06-02 隔离与扫描**，见 `machine/runs/S06_P62_QUARANTINE_SCAN.md`。在 P6.1 parent `59c65933…` 上增加默认关闭的 `KMFA_FILE_SECURITY_ENABLED`、schema `5` durable assessment + append-only events、quarantine-first projection、HMAC/nonce/stream checksum 私网 scanner protocol、credential-minimal scanner container 与 bounded retry worker。最终镜像 `sha256:33f5325d…c333f` 上 EICAR、ZIP traversal/bomb、malformed media 等恶意/畸形 clean-or-preview escape `0`；MIME spoof、双扩展、macro、unknown 均 attachment-only；合法误拒 policy `0/100`、final-image `0/12`。timeout/unavailable 均非 clean 且恢复后 retry 收敛；scanner non-root/read-only/internal、DB/object env `0`、state mount/host port `0`；重启、full restore 和 Flag rollback 均保留状态，persisted rejected 继续 `409`。浏览器根页真实创建/上传/clean 提示/下载 PASS、preview control `0`，full backend `269/269`。这是有界 content firewall，不冒充 signature-complete 商业 AV；所有状态仍不执行、不预览。本 phase 未做 S06 整体复审、GitHub upload、部署或生产 rollout。
...
- 2026-07-26 收口前已自行 fetch：`origin/main=c00a90f5…` 相对共同基线 `12d6fa9f…` 新增 `43` 个 KMIDS commits；`KMFA/` 与 `.github/workflows/` scoped diff 为 `0`。本地 P6.1/P6.2 chain 未被远端删除；S06 whole-stage review/push 前须整合当时最新远端并重跑完整门禁，当前脏 phase 不做 merge/rebase/reset。
...
- 当前总进度：Task `26/56`；S06 phase `2/4`，published Stage `6/14`。P6.1/P6.2 phase findings `9/9 + 7/7` resolved、open/accepted risk `0/0`；full backend `269/269`，final-image quarantine/scan 与浏览器 Oracle PASS。生产 P6.1/P6.2 Flags 仍须保持 `0` 至 S06 whole-stage review；G2 仍需完成 S06/S07，GA 未通过。
- 下一个新 run 只可执行 **S06 / P6.3 / T-S06-03** 版本、血缘与预览；不得提前进入 P6.4、做 S06 whole-stage review、上传本中间 phase 到 GitHub、启用生产 P6.1/P6.2，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 assessment/event/intent/chunk/object/backup、既有 verifier/session 或 v1.5 恢复资产。P6.2 快速回滚使用同一 schema `5` binary，先置 `KMFA_FILE_SECURITY_ENABLED=0` 再停止 scanner/worker；persisted rejected 继续阻断，未判定文件仅 attachment-only，禁止 binary/schema downgrade。若需停全部新上传再独立置 `KMFA_CONSISTENCY_STATE_MODE=paused`。
