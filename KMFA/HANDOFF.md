...
- 下一步：若 owner 要继续推进，应直接打开 S111 `ocr_fact_candidate_owner_decision_review_batch.xlsx`，先看受保护的 `Review Summary` 确认 12 条候选的 metric/focus status 分布，再在 `Owner Review` 中只填写解锁的 owner 输入列，之后运行 `install_owner_decision_manifest.py --draft-xlsx-path <reviewed.xlsx>` validation-only dry-run；不得把相邻聊天上下文自动解释为字段授权、账本写入或管理结论。
