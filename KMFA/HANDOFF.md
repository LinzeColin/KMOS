...
- 当前执行合同：`KMFA_每日资金_v0.0.0.1_FINAL_TASKPACK.zip`，SHA-256 `072ab87c8d48acbd1732f47ff2edc76cf819c4537e60637f6bd5bf39233252b1`。2026-08-02 收尾取移动基线：`origin/main=507fa187cda1ca3f08edd20556435703ab44af75`，与本地 HEAD 同一提交。Coolify 应用清单仍记录 `running:healthy`，但最新 deployments 只读查询（Action `30717596148`）返回 0 条，container execute（`30717536566`）返回 404；故当前生产 source/image/deployment identity 为 `UNKNOWN`。`f75a1dc6…` 仅是最后一条历史源码记录，不能冒充现网身份，更不得把平台状态写成业务链路 PASS。
...
- 采集请求现在先由 DWS 以 `--conversation-ids <唯一群>` 与 `--user <唯一发送人>` 双过滤，再由本地逐条校验群、发送人及文档族三重门禁。恢复演练专用 D1 现已纳入正常 runtime 配置的必填项，且必须与正式 D1 不同；空值/同值均 fail-closed，普通 preflight/runtime-audit 报 `CONFIG_INVALID`，恢复演练保留 `RESTORE_DRILL_CONFIG_INVALID` 的可操作状态。runtime-audit 仅写脱敏卷/进程/网络计数证据；T01 时 27 项合约测试、Python 编译、shell 语法、Compose 配置、workflow YAML 与 diff 检查均通过，T03 收尾计数为 35（T04 当前计数见上）。Docker daemon 未运行，故本地容器启动/挂载 Oracle 为 `UNKNOWN`，不是 PASS。
