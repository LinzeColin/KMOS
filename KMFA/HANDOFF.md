...
- 最近完成的唯一执行单元：**S06 whole-stage review + exposed finding repair**，见 `machine/runs/S06_STAGE_REVIEW.md`。P6.1-P6.4 的 `AC-UP-001..004` 全部本地 PASS，10 个跨 phase finding 全部解决，open/waived risk `0/0`。Owner 已禁止真实时间 soak、观察/等待窗口、全量测试与连续运行 N 小时作为前置；P6.4 v2 改为 12 个固定 Fixture 即时回放、scanner synchronous recovery、object unavailable `503 → replay 200` 与 Fake Clock abuse focused tests，`10/10` negative matrix、data invariant/isolation/unexplained failure `0/0/0`，`real_time_soak_used=false`。reviewed runtime evidence image 为 `sha256:ed2175dc…d3812`；它不是发布候选。本地 Task `28/56`，S06 `4/4` + 整体复审已通过，但按最新 Owner 合同，完成整个 v1.5.2 Taskpack 前不上传 GitHub、不部署、不灰度。
...
- 当前总进度：Task `28/56`；S06 phase `4/4` 与 whole-stage review 均已本地 PASS，10 个整体复审 finding 全部解决，open/waived risk `0/0`。即时确定性证据为 P6.4 固定 Fixture negative matrix `10/10`、data invariant/isolation/unexplained failure `0/0/0`，以及 post-method focused regression `62 passed`（1 条继承依赖弃用 warning）；此前 full-backend 结果仅作历史诊断，不是当前或后续前置。published Stage 仍为 `6/14`，因为中间 phase/Stage 均不上传，须待全部 `56/56` Task 与最终整包复审完成后一次性上传。生产 P6.1/P6.2/P6.3 Flags 保持 `0`，G2 与 GA 均未晋级。
- 下一个新 run 只可执行 **S07 / P7.1 / T-S07-01**，且最多一个 phase；不得在该 run 上传、部署、灰度、启用生产 P6.1/P6.2/P6.3，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 assessment/event/intent/chunk/original/derivative/backup、既有 verifier/session 或 v1.5 恢复资产。时间逻辑只能使用 Fake Clock、历史回放、Fixture、模拟消息和即时故障注入；禁止真实时间 soak、等待窗口和把全量测试作为前置。快速回滚先降低上传并发/文件上限、排队或降级 scanner，再按需置 `KMFA_ARTIFACT_DERIVATION_ENABLED=0`、`KMFA_FILE_SECURITY_ENABLED=0`；若需停全部新上传，再置 `KMFA_CONSISTENCY_STATE_MODE=paused`。保留 schema `6`、原件、versions、lineage、processing runs、derivatives、所有对象与备份，禁止 binary/schema downgrade。仅在整个 v1.5.2 Taskpack 完成并通过最终整包复审后一次性上传 GitHub。
