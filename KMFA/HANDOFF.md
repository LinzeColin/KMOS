...
- **2026-08-10 T10 最新 main 生产复核（当前，优先于本节内所有较早状态）**：当前生产源码与执行时 `origin/main` 均为 `e1cf006b6483ecb9323f15d3f9f9cc992cfaee17`；只读 deployment-info 回执闭合了该 source、镜像 digest 和 finished deployment 的同一证据链。根页面保持匿名 `200`，未携带 Access 身份的 `/ops` 为 `403`。固定 Access history-probe 已在该版本真实完成：transport `OK`、`DWS_GROUP_HISTORY_PROBE_COMPLETED`、`GROUP_HISTORY_V2_FIRST_PAGE_TERMINAL`，本次临时 Access policy/token 已精确清理；该探针按设计不下载附件、不写 raw。生产公开 status 随后观测到连续两条部署后的 `*/15` 轮询：`2026-08-09T16:15:03Z` 与 `2026-08-09T16:30:06Z`，间隔 `903` 秒，均终态为 `SOURCE_MATCH_ZERO`；这证明当前独立调度在运行，只表示两轮未形成目标文档事实，不能扩展为全历史为空或任何金额结论。当前每日资金合约 `196 passed, 1 skipped`、后端 API `20 passed`、前端 production build、仓库投影和原任务包 verifier（`24 requirements / 11 tasks / 17 acceptance`）均通过；通用 Verifier 因其自身不接受包内 `PURSUING_GOAL.txt` 文件名而未形成正式导入结论，这只是工具兼容缺口，不能冒充产品或资金 PASS。当前没有真实已验证 parser-open 的同日余额＋流水事实对，故整数分勾稽、D1、R2、OCI、恢复与资金 publication 仍一律为 `NOT_RUN/NOT_MET`，严禁发布资金金额。详见 `machine/runs/daily_funds/T10_CURRENT_MAIN_PRODUCTION_RECONCILE_20260810.json`。
