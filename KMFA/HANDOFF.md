...
- Codex App automation `kmfa` 当前契约为 `Australia/Sydney` 本地每天 11:30，repo contract 与本机 automation drift check 已纳入验证。
...
- runner 现在生成 `automation_readiness.csv`，只读核对 tracked contract 与本机 Codex automation TOML；`schedule_ready=true` 需要 contract `Australia/Sydney`、当前 daily 11:30 本地排程匹配，且 live TOML 如显式写 timezone 不得漂移。该证据只解除 automation schedule 外部检查，不放行管理结论。
...
- 本地 Codex automation `kmfa` 已重新与 repo mirror 对齐：tracked `daily_1130_sydney.prompt.md` 和 `codex_app_automation.contract.toml` 反映当前本机 symlink alias cwds、每天 11:30 本地排程与上游 DWS zip 优先级，`check_codex_app_automation.py` 返回 `CODEX_AUTOMATION_READY`。
...
- 定时语义已按当前 active goal 改为本机 Sydney 时间每天 `11:30`；北京时间 `09:30` 仅作为当前 UTC+10 偏移下的业务参照，不作为本机调度时区。
...
- 2026-07-08 S38 历史更新：Codex App automation `kmfa` / `KMFA资金周报自动化` 当时改为每周一、周六 11:00 悉尼本地时间；该时间一度被后续每天 11:30 contract 取代，S101/S105 又短暂恢复 weekly，当前按 active goal 口径再次以 Sydney 本地每天 11:30 为准。
...
- 下一步：若 owner 要继续推进，应先人工补 `owner_corrected_company,owner_corrected_bank` 的 private owner decision manifest，并只做 validation-only 复核；不得把相邻聊天上下文自动解释为字段授权、账本写入或管理结论。
