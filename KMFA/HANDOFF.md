...
- 最近完成的唯一执行单元：**S03 / P3.1 / T-S03-01 本地 release candidate**。`/` 现为 GET/HEAD 直接 `200` 的 canonical App 入口；`/ui`、`/ui/` 为单跳 `308 → /`；assets/canonical/og:url/sitemap 已收敛到根路径；公共浅健康不再泄露 facts。OpenAPI、Swagger、深健康和既有 `/api*` 全部进入 `/ops*`/私有运维面，生产 compose 默认启用 Cloudflare Access JWT 源站验签且缺配置 fail-closed。focused Playwright、旧 App 全流 `11/11`、backend `94/94`、Docker build/container 和依赖审计均 PASS，见 `machine/runs/S03_P31_EDGE_ROUTING.md`。本 phase 未上传、未改生产边缘；fresh production Oracle 仍为 Access `302`，所以只记 local PASS，不冒充 AC production PASS，也未启动 P3.2。
...
- 当前总进度：本地完成 Task `13/56`，已发布 Stage `3/14`；S03 尚未完成或发布。2026-07-23 fresh anonymous production GET/HEAD 对 `/`、`/ui`、`/ui/`、错误 canary、`/api/状态`、`/ops/healthz` 仍全部返回 Cloudflare Access `302`。P3.1 候选已本地证明路由与私有面边界，但匿名完整使用、耐久 DB/object、任意文件安全上传下载、备份恢复、canary/rollback 等总体能力均尚未完成。
- S03 中间 phase 只允许本地 commit，不上传。下一个新 run 只能执行 **S03 / P3.2 / T-S03-02**；P3.3、P3.4 与 whole-S03 review 仍须顺序完成。只有 whole-Stage 复审发现全部解决后，才允许在 host 登录墙仍开启时先建四类路径 Access app/配置 Audience，再一次 S03 GitHub upload 触发 Coolify guarded image，私有 canary 通过后最后把 host 改为 public Bypass 并跑 production Oracle；边缘回滚优先恢复 host 级 Owner Allow，源站 JWT guard 不得关闭。
