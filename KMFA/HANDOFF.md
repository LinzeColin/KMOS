...
- 最近完成的唯一执行单元：**S04 / P4.1 / T-S04-01 匿名身份本地 phase candidate**，见 `machine/runs/S04_P41_ANONYMOUS_IDENTITY.md`。新 workspace ID 使用 128-bit CSPRNG，workspace secret 与一小时 access token 使用 256-bit CSPRNG；新增 ID+secret 的 `/public-api/walking-skeleton/v1/sessions` 交换 API，未知 ID/错误 secret 统一 404 并走 constant-time verifier compare。10,000 workspace 的 ID/secret/token 碰撞均为 `0`，256 并发创建 collision/error `0`，SQLite/WAL/SHM 明文 secret/token 前缀命中 `0`；P4.1+S03 focused `23/23`、backend `127/127` PASS。S03 96-bit legacy workspace ID 继续兼容，schema/恢复资产/卷未迁移、重写或删除。本 phase 未做 P4.2、S04 review、GitHub push 或生产部署。
...
- 当前总进度：本地完成 Task `17/56`；published Stage `4/14`。P4.1 phase review findings `5/5` 已最小修复、open `0`；它只可本地提交，不上传、不部署。完整匿名使用、耐久 DB/object、备份恢复、明确删除和 `G2` 仍未完成。
- 下一个新 run 只能执行 **S04 / P4.2 / T-S04-02**：实现恢复码与 `.kmfa-recovery` 文件、跨设备/重启/错误码/截断/撤销/轮换 E2E。不得顺带进入 P4.3、S04 review 或 GitHub upload；恢复材料不得进入 URL/分析系统，`kmfa-app-state`、既有 verifier 与 v1.5 恢复资产不得删除。
