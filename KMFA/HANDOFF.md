...
- 最近完成的唯一执行单元：**S05 / P5.2 / T-S05-02 对象存储**，见 `machine/runs/S05_P52_OBJECT_STORAGE.md`。仅完成 `R-DATA-002 / AC-DATA-002`：加入默认关闭的私有 S3-compatible adapter、每 artifact version 独占且 `If-None-Match:*` 防覆盖的应用版本 key、SHA-256/size/metadata/lineage 核验、backend-specific 双读、private prefix policy、DB/object 深 inventory 对账与 CI Oracle；默认仍为 `legacy-filesystem`，v1.5 filesystem reader 永久保留，未生产切换、未进入 P5.3。runtime-frozen image `sha256:8b451cbd…35829` 的对象 Oracle `19/19` PASS：4 个多类型/大小夹具、同名 3、重复内容 2，正常与修复后一致率 `100%`，missing/mismatch/orphan `3/3` 全部分类、unexplained `0`，匿名 GET/LIST、越 prefix 读写/列举、App delete 与覆盖均拒绝；双 App、对象容器保卷替换、显式 outage 503、清空浏览器恢复与切回 legacy 后 S3 双读全闭环。完整 backend `172/172`，P5.1 `14/14`、S03/P3.4+S04/P4.2-P4.3、P4.4、P3.2/P3.3 exact-image 回归均 PASS；复审 `8/8` findings resolved、open `0`。本 run 不改生产数据、Cloudflare/Coolify/R2/WAF，不上传 GitHub。
...
- P0.4/S00 的失败边界已局部刷新：生产当前仍以 `kmfa-app-state` 的 SQLite+私有文件 adapter 服务；P5.1 已在本地证明共享 PostgreSQL 结构化状态，P5.2 已在本地证明私有 S3-compatible 对象层、双 App、保卷替换和确定对账，但两者均未生产 cutover，P5.3 状态机与 P5.4 backup/restore 仍未完成，所以有限 RPO/RTO 继续不可证明。不得把 synthetic Oracle、MinIO native versioning 或 named volume replacement 伪报为整套长期持久化/备份恢复 PASS。首次创建 S04 22-char workspace ID 后，快速回滚仍必须保留 S04/P4.1 dual reader、DB/对象配置与卷，先 Flag/mode 回滚再 forward-fix，禁止删卷/对象、撤既有 S3 reader 凭据、改 verifier 或 recovery replay。
- 当前总进度：Task `22/56`；S05 phase `2/4` 本地完成；published Stage=`5/14`。P5.2 findings `8/8` resolved、open `0`，full backend `172/172`、对象 Oracle `19/19`、P5.1 `14/14`、S03/S04 exact-image public/recovery/secret/abuse regressions均 PASS。中间 phase 按合同只做本地 commit，不 push；S05 必须在 P5.1–P5.4 全部完成后另开 whole-stage review，修完 review findings 才整体上传。
- 下一个新 run 只可执行 **S05 / P5.3 / T-S05-03 一致性状态机**。每次 run 仍最多一个 phase；不得顺带 P5.4、不得做 S05 Stage Review、不得切生产 PostgreSQL/S3、不得上传 GitHub，也不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、既有 verifier/session/object 或 v1.5 恢复资产。
