...
- 下一步：若 owner 要继续推进，应先根据 S111 `ocr_fact_candidate_owner_decision_intake_validation_report.csv` 和新版 `ocr_fact_candidate_owner_decision_review_batch.xlsx` 的 OCR 摘录、focus status、line range/focus line/match value 填写 12 条候选的 `owner_authorization_decision,owner_corrected_company,owner_corrected_bank`，然后运行 `install_owner_decision_manifest.py --draft-xlsx-path <reviewed.xlsx>` 或 `--draft-csv-path <reviewed.csv>` validation-only dry-run；不得把相邻聊天上下文自动解释为字段授权、账本写入或管理结论。
