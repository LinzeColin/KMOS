...
- 定时语义已按当前 owner contract 改为本机 Sydney 时间每天 `11:30`；北京时间 `09:30` 仅作为当前 UTC+10 偏移下的业务参照，不作为本机调度时区。
...
- 2026-07-08 S38 历史更新：Codex App automation `kmfa` / `KMFA资金周报自动化` 当时改为每周一、周六 11:00 悉尼本地时间；该时间一度被后续每天 11:30 contract 取代，S101 又短暂恢复 weekly，当前按 goal 口径再次以 Sydney 本地每天 11:30 为准。
