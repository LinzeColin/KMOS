...
- 最近完成的唯一执行单元：**S06 / P6.1 / T-S06-01 上传合同**，见 `machine/runs/S06_P61_UPLOAD_CONTRACT.md`。本地候选在 parent `12d6fa9f…` 上增加默认关闭的 `KMFA_RESUMABLE_UPLOAD_ENABLED`、`kmfa-offset-v1` upload session、4 MiB 固定分片、64 MiB 单文件、服务端 offset、分片/完整 SHA-256、幂等 replay、大小/配额/进度 UI 与显式取消清理；复用 S05 durable intent/object/outbox，不新建数据平面。最终镜像 `sha256:6ddb1800…9180e` 上 7 类文件 attachment-only `7/7`、执行/预览成功 `0`，4 个分片边界恢复 `4/4`，真实容器重启 `1`、socket 半分片断线 `1`、partial accepted `0`，篡改/超限/重复增长 `0`，并发 2 请求收敛 durable chunk `1`；取消与在途分片的竞态由同一跨进程 session lock 收口，取消先完成时残片 `0` 且重复取消幂等。Flag off 后既有 hash 保留、标准 upload `200`、未完成 chunk 保留。浏览器真实 `/upload-sessions`、quota、重启恢复/下载/secret hygiene Oracle PASS；focused `34/34`、full backend `247/247`。本 phase 未做 S06 整体复审、GitHub upload 或生产 rollout，P6.2 malware/quarantine 仍未开始。
...
- 当前总进度：Task `25/56`；S06 phase `1/4`，published Stage `6/14`。P6.1 phase findings `9/9` resolved、open/accepted risk `0/0`；full backend `247/247`，final-image upload 与浏览器 Oracle PASS。生产 P6.1 Flag 仍须保持 `0` 至 S06 whole-stage review；G2 仍需完成 S06/S07，GA 未通过。
- 下一个新 run 只可执行 **S06 / P6.2 / T-S06-02** 隔离与扫描；不得提前进入 P6.3/P6.4、做 S06 whole-stage review、上传本中间 phase 到 GitHub、启用生产 P6.1，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 intent/chunk/object/backup、既有 verifier/session 或 v1.5 恢复资产。P6.1 快速回滚只置 `KMFA_RESUMABLE_UPLOAD_ENABLED=0`，保留标准 8 MiB 上传；若需停全部新上传再独立置 `KMFA_CONSISTENCY_STATE_MODE=paused`。
