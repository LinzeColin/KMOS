...
- 最近完成的唯一执行单元：**S04 / P4.2 / T-S04-02 跨设备恢复本地 phase candidate**，见 `machine/runs/S04_P42_RECOVERY_EXPERIENCE.md`。新增严格四字段、4 KiB 上限的 `.kmfa-recovery` POST 导出/导入、创建前不可邮件/账号/客服找回警告，以及原子 verifier 轮换；旧恢复码、旧文件和旧 ID+secret 交换在轮换后全部 404，既有短时 session 暂保留给 P4.3 收口。真实镜像 Oracle 完成设备 A 导出→数据库重启→隔离设备 B 文件恢复/比对/轮换→6 类错误/截断/撤销输入→隔离设备 C 新码恢复→Flag `1→0→1` 保留→隔离设备 D 新文件恢复：数据一致率 `100%`、无效授权成功 `0`、对象 `1→1→1` 且 SHA-256 不变、state/log/evidence secret 命中 `0`；backend `139/139`、Vite build 与 Docker/E2E PASS。S03 legacy workspace ID、schema、恢复资产和卷未迁移、重写或删除。本 phase 未做 P4.3、S04 review、GitHub push 或生产部署。
...
- 当前总进度：本地完成 Task `18/56`；S04 phase `2/4`；published Stage `4/14`。P4.2 phase review findings `7/7` 已最小修复、open `0`；它只可本地提交，不上传、不部署。完整匿名使用、耐久 DB/object、备份恢复、明确删除和 `G2` 仍未完成。
- 下一个新 run 只能执行 **S04 / P4.3 / T-S04-03**：完成 workspace secret 的 URL/Referer/request log/app log/event/error/cache 全链 canary 扫描，并实现 Secure/HttpOnly/SameSite 或等效保护且可撤销的短时会话。不得顺带进入 P4.4、S04 review 或 GitHub upload；不得把本轮局部 state/log/evidence 扫描冒充 AC-WS-003，`kmfa-app-state`、既有 verifier 与 v1.5 恢复资产不得删除。
