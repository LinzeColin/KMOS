...
- **2026-08-10 Owner 无登录公开面合同**：Owner 明确要求 KMFA 网站公开、无需登录。`coolify-ops` 的 `public-dashboard` 仅匹配 `kmfa.linzezhang.com` 当前驾驶舱使用的 `/api`、`/ops` 与项目成本路径，先为精确 Access Application 写入 `Bypass / Include Everyone`，再将源站 `KMFA_PRIVATE_OPS_REQUIRE_ACCESS` 置 `0` 并触发重部署。代码候选不等同于现网完成；仅当匿名实时请求不再重定向登录且页面不再出现 `Failed to fetch` 时，才可记录生产修复完成。

- **2026-08-10 T10 最新 main 生产复核（当前，优先于本节内所有较早状态）**：当前生产源码与执行时 `origin/main` 均为 `6318b2d01648fde29bdfb23f6342becdeec5bcfd`；GitHub deploy `31324132082` 的完整 E2E、Golden verify 与 Coolify deploy 已成功，deployment-info `31324713238` 将该 source、镜像 `sha256:227b76c085c5b711ceb631281f8259a87d44dff564501411bf3bc5cb0e4bd587` 和 finished deployment（`2026-08-09T16:46:31Z`）闭合为同一证据链。根页面保持匿名 `200`，未携带 Access 身份的 `/ops` 为 `403`。最终镜像的固定 Access history-probe `31324754567` 已真实完成：transport `OK`、`DWS_GROUP_HISTORY_PROBE_COMPLETED`、`GROUP_HISTORY_V2_FIRST_PAGE_TERMINAL`，本次临时 Access policy/token 已精确清理；该探针按设计不下载附件、不写 raw。此前等价 runtime 已观测到连续两条 `*/15` 轮询：`2026-08-09T16:15:03Z` 与 `2026-08-09T16:30:06Z`，间隔 `903` 秒，均终态为 `SOURCE_MATCH_ZERO`；最终提交只变更测试和证据、未修改每日资金 runtime 文件，且 final CI 的独立容器 gate 通过。它证明独立调度与受控来源路径可运行，只表示未形成目标文档事实，不能扩展为全历史为空或任何金额结论。当前每日资金合约 `196 passed, 1 skipped`、后端 API `20 passed`、前端 production build、仓库投影和原任务包 verifier（`24 requirements / 11 tasks / 17 acceptance`）均通过；通用 Verifier 因其自身不接受包内 `PURSUING_GOAL.txt` 文件名而未形成正式导入结论，这只是工具兼容缺口，不能冒充产品或资金 PASS。当前没有真实已验证 parser-open 的同日余额＋流水事实对，故整数分勾稽、D1、R2、OCI、恢复与资金 publication 仍一律为 `NOT_RUN/NOT_MET`，严禁发布资金金额。详见 `machine/runs/daily_funds/T10_FINAL_POST_DEPLOY_RECONCILE_20260810.json`。

- **2026-08-09 T14 精确群历史采集适配（历史候选，已被当前 main 生产回执取代）**：现网 `0c6e65a8…` 已由 deployment-info 回读为实际运行源码；其固定 Access probe 成功证明同一独立 DWS 身份可以用官方 `+chat-messages` 在精确群、固定窗口中得到 `GROUP_HISTORY_V2_FIRST_PAGE_TERMINAL`，临时 Access policy/token 均已精确清理。为处理同一来源的 `search-advanced` 终页缺少显式记录列表，候选运行时保留原 opaque-cursor 路径；仅该特定失败才调用官方精确群 full-ledger reader，并强制 `complete=true`、`hasMore=false`、已知分页、零失败/零截断、精确群和稳定发送人门禁后才交给 raw 流程。它不从投影时间构造 cursor；full-ledger 不完整、群/候选发送人身份不确定或任一页异常时不推进 durable cursor/high-water。当前本地每日资金回归 `189 passed, 1 skipped`、任务包投影/变异门禁和每日资金 API `20 passed`；该候选尚未部署，故真实采集、raw、解析、双事实、整数分勾稽、D1/R2/OCI、恢复和资金 publication 仍均为 `NOT_RUN`，不可写 PASS。详见 `machine/runs/daily_funds/T14_GROUP_HISTORY_COLLECTOR_RECONCILE_20260809.json`。
