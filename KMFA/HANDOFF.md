...
- 最近完成的唯一执行单元：**S04 whole-stage review / correction / guarded upload**，见 `machine/runs/S04_STAGE_REVIEW.md`。整体重放 P4.1–P4.4、`R/AC-WS-001..004`、source→image→evidence、恢复资产、CI 与回滚，发现并修复 `11/11`：P4.1 独立数据库证据被错误聚合、validation `422` 回显恢复 canary、URL path 漏检、invalid/replayed/challenge/concurrency 拒绝未计入 global attempt budget、攻击中正常用户隔离证据不足、status/docs 语义不实、device Cookie 未进 process redaction、pre-P4.1 binary rollback 不兼容 22-char ID、machine navigation 缺 Stage receipt、first GitHub run 暴露的 abuse Oracle 场景共享 10 秒 global upload bucket，以及 corrective full regression 暴露的 SQLite cold-start WAL/schema 初始化竞态；open `0`。corrective runtime-frozen image `sha256:6a37c2cc…763de` 上 backend `157/157`、32 线程 cold-start `10/10`、恢复一致率 `100%`、invalid authorization `0`、secret canary hit `0`、正常误伤 `0/100`、五类攻击绕过 `0`；六路慢上传活跃时独立 root/create=`200/201`，并发 admitted/blocked=`2/4` 且拒绝原因仅 `concurrency`，global sustained `600` 后阻断并在下一窗口恢复。公共壳 `4/4`、三浏览器 a11y/index 与私有 `11/11` 全 PASS；taskpack/governance、依赖、actionlint、Compose、恢复 ZIP/bundle verify 全 PASS。本 run 不进入 S05，不手工修改 Cloudflare/Coolify/WAF/生产数据。
...
- 最近完成发布合同的 Stage 是 **S04 全 Stage Review + guarded upload**；该提交在同 SHA deploy Gate、Coolify manifest 与 post-deploy Oracle 未闭合前只算 GitHub candidate，不能冒充生产 identity。S03 rollout 的根域 Everyone Bypass、`/api*`/`/ops*` owner-only Access、源站 403 fail-closed、Walking Skeleton `1→0→1` 与旧数据恢复/hash 继续是前置条件。S04 publication 闭合后 published Stage=`5/14`；完整匿名旅程、长期持久化、明确删除、G2 与 GA 仍未标绿。
...
- S04 review 开始时的生产身份由 read-only query run `29996883514` 回显：deployment source `ce9c27da2c6518fec2c740897c788bd63069678f`、image `sha256:bafa6a96547ed9bed3f62e0ea956a27963be5078b91bf68d24f77c857a44e60c`、Coolify deployment `amffqczj7p2o22yz3en1or13`、completed `2026-07-23T09:42:56.000000Z`、status `finished`。它只是 S04 上传前快照；发布后必须从实际 manifest 重新读取 source/image/deployment/completed_at，corrective review-time image `sha256:6a37c2cc…763de` 也不得冒充生产 digest。
...
- P0.4/S00 的失败边界已局部刷新：生产现已挂载 `kmfa-app-state`，并以合成项目/文件证明单节点跨部署重启和 Flag 回滚保留；但生产关系数据库、S3-compatible object adapter 与 backup/restore 均未实现，所以有限 RPO/RTO 仍不可证明，继续按 `unbounded/not recoverable` 处理。Named volume 不得被伪报为 S05 durable PASS。首次创建 S04 22-char workspace ID 后，禁止在 Walking Skeleton 开启状态回滚到 pre-P4.1/S03 reader；快速安全回滚必须保留 S04/P4.1 dual reader 与 named volume，先 Flag off/紧急策略，再 roll forward，禁止删卷、改 verifier 或 recovery replay。
- 当前总进度：Task `20/56`；S04 phase `4/4`；S04 Stage Review findings `11/11` resolved、open `0`。初始 main `e1b983e8...` 的 deploy run `30009779537` 在 Linux abuse E2E 正确暴露：前序 upload flood 与随后 concurrency flood 落入同一 10 秒 global upload bucket，恢复请求合法命中 global cap；`golden-path` skipped、Coolify 未触发，禁止 rerun 红 SHA。corrective Oracle 先进入全新策略窗口、锁定四个拒绝均为 `concurrency`，并以持久层 lease=`0` 验证恢复前置条件；随后本地 full regression 又发现并修复 cold-start WAL/schema 竞态，32 线程 barrier `10/10`、full backend `157/157`、corrective exact-image 全流均 PASS。GitHub corrective fast-forward、同 SHA CI/deployment identity/post-deploy Oracle 全闭合后 published Stage=`5/14`；闭合前仍按 `4/14 + S04 candidate`。完整匿名使用、耐久 DB/object、备份恢复、明确删除和 `G2` 仍未完成。
- 下一个新 run 只有在 S04 GitHub upload、同 SHA Gate、deployment tuple 与 post-deploy Oracle 全部闭合后，才可执行 **S05 / P5.1 / T-S05-01 结构化数据库**。每次 run 仍最多一个 phase；不得顺带 P5.2，不得把 named volume/SQLite 冒充 durable database/object/backup PASS，`kmfa-app-state`、既有 verifier/session/object 与 v1.5 恢复资产不得删除。
