...
- runner 现在生成 `automation_readiness.csv`，只读核对 tracked contract 与本机 Codex automation TOML；`schedule_ready=true` 需要 contract `Australia/Sydney`、当前 Monday/Saturday 11:00 本地排程匹配，且 live TOML 如显式写 timezone 不得漂移。该证据只解除 automation schedule 外部检查，不放行管理结论。
...
- 定时语义已按当前 owner contract 改为本机 Sydney 时间每周一、周六 `11:00`；北京时间 `09:00` 仅作为当前 UTC+10 偏移下的业务参照，不作为本机调度时区。
...
- 2026-07-08 S38 历史更新：Codex App automation `kmfa` / `KMFA资金周报自动化` 当时改为每周一、周六 11:00 悉尼本地时间；该时间一度被后续每天 11:30 contract 取代，S101 又短暂恢复 weekly，当前按最新 owner 指令再次以 Sydney 本地每周一/周六 11:00 为准。
