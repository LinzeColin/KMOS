...
v1.2 FULL_HTML_NO_OMISSION 完整任务包已成为 KMFA 后续开发基线，并已按 v1.2 重新走完 Stage 1。Stage 2、Stage 3 和 Stage 4 均已完成本地实现、验证、整体复审和 GitHub main 上传。当前 Stage 5 已完成 `S05-P1｜A0 文件登记`、`S05-P2｜字段级黄金基准` 和 `S05-P3｜权威基准锁定` 的本地实现与验证。S05-P3 基于 S05-P2 active owner/授权降级决策，锁定 40 条 PDF 字段 hash/source-anchor 记录为 public-safe Q5 calculation baseline，并将 5 条 Excel 字段排除为 `cross_source_support_only`。Stage 5 尚未整体复审，也未上传 GitHub；下一轮只能做 Stage 5 整体复审，不能做 UI、报告、事实层、zero-delta、外部接口或 GitHub 上传。
...
- S05-P3 不提交 raw business data、zip、Excel、PDF、私有 CSV 或字段明文；`formal_report_allowed=false`、`stage5_review_completed=false`、`github_upload_allowed=false`。
...
- S05-P3 lock 是 Stage 5 phase-level local completion，不代表 Stage 5 review、zero-delta、lineage、事实层或报告发布完成。
...
- Stage 5 已完成 S05-P1、S05-P2 和 S05-P3 本地验证；S05-P3 已锁定 40 条 public-safe hash/source-anchor 字段并排除 5 条 Excel 字段；Stage 5 尚未整体复审，本轮不得上传 GitHub。
...
- S05-P3 只完成 A0 authority baseline lock；不能把它扩展解释为正式项目成本事实层、zero-delta 或报告发布。
...
下一步若继续开发，只执行 Stage 5 整体复审；不扩大到 UI、报告、事实层、zero-delta、自动接口或 GitHub 上传，且不得提交真实原始业务文件。
