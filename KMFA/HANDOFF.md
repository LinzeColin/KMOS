...
- runner 现在生成 `automation_readiness.csv`，只读核对 tracked contract 与本机 Codex automation TOML；`schedule_ready=true` 需要 contract `Australia/Sydney`、`FREQ=DAILY;BYHOUR=11;BYMINUTE=30` rrule 匹配，且 live TOML 如显式写 timezone 不得漂移。该证据只解除 automation schedule 外部检查，不放行管理结论。
...
- 2026-07-08 S38 历史更新：Codex App automation `kmfa` / `KMFA资金周报自动化` 当时改为每周一、周六 11:00 悉尼本地时间；该时间已被后续每天 11:30 contract 取代。
