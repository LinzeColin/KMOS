...
- 最近完成的唯一执行单元：**S05 / P5.1 / T-S05-01 结构化数据库**，见 `machine/runs/S05_P51_STRUCTURED_DATABASE.md`。仅完成 `R-DATA-001 / AC-DATA-001`：加入 SQLite/PostgreSQL 双 backend、checksum-locked expand-only schema v2、projects/progress/score/finance/artifact version/tasks/audit 的 repository/service transaction 边界、v1/v2 SQLite 只读幂等 import、opt-in PostgreSQL 17.10 Compose profile 与数据库感知 healthcheck；默认仍为 `legacy-sqlite`，未生产切换、未进入 P5.2。runtime-frozen image `sha256:4d9f40ac…d74be` 的 PostgreSQL Oracle `14/14` PASS：两个 App 节点、20 个并发写入、App 滚动替换、PG 容器保卷替换、清空客户端 Cookie 后恢复、文件 hash、append-only audit 与 v1/v2 import 全闭环；业务状态 hash 前后一致，丢失/重复/部分提交/secret-DSN 命中均 `0`。完整 backend `165/165`，S03/P3.4+S04/P4.2-P4.3 恢复 Oracle 与 P4.4 正常 `0/100` 误伤/攻击绕过 `0` 均 PASS。全回归暴露并修复跨 backend 丢失 `rowid` 最旧 session 顺序、import order collision、v2 source 漏导、audit append 误判与 shallow health 不验库，共 `6/6` resolved、open `0`。本 run 不改生产数据、Cloudflare/Coolify/WAF，不上传 GitHub。
...
- 最近完成发布合同的 Stage 是 **S04 全 Stage Review + guarded upload**，publication 已闭合，published Stage=`5/14`。当前 S05/P5.1 只是 detached worktree 的本地 phase candidate，不是 GitHub/生产 identity。S03/S04 rollout 的根域 Everyone Bypass、`/api*`/`/ops*` owner-only Access、源站 403 fail-closed、Walking Skeleton 与 abuse Gate 继续是前置条件；完整匿名旅程、对象存储、备份恢复、明确删除、G2 与 GA 仍未标绿。
...
- P0.4/S00 的失败边界已局部刷新：生产当前仍以 `kmfa-app-state` 的 SQLite+私有文件 adapter 服务；P5.1 已在本地证明共享 PostgreSQL 结构化状态、双 App 节点与数据库保卷替换，但尚未生产 cutover，P5.2 object、P5.4 backup/restore 均未完成，所以有限 RPO/RTO 继续不可证明。不得把 P5.1 synthetic Oracle 或 named volume 伪报为整套长期持久化 PASS。首次创建 S04 22-char workspace ID 后，快速回滚仍必须保留 S04/P4.1 dual reader 与卷，先 Flag/mode 回滚再 forward-fix，禁止删卷、改 verifier 或 recovery replay。
- 当前总进度：Task `21/56`；S05 phase `1/4` 本地完成；published Stage=`5/14`。P5.1 findings `6/6` resolved、open `0`，full backend `165/165`、PostgreSQL Oracle `14/14`、S04 exact-image recovery/secret/abuse regressions均 PASS。中间 phase 按合同只做本地 commit，不 push；S05 必须在 P5.1–P5.4 全部完成后另开 whole-stage review，修完 review findings 才整体上传。
- 下一个新 run 只可执行 **S05 / P5.2 / T-S05-02 对象存储**。每次 run 仍最多一个 phase；不得顺带 P5.3/P5.4、不得做 S05 Stage Review、不得切生产 PostgreSQL、不得上传 GitHub，也不得删除 `kmfa-app-state`、`kmfa-postgres-data`、既有 verifier/session/object 或 v1.5 恢复资产。
