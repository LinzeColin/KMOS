...
- 下一步：若 owner 要继续推进，应直接打开 S111 `ocr_fact_candidate_owner_decision_review_batch.xlsx`，先看 `Review Summary` 确认 12 条候选的 metric/focus status 分布，再在 `Owner Review` sheet 逐行填写 `owner_authorization_decision,owner_corrected_company,owner_corrected_bank`，然后运行 `install_owner_decision_manifest.py --draft-xlsx-path <reviewed.xlsx>` validation-only dry-run；不得把相邻聊天上下文自动解释为字段授权、账本写入或管理结论。
