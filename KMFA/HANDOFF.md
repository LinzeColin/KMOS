...
- 最近完成的唯一执行单元：**S03 / P3.4 / T-S03-04 本地 + final-image release candidate**。`KMFA_WALKING_SKELETON_ENABLED` 默认 `0`，显式启用后根页可创建一个无账号服务器工作区、保存项目名与 0–100 进度、上传一个任意类型且不超过 8 MiB 的文件、用 256-bit 恢复码换取一小时短时会话，并 attachment-only 下载；恢复/访问 capability 服务端只存 SHA-256，浏览器不用 cookie/localStorage/sessionStorage。SQLite 结构状态与私有随机对象文件挂 `kmfa-app-state` volume；application image `sha256:b643fda0…3810` 已真跑 create→save→upload→restart→fresh-context recover→browser/server SHA equal→Flag `1→0→1`，两次下载均为 `501b484c…af011`，关闭时 root `200`、恢复 `404`、DB rows/object hash 不变，secret 在 state/log/URL 命中 `0`。focused `16/16`、backend `117/117`、公共壳 `4/4`、三浏览器 axe critical/serious `0`、未发布 index hit `0`、旧经营全流 `11/11` 均 PASS，见 `machine/runs/S03_P34_WALKING_SKELETON.md`。这是单节点早期 adapter，不是 S04-S07、备份/RPO/RTO、长期持久或 GA PASS；本 phase 未上传、未部署。
...
- 最近已发布 Stage 是 **S02 全 Stage Review**：远端 `main=a991e1b8eade5e852cc750095ffe466bce1d1bb9`，Dual-Plane run `29939643462` PASS，该 SHA 精确 deploy run 为 `0`；P2.1–P2.4、S02 Stage Gate 与 `G1 Product Contract` 整体 PASS，见 `machine/runs/S02_STAGE_REVIEW.md`。此前 S01 的四个 sealed Task Test 与八项 Stage Gate 也保持 PASS；G1 只授权 Walking Skeleton 实现，不是 runtime/GA PASS。
...
- 当前总进度：本地完成 Task `16/56`，已发布 Stage `3/14`；S03 四个 Phase 均有本地 commit 候选，但 whole-Stage review 尚未执行，所以 S03 仍未完成或发布。2026-07-23 本 run 自行只读刷新确认 `origin/main=a991e1b8…`、最新 deploy run 仍为 `29916233128`，生产唯一链仍是 `68306e85… / sha256:adfc849b… / boh5fsnxe82umwcpqzooam1p / 2026-07-22T11:39:29Z`；最近 fresh anonymous production Oracle 仍显示 host Access `302`。P3.1-P3.4 均未上传或部署，故没有把本地根页、骨架、a11y、索引或回滚证据冒充线上状态。完整匿名使用、耐久 DB/object、备份恢复和 production canary/rollback 等总体能力仍未完成。
- 下一个新 run 只能执行 **whole-S03 review**：把 P3.1-P3.4、S03 objective/deliverable/gate、相关 AC 与 release/rollback 作为整体复审，修清全部 findings 后才允许一次性上传 S03；不得先进入 P4.1。上传后仍须在 host 登录墙保持开启时先建路径 Access app/配置 Audience、部署 guarded image 与状态卷，私有/恢复/未发布 canary 全绿后才把 host 改为 public Bypass；索引开关继续保持 `0`，production Oracle 全绿后再独立灰度为 `1`。边缘回滚优先恢复 host Owner Allow，骨架与索引 Flag 均置 `0`，不得删 `kmfa-app-state`，源站 JWT guard 不得关闭。
