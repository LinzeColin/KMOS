...
- 最近完成的唯一执行单元：**S04 / P4.3 / T-S04-03 secret hygiene 本地 phase candidate**，见 `machine/runs/S04_P43_SECRET_HYGIENE.md`。浏览器 API 不再返回 access token 明文，改用 host-only `__Secure-kmfa_session`（Secure/HttpOnly/SameSite=Strict/API Path/1h），携 Cookie 写操作强制 scheme/host 同源；新增服务端显式撤销，恢复 secret 轮换会原子撤销全部旧 session 并签发替代 session，既有 bearer 只读兼容。全局边界拒绝 raw/percent-encoded capability URL/Referer，服务端与浏览器 console 均脱敏、raw access log 关闭、app 错误只发静态码，CSP `connect-src 'self'` 且无第三方分析/错误 SDK。最终镜像 `sha256:d555a0ee…d07f` Oracle 扫描 request URL `43`、performance URL `38`、browser raw-Error canary `1`、error sample `7`、walking no-store response `18`、日志/状态/事件/cache/screenshot，全部 canary 命中 `0`、foreign request `0`；轮换旧 session 与显式撤销旧 session 重放均 `404`，P4.2 重启/恢复/hash/Flag 旅程、公共壳 4 模式和三浏览器 a11y/index 均 PASS。backend `146/146`、Ruff、Vite、Docker/E2E PASS。S03 legacy ID、schema v1、verifier、卷、对象和 v1.5 恢复资产未迁移、重写或删除。本 phase 未做 P4.4、S04 review、GitHub push 或生产部署。
...
- 当前总进度：本地完成 Task `19/56`；S04 phase `3/4`；published Stage `4/14`。P4.3 phase review findings `9/9` 已最小修复、open `0`；它只可本地提交，不上传、不部署。完整匿名使用、耐久 DB/object、备份恢复、明确删除和 `G2` 仍未完成。
- 下一个新 run 只能执行 **S04 / P4.4 / T-S04-04**：实现分层配额、单位时间速率限制、异常/重复流量挑战或阻断、无账号公平性与拒绝事件可审计，并以正常/攻击流量发生器满足 `AC-WS-004`。不得顺带进入 S04 overall review、S05 或 GitHub upload；不得把 P4.3 的 secret Gate 冒充反滥用 Gate，`kmfa-app-state`、既有 verifier、session 与 v1.5 恢复资产不得删除。
