...
- 最近完成的唯一执行单元：**S06 / P6.3 / T-S06-03 版本、血缘与预览**，见 `machine/runs/S06_P63_VERSION_LINEAGE_PREVIEW.md`。在 P6.2 parent `e30963ce…` 上增加 schema `6` immutable version/parent/source-operation lineage、固定 processor registry、durable processing runs/generations、derivative object metadata，以及默认关闭的 `KMFA_ARTIFACT_DERIVATION_ENABLED`。同一 logical artifact 的同名/同内容/修改内容每次形成独立版本；clean text/JSON 才由 worker 生成最多 `64 KiB` 的 strict UTF-8 `text/plain`，Web 不解析原件、用户代码/宏不执行，高风险继续 attachment-only。最终同镜像 `sha256:98c08e58…c9bea` 上版本 `3/3`、原件覆盖 `0`、parent/lineage gap `0`、reprocess generation `2` 且 replay audit side effect `1`，Chromium SHA 校验/React text render、重启和 Flag rollback 均 PASS；PostgreSQL `14/14`、私有 S3 `19/19`（一致率 `100%`、未解释异常 `0`），full backend `274/274`。备份恢复、对象对账、生命周期删除和容量预算均纳入 derivative。本 phase 只交付安全 text-preview derivative，不冒充通用 thumbnail/parser，也未做 P6.4、S06 整体复审、GitHub upload、部署或生产 rollout。
...
- 2026-07-26 P6.3 收口前已自行 fetch：`origin/main=c00a90f5…` 相对共同基线 `12d6fa9f…` 新增 `43` 个 KMIDS commits；`KMFA/` 与 `.github/workflows/` scoped diff 为 `0`。本地 P6.1/P6.2/P6.3 chain 未被远端删除；S06 whole-stage review/push 前须整合当时最新远端并重跑完整门禁，当前 phase 不做 merge/rebase/reset。
...
- 当前总进度：Task `27/56`；S06 phase `3/4`，published Stage `6/14`。P6.1/P6.2/P6.3 phase findings `9/9 + 7/7 + 7/7` resolved、open/accepted risk `0/0`；full backend `274/274`，final-image version/lineage/preview、Chromium、PostgreSQL 与私有 S3 Oracles PASS。生产 P6.1/P6.2/P6.3 Flags 仍须保持 `0` 至 S06 whole-stage review；G2 仍需完成 S06/S07，GA 未通过。
- 下一个新 run 只可执行 **S06 / P6.4 / T-S06-04** 上传质量门；不得提前做 S06 whole-stage review、上传本中间 phase 到 GitHub、进入 S07、启用生产 P6.1/P6.2/P6.3，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 assessment/event/intent/chunk/original/derivative/backup、既有 verifier/session 或 v1.5 恢复资产。P6.3 快速回滚使用同一 schema `6` binary，先置 `KMFA_ARTIFACT_DERIVATION_ENABLED=0` 再停止 derivation worker；原件、versions、lineage、processing runs、derivatives、所有对象与备份保持不变，禁止 binary/schema downgrade。若需回退 scanner，再独立置 `KMFA_FILE_SECURITY_ENABLED=0`；若需停全部新上传，再独立置 `KMFA_CONSISTENCY_STATE_MODE=paused`。
