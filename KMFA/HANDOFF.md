...
- 下一步：若 owner 要继续推进，应先人工填写 S111 `ocr_fact_candidate_owner_decision_review_batch.csv` 中 12 条候选的 `owner_authorization_decision,owner_corrected_company,owner_corrected_bank`，然后只跑 validation-only dry-run；不得把相邻聊天上下文自动解释为字段授权、账本写入或管理结论。
