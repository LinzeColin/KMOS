...
- 最近完成的唯一执行单元：**S05 / P5.4 / T-S05-04 保留、备份与恢复**，见 `machine/runs/S05_P54_RETENTION_BACKUP_RESTORE.md`。schema `4` 默认无自动到期，明确删除须 session+recovery secret+确认文本+idempotency key+当前 restore proof 且无 legal hold/partial operation；相同删除请求可在 access 已撤销/完成后按 hash-only verifier 精确回放，10 分钟 worker lease 防重入并允许 stale recovery，public purge 以实际完成时间计 SLA、超时在对象删除前 fail closed 并退出自动 due 队列。生产 active lifecycle 只支持凭据隔离的 S3-compatible backend；legacy filesystem 破坏性 adapter 默认拒绝，测试 override 不进入 compose。backup 机械拒绝 pending consistency、symlink 与无效 incident，portable full+logical incremental 同时封存 DB 行/对象字节/tombstone，恢复只接受空 DB+空对象目标并使 imported proof 失效。最终 runtime-frozen image `sha256:f85a90f…74fb13` 的 PostgreSQL+versioned private MinIO Oracle PASS：标准 fixture `1/1`、invariant failure `0`、synthetic RPO/RTO `4000ms/246ms`，progress `73`、score `91`、financial/task/object `1/1/1`；provider versions `4→0`、误删 `0`、legal hold 无数据损失、失败重试保留业务+对象、删除 tombstone 恢复后 business/object/old recovery/active proof 均不复活。完整 backend `220/220`，P5.1 `14/14`、P5.2 `19/19`、P5.3 `28 crash + 2 timeout` 及 S03/S04 exact-image 回归全 PASS；复审 `14/14` findings resolved、open `0`。这是本地 phase candidate，不是生产备份/RPO/RTO 或删除启用证明；本 run 不改生产数据、Cloudflare/Coolify/R2/WAF，不上传 GitHub。
...
- 最近完成发布合同的 Stage 是 **S04 全 Stage Review + guarded upload**，publication 已闭合，published Stage=`5/14`。当前 S05/P5.1-P5.4 只是 detached worktree 的本地 phase candidate，不是 GitHub/生产 identity；必须另开 whole-S05 review，修完 findings 后才可一次 guarded upload。S03/S04 rollout 的根域 Everyone Bypass、`/api*`/`/ops*` owner-only Access、源站 403 fail-closed、Walking Skeleton 与 abuse Gate 继续是前置条件；完整匿名旅程、生产 DB/object/backup 切换、明确删除启用、G2 与 GA 仍未标绿。
...
- P0.4/S00 的失败边界已局部刷新：生产当前仍以 `kmfa-app-state` 的 SQLite+私有文件 adapter 服务；P5.1-P5.3 已在本地证明共享 PostgreSQL、私有 S3-compatible 对象层及跨 DB/object/outbox 的可恢复一致性，P5.4 又在合成隔离环境证明 DB+对象 full/incremental/tombstone、空环境恢复和明确删除生命周期，但四者均未生产 cutover。不得把 synthetic RPO/RTO、MinIO native versioning、same-host backup 或 named volume replacement 伪报为生产长期持久化/DR PASS。快速回滚使用 schema `4` 当前 binary，同时置 `KMFA_LIFECYCLE_MODE=paused` 与 `KMFA_CONSISTENCY_STATE_MODE=paused`，保留 S04/P4.1 dual reader、DB/对象/备份配置和所有卷并 forward-fix；禁止旧 binary/schema downgrade、删卷/对象/备份、撤既有 reader 凭据、改 verifier 或 recovery replay。
- 当前总进度：Task `24/56`；S05 phase `4/4` 本地完成；published Stage=`5/14`。P5.4 findings `14/14` resolved、open `0`，full backend `220/220`；final image 上 P5.1 `14/14`、P5.2 `19/19`、P5.3 partial/unexplained/duplicate/raw-delete/staged-residue 全 `0`，P5.4 fixture/invariant/accidental-delete `1/1 / 0 / 0`，S03/S04 recovery/secret/abuse 回归全 PASS。中间 phase 按合同只做本地 commit，不 push；S05 必须另开 whole-stage review，修完 review findings 才整体上传。
- 下一个新 run 只可执行 **S05 whole-stage review（P5.1-P5.4）**。不得进入 S06、不得在 review 前上传 GitHub、不得切生产 PostgreSQL/S3/backup/lifecycle、不得启用生产删除，也不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 backup、既有 verifier/session/object 或 v1.5 恢复资产。
