...
- 最近完成的唯一执行单元：**S07 / P7.1 / T-S07-01 单文件下载**，见 `machine/runs/S07_P71_SINGLE_FILE_DOWNLOAD.md`。默认关闭的 `KMFA_SINGLE_FILE_DOWNLOAD_ENABLED` 为已授权 workspace 提供 JSON body opaque selector，可精确下载历史原件版本、registry-validated 派生物及已进入 workspace artifact registry 的报告文件；asset ID 不进入 URL，响应强制 attachment + nosniff，并返回/由浏览器复核类型、大小、checksum、源版本与上传 operation/processor。最终本地 image `sha256:7a6073f9…ca20` 上 originals/derivative/stored-report=`2/1/1`，byte/metadata mismatch、unauthorized、high-risk inline、secret hit=`0/0/0/0`，即时对象篡改 `1/1` 被 `503` 阻断，重启、Flag rollback 与 re-enable 均保留资产。旧全局报告中心未接入；Range/ZIP、export job、published snapshot 分别留给 P7.2/P7.3/S08。没有真实时间 soak、观察期、等待窗口、全量测试前置、上传、部署或灰度。
...
- 当前总进度：Task `29/56`；S07 phase `1/4`，S07 整体复审未开始；P7.1 focused regression `48 passed`，final-image API + Chromium Oracle PASS，byte/metadata mismatch、unauthorized、high-risk inline、secret hit=`0/0/0/0`。此前 full-backend 结果仅作历史诊断，不是当前或后续前置。published Stage 仍为 `6/14`，因为中间 phase/Stage 均不上传，须待全部 `56/56` Task 与最终整包复审完成后一次性上传。生产 P6.1/P6.2/P6.3/P7.1 Flags 保持 `0`，G2 与 GA 均未晋级。
- 下一个新 run 只可执行 **S07 / P7.2 / T-S07-02**，且最多一个 phase；不得提前进入 P7.3/P7.4、S07 整体复审、上传、部署、灰度、启用生产 Flags，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 assessment/event/intent/chunk/original/derivative/backup、既有 verifier/session 或 v1.5 恢复资产。时间逻辑只能使用 Fake Clock、历史回放、Fixture、模拟消息和即时故障注入；禁止真实时间 soak、等待窗口和把全量测试作为前置。P7.1 快速回滚置 `KMFA_SINGLE_FILE_DOWNLOAD_ENABLED=0`，继续保留旧最新原件 attachment 下载及 schema `6` 全部状态；禁止 binary/schema downgrade。仅在整个 v1.5.2 Taskpack 完成并通过最终整包复审后一次性上传 GitHub。
