...
- 2026-07-26 Owner 要求立即冻结现有开发并交给新的开发 agent 接手，因此当前完整本地链已非强制上传到远端分支 `codex/kmfa-v1.5.2-s07-p71`，Draft PR `#194`（`https://github.com/LinzeColin/KMOS/pull/194`）以 `main` 为 base；代码冻结点为 `b4e45b76b18de3d0e07681ac4da0bdb1414b0d9e`。这是对“中间 phase 不上传”的一次明确交接例外，只提供远端源码真源，不代表 S07 Stage 完成、GA、生产部署或 Flag 晋级。任何 agent（包括本线程恢复后）必须先 `fetch` 并读取远端 PR、最新 `HANDOFF.md` 和提交历史；若新 agent 已推进或合并，不得以本地冻结点覆盖、重复执行或回退其成果。
- 最近完成的唯一执行单元：**S07 / P7.3 / T-S07-03 显式导出 Job**，见 `machine/runs/S07_P73_EXPLICIT_EXPORT_JOBS.md`。旧 `GET|HEAD /api/报告中心/导出` 永久改为无副作用 `405` 弃用响应；canonical `POST /api/exports/jobs` 要求 `Idempotency-Key`，由 SQLite durable queue、append-only events 和独立 worker 提供状态/artifact/cancel/metrics。固定边界为 active/running/attempts=`64/2/3`、active/per-job cost=`256/64`、source/artifact=`2/16 MiB`，worker 原子提交 succeeded + export hash + audit；取消/lease recovery 清理未登记制品，source snapshot hash 关闭 TOCTOU。最终本地 image `sha256:502371f…42c05` 上 24 个并发请求只产生 1 个业务结果，GET/HEAD 24 probes business-state delta=`0`，cancel/retry/timeout/source-change/expiry/restart/rollback/re-enable 与 Chromium 创建→刷新→下载全 PASS，console/privacy hit=`0/0`。Flag 默认 `0`，没有上传、部署、灰度、真实时间 soak、观察期、业务等待窗口、后台空转式验收或全量测试前置。
- P7.2 Range 与批量 ZIP 继续有效，见 `machine/runs/S07_P72_RANGE_BATCH_DOWNLOAD.md`。默认关闭的 `KMFA_RANGE_BATCH_DOWNLOAD_ENABLED` 在 P7.1 精确 selector 上增加单 Range/稳定 SHA-256 ETag/并行续传，以及最多 `500` 项、`512 MiB` 原始字节预算的 deterministic ZIP32 stream。归档只保留有界中央目录元数据、一次物化并复核一个对象、每 chunk 不超过 `64 KiB`；ordinal 独立目录阻止重名覆盖和 zip-slip，canonical `manifest.json` 提供逐项来源/size/hash。最终本地 image `sha256:3dfe4a4d…a63ab5` 上并行 Range=`2`、重组 hash match=`true`，ZIP missing/overwrite/traversal=`0/0/0`，server whole-archive buffered=`false`、archive temp=`0`；Chromium 即时注入一次 connection reset 后只重试当前 chunk 一次并完成，批量 ZIP 已实际打开复核。重启、P7.2 rollback 与 re-enable 均保留资产，P7.1 单文件继续可用。
- P7.1 单文件下载继续有效，见 `machine/runs/S07_P71_SINGLE_FILE_DOWNLOAD.md`：默认关闭的 `KMFA_SINGLE_FILE_DOWNLOAD_ENABLED` 为已授权 workspace 提供 JSON body opaque selector，可精确下载历史原件版本、registry-validated 派生物及已进入 workspace artifact registry 的报告文件；asset ID 不进入 URL，响应强制 attachment + nosniff，并返回/由浏览器复核类型、大小、checksum、源版本与上传 operation/processor。P7.3 继续只服务既有 Access 私有报告中心，未把旧全局报告旁路接入匿名 workspace；published snapshot 仍留给 S08。
- 最近完成的完整 Stage 仍为 **S06 whole-stage review + exposed finding repair**，见 `machine/runs/S06_STAGE_REVIEW.md`。P6.1-P6.4 的 `AC-UP-001..004` 全部本地 PASS，10 个跨 phase finding 全部解决，open/waived risk `0/0`。P6.4 使用 12 个固定 Fixture 即时回放、scanner synchronous recovery、object unavailable `503 → replay 200` 与 Fake Clock abuse focused tests，`10/10` negative matrix、data invariant/isolation/unexplained failure `0/0/0`，`real_time_soak_used=false`。reviewed runtime evidence image 为 `sha256:ed2175dc…d3812`；它不是发布候选。
- P6.3 版本、血缘与预览继续有效，见 `machine/runs/S06_P63_VERSION_LINEAGE_PREVIEW.md`：schema `6` immutable version/parent/source-operation lineage、固定 processor registry、durable processing runs/generations、derivative object metadata，以及默认关闭的 `KMFA_ARTIFACT_DERIVATION_ENABLED`。同一 logical artifact 的同名/同内容/修改内容每次形成独立版本；clean text/JSON 才由 worker 生成最多 `64 KiB` 的 strict UTF-8 `text/plain`，Web 不解析原件、用户代码/宏不执行，高风险继续 attachment-only。P6.4 最终镜像复跑同名/同内容/修改内容、Chromium SHA 校验/React text render、重启和 Flag rollback 均 PASS，原件覆盖、parent/lineage gap 均 `0`。
- P6.2 隔离与扫描继续有效，见 `machine/runs/S06_P62_QUARANTINE_SCAN.md`：默认关闭的 `KMFA_FILE_SECURITY_ENABLED`、schema `5` durable assessment + append-only events、quarantine-first projection、HMAC/nonce/stream checksum 私网 scanner protocol、credential-minimal scanner container 与 bounded retry worker。最终镜像 `sha256:33f5325d…c333f` 上恶意/畸形 clean-or-preview escape `0`、合法误拒 policy `0/100` 和 final-image `0/12`；timeout/unavailable 均非 clean 且 retry 收敛，persisted rejected 在 Flag rollback 后继续 `409`。这是有界 content firewall，不冒充 signature-complete 商业 AV。
- P6.1 上传合同继续有效，见 `machine/runs/S06_P61_UPLOAD_CONTRACT.md`：`KMFA_RESUMABLE_UPLOAD_ENABLED` 默认关闭，固定 4 MiB 分片/64 MiB 文件、服务端 offset、分片/完整 SHA-256、幂等 replay、大小/配额/进度 UI 与显式取消清理复用 S05 durable intent/object/outbox；最终镜像 `sha256:6ddb1800…9180e` 的 7 类文件、4 个分片边界、真实重启/半分片断线、篡改/超限/并发/取消竞态与 Flag rollback 均 PASS。未进入生产 rollout。
- 2026-07-26 S06 整体复审已自行 fetch 并把 `origin/main=c00a90f5…` 作为第二父提交无冲突纳入本地 merge `4454aa6b…`；该远端批次为 KMIDS 范围，合并前 `KMFA/` 与 `.github/workflows/` scoped diff 为 `0`。没有 force-push；S06/S07 当前链仅因上述 Owner 接管指令进入 Draft PR `#194`，不计作 Stage publication。除这次交接例外外，中间 phase/Stage 仍不单独 push；正式发布仍须全部 `56/56` Task 与最终整包复审完成。
- 上一完整 Stage：**S05 whole-stage review + publication repair（P5.1-P5.4）**，见 `machine/runs/S05_STAGE_REVIEW.md`。本地 phase chain `c2c0b889 → 4c3d5ae0 → 34eac93f → e2409eec` 已与 reviewed `origin/main=3f7cfd61…` 整合；10 个跨 phase/origin/runner/edge findings 全部修复，open/waived risk 均为 `0`。首个 publication SHA `56ea0935…` 在 Ubuntu P5.4 暴露 `0700/root` bind-state 权限差异且未部署；容器 helper 修复后的 `afbd6cb6…` 全门禁和部署成功，但生产只读浏览器 Oracle 又发现 Cloudflare 自动注入 Insights beacon，CSP 正确阻断却产生 console error。第二修复保留严格 CSP，并只给 HTML 增加 `no-transform`，hold/private 仍为 `private, no-store`，JSON cache 语义不变，Browser Oracle 同时拒绝 beacon markup。second corrective local image `sha256:2a071ceb…0ec4cc9d` 上 focused public-entry `28/28`、full backend `225/225`、公共 Shell 与 Chromium/Firefox/WebKit a11y/index 全 PASS；前一 corrective image 上 P5.1 `14`、P5.2 `19`、P5.3 `28 crash + 2 timeout`、P5.4、恢复/secret/abuse 全 PASS。该结果是 second corrective guarded candidate，不是生产 PostgreSQL/S3/backup/lifecycle cutover 或生产 RPO/RTO 证明。
...
- S05 whole-stage publication 已闭合：source `12d6fa9f46786387ee21d9bd3c682175464f3554`、image `sha256:d399bad05f45b90f6393bd35988482a993df097924901e2b2b9f1fbb32ff0ce1`、Coolify deployment `l47ql1303jbnby0tb1i7njf2`、completed `2026-07-26T02:39:59Z`。published Stage 为 `6/14`。S03/S04 rollout 的根域 Everyone Bypass、`/api*`/`/ops*` owner-only Access、源站 403 fail-closed、Walking Skeleton 与 abuse Gate 继续是前置条件；生产仍为 SQLite/filesystem，未切 PostgreSQL/S3/backup/lifecycle，明确删除未启用，G2 与 GA 仍未标绿。
...
- v1.5 恢复 bundle `1ee7fb111` 及其 SHA-256 `2d0b516f...` 已原样保留，但当前仓库缺少其前置提交 `97edb1b8750d49409a4f9372a784d4772fea258e`，受保护 dry-run 仓也不在本机，因此本轮只能确认文件哈希与 recovery ref 存在，**不能声称 restore verify PASS**。历史仓公开 recovery ref `268acce792` 仍是 PARTIAL，S24 路径为 0；受保护 full-sweep 的 1060 路径分类记录仍为 `Adopt 239 / Redo 750 / Discard 71 / Conflict 0 / 未分类 0`。本轮没有 import、replay、merge、force-push 或复制私有元数据。
...
- 当前总进度：Task `31/56`；S07 phase `3/4`，S07 整体复审未开始；P7.3 + report/audit/public/access/abuse focused regression `136 passed`，final-image synthetic API + Chromium Oracle PASS，同 key concurrent/business-result=`24/1`，GET/HEAD state-delta/long-task=`0/0`，cancel/retry/timeout/source-change/expiry/restart/rollback 全 PASS。此前 full-backend 结果仅作历史诊断，不是当前或后续前置。published Stage 仍为 `6/14`；Draft PR `#194` 只是接管快照，尚未合并 `main`，不改变 published Stage、G2 或 GA。生产 P6.1/P6.2/P6.3/P7.1/P7.2/P7.3 Flags 保持 `0`。
- 若远端没有更新，下一个新 run 只可执行 **S07 / P7.4 / T-S07-04**，且最多一个 phase；若新 agent 已推进，则以远端最新 HANDOFF/receipt/测试为准，不得重复或回退。不得提前进入 S07 整体复审、S08、合并 `main`、部署、灰度、启用生产 Flags，或切生产 PostgreSQL/S3/backup/lifecycle、启用生产删除。不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、任何 export job/event/artifact、assessment/event/intent/chunk/original/derivative/backup、既有 verifier/session 或 v1.5 恢复资产。时间逻辑只能使用 Fake Clock、历史回放、Fixture、模拟消息和即时故障注入；禁止真实时间 soak、观察期、等待窗口、后台空转式验收、重复审批、形式化 Gate、无限重试和把全量测试作为前置。P7.3 快速回滚置 `KMFA_EXPORT_JOBS_ENABLED=0` 并停止 worker，继续保留既有 status/artifact/cancel、export/audit 与源报告；禁止 binary/schema downgrade。正式发布仍仅在整个 v1.5.2 Taskpack 完成并通过最终整包复审后进行。
