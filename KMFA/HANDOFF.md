...
- runner 现在将截图相邻真实 `.ocr.txt` 文本侧车写入 `ocr_text_candidates.csv`，关联原截图 evidence 并追加 `OCR_TEXT_PENDING_REVIEW` 任务；OCR 文本只进入待复核链路，`financial_fact_promoted=false`，不自动生成金额或管理结论。
