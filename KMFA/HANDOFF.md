...
- 最近完成的唯一执行单元：**S05 / P5.3 / T-S05-03 一致性状态机**，见 `machine/runs/S05_P53_CONSISTENCY_STATE.md`。仅完成 `R-ARCH-002 / AC-ARCH-002`：schema `3` 增加 upload/process/index/export operation journal、hash-only idempotency、transactional outbox、consumer receipt、append-only trace、attempt-fenced lease/retry、reconciliation 与 persistent quarantine；真实上传路径已按 intent→object verify→DB+outbox→converged 续跑，恢复按 operation 持久化 backend 选 adapter，SQLite→PG trace 按源 `seq` 保序，原始对象不作补偿删除。runtime-frozen image `sha256:2fda6a3…85dd9` 的 PostgreSQL+private MinIO+external effect Oracle PASS：崩溃点 `28`、timeout-after-apply `2`，operation `31=30 converged+1 isolated`、outbox `30/30 delivered`，partial/unexplained terminal/invalid trace/duplicate external effect/raw delete/staged `.part` 均 `0`，最大 synthetic recovery `1.887s < 30s`；完整 backend `199/199`，P5.1 `14/14`、P5.2 `19/19` 与 S03/S04 exact-image 回归全 PASS；复审 `12/12` findings resolved、open `0`。process/index/export 的真实业务 consumer 仍由后续 owner Stage 接线，P5.4 backup/restore/delete 未开始；本 run 不改生产数据、Cloudflare/Coolify/R2/WAF，不上传 GitHub。
...
- 最近完成发布合同的 Stage 是 **S04 全 Stage Review + guarded upload**，publication 已闭合，published Stage=`5/14`。当前 S05/P5.1-P5.3 只是 detached worktree 的本地 phase candidate，不是 GitHub/生产 identity。S03/S04 rollout 的根域 Everyone Bypass、`/api*`/`/ops*` owner-only Access、源站 403 fail-closed、Walking Skeleton 与 abuse Gate 继续是前置条件；完整匿名旅程、生产对象切换、备份恢复、明确删除、G2 与 GA 仍未标绿。
...
- S04 发布后的当前生产身份已由同 SHA Gate、deployment query 与只读 Oracle 闭合：source `031bf9923b92d8d2ac4a690a39476825940ba587`、image `sha256:cef4592801cafa05b152a06638d7c0ce105c6b9fb4ebac6aba5700d62acf2a9c`、Coolify deployment `r8pvdppp781azpor35cu8ol2`、completed `2026-07-23T13:39:32.000000Z`；deploy run `30011560083`、governance `30011559631`、query `30012248647` 均 SUCCESS，root/index/Walking/private-boundary Oracle PASS。P5.3 local image `sha256:2fda6a3…85dd9` 不是生产 digest。
...
- P0.4/S00 的失败边界已局部刷新：生产当前仍以 `kmfa-app-state` 的 SQLite+私有文件 adapter 服务；P5.1 已在本地证明共享 PostgreSQL 结构化状态，P5.2 已在本地证明私有 S3-compatible 对象层、双 App、保卷替换和确定对账，P5.3 已在本地证明跨 DB/object/outbox 的可恢复状态、去重与隔离，但三者均未生产 cutover，P5.4 backup/restore/delete 仍未完成，所以有限 RPO/RTO 继续不可证明。不得把 synthetic Oracle、MinIO native versioning 或 named volume replacement 伪报为整套长期持久化/备份恢复 PASS。快速回滚使用 schema `3` 当前 binary 的 `KMFA_CONSISTENCY_STATE_MODE=paused`，保留 S04/P4.1 dual reader、DB/对象配置与卷并 forward-fix；禁止旧 binary schema downgrade、删卷/对象、撤既有 S3 reader 凭据、改 verifier 或 recovery replay。
- 当前总进度：Task `23/56`；S05 phase `3/4` 本地完成；published Stage=`5/14`。P5.3 findings `12/12` resolved、open `0`，full backend `199/199`、consistency Oracle 的 partial/unexplained/duplicate/raw-delete/staged-residue 均 `0`，P5.1 `14/14`、P5.2 `19/19`、S03/S04 exact-image public/recovery/secret/abuse regressions均 PASS。中间 phase 按合同只做本地 commit，不 push；S05 必须在 P5.1–P5.4 全部完成后另开 whole-stage review，修完 review findings 才整体上传。
- 下一个新 run 只可执行 **S05 / P5.4 / T-S05-04 保留、备份与恢复**。每次 run 仍最多一个 phase；不得顺带 S05 Stage Review、不得切生产 PostgreSQL/S3、不得上传 GitHub，也不得删除 `kmfa-app-state`、`kmfa-postgres-data`、`kmfa-object-data`、既有 verifier/session/object 或 v1.5 恢复资产。
