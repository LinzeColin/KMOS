...
- 最近完成的唯一执行单元：**S03 / P3.2 / T-S03-02 本地 + final-image release candidate**。根路径现按路由只加载公共 App Shell，项目/上传/搜索/进度/报告/帮助 `6/6` 可操作且对未接入能力显示真实边界；公共搜索只查本页说明，不读私有数据。桌面、390px 移动、JavaScript disabled、`/healthz` degraded 四种 Playwright 模式全绿，公共 `/api*`/`/ops*`/私有 bundle 请求、权限错误、runtime error 均为 `0`。完整旧经营 App 已迁入受守卫的 `/ops/app` 并保持真实全流 `11/11`。`KMFA_PUBLIC_SHELL_ENABLED=0` 可回退到仍含六入口的稳定静态壳，不动数据或私有守卫。backend `97/97`、Docker final image/browser、官方与仓库 taskpack、变异和 dual-plane 均 PASS，见 `machine/runs/S03_P32_APP_SHELL.md`。本 phase 未上传、未部署；AC-PUB-002 的 workspace→upload→restore→download→export 完整旅程仍未实现，严禁把导航壳冒充完整匿名产品。
- 前一执行单元 **S03 / P3.1 / T-S03-01** 继续有效：`/` GET/HEAD 直接 `200`；`/ui*` 单跳 `308 → /`；公共浅健康不泄露 facts；OpenAPI、Swagger、深健康和既有 `/api*` 均进入 `/ops*`/私有运维面，生产 compose 默认启用 Cloudflare Access JWT 源站验签且缺配置 fail-closed。见 `machine/runs/S03_P31_EDGE_ROUTING.md`。
...
- 当前总进度：本地完成 Task `14/56`，已发布 Stage `3/14`；S03 为 `2/4` 且尚未完成或发布。最近一次 fresh anonymous production Oracle（P3.1 run，2026-07-23）对 `/`、`/ui`、`/ui/`、错误 canary、`/api/状态`、`/ops/healthz` 仍全部返回 Cloudflare Access `302`；P3.2 没有上传/部署，故没有把本地壳冒充线上状态。匿名完整使用、耐久 DB/object、任意文件安全上传下载、备份恢复和 production canary/rollback 等总体能力仍未完成。
- S03 中间 phase 只允许本地 commit，不上传。下一个新 run 只能执行 **S03 / P3.3 / T-S03-03**；P3.4 与 whole-S03 review 仍须顺序完成。只有 whole-Stage 复审发现全部解决后，才允许在 host 登录墙仍开启时先建四类路径 Access app/配置 Audience，再一次 S03 GitHub upload 触发 Coolify guarded image，私有 canary 通过后最后把 host 改为 public Bypass 并跑 production Oracle；边缘回滚优先恢复 host 级 Owner Allow，源站 JWT guard 不得关闭。
