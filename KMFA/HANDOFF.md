...
- runner 现在生成 `automation_readiness.csv`，只读核对 tracked contract 与本机 Codex automation TOML；`schedule_ready=true` 需要 contract `Australia/Sydney`、当前每周一/周六 11:00 本地排程匹配，且 live TOML 如显式写 timezone 不得漂移。该证据只解除 automation schedule 外部检查，不放行管理结论。
...
- 本地 Codex automation `kmfa` 已重新与 repo mirror 对齐：tracked `weekly_mon_sat_1100_sydney.prompt.md` 和 `codex_app_automation.contract.toml` 反映当前本机 symlink alias cwds、每周一/周六 11:00 本地排程与上游 DWS zip 优先级，`check_codex_app_automation.py` 返回 `CODEX_AUTOMATION_READY`。
...
- S38 历史版本曾将 live Codex App automation `kmfa`、repo contract、prompt 和 launchd fallback 改为每周一/周六 11:00 悉尼本地时间；该时间一度被 S97/S103/S106 每天 11:30 contract 取代，S101/S105 又短暂恢复 weekly，当前按 2026-07-09 最新 owner 指令再次以 Sydney 本地每周一/周六 11:00 为准。真实运行 `run_fund_weekly_analysis.py --run-id s38_schedule_update_real_run_20260708` 索引 295 个真实源文件，生成 workbook `资金与税费管理母版_s38_schedule_update_real_run_20260708.xlsx`，状态仍为 `INDEXED_PENDING_EXTRACTION`，`generated_financial_amount_count=0`、`management_conclusion_allowed=false`。
...
- S79 已新增 `ocr_fact_evidence_review_queue.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 259 条 OCR staging 候选聚合为 44 个证据级 owner review queue，43 个仍 blocked、1 个为 S75 validation-only ready；所有行 no-write/no-promote/no-conclusion，evidence gate 仍 blocked。
- S80 已新增 `ocr_fact_candidate_owner_worklist.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 259 条 OCR fact staging 候选逐行进入 owner worklist，并关联 `ocr_fact_evidence_review_queue_id`；1 条 validation-only ready、258 条仍 blocked，所有行 `owner_authorization_decision=pending_owner_review` 且 no-write/no-promote/no-conclusion。
- S81 已新增 `ocr_fact_candidate_owner_decision_template.json` 和 `ocr_fact_candidate_owner_decision_preview.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 259 条候选均因缺 private `ocr_fact_candidate_owner_decisions/<run_id>.json` 输出 `blocked_missing_owner_decision_manifest`；所有 preview 行 no-write/no-promote/no-conclusion。
- S82 已新增 `ocr_fact_candidate_owner_authorization_update_draft.json` 和 `ocr_fact_candidate_owner_authorization_update_preview.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后因 owner decision preview ready=0，authorization update draft 为 0 条，259 条 update preview 全部 blocked；不会写入 private authorization manifest，仍 no-write/no-promote/no-conclusion。
- S83 已新增 `ocr_fact_controlled_ledger_row_preview.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 259 条 OCR staging 候选中 1 条 validation-only ready 被映射为正式账本形状 no-write preview，258 条仍 blocked；新预览所有行 `fund_ledger_write_allowed=false`、`formal_fund_ledger_write_allowed=false`、`financial_fact_promoted=false`、`management_conclusion_allowed=false`，`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false` 继续 fail-closed。
- S84 已新增 `ocr_fact_controlled_ledger_apply_gate.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 1 条 controlled ledger preview 因 `company,bank` 缺失输出 `blocked_missing_required_ledger_fields`，ready=0、blocking=1、planned_apply_count=0、write_allowed_count=0；`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行，`management_conclusion_allowed=false` 继续 fail-closed。
- S85 已将 owner decision preview 的 company/bank 修正接入 controlled ledger preview/apply gate：只有 `ready_for_private_ocr_fact_authorization_update_no_write` 的 owner decision 才会把非空 `owner_corrected_company/owner_corrected_bank` 应用到 `ocr_fact_controlled_ledger_row_preview.csv`，并在 preview/apply gate 中记录 `owner_decision_preview_status`、`owner_correction_applied`、`company_source`、`bank_source`。真实复跑 `s72_scheduled_full_retry_budget_20260708` 后因 private owner decision manifest 仍缺失，修正未应用，1 条 apply gate 仍因 `company,bank` 缺失 blocked；`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行。
- S86 已新增 `ocr_fact_owner_decision_correction_queue.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后从 blocked controlled ledger apply gate 派生 1 条 owner 修正任务，`missing_required_fields=company,bank`、`required_owner_fields=owner_corrected_company,owner_corrected_bank`、private owner decision manifest path 指向当前 run_id；queue/write_allowed=0，`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行。
- S87 已新增 `ocr_fact_owner_decision_correction_draft.json`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后从 correction queue 生成 1 条 owner decision 草稿，默认 `owner_authorization_decision=needs_correction`，`owner_corrected_company/bank` 仍为空并列出 required owner fields；draft 只保存在 run 目录，不写 private owner decision manifest，`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行。
- S88 已新增 `ocr_fact_owner_decision_correction_apply_preview.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 1 条 draft apply preview 输出 `blocked_draft_still_needs_owner_values`，`missing_owner_values=owner_corrected_company,owner_corrected_bank`、manual_save_ready=false、write_allowed=0；仍不写 private owner decision manifest，`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行。
- S89 已新增 `ocr_fact_owner_decision_correction_roundtrip_audit.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 1 条 roundtrip audit 输出 `blocked_owner_correction_required`，owner correction applied=false、write_allowed=0；`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行，`management_conclusion_allowed=false`。
- S90 已新增 `ocr_fact_owner_decision_correction_evidence_packet.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 1 条 evidence packet 输出 `ready_for_owner_field_review_no_write`，关联真实 source image、OCR sidecar、line excerpt `收到3.10号转出投标保证金退回`、金额 3.10 和 required owner fields；仍不自动补 company/bank、不写 private owner decision manifest，`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行。
- S91 已新增 `ocr_fact_owner_decision_correction_chat_context.csv`：真实复跑 `s72_scheduled_full_retry_budget_20260708` 后 1 条 chat context 输出 `ready_chat_context_no_write`，关联 manifest row 164、chat row 298、sender=杨婷、message_time=`2026-06-10 10:20:33`、chat excerpt `6.9资金明细`；`owner_field_autofill_allowed=false`，仍不自动补 company/bank、不写 private owner decision manifest，`fund_ledger.csv` / `formal_fund_ledger.csv` 仍 0 行。
...
- 当前目标为 `S21｜fund-weekly-analysis-skill 资金与税费 Excel 原生包`。
...
- 定时语义已按 2026-07-09 最新 owner 指令改为本机 Sydney 时间每周一/周六 `11:00`；北京时间不作为本机调度时区。
...
- 2026-07-08 S38 历史更新：Codex App automation `kmfa` / `KMFA资金周报自动化` 当时改为每周一、周六 11:00 悉尼本地时间；该时间一度被后续每天 11:30 contract 取代，S101/S105 又短暂恢复 weekly，当前按 2026-07-09 最新 owner 指令再次以 Sydney 本地每周一/周六 11:00 为准。
...
- 2026-07-09 S92 更新：`ocr_fact_owner_decision_correction_chat_neighbor_context.csv` 已加入 runner、任务包规则和治理台账，用于围绕 correction chat context 的目标 `chat_record_row_number` 前后各 2 行输出真实聊天上下文；当前 S72 真实 run 输出 row 296-300、offset `-2..2`，包含目标 row 298 `6.9资金明细` 和后续 row 300 `湖北中行300万已提回`，所有行 `owner_field_autofill_allowed=false`、write_allowed=0，正式账本、fact promotion 和管理结论继续 fail-closed。
- 2026-07-09 S93 更新：`ocr_fact_owner_decision_correction_ocr_line_context.csv` 已加入 runner、任务包规则和治理台账，用于围绕 correction evidence packet 的目标 `candidate_line_number` 前后各 3 行输出真实 OCR sidecar 原文；当前 S72 真实 run 输出 OCR line 38-44、offset `-3..3`，包含 line 39 `转湖北中行`、line 40 `收到嘉施利（宜城）化肥有限公司...投标保证金退回`、line 41 目标 `收到3.10号转出投标保证金退回`，所有行 `owner_field_autofill_allowed=false`、write_allowed=0，正式账本、fact promotion 和管理结论继续 fail-closed。
- 2026-07-09 S94 更新：`ocr_fact_owner_decision_correction_owner_review_packet.csv` 已加入 runner、任务包规则和治理台账，用于将 correction evidence packet、OCR line context、chat context 和 chat neighbor context 合并为 owner-facing 单行复核包；当前 S72 真实 run 输出 1 条 ready packet，缺 `company,bank`，要求 `owner_corrected_company,owner_corrected_bank`，候选日期 `2026-06-10`、金额 `3.10`，OCR/chat/neighbor ready count 为 `7/1/5`，所有写入/自动填充标志仍为 false，正式账本、fact promotion 和管理结论继续 fail-closed。
- 2026-07-09 S95 更新：`ocr_fact_owner_decision_correction_manifest_readiness.csv` 已加入 runner、任务包规则和治理台账，用于将 owner review packet 逐行对齐 private owner decision manifest 的存在性、entry 覆盖、approve 状态和 required owner fields 完整性；当前 S72 真实 run 输出 1 条 blocking readiness，`decision_manifest_status=missing_decision_manifest`，`manifest_readiness_status=blocked_missing_owner_decision_manifest`，缺 `owner_corrected_company,owner_corrected_bank`，write_allowed=0，正式账本、fact promotion 和管理结论继续 fail-closed。
- 2026-07-09 S96 更新：`tools/install_owner_decision_manifest.py` 已加入任务包，作为 owner 补完 correction draft 后的 private manifest 安装器；默认 dry-run，只有 `--apply --acknowledge-owner-reviewed-values` 且所有 `required_owner_fields` 非空、decision 为 `approve_for_review_authorization` 时才写 `ocr_fact_candidate_owner_decisions/<run_id>.json`。当前 S72 真实 dry-run 返回 `BLOCKED_OWNER_VALUES_MISSING`，缺 `owner_corrected_company,owner_corrected_bank`，`apply_performed=false`，目标 private manifest 仍不存在。
- 2026-07-09 S97 更新：Codex App automation `kmfa`、repo contract、prompt、launchd fallback、config、runner readiness、任务包验证器、功能清单和模型参数已同步为每天 11:30 `Australia/Sydney`；RRULE 为 `FREQ=DAILY;BYHOUR=11;BYMINUTE=30`。真实 run `s97_daily_1130_schedule_sync_20260709` 输出 `CODEX_AUTOMATION_READY`、mismatch=0、schedule_ready=true、workbook quality 7/7 PASS；`generated_financial_amount_count=0`、`management_conclusion_allowed=false` 继续 fail-closed。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `359ffb67963a8d5b73ad5efdd6c58760e4bc5ae1d5ee8444f39294c3bdf60410`，33 entries，无 pycache。
- 2026-07-09 S98 更新：已用真实 scheduled shell 入口验证 daily 11:30 路径，命令设置 `KMFA_FUND_RUN_ID=s98_daily_1130_entrypoint_smoke_20260709`、`KMFA_FUND_VISION_LIMIT=1`、`KMFA_FUND_VISION_RETRY_MAX_ROWS=0`、`KMFA_SKIP_CODEX_EXEC=1`。入口先 `git fetch/merge --ff-only`，source readiness `READY`、file_count=295；runner 输出 `INDEXED_PENDING_EXTRACTION`；Vision OCR 计划 1 条并生成 1 条 private sidecar，随后同 run id 二次索引。最终 `automation_readiness=CODEX_AUTOMATION_READY`、`screenshot_ocr_ready_count=1`、`ocr_text_candidate_count=1`、`ocr_value_candidate_count=6`、`ocr_financial_fact_candidate_count=0`、`generated_financial_amount_count=0`、`management_conclusion_allowed=false`，Git tracked worktree 保持干净。本轮不执行完整 OCR、不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写正式账本。
- 2026-07-09 S99/S100 更新：已用真实 daily 11:30 scheduled entrypoint 跑完整当前 run `s99_daily_1130_full_entrypoint_20260709`。S99 默认 OCR 预算下 planned=272、generated=232、retry attempted/generated=24/24、deferred=40；随后 S100 用同一 run id、`KMFA_FUND_VISION_RETRY_MAX_ROWS=40` 受控 resume，最终 `screenshot_ocr_ready_count=272`、`screenshot_ocr_missing_count=0`、`ocr_text_candidate_count=272`、`ocr_value_candidate_count=2736`、`ocr_financial_fact_candidate_count=274`。`automation_readiness=CODEX_AUTOMATION_READY`、workbook quality blocking=0、`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`，Git tracked worktree 保持干净。本轮只写 private runtime OCR sidecars，不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写正式账本。
- 2026-07-09 S101 更新：按当前 owner 指令将 Codex App automation `kmfa`、repo contract、prompt、launchd fallback、config、runner readiness、taskpack validator、功能清单、模型参数和 handoff 同步为每周一/周六 11:00 `Australia/Sydney`；prompt 改为 `weekly_mon_sat_1100_sydney.prompt.md`，旧 `daily_1130_sydney.prompt.md` 已移出任务包。真实 run `s101_weekly_mon_sat_1100_schedule_sync_20260709` 基于 295 个真实输入文件输出 `INDEXED_PENDING_EXTRACTION`，`automation_readiness=CODEX_AUTOMATION_READY`、mismatch=0、schedule_ready=true、workbook quality blocking=0、`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`。验证已通过：59 条 fund weekly 单测、skill static tests、taskpack static validation、py_compile、automation readiness、真实 runner smoke 和 zip integrity check。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `26e61e56c29c15472a8d39a9c77b492ce3920495834a379257b2eb5104609d66`，33 entries，无 pycache。
- 2026-07-09 S102 更新：`tools/install_owner_decision_manifest.py` 已接受 pending-review `ocr_fact_candidate_owner_decision_template.json` 作为 dry-run 输入，并在默认 correction draft 为空时自动落到候选模板；S99 默认 dry-run 返回 `BLOCKED_OWNER_DECISION_NOT_APPROVED`，owner_decision_count=274、not_approved_count=274、apply_performed=false，所有 promotion/write/conclusion 标志仍为 false。本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S103 更新：按当前 goal 口径重新将 Codex App automation `kmfa`、repo contract、prompt、launchd fallback、config、runner readiness、taskpack validator、功能清单、模型参数和 handoff 同步为本机 Sydney 时间每天 11:30；北京时间 09:30 只作为当前 UTC+10 偏移下的说明，不作为 scheduler source。prompt 恢复为 `daily_1130_sydney.prompt.md`，`weekly_mon_sat_1100_sydney.prompt.md` 已移出任务包。真实 run `s103_daily_1130_goal_realign_20260709` 基于 295 个真实输入文件输出 `INDEXED_PENDING_EXTRACTION`，`automation_readiness=CODEX_AUTOMATION_READY`、mismatch=0、schedule_ready=true、workbook quality blocking=0、`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`。验证已通过：RED/GREEN focused tests、59 条 fund weekly 单测、skill static tests、taskpack static validation、py_compile、launchd plist lint、automation readiness、真实 runner smoke 和 zip integrity check。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `33692dbe57cc9ae06ae5907b85c6b0647d6dd54186bf20eeade92eae55e6efd0`，33 entries，无 pycache。本轮不执行 OCR 生成、不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S104 更新：已用真实 scheduled shell 入口验证最新 daily 11:30 path，而不是只跑 runner 单体。命令设置 `KMFA_FUND_RUN_ID=s104_daily_1130_entrypoint_after_realign_20260709`、`KMFA_FUND_VISION_LIMIT=1`、`KMFA_FUND_VISION_RETRY_MAX_ROWS=0`、`KMFA_SKIP_CODEX_EXEC=1`；入口先 `git fetch/merge --ff-only`，source readiness 返回 `READY`、file_count=295，runner 输出 `INDEXED_PENDING_EXTRACTION`，Vision OCR planned=1/generated=1/text_available=1，随后同 run id 二次索引并跳过末尾 Codex CLI。最终 `automation_readiness=CODEX_AUTOMATION_READY`、rrule=`FREQ=DAILY;BYHOUR=11;BYMINUTE=30`、mismatch=0、`screenshot_ocr_ready_count=1`、`screenshot_ocr_missing_count=271`、`ocr_text_candidate_count=1`、`ocr_value_candidate_count=6`、`ocr_financial_fact_candidate_count=0`、`workbook_quality_blocking_count=0`、`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`。本轮不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S105 更新：按最新 owner 指令将 Codex App automation `kmfa`、repo contract、prompt、launchd fallback、config、runner readiness、taskpack validator、功能清单、模型参数和 handoff 同步为每周一/周六 11:00 `Australia/Sydney`；prompt 改为 `weekly_mon_sat_1100_sydney.prompt.md`，旧 `daily_1130_sydney.prompt.md` 已移出任务包。真实 runner smoke `s105_weekly_mon_sat_1100_schedule_sync_20260709` 基于 295 个真实输入文件输出 `INDEXED_PENDING_EXTRACTION`，`automation_readiness=CODEX_AUTOMATION_READY`、mismatch=0、schedule_ready=true、workbook quality blocking=0、`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`；本轮不执行 OCR 生成、不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。验证已通过：59 条 fund weekly 单测、skill static pytest、taskpack static validation、py_compile、launchd plist lint、automation readiness、真实 runner smoke 和 zip integrity check。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `e72ab33f39951646a3d4f34b525f5a4c3e2e06dd0788ed57db8fff33f9d201a5`，33 entries，无 pycache，无 daily prompt。
- 2026-07-09 S106 更新：按 active goal 口径将 Codex App automation `kmfa`、repo contract、prompt、launchd fallback、config、runner readiness、taskpack validator、功能清单、模型参数和 handoff 重新同步为本机 Sydney 时间每天 11:30；北京时间 09:30 只作为当前 UTC+10 偏移说明，不作为 scheduler source。prompt 恢复为 `daily_1130_sydney.prompt.md`，`weekly_mon_sat_1100_sydney.prompt.md` 已移出任务包。真实 scheduled shell 验证 `s106_daily_1130_entrypoint_realign_20260709` 基于 295 个真实输入文件输出 `INDEXED_PENDING_EXTRACTION`，Vision OCR planned=1/generated=1/text_available=1，`automation_readiness=CODEX_AUTOMATION_READY`、rrule=`FREQ=DAILY;BYHOUR=11;BYMINUTE=30`、mismatch=0、schedule_ready=true、`screenshot_ocr_ready_count=1`、`screenshot_ocr_missing_count=271`、`ocr_text_candidate_count=1`、`ocr_value_candidate_count=6`、`ocr_financial_fact_candidate_count=0`、workbook quality blocking=0、`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`。验证已通过：RED/GREEN fund weekly 单测、skill static pytest、taskpack static validation、py_compile、launchd plist lint、automation readiness、真实 scheduled shell smoke 和 zip integrity check。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `92f770716ccafa6b7615ed6e19fd3baea86cc9b251f4a71704afa74ce51d61df`，33 entries，无 pycache，无 weekly prompt。本轮不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S107 更新：runner 新增 `ocr_fact_candidate_owner_decision_progress_summary.csv`，把 owner decision preview 汇总为全量和按 metric 的 no-write 进度看板。真实复跑 `s99_daily_1130_full_entrypoint_20260709` 后输出 7 行 summary：274 条 OCR fact candidates 全部 `ready_count=0`、`blocking_count=274`、`missing_owner_decision_manifest_count=274`、`pending_owner_review_count=274`，metric 分布为 bank_deposit=7、deposit_release=1、electronic_bill=8、loan=6、payment_outflow=245、tax_payment=7；所有行仍 `fund_ledger_write_allowed=false`、`financial_fact_promoted=false`、`management_conclusion_allowed=false`。`generated_financial_amount_count=0`、`fund_ledger.csv` 0 行、`formal_fund_ledger.csv` 0 行、`management_conclusion_allowed=false` 继续 fail-closed。验证已通过：RED/GREEN focused test、59 条 fund weekly 单测、skill static pytest、taskpack static validation、py_compile 和 source readiness READY。本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S108 更新：`tools/install_owner_decision_manifest.py` 新增 `--draft-csv-path`，允许 owner 用 CSV 表格提交逐候选决策进入同一 validation-only 安装门禁；默认 dry-run，仍需 `--apply --acknowledge-owner-reviewed-values` 才可能写 private manifest。真实 S99 dry-run 使用 `ocr_fact_candidate_owner_worklist.csv` 作为 CSV 输入，返回 `BLOCKED_OWNER_DECISION_NOT_APPROVED`、`draft_format=csv`、owner_decision_count=274、not_approved_count=274、apply_performed=false，target manifest 不存在，promotion/write/conclusion 均 false。验证已通过：RED/GREEN focused CSV test 和真实 S99 CSV dry-run。本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S109 更新：新增 `tools/export_owner_decision_review_csv.py`，从 owner worklist 导出小批量 owner-facing CSV。真实 S99 导出 `deposit_release,bank_deposit,tax_payment`、每类最多 2 条，生成 ignored private runtime 文件 `ocr_fact_candidate_owner_decision_review_batch.csv`，共 5 条：deposit_release=1、bank_deposit=2、tax_payment=2；每条默认 pending、required owner fields 为 company/bank 修正、write/promote/conclusion 均 false。随后对该 CSV 运行 `install_owner_decision_manifest.py --draft-csv-path` dry-run，返回 `BLOCKED_OWNER_VALUES_MISSING`、owner_decision_count=5、apply_performed=false，target manifest 不存在。验证已通过：RED/GREEN focused export test 和真实 S99 export+intake dry-run。本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S110 更新：按最新 owner 指令将 Codex App automation `kmfa`、repo contract、prompt、launchd fallback、config、runner readiness、taskpack validator、功能清单、模型参数和 handoff 同步为每周一/周六 11:00 `Australia/Sydney`；RRULE 为 `FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`，prompt 为 `weekly_mon_sat_1100_sydney.prompt.md`，旧 `daily_1130_sydney.prompt.md` 已移出任务包。真实 `check_codex_app_automation.py` 返回 `CODEX_AUTOMATION_READY`；真实 runner smoke `s110_weekly_mon_sat_1100_schedule_sync_20260709` 基于 295 个真实输入文件输出 `INDEXED_PENDING_EXTRACTION`，`automation_readiness.csv` 和 workbook H06 均写入新 RRULE，workbook quality blocking=0，`generated_financial_amount_count=0`、`formal_fund_ledger_row_count=0`、`management_conclusion_allowed=false`。验证已通过：RED/GREEN schedule focused tests、61 条 fund weekly 单测、skill static pytest、taskpack static validation、py_compile、launchd plist lint、automation readiness、source readiness READY 和真实 runner smoke。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `94b32b81d47758a33670462c31d2086a0d527113e85f1999941a2f0059ae5297`，34 entries，无 pycache，无 daily prompt；GitHub push 待本轮收尾完成。本轮不改 OneDrive 源、不写 owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S111 更新：已用当前每周一/周六 11:00 `Australia/Sydney` contract 跑真实 scheduled shell full-entrypoint 验收，run_id 为 `s111_weekly_mon_sat_1100_full_entrypoint_20260709`，设置 `KMFA_SKIP_CODEX_EXEC=1`。首次 OCR planned=272、text_available=208、deferred=64；随后同一 run id 受控 resume `KMFA_FUND_VISION_RETRY_MAX_ROWS=64`，最终 `screenshot_ocr_ready_count=272`、`screenshot_ocr_missing_count=0`、`ocr_text_candidate_count=272`、`ocr_value_candidate_count=2630`、`ocr_financial_fact_candidate_count=217`。`automation_readiness.csv` 和 workbook H06 均为 RRULE `FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`，`schedule_ready=true`，workbook quality blocking=0。已为当前 S111 worklist 导出 12 条小批量 `ocr_fact_candidate_owner_decision_review_batch.csv`，覆盖 deposit_release、bank_deposit、tax_payment、loan、electronic_bill、payment_outflow 各 2 条；CSV intake dry-run 返回 `BLOCKED_OWNER_VALUES_MISSING`，缺 `owner_corrected_company,owner_corrected_bank`，target private manifest 不存在。验证已通过：61 条 fund weekly 单测、skill static pytest、taskpack static validation、py_compile、launchd plist lint、automation readiness 和 zip integrity check。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `94b32b81d47758a33670462c31d2086a0d527113e85f1999941a2f0059ae5297`，34 entries，无 pycache，无 daily prompt。全程不改 OneDrive 源、不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论；entrypoint fast-forward 到远端 `0ee280336` 后出现 unrelated OpenAIDatabase missing-file 状态，已从 HEAD 恢复，最终工作树重新干净。
- 2026-07-09 S112 更新：`tools/export_owner_decision_review_csv.py` 新增 `--xlsx`，可在既有 CSV 之外生成原生 `ocr_fact_candidate_owner_decision_review_batch.xlsx`。真实 S111 小批量 review workbook 已生成 12 行 / 25 列，sheet=`Owner Review`，含 owner decision 下拉、冻结表头和 no-write/no-promote/no-conclusion 字段；strict openpyxl 可读且无 style warning。验证覆盖 focused RED/GREEN test、真实 S111 XLSX 读取、taskpack validator 和 zip integrity。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `4103a070ebe94e09cc5d235a00b009c7ecb3babb61e82487890a93e7341781a7`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S113 更新：`tools/install_owner_decision_manifest.py` 新增 `--draft-xlsx-path`，可直接读取 owner review XLSX 并走与 CSV 相同的 validation-only 安装门禁。focused RED/GREEN test 覆盖已填写 XLSX dry-run：`draft_format=xlsx`、`READY_DRY_RUN`、`apply_performed=false`，目标 private manifest 不存在，promotion/write/conclusion 均 false。真实 S111 未填写 XLSX dry-run 返回 `BLOCKED_OWNER_VALUES_MISSING`，缺 `owner_corrected_company,owner_corrected_bank`，仍不写 manifest。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `bc969688dcf578a0d1076653a39185ba001e96fb0ffe8a06f21920e2bb9023f7`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S114 更新：`install_owner_decision_manifest.py` 新增 private `ocr_fact_candidate_owner_decision_intake_validation_report.csv`，对 JSON/CSV/XLSX intake 逐行输出缺字段、未 approve 或 ready 状态。真实 S111 未填写 XLSX dry-run 继续返回 `BLOCKED_OWNER_VALUES_MISSING`，但现在同时写入逐行 report，帮助 owner 定位 12 条候选缺 `owner_corrected_company,owner_corrected_bank`。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `5fd95ea1e37b0c027660e6639daa4259e25158d46aafbb4814dbda5fc8765448`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S115 更新：`ocr_fact_candidate_owner_decision_intake_validation_report.csv` 现在包含 `input_row_number`、`source_evidence_id`、OCR 文本路径、日期、金额和币种等填表上下文。真实 S111 report 仍为 12 行全部 `blocked_missing_owner_values`，但可直接用于定位每条候选的证据/金额上下文。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `7ba5bee4af0bc1a26ad1fe613c4e9f6ce443ace7e73a4d09da3fa908b7a12f84`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S116 更新：`ocr_fact_candidate_owner_decision_review_batch.csv` / `.xlsx` 新增 `owner_review_completion_status` 和 `missing_owner_fields_current`，用于在 owner 填表前直接显示每行缺值或未 approve 状态。真实 S111 review batch 已重新导出 12 行，新增列全为 `blocked_missing_owner_values` / `owner_corrected_company,owner_corrected_bank`；intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `384390287130a5e1f53568ab951b0d1c3e5c08e3c408b0ea6378a98c11cbca08`，34 entries，无 pycache，无 daily prompt。新增列不参与账本写入、不自动填 company/bank、不放行 manifest；`install_owner_decision_manifest.py` 仍以 owner 填好的 `owner_authorization_decision,owner_corrected_company,owner_corrected_bank` 做 validation-only dry-run。
- 2026-07-09 S117 更新：`ocr_fact_candidate_owner_decision_review_batch.xlsx` 的 `owner_review_completion_status` 和 `missing_owner_fields_current` 已改为公式列；owner 填写 `owner_authorization_decision,owner_corrected_company,owner_corrected_bank` 后，表内状态会用 `TEXTJOIN` 动态显示缺值或 ready。真实 S111 workbook 已重新导出，`Owner Review` 13 行 / 27 列，T2/U2 为公式，intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `d773f163fd15f8d55c3bcd93e1439e1c3b2d7be4fd855c3d9fe74b4a4d9ae50d`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S118 更新：`ocr_fact_candidate_owner_decision_review_batch.csv` / `.xlsx` 新增 `source_ocr_text_excerpt`，从 repo-local private OCR sidecar 复制短摘录，帮助 owner 直接在表内查看 OCR 上下文。真实 S111 review batch 已重新导出 12 行，12 行均带非空摘录；formula status columns 仍存在，intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `56af6943f15328377f035cbe2bcaba13c5e6255587e0bceb892cef28b779dcd5`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S119 更新：`source_ocr_text_excerpt` 现在优先围绕候选金额或业务日期命中行输出 OCR 上下文，只有无命中才回退 OCR 开头。真实 S111 review batch 已重新导出 12 行，12 行摘录非空，7 行命中候选金额；intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `02593eb892820bc8468401e80a4efea37fe783d2684a31e44e54f879c94d8c8f`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S120 更新：`ocr_fact_candidate_owner_decision_review_batch.csv` / `.xlsx` 新增 `source_ocr_excerpt_focus_status`，标出 OCR 摘录是 `focused_amount`、`focused_business_date`、`fallback_file_start` 还是 OCR path/sidecar 异常状态。真实 S111 review batch 已重新导出 12 行，状态分布为 `focused_amount=9`、`focused_business_date=1`、`fallback_file_start=2`，12 行摘录非空；XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，缺 `owner_corrected_company,owner_corrected_bank`，target private manifest 不存在。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `07fe0d492382cffcd27ded9d9ebd41cde798f061d74ac82aec653ce277706c86`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S121 更新：`ocr_fact_candidate_owner_decision_review_batch.csv` / `.xlsx` 新增 `source_ocr_excerpt_line_range`、`source_ocr_excerpt_focus_line_number` 和 `source_ocr_excerpt_match_value`，让 owner 可按 private OCR sidecar 物理行号回查摘录。真实 S111 review batch 已重新导出 12 行，12 行有 line range，10 行有 focus line/match value，2 行为 fallback；XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，缺 `owner_corrected_company,owner_corrected_bank`，target private manifest 不存在。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `e327240b90bc61c0baac26a9a0eb2f8c6f2750ec1527f9c48040bfdaf0fae10f`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S122 更新：`ocr_fact_candidate_owner_decision_intake_validation_report.csv` 现在从 CSV/XLSX intake 透传 `source_ocr_text_excerpt`、`source_ocr_excerpt_focus_status`、`source_ocr_excerpt_line_range`、`source_ocr_excerpt_focus_line_number` 和 `source_ocr_excerpt_match_value`。真实 S111 XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，report 12 行全部保留 OCR 摘录/状态/line range，10 行保留 focus line/match value，所有 write/promote/conclusion 标志仍为 false。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `63c2c29f09c09478adda90dcf3b5372b666b8dbc6f70120a94497a16c7895a09`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S123 更新：`install_owner_decision_manifest.py` 新增 private `ocr_fact_candidate_owner_decision_intake_summary.csv`，对 JSON/CSV/XLSX intake 的 validation rows 生成 `all` 和按 `candidate_metric` 的 owner action 汇总。真实 S111 XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，summary 7 行：`all=12 candidates/0 ready/12 blocking`，并按 bank_deposit、deposit_release、electronic_bill、loan、payment_outflow、tax_payment 分组；所有 write/promote/conclusion 标志仍为 false。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `015b8cd38b8d9834c4849d41f80de9b34a85e093dd409c8298c3a051b9f689c0`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S124 更新：`ocr_fact_candidate_owner_decision_intake_summary.csv` 现在同时输出按 `source_ocr_excerpt_focus_status` 的 owner action 汇总。真实 S111 XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，summary 10 行：`all=12 candidates/0 ready/12 blocking`，focus status 分布为 `focused_amount=9`、`focused_business_date=1`、`fallback_file_start=2`，所有 write/promote/conclusion 标志仍为 false。当前本机 automation 已复核为每周一/周六 11:00 `Australia/Sydney`，RRULE=`FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`，`CODEX_AUTOMATION_READY`；source readiness `READY`、file_count=295。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `6285a3b82f04f2a39527773484be1826610e4d83f558c3c576d911b0240225a3`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S125 更新：`ocr_fact_candidate_owner_decision_review_batch.xlsx` 现在包含 `Owner Review` 和静态 `Review Summary` 两个 sheet；`Review Summary` 按 `all`、`candidate_metric`、`source_ocr_excerpt_focus_status` 汇总当前导出的 12 条 owner review batch。真实 S111 workbook 已重新导出，bundled openpyxl 读取为 `Owner Review` 13x32、`Review Summary` 11x12，summary all row 为 `12 candidates/0 ready/12 blocking`；XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，validation summary 10 行，所有 write/promote/conclusion 标志仍为 false。当前本机 automation 仍为每周一/周六 11:00 `Australia/Sydney`，RRULE=`FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`，`CODEX_AUTOMATION_READY`；source readiness `READY`、file_count=295。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `a44bda296f06261388701f4b4ab71002dc118fcd41f5a771f42ffe9cde6a527c`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S126 更新：`ocr_fact_candidate_owner_decision_review_batch.xlsx` 的 `Review Summary` sheet 已启用 sheet protection，防止误改静态汇总；`Owner Review` 保持可编辑。真实 S111 workbook 已重新导出，bundled openpyxl 验证 `Owner Review.protection.sheet=false`、`Review Summary.protection.sheet=true`，summary all row 为 `12 candidates/0 ready/12 blocking`；XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，validation summary 10 行，所有 write/promote/conclusion 标志仍为 false。当前本机 automation 仍为每周一/周六 11:00 `Australia/Sydney`，RRULE=`FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`，`CODEX_AUTOMATION_READY`；source readiness `READY`、file_count=295。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `478467b66e3e645ac7c986087743eee5907b67787803fcd4337f8c09fbefbcfd`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S127 更新：`ocr_fact_candidate_owner_decision_review_batch.xlsx` 的 `Owner Review` sheet 也已启用 sheet protection，仅解锁 `owner_authorization_decision`、`owner_corrected_company`、`owner_corrected_bank`、`owner_note` 四个 owner 输入列。真实 S111 workbook 已重新导出，bundled openpyxl 验证 `Owner Review.protection.sheet=true`、`Review Summary.protection.sheet=true`、V2/W2/X2/AB2 unlocked、A2/H2/Y2/Z2/AA2/AF2 locked，summary all row 为 `12 candidates/0 ready/12 blocking`；XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，所有 write/promote/conclusion 标志仍为 false。当前本机 automation 仍为每周一/周六 11:00 `Australia/Sydney`，RRULE=`FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`，`CODEX_AUTOMATION_READY`；source readiness `READY`、file_count=295。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `a69739d0b04148194f09c47973ecc7ff8a1055cc12f346c33f70cadec8a71495`，34 entries，无 pycache，无 daily prompt；本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S128 更新：新增 `tools/validate_owner_review_workbook.py`，对原生 `ocr_fact_candidate_owner_decision_review_batch.xlsx` 做 OOXML 只读校验，要求 `Owner Review` / `Review Summary` 存在且受保护、owner 输入列解锁、证据/状态/写入标志列锁定、owner decision 下拉存在、row count 非零、所有 write/promote/conclusion flags 为 false，成功返回 `OWNER_REVIEW_WORKBOOK_READY`。真实 S111 workbook validator 返回 ready：owner rows=12、summary rows=10、failed_checks=[]；XLSX intake dry-run 仍返回 `BLOCKED_OWNER_VALUES_MISSING`，未写 private manifest。当前本机 automation 仍为每周一/周六 11:00 `Australia/Sydney`，RRULE=`FREQ=WEEKLY;BYDAY=MO,SA;BYHOUR=11;BYMINUTE=0`；任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `a3f7169d9d2b7073250ce0281c720aa9a7ad36bb32af5eea411f198dad55f72e`，35 entries，无 pycache，无 daily prompt。本轮不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 2026-07-09 S129 更新：`tools/check_codex_app_automation.py` 新增 tracked contract 自检，防止 repo contract 和 live automation 同时漂移时被误判 ready；若 rrule、timezone、weekly prompt、真实 OneDrive 输入目录、source readiness gate 或 `main_only_no_branch_no_pr_no_worktree` 偏离当前 owner-approved contract，即返回 `CODEX_AUTOMATION_CONTRACT_INVALID`。RED/GREEN focused test 覆盖 stale daily 11:30 contract 即使 live 匹配也必须 fail-closed；当前真实 `check_codex_app_automation.py` 仍返回 `CODEX_AUTOMATION_READY`，source readiness `READY`、file_count=295。验证已通过：66 条 fund weekly 单测、skill static pytest、taskpack static validation、py_compile、automation readiness、source readiness。任务包 zip 已刷新到 `/Users/linzezhang/Downloads/fund-weekly-analysis-skill-taskpack.zip`，SHA256 `90878d86f21ad7de674a062b90a6b58475b02e4b8bd73faed578c8bce36318a7`，35 entries，无 pycache，无 daily prompt。本轮不改本机 automation 时间、不改 OneDrive 源、不写 private owner decision manifest、不晋升事实、不写 fund/formal ledger、不生成管理结论。
- 下一步：若 owner 要继续推进，应直接打开 S111 `ocr_fact_candidate_owner_decision_review_batch.xlsx`，先看受保护的 `Review Summary` 确认 12 条候选的 metric/focus status 分布，再在 `Owner Review` 中只填写解锁的 owner 输入列；保存后先运行 `validate_owner_review_workbook.py --workbook-path <reviewed.xlsx>`，通过 `OWNER_REVIEW_WORKBOOK_READY` 后再运行 `install_owner_decision_manifest.py --draft-xlsx-path <reviewed.xlsx>` validation-only dry-run；不得把相邻聊天上下文自动解释为字段授权、账本写入或管理结论。
