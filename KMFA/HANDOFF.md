...
v1.2 FULL_HTML_NO_OMISSION 完整任务包已成为 KMFA 后续开发基线，并已按 v1.2 重新走完 Stage 1。Stage 2 数据治理内核与 metadata 协议已完成、复审通过并上传 GitHub main。Stage 3 已完成 `S03-P1｜文件型导入`、`S03-P2｜数据源检查矩阵` 和 `S03-P3｜源优先级` 的本地实现、验证、整体复审和 GitHub main 上传。当前 Stage 4 已完成 `S04-P1｜金额工具`、`S04-P2｜字段标准化` 和 `S04-P3｜基础工具测试` 的本地实现、验证和整体复审；复审修复了 owner-readable 金额工具详情缺口，当前为 upload-ready local-only，尚未上传 GitHub。
...
- Stage 4 复审修复了 `功能清单.md` 中 `FEAT-KMFA-016` 金额工具详情缺口；GitHub 上传尚未执行。

## 关键决策

- canonical GitHub 目录为 `LinzeColin/CodexProject/KMFA`。
- 本地主仓库 root 为 `/Users/linzezhang/Documents/Codex/CodexProject`；普通开发必须使用 `/Users/linzezhang/Documents/Codex/main_worktree/CodexProject/<task>` 下的独立 task worktree。
...
- Stage 4 整体复审已通过并处于 upload-ready local-only；GitHub 上传尚未执行，不能声明已上传。
...
下一步若继续开发，只执行 `S04｜金额精度、字段标准化与基础工具` 的 final GitHub upload：从干净隔离 worktree 对齐最新 `origin/main`，纳入 Stage 4 review 提交，复跑全部 validator，生成 upload 证据并 push；不扩大到 S05、UI、报告、事实层、zero-delta 或自动接口。
