...
const MODULES = [
  ...
  },
  {
    ...
    detail: '断点灰度开启时，下方早期骨架接受一个任意类型、最大 64 MiB 的文件并固定分片续传；回滚时保留标准 8 MiB 上传。安全灰度开启后，原始字节先隔离并由私网扫描器分类；只有 clean 的 UTF-8 文本或 JSON 可生成有界纯文本预览，未知、高风险、超时或异常结果从不冒充 clean，原件永不执行。',
    facts: ['断线后重选同一文件从服务器偏移继续', '拒绝项保持隔离且禁止下载', '分片、完整文件与下载均校验 SHA-256'],
  },
  {
    key: 'search',
    code: '03',
