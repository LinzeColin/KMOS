...
const MODULES = [
  ...
  },
  {
    ...
    detail: '断点灰度开启时，下方早期骨架接受一个任意类型、最大 64 MiB 的文件并固定分片续传；回滚时保留标准 8 MiB 上传。原始字节私有保存且只按附件下载，恶意文件扫描和多文件生命周期仍待后续阶段完成。',
    facts: ['断线后重选同一文件从服务器偏移继续', '未知扩展名可存储但不执行或预览', '分片与完整文件均校验 SHA-256'],
  },
  {
    key: 'search',
    code: '03',
