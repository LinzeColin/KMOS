...
SAMPLE = {
    ...
    "项目": [{
        "合同编号": "KMX2026120-004", "甲方名称": "某公司", "阶段": "已完工",
        "含税合同金额": "98368.00", "业务台账成本": "5431.00", "金蝶归集成本": "4197.00",
        "已知成本下限": "5431.00", "毛利上限": "92937.00", "毛利上限率": "94.5%",
        "成本数据": "两口径均有数", "数据提示": [],
    }],
}
...
def test_both_cost_calibers_are_served_without_being_merged(tmp_path, monkeypatch):
    ...
    assert row["业务台账成本"] == "5431.00" and row["金蝶归集成本"] == "4197.00"
    assert row["已知成本下限"] == "5431.00", "取大者——成本本就被低估，取小会让上限更虚高"
