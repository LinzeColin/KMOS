...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s11-stage-review` |
| 当前 Stage | `S11｜前端基础界面与数据源检查板` |
| 当前 Phase | `S11_STAGE_REVIEW｜整体复审｜本地通过` |
| 当前 Task | `KMFA-S11-STAGE-REVIEW-20260701` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5、Stage 6、Stage 7、Stage 8、Stage 9 和 Stage 10 均已完成三个 Phase、整体复审和 GitHub upload；Stage 11 三个 Phase 与整体复审已本地通过；下一步只能执行 Stage 11 final GitHub upload gate |
...
当前 Stage 6、Stage 7、Stage 8、Stage 9 和 Stage 10 均已完成整体复审和 GitHub upload；S10-P1 已建立 2 个 public-safe 报告模板和 11 个管理可读章节，S10-P2 已将 2 条报告可信等级记录锁定为 D，S10-P3 已生成 2 个 public-safe HTML 预览和 2 个 CSV/Excel-compatible 附表，Stage 10 upload 证据位于 `KMFA/stage_artifacts/S10_STAGE_REVIEW/`。S11-P1 已完成 public-safe 首页与导航本地验证，生成 8 个首页模块和蓝色商务风 HTML 首页样张。S11-P2 已完成 public-safe 数据源检查板本地验证，生成 13 行来源状态矩阵和蓝灰商务风 HTML 检查板样张。S11-P3 已完成 public-safe 项目成本页面本地验证，生成 4 条项目页面记录和蓝色商务风 HTML 项目成本页面。Stage 11 整体复审已本地通过，证据位于 `KMFA/stage_artifacts/S11_STAGE_REVIEW/`；S10 导出物与 S11 前端页面仍显示正式报告阻断，不能作为正式经营决策依据；Stage 11 GitHub upload、lineage 完整检查、正式报告和外部接口仍未完成。下一步只能执行 Stage 11 final GitHub upload gate。
...
当前 Stage 11 review 验证:

```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest KMFA.tests.test_s11_stage_review -q
PYTHONDONTWRITEBYTECODE=1 python3 KMFA/tools/check_s11_stage_review.py
