...
| 当前 Phase | `S12-P3 manual rerun mechanism completed local_only_no_go_upload_deferred` |
| 当前 Task | `KMFA-V014-S12-P3-MANUAL-RERUN-MECHANISM-20260705` |
...
| Stage 上传规则 | v0.1.4 不按单个 Stage 做 GitHub upload gate；GitHub main upload 延期到 v1.4 Stage 1-18 全部完成、整体复审通过并修复 findings 后一次性执行。当前已完成 S12-P3：source previews=5、eligible events=2、blocked previews=3、cache invalidations=2、rerun steps=8、same-source consistency checks=2；只生成 public-safe 重跑机制和 HTML 样张，不执行 Stage 12 review、GitHub upload、protected source matching、lineage full check、正式报告、UI runtime、live connector、app reinstall、OpMe 或业务执行；当前 Go/No-Go 仍为 `NO_GO`，`delivery_allowed=false` |
