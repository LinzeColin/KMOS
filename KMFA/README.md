...
| 当前 Phase | `S05-P3 权威基准锁定` |
| 当前 Task | `S05-P3-T1 人工确认后将 Q4 字段升级为 Q5 计算基准` |
| 当前验收链 | `8 PDF + 1 Excel 摘要 -> 字段级来源 -> Owner 确认 -> Q5 冻结 -> 零差异` |
| 历史结构验证 | `S01-S18` 校验器和 stage artifacts 只证明结构曾通过，不证明真实 A0 数字已接通 |
| 风险等级 | `T3`，涉及经营数据、金额、税务、隐私和公开仓库边界 |
| GitHub 目录 | `LinzeColin/KMOS/KMFA` |
| 私有恢复入口 | PRIVATE `LinzeColin/KMFA-Private-Runtime`，Release `cleanup-handoff-20260717` |
| 当前基线 | `Q4 / D / NO_GO / 3-9-2-1`；真实 A0 源优先级链未接通，`lineage_complete=false` |
| GitHub 规则 | public-safe 源码验证后直接推送 `origin/main`；raw/private/凭据始终排除，代码同步不等于业务 release |
...
| 文件 | 用途 |
|---|---|
| `文档/00_我在哪.md` | 当前状态、路线图与阻塞 |
| `文档/01_产品需求.md` | 用户、目标与非目标 |
| `文档/02_系统架构.md` | 功能、参数和数据结构 |
| `文档/03_口径字典.md` | 术语、数字与恒真规则 |
| `文档/04_操作流程.md` | 主业务流程 |
| `文档/05_执行与验收.md` | 当前任务、验收与最近运行 |
| `文档/06_运维手册.md` | 运行、故障与变更 |
| `HANDOFF.md` | 清理、恢复和跨线程接管摘要 |

### 机器可读面

| 文件/目录 | 用途 |
|---|---|
| `machine/facts/` | 当前唯一事实源；修改后由 renderer 生成 `文档/` |
| `machine/runs/` | 当前执行记录 |
| `machine/legacy/` | 旧 handoff 与历史治理归档，只作取证 |
| `docs/governance/project.yaml` | Lean v2 历史兼容事实 |
