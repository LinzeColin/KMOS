...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.4-s03p1-file-registration` |
| 当前 Stage | `v0.1.4 Stage 3` |
| 当前 Phase | `S03-P1 completed_validated_local_only_no_go_upload_deferred` |
| 当前 Task | `KMFA-V014-S03-P1-FILE-REGISTRATION-20260703` |
...
| Stage 上传规则 | v0.1.4 不按单个 Stage 做 GitHub upload gate；GitHub main upload 延期到 v1.4 Stage 1-18 全部完成、整体复审通过并修复 findings 后一次性执行。当前已完成 S03-P1 文件型导入登记，新增 `V014_S03_P1_FILE_REGISTRATION` evidence、raw root read-only registration tool、validator 和 focused unit test；本机 raw root 只读登记文件数 `5`、支持类型数 `5`、总字节数 `62788056`；公开仓库只保存聚合计数、类型/大小/status 和 private refs，raw 细节留在 git-ignored private runtime；未执行 S03-P2、Stage 3 review、GitHub upload、raw value matching、lineage full check、正式报告、live connector 或业务执行；当前 Go/No-Go 仍为 `NO_GO`，`delivery_allowed=false` |
