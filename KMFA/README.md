...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s06-stage-review` |
| 当前 Stage | `S06｜零差异校验与差异处理队列` |
| 当前 Phase | `S06-UPLOAD｜Stage 6 final GitHub upload 待执行` |
| 当前 Task | `KMFA-S06-FINAL-GITHUB-UPLOAD` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5 已完成三个 Phase、整体复审和 GitHub upload；S06-P1/S06-P2/S06-P3 已本地验证，Stage 6 整体复审已通过，本轮未 push；下一步只执行 Stage 6 final GitHub upload gate |
...
| `stage_artifacts/S06_STAGE_REVIEW/` | Stage 6 整体复审证据包，记录 S06-P1/P2/P3 复跑、治理 validator、raw/secret scan 和 evidence consistency check |
...
当前 `S06-P1｜零差异校验器` 已本地完成：`KMFA/tools/zero_delta_validator.py` 对 public-safe 已结构化整数分字段逐字段比较，任意 1 分差异失败，并输出 mismatch report；证据位于 `KMFA/stage_artifacts/S06_P1_zero_delta_validator/`。当前 `S06-P2｜跨源差异队列` 也已本地完成：`KMFA/tools/cross_source_difference_queue.py` 将 public-safe PDF/Excel 同项目同字段 1 分差异送入人工差异队列，禁止自动修正、平均、四舍五入掩盖和自动选边；未关闭差异会阻断 A 级报告，证据位于 `KMFA/stage_artifacts/S06_P2_cross_source_difference_queue/`。当前 `S06-P3｜校验证据输出` 已本地完成：`KMFA/tools/validation_evidence_output.py` 将 S06-P1/S06-P2 public-safe 结果输出到 stage artifact，并写入 `KMFA/metadata/quality` 的 hash/ref/status/evidence 证据面；metadata 不新增字段明文或原始金额值。Stage 6 整体复审已通过，证据位于 `KMFA/stage_artifacts/S06_STAGE_REVIEW/`；本轮仍不实现事实层、lineage 完整检查、正式报告、UI、外部接口或 GitHub upload。
