...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5 已完成三个 Phase 本地验证，Stage 5 整体复审和 GitHub upload 尚未执行 |
...
当前 Stage 5 的 S05-P1、S05-P2、S05-P3 已完成本地验证：S05-P1 登记 8 个 PDF + 1 个 Excel 的 public-safe A0 文件清单；S05-P2 生成字段合同和 45 条候选，并通过 active owner/授权降级决策将 Excel candidate 排除为 cross-source support only；S05-P3 将 40 条 PDF 字段 hash/source-anchor 记录锁定为 public-safe Q5 calculation baseline，5 条 Excel 字段不进入正式报告。仍不实现 zero-delta、事实层、lineage 完整检查、报告、UI 或 GitHub upload。
