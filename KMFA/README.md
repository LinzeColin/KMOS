...
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s01p3` |
| 当前 Stage | `S01｜只读启动与项目治理基线` |
| 当前 Phase | `S01 Stage Review｜阶段复审` |
| 当前 Task | `KMFA-S01-STAGE-REVIEW-20260629` |
| 风险等级 | `T3`，涉及经营数据、金额、税务、隐私和公开仓库边界 |
| GitHub 目录 | `LinzeColin/CodexProject/KMFA` |
| Stage 上传规则 | S01 三个 Phase 已完成且阶段复审通过；上传必须使用隔离 worktree，避免混入非 KMFA 变更 |
...
当前 `S01-P3` 已建立防遗漏基线，Stage 1 已完成总复审，但仍不实现上述业务链路。
