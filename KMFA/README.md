...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.3-s10p2-report-grade-runtime-replay` |
| 当前 Stage | `v0.1.3 S10｜报告模板与可信等级` |
| 当前 Phase | `S10-P2 report grade runtime replay completed local only` |
| 当前 Task | `KMFA-V013-S10-P2-REPORT-GRADE-RUNTIME-REPLAY-20260703` |
...
| Stage 上传规则 | v0.1.3 不按单个 Stage 做 GitHub upload gate；GitHub main upload 延期到 Stage 1-10 全部完成、整体复审通过并修复 findings 后一次性执行；当前 S10-P2 report grade runtime replay 已本地通过，锁定 2 条 public-safe 报告等级记录且均为 `D`，12 条 pending reconciliation、zero-delta=false、缺少完整 lineage 和人工确认继续阻断完整可信报告、正式报告、导出和经营决策依据；未执行 S10-P3、Stage 10 review、GitHub upload、raw value matching、lineage full check、正式报告或业务执行；当前 Go/No-Go 仍为 `NO_GO`，`delivery_allowed=false` |
