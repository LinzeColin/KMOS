...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s08-stage-review` |
| 当前 Stage | `S08｜业务实体与项目身份匹配` |
| 当前 Phase | `S08-FINAL-GITHUB-UPLOAD｜Stage 8 final GitHub upload 待开始` |
| 当前 Task | `KMFA-S08-FINAL-GITHUB-UPLOAD-20260630` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5、Stage 6 和 Stage 7 均已完成三个 Phase、整体复审和 GitHub upload；S08-P1、S08-P2、S08-P3 和 Stage 8 整体复审已本地完成并提交证据，下一步只执行 Stage 8 final GitHub upload |
...
当前 Stage 6 已完成整体复审和 GitHub upload：`S06-P1｜零差异校验器` 对 public-safe 已结构化整数分字段逐字段比较，任意 1 分差异失败并输出 mismatch report；`S06-P2｜跨源差异队列` 将 public-safe PDF/Excel 同项目同字段 1 分差异送入人工差异队列，禁止自动修正、平均、四舍五入掩盖和自动选边，并阻断未关闭差异的 A 级报告；`S06-P3｜校验证据输出` 将 S06-P1/S06-P2 public-safe 结果输出到 stage artifact，并写入 `KMFA/metadata/quality` 的 hash/ref/status/evidence 证据面。Stage 6 upload evidence 位于 `KMFA/stage_artifacts/S06_STAGE_REVIEW/`。Stage 7 已完成整体复审和 GitHub upload：S07-P1 财务文件适配覆盖经营分析、日记账、客户账龄、现金、纳税、开票、账户、贷款、研发费用 9 类支撑源，生成 45 条 hash-only 字段候选和 9 条只读字段报告；S07-P2 WPS 文件适配覆盖回款、应收账龄、生产项目状态、保证金 4 类导出，生成 20 条 hash-only 字段映射、4 条转换提示、4 条只读字段报告和 1 个映射规则版本；S07-P3 红圈经营、合同、回款、财务 4 类导出仅预留模板，D15 文件型 MVP 不接自动接口，后续接入必须只读、留 hash、可回滚。Stage 7 upload evidence 位于 `KMFA/stage_artifacts/S07_STAGE_REVIEW/`。S08-P1 已完成本地验证：项目组合键将合同编号、项目名称、对手方、主体、时间、金额签名、责任人、来源 hash 组合为 public-safe profile，使用整数权重评分，单字段缺失不全阻断，低于强匹配阈值进入人工复核队列。S08-P2 已完成本地验证：定义 customer、contract、project、cost_record、invoice、collection、receivable、tax_evidence 8 类 public-safe 业务实体，建立 14 条关系和 32 条生命周期状态，并输出 schema 文档与 metadata。S08-P3 已完成本地验证：覆盖同名项目、多主体、多账户、多期间 4 类 public-safe 匹配质量场景，生成 4 条 quality cases、3 条人工复核队列记录和 1 份 entity_matching_report，中高风险候选不得自动合并。Stage 8 整体复审已本地通过并记录 `KMFA/stage_artifacts/S08_STAGE_REVIEW/`，但尚未执行 GitHub upload；事实层、lineage 完整检查、正式报告、UI、外部接口仍未完成。
