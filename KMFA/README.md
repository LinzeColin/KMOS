...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s02p3-v12baseline` |
| 当前 Stage | `S01｜v1.2 完整任务包重放已完成；S02 已保留为已上传历史` |
| 当前 Phase | `S01-v1.2-REPLAY｜FULL_HTML_NO_OMISSION 基线承接` |
| 当前 Task | `KMFA-S01-V12-REBASE-20260629` |
...
| Stage 上传规则 | v1.2 Stage 1 重放完成后复审；公开仓库只保存可提交任务包、HTML样板、hash/manifest 和治理证据 |
...
当前 `S02-P3` 已建立 Q0-Q5 数据质量等级、A/B/C/D 报告可信等级和报告发布门禁协议；本轮又按 v1.2 完整任务包重放 Stage 1，将 HTML/UIUX/报告样板升级为后续 S10/S11/S12/S18 的硬门禁。仍不实现上述业务链路。
