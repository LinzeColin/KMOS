...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s10-stage-review` |
| 当前 Stage | `S10｜报告可信等级与经营报告生成` |
| 当前 Phase | `S10-GITHUB-UPLOAD｜待开始` |
| 当前 Task | `KMFA-S10-GITHUB-UPLOAD-20260630` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5、Stage 6、Stage 7、Stage 8 和 Stage 9 均已完成三个 Phase、整体复审和 GitHub upload；Stage 10 三个 phase 与整体复审已本地完成，下一步只执行 Stage 10 final GitHub upload gate |
...
当前 Stage 6、Stage 7、Stage 8 和 Stage 9 均已完成整体复审和 GitHub upload；S10-P1 已建立 2 个 public-safe 报告模板和 11 个管理可读章节，S10-P2 已将 2 条报告可信等级记录锁定为 D，S10-P3 已生成 2 个 public-safe HTML 预览和 2 个 CSV/Excel-compatible 附表，Stage 10 整体复审已本地通过。S10 导出物仍显示 D 级阻断，不能作为正式经营决策依据；lineage 完整检查、正式报告、UI 和外部接口仍未完成。下一步只进入 `S10-GITHUB-UPLOAD｜final upload gate`。
