...
| 当前 Phase | `Stage 7 整体复审 待开始` |
| 当前 Task | `KMFA-S07-STAGE-REVIEW-GATE` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5 和 Stage 6 均已完成三个 Phase、整体复审和 GitHub upload；S07-P1、S07-P2、S07-P3 已本地验证完成，下一步只执行 Stage 7 整体复审 |
...
当前 Stage 6 已完成整体复审和 GitHub upload：`S06-P1｜零差异校验器` 对 public-safe 已结构化整数分字段逐字段比较，任意 1 分差异失败并输出 mismatch report；`S06-P2｜跨源差异队列` 将 public-safe PDF/Excel 同项目同字段 1 分差异送入人工差异队列，禁止自动修正、平均、四舍五入掩盖和自动选边，并阻断未关闭差异的 A 级报告；`S06-P3｜校验证据输出` 将 S06-P1/S06-P2 public-safe 结果输出到 stage artifact，并写入 `KMFA/metadata/quality` 的 hash/ref/status/evidence 证据面。Stage 6 upload evidence 位于 `KMFA/stage_artifacts/S06_STAGE_REVIEW/`。S07-P1 已完成本地验证：财务文件适配覆盖经营分析、日记账、客户账龄、现金、纳税、开票、账户、贷款、研发费用 9 类支撑源，生成 45 条 hash-only 字段候选和 9 条只读字段报告。S07-P2 已完成本地验证：WPS 文件适配覆盖回款、应收账龄、生产项目状态、保证金 4 类导出，生成 20 条 hash-only 字段映射、4 条转换提示、4 条只读字段报告和 1 个映射规则版本。S07-P3 已完成本地验证：红圈经营、合同、回款、财务 4 类导出仅预留模板，D15 文件型 MVP 不接自动接口，后续接入必须只读、留 hash、可回滚；本轮仍不实现 Stage 7 review、事实层、lineage 完整检查、正式报告、UI 或外部接口。
