...
| 当前 Phase | `S04-P1｜金额工具` |
| 当前 Task | `S4PAT01-S4PAT03` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；S04-P1 已本地验证，Stage 4 尚未复审或上传 |
...
当前 `S04-P1` 已建立金额工具：`normalize_amount_to_cents` 使用 `Decimal` 和整数分输出，支持元、万元、千元、千分位、负数和括号负数；`check_no_float_money.py` 阻断 KMFA Python 代码中的 float literal、`float()` 调用和 float 标注。仍不做 S04-P2 字段标准化、不建立 A0 黄金基准、不实现 zero-delta、事实层或报告。
