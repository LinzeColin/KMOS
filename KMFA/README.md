...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.4-s18-p2-post-remediation-full-regression-acceptance` |
| 当前 Stage | `v0.1.4 Stage 18` |
| 当前 Phase | `V014_S18_P2_POST_REMEDIATION_FULL_REGRESSION_ACCEPTANCE completed_validated_local_only_no_go_upload_deferred` |
| 当前 Task | `KMFA-V014-S18-P2-POST-REMEDIATION-FULL-REGRESSION-ACCEPTANCE-20260712` |
| 当前验收链 | `V014_S18_P1_POST_REMEDIATION_PRECISION_STRESS -> V014_S18_P2_POST_REMEDIATION_FULL_REGRESSION_ACCEPTANCE` |
...
| Stage 上传规则 | v0.1.4 不按单个 Stage 做 GitHub upload gate；GitHub main upload 延期到 Stage 1-18 全部完成、最终整体复审通过并修复 findings 后一次性执行。当前已完成 current S18-P2：5 类 checks 全部执行、17 个 Stage review evidence 有效、Playwright 54/54 PASS、lineage full=false、raw exact=true；S18-P3、Stage 18 review、upload、reinstall、正式报告和业务动作均未执行；当前 Go/No-Go 仍为 `NO_GO`，`delivery_allowed=false`。历史 S18 产物不构成当前 active gate |
