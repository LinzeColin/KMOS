...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s05-stage-review` |
| 当前 Stage | `S05｜A0 权威项目成本黄金基准` |
| 当前 Phase | `S05-STAGE-REVIEW｜Stage 5 整体复审本地通过，下一步 Stage 5 final GitHub upload` |
| 当前 Task | `KMFA-S05-STAGE-REVIEW-20260630` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5 已完成三个 Phase 和整体复审本地验证，GitHub upload 尚未执行 |
...
当前 Stage 5 的 S05-P1、S05-P2、S05-P3 和整体复审已完成本地验证：S05-P1 登记 8 个 PDF + 1 个 Excel 的 public-safe A0 文件清单；S05-P2 生成字段合同和 45 条候选，并通过 active owner/授权降级决策将 Excel candidate 排除为 cross-source support only；S05-P3 将 40 条 PDF 字段 hash/source-anchor 记录锁定为 public-safe Q5 calculation baseline，5 条 Excel 字段不进入正式报告；Stage 5 复审证据位于 `KMFA/stage_artifacts/S05_STAGE_REVIEW/`。仍不实现 zero-delta、事实层、lineage 完整检查、报告、UI 或 GitHub upload。
