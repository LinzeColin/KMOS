...
|---|---|
| project_id | `KMFA` |
| 当前版本 | `0.1.0-s06-p2-cross-source-difference-queue` |
| 当前 Stage | `S06｜零差异校验与差异处理队列` |
| 当前 Phase | `S06-P3｜校验证据输出待开始` |
| 当前 Task | `S6PCT01-S6PCT03` |
...
| Stage 上传规则 | 中间 Phase 不上传 GitHub；Stage 5 已完成三个 Phase、整体复审和 GitHub upload；S06-P1/S06-P2 已本地验证，S06-P3 尚未开始 |
...
当前 `S06-P1｜零差异校验器` 已本地完成：`KMFA/tools/zero_delta_validator.py` 对 public-safe 已结构化整数分字段逐字段比较，任意 1 分差异失败，并输出包含来源、字段、权威值、系统值和差额的 mismatch report；证据位于 `KMFA/stage_artifacts/S06_P1_zero_delta_validator/`。当前 `S06-P2｜跨源差异队列` 也已本地完成：`KMFA/tools/cross_source_difference_queue.py` 将 public-safe PDF/Excel 同项目同字段 1 分差异送入人工差异队列，禁止自动修正、平均、四舍五入掩盖和自动选边；未关闭差异会阻断 A 级报告，证据位于 `KMFA/stage_artifacts/S06_P2_cross_source_difference_queue/`。仍不实现 S06-P3 metadata/quality 运行时证据输出、事实层、lineage 完整检查、报告或 UI。
