...
| 当前 Phase | `S12-P1 manual resolution events completed local_only_no_go_upload_deferred` |
| 当前 Task | `KMFA-V014-S12-P1-MANUAL-RESOLUTION-EVENTS-20260704` |
...
| Stage 上传规则 | v0.1.4 不按单个 Stage 做 GitHub upload gate；GitHub main upload 延期到 v1.4 Stage 1-18 全部完成、整体复审通过并修复 findings 后一次性执行。当前已完成 S12-P1：manual events=5、action kinds=4、event types=4、approved events=1、reverse events=1、HTML exports=1；只追加 public-safe 人工处理控制事件，不执行影响预览、派生重跑、Stage 12 review、GitHub upload、raw value matching、lineage full check、正式报告、UI runtime、live connector、app reinstall、OpMe 或业务执行；当前 Go/No-Go 仍为 `NO_GO`，`delivery_allowed=false` |
...
| `stage_artifacts/V014_S12_P1_MANUAL_RESOLUTION_EVENTS/` | v0.1.4 S12-P1 public-safe 人工处理事件证据包；锁定 5 条 append-only 事件、4 类动作、1 条 approved event、1 条 reverse event、1 个 HTML 工作台；GitHub upload deferred，下一步只能 S12-P2 |
