...
12. After the runner writes `screenshot_ocr_coverage.csv`, generate private local Vision OCR sidecars with `tools/generate_screenshot_ocr_sidecars.py --engine vision --apply`; empty OCR output must not be written, OCR text remains private runtime only, and generated OCR text must not be promoted into financial facts without review.
