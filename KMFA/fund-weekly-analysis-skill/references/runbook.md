...
18a. When owner correction values are reviewed outside the runner, use `tools/install_owner_decision_manifest.py --run-id <run_id>` first as a dry-run. It may write the private `ocr_fact_candidate_owner_decisions/<run_id>.json` manifest only with `--apply --acknowledge-owner-reviewed-values`, only when every `required_owner_fields` value is filled and every owner decision is `approve_for_review_authorization`. Missing values must return `BLOCKED_OWNER_VALUES_MISSING`; successful writes remain validation-only and do not write ledgers, promote facts, mutate sources, or release management conclusions.
...
26. Build `automation_readiness.csv`: compare the tracked Codex automation contract with the local automation TOML; require `Australia/Sydney` and `FREQ=DAILY;BYHOUR=11;BYMINUTE=30` for `schedule_ready=true`; keep `management_conclusion_allowed=false`.
