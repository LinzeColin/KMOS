...
* Scheduled Vision OCR must retry timeout rows with a smaller batch and longer timeout while still writing OCR text only to ignored private runtime sidecars and keeping `financial_fact_promoted=false`.
...
* The generated workbook hidden `H06_配置规则` sheet must contain the runtime rules table with `run_id`, `source_input_dir`, `schedule_rrule`, `no_hallucinated_data_policy`, `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, and `management_conclusion_allowed=false`.
* `automation_readiness.csv` must be emitted for successful output packages. It may set `schedule_ready=true` only when the tracked contract uses `timezone=Australia/Sydney`, the local automation TOML has no explicit timezone drift, and `rrule=FREQ=WEEKLY;BYHOUR=11;BYMINUTE=0;BYDAY=MO,SA`; every row must keep `management_conclusion_allowed=false`.
...
* `fact_promotion_owner_review_batch.csv` must be emitted for successful output packages and must be derived from `fact_promotion_review_packet.csv`; every row must keep `financial_fact_promotion_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
...
* `fact_promotion_execution_gate.csv` must be emitted for successful output packages. It may mark rows as `ready_for_controlled_fact_promotion_execution` only when owner authorization is valid and the review area has no blockers. Rows where owner authorization is not required must be explicit `not_required_*` no-op rows and must not count as blocked. Every row must keep `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
* `fact_promotion_execution_dry_run.csv` must be emitted for successful output packages. It may mark ready rows as `ready_for_controlled_execution_preview_no_write` and report `dry_run_impact_count`, but every row must keep `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
* `fact_promotion_execution_plan.csv` must be emitted for successful output packages. It may mark ready rows as `ready_for_owner_execution_authorization_no_write` with `required_authorization_scope=controlled_fact_promotion_execution`, but every row must keep `source_mutation_allowed=false`, `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
* `fact_promotion_execution_authorization_template.json` and `fact_promotion_execution_authorization_preview.csv` must be emitted for successful output packages. The template must default every `execution_plan_authorizations[].authorized` value to `false`, use `authorization_scope=controlled_fact_promotion_execution`, and keep all execution/write/conclusion flags false. The preview may mark valid private manifest rows as `ready_for_controlled_execution_run_no_write`, but every row must keep `source_mutation_allowed=false`, `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
* `fact_promotion_execution_apply_gate.csv` must be emitted for successful output packages. It may mark valid rows as `ready_for_controlled_execution_apply_no_write` and report `planned_apply_count`, but every row must keep `source_mutation_allowed=false`, `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
