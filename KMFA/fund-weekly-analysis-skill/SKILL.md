...
* Scheduled local runs call `tools/generate_screenshot_ocr_sidecars.py --engine vision --apply` after the runner to produce `screenshot_ocr_sidecar_generation_plan.csv`, `screenshot_ocr_sidecar_generation_summary.json`, and non-empty local Vision OCR text sidecars under private runtime only. The tool itself remains dry-run by default for manual use; scheduled `--apply` still never writes OCR text to Git-tracked paths, never modifies OneDrive source files, and never promotes financial facts. Vision OCR runs through `tools/ocr_with_vision.swift` in bounded batches using `--vision-batch-size` and per-batch `--timeout-seconds`.
