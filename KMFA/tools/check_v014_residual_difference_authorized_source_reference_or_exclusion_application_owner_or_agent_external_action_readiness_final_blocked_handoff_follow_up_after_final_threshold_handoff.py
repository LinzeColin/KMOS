#!/usr/bin/env python3
"""Validate external-action follow-up evidence after the final-threshold readiness handoff."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_readiness_final_blocked_handoff_follow_up_after_final_threshold_handoff import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_private_readiness_blocker_records_read_by_this_phase",
        "private_external_action_follow_up_diagnostic_written_by_this_phase",
        "private_external_action_follow_up_queue_written_by_this_phase",
        "private_external_action_follow_up_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_readiness_diagnostic_mutated_by_this_phase",
        "source_private_readiness_blocker_records_mutated_by_this_phase",
        "source_private_readiness_question_list_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.follow_up", summary.get("follow_up_conclusion"), FOLLOW_UP_CONCLUSION)
    ...
    _require_equal("summary.source blockers", summary.get("source_external_owner_action_blocker_count"), 48)
    _require_equal(
    ...
    _require_equal("summary.follow up ready", summary.get("external_action_follow_up_ready_count"), 0)
    _require_equal("summary.follow up blocker", summary.get("external_action_follow_up_blocker_count"), 48)
    _require_equal("summary.follow up required", summary.get("external_action_follow_up_required_count"), 48)
    _require_equal("summary.actionable owner resolution", summary.get("actionable_owner_resolution_count"), 0)
    _require_equal(
        "summary.source ref follow up",
        summary.get("source_reference_or_owner_exclusion_follow_up_count"),
        40,
    )
    _require_equal("summary.formula follow up", summary.get("formula_or_non_numeric_mapping_follow_up_count"), 8)
    _require_equal("summary.binding ready", summary.get("binding_ready_after_external_action_follow_up_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_external_action_follow_up_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_external_action_follow_up_diagnostic_written",
        "private_external_action_follow_up_queue_written",
        "private_external_action_follow_up_report_written",
        "private_external_action_follow_up_diagnostic_gitignored",
        "private_external_action_follow_up_queue_gitignored",
        "private_external_action_follow_up_report_gitignored",
        ...
        "external_action_follow_up_checked_by_this_phase",
    ):
    ...
    for code in (
        "source_readiness_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_readiness_blockers_complete",
        "source_private_readiness_records_read",
        "follow_up_queue_complete",
        "follow_up_blockers_locked",
    ...
    _require_equal("go_no_go.follow_up", go_no_go.get("follow_up_conclusion"), FOLLOW_UP_CONCLUSION)
    _require_equal("go_no_go.follow up ready", go_no_go.get("external_action_follow_up_ready_count"), 0)
    _require_equal("go_no_go.follow up blocker", go_no_go.get("external_action_follow_up_blocker_count"), 48)
    _require_equal("go_no_go.follow up required", go_no_go.get("external_action_follow_up_required_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_EXTERNAL_ACTION_FOLLOW_UP_DIAGNOSTIC_PATH)
    follow_up_rows = _read_jsonl(PRIVATE_EXTERNAL_ACTION_FOLLOW_UP_QUEUE_PATH)
    report = PRIVATE_EXTERNAL_ACTION_FOLLOW_UP_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal("private diagnostic rows", len(diagnostic.get("follow_up_rows") or []), 48)
    _require_equal("private follow up row count", len(follow_up_rows), 48)
    if "owner/授权代理 external action follow-up 队列" not in report:
        raise ValidationError("private follow-up report missing Chinese follow-up heading")
    counts = Counter(row.get("diagnostic_kind") for row in follow_up_rows)
    ...
    for row in follow_up_rows:
        _require_equal("private follow up status", row.get("external_action_follow_up_status"), FOLLOW_UP_CONCLUSION)
        _require_true("private follow up required", row.get("external_action_follow_up_required"))
        _require_false("private follow up ready", row.get("external_action_follow_up_ready"))
        _require_true("private follow up blocker", row.get("external_action_follow_up_blocker"))
        _require_false("private actionable owner resolution", row.get("actionable_owner_resolution_ready"))
        _require_false("private binding", row.get("binding_ready_after_external_action_follow_up"))
        _require_false("private retry", row.get("comparison_retry_ready_after_external_action_follow_up"))
        _require_false("private public commit", row.get("public_commit_allowed"))


def validate(*, require_private_external_action_follow_up: bool = False) -> dict[str, Any]:
    ...
    if require_private_external_action_follow_up:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-external-action-follow-up", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(require_private_external_action_follow_up=args.require_private_external_action_follow_up)
    except ValidationError as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1
    summary = manifest["summary"]
    print(
        "PASS: validated V014 owner/authorized-agent external action follow-up after final-threshold handoff "
