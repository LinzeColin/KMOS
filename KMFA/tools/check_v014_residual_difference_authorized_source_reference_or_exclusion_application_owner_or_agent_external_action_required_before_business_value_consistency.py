#!/usr/bin/env python3
"""Validate the external-action requirement gate before business-value consistency."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_required_before_business_value_consistency import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_processed_reconciliation_public_artifacts_read_by_this_phase",
        "source_private_processed_reconciliation_requirement_queue_read_by_this_phase",
        "private_business_value_consistency_requirement_diagnostic_written_by_this_phase",
        "private_business_value_consistency_requirement_queue_written_by_this_phase",
        "private_business_value_consistency_requirement_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_processed_reconciliation_requirement_diagnostic_mutated_by_this_phase",
        "source_private_processed_reconciliation_requirement_queue_mutated_by_this_phase",
        "source_private_processed_reconciliation_requirement_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_reconciliation.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_reconciliation.PHASE_ID)
    ...
    _require_equal(
        "summary.source processed reconciliation ready",
        summary.get("source_processed_data_reconciliation_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.source processed reconciliation blockers",
        summary.get("source_processed_data_reconciliation_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source processed reconciliation required",
        summary.get("source_processed_data_reconciliation_requirement_required_count"),
        48,
    )
    _require_equal(
        "summary.source private processed reconciliation rows",
        summary.get("source_private_processed_data_reconciliation_requirement_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        "summary.business consistency req ready",
        summary.get("business_value_consistency_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.business consistency req blockers",
        summary.get("business_value_consistency_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.business consistency req required",
        summary.get("business_value_consistency_requirement_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_business_value_consistency_requirement_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_business_value_consistency_requirement_count"),
        8,
    )
    _require_equal("summary.binding application ready count", summary.get("authoritative_binding_application_ready_count"), 0)
    _require_equal("summary.processed reconciliation ready count", summary.get("raw_to_processed_value_comparison_ready_count"), 0)
    _require_equal("summary.processed reconciliation ready count", summary.get("processed_data_reconciliation_ready_count"), 0)
    _require_equal("summary.business consistency ready count", summary.get("business_value_consistency_ready_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_business_value_consistency_requirement_diagnostic_written",
        "private_business_value_consistency_requirement_queue_written",
        "private_business_value_consistency_requirement_report_written",
        "private_business_value_consistency_requirement_diagnostic_gitignored",
        "private_business_value_consistency_requirement_queue_gitignored",
        "private_business_value_consistency_requirement_report_gitignored",
        "source_private_processed_data_reconciliation_requirement_diagnostic_gitignored",
        "source_private_processed_data_reconciliation_requirement_queue_gitignored",
        "source_private_processed_data_reconciliation_requirement_report_gitignored",
    ...
    for code in (
        "source_processed_reconciliation_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_processed_reconciliation_blockers_complete",
    ...
    _require_equal("go_no_go.business consistency req ready", go_no_go.get("business_value_consistency_requirement_ready_count"), 0)
    _require_equal("go_no_go.business consistency req blocker", go_no_go.get("business_value_consistency_requirement_blocker_count"), 48)
    _require_equal("go_no_go.business consistency req required", go_no_go.get("business_value_consistency_requirement_required_count"), 48)
    ...
    _require_false("go_no_go.processed reconciliation allowed", go_no_go.get("raw_to_processed_value_comparison_allowed"))
    _require_false(
        "go_no_go.processed reconciliation",
        go_no_go.get("raw_to_processed_value_comparison_performed_by_this_phase"),
    )
    _require_false("go_no_go.business consistency allowed", go_no_go.get("processed_data_reconciliation_allowed"))
    _require_false("go_no_go.business consistency", go_no_go.get("processed_data_reconciliation_performed_by_this_phase"))
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_BUSINESS_VALUE_CONSISTENCY_REQUIREMENT_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_BUSINESS_VALUE_CONSISTENCY_REQUIREMENT_QUEUE_PATH)
    report = PRIVATE_BUSINESS_VALUE_CONSISTENCY_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("business_value_consistency_requirement_rows") or []),
        48,
    )
    _require_equal("private business consistency requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 external action required before business-value consistency 队列" not in report:
        raise ValidationError("private business consistency requirement report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            "private business consistency requirement status",
            row.get("business_value_consistency_requirement_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private business consistency requirement required",
            row.get("business_value_consistency_requirement_required"),
        )
        _require_false(
            "private business consistency requirement ready",
            row.get("business_value_consistency_requirement_ready"),
        )
        _require_true(
            "private business consistency requirement blocker",
            row.get("business_value_consistency_requirement_blocker"),
        )
        ...
        _require_false("private business consistency ready", row.get("business_value_consistency_ready"))
        _require_false(
            "private business consistency verified",
            row.get("business_value_consistency_verified_by_this_phase"),
        )
        _require_false("private public commit", row.get("public_commit_allowed"))


def validate(*, require_private_business_value_consistency_requirement: bool = False) -> dict[str, Any]:
    ...
    if require_private_business_value_consistency_requirement:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-business-value-consistency-requirement", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_business_value_consistency_requirement=(
                args.require_private_business_value_consistency_requirement
            )
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent external action required before business consistency "
