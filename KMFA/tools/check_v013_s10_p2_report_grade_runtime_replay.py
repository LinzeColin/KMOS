#!/usr/bin/env python3
"""Validate KMFA v0.1.3 S10-P2 report grade runtime replay evidence."""
...
from KMFA.tools.v013_s10_p2_report_grade_runtime_replay import (
    MANIFEST_PATH,
    NEXT_REQUIRED_STEP,
    PHASE_SCOPE,
    RAW_DIR,
...
def validate_v013_s10_p2_report_grade_runtime_replay(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    ...
    require(manifest.get("phase_id") == "S10-P2", "phase_id must be S10-P2")
    require(manifest.get("task_id") == TASK_ID, "task_id mismatch")
    require(manifest.get("phase_scope") == PHASE_SCOPE, "phase scope mismatch")
    require(
        manifest.get("status")
        == "completed_validated_local_only_no_go_upload_deferred_report_grade_runtime_replayed",
        "status mismatch",
    )
    require(manifest.get("completed_task_ids") == ["S10PBT01", "S10PBT02", "S10PBT03"], "completed tasks mismatch")
    require(
        manifest.get("acceptance_ids") == ["ACC-V013-S10-P2-REPORT-GRADE-RUNTIME-REPLAY"],
        "acceptance ids mismatch",
    )
    require(manifest.get("s10_p1_dependency_validated") is True, "S10-P1 dependency flag mismatch")
    require(s10_p1.get("phase_id") == "S10-P1", "S10-P1 dependency did not validate")
    require(s10_p1.get("s10_p2_performed") is False, "S10-P1 dependency scope mismatch")
    require(manifest.get("legacy_s10_p2_dependency_validated") is True, "legacy S10-P2 dependency flag mismatch")
    ...
    summary = manifest.get("legacy_s10_p2_summary", {})
    expected_summary = {
        "template_count": 2,
    ...
    require(summary.get("computed_grades") == ["D"], "computed grades mismatch")
    require(summary.get("release_permissions") == ["blocked_decision_use"], "release permissions mismatch")
    ...
    for block in (
        ...
        require(hard_block_counts.get(block) == 2, f"legacy hard block count mismatch for {block}")
    require(summary.get("hard_block_count") == 8, "legacy hard block count mismatch")

    policy = manifest.get("report_grade_runtime_policy", {})
    require(policy.get("allowed_report_grades") == ["A", "B", "C", "D"], "allowed grades mismatch")
    require(policy.get("current_report_grade") == "D", "current report grade mismatch")
    require(policy.get("current_data_quality_grade") == "Q4", "current data quality grade mismatch")
    for field_name in (
    ...
    require(policy.get("complete_trusted_report_display_allowed") is False, "complete trusted display must be false")
    require(policy.get("full_trusted_report_allowed") is False, "full trusted report must be false")
    ...
    require(policy.get("s10_p3_export_allowed") is False, "S10-P3 export must be false")
    require(policy.get("report_runtime_scope_count") == 2, "report runtime scope count mismatch")
    ...
    require(phase_boundaries.get("s10_p2_report_grade_runtime_scope_included") is True, "S10-P2 scope flag mismatch")
    for key in (
    ...
    return {
        ...
        "s10_p3_export_scope_included": phase_boundaries["s10_p3_report_export_scope_included"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.3 S10-P2 report grade runtime replay.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH)
    args = parser.parse_args(argv)
    try:
        result = validate_v013_s10_p2_report_grade_runtime_replay(args.manifest)
    except (OSError, json.JSONDecodeError, ValidationError, RuntimeError) as exc:
        print(f"FAIL: KMFA v0.1.3 S10-P2 report grade runtime replay validation failed ({exc})")
        return 1

    print(
        "PASS: KMFA v0.1.3 S10-P2 report grade runtime replay validated "
