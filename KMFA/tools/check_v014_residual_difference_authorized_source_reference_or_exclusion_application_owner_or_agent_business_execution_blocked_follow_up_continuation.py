#!/usr/bin/env python3
"""Validate the business-execution blocked follow-up continuation."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_business_execution_blocked_follow_up_continuation import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_business_execution_blocked_follow_up_public_artifacts_read_by_this_phase",
        "source_private_business_execution_blocked_follow_up_queue_read_by_this_phase",
        "private_business_execution_blocked_follow_up_continuation_diagnostic_written_by_this_phase",
        "private_business_execution_blocked_follow_up_continuation_queue_written_by_this_phase",
        "private_business_execution_blocked_follow_up_continuation_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_business_execution_blocked_follow_up_diagnostic_mutated_by_this_phase",
        "source_private_business_execution_blocked_follow_up_queue_mutated_by_this_phase",
        "source_private_business_execution_blocked_follow_up_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_blocked_follow_up.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_blocked_follow_up.PHASE_ID)
    ...
    _require_equal(
        ...
        summary.get("source_business_execution_blocked_follow_up_ready_count"),
        0,
    )
    _require_equal(
        "summary.source business execution blocked follow-up blockers",
        summary.get("source_business_execution_blocked_follow_up_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source business execution blocked follow-up items",
        summary.get("source_business_execution_blocked_follow_up_item_count"),
        48,
    )
    _require_equal(
        "summary.source business execution blocked follow-up required",
        summary.get("source_business_execution_blocked_follow_up_required_count"),
        48,
    )
    _require_equal(
        "summary.source private blocked follow-up rows",
        summary.get("source_private_business_execution_blocked_follow_up_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("business_execution_blocked_follow_up_continuation_ready_count"),
        0,
    )
    _require_equal(
        "summary.business execution req blockers",
        summary.get("business_execution_blocked_follow_up_continuation_blocker_count"),
        48,
    )
    _require_equal(
        "summary.business execution req items",
        summary.get("business_execution_blocked_follow_up_continuation_item_count"),
        48,
    )
    _require_equal(
        "summary.business execution req required",
        summary.get("business_execution_blocked_follow_up_continuation_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_business_execution_blocked_follow_up_continuation_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_business_execution_blocked_follow_up_continuation_count"),
        8,
    )
    ...
    for key in (
        "private_business_execution_blocked_follow_up_continuation_diagnostic_written",
        "private_business_execution_blocked_follow_up_continuation_queue_written",
        "private_business_execution_blocked_follow_up_continuation_report_written",
        "private_business_execution_blocked_follow_up_continuation_diagnostic_gitignored",
        "private_business_execution_blocked_follow_up_continuation_queue_gitignored",
        "private_business_execution_blocked_follow_up_continuation_report_gitignored",
        "source_private_business_execution_blocked_follow_up_diagnostic_gitignored",
        "source_private_business_execution_blocked_follow_up_queue_gitignored",
        "source_private_business_execution_blocked_follow_up_report_gitignored",
    ...
    for code in (
        "source_business_execution_blocked_follow_up_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_business_execution_blocked_follow_up_blockers_complete",
    ...
    _require_equal("go_no_go.business execution req ready", go_no_go.get("business_execution_blocked_follow_up_continuation_ready_count"), 0)
    _require_equal("go_no_go.business execution req blocker", go_no_go.get("business_execution_blocked_follow_up_continuation_blocker_count"), 48)
    _require_equal("go_no_go.business execution req items", go_no_go.get("business_execution_blocked_follow_up_continuation_item_count"), 48)
    _require_equal("go_no_go.business execution req required", go_no_go.get("business_execution_blocked_follow_up_continuation_required_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_BUSINESS_EXECUTION_BLOCKED_FOLLOW_UP_CONTINUATION_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_BUSINESS_EXECUTION_BLOCKED_FOLLOW_UP_CONTINUATION_QUEUE_PATH)
    report = PRIVATE_BUSINESS_EXECUTION_BLOCKED_FOLLOW_UP_CONTINUATION_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("business_execution_blocked_follow_up_continuation_rows") or []),
        48,
    )
    _require_equal("private business execution requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 business execution blocked follow-up continuation 队列" not in report:
        raise ValidationError("private business execution blocked follow-up continuation report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            ...
            row.get("business_execution_blocked_follow_up_continuation_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private business execution requirement required",
            row.get("business_execution_blocked_follow_up_continuation_required"),
        )
        _require_false(
            "private business execution requirement ready",
            row.get("business_execution_blocked_follow_up_continuation_ready"),
        )
        _require_true(
            "private business execution requirement blocker",
            row.get("business_execution_blocked_follow_up_continuation_blocker"),
        )
...
def validate(*, require_private_business_execution_blocked_follow_up_continuation: bool = False) -> dict[str, Any]:
    ...
    if require_private_business_execution_blocked_follow_up_continuation:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="require_private_business_execution_blocked_follow_up_continuation",
    )
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_business_execution_blocked_follow_up_continuation=args.require_private_business_execution_blocked_follow_up_continuation
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent business execution blocked follow-up continuation "
