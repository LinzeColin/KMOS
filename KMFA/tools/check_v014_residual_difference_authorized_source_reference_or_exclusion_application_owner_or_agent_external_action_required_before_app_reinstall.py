#!/usr/bin/env python3
"""Validate the external-action requirement gate before app reinstall."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_required_before_app_reinstall import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_github_upload_public_artifacts_read_by_this_phase",
        "source_private_github_upload_requirement_queue_read_by_this_phase",
        "private_app_reinstall_requirement_diagnostic_written_by_this_phase",
        "private_app_reinstall_requirement_queue_written_by_this_phase",
        "private_app_reinstall_requirement_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_github_upload_requirement_diagnostic_mutated_by_this_phase",
        "source_private_github_upload_requirement_queue_mutated_by_this_phase",
        "source_private_github_upload_requirement_report_mutated_by_this_phase",
        ...
        "app_reinstall_ready_by_this_phase",
        "app_reinstall_allowed_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_github_upload.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_github_upload.PHASE_ID)
    ...
    _require_equal(
        ...
        summary.get("source_github_upload_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.source lineage full check blockers",
        summary.get("source_github_upload_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source lineage full check required",
        summary.get("source_github_upload_requirement_required_count"),
        48,
    )
    _require_equal(
        "summary.source private lineage full check rows",
        summary.get("source_private_github_upload_requirement_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        "summary.app reinstall req ready",
        summary.get("app_reinstall_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.app reinstall req blockers",
        summary.get("app_reinstall_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.app reinstall req required",
        summary.get("app_reinstall_requirement_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_app_reinstall_requirement_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_app_reinstall_requirement_count"),
        8,
    )
    ...
    _require_equal("summary.app reinstall ready count", summary.get("app_reinstall_ready_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_app_reinstall_requirement_diagnostic_written",
        "private_app_reinstall_requirement_queue_written",
        "private_app_reinstall_requirement_report_written",
        "private_app_reinstall_requirement_diagnostic_gitignored",
        "private_app_reinstall_requirement_queue_gitignored",
        "private_app_reinstall_requirement_report_gitignored",
        "source_private_github_upload_requirement_diagnostic_gitignored",
        "source_private_github_upload_requirement_queue_gitignored",
        "source_private_github_upload_requirement_report_gitignored",
    ...
    for code in (
        "source_github_upload_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_github_upload_blockers_complete",
    ...
    _require_equal("go_no_go.app reinstall req ready", go_no_go.get("app_reinstall_requirement_ready_count"), 0)
    _require_equal("go_no_go.app reinstall req blocker", go_no_go.get("app_reinstall_requirement_blocker_count"), 48)
    _require_equal("go_no_go.app reinstall req required", go_no_go.get("app_reinstall_requirement_required_count"), 48)
    ...
    _require_false("go_no_go.github upload ready", go_no_go.get("github_upload_ready"))
    _require_false("go_no_go.github upload allowed", go_no_go.get("github_upload_allowed"))
    _require_false("go_no_go.github upload performed", go_no_go.get("github_upload_performed_by_this_phase"))
    _require_false("go_no_go.github upload performed locked", go_no_go.get("github_upload_performed"))
    ...
    _require_false("go_no_go.business execution performed", go_no_go.get("business_execution_performed"))
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_APP_REINSTALL_REQUIREMENT_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_APP_REINSTALL_REQUIREMENT_QUEUE_PATH)
    report = PRIVATE_APP_REINSTALL_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("app_reinstall_requirement_rows") or []),
        48,
    )
    _require_equal("private app reinstall requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 external action required before app reinstall 队列" not in report:
        raise ValidationError("private app reinstall requirement report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            "private app reinstall requirement status",
            row.get("app_reinstall_requirement_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private app reinstall requirement required",
            row.get("app_reinstall_requirement_required"),
        )
        _require_false(
            "private app reinstall requirement ready",
            row.get("app_reinstall_requirement_ready"),
        )
        _require_true(
            "private app reinstall requirement blocker",
            row.get("app_reinstall_requirement_blocker"),
        )
...
def validate(*, require_private_app_reinstall_requirement: bool = False) -> dict[str, Any]:
    ...
    if require_private_app_reinstall_requirement:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-app-reinstall-requirement", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_app_reinstall_requirement=args.require_private_app_reinstall_requirement
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent external action required before app reinstall "
