#!/usr/bin/env python3
"""Validate owner or authorized-agent action readiness after blocked handoff."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_action_readiness_after_blocked_handoff import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "private_action_readiness_diagnostic_written_by_this_phase",
        "private_action_readiness_blocker_records_written_by_this_phase",
        "private_action_readiness_question_list_written_by_this_phase",
    )
    false_keys = (
...
def _check_summary(
    ...
    _require_equal("summary.source owner action", summary.get("source_owner_action_item_count"), 48)
    ...
    _require_true("summary.source threshold", summary.get("source_actionability_blocked_audit_threshold_met"))
    _require_equal("summary.goal", summary.get("goal_status_recommendation"), "blocked")
    _require_equal("summary.owner ready", summary.get("owner_action_ready_count"), 0)
    _require_equal("summary.owner blocker", summary.get("owner_action_blocker_count"), 48)
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_owner_action_blocker_count"),
        40,
    )
    _require_equal(
        "summary.formula blockers",
        summary.get("formula_or_non_numeric_mapping_owner_action_blocker_count"),
        8,
    )
    _require_equal("summary.binding ready", summary.get("binding_ready_after_owner_action_readiness_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_owner_action_readiness_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_action_readiness_diagnostic_written",
        "private_action_readiness_blocker_records_written",
        "private_action_readiness_question_list_written",
        "private_action_readiness_diagnostic_gitignored",
        "private_action_readiness_blocker_records_gitignored",
        "private_action_readiness_question_list_gitignored",
        ...
        "owner_action_readiness_checked_by_this_phase",
    ):
    ...
    for code in (
        ...
        "owner_action_readiness_checked",
        "owner_action_blockers_locked",
    ...
    _require_equal("go_no_go.owner ready", go_no_go.get("owner_action_ready_count"), 0)
    _require_equal("go_no_go.owner blocker", go_no_go.get("owner_action_blocker_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_ACTION_READINESS_DIAGNOSTIC_PATH)
    blocker_rows = _read_jsonl(PRIVATE_ACTION_READINESS_BLOCKER_RECORDS_PATH)
    question_list = PRIVATE_ACTION_READINESS_QUESTION_LIST_PATH.read_text(encoding="utf-8")
    ...
    if "owner/授权代理 action readiness 问题清单" not in question_list:
    ...
    for row in blocker_rows:
        _require_equal("private readiness status", row.get("owner_action_readiness_status"), READINESS_CONCLUSION)
        _require_false("private owner action ready", row.get("owner_action_ready"))
        _require_true("private owner action blocker", row.get("owner_action_blocker"))
        _require_false("private actionable owner resolution", row.get("actionable_owner_resolution_ready"))
        _require_false("private binding", row.get("binding_ready_after_owner_action_readiness"))
        _require_false("private retry", row.get("comparison_retry_ready_after_owner_action_readiness"))
        _require_false("private public commit", row.get("public_commit_allowed"))


def validate(*, require_private_action_readiness: bool = False) -> dict[str, Any]:
    ...
    if require_private_action_readiness:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-action-readiness", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(require_private_action_readiness=args.require_private_action_readiness)
    except ValidationError as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1
    summary = manifest["summary"]
    print(
        "PASS: validated V014 owner/authorized-agent action readiness after blocked handoff "
