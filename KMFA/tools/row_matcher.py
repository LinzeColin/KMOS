#!/usr/bin/env python3
"""行级匹配引擎 v2（GOVX.0003 完整版第一件：回款台账行 ↔ 银行收款行）。

匹配规则（银行行每行至多用一次）：
  P1 金额精确 + 同日（一对一贪心）
  P2 金额精确 + 日期窗 ±5 天（一对一贪心，取最近）
  P3 聚合匹配：同日同账户 2-5 行台账合计 = 一笔银行入账（±3 天）——批量入账形态
...
def main() -> int:
    ...
    for src, sheet, ridx, ldate, cents in ledger:
        ...
        if best is not None:
            best[1][3] = True
            matches.append((src, sheet, ridx, ldate, cents, best[1][1], best[1][0], best[1][2], best[0]))
            p1 += best[0] == 0
    ...
    con.execute("""CREATE TABLE _staging.row_matches (
        ...
        b_rowid BIGINT, b_date DATE, b_category VARCHAR, day_gap INTEGER, match_kind VARCHAR)""")
    con.executemany("INSERT INTO _staging.row_matches VALUES (?,?,?,?,?,?,?,?,?,'one2one')", matches)

    # P3 聚合匹配：未匹配台账按（日期,账户）分组，组内 2-5 行合计 = 未匹配银行行（±3 天）
    from itertools import combinations
    matched_keys = {(m[0], m[1], m[2]) for m in matches}
    used_bank = {m[5] for m in matches}
    groups = defaultdict(list)
    for src, sheet, ridx, ldate, cents in ledger:
        if (src, sheet, ridx) not in matched_keys:
            groups[ldate].append((src, sheet, ridx, ldate, cents))
    bank_free = defaultdict(list)
    for rid, jdate, cents, cat in bank:
        if rid not in used_bank:
            bank_free[cents].append((rid, jdate, cat))
    agg_rows, agg_deposits = [], 0
    for ldate, rows in groups.items():
        while len(rows) >= 2:
            hit = None
            n = min(len(rows), 8)
            for k in range(2, min(n, 5) + 1):
                for combo in combinations(rows[:n], k):
                    s = sum(c[4] for c in combo)
                    for b in bank_free.get(s, []):
                        if abs((b[1] - ldate).days) <= 3:
                            hit = (combo, b, s)
                            break
                    if hit:
                        break
            ...
            if not hit:
                break
            combo, b, s = hit
            bank_free[s].remove(b)
            agg_deposits += 1
            for c in combo:
                agg_rows.append((c[0], c[1], c[2], str(c[3]), c[4], b[0], str(b[1]), b[2],
                                 abs((b[1] - c[3]).days), "aggregate"))
                rows.remove(c)
    con.executemany("INSERT INTO _staging.row_matches VALUES (?,?,?,?,?,?,?,?,?,?)", agg_rows)

    total_l = len(ledger)
    matched = len(matches) + len(agg_rows)
    cat_dist = Counter([m[7] for m in matches] + [a[7] for a in agg_rows])
    month_stats = defaultdict(lambda: [0, 0])
    for m in list(matches) + [(a[0],a[1],a[2],a[3][:10],*a[4:]) for a in agg_rows]:
        month_stats[str(m[3])[:7]][0] += 1
    for src, sheet, ridx, ldate, cents in ledger:
    ...
    out = {
        "task_id": "GOVX.0003-full/row-matcher-v2", "date": "2026-07-17", "scope": "collection↔bank 2025",
        "ledger_rows": total_l, "matched": matched, "match_rate": round(matched / total_l, 4),
        "matched_same_day": p1, "matched_window": len(matches) - p1,
        "matched_aggregate_rows": len(agg_rows), "aggregate_deposits_explained": agg_deposits,
