...
PRIVATE_DIR = PROJECT_ROOT / ".codex_private_runtime/v014_processed_value_source_map_completion_owner_confirmation_response_packet"
PRIVATE_RESPONSE_PACKET_PATH = PRIVATE_DIR / "private_owner_confirmation_response_packet.json"
PRIVATE_RESPONSE_DRAFT_PATH = PRIVATE_DIR / "private_owner_confirmation_response_draft.json"
PRIVATE_RESPONSE_MARKDOWN_PATH = PRIVATE_DIR / "private_owner_confirmation_response_packet.md"
PRIVATE_DIAGNOSTIC_PATH = PRIVATE_DIR / "private_owner_confirmation_response_packet_diagnostic.json"
...
PUBLIC_ARTIFACTS = [
    ...
    MANIFEST_PATH,
    GO_NO_GO_PATH,
    HUMAN_DIR / "processed_value_source_map_completion_owner_confirmation_response_packet_report.md",
    HUMAN_DIR / "go_no_go_record.md",
    HUMAN_DIR / "test_results.md",
    HUMAN_DIR / "risk_register.md",
    HUMAN_DIR / "rollback_plan.md",
    QUALITY_DIR / "v014_owner_confirmation_response_packet_summary.json",
    QUALITY_DIR / "v014_owner_confirmation_response_packet_manifest.json",
    QUALITY_DIR / "v014_owner_confirmation_response_packet_go_no_go_report.json",
]
...
def _check_public_safety(safety: dict[str, Any]) -> None:
    ...
    for key in (
        ...
        "private_response_packet_committed",
        "private_response_draft_committed",
        "private_response_markdown_committed",
...
def _check_private_owner_confirmation_packet() -> None:
    ...
    markdown = PRIVATE_RESPONSE_MARKDOWN_PATH.read_text(encoding="utf-8")
    ...
    _require_equal("private.source_pending_owner_response_item_count", packet_summary.get("source_pending_owner_response_item_count"), 3)
    _require_equal("private.source_pending_owner_response_target_slot_count", packet_summary.get("source_pending_owner_response_target_slot_count"), 12)
    _require_equal("private.source_owner_response_applied_item_count", packet_summary.get("source_owner_response_applied_item_count"), 0)
    _require_equal("private.response_packet_item_count", packet_summary.get("response_packet_item_count"), 3)
    _require_equal("private.response_packet_target_slot_count", packet_summary.get("response_packet_target_slot_count"), 12)
    _require_equal("private.response_draft_item_count", packet_summary.get("response_draft_item_count"), 3)
    _require_equal("private.missing_required_field_counts", packet_summary.get("missing_required_field_counts"), EXPECTED_MISSING_FIELD_COUNTS)
    _require_equal("private.recommendation_counts", packet_summary.get("recommendation_counts"), EXPECTED_RECOMMENDATION_COUNTS)
    _require_equal("private.candidate_status_group_counts", packet_summary.get("candidate_status_group_counts"), EXPECTED_STATUS_GROUP_COUNTS)
    _require_true("private.packet_ready", packet_summary.get("owner_confirmation_response_packet_ready"))
    _require_false("private.response_supplied", packet_summary.get("owner_confirmation_response_supplied"))
    _require_false("private.active_ready", packet_summary.get("active_owner_authorized_fill_record_ready"))
    _require_false("private.reapplication_ready", packet_summary.get("source_map_completion_reapplication_ready"))
    _require_false("private.business_consistency", packet_summary.get("business_value_consistency_verified"))
    response_items = packet.get("response_items", [])
    draft_items = draft.get("items", [])
    if not isinstance(response_items, list) or not isinstance(draft_items, list):
        raise ValidationError("private response packet lists must be lists")
    _require_equal("private.response_items length", len(response_items), 3)
    _require_equal("private.draft_items length", len(draft_items), 3)
    target_total = sum(int(item.get("target_slot_count") or 0) for item in response_items if isinstance(item, dict))
    _require_equal("private.response target total", target_total, 12)
    if "待回答项: 3" not in markdown or "当前状态: 未收到 owner/授权人响应" not in markdown:
        raise ValidationError("private markdown packet does not contain expected Chinese packet markers")
    for path in (PRIVATE_RESPONSE_PACKET_PATH, PRIVATE_RESPONSE_DRAFT_PATH, PRIVATE_RESPONSE_MARKDOWN_PATH, PRIVATE_DIAGNOSTIC_PATH):
        _check_private_file_ignored_and_untracked(path)


def validate(*, require_private_owner_confirmation_packet: bool = False) -> dict[str, Any]:
    summary = _read_json(SUMMARY_PATH)
    manifest = _read_json(MANIFEST_PATH)
    go_no_go = _read_json(GO_NO_GO_PATH)
    metadata_summary = _read_json(QUALITY_DIR / "v014_owner_confirmation_response_packet_summary.json")
    metadata_manifest = _read_json(QUALITY_DIR / "v014_owner_confirmation_response_packet_manifest.json")
    metadata_go_no_go = _read_json(QUALITY_DIR / "v014_owner_confirmation_response_packet_go_no_go_report.json")
    ...
    _require_equal("summary.source_owner_response_application_decision", summary.get("source_owner_response_application_decision"), "NO_GO")
    _require_equal("summary.source_pending_owner_response_item_count", summary.get("source_pending_owner_response_item_count"), 3)
    _require_equal("summary.source_pending_owner_response_target_slot_count", summary.get("source_pending_owner_response_target_slot_count"), 12)
    _require_equal("summary.source_owner_response_applied_item_count", summary.get("source_owner_response_applied_item_count"), 0)
    _require_equal("summary.response_packet_item_count", summary.get("response_packet_item_count"), 3)
    _require_equal("summary.response_packet_target_slot_count", summary.get("response_packet_target_slot_count"), 12)
    ...
    for key in (
        "owner_confirmation_response_packet_ready",
        "private_response_packet_written",
        "private_response_packet_gitignored",
        "private_response_draft_written",
        "private_response_draft_gitignored",
        "private_response_markdown_written",
        "private_response_markdown_gitignored",
    ...
    _require_equal("go_no_go.response_packet_item_count", go_no_go.get("response_packet_item_count"), 3)
    _require_equal("go_no_go.response_packet_target_slot_count", go_no_go.get("response_packet_target_slot_count"), 12)
    ...
    if require_private_owner_confirmation_packet:
        _check_private_owner_confirmation_packet()
    return manifest


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--require-private-owner-confirmation-packet", action="store_true")
    args = parser.parse_args()
    manifest = validate(require_private_owner_confirmation_packet=args.require_private_owner_confirmation_packet)
    print(
        "PASS: KMFA v0.1.4 owner confirmation response packet validated "
