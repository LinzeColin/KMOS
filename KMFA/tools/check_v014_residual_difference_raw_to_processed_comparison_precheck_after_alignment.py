#!/usr/bin/env python3
"""Validate KMFA v0.1.4 post-alignment residual comparison precheck."""
...
from KMFA.tools.v014_residual_difference_raw_to_processed_comparison_precheck_after_alignment import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_alignment_summary_read_by_this_phase",
        "source_public_alignment_manifest_read_by_this_phase",
        "source_public_alignment_matrix_read_by_this_phase",
        "source_private_alignment_read_by_this_phase",
        "source_private_alignment_diagnostic_read_by_this_phase",
        "source_private_alignment_items_read_by_this_phase",
    ...
    _require_false(
        "raw_boundary.source_private_alignment_outputs_mutated_by_this_phase",
        raw_boundary.get("source_private_alignment_outputs_mutated_by_this_phase"),
    )
    _require_false(
...
def _check_summary(summary: dict[str, Any], matrix: dict[str, Any], go_no_go: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    _require_equal("summary.source alignment count", summary.get("source_alignment_item_count"), 72)
    _require_equal("summary.source anchor count", summary.get("source_raw_candidate_anchor_draft_item_count"), 72)
    _require_equal("summary.owner anchor count", summary.get("owner_authorized_comparison_anchor_count"), 0)
    _require_equal("summary.anchor required count", summary.get("alignment_authorization_anchor_required_count"), 72)
    ...
    _require_true(
        "summary.precheck after alignment performed",
        summary.get("raw_to_processed_value_comparison_precheck_after_alignment_performed_by_this_phase"),
    )
    _require_false(
        "summary.precheck after alignment passed",
        summary.get("raw_to_processed_value_comparison_precheck_after_alignment_passed"),
    )
    for key in (
    ...
    _require_equal("matrix.check count", matrix.get("check_count"), 8)
    _require_equal("matrix.pass count", matrix.get("pass_count"), 6)
    _require_equal("matrix.fail count", matrix.get("fail_count"), 2)
    ...
    _require_equal("matrix.owner anchor count", matrix.get("owner_authorized_comparison_anchor_count"), 0)
...
def _check_private_precheck(summary: dict[str, Any]) -> None:
    ...
    for row in blockers:
        _require_equal("blocker status", row.get("comparison_precheck_status"), "missing_owner_authorized_comparison_anchor")
        _require_false("blocker owner authorized anchor", row.get("owner_authorized_anchor"))
...
def main() -> int:
    ...
    print(
        "PASS: KMFA v0.1.4 post-alignment residual raw comparison precheck artifacts validated "
