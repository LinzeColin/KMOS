#!/usr/bin/env python3
"""Validate owner/agent diagnostic blocker threshold recheck after readiness recheck."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_diagnostic_blocker_threshold_recheck_after_readiness_recheck import (  # noqa: E402
    ACCEPTANCE_ID,
    AUDIT_CONCLUSION,
    DECISION,
...
FORBIDDEN_PUBLIC_PATTERNS = [
    ...
    re.compile(
        ...
        + source_audit.source_readiness.source_intake.source_packet.PRIVATE_SLOT_KEY
        + r')"',
        re.IGNORECASE,
    ),
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_audit_summary_read_by_this_phase",
        "source_public_audit_manifest_read_by_this_phase",
        "source_public_audit_go_no_go_read_by_this_phase",
        "source_public_audit_matrix_read_by_this_phase",
        "source_private_audit_diagnostic_read_by_this_phase",
        "source_private_audit_queue_read_by_this_phase",
        "source_private_audit_report_read_by_this_phase",
    ...
    for key in (
        "source_private_audit_diagnostic_mutated_by_this_phase",
        "source_private_audit_queue_mutated_by_this_phase",
        "source_private_audit_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source private diagnostic phase", summary.get("source_private_diagnostic_phase_id"), source_audit.PHASE_ID)
    ...
    _require_equal("summary.source audit queue", summary.get("source_private_audit_queue_item_count"), 48)
    ...
    _require_equal("summary.threshold records", summary.get("private_threshold_records_item_count"), 48)
    ...
    _require_equal("summary.binding ready", summary.get("binding_ready_after_blocker_threshold_recheck_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_blocker_threshold_recheck_count"), 0)
    ...
    for key in (
        ...
        "source_private_audit_diagnostic_gitignored",
        "source_private_audit_queue_gitignored",
        "source_private_audit_report_gitignored",
    ):
    ...
    _require_equal("go_no_go.diagnostic_blocker_observation_count", go_no_go.get("diagnostic_blocker_observation_count"), 3)
...
def _check_private_artifacts() -> None:
    ...
    _require_equal("source private audit rows", len(source_rows), 48)
    _require_equal("private threshold rows", len(threshold_rows), 48)
    ...
    for row in threshold_rows:
        _require_equal("private threshold blocker_code", row.get("blocker_code"), "missing_valid_owner_or_agent_diagnostic_response")
        _require_equal("private threshold status", row.get("threshold_status"), "blocked_threshold_met")
        _require_equal("private threshold prior observation", row.get("prior_diagnostic_blocker_observation_count"), 2)
        _require_equal("private threshold current observation", row.get("diagnostic_blocker_observation_count"), 3)
        _require_true("private threshold met", row.get("diagnostic_blocked_audit_threshold_met"))
        _require_false("private threshold valid response", row.get("valid_diagnostic_response"))
        _require_false("private threshold actionable", row.get("actionable_resolution_ready"))
        _require_false("private threshold public commit", row.get("public_commit_allowed"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("PASS: validated V014 authorized source reference or exclusion application owner/agent diagnostic blocker threshold recheck after readiness recheck")
    return 0
