...
def _validate_html(errors: list[str]) -> None:
    ...
    _require(text.count("data-stage-link=") == 1, "HTML stage link count mismatch", errors)
    _require(phase.P3_HREF in text, "HTML S14-P3 href missing", errors)
    _require((phase.HTML_PATH.parent / phase.P3_HREF).resolve().is_file(), "HTML S14-P3 target missing", errors)
    _require("Stage 14 三个 phase 均已完成" in text, "current Stage 14 status missing", errors)
    _require("本 phase 仅完成 S14-P2" not in text, "stale S14-P2 status remains", errors)
