#!/usr/bin/env python3
"""Validate KMFA v0.1.4 S04-P2 field standardization evidence."""
...
from KMFA.tools.v014_s04_p2_field_standardization import (
...
def validate_v014_s04_p2_field_standardization(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    ...
    require(manifest.get("phase_id") == "S04-P2", "phase_id must be S04-P2", errors)
    require(
        manifest.get("phase_scope") == "v014_s04_p2_field_standardization_only",
        "phase scope mismatch",
        errors,
    )
    require(manifest.get("task_id") == TASK_ID, "task_id mismatch", errors)
    require(manifest.get("acceptance_id") == "ACC-V014-S04-P2-FIELD-STANDARDIZATION", "acceptance id mismatch", errors)
    require(manifest.get("status") == "completed_validated_local_only_no_go_upload_deferred", "status mismatch", errors)
    require(manifest.get("completed_task_ids") == ["S4PBT01", "S4PBT02", "S4PBT03"], "completed task ids mismatch", errors)
    ...
    require(manifest.get("s04_p1_dependency_validated") is True, "S04-P1 dependency flag must be true", errors)

    require(manifest.get("field_standardization_dependency_validated") is True, "field dependency must be true", errors)
    require(manifest.get("field_standardization_performed") is True, "field standardization must be true", errors)
    require(manifest.get("field_alias_mapping_performed") is True, "field alias mapping must be true", errors)
    require(manifest.get("canonical_fields") == replay["canonical_fields"], "canonical fields mismatch", errors)
    require(manifest.get("canonical_field_count") == replay["canonical_field_count"] == 6, "canonical field count mismatch", errors)
    require(
        manifest.get("alias_dictionary_row_count") == replay["alias_dictionary_row_count"],
        "alias dictionary row count mismatch",
        errors,
    )
    require(manifest.get("alias_dictionary_row_count", 0) >= 30, "alias dictionary row count too low", errors)
    require(manifest.get("mapping_record_count") == replay["mapping_record_count"] == 6, "mapping record count mismatch", errors)
    require(manifest.get("mapping_records_public_safe") is True, "mapping records must be public-safe", errors)
    require(manifest.get("standardization_case_count") == replay["standardization_case_count"] == 6, "case count mismatch", errors)
    require(
    ...
    require(manifest.get("quality_status_count") == replay["quality_status_count"] == 5, "quality status count mismatch", errors)
    require(
        manifest.get("quality_status_issue_types") == ["invalid_field_value", "missing_required_field"],
        "quality status issue type mismatch",
        errors,
    )
    require(manifest.get("quality_statuses_public_safe") is True, "quality statuses must be public-safe", errors)
    require(manifest.get("quality_passed_for_complete_synthetic_row") is True, "complete synthetic row must pass", errors)
    require(manifest.get("quality_passed_for_incomplete_synthetic_row") is False, "incomplete synthetic row must fail", errors)
    ...
    require("S04-P3" in manifest.get("next_required_step", ""), "next step must point to S04-P3", errors)
    ...
    for ref in manifest.get("field_standardization_refs", []):
        require(Path(ref).exists(), f"missing field standardization ref: {ref}", errors)
    for ref in manifest.get("evidence_refs", []):
        path = Path(ref)
        require(path.exists(), f"missing evidence ref: {ref}", errors)
        require(path.suffix.lower() not in FORBIDDEN_PUBLIC_SUFFIXES, f"forbidden evidence extension: {ref}", errors)
        if path.suffix.lower() in {".md", ".json", ".jsonl", ".csv", ".yaml", ".yml"}:
            check_public_safe_file(path, errors)
    for ref in (ALIAS_DICTIONARY_PATH, FIELD_POLICY_PATH, FIELD_QUALITY_PROTOCOL_PATH):
        check_public_safe_file(ref, errors)
    ...
    return {
        ...
        "mapping_record_count": manifest["mapping_record_count"],
        ...
        "field_standardization_performed": manifest["field_standardization_performed"],
        "field_alias_mapping_performed": manifest["field_alias_mapping_performed"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.4 S04-P2 field standardization evidence.")
    parser.add_argument("--manifest", default=str(MANIFEST_PATH))
    args = parser.parse_args(argv)
    try:
        result = validate_v014_s04_p2_field_standardization(Path(args.manifest))
    except ValidationError as exc:
        print("FAIL: KMFA v0.1.4 S04-P2 field standardization validation failed")
        print(exc)
        return 1
    print(
        "PASS: KMFA v0.1.4 S04-P2 field standardization validator passed "
