#!/usr/bin/env python3
"""Validate generated owner/agent diagnostic response actionability blocker audit."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_generated_diagnostic_response_actionability_blocker_audit import (  # noqa: E402
    ACCEPTANCE_ID,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_actionability_summary_read_by_this_phase",
        "source_public_actionability_manifest_read_by_this_phase",
        "source_public_actionability_go_no_go_read_by_this_phase",
        "source_public_actionability_matrix_read_by_this_phase",
        "source_private_actionability_diagnostic_read_by_this_phase",
        "source_private_actionability_blocker_queue_read_by_this_phase",
        "source_private_actionability_report_existence_checked_by_this_phase",
        "private_actionability_blocker_audit_diagnostic_written_by_this_phase",
        "private_actionability_blocker_audit_queue_written_by_this_phase",
        "private_actionability_blocker_audit_report_written_by_this_phase",
    ):
    ...
    for key in (
        ...
        "source_private_actionability_diagnostic_mutated_by_this_phase",
        "source_private_actionability_blocker_queue_mutated_by_this_phase",
        "source_private_actionability_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal(
        "summary.actionability_audit_conclusion",
        summary.get("actionability_audit_conclusion"),
        ACTIONABILITY_AUDIT_CONCLUSION,
    )
    _require_equal("summary.source_phase_id", summary.get("source_phase_id"), source_recheck.PHASE_ID)
    _require_equal("summary.source_manifest_phase_id", summary.get("source_manifest_phase_id"), source_recheck.PHASE_ID)
    ...
    _require_equal("summary.source actionability recheck", summary.get("source_actionability_recheck_item_count"), 48)
    _require_equal("summary.source ready", summary.get("source_actionability_ready_count"), 0)
    _require_equal("summary.source blocker", summary.get("source_actionability_blocker_count"), 48)
    _require_equal("summary.source private blocker queue", summary.get("source_private_actionability_blocker_queue_item_count"), 48)
    _require_equal("summary.prior observation", summary.get("prior_actionability_blocker_observation_count"), 0)
    _require_equal("summary.observation", summary.get("actionability_blocker_observation_count"), 1)
    ...
    _require_equal("summary.binding ready", summary.get("binding_ready_after_actionability_blocker_audit_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_actionability_blocker_audit_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_actionability_blocker_audit_diagnostic_written",
        "private_actionability_blocker_audit_queue_written",
        "private_actionability_blocker_audit_report_written",
        "private_actionability_blocker_audit_diagnostic_gitignored",
        "private_actionability_blocker_audit_queue_gitignored",
        "private_actionability_blocker_audit_report_gitignored",
        "source_private_actionability_diagnostic_gitignored",
        "source_private_actionability_blocker_queue_gitignored",
        "source_private_actionability_report_gitignored",
        "owner_or_agent_valid_response_available",
        "generated_diagnostic_response_actionability_blocker_audit_performed",
    ):
    ...
    for code in (
        "source_actionability_recheck_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_actionability_recheck_complete",
        "source_actionability_blockers_complete",
        "source_private_actionability_blocker_queue_complete",
        "actionability_blocker_audit_complete",
        "first_blocker_observation_recorded",
    ...
    _require_equal(
        "go_no_go.actionability_audit_conclusion",
        go_no_go.get("actionability_audit_conclusion"),
        ACTIONABILITY_AUDIT_CONCLUSION,
    )
    _require_equal("go_no_go.recheck", go_no_go.get("source_actionability_recheck_item_count"), 48)
    _require_equal("go_no_go.audit", go_no_go.get("actionability_blocker_audit_item_count"), 48)
    _require_equal("go_no_go.prior observation", go_no_go.get("prior_actionability_blocker_observation_count"), 0)
    _require_equal("go_no_go.observation", go_no_go.get("actionability_blocker_observation_count"), 1)
    _require_false("go_no_go.threshold", go_no_go.get("actionability_blocked_audit_threshold_met"))
...
def _check_private_artifacts() -> None:
    ...
    source_diagnostic = _read_json(SOURCE_PRIVATE_ACTIONABILITY_DIAGNOSTIC_PATH)
    source_rows = _read_jsonl(SOURCE_PRIVATE_ACTIONABILITY_BLOCKER_QUEUE_PATH)
    audit_diagnostic = _read_json(PRIVATE_ACTIONABILITY_BLOCKER_AUDIT_DIAGNOSTIC_PATH)
    audit_rows = _read_jsonl(PRIVATE_ACTIONABILITY_BLOCKER_AUDIT_QUEUE_PATH)
    _require_equal("source diagnostic actionability rows", len(source_diagnostic.get("actionability_rows", [])), 48)
    _require_equal("source actionability rows", len(source_rows), 48)
    _require_equal("audit diagnostic rows", len(audit_diagnostic.get("actionability_blocker_audit_rows", [])), 48)
    ...
    _require_equal("source/audit sets", source_ids, audit_ids)
    diagnostic_counts = Counter(row.get("diagnostic_kind") for row in audit_rows)
    _require_equal(
    ...
    for source_row in source_rows:
        _require_equal("source actionability status", source_row.get("actionability_status"), "blocked_non_actionable_generated_response")
    ...
    for row in audit_rows:
        _require_equal(
            "audit status",
            row.get("actionability_audit_status"),
            "blocked_non_actionable_generated_response_actionability_audit",
        )
        _require_equal("prior observation", row.get("prior_actionability_blocker_observation_count"), 0)
        _require_equal("observation", row.get("actionability_blocker_observation_count"), 1)
        _require_false("threshold", row.get("actionability_blocked_audit_threshold_met"))
        ...
        _require_false("binding", row.get("binding_ready_after_actionability_blocker_audit"))
        _require_false("comparison", row.get("comparison_retry_ready_after_actionability_blocker_audit"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("PASS: validated V014 generated owner/agent diagnostic response actionability blocker audit")
    return 0
