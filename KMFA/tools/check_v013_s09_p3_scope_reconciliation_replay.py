#!/usr/bin/env python3
"""Validate KMFA v0.1.3 S09-P3 scope reconciliation replay evidence."""
...
from KMFA.tools.v013_s09_p3_scope_reconciliation_replay import (
    MANIFEST_PATH,
    NEXT_REQUIRED_STEP,
    PHASE_SCOPE,
    RAW_DIR,
    ...
    TASK_ID,
    TEST_RESULTS_PATH,
    validate_legacy_s09_p3_artifacts,
    validate_s09_p2_dependency,
)
...
def validate_v013_s09_p3_scope_reconciliation_replay(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    ...
    require(manifest.get("phase_id") == "S09-P3", "phase_id must be S09-P3")
    require(manifest.get("task_id") == TASK_ID, "task_id mismatch")
    require(manifest.get("phase_scope") == PHASE_SCOPE, "phase scope mismatch")
    require(
        manifest.get("status") == "completed_validated_local_only_no_go_upload_deferred_scope_reconciliation_replayed",
        "status mismatch",
    )
    require(manifest.get("completed_task_ids") == ["S9PCT01", "S9PCT02", "S9PCT03"], "completed tasks mismatch")
    require(
        manifest.get("acceptance_ids") == ["ACC-V013-S09-P3-SCOPE-RECONCILIATION-REPLAY"],
        "acceptance ids mismatch",
    )
    require(manifest.get("s09_p2_dependency_validated") is True, "S09-P2 dependency flag mismatch")
    require(s09_p2.get("phase_id") == "S09-P2", "S09-P2 dependency did not validate")
    require(manifest.get("legacy_s09_p3_dependency_validated") is True, "legacy S09-P3 flag mismatch")
    ...
    require(progress.get("s09_p3_performed") is True, "S09-P3 must be performed")
    require(progress.get("stage9_review_performed") is False, "Stage 9 review must not be performed")

    summary = manifest.get("legacy_s09_p3_summary", {})
    expected_summary = {
    ...
    require(
        summary.get("required_reconciliation_domains") == legacy["required_reconciliation_domains"],
        "required domains mismatch",
    )
    require(summary.get("required_human_fields") == legacy["required_human_fields"], "required fields mismatch")
    require(
        summary.get("reconciliation_status")
        == "public_safe_reconciliation_records_created_pending_owner_confirmation",
        "reconciliation status mismatch",
    )

    policy = manifest.get("scope_reconciliation_policy", {})
    require(
        policy.get("reconciliation_records_by_domain")
        == {
            "authority_pdf_excel_vs_system_recomputed": 8,
            "bank_collection_vs_receivable_aging": 4,
        },
        "records by domain mismatch",
    )
    require(
        policy.get("domain_controls_by_domain")
        == {
            "authority_pdf_excel_vs_system_recomputed": 1,
            "bank_collection_vs_receivable_aging": 1,
            "contract_vs_project_revenue": 1,
            "invoice_vs_contract_settlement_tax": 1,
            "project_cost_vs_finance_expense": 1,
            "rd_expense_vs_project_personnel_evidence": 1,
        },
        "domain controls by domain mismatch",
    )
    require(policy.get("pending_review_count") == 12, "pending review count mismatch")
    require(policy.get("pending_domain_control_count") == 6, "pending domain control count mismatch")
    require(policy.get("closed_record_count") == 0, "closed record count mismatch")
    require(policy.get("closed_domain_control_count") == 0, "closed domain count mismatch")
    require(policy.get("confirmed_for_rerun_count") == 0, "confirmed rerun count mismatch")
    require(policy.get("domain_confirmed_for_rerun_count") == 0, "domain confirmed count mismatch")
    require(policy.get("derived_metric_rerun_allowed") is False, "derived rerun flag must be false")
    require(policy.get("derived_metric_rerun_allowed_count") == 0, "derived rerun count mismatch")
    require(policy.get("formal_report_rerun_allowed") is False, "formal rerun flag must be false")
    require(policy.get("formal_report_rerun_allowed_count") == 0, "formal rerun count mismatch")
    require(policy.get("public_amount_values_committed_count") == 0, "public amount count mismatch")
    require(policy.get("raw_layer_write_allowed_count") == 0, "raw layer write count mismatch")
    require(policy.get("record_public_amount_values_blocked_count") == 12, "record amount blocked count mismatch")
    require(policy.get("domain_public_amount_values_blocked_count") == 6, "domain amount blocked count mismatch")
    require(policy.get("all_record_human_fields_present") is True, "record human fields missing")
    require(policy.get("all_domain_human_fields_present") is True, "domain human fields missing")
    for key in (
        "reconciliation_records_by_domain",
        "domain_controls_by_domain",
        "pending_review_count",
        "pending_domain_control_count",
        "closed_record_count",
        "closed_domain_control_count",
        "confirmed_for_rerun_count",
        "domain_confirmed_for_rerun_count",
        ...
        "record_public_amount_values_blocked_count",
        "domain_public_amount_values_blocked_count",
        "quality_gate_false_count",
        "public_safety_false_count",
        "record_public_safety_false_count",
        "domain_public_safety_false_count",
        "all_record_human_fields_present",
        "all_domain_human_fields_present",
    ):
        require(policy.get(key) == legacy.get(key), f"legacy policy mismatch for {key}")

    phase_boundaries = manifest.get("phase_boundaries", {})
    require(phase_boundaries.get("s09_p2_dependency_included") is True, "S09-P2 dependency flag mismatch")
    require(
        phase_boundaries.get("s09_p3_scope_reconciliation_scope_included") is True,
        "S09-P3 scope flag mismatch",
    )
    for key in (
    ...
    for key in (
        "legacy_manifest",
        "legacy_reconciliation_records",
        "legacy_domain_controls",
    ...
    return {
        ...
        "reconciliation_record_count": summary["reconciliation_record_count"],
        "domain_control_count": summary["domain_control_count"],
        "confirmed_resolution_count": summary["confirmed_resolution_count"],
        "pending_resolution_count": summary["pending_resolution_count"],
        "upstream_margin_record_count": summary["upstream_margin_record_count"],
        "source_difference_summary_count": summary["source_difference_summary_count"],
        "required_reconciliation_domain_count": summary["required_reconciliation_domain_count"],
        "reconciliation_records_by_domain": policy["reconciliation_records_by_domain"],
        "derived_metric_rerun_allowed": quality["derived_metric_rerun_allowed"],
        "formal_report_rerun_allowed": quality["formal_report_rerun_allowed"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.3 S09-P3 scope reconciliation replay.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH)
    args = parser.parse_args(argv)

    result = validate_v013_s09_p3_scope_reconciliation_replay(args.manifest)
    print(
        "PASS: KMFA v0.1.3 S09-P3 scope reconciliation replay validated "
        f"(reconciliation_records={result['reconciliation_record_count']}, "
        f"domain_controls={result['domain_control_count']}, "
