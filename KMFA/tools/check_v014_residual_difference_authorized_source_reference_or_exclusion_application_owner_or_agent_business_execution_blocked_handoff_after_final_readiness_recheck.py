#!/usr/bin/env python3
"""Validate the business-execution blocked handoff after final readiness recheck."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_business_execution_blocked_handoff_after_final_readiness_recheck import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_business_execution_final_readiness_recheck_public_artifacts_read_by_this_phase",
        "source_private_business_execution_final_readiness_recheck_queue_read_by_this_phase",
        "private_business_execution_blocked_handoff_diagnostic_written_by_this_phase",
        "private_business_execution_blocked_handoff_queue_written_by_this_phase",
        "private_business_execution_blocked_handoff_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_business_execution_final_readiness_recheck_diagnostic_mutated_by_this_phase",
        "source_private_business_execution_final_readiness_recheck_queue_mutated_by_this_phase",
        "source_private_business_execution_final_readiness_recheck_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_final_readiness_recheck.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_final_readiness_recheck.PHASE_ID)
    ...
    _require_equal(
        "summary.source business execution final readiness recheck ready",
        summary.get("source_business_execution_final_readiness_recheck_ready_count"),
        0,
    )
    _require_equal(
        "summary.source business execution final readiness recheck blockers",
        summary.get("source_business_execution_final_readiness_recheck_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source business execution final readiness recheck items",
        summary.get("source_business_execution_final_readiness_recheck_item_count"),
        48,
    )
    _require_equal(
        "summary.source business execution final readiness recheck required",
        summary.get("source_business_execution_final_readiness_recheck_required_count"),
        48,
    )
    _require_equal(
        "summary.source private final readiness recheck rows",
        summary.get("source_private_business_execution_final_readiness_recheck_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("business_execution_blocked_handoff_ready_count"),
        0,
    )
    _require_equal(
        "summary.business execution req blockers",
        summary.get("business_execution_blocked_handoff_blocker_count"),
        48,
    )
    _require_equal(
        "summary.business execution req items",
        summary.get("business_execution_blocked_handoff_item_count"),
        48,
    )
    _require_equal(
        "summary.business execution req required",
        summary.get("business_execution_blocked_handoff_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_business_execution_blocked_handoff_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_business_execution_blocked_handoff_count"),
        8,
    )
    ...
    for key in (
        "private_business_execution_blocked_handoff_diagnostic_written",
        "private_business_execution_blocked_handoff_queue_written",
        "private_business_execution_blocked_handoff_report_written",
        "private_business_execution_blocked_handoff_diagnostic_gitignored",
        "private_business_execution_blocked_handoff_queue_gitignored",
        "private_business_execution_blocked_handoff_report_gitignored",
        "source_private_business_execution_final_readiness_recheck_diagnostic_gitignored",
        "source_private_business_execution_final_readiness_recheck_queue_gitignored",
        "source_private_business_execution_final_readiness_recheck_report_gitignored",
    ...
    for code in (
        "source_business_execution_final_readiness_recheck_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_business_execution_final_readiness_recheck_blockers_complete",
    ...
    _require_equal("go_no_go.business execution req ready", go_no_go.get("business_execution_blocked_handoff_ready_count"), 0)
    _require_equal("go_no_go.business execution req blocker", go_no_go.get("business_execution_blocked_handoff_blocker_count"), 48)
    _require_equal("go_no_go.business execution req items", go_no_go.get("business_execution_blocked_handoff_item_count"), 48)
    _require_equal("go_no_go.business execution req required", go_no_go.get("business_execution_blocked_handoff_required_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_BUSINESS_EXECUTION_BLOCKED_HANDOFF_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_BUSINESS_EXECUTION_BLOCKED_HANDOFF_QUEUE_PATH)
    report = PRIVATE_BUSINESS_EXECUTION_BLOCKED_HANDOFF_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("business_execution_blocked_handoff_rows") or []),
        48,
    )
    _require_equal("private business execution requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 business execution blocked handoff after final readiness recheck 队列" not in report:
        raise ValidationError("private business execution blocked handoff report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            ...
            row.get("business_execution_blocked_handoff_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private business execution requirement required",
            row.get("business_execution_blocked_handoff_required"),
        )
        _require_false(
            "private business execution requirement ready",
            row.get("business_execution_blocked_handoff_ready"),
        )
        _require_true(
            "private business execution requirement blocker",
            row.get("business_execution_blocked_handoff_blocker"),
        )
...
def validate(*, require_private_business_execution_blocked_handoff: bool = False) -> dict[str, Any]:
    ...
    if require_private_business_execution_blocked_handoff:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="require_private_business_execution_blocked_handoff",
    )
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_business_execution_blocked_handoff=args.require_private_business_execution_blocked_handoff
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent business execution blocked handoff after final readiness recheck "
