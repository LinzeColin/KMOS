...
PRIVATE_DIAGNOSTIC_PATH = (
    PROJECT_ROOT
    / ".codex_private_runtime/v014_processed_value_source_map_completion_blocker_audit/private_blocker_audit_diagnostic.json"
)
...
PUBLIC_ARTIFACTS = [
    ...
    MANIFEST_PATH,
    GO_NO_GO_PATH,
    HUMAN_DIR / "processed_value_source_map_completion_blocker_audit_report.md",
    HUMAN_DIR / "owner_agent_blocker_packet.md",
    HUMAN_DIR / "go_no_go_record.md",
    HUMAN_DIR / "risk_register.md",
    HUMAN_DIR / "rollback_plan.md",
    QUALITY_DIR / "v014_processed_value_source_map_completion_blocker_audit_summary.json",
    QUALITY_DIR / "v014_processed_value_source_map_completion_blocker_audit_manifest.json",
    QUALITY_DIR / "v014_processed_value_source_map_completion_blocker_audit_go_no_go_report.json",
]
...
def _check_private_diagnostic() -> None:
    ...
    _require_equal("private_diagnostic.repeated_blocker_count", diagnostic.get("repeated_blocker_count"), 3)
    _require_true("private_diagnostic.blocked_threshold_met", diagnostic.get("blocked_threshold_met"))
...
def validate(*, require_private_diagnostic: bool = False) -> dict[str, Any]:
    ...
    _require_equal("summary.blocker_condition", summary.get("blocker_condition"), "private_completion_template_unfilled_authorized_sources_not_supplied")
    _require_equal("summary.consecutive_goal_turn_blocker_count", summary.get("consecutive_goal_turn_blocker_count"), 3)
    ...
    _require_equal("summary.prior_application_decision", summary.get("prior_application_decision"), "NO_GO")
    _require_equal("summary.prior_application_valid_completion_item_count", summary.get("prior_application_valid_completion_item_count"), 0)
    _require_equal("summary.prior_readiness_decision", summary.get("prior_readiness_decision"), "NO_GO")
    _require_equal("summary.prior_readiness_valid_completion_item_count", summary.get("prior_readiness_valid_completion_item_count"), 0)
    ...
    _require_equal("go_no_go.consecutive_goal_turn_blocker_count", go_no_go.get("consecutive_goal_turn_blocker_count"), 3)
...
def main() -> None:
    ...
    print(
        "PASS: KMFA v0.1.4 processed value source-map completion blocker audit validated "
