#!/usr/bin/env python3
"""Validate KMFA v0.1.4 owner-authorized anchor confirmation readiness."""
...
from KMFA.tools.v014_residual_difference_owner_authorized_anchor_confirmation_readiness import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_after_alignment_summary_read_by_this_phase",
        "source_public_after_alignment_manifest_read_by_this_phase",
        "source_public_after_alignment_matrix_read_by_this_phase",
        "source_private_after_alignment_blockers_read_by_this_phase",
        "source_private_alignment_read_by_this_phase",
        "source_private_anchor_draft_read_by_this_phase",
        "private_readiness_written_by_this_phase",
        "private_ready_queue_written_by_this_phase",
        "private_blocker_queue_written_by_this_phase",
    ):
        _require_true(f"raw_boundary.{key}", raw_boundary.get(key))
    for key in (
        "source_private_anchor_draft_mutated_by_this_phase",
        "source_private_after_alignment_outputs_mutated_by_this_phase",
...
def _check_summary(summary: dict[str, Any], matrix: dict[str, Any], go_no_go: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    _require_equal("summary.source after-alignment blockers", summary.get("source_after_alignment_blocker_count"), 72)
    ...
    _require_equal("summary.owner select count", summary.get("owner_select_one_authoritative_candidate_count"), 24)
    _require_equal(
        "summary.authoritative source ref count",
        summary.get("provide_authoritative_source_reference_or_owner_exclusion_count"),
        40,
    )
    _require_equal("summary.formula mapping count", summary.get("provide_formula_or_non_numeric_mapping_count"), 8)
    for key in (
    ...
    for key in (
        "private_readiness_written",
        "private_readiness_gitignored",
        "private_ready_queue_written",
        "private_ready_queue_gitignored",
        "private_blocker_queue_written",
        "private_blocker_queue_gitignored",
    ...
    _require_equal("matrix.check count", matrix.get("check_count"), 9)
    _require_equal("matrix.pass count", matrix.get("pass_count"), 6)
    _require_equal("matrix.fail count", matrix.get("fail_count"), 3)
    _require_equal("matrix.owner ready count", matrix.get("owner_authorized_anchor_ready_count"), 0)
    _require_equal("matrix.owner blocker count", matrix.get("owner_authorized_anchor_blocker_count"), 72)
    ...
    _require_equal("go_no_go.owner ready count", go_no_go.get("owner_authorized_anchor_ready_count"), 0)
    _require_equal("go_no_go.owner blocker count", go_no_go.get("owner_authorized_anchor_blocker_count"), 72)
...
def _check_private_readiness(summary: dict[str, Any]) -> None:
    ...
    readiness = _read_json(PRIVATE_READINESS_PATH)
    diagnostic = _read_json(PRIVATE_DIAGNOSTIC_PATH)
    ready = _read_jsonl(PRIVATE_READY_QUEUE_PATH)
    blockers = _read_jsonl(PRIVATE_BLOCKER_QUEUE_PATH)
    for payload_name, payload in (("readiness", readiness), ("diagnostic", diagnostic)):
    ...
    _require_equal("private blocker queue length", len(blockers), 72)
    _require_equal("readiness.ready count", readiness.get("owner_authorized_anchor_ready_count"), 0)
    _require_equal("readiness.blocker count", readiness.get("owner_authorized_anchor_blocker_count"), 72)
    _require_equal("diagnostic.blocker count", diagnostic.get("owner_authorized_anchor_blocker_count"), 72)
    _require_false("anchor confirmation ready", readiness.get("anchor_confirmation_ready"))
    _require_false("raw comparison ready", readiness.get("raw_to_processed_value_comparison_ready"))
    _require_false("raw comparison performed", readiness.get("raw_to_processed_value_comparison_performed"))
    _require_false("business consistency", readiness.get("business_value_consistency_verified"))
    ...
    sample_count = sum(1 for row in blockers if row.get("private_candidate_sample_available") is True)
    missing_sample_count = sum(1 for row in blockers if row.get("private_candidate_sample_available") is False)
    _require_equal("private blocker sample count", sample_count, 24)
    _require_equal("private blocker missing sample count", missing_sample_count, 48)
    for row in blockers:
        _require_equal(
            "blocker status",
            row.get("owner_authorized_anchor_confirmation_status"),
            "blocked_missing_owner_authorized_anchor_inputs",
        )
        _require_false("blocker anchor ready", row.get("anchor_confirmation_ready"))
    ...
    _require_equal("summary blocker count", summary.get("owner_authorized_anchor_blocker_count"), len(blockers))
    _require_equal("summary ready count", summary.get("owner_authorized_anchor_ready_count"), len(ready))
...
def main() -> int:
    ...
    print(
        "PASS: KMFA v0.1.4 owner-authorized anchor confirmation readiness artifacts validated "
