#!/usr/bin/env python3
"""Validate the external-action requirement gate before lineage full check."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_required_before_lineage_full_check import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_business_value_consistency_public_artifacts_read_by_this_phase",
        "source_private_business_value_consistency_requirement_queue_read_by_this_phase",
        "private_lineage_full_check_requirement_diagnostic_written_by_this_phase",
        "private_lineage_full_check_requirement_queue_written_by_this_phase",
        "private_lineage_full_check_requirement_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_business_value_consistency_requirement_diagnostic_mutated_by_this_phase",
        "source_private_business_value_consistency_requirement_queue_mutated_by_this_phase",
        "source_private_business_value_consistency_requirement_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_business_consistency.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_business_consistency.PHASE_ID)
    ...
    _require_equal(
        "summary.source business-value consistency ready",
        summary.get("source_business_value_consistency_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.source business-value consistency blockers",
        summary.get("source_business_value_consistency_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source business-value consistency required",
        summary.get("source_business_value_consistency_requirement_required_count"),
        48,
    )
    _require_equal(
        "summary.source private business-value consistency rows",
        summary.get("source_private_business_value_consistency_requirement_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        "summary.lineage full check req ready",
        summary.get("lineage_full_check_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.lineage full check req blockers",
        summary.get("lineage_full_check_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.lineage full check req required",
        summary.get("lineage_full_check_requirement_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_lineage_full_check_requirement_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_lineage_full_check_requirement_count"),
        8,
    )
    ...
    for key in (
        "private_lineage_full_check_requirement_diagnostic_written",
        "private_lineage_full_check_requirement_queue_written",
        "private_lineage_full_check_requirement_report_written",
        "private_lineage_full_check_requirement_diagnostic_gitignored",
        "private_lineage_full_check_requirement_queue_gitignored",
        "private_lineage_full_check_requirement_report_gitignored",
        "source_private_business_value_consistency_requirement_diagnostic_gitignored",
        "source_private_business_value_consistency_requirement_queue_gitignored",
        "source_private_business_value_consistency_requirement_report_gitignored",
    ...
    for code in (
        "source_business_value_consistency_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_business_value_consistency_blockers_complete",
    ...
    _require_equal("go_no_go.lineage full check req ready", go_no_go.get("lineage_full_check_requirement_ready_count"), 0)
    _require_equal("go_no_go.lineage full check req blocker", go_no_go.get("lineage_full_check_requirement_blocker_count"), 48)
    _require_equal("go_no_go.lineage full check req required", go_no_go.get("lineage_full_check_requirement_required_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_LINEAGE_FULL_CHECK_REQUIREMENT_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_LINEAGE_FULL_CHECK_REQUIREMENT_QUEUE_PATH)
    report = PRIVATE_LINEAGE_FULL_CHECK_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("lineage_full_check_requirement_rows") or []),
        48,
    )
    _require_equal("private lineage full check requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 external action required before lineage full check 队列" not in report:
        raise ValidationError("private lineage full check requirement report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            "private lineage full check requirement status",
            row.get("lineage_full_check_requirement_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private lineage full check requirement required",
            row.get("lineage_full_check_requirement_required"),
        )
        _require_false(
            "private lineage full check requirement ready",
            row.get("lineage_full_check_requirement_ready"),
        )
        _require_true(
            "private lineage full check requirement blocker",
            row.get("lineage_full_check_requirement_blocker"),
        )
...
def validate(*, require_private_lineage_full_check_requirement: bool = False) -> dict[str, Any]:
    ...
    if require_private_lineage_full_check_requirement:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-lineage-full-check-requirement", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_lineage_full_check_requirement=(
                args.require_private_lineage_full_check_requirement
            )
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent external action required before lineage full check "
