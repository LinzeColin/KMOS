#!/usr/bin/env python3
"""Validate KMFA v0.1.4 full reconciliation blocker audit."""
...
EXPECTED_BLOCKER_CODES = {
    "NON_ACTIONABLE_KEEP_NO_GO_GROUPS",
    "FULL_SOURCE_MAP_REAPPLICATION_NOT_READY",
    "FULL_RAW_TO_PROCESSED_COMPARISON_NOT_PERFORMED",
...
PRIVATE_DIR = PROJECT_ROOT / ".codex_private_runtime/v014_processed_value_source_map_completion_full_reconciliation_blocker_audit"
PRIVATE_DIAGNOSTIC_PATH = PRIVATE_DIR / "private_full_reconciliation_blocker_audit_diagnostic.json"
...
PUBLIC_ARTIFACTS = [
    ...
    MANIFEST_PATH,
    GO_NO_GO_PATH,
    HUMAN_DIR / "processed_value_source_map_completion_full_reconciliation_blocker_audit_report.md",
    HUMAN_DIR / "go_no_go_record.md",
    HUMAN_DIR / "test_results.md",
    HUMAN_DIR / "risk_register.md",
    HUMAN_DIR / "rollback_plan.md",
    QUALITY_DIR / "v014_full_reconciliation_blocker_audit_summary.json",
    QUALITY_DIR / "v014_full_reconciliation_blocker_audit_manifest.json",
    QUALITY_DIR / "v014_full_reconciliation_blocker_audit_go_no_go_report.json",
]
...
def _check_private_diagnostic() -> None:
    ...
    audit_counts = diagnostic.get("audit_counts", {})
    _require_equal("diagnostic.partial_application_slots", audit_counts.get("partial_application_target_slot_count"), 101)
    _require_equal("diagnostic.partial_blocked_slots", audit_counts.get("partial_blocked_target_slot_count"), 12)
    _require_true("diagnostic.partial_replay_complete", audit_counts.get("partial_materialization_replay_complete"))
    _require_true("diagnostic.partial_fingerprint_passed", audit_counts.get("partial_fingerprint_comparison_passed"))
    _require_equal("diagnostic.non_actionable_groups", audit_counts.get("non_actionable_group_count"), 3)
    _require_equal("diagnostic.non_actionable_slots", audit_counts.get("non_actionable_target_slot_count"), 12)
    _require_equal("diagnostic.keep_no_go", audit_counts.get("keep_no_go_resolution_count"), 3)
    blockers = diagnostic.get("blockers", [])
    if not isinstance(blockers, list):
        raise ValidationError("private blockers must be a list")
    _require_equal("diagnostic.blocker_count", len(blockers), 4)
    _require_equal("diagnostic.blocker_codes", {str(item.get("blocker_code")) for item in blockers}, EXPECTED_BLOCKER_CODES)
...
def validate(*, require_private_diagnostic: bool = False) -> dict[str, Any]:
    ...
    metadata_summary = _read_json(QUALITY_DIR / "v014_full_reconciliation_blocker_audit_summary.json")
    metadata_manifest = _read_json(QUALITY_DIR / "v014_full_reconciliation_blocker_audit_manifest.json")
    metadata_go_no_go = _read_json(QUALITY_DIR / "v014_full_reconciliation_blocker_audit_go_no_go_report.json")
    ...
    _require_equal("summary.partial_application_slots", summary.get("partial_application_target_slot_count"), 101)
    _require_equal("summary.partial_blocked_slots", summary.get("partial_blocked_target_slot_count"), 12)
    _require_true("summary.partial_replay_complete", summary.get("partial_materialization_replay_complete"))
    _require_true("summary.partial_fingerprint_passed", summary.get("partial_fingerprint_comparison_passed"))
...
def main() -> None:
    ...
    print(
        "PASS: KMFA v0.1.4 full reconciliation blocker audit validated "
