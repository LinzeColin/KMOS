#!/usr/bin/env python3
"""Validate the external-action requirement gate before formal report."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_required_before_formal_report import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_lineage_full_check_public_artifacts_read_by_this_phase",
        "source_private_lineage_full_check_requirement_queue_read_by_this_phase",
        "private_formal_report_requirement_diagnostic_written_by_this_phase",
        "private_formal_report_requirement_queue_written_by_this_phase",
        "private_formal_report_requirement_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_lineage_full_check_requirement_diagnostic_mutated_by_this_phase",
        "source_private_lineage_full_check_requirement_queue_mutated_by_this_phase",
        "source_private_lineage_full_check_requirement_report_mutated_by_this_phase",
        ...
        "formal_report_ready_by_this_phase",
        "formal_report_allowed_by_this_phase",
        "formal_report_generated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_lineage_full_check.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_lineage_full_check.PHASE_ID)
    ...
    _require_equal(
        ...
        summary.get("source_lineage_full_check_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.source lineage full check blockers",
        summary.get("source_lineage_full_check_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source lineage full check required",
        summary.get("source_lineage_full_check_requirement_required_count"),
        48,
    )
    _require_equal(
        "summary.source private lineage full check rows",
        summary.get("source_private_lineage_full_check_requirement_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        "summary.formal report req ready",
        summary.get("formal_report_requirement_ready_count"),
        0,
    )
    _require_equal(
        "summary.formal report req blockers",
        summary.get("formal_report_requirement_blocker_count"),
        48,
    )
    _require_equal(
        "summary.formal report req required",
        summary.get("formal_report_requirement_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_formal_report_requirement_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_formal_report_requirement_count"),
        8,
    )
    ...
    _require_equal("summary.formal report ready count", summary.get("formal_report_ready_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_formal_report_requirement_diagnostic_written",
        "private_formal_report_requirement_queue_written",
        "private_formal_report_requirement_report_written",
        "private_formal_report_requirement_diagnostic_gitignored",
        "private_formal_report_requirement_queue_gitignored",
        "private_formal_report_requirement_report_gitignored",
        "source_private_lineage_full_check_requirement_diagnostic_gitignored",
        "source_private_lineage_full_check_requirement_queue_gitignored",
        "source_private_lineage_full_check_requirement_report_gitignored",
    ...
    for code in (
        "source_lineage_full_check_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_lineage_full_check_blockers_complete",
    ...
    _require_equal("go_no_go.formal report req ready", go_no_go.get("formal_report_requirement_ready_count"), 0)
    _require_equal("go_no_go.formal report req blocker", go_no_go.get("formal_report_requirement_blocker_count"), 48)
    _require_equal("go_no_go.formal report req required", go_no_go.get("formal_report_requirement_required_count"), 48)
    ...
    _require_false("go_no_go.formal report ready", go_no_go.get("formal_report_ready"))
    _require_false("go_no_go.formal report allowed", go_no_go.get("formal_report_allowed"))
    _require_false("go_no_go.formal report generated", go_no_go.get("formal_report_generated_by_this_phase"))
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_FORMAL_REPORT_REQUIREMENT_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_FORMAL_REPORT_REQUIREMENT_QUEUE_PATH)
    report = PRIVATE_FORMAL_REPORT_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("formal_report_requirement_rows") or []),
        48,
    )
    _require_equal("private formal report requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 external action required before formal report 队列" not in report:
        raise ValidationError("private formal report requirement report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            "private formal report requirement status",
            row.get("formal_report_requirement_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private formal report requirement required",
            row.get("formal_report_requirement_required"),
        )
        _require_false(
            "private formal report requirement ready",
            row.get("formal_report_requirement_ready"),
        )
        _require_true(
            "private formal report requirement blocker",
            row.get("formal_report_requirement_blocker"),
        )
        ...
        _require_false("private formal report ready", row.get("formal_report_ready"))
        _require_false("private formal report allowed", row.get("formal_report_allowed"))
        _require_false("private formal report generated", row.get("formal_report_generated_by_this_phase"))
        _require_false("private public commit", row.get("public_commit_allowed"))


def validate(*, require_private_formal_report_requirement: bool = False) -> dict[str, Any]:
    ...
    if require_private_formal_report_requirement:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-formal-report-requirement", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_formal_report_requirement=(
                args.require_private_formal_report_requirement
            )
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent external action required before formal report "
