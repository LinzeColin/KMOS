#!/usr/bin/env python3
"""Validate fingerprint pair completion blocker final threshold recheck artifacts."""
...
from KMFA.tools.v014_residual_difference_raw_to_processed_comparison_fingerprint_pair_completion_blocker_final_threshold_recheck_after_owner_anchor_confirmation import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
    ...
    RISK_REGISTER_PATH,
    ROLLBACK_PATH,
    SOURCE_BLOCKER_THRESHOLD_GO_NO_GO_PATH,
    SOURCE_BLOCKER_THRESHOLD_MANIFEST_PATH,
    SOURCE_BLOCKER_THRESHOLD_MATRIX_PATH,
    SOURCE_BLOCKER_THRESHOLD_SUMMARY_PATH,
    SOURCE_PRIVATE_BLOCKER_THRESHOLD_DIAGNOSTIC_PATH,
    SOURCE_PRIVATE_BLOCKER_THRESHOLD_RECORDS_PATH,
    SOURCE_PRIVATE_BLOCKER_THRESHOLD_REPORT_PATH,
    STATUS,
    SUMMARY_PATH,
    TASK_ID,
    TEST_RESULTS_PATH,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_blocker_threshold_summary_read_by_this_phase",
        "source_private_blocker_threshold_records_read_by_this_phase",
        "private_final_threshold_diagnostic_written_by_this_phase",
        "private_final_threshold_records_written_by_this_phase",
        "private_final_threshold_report_written_by_this_phase",
    ):
    ...
    for key in (
        "source_private_blocker_threshold_diagnostic_mutated_by_this_phase",
        "source_private_blocker_threshold_records_mutated_by_this_phase",
        "source_private_blocker_threshold_report_mutated_by_this_phase",
    ):
...
def _check_summary(summary: dict[str, Any], matrix: dict[str, Any], go_no_go: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    _require_equal("summary.source threshold blockers", summary.get("source_fingerprint_pair_completion_blocker_count"), 48)
    _require_equal(
        "summary.source observation",
        summary.get("source_fingerprint_pair_completion_blocker_observation_count"),
        2,
    )
    _require_false(
        "summary.source threshold",
        summary.get("source_fingerprint_pair_completion_blocked_audit_threshold_met"),
    )
    _require_equal("summary.source threshold items", summary.get("source_blocker_threshold_item_count"), 48)
    ...
    _require_equal("summary.source private rows", summary.get("source_private_blocker_threshold_record_count"), 48)
    _require_equal("summary.prior observation", summary.get("prior_fingerprint_pair_completion_blocker_observation_count"), 2)
    _require_equal("summary.current observation", summary.get("fingerprint_pair_completion_blocker_observation_count"), 3)
    _require_true("summary.threshold", summary.get("fingerprint_pair_completion_blocked_audit_threshold_met"))
    _require_equal("summary.goal status recommendation", summary.get("goal_status_recommendation"), "blocked")
    _require_true("summary.final threshold recheck performed", summary.get("fingerprint_pair_completion_blocker_final_threshold_recheck_performed_by_this_phase"))
    ...
    for key in (
        "private_final_threshold_diagnostic_written",
        "private_final_threshold_records_written",
        "private_final_threshold_report_written",
        "private_final_threshold_diagnostic_gitignored",
        "private_final_threshold_records_gitignored",
        "private_final_threshold_report_gitignored",
        "source_private_blocker_threshold_diagnostic_gitignored",
        "source_private_blocker_threshold_records_gitignored",
        "source_private_blocker_threshold_report_gitignored",
    ):
    ...
    for code in (
        ...
        "no_retry_ready_after_final_threshold",
    ...
    _require_equal("go_no_go.prior observation", go_no_go.get("prior_fingerprint_pair_completion_blocker_observation_count"), 2)
    _require_equal("go_no_go.current observation", go_no_go.get("fingerprint_pair_completion_blocker_observation_count"), 3)
    _require_true("go_no_go.threshold", go_no_go.get("fingerprint_pair_completion_blocked_audit_threshold_met"))
...
def _check_private_artifacts() -> None:
    ...
    source_diagnostic = _read_json(SOURCE_PRIVATE_BLOCKER_THRESHOLD_DIAGNOSTIC_PATH)
    source_rows = _read_jsonl(SOURCE_PRIVATE_BLOCKER_THRESHOLD_RECORDS_PATH)
    threshold_diagnostic = _read_json(PRIVATE_FINAL_THRESHOLD_DIAGNOSTIC_PATH)
    threshold_rows = _read_jsonl(PRIVATE_FINAL_THRESHOLD_RECORDS_PATH)

    _require_equal("source diagnostic threshold items", source_diagnostic.get("threshold_recheck_item_count"), 48)
    _require_equal("source rows", len(source_rows), 48)
    _require_equal("threshold diagnostic items", threshold_diagnostic.get("final_threshold_recheck_item_count"), 48)
    ...
    for row in threshold_rows:
        _require_equal("threshold status", row.get("threshold_recheck_status"), "missing_raw_candidate_fingerprint_observation_3")
        _require_equal("prior observation", row.get("prior_fingerprint_pair_completion_blocker_observation_count"), 2)
        _require_equal("current observation", row.get("fingerprint_pair_completion_blocker_observation_count"), 3)
...
def validate(*, require_private_final_threshold: bool = False) -> dict[str, Any]:
    ...
    _read_json(SOURCE_BLOCKER_THRESHOLD_SUMMARY_PATH)
    _read_json(SOURCE_BLOCKER_THRESHOLD_MANIFEST_PATH)
    _read_json(SOURCE_BLOCKER_THRESHOLD_GO_NO_GO_PATH)
    _read_json(SOURCE_BLOCKER_THRESHOLD_MATRIX_PATH)
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("PASS: KMFA v0.1.4 fingerprint pair completion blocker final threshold recheck after owner anchor confirmation artifacts validated")
    return 0
