#!/usr/bin/env python3
"""Validate owner/agent diagnostic blocker audit after readiness recheck."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_diagnostic_blocker_audit_after_readiness_recheck import (  # noqa: E402
    ACCEPTANCE_ID,
    AUDIT_CONCLUSION,
    DECISION,
...
FORBIDDEN_PUBLIC_PATTERNS = [
    ...
    re.compile(
        ...
        + source_readiness.source_intake.source_packet.PRIVATE_SLOT_KEY
        + r')"',
        re.IGNORECASE,
    ),
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_readiness_go_no_go_read_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source_phase_id", summary.get("source_phase_id"), source_readiness.PHASE_ID)
    _require_equal("summary.source_manifest_phase_id", summary.get("source_manifest_phase_id"), source_readiness.PHASE_ID)
    _require_equal("summary.source private diagnostic phase", summary.get("source_private_diagnostic_phase_id"), source_readiness.PHASE_ID)
    ...
    _require_equal("summary.source blocker queue", summary.get("source_private_readiness_blocker_queue_item_count"), 48)
    _require_equal("summary.source ready", summary.get("source_diagnostic_response_ready_count"), 0)
    ...
    _require_equal("summary.prior observation", summary.get("prior_diagnostic_blocker_observation_count"), 1)
    _require_equal("summary.current observation", summary.get("diagnostic_blocker_observation_count"), 2)
    _require_false("summary.threshold", summary.get("diagnostic_blocked_audit_threshold_met"))
    _require_equal("summary.goal status", summary.get("goal_status_recommendation"), "continue_waiting_for_valid_diagnostic_response")
    ...
    _require_equal("summary.audit queue", summary.get("private_audit_queue_item_count"), 48)
    ...
    _require_equal("summary.source ref blockers", summary.get("source_reference_or_owner_exclusion_blocker_count"), 40)
    _require_equal("summary.formula blockers", summary.get("formula_or_non_numeric_mapping_blocker_count"), 8)
    _require_equal("summary.binding ready", summary.get("binding_ready_after_blocker_audit_count"), 0)
    ...
    _require_equal("go_no_go.audit_conclusion", go_no_go.get("audit_conclusion"), AUDIT_CONCLUSION)
    _require_equal("go_no_go.diagnostic_blocker_observation_count", go_no_go.get("diagnostic_blocker_observation_count"), 2)
    _require_false("go_no_go.threshold", go_no_go.get("diagnostic_blocked_audit_threshold_met"))
    _require_equal("go_no_go.goal status", go_no_go.get("goal_status_recommendation"), "continue_waiting_for_valid_diagnostic_response")
    _require_equal("go_no_go.blockers", go_no_go.get("diagnostic_response_blocker_count"), 48)
    _require_equal("go_no_go.pending", go_no_go.get("pending_diagnostic_response_count"), 48)
    _require_equal("go_no_go.valid", go_no_go.get("valid_diagnostic_response_count"), 0)
...
def _check_private_artifacts() -> None:
    ...
    source_rows = _read_jsonl(SOURCE_PRIVATE_BLOCKER_QUEUE_PATH)
    audit_rows = _read_jsonl(PRIVATE_AUDIT_QUEUE_PATH)
    _require_equal("source private blocker rows", len(source_rows), 48)
    _require_equal("private audit rows", len(audit_rows), 48)
    ...
    for row in audit_rows:
        _require_equal("private audit blocker_code", row.get("blocker_code"), "missing_valid_owner_or_agent_diagnostic_response")
        _require_equal("private audit audit_status", row.get("audit_status"), "still_blocked")
        _require_false("private audit valid response", row.get("valid_diagnostic_response"))
        _require_false("private audit actionable", row.get("actionable_resolution_ready"))
        _require_false("private audit public commit", row.get("public_commit_allowed"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("PASS: validated V014 authorized source reference or exclusion application owner/agent diagnostic blocker audit after readiness recheck")
    return 0
