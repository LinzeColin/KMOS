#!/usr/bin/env python3
"""Validate owner or authorized-agent action intake after blocked handoff."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_action_intake_after_blocked_handoff import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
    ...
    PRIVATE_ACTION_INTAKE_REPORT_PATH,
    PROJECT_ROOT,
    REPORT_PATH,
    RISK_REGISTER_PATH,
    ROLLBACK_PATH,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_action_readiness_public_artifacts_read_by_this_phase",
        "source_action_readiness_manifest_read_by_this_phase",
        "source_action_readiness_go_no_go_read_by_this_phase",
        "source_action_readiness_matrix_read_by_this_phase",
        "source_private_action_readiness_diagnostic_read_by_this_phase",
        "source_private_action_readiness_blocker_records_read_by_this_phase",
        "source_private_action_readiness_question_list_existence_checked_by_this_phase",
        "private_action_intake_diagnostic_written_by_this_phase",
        "private_action_intake_blocker_records_written_by_this_phase",
        "private_action_intake_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_action_readiness_diagnostic_mutated_by_this_phase",
        "source_private_action_readiness_blocker_records_mutated_by_this_phase",
        "source_private_action_readiness_question_list_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.intake", summary.get("intake_conclusion"), INTAKE_CONCLUSION)
    ...
    _require_equal("summary.source owner action blockers", summary.get("source_owner_action_blocker_count"), 48)
    _require_equal("summary.source owner action ready", summary.get("source_owner_action_ready_count"), 0)
    _require_equal(
        "summary.private readiness rows",
        summary.get("source_private_action_readiness_blocker_records_item_count"),
        48,
    )
    ...
    _require_equal(
        "summary.source ref intake blockers",
        summary.get("source_reference_or_owner_exclusion_intake_blocker_count"),
        40,
    )
    _require_equal(
        "summary.formula intake blockers",
        summary.get("formula_or_non_numeric_mapping_intake_blocker_count"),
        8,
    )
    _require_equal("summary.binding ready", summary.get("binding_ready_after_owner_action_intake_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_owner_action_intake_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_action_intake_diagnostic_written",
        "private_action_intake_blocker_records_written",
        "private_action_intake_report_written",
        "private_action_intake_diagnostic_gitignored",
        "private_action_intake_blocker_records_gitignored",
        "private_action_intake_report_gitignored",
        "source_private_action_readiness_diagnostic_gitignored",
        "source_private_action_readiness_blocker_records_gitignored",
        "source_private_action_readiness_question_list_gitignored",
        "owner_action_intake_checked_by_this_phase",
    ):
    ...
    for code in (
        "source_action_readiness_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_action_readiness_blockers_complete",
        "source_owner_action_ready_absent",
        "source_private_action_readiness_records_read",
        "owner_action_intake_checked",
        "owner_action_intake_blockers_locked",
    ...
    _require_equal("go_no_go.intake", go_no_go.get("intake_conclusion"), INTAKE_CONCLUSION)
    _require_equal("go_no_go.source blockers", go_no_go.get("source_owner_action_blocker_count"), 48)
    _require_equal("go_no_go.source ready", go_no_go.get("source_owner_action_ready_count"), 0)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_ACTION_INTAKE_DIAGNOSTIC_PATH)
    blocker_rows = _read_jsonl(PRIVATE_ACTION_INTAKE_BLOCKER_RECORDS_PATH)
    report_text = PRIVATE_ACTION_INTAKE_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal("private diagnostic rows", len(diagnostic.get("blocker_rows") or []), 48)
    _require_equal("private blocker row count", len(blocker_rows), 48)
    if "No executable owner or authorized-agent action was detected" not in report_text:
        raise ValidationError("private intake report missing no-action conclusion")
    counts = Counter(row.get("diagnostic_kind") for row in blocker_rows)
    _require_equal(
    ...
    for row in blocker_rows:
        _require_equal("private intake status", row.get("owner_action_intake_status"), INTAKE_CONCLUSION)
        ...
        _require_false("private binding", row.get("binding_ready_after_owner_action_intake"))
        _require_false("private retry", row.get("comparison_retry_ready_after_owner_action_intake"))
        _require_false("private public commit", row.get("public_commit_allowed"))


def validate(*, require_private_action_intake: bool = False) -> dict[str, Any]:
    ...
    if require_private_action_intake:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-action-intake", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(require_private_action_intake=args.require_private_action_intake)
    except ValidationError as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1
    summary = manifest["summary"]
    print(
        "PASS: validated V014 owner/authorized-agent action intake after blocked handoff "
