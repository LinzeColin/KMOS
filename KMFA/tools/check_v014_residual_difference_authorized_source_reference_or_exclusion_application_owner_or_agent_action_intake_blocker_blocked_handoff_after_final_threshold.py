#!/usr/bin/env python3
"""Validate blocked handoff after action-intake blocker final threshold."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_action_intake_blocker_blocked_handoff_after_final_threshold import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_final_threshold_public_artifacts_read_by_this_phase",
        "source_final_threshold_manifest_read_by_this_phase",
        "source_final_threshold_go_no_go_read_by_this_phase",
        "source_final_threshold_matrix_read_by_this_phase",
        "source_private_action_intake_blocker_final_threshold_diagnostic_read_by_this_phase",
        "source_private_action_intake_blocker_final_threshold_records_read_by_this_phase",
        "source_private_action_intake_blocker_final_threshold_report_existence_checked_by_this_phase",
        "private_action_intake_blocker_blocked_handoff_diagnostic_written_by_this_phase",
        "private_action_intake_blocker_blocked_handoff_records_written_by_this_phase",
        "private_action_intake_blocker_owner_action_queue_written_by_this_phase",
        "private_action_intake_blocker_blocked_handoff_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_action_intake_blocker_final_threshold_diagnostic_mutated_by_this_phase",
        "source_private_action_intake_blocker_final_threshold_records_mutated_by_this_phase",
        "source_private_action_intake_blocker_final_threshold_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_final_threshold.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_final_threshold.PHASE_ID)
    ...
    _require_equal(
        "summary.source final threshold",
        summary.get("source_action_intake_blocker_final_threshold_recheck_item_count"),
        48,
    )
    _require_equal("summary.source blockers", summary.get("source_owner_action_intake_blocker_count"), 48)
    _require_equal("summary.source ready", summary.get("source_owner_action_intake_ready_count"), 0)
    _require_equal(
        "summary.source private final threshold records",
        summary.get("source_private_action_intake_blocker_final_threshold_records_item_count"),
        48,
    )
    _require_equal("summary.blocked handoff", summary.get("blocked_handoff_item_count"), 48)
    _require_equal("summary.owner action", summary.get("owner_action_item_count"), 48)
    ...
    _require_equal("summary.ready", summary.get("owner_action_intake_ready_count"), 0)
    _require_equal("summary.blocker", summary.get("owner_action_intake_blocker_count"), 48)
    _require_equal("summary.valid", summary.get("valid_owner_action_intake_blocker_count"), 48)
    _require_equal("summary.actionable", summary.get("actionable_owner_resolution_count"), 0)
    _require_equal(
        "summary.source ref owner actions",
        summary.get("source_reference_or_owner_exclusion_owner_action_count"),
        40,
    )
    _require_equal("summary.formula owner actions", summary.get("formula_or_non_numeric_mapping_owner_action_count"), 8)
    ...
    for key in (
        "private_action_intake_blocker_blocked_handoff_diagnostic_written",
        "private_action_intake_blocker_blocked_handoff_records_written",
        "private_action_intake_blocker_owner_action_queue_written",
        "private_action_intake_blocker_blocked_handoff_report_written",
        "private_action_intake_blocker_blocked_handoff_diagnostic_gitignored",
        "private_action_intake_blocker_blocked_handoff_records_gitignored",
        "private_action_intake_blocker_owner_action_queue_gitignored",
        "private_action_intake_blocker_blocked_handoff_report_gitignored",
        "source_private_action_intake_blocker_final_threshold_diagnostic_gitignored",
        "source_private_action_intake_blocker_final_threshold_records_gitignored",
        "source_private_action_intake_blocker_final_threshold_report_gitignored",
        "blocked_handoff_prepared_by_this_phase",
        "owner_action_queue_prepared_by_this_phase",
    ):
    ...
    for code in (
        "source_final_threshold_phase_loaded",
    ...
    _require_equal("go_no_go.owner action", go_no_go.get("owner_action_item_count"), 48)
    ...
    _require_equal("go_no_go.ready", go_no_go.get("owner_action_intake_ready_count"), 0)
    _require_equal("go_no_go.blocker", go_no_go.get("owner_action_intake_blocker_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_ACTION_INTAKE_BLOCKER_BLOCKED_HANDOFF_DIAGNOSTIC_PATH)
    handoff_rows = _read_jsonl(PRIVATE_ACTION_INTAKE_BLOCKER_BLOCKED_HANDOFF_RECORDS_PATH)
    owner_action_rows = _read_jsonl(PRIVATE_ACTION_INTAKE_BLOCKER_OWNER_ACTION_QUEUE_PATH)
    ...
    _require_equal("private diagnostic owner action rows", len(diagnostic.get("owner_action_queue_rows") or []), 48)
    _require_equal("private handoff row count", len(handoff_rows), 48)
    _require_equal("private owner action row count", len(owner_action_rows), 48)
    ...
    for row in handoff_rows:
        ...
        _require_true("private owner action required", row.get("owner_action_required"))
        _require_equal("private owner action status", row.get("owner_action_status"), "required_before_binding_or_value_comparison")
        _require_true("private threshold met", row.get("action_intake_blocked_audit_threshold_met"))
    ...
    for row in owner_action_rows:
        _require_equal("owner action status", row.get("owner_action_status"), "required_before_binding_or_value_comparison")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "PASS: validated V014 owner/agent action-intake blocker blocked handoff after final threshold "
