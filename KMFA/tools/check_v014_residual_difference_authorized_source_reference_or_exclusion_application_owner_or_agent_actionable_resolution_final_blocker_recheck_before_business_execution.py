...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_actionable_resolution_final_blocker_recheck_before_business_execution import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_actionable_resolution_blocker_audit_public_artifacts_read_by_this_phase",
        "source_private_actionable_resolution_blocker_audit_queue_read_by_this_phase",
        "private_actionable_resolution_final_blocker_recheck_diagnostic_written_by_this_phase",
        "private_actionable_resolution_final_blocker_recheck_queue_written_by_this_phase",
        "private_actionable_resolution_final_blocker_recheck_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_actionable_resolution_blocker_audit_diagnostic_mutated_by_this_phase",
        "source_private_actionable_resolution_blocker_audit_queue_mutated_by_this_phase",
        "source_private_actionable_resolution_blocker_audit_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal(
        ...
        summary.get("source_actionable_resolution_blocker_audit_ready_count"),
        0,
    )
    _require_equal(
        "summary.source lineage full check blockers",
        summary.get("source_actionable_resolution_blocker_audit_blocker_count"),
        48,
    )
    _require_equal(
        "summary.source lineage full check items",
        summary.get("source_actionable_resolution_blocker_audit_item_count"),
        48,
    )
    _require_equal(
        "summary.source lineage full check required",
        summary.get("source_actionable_resolution_blocker_audit_required_count"),
        48,
    )
    _require_equal(
        "summary.source private lineage full check rows",
        summary.get("source_private_actionable_resolution_blocker_audit_queue_item_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("actionable_resolution_final_blocker_recheck_ready_count"),
        0,
    )
    _require_equal(
        "summary.business execution req blockers",
        summary.get("actionable_resolution_final_blocker_recheck_blocker_count"),
        48,
    )
    _require_equal(
        "summary.business execution req items",
        summary.get("actionable_resolution_final_blocker_recheck_item_count"),
        48,
    )
    _require_equal(
        "summary.business execution req required",
        summary.get("actionable_resolution_final_blocker_recheck_required_count"),
        48,
    )
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_actionable_resolution_final_blocker_recheck_count"),
        40,
    )
    _require_equal(
        "summary.formula reconciliation requirement",
        summary.get("formula_or_non_numeric_mapping_actionable_resolution_final_blocker_recheck_count"),
        8,
    )
    ...
    for key in (
        "private_actionable_resolution_final_blocker_recheck_diagnostic_written",
        "private_actionable_resolution_final_blocker_recheck_queue_written",
        "private_actionable_resolution_final_blocker_recheck_report_written",
        "private_actionable_resolution_final_blocker_recheck_diagnostic_gitignored",
        "private_actionable_resolution_final_blocker_recheck_queue_gitignored",
        "private_actionable_resolution_final_blocker_recheck_report_gitignored",
        "source_private_actionable_resolution_blocker_audit_diagnostic_gitignored",
        "source_private_actionable_resolution_blocker_audit_queue_gitignored",
        "source_private_actionable_resolution_blocker_audit_report_gitignored",
    ...
    for code in (
        "source_actionable_resolution_blocker_audit_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_actionable_resolution_blocker_audit_blockers_complete",
    ...
    _require_equal("go_no_go.business execution req ready", go_no_go.get("actionable_resolution_final_blocker_recheck_ready_count"), 0)
    _require_equal("go_no_go.business execution req blocker", go_no_go.get("actionable_resolution_final_blocker_recheck_blocker_count"), 48)
    _require_equal("go_no_go.business execution req items", go_no_go.get("actionable_resolution_final_blocker_recheck_item_count"), 48)
    _require_equal("go_no_go.business execution req required", go_no_go.get("actionable_resolution_final_blocker_recheck_required_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_ACTIONABLE_RESOLUTION_FINAL_BLOCKER_RECHECK_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_ACTIONABLE_RESOLUTION_FINAL_BLOCKER_RECHECK_QUEUE_PATH)
    report = PRIVATE_ACTIONABLE_RESOLUTION_FINAL_BLOCKER_RECHECK_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("actionable_resolution_final_blocker_recheck_rows") or []),
        48,
    )
    _require_equal("private business execution requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 actionable resolution final blocker recheck before business execution 队列" not in report:
        raise ValidationError("private actionable resolution final blocker recheck report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            ...
            row.get("actionable_resolution_final_blocker_recheck_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true(
            "private business execution requirement required",
            row.get("actionable_resolution_final_blocker_recheck_required"),
        )
        _require_false(
            "private business execution requirement ready",
            row.get("actionable_resolution_final_blocker_recheck_ready"),
        )
        _require_true(
            "private business execution requirement blocker",
            row.get("actionable_resolution_final_blocker_recheck_blocker"),
        )
...
def validate(*, require_private_actionable_resolution_final_blocker_recheck: bool = False) -> dict[str, Any]:
    ...
    if require_private_actionable_resolution_final_blocker_recheck:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-actionable-resolution-final-blocker-recheck", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_actionable_resolution_final_blocker_recheck=args.require_private_actionable_resolution_final_blocker_recheck
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent actionable resolution final blocker recheck before business execution "
