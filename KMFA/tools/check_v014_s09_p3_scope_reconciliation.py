#!/usr/bin/env python3
"""Validate KMFA v0.1.4 S09-P3 scope reconciliation evidence."""
...
from KMFA.tools.v014_s09_p3_scope_reconciliation import (
    ACCEPTANCE_ID,
    MANIFEST_PATH,
    NEXT_REQUIRED_STEP,
    PHASE_SCOPE,
...
def validate_v014_s09_p3_scope_reconciliation(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    ...
    require(
        manifest.get("status") == "completed_validated_local_only_no_go_upload_deferred_scope_reconciliation",
        "status mismatch",
    )
    ...
    require(
        raw_boundary.get("raw_inbox_read_allowed_only_when_phase_requires") is True,
        "raw read policy mismatch",
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.4 S09-P3 scope reconciliation.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH)
    args = parser.parse_args(argv)

    result = validate_v014_s09_p3_scope_reconciliation(args.manifest)
    print(
        "PASS: KMFA v0.1.4 S09-P3 scope reconciliation validated "
