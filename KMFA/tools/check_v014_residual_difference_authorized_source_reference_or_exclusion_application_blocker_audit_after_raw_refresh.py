#!/usr/bin/env python3
"""Validate application-blocker-audit evidence after raw refresh."""
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_readiness_public_artifacts_read_by_this_phase",
        "source_readiness_manifest_read_by_this_phase",
        "source_readiness_go_no_go_read_by_this_phase",
        "source_readiness_matrix_read_by_this_phase",
        ...
        "source_private_readiness_report_read_by_this_phase",
        "private_application_blocker_audit_diagnostic_written_by_this_phase",
        "private_application_blocker_audit_records_written_by_this_phase",
        "private_application_blocker_audit_report_written_by_this_phase",
    ):
        _require_true(f"raw_boundary.{key}", raw_boundary.get(key))
    for key in (
        "source_private_readiness_diagnostic_mutated_by_this_phase",
        "source_private_ready_queue_mutated_by_this_phase",
        "source_private_blocker_queue_mutated_by_this_phase",
        "source_private_readiness_report_mutated_by_this_phase",
...
def _check_summary(summary: dict[str, Any], matrix: dict[str, Any], go_no_go: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    _require_equal(
        "summary.source_application_readiness_item_count",
        summary.get("source_application_readiness_item_count"),
        48,
    )
    _require_equal("summary.source_application_ready_item_count", summary.get("source_application_ready_item_count"), 0)
    _require_equal(
        "summary.source_application_blocker_item_count",
        summary.get("source_application_blocker_item_count"),
        48,
    )
    _require_equal("summary.source_private_ready_queue_count", summary.get("source_private_ready_queue_count"), 0)
    _require_equal("summary.source_private_blocker_queue_count", summary.get("source_private_blocker_queue_count"), 48)
    _require_equal("summary.application_blocker_audit_item_count", summary.get("application_blocker_audit_item_count"), 48)
    _require_equal(
        "summary.application_blocker_audit_ready_item_count",
        summary.get("application_blocker_audit_ready_item_count"),
        0,
    )
    _require_equal(
        "summary.source_reference_or_owner_exclusion_audit_blocker_count",
        summary.get("source_reference_or_owner_exclusion_audit_blocker_count"),
        40,
    )
    _require_equal(
        "summary.formula_or_non_numeric_mapping_audit_blocker_count",
        summary.get("formula_or_non_numeric_mapping_audit_blocker_count"),
        8,
    )
    _require_equal("summary.active_authoritative_decision_count", summary.get("active_authoritative_decision_count"), 0)
    _require_equal(
        "summary.binding_ready_after_application_blocker_audit_count",
        summary.get("binding_ready_after_application_blocker_audit_count"),
        0,
    )
    _require_equal(
        "summary.comparison_retry_ready_after_application_blocker_audit_count",
        summary.get("comparison_retry_ready_after_application_blocker_audit_count"),
        0,
    )
    _require_equal("summary.unresolved", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_application_blocker_audit_diagnostic_gitignored",
        "private_application_blocker_audit_records_gitignored",
        "private_application_blocker_audit_report_gitignored",
        "source_private_readiness_diagnostic_gitignored",
        "source_private_ready_queue_gitignored",
        "source_private_blocker_queue_gitignored",
        "source_private_readiness_report_gitignored",
    ):
    ...
    _require_equal("go_no_go.application_blocker_audit_item_count", go_no_go.get("application_blocker_audit_item_count"), 48)
    _require_equal("go_no_go.application_blocker_audit_ready_item_count", go_no_go.get("application_blocker_audit_ready_item_count"), 0)
...
def _check_private_audit(summary: dict[str, Any], require_private_audit: bool) -> None:
    if not require_private_audit:
        return
    for path in (PRIVATE_AUDIT_DIAGNOSTIC_PATH, PRIVATE_AUDIT_RECORDS_PATH, PRIVATE_AUDIT_REPORT_PATH):
        if not path.exists():
            raise ValidationError(f"missing private audit artifact: {path}")
        if not _git_check_ignored(path):
            raise ValidationError(f"private audit artifact is not ignored: {path}")
    diagnostic = _read_json(PRIVATE_AUDIT_DIAGNOSTIC_PATH)
    audit_rows = _read_jsonl(PRIVATE_AUDIT_RECORDS_PATH)
    _require_equal("private.diagnostic.phase_id", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal("private.diagnostic.application_blocker_audit_item_count", diagnostic.get("application_blocker_audit_item_count"), 48)
    _require_equal("private.audit.count", len(audit_rows), 48)
    track_counts = Counter(row.get("intake_track") for row in audit_rows)
    _require_equal("private.audit.source_reference_or_owner_exclusion", track_counts[SOURCE_REFERENCE_INTAKE_TRACK], 40)
    _require_equal("private.audit.formula_or_non_numeric_mapping", track_counts[FORMULA_MAPPING_INTAKE_TRACK], 8)
    blocker_codes = Counter(row.get("application_blocker_code") for row in audit_rows)
    _require_equal(
        "private.audit.source_ref_code",
        blocker_codes["missing_authoritative_source_reference_or_owner_exclusion_application"],
        40,
    )
    _require_equal(
        "private.audit.formula_code",
        blocker_codes["missing_formula_or_non_numeric_mapping_application"],
        8,
    )
    for row in audit_rows:
        _require_equal("private.audit.phase_id", row.get("phase_id"), PHASE_ID)
        _require_true(
            "private.audit.application_blocker_audit_performed",
            row.get("application_blocker_audit_performed_by_this_phase"),
        )
        _require_false("private.audit.binding_ready", row.get("binding_ready_after_application_blocker_audit"))
        _require_false("private.audit.comparison_retry", row.get("comparison_retry_ready_after_application_blocker_audit"))
        _require_false("private.audit.public_commit_allowed", row.get("public_commit_allowed"))
    _require_equal("summary.application_blocker_audit_item_count", summary.get("application_blocker_audit_item_count"), len(audit_rows))
...
def validate(*, require_private_audit: bool = False) -> dict[str, Any]:
    ...
    _check_private_audit(summary, require_private_audit)
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(
        "PASS: KMFA v0.1.4 authorized source reference or exclusion application blocker audit "
