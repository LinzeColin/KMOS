...
PRIVATE_DIR = PROJECT_ROOT / ".codex_private_runtime/v014_processed_value_source_map_completion_owner_group_decision_application"
PRIVATE_DIAGNOSTIC_PATH = PRIVATE_DIR / "private_owner_group_decision_application_diagnostic.json"
PRIVATE_PENDING_QUEUE_PATH = PRIVATE_DIR / "private_owner_group_decision_pending_queue.json"
...
PUBLIC_ARTIFACTS = [
    ...
    MANIFEST_PATH,
    GO_NO_GO_PATH,
    HUMAN_DIR / "processed_value_source_map_completion_owner_group_decision_application_report.md",
    HUMAN_DIR / "go_no_go_record.md",
    HUMAN_DIR / "test_results.md",
    HUMAN_DIR / "risk_register.md",
    HUMAN_DIR / "rollback_plan.md",
    QUALITY_DIR / "v014_processed_value_source_map_completion_owner_group_decision_application_summary.json",
    QUALITY_DIR / "v014_processed_value_source_map_completion_owner_group_decision_application_manifest.json",
    QUALITY_DIR / "v014_processed_value_source_map_completion_owner_group_decision_application_go_no_go_report.json",
]
...
def _check_public_safety(safety: dict[str, Any]) -> None:
    ...
    for key in (
        ...
        "private_application_diagnostic_committed",
...
def _check_private_diagnostic() -> None:
    ...
    _require_equal("diagnostic.pending_group_decision_row_count", diagnostic.get("pending_group_decision_row_count"), 113)
    _require_equal("diagnostic.valid_group_decision_row_count", diagnostic.get("valid_group_decision_row_count"), 0)
    _require_equal("diagnostic.invalid_group_decision_row_count", diagnostic.get("invalid_group_decision_row_count"), 0)
    _require_equal(
        "diagnostic.pending code count",
        diagnostic.get("owner_decision_code_counts", {}).get(PENDING_DECISION_CODE),
        113,
    )
    ...
    _require_false("diagnostic.active_written", diagnostic.get("active_owner_authorized_fill_record_written"))
    _require_false(
    ...
    _require_equal("pending_queue.review_group_count", pending_queue.get("review_group_count"), 22)
    _require_equal("pending_queue.pending_group_decision_row_count", pending_queue.get("pending_group_decision_row_count"), 113)
    ...
    if not isinstance(rows, list):
        raise ValidationError("pending_queue.pending_rows must be a list")
    _require_equal("pending_queue.pending_rows length", len(rows), 113)
...
def validate(*, require_private_diagnostic: bool = False) -> dict[str, Any]:
    ...
    _require_equal("summary.source_review_groups_phase_id", summary.get("source_review_groups_phase_id"), "V014_PROCESSED_VALUE_SOURCE_MAP_COMPLETION_OWNER_REVIEW_GROUPS_PATH")
    _require_true("summary.source_review_groups_path_prepared", summary.get("source_review_groups_path_prepared"))
    ...
    _require_equal("summary.pending_group_decision_row_count", summary.get("pending_group_decision_row_count"), 113)
    _require_equal("summary.valid_group_decision_row_count", summary.get("valid_group_decision_row_count"), 0)
    _require_equal("summary.invalid_group_decision_row_count", summary.get("invalid_group_decision_row_count"), 0)
    ...
    _require_equal("go_no_go.pending_group_decision_row_count", go_no_go.get("pending_group_decision_row_count"), 113)
    _require_equal("go_no_go.valid_group_decision_row_count", go_no_go.get("valid_group_decision_row_count"), 0)
    _require_equal("go_no_go.invalid_group_decision_row_count", go_no_go.get("invalid_group_decision_row_count"), 0)
...
def main() -> None:
    ...
    print(
        "PASS: KMFA v0.1.4 owner group decision application validated "
