#!/usr/bin/env python3
"""Validate owner or authorized-agent external action final-threshold blocked handoff action readiness after final-threshold handoff."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_final_threshold_blocked_handoff_action_readiness_after_final_threshold_handoff import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_private_owner_action_queue_read_by_this_phase",
        "source_private_blocked_handoff_report_existence_checked_by_this_phase",
        "private_external_action_readiness_diagnostic_written_by_this_phase",
        "private_external_action_readiness_blocker_records_written_by_this_phase",
        "private_external_action_readiness_question_list_written_by_this_phase",
    )
    false_keys = (
        ...
        "source_private_owner_action_queue_mutated_by_this_phase",
        "source_private_blocked_handoff_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.readiness", summary.get("readiness_conclusion"), READINESS_CONCLUSION)
    ...
    _require_equal(
        "summary.source final-threshold blocked handoff",
        summary.get("source_external_action_final_threshold_blocked_handoff_item_count"),
        48,
    )
    _require_equal("summary.source owner action packet", summary.get("source_owner_action_packet_item_count"), 48)
    _require_equal("summary.source owner action required", summary.get("source_owner_action_required_count"), 48)
    _require_equal("summary.private handoff rows", summary.get("source_private_blocked_handoff_records_item_count"), 48)
    _require_equal("summary.private owner action rows", summary.get("source_private_owner_action_queue_item_count"), 48)
    _require_equal("summary.source goal", summary.get("source_goal_status_recommendation"), "blocked")
    _require_true("summary.source threshold", summary.get("source_external_action_blocked_audit_threshold_met"))
    _require_equal("summary.goal", summary.get("goal_status_recommendation"), "blocked")
    _require_equal("summary.owner ready", summary.get("external_owner_action_ready_count"), 0)
    _require_equal("summary.owner blocker", summary.get("external_owner_action_blocker_count"), 48)
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_external_action_blocker_count"),
        40,
    )
    _require_equal(
        "summary.formula blockers",
        summary.get("formula_or_non_numeric_mapping_external_action_blocker_count"),
        8,
    )
    _require_equal("summary.binding ready", summary.get("binding_ready_after_external_action_readiness_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_external_action_readiness_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_external_action_readiness_diagnostic_written",
        "private_external_action_readiness_blocker_records_written",
        "private_external_action_readiness_question_list_written",
        "private_external_action_readiness_diagnostic_gitignored",
        "private_external_action_readiness_blocker_records_gitignored",
        "private_external_action_readiness_question_list_gitignored",
        "source_private_blocked_handoff_diagnostic_gitignored",
        "source_private_blocked_handoff_records_gitignored",
        "source_private_owner_action_queue_gitignored",
        "external_action_readiness_checked_by_this_phase",
    ):
    ...
    for code in (
        "source_blocked_handoff_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_blocked_handoff_complete",
        "source_owner_action_queue_complete",
        "private_owner_action_queue_read",
        "external_action_readiness_checked",
        "external_owner_action_blockers_locked",
    ...
    _require_equal("go_no_go.readiness", go_no_go.get("readiness_conclusion"), READINESS_CONCLUSION)
    _require_equal("go_no_go.source blocked handoff", go_no_go.get("source_blocked_handoff_item_count"), 48)
    _require_equal("go_no_go.source owner action", go_no_go.get("source_owner_action_item_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_EXTERNAL_ACTION_READINESS_DIAGNOSTIC_PATH)
    blocker_rows = _read_jsonl(PRIVATE_EXTERNAL_ACTION_READINESS_BLOCKER_RECORDS_PATH)
    question_list = PRIVATE_EXTERNAL_ACTION_READINESS_QUESTION_LIST_PATH.read_text(encoding="utf-8")
    ...
    if "owner/授权代理 external action readiness 问题清单" not in question_list:
        raise ValidationError("private question list missing Chinese readiness heading")
    ...
    for row in blocker_rows:
        _require_equal("private readiness status", row.get("external_action_readiness_status"), READINESS_CONCLUSION)
        ...
        _require_false("private binding", row.get("binding_ready_after_external_action_readiness"))
        _require_false("private retry", row.get("comparison_retry_ready_after_external_action_readiness"))
        _require_false("private public commit", row.get("public_commit_allowed"))


def validate(*, require_private_external_action_readiness: bool = False) -> dict[str, Any]:
    ...
    if require_private_external_action_readiness:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-external-action-readiness", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(require_private_external_action_readiness=args.require_private_external_action_readiness)
    except ValidationError as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1
    summary = manifest["summary"]
    print(
        "PASS: validated V014 owner/authorized-agent external action final-threshold blocked handoff action readiness after final-threshold handoff "
