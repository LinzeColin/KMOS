#!/usr/bin/env python3
"""Validate owner-authorized anchor blocker final threshold artifacts."""
...
from KMFA.tools.v014_residual_difference_owner_authorized_anchor_confirmation_blocker_final_threshold_recheck import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_anchor_blocker_threshold_summary_read_by_this_phase",
        "source_public_anchor_blocker_threshold_manifest_read_by_this_phase",
        "source_public_anchor_blocker_threshold_go_no_go_read_by_this_phase",
        "source_public_anchor_blocker_threshold_matrix_read_by_this_phase",
        "source_private_anchor_blocker_threshold_diagnostic_read_by_this_phase",
        "source_private_anchor_blocker_threshold_queue_read_by_this_phase",
        "source_private_anchor_blocker_threshold_report_read_by_this_phase",
        "private_final_threshold_diagnostic_written_by_this_phase",
        "private_final_threshold_queue_written_by_this_phase",
...
def _check_summary(summary: dict[str, Any], matrix: dict[str, Any], go_no_go: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    _require_equal(
        ...
        "V014_RESIDUAL_DIFFERENCE_OWNER_AUTHORIZED_ANCHOR_CONFIRMATION_BLOCKER_THRESHOLD_RECHECK",
    )
    ...
    _require_equal("summary.source observation", summary.get("source_owner_authorized_anchor_blocker_observation_count"), 2)
    ...
    _require_equal("summary.source private threshold rows", summary.get("source_private_threshold_queue_item_count"), 72)
    _require_true("summary.final threshold recheck performed", summary.get("owner_authorized_anchor_blocker_final_threshold_recheck_performed"))
    _require_equal("summary.prior observation", summary.get("prior_owner_authorized_anchor_blocker_observation_count"), 2)
    _require_equal("summary.current observation", summary.get("owner_authorized_anchor_blocker_observation_count"), 3)
    _require_true("summary.threshold", summary.get("owner_authorized_anchor_blocked_audit_threshold_met"))
    ...
    _require_equal("go_no_go.observation", go_no_go.get("owner_authorized_anchor_blocker_observation_count"), 3)
    _require_true("go_no_go.threshold", go_no_go.get("owner_authorized_anchor_blocked_audit_threshold_met"))
    _require_equal("go_no_go.goal status", go_no_go.get("goal_status_recommendation"), "blocked")
...
def _check_private_artifacts() -> None:
    ...
    final_rows = _read_jsonl(PRIVATE_FINAL_THRESHOLD_QUEUE_PATH)
    _require_equal(
        "source diagnostic observation",
        source_diagnostic.get("owner_authorized_anchor_blocker_observation_count"),
        2,
    )
    _require_false(
        "source diagnostic threshold",
        source_diagnostic.get("owner_authorized_anchor_blocked_audit_threshold_met"),
    )
    ...
    _require_equal(
        "final diagnostic observation",
        final_diagnostic.get("owner_authorized_anchor_blocker_observation_count"),
        3,
    )
    _require_true(
        "final diagnostic threshold",
        final_diagnostic.get("owner_authorized_anchor_blocked_audit_threshold_met"),
    )
    _require_equal("final rows", len(final_rows), 72)
    _require_equal(
        "private target sets",
    ...
    _require_equal(
        "final row track counts",
        dict(Counter(row.get("diagnostic_track") for row in final_rows)),
        EXPECTED_TRACK_COUNTS,
    )
    for row in source_rows:
        _require_false("source threshold anchor ready", row.get("anchor_confirmation_ready_after_threshold_recheck"))
        _require_false(
            "source threshold raw comparison ready",
            row.get("raw_to_processed_value_comparison_ready_after_threshold_recheck"),
        )
        _require_false(
            "source threshold business consistency",
            row.get("business_value_consistency_verified_after_threshold_recheck"),
        )
        _require_false("source threshold met", row.get("threshold_met_after_this_phase"))
        _require_false("source public", row.get("public_commit_allowed"))
        _require_equal(
            "source threshold observation",
            row.get("owner_authorized_anchor_blocker_observation_count_after_this_phase"),
            2,
        )
    for row in final_rows:
        _require_false("final anchor ready", row.get("anchor_confirmation_ready_after_final_threshold_recheck"))
        _require_false(
            "final raw comparison ready",
            row.get("raw_to_processed_value_comparison_ready_after_final_threshold_recheck"),
        )
        _require_false(
            "final business consistency",
            row.get("business_value_consistency_verified_after_final_threshold_recheck"),
        )
        _require_true("final threshold met", row.get("threshold_met_after_this_phase"))
        _require_false("final public", row.get("public_commit_allowed"))
        _require_equal(
            "final observation",
            row.get("owner_authorized_anchor_blocker_observation_count_after_this_phase"),
            3,
...
def main() -> int:
    ...
    print(
        "PASS: KMFA v0.1.4 owner-authorized anchor blocker final threshold artifacts validated "
