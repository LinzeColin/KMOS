#!/usr/bin/env python3
"""Validate the external-action requirement gate before authoritative binding."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_external_action_required_before_authoritative_binding import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_follow_up_public_artifacts_read_by_this_phase",
        "source_private_follow_up_queue_read_by_this_phase",
        "private_authoritative_binding_requirement_diagnostic_written_by_this_phase",
        "private_authoritative_binding_requirement_queue_written_by_this_phase",
        "private_authoritative_binding_requirement_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_follow_up_diagnostic_mutated_by_this_phase",
        "source_private_follow_up_queue_mutated_by_this_phase",
        "source_private_follow_up_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_follow_up.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_follow_up.PHASE_ID)
    ...
    _require_equal("summary.source ready", summary.get("source_external_action_follow_up_ready_count"), 0)
    _require_equal("summary.source blockers", summary.get("source_external_action_follow_up_blocker_count"), 48)
    _require_equal("summary.source required", summary.get("source_external_action_follow_up_required_count"), 48)
    _require_equal(
        "summary.source private follow-up rows",
        summary.get("source_private_external_action_follow_up_queue_item_count"),
        48,
    )
    ...
    _require_equal("summary.binding req ready", summary.get("authoritative_binding_requirement_ready_count"), 0)
    _require_equal("summary.binding req blockers", summary.get("authoritative_binding_requirement_blocker_count"), 48)
    _require_equal("summary.binding req required", summary.get("authoritative_binding_requirement_required_count"), 48)
    _require_equal("summary.actionable owner resolution", summary.get("actionable_owner_resolution_count"), 0)
    _require_equal(
        "summary.source ref binding requirement",
        summary.get("source_reference_or_owner_exclusion_binding_requirement_count"),
        40,
    )
    _require_equal(
        "summary.formula binding requirement",
        summary.get("formula_or_non_numeric_mapping_binding_requirement_count"),
        8,
    )
    ...
    for key in (
        "private_authoritative_binding_requirement_diagnostic_written",
        "private_authoritative_binding_requirement_queue_written",
        "private_authoritative_binding_requirement_report_written",
        "private_authoritative_binding_requirement_diagnostic_gitignored",
        "private_authoritative_binding_requirement_queue_gitignored",
        "private_authoritative_binding_requirement_report_gitignored",
        "source_private_external_action_follow_up_diagnostic_gitignored",
        "source_private_external_action_follow_up_queue_gitignored",
        "source_private_external_action_follow_up_report_gitignored",
    ...
    for code in (
        "source_follow_up_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_follow_up_blockers_complete",
        "source_private_follow_up_queue_read",
        "binding_requirement_queue_complete",
        "binding_requirement_blockers_locked",
    ...
    _require_equal("go_no_go.binding req ready", go_no_go.get("authoritative_binding_requirement_ready_count"), 0)
    _require_equal("go_no_go.binding req blocker", go_no_go.get("authoritative_binding_requirement_blocker_count"), 48)
    _require_equal("go_no_go.binding req required", go_no_go.get("authoritative_binding_requirement_required_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_AUTHORITATIVE_BINDING_REQUIREMENT_DIAGNOSTIC_PATH)
    requirement_rows = _read_jsonl(PRIVATE_AUTHORITATIVE_BINDING_REQUIREMENT_QUEUE_PATH)
    report = PRIVATE_AUTHORITATIVE_BINDING_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal(
        "private diagnostic rows",
        len(diagnostic.get("authoritative_binding_requirement_rows") or []),
        48,
    )
    _require_equal("private binding requirement row count", len(requirement_rows), 48)
    if "owner/授权代理 external action required before authoritative binding 队列" not in report:
        raise ValidationError("private authoritative-binding requirement report missing Chinese heading")
    ...
    for row in requirement_rows:
        _require_equal(
            "private binding requirement status",
            row.get("authoritative_binding_requirement_status"),
            REQUIREMENT_CONCLUSION,
        )
        _require_true("private binding requirement required", row.get("authoritative_binding_requirement_required"))
        _require_false("private binding requirement ready", row.get("authoritative_binding_requirement_ready"))
        _require_true("private binding requirement blocker", row.get("authoritative_binding_requirement_blocker"))
...
def validate(*, require_private_authoritative_binding_requirement: bool = False) -> dict[str, Any]:
    ...
    if require_private_authoritative_binding_requirement:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-authoritative-binding-requirement", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(
            require_private_authoritative_binding_requirement=args.require_private_authoritative_binding_requirement
        )
    ...
    print(
        "PASS: validated V014 owner/authorized-agent external action required before authoritative binding "
