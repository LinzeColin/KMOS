#!/usr/bin/env python3
"""Validate application-blocker-threshold evidence after raw refresh."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_blocker_threshold_recheck_after_raw_refresh import (  # noqa: E402
    ACCEPTANCE_ID,
    DECISION,
    ...
    RISK_REGISTER_PATH,
    ROLLBACK_PATH,
    SOURCE_APPLICATION_BLOCKER_AUDIT_GO_NO_GO_PATH,
    SOURCE_APPLICATION_BLOCKER_AUDIT_MANIFEST_PATH,
    SOURCE_APPLICATION_BLOCKER_AUDIT_MATRIX_PATH,
    SOURCE_APPLICATION_BLOCKER_AUDIT_SUMMARY_PATH,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_application_blocker_audit_public_artifacts_read_by_this_phase",
        "source_application_blocker_audit_manifest_read_by_this_phase",
        "source_application_blocker_audit_go_no_go_read_by_this_phase",
        "source_application_blocker_audit_matrix_read_by_this_phase",
        "source_private_application_blocker_audit_diagnostic_read_by_this_phase",
        "source_private_application_blocker_audit_records_read_by_this_phase",
        "source_private_application_blocker_audit_report_read_by_this_phase",
        "private_application_blocker_threshold_diagnostic_written_by_this_phase",
        "private_application_blocker_threshold_records_written_by_this_phase",
        "private_application_blocker_threshold_report_written_by_this_phase",
    ):
    ...
    for key in (
        ...
        "source_private_application_blocker_audit_diagnostic_mutated_by_this_phase",
        "source_private_application_blocker_audit_records_mutated_by_this_phase",
        "source_private_application_blocker_audit_report_mutated_by_this_phase",
    ):
...
def _check_summary(
    ...
    _require_equal("summary.source audit items", summary.get("source_application_blocker_audit_item_count"), 48)
    _require_equal("summary.source audit ready", summary.get("source_application_blocker_audit_ready_item_count"), 0)
    _require_equal("summary.source private rows", summary.get("source_private_application_blocker_audit_record_count"), 48)
    _require_equal("summary.source ref audit blockers", summary.get("source_reference_or_owner_exclusion_audit_blocker_count"), 40)
    _require_equal("summary.formula audit blockers", summary.get("formula_or_non_numeric_mapping_audit_blocker_count"), 8)
    _require_equal("summary.active decisions", summary.get("active_authoritative_decision_count"), 0)
    _require_equal("summary.prior observation", summary.get("prior_application_blocker_observation_count"), 1)
    _require_equal("summary.current observation", summary.get("application_blocker_observation_count"), 2)
    _require_false("summary.threshold", summary.get("application_blocked_audit_threshold_met"))
    _require_equal(
    ...
    _require_true("summary.threshold recheck performed", summary.get("application_blocker_threshold_recheck_performed_by_this_phase"))
    _require_equal("summary.threshold items", summary.get("application_blocker_threshold_recheck_item_count"), 48)
    _require_equal(
    ...
    _require_equal("summary.binding ready", summary.get("binding_ready_after_threshold_recheck_count"), 0)
    ...
    for key in (
        "private_application_blocker_threshold_diagnostic_written",
        "private_application_blocker_threshold_records_written",
        "private_application_blocker_threshold_report_written",
        "private_application_blocker_threshold_diagnostic_gitignored",
        "private_application_blocker_threshold_records_gitignored",
        "private_application_blocker_threshold_report_gitignored",
        "source_private_application_blocker_audit_diagnostic_gitignored",
        "source_private_application_blocker_audit_records_gitignored",
        "source_private_application_blocker_audit_report_gitignored",
    ):
    ...
    for code in (
        ...
        "source_matrix_clean",
        "source_audit_item_count_locked",
        "source_audit_ready_zero",
        "source_private_audit_records_loaded",
        "threshold_recheck_item_count_locked",
        "observation_count_recorded",
        "strict_threshold_not_met",
        "distribution_locked",
        "formula_distribution_locked",
        "no_binding_or_retry_ready_after_threshold",
    ...
    _require_equal("go_no_go.prior observation", go_no_go.get("prior_application_blocker_observation_count"), 1)
    _require_equal("go_no_go.current observation", go_no_go.get("application_blocker_observation_count"), 2)
    _require_false("go_no_go.threshold", go_no_go.get("application_blocked_audit_threshold_met"))
    _require_equal("go_no_go.threshold items", go_no_go.get("application_blocker_threshold_recheck_item_count"), 48)
    _require_equal("go_no_go.binding ready", go_no_go.get("binding_ready_after_threshold_recheck_count"), 0)
...
def _check_private_artifacts(summary: dict[str, Any]) -> None:
    ...
    _require_equal("source diagnostic audit items", source_diagnostic.get("application_blocker_audit_item_count"), 48)
    _require_equal("source rows", len(source_rows), 48)
    _require_equal(
        "threshold diagnostic items",
        threshold_diagnostic.get("application_blocker_threshold_recheck_item_count"),
        48,
    )
    _require_equal("threshold rows", len(threshold_rows), 48)
    _require_equal(
        "source slots vs threshold slots",
    ...
    threshold_track_counts = Counter(row.get("intake_track") for row in threshold_rows)
    for track, expected in EXPECTED_THRESHOLD_TRACK_COUNTS.items():
    ...
    _require_equal("summary source ref threshold", summary.get("source_reference_or_owner_exclusion_threshold_blocker_count"), 40)
    _require_equal("summary formula threshold", summary.get("formula_or_non_numeric_mapping_threshold_blocker_count"), 8)
    _require_equal("threshold source ref track", threshold_track_counts[SOURCE_REFERENCE_INTAKE_TRACK], 40)
    _require_equal("threshold formula track", threshold_track_counts[FORMULA_MAPPING_INTAKE_TRACK], 8)

    for row in threshold_rows:
        _require_equal(
            "threshold status",
            row.get("threshold_recheck_status"),
            "application_blocker_observation_2_missing_authorized_resolution_application",
        )
        _require_equal("prior observation", row.get("prior_application_blocker_observation_count"), 1)
        _require_equal("current observation", row.get("application_blocker_observation_count"), 2)
        _require_false("threshold met", row.get("application_blocked_audit_threshold_met"))
        _require_false("resolution available", row.get("authoritative_resolution_application_available"))
        _require_false("binding ready", row.get("binding_ready_after_threshold_recheck"))
...
def main() -> int:
    ...
    print(
        ...
        "threshold recheck after raw refresh artifacts validated"
    )
    return 0
