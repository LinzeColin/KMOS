#!/usr/bin/env python3
"""Validate KMFA v0.1.4 S08-P2 business entity model evidence."""
...
from KMFA.tools.business_entity_model import LIFECYCLE_STATUSES, REQUIRED_ENTITY_TYPES
from KMFA.tools.v014_s08_p2_business_entity_model import (
    ACCEPTANCE_ID,
    MANIFEST_PATH,
    NEXT_INSTRUCTION,
    NEXT_PHASE,
...
VALIDATION_KEYS = (
    "py_compile",
    "s08_p1_dependency_validator",
    "legacy_s08_p2_generator",
    "legacy_s08_p2_validator",
    "legacy_s08_p2_unit",
    "v014_s08_p2_generator",
    "v014_s08_p2_validator",
    "focused_unit_test",
    "no_omission_check",
...
def validate_v014_s08_p2_business_entity_model(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    manifest = read_json(manifest_path)
    dependency = validate_s08_p1_dependency()
    ...
    require(manifest.get("phase_id") == "S08-P2", "phase_id must be S08-P2", errors)
    ...
    require(
        manifest.get("status") == "completed_validated_local_only_no_go_upload_deferred_business_entity_model",
        "status mismatch",
        errors,
    )
    require(dependency.get("phase_id") == "S08-P1", "S08-P1 dependency did not validate", errors)
    require(manifest.get("s08_p1_dependency_validated") is True, "S08-P1 dependency flag mismatch", errors)
    require(manifest.get("legacy_s08_p2_dependency_validated") is True, "legacy S08-P2 flag mismatch", errors)
    require(manifest.get("completed_task_ids") == ["S08P2T01", "S08P2T02", "S08P2T03"], "task ids mismatch", errors)
    ...
    summary = manifest.get("business_entity_summary", {})
    require(summary.get("required_entity_type_count") == 8, "entity type count mismatch", errors)
    require(tuple(summary.get("required_entity_types", [])) == REQUIRED_ENTITY_TYPES, "entity types mismatch", errors)
    require(summary.get("relationship_count") == 14, "relationship count mismatch", errors)
    require(summary.get("lifecycle_status_count") == 32, "lifecycle status count mismatch", errors)
    require(summary.get("lifecycle_status_per_entity_count") == 4, "lifecycle status per entity mismatch", errors)
    require(summary.get("schema_entity_definition_count") == 8, "schema entity definition count mismatch", errors)
    require(summary.get("relationship_graph_required_links_present") is True, "relationship graph mismatch", errors)
    require(summary.get("relationship_graph_required_link_count") == 7, "relationship graph count mismatch", errors)
    require(tuple(summary.get("lifecycle_statuses", [])) == LIFECYCLE_STATUSES, "lifecycle statuses mismatch", errors)
    ...
    require(policy.get("private_ref_required") is True, "private ref policy mismatch", errors)
    require(policy.get("entity_values_hash_ref_only") is True, "entity hash/ref policy mismatch", errors)
    require(policy.get("relationship_values_schema_only") is True, "relationship schema policy mismatch", errors)
    require(policy.get("lifecycle_values_status_only") is True, "lifecycle status policy mismatch", errors)
    require(policy.get("relationship_graph_required_links_present") is True, "relationship graph policy mismatch", errors)
    ...
    require(boundaries.get("s08_p1_dependency_validated") is True, "S08-P1 boundary mismatch", errors)
    require(boundaries.get("s08_p2_scope_included") is True, "S08-P2 scope mismatch", errors)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.4 S08-P2 business entity model evidence.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH)
    args = parser.parse_args(argv)
    try:
        result = validate_v014_s08_p2_business_entity_model(args.manifest)
    except ValidationError as exc:
        print("FAIL: KMFA v0.1.4 S08-P2 business entity model validation failed")
        print(exc)
        return 1
    summary = result["business_entity_summary"]
    print(
        "PASS: KMFA v0.1.4 S08-P2 business entity model validated "
