#!/usr/bin/env python3
"""Validate generated owner/agent diagnostic response actionability recheck."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_generated_diagnostic_response_actionability_recheck import (  # noqa: E402
    ACCEPTANCE_ID,
    ...
    SOURCE_PRIVATE_RESPONSE_REPORT_PATH,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    for key in (
        ...
        "source_public_generated_response_summary_read_by_this_phase",
        "source_public_generated_response_manifest_read_by_this_phase",
        "source_public_generated_response_go_no_go_read_by_this_phase",
        "source_public_generated_response_matrix_read_by_this_phase",
        "source_private_response_items_read_by_this_phase",
        "source_private_non_actionable_queue_read_by_this_phase",
        "source_private_response_report_existence_checked_by_this_phase",
        "private_actionability_diagnostic_written_by_this_phase",
        "private_actionability_blocker_queue_written_by_this_phase",
        "private_actionability_report_written_by_this_phase",
    ):
    ...
    for key in (
        ...
        "source_private_response_items_mutated_by_this_phase",
        "source_private_non_actionable_queue_mutated_by_this_phase",
        "source_private_response_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.actionability_conclusion", summary.get("actionability_conclusion"), ACTIONABILITY_CONCLUSION)
    _require_equal("summary.source_phase_id", summary.get("source_phase_id"), source_import.PHASE_ID)
    _require_equal("summary.source_manifest_phase_id", summary.get("source_manifest_phase_id"), source_import.PHASE_ID)
    ...
    _require_equal("summary.source valid", summary.get("source_valid_diagnostic_response_count"), 48)
    _require_equal("summary.source non actionable", summary.get("source_non_actionable_diagnostic_response_count"), 48)
    _require_equal("summary.source response items", summary.get("source_private_response_item_count"), 48)
    _require_equal("summary.source non actionable queue", summary.get("source_private_non_actionable_queue_item_count"), 48)
    _require_equal("summary.actionability recheck", summary.get("actionability_recheck_item_count"), 48)
    _require_equal("summary.actionability ready", summary.get("actionability_ready_count"), 0)
    _require_equal("summary.actionability blocker", summary.get("actionability_blocker_count"), 48)
    ...
    _require_equal("summary.binding ready", summary.get("binding_ready_after_actionability_recheck_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_actionability_recheck_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_actionability_diagnostic_written",
        "private_actionability_blocker_queue_written",
        "private_actionability_report_written",
        "private_actionability_diagnostic_gitignored",
        "private_actionability_blocker_queue_gitignored",
        "private_actionability_report_gitignored",
        "source_private_response_items_gitignored",
        "source_private_non_actionable_queue_gitignored",
        "source_private_response_report_gitignored",
        "owner_or_agent_valid_response_available",
        "generated_diagnostic_response_actionability_recheck_performed",
    ):
    ...
    for code in (
        "source_generated_response_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_valid_responses_complete",
        "source_non_actionable_queue_complete",
        "source_private_response_items_complete",
        "source_private_non_actionable_queue_complete",
        "actionability_recheck_complete",
        "all_generated_responses_remain_non_actionable",
    ...
    _require_equal("go_no_go.actionability_conclusion", go_no_go.get("actionability_conclusion"), ACTIONABILITY_CONCLUSION)
    _require_equal("go_no_go.valid", go_no_go.get("valid_diagnostic_response_count"), 48)
    _require_equal("go_no_go.recheck", go_no_go.get("actionability_recheck_item_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    non_actionable = _read_jsonl(SOURCE_PRIVATE_NON_ACTIONABLE_QUEUE_PATH)
    diagnostic = _read_json(PRIVATE_ACTIONABILITY_DIAGNOSTIC_PATH)
    actionability = _read_jsonl(PRIVATE_ACTIONABILITY_BLOCKER_QUEUE_PATH)
    _require_equal("response item rows", len(response_items), 48)
    _require_equal("non actionable rows", len(non_actionable), 48)
    _require_equal("diagnostic actionability rows", len(diagnostic.get("actionability_rows", [])), 48)
    _require_equal("actionability rows", len(actionability), 48)
    response_ids = {row.get("generated_response_item_id") for row in response_items}
    non_actionable_ids = {row.get("generated_response_item_id") for row in non_actionable}
    actionability_ids = {row.get("source_generated_response_item_id") for row in actionability}
    _require_equal("response/non actionable sets", response_ids, non_actionable_ids)
    _require_equal("response/actionability sets", response_ids, actionability_ids)
    diagnostic_counts = Counter(row.get("diagnostic_kind") for row in actionability)
    _require_equal(
    ...
    for row in actionability:
        _require_equal("actionability status", row.get("actionability_status"), "blocked_non_actionable_generated_response")
        ...
        _require_false("binding", row.get("binding_ready_after_actionability_recheck"))
        _require_false("comparison", row.get("comparison_retry_ready_after_actionability_recheck"))
...
def validate(*, require_private_actionability: bool = False) -> dict[str, Any]:
    ...
    if require_private_actionability:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-actionability", action="store_true")
    args = parser.parse_args(argv)
    try:
        validate(require_private_actionability=args.require_private_actionability)
    except ValidationError as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1
    print("PASS: validated V014 generated owner/agent diagnostic response actionability recheck")
    return 0
