...
<head>
  ...
  <style>
    *{{box-sizing:border-box}} body{{margin:0;background:#f2f5f7;color:#172522;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;letter-spacing:0}} button,a{{font:inherit}}
    ...
    .footer-band{{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:13px 16px;background:#f6f8f7;border-top:1px solid #d9e1de}} .footer-band p{{margin:0;color:#5b6e68;font-size:13px}} .footer-band a{{color:#136c94;text-decoration:none;margin-left:12px;font-size:13px}} footer{{color:#687b75;font-size:12px;line-height:1.7}} @media(max-width:720px){{.topbar-inner,.headline,.footer-band{{display:block}} .top-links{{justify-content:flex-start;margin-top:12px}} main{{padding:18px 12px 36px}} h1{{font-size:24px}} .badges{{margin-top:12px}} .metrics{{grid-template-columns:1fr 1fr}} .metric:nth-child(2){{border-right:0}} .metric:nth-child(-n+2){{border-bottom:1px solid #cfd9d6}} .method-layout{{grid-template-columns:1fr}} .method-nav{{border-right:0;border-bottom:1px solid #d9e1de;display:grid;grid-template-columns:repeat(3,1fr);gap:5px}} .method-nav button{{text-align:center;padding:7px 4px}} dl,.cash-grid{{grid-template-columns:1fr}} .cash-item{{border-right:0;border-bottom:1px solid #d9e1de}} .footer-band .links{{margin-top:10px}} .footer-band a{{display:inline-block;margin:4px 10px 4px 0}}}}
  </style>
</head>
<body data-page-id="s14-p2-invoice-tax" data-active-method="pending_invoice" data-ui-ready="false">
  <header class="topbar"><div class="topbar-inner"><div class="brand"><div class="mark">KM</div><div><strong>KMFA 经营分析系统</strong><span>S14-P2 · 财务税务</span></div></div><nav class="top-links" aria-label="阶段页面导航">{links}</nav></div></header>
  <main>
    ...
    <footer>本 phase 仅完成 S14-P2；S14-P3、Stage 14 整体复审、发票开具、纳税申报、GitHub upload、app reinstall、正式报告、差异关闭和业务执行均未执行。</footer>
  </main>
  <script>
    const labels={labels};
