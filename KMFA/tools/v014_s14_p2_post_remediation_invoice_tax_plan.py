#!/usr/bin/env python3
"""Generate current public-safe KMFA v0.1.4 S14-P2 evidence."""
...
PHASE_ID = "V014_S14_P2_POST_REMEDIATION_INVOICE_TAX_PLAN"
ROADMAP_PHASE_ID = "S14-P2"
TASK_ID = "KMFA-V014-S14-P2-POST-REMEDIATION-INVOICE-TAX-PLAN-20260711"
ACCEPTANCE_ID = "ACC-V014-S14-P2-POST-REMEDIATION-INVOICE-TAX-PLAN"
...
DECISION = "NO_GO"
FORMULA_ID = "FORM-KMFA-V014-S14P2-POST-REMEDIATION-INVOICE-TAX-PLAN-001"
PARAMETER_IDS = (
    "PARAM-KMFA-1754",
    "PARAM-KMFA-1755",
    "PARAM-KMFA-1756",
...
MODEL_REGISTRY_KEY = "kmfa_v014_s14_p2_post_remediation_invoice_tax_plan"

OUTPUT_DIR = Path("KMFA/stage_artifacts") / PHASE_ID
MACHINE_DIR = OUTPUT_DIR / "machine"
HUMAN_DIR = OUTPUT_DIR / "human"
HTML_DIR = OUTPUT_DIR / "exports/html"
SUMMARY_PATH = MACHINE_DIR / "invoice_tax_plan_summary.json"
MANIFEST_PATH = MACHINE_DIR / "invoice_tax_plan_manifest.json"
LANES_PATH = MACHINE_DIR / "source_lane_status_public_safe.json"
ISSUE_METHODS_PATH = MACHINE_DIR / "issue_review_method_definitions_public_safe.json"
CASH_METHODS_PATH = MACHINE_DIR / "cash_summary_method_definitions_public_safe.json"
MATRIX_PATH = MACHINE_DIR / "invoice_tax_plan_acceptance_matrix.json"
GO_NO_GO_PATH = MACHINE_DIR / "invoice_tax_plan_go_no_go.json"
HTML_PATH = HTML_DIR / "invoice_tax_plan_workbench.html"
REPORT_PATH = HUMAN_DIR / "invoice_tax_plan_report_zh.md"
...
METADATA_SUMMARY_PATH = QUALITY_DIR / "v014_s14_p2_post_remediation_invoice_tax_plan_summary.json"
METADATA_MANIFEST_PATH = QUALITY_DIR / "v014_s14_p2_post_remediation_invoice_tax_plan_manifest.json"
METADATA_LANES_PATH = QUALITY_DIR / "v014_s14_p2_post_remediation_source_lane_status_public_safe.json"
METADATA_ISSUE_METHODS_PATH = (
    QUALITY_DIR / "v014_s14_p2_post_remediation_issue_review_method_definitions_public_safe.json"
)
METADATA_CASH_METHODS_PATH = (
    QUALITY_DIR / "v014_s14_p2_post_remediation_cash_summary_method_definitions_public_safe.json"
)
METADATA_MATRIX_PATH = QUALITY_DIR / "v014_s14_p2_post_remediation_invoice_tax_plan_acceptance_matrix.json"
METADATA_GO_NO_GO_PATH = QUALITY_DIR / "v014_s14_p2_post_remediation_invoice_tax_plan_go_no_go.json"

PRIVATE_DIR = Path("KMFA/.codex_private_runtime/v014_s14_p2_post_remediation_invoice_tax_plan")
...
PRIVATE_PROBE_PATH = PRIVATE_DIR / "raw_invoice_tax_candidate_probe.json"
PRIVATE_REPORT_PATH = PRIVATE_DIR / "s14_p2_private_difference_report_zh.md"
...
PRIVATE_CURRENT_AUDIT_PATH = PRIVATE_DIR / "current_invoice_tax_workbench_audit.csv"
...
LANE_SPECS = (
    {
        ...
        "categories": ("invoice",),
        "status_label": "开票结构候选已接入；发票、项目、客户、结算、期间和金额绑定未被证明。",
    },
    {
        "lane_id": "tax_detail",
        "visible_lane_name": "纳税明细",
        "categories": ("tax",),
        "status_label": "税务结构候选已接入；税种、税率、税额、期间和主体绑定未被证明。",
    },
    {
        "lane_id": "invoice_tax_cash_summary",
        "visible_lane_name": "开票纳税资金汇总",
        "categories": ("invoice", "tax", "cash", "journal"),
        "status_label": "开票、税务、现金和凭证结构可关联；数值与期间未形成权威汇总。",
    },
)

RAW_LANE_TERMS = {
    "invoice_plan": ("开票", "发票", "销项"),
    "tax_detail": (
        "纳税",
        "税务",
        "税率",
...
ISSUE_METHOD_SPECS = (
    {
        "method_id": "pending_invoice",
        "visible_name": "待开票",
        "required_lanes": ("invoice_plan",),
        "review_sequence": 1,
        "method_note": "需权威合同、结算、开票状态、期间和金额逐行绑定后才能识别候选。",
    },
    {
        "method_id": "invoiced_not_collected",
        "visible_name": "已开票未回款",
        "required_lanes": ("invoice_plan", "invoice_tax_cash_summary"),
        "review_sequence": 2,
        "method_note": "需权威发票记录、回款记录、客户、项目、期间和金额逐行匹配后才能识别候选。",
    },
    {
        "method_id": "tax_rate_exception_candidate",
        "visible_name": "税率异常候选",
        "required_lanes": ("tax_detail",),
        "review_sequence": 3,
        "method_note": "需权威税种、适用依据、税率、期间和主体绑定后才能形成异常候选。",
    },
)

CASH_METHOD_SPECS = (
    {
        "method_id": "invoice_expected_cash_inflow",
        "visible_name": "开票预计现金流入",
        "required_lanes": ("invoice_plan", "invoice_tax_cash_summary"),
    },
    {
        "method_id": "tax_expected_cash_outflow",
        "visible_name": "纳税预计现金流出",
        "required_lanes": ("tax_detail", "invoice_tax_cash_summary"),
    },
    {
        "method_id": "invoice_tax_net_pressure",
        "visible_name": "开票纳税净资金压力",
        "required_lanes": ("invoice_plan", "tax_detail", "invoice_tax_cash_summary"),
    },
)


def _upsert_jsonl(path: Path, row: dict[str, Any]) -> None:
    target_phase = str(row.get("phase_id") or "")
    if not target_phase:
        raise ValueError("phase-local JSONL upsert requires phase_id")
    preserved: list[str] = []
    if path.is_file():
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip() and json.loads(line).get("phase_id") != target_phase:
...
def _load_dependency() -> dict[str, Any]:
    ...
    if manifest.get("next_phase") != "S14-P2":
        raise ValueError("S14-P1 must route to S14-P2")
    if summary.get("s14_p2_performed") is not False:
        raise ValueError("S14-P1 dependency already claims S14-P2")
    if summary.get("decision") != "NO_GO" or summary.get("current_report_grade") != "D":
        raise ValueError("S14-P1 quality state drift")
    return manifest
...
def _load_contract() -> dict[str, Any]:
    ...
    for token in ("开票/纳税/税务政策线", "证据缺口和风险提示", "不做正式纳税申报", "不自动开发票"):
...
def _build_source_lanes(probe: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    registry = p1._read_json(CURRENT_SOURCE_REGISTRY_PATH)
    candidates = p1._read_jsonl(CURRENT_CANDIDATES_PATH)
    ...
    direct_counts = {
        "invoice_plan": probe["private_invoice_candidate_sheet_count"],
        "tax_detail": probe["private_tax_candidate_sheet_count"],
        "invoice_tax_cash_summary": 0,
    }
    supporting_counts = {
        "invoice_plan": probe["private_invoice_candidate_sheet_count"],
        "tax_detail": probe["private_tax_candidate_sheet_count"],
        "invoice_tax_cash_summary": probe["private_unique_invoice_tax_candidate_sheet_count"],
    }
    ...
    for spec in LANE_SPECS:
        ...
        is_direct = spec["lane_id"] != "invoice_tax_cash_summary"
        lanes.append(
            {
                ...
                "private_direct_candidate_sheet_count": direct_counts[spec["lane_id"]],
                "private_supporting_candidate_sheet_count": supporting_counts[spec["lane_id"]],
                "structure_connected": True,
                "private_direct_candidate_structure_parseable": is_direct,
                "derived_summary_lane": not is_direct,
                ...
                "contains_tax_rate_values": False,
                "contains_invoice_identifiers": False,
...
def _build_issue_methods() -> list[dict[str, Any]]:
    return [
        {
            ...
            "business_alert_allowed": False,
            "invoice_issuance_allowed": False,
            "tax_filing_allowed": False,
            "payment_or_bank_operation_allowed": False,
            "formal_report_allowed": False,
        }
        for spec in ISSUE_METHOD_SPECS
    ]


def _build_cash_methods() -> list[dict[str, Any]]:
    return [
        {
            "method_id": spec["method_id"],
            "visible_name": spec["visible_name"],
            "required_lanes": list(spec["required_lanes"]),
            "method_definition_complete": True,
            "materialized_summary_count": 0,
            "public_business_amount_count": 0,
            "current_binding_status": "unproven",
            "current_output_status": "blocked_no_authoritative_period_value_binding",
        ...
        for spec in CASH_METHOD_SPECS
    ]


def _classify_sheet(sheet: Any) -> tuple[list[str], dict[str, list[str]]]:
    ...
    for lane_id, terms in RAW_LANE_TERMS.items():
        hits = [term for term in terms if term in searchable]
        if hits:
            matched_lanes.append(lane_id)
            matched_terms[lane_id] = hits
    return sorted(matched_lanes), matched_terms


def _sheet_probe(sheet: Any, *, row_limit: int = 50, column_limit: int = 30) -> dict[str, Any]:
    ...
    for row in sheet.iter_rows(min_row=1, max_row=max_row, max_col=max_column, values_only=True):
        normalized = [p1._normalize_cell(value) for value in row]
    ...
    return {
        ...
        "probe_fingerprint": p1._sha256_json(normalized_rows),
...
def _scan_workbook(
    ...
    *,
    ...
    for sheet_index, sheet in enumerate(workbook.worksheets, 1):
        matched_lanes, matched_terms = _classify_sheet(sheet)
        ...
        records.append(
            {
                ...
                "member_sha256": p1._sha256_bytes(payload),
...
def _raw_candidate_probe(raw_root: Path) -> dict[str, Any]:
        ...
        raw_records.append(
            {
                ...
                "raw_sha256": p1._sha256_bytes(raw_bytes),
    ...
    invoice_count = sum("invoice_plan" in row["matched_lanes"] for row in candidate_records)
    tax_count = sum("tax_detail" in row["matched_lanes"] for row in candidate_records)
    overlap_count = sum(
        "invoice_plan" in row["matched_lanes"] and "tax_detail" in row["matched_lanes"]
        for row in candidate_records
    )
    return {
        "schema_version": "kmfa.v014.s14_p2_private_raw_candidate_probe.v1",
        ...
        "private_invoice_candidate_sheet_count": invoice_count,
        "private_tax_candidate_sheet_count": tax_count,
        "private_invoice_tax_overlap_sheet_count": overlap_count,
        "private_unique_invoice_tax_candidate_sheet_count": len(candidate_records),
        "private_parseable_direct_lane_count": sum(count > 0 for count in (invoice_count, tax_count)),
...
def _render_html(
    lanes: list[dict[str, Any]],
    issue_methods: list[dict[str, Any]],
    cash_methods: list[dict[str, Any]],
) -> str:
    lane_rows = "".join(
        f"<tr><td>{html.escape(row['visible_lane_name'])}</td><td>已接入</td>"
        f"<td>{row['private_supporting_candidate_sheet_count']}</td><td>未证明</td></tr>"
        for row in lanes
    )
    buttons = "".join(
    ...
    panels = "".join(
        ...
        '<div class="limit">不得据此开具发票、生成纳税申报、发起付款或执行银行操作。</div></section>'
        for row in issue_methods
    )
    cash_rows = "".join(
        f'<div class="cash-item"><strong>{html.escape(row["visible_name"])}</strong>'
        '<span>方法已定义 · 数值未物化</span></div>'
    ...
    stage_link = f'<a data-stage-link="policy-evidence" href="{P3_HREF}">政策证据工作台</a>'
    labels = json.dumps(
        {row["method_id"]: row["visible_name"] for row in issue_methods},
        ensure_ascii=False,
        separators=(",", ":"),
    )
...
<head>
  ...
  <style>
    *{{box-sizing:border-box}} body{{margin:0;background:#f2f5f7;color:#172522;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;letter-spacing:0}} button,a{{font:inherit}}
    ...
    .table-wrap{{overflow-x:auto}} table{{width:100%;border-collapse:collapse;min-width:660px}} th,td{{padding:12px 16px;text-align:left;border-bottom:1px solid #e2e8e6;font-size:13px}} th{{background:#f6f8f7;color:#5e716b}} tbody tr:last-child td{{border-bottom:0}} .method-layout{{display:grid;grid-template-columns:210px minmax(0,1fr)}} .method-nav{{padding:12px;background:#f4f7f6;border-right:1px solid #d9e1de}} .method-nav button{{display:block;width:100%;min-height:42px;text-align:left;border:1px solid transparent;background:transparent;padding:9px 10px;color:#1c322c;cursor:pointer;border-radius:4px}} .method-nav button.active{{background:#fff;border-color:#78ae9d;color:#116b51;font-weight:700}}
    .method-panels{{padding:20px}} .panel-head{{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}} .panel-head span{{font-size:12px;color:#6a7d77}} dl{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:20px 0}} dl div{{border-left:3px solid #8ebfaf;padding-left:10px}} dt{{font-size:12px;color:#6a7d77}} dd{{margin:4px 0 0;font-weight:700}} .method-panels p{{line-height:1.7}} .limit{{border:1px solid #edb5b0;background:#fff2f0;color:#8d342f;padding:12px}} .cash-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:0}} .cash-item{{padding:16px;border-right:1px solid #d9e1de}} .cash-item:last-child{{border-right:0}} .cash-item strong,.cash-item span{{display:block}} .cash-item span{{font-size:12px;color:#687b75;margin-top:5px}}
    .footer-band{{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:13px 16px;background:#f6f8f7;border-top:1px solid #d9e1de}} .footer-band p{{margin:0;color:#5b6e68;font-size:13px}} .footer-band a{{color:#136c94;text-decoration:none;margin-left:12px;font-size:13px}} footer{{color:#687b75;font-size:12px;line-height:1.7}} @media(max-width:720px){{.topbar-inner,.headline,.footer-band{{display:block}} .top-links{{justify-content:flex-start;margin-top:12px}} main{{padding:18px 12px 36px}} h1{{font-size:24px}} .badges{{margin-top:12px}} .metrics{{grid-template-columns:1fr 1fr}} .metric:nth-child(2){{border-right:0}} .metric:nth-child(-n+2){{border-bottom:1px solid #cfd9d6}} table{{min-width:0;table-layout:fixed}} th,td{{padding:9px 6px;font-size:11px;word-break:break-word}} .method-layout{{grid-template-columns:1fr}} .method-nav{{border-right:0;border-bottom:1px solid #d9e1de;display:grid;grid-template-columns:repeat(3,1fr);gap:5px}} .method-nav button{{text-align:center;padding:7px 4px}} dl,.cash-grid{{grid-template-columns:1fr}} .cash-item{{border-right:0;border-bottom:1px solid #d9e1de}} .footer-band .links{{margin-top:10px}} .footer-band a{{display:inline-block;margin:4px 10px 4px 0}}}}
  </style>
</head>
<body data-page-id="s14-p2-invoice-tax" data-active-method="pending_invoice" data-ui-ready="false">
  <header class="topbar"><div class="topbar-inner"><div class="brand"><div class="mark">KM</div><div><strong>KMFA 经营分析系统</strong><span>S14-P2 · 财务税务</span></div></div><nav class="top-links" aria-label="阶段页面导航">{links}{stage_link}</nav></div></header>
  <main>
    <section class="headline"><div><h1>开票纳税计划工作台</h1><p class="subtitle">原始候选结构已只读解析；当前仅展示结构状态与复核方法，不展示发票号、税率、金额或申报明细。</p></div><div class="badges"><span class="tag ok">Q4 / D</span><span class="tag blocked">NO_GO</span><span class="tag">内部复核</span></div></section>
    <div class="notice"><strong>门禁：</strong> 3 条主题均缺权威行级与数值绑定；待开票、已开票未回款和税率异常均不得形成业务结论。</div>
    <section class="metrics" aria-label="状态摘要"><div class="metric"><strong>3 / 3</strong><span>结构主题已接入</span></div><div class="metric"><strong>2 / 2</strong><span>私有直接候选可解析</span></div><div class="metric"><strong>0 / 3</strong><span>行级绑定已证明</span></div><div class="metric"><strong>0</strong><span>已证明业务事项</span></div></section>
    <section class="band"><div class="band-header"><h2>开票纳税来源主题</h2><span class="tag blocked">不含业务金额</span></div><div class="table-wrap"><table><thead><tr><th>主题</th><th>公开结构</th><th>私有结构支持</th><th>行级绑定</th></tr></thead><tbody>{lane_rows}</tbody></table></div></section>
    <section class="band"><div class="band-header"><h2>三类问题复核方法</h2><span class="tag blocked">0 项已证明</span></div><div class="method-layout"><nav class="method-nav" aria-label="问题复核方法">{buttons}</nav><div class="method-panels">{panels}</div></div><div class="footer-band"><p id="interaction-status" aria-live="polite">已显示“待开票”复核方法；当前不形成业务结论。</p><div class="links">{links}</div></div></section>
    <section class="band"><div class="band-header"><h2>开票纳税资金方法</h2><span class="tag blocked">0 项已物化</span></div><div class="cash-grid">{cash_rows}</div></section>
    <footer>Stage 14 三个 phase 均已完成；整体复审保持 Q4 / D · NO_GO，不执行发票开具、纳税申报、GitHub upload、app reinstall、正式报告、差异关闭或业务执行。</footer>
  </main>
  <script>
    const labels={labels};
    let actionSequence=0;
    function activate(id){{document.body.dataset.activeMethod=id;document.body.dataset.lastAction="method:"+id+":"+String(++actionSequence);document.querySelectorAll("[data-method-button]").forEach(function(button){{button.classList.toggle("active",button.dataset.methodButton===id);}});document.querySelectorAll("[data-method-panel]").forEach(function(panel){{panel.hidden=panel.dataset.methodPanel!==id;}});document.getElementById("interaction-status").textContent="已显示“"+labels[id]+"”复核方法；当前不形成业务结论。";}}
    document.querySelectorAll("[data-method-button]").forEach(function(button){{button.addEventListener("click",function(){{activate(button.dataset.methodButton);}});}});
    activate("pending_invoice");document.body.dataset.uiReady="true";
  </script>
</body>
</html>"""
...
def _browser_worker() -> dict[str, Any]:
    ...
    handler = functools.partial(p1._QuietHandler, directory=str(root))
    ...
    try:
        with sync_playwright() as playwright:
            ...
            for mode, width, height in (("desktop", 1440, 900), ("mobile", 390, 844)):
                ...
                viewport_checks.append(
                    {
                        ...
                        "width": width,
                        "height": height,
                        "marker_visible": page.get_by_role("heading", name="开票纳税计划工作台").is_visible(),
                ...
                for method in ISSUE_METHOD_SPECS:
                ...
                page.screenshot(path=str(PRIVATE_SCREENSHOT_DIR / f"invoice_tax_{mode}.png"), full_page=True)
                page.close()
    ...
    result = {
        ...
        "issue_method_interaction_checks": method_checks,
    ...
    if not (
        ...
        result["status"] = "FAIL"
    p1._write_json(PRIVATE_BROWSER_PATH, result)
    return result
...
def _run_browser_review() -> dict[str, Any]:
    ...
    browser = p1._read_json(PRIVATE_BROWSER_PATH)
    if browser.get("status") != "PASS":
        raise RuntimeError("S14-P2 browser validation failed")
    ...
    if current["fail_count"] or current["warn_count"] or current["pass_count"] != current["control_row_count"]:
        raise RuntimeError("S14-P2 HTML audit failed")
    return {
        "status": "PASS",
        ...
        "issue_method_interaction_check_count": len(browser["issue_method_interaction_checks"]),
...
def _quality_gate() -> dict[str, Any]:
    return {
        ...
        "identified_issue_candidate_business_use_allowed": False,
        "tax_rate_exception_business_alert_allowed": False,
        "cash_summary_business_use_allowed": False,
...
def _public_safety() -> dict[str, bool]:
    return {
        ...
        "tax_rate_value_committed": False,
        "invoice_identifier_committed": False,
        "tax_declaration_detail_committed": False,
...
def _acceptance_matrix(summary: dict[str, Any]) -> dict[str, Any]:
    checks = {
        "current_s14_p1_dependency": summary["s14_p1_post_remediation_dependency_validated"],
        "three_structure_lanes": summary["source_lane_count"] == 3 == summary["structure_connected_lane_count"],
        "two_private_direct_candidate_lanes": summary["private_parseable_direct_lane_count"] == 2,
        "zero_row_value_bindings": summary["row_level_binding_proven_lane_count"] == 0
        and summary["value_binding_proven_lane_count"] == 0,
        "three_issue_method_definitions": summary["issue_review_method_definition_count"] == 3,
        "three_cash_method_definitions": summary["cash_summary_method_definition_count"] == 3,
        "zero_business_candidates_and_summaries": summary["identified_issue_candidate_count"] == 0
        and summary["materialized_cash_summary_count"] == 0,
        ...
        "no_later_scope": not summary["s14_p3_performed"]
        and not summary["github_upload_performed"]
        and not summary["invoice_issuance_performed"]
        and not summary["tax_filing_performed"],
    }
    return {
        "schema_version": "kmfa.v014.s14_p2_acceptance_matrix.v1",
        "checks": checks,
...
def _write_governance(generated_at: str) -> None:
    ...
    _upsert_jsonl(
        DEVELOPMENT_EVENTS_PATH,
        {
            "event_id": "DEV-KMFA-20260711-V014-S14-P2-POST-REMEDIATION-INVOICE-TAX-PLAN",
    ...
    _upsert_jsonl(
        TASK_STATUS_PATH,
        {
            ...
            "project_id": "KMFA",
            "stage_id": "S14",
            "governance_stage_id": "INVOICE-TAX",
            ...
            "version": VERSION,
            "name": "v0.1.4 S14-P2 post-remediation invoice tax plan",
            "phase_goal": "connect invoice tax structures and methods without inventing business values or actions",
            "status": STATUS,
            "fact_level": "EXTRACTED",
            "derived_percent": 100,
...
def _render_report(summary: dict[str, Any]) -> str:
    return f"""# KMFA v0.1.4 S14-P2 开票纳税计划
...
本 phase 已对只读 raw 容器执行候选解析与二次探针指纹复验，探针不一致数为 {summary['private_probe_roundtrip_mismatch_count']}。候选工作表只能证明结构相关，不能证明权威发票、项目、客户、结算、回款、税种、税率、期间或数值绑定，因此不生成问题实例、税率异常告警或资金金额。
...
def _render_private_report(summary: dict[str, Any]) -> str:
    return f"""# S14-P2 私有 raw 解析与差异记录
...
- 开票 / 税务 / 重叠 / 唯一候选：{summary['private_invoice_candidate_sheet_count']} / {summary['private_tax_candidate_sheet_count']} / {summary['private_invoice_tax_overlap_sheet_count']} / {summary['private_unique_invoice_tax_candidate_sheet_count']}
- 二次探针指纹不一致：{summary['private_probe_roundtrip_mismatch_count']}
- 行级 / 数值权威绑定：0 / 0
- 已证明问题候选 / 已物化资金汇总：0 / 0
- raw 前后、跨 S14-P1 与当前快照：exact match
- 当前差异原因：缺少 raw 候选到公开来源引用的唯一权威锚点，且发票、项目、客户、结算、回款、税种、税率、期间和数值没有逐行证明。
- 本轮结论：候选解析已跑通且未损伤原始文件；不得把结构候选转换为业务候选、税率异常告警或申报金额。
...
def generate(*, final_validation: bool = False, write_governance: bool = True) -> dict[str, Any]:
    ...
    raw_before = raw_helper._raw_snapshot("before_v014_s14_p2_post_remediation_invoice_tax_plan")
    dependency = _load_dependency()
    contract = _load_contract()
    historical = p1._read_json(HISTORICAL_MANIFEST_PATH)
    ...
    for key, expected in expected_probe.items():
        if raw_probe.get(key) != expected:
            raise ValueError(f"private raw invoice/tax candidate structure drift: {key}")
    lanes, structure = _build_source_lanes(raw_probe)
    issue_methods = _build_issue_methods()
    cash_methods = _build_cash_methods()
    p1._write_text(HTML_PATH, _render_html(lanes, issue_methods, cash_methods))
    browser = _run_browser_review()
    raw_after = raw_helper._raw_snapshot("after_v014_s14_p2_post_remediation_invoice_tax_plan")
    prior_raw = p1._read_json(p1.PRIVATE_RAW_AFTER_PATH)
    current_raw = raw_helper._raw_snapshot("current_v014_s14_p2_post_remediation_invoice_tax_plan")
    ...
    if not raw_exact or not raw_cross:
        raise ValueError("raw source changed during S14-P2")

    upstream = dependency["summary"]
    summary = {
        "schema_version": "kmfa.v014.s14_p2_post_remediation_summary.v1",
        "project_id": "KMFA",
        "stage_id": "S14",
        "phase_id": PHASE_ID,
        "roadmap_phase_id": ROADMAP_PHASE_ID,
        ...
        **structure,
        "private_parseable_direct_lane_count": raw_probe["private_parseable_direct_lane_count"],
        "row_level_binding_proven_lane_count": 0,
        "value_binding_proven_lane_count": 0,
        "issue_review_method_definition_count": len(issue_methods),
        "cash_summary_method_definition_count": len(cash_methods),
        ...
        "private_invoice_candidate_sheet_count": raw_probe["private_invoice_candidate_sheet_count"],
        "private_tax_candidate_sheet_count": raw_probe["private_tax_candidate_sheet_count"],
        "private_invoice_tax_overlap_sheet_count": raw_probe["private_invoice_tax_overlap_sheet_count"],
        "private_unique_invoice_tax_candidate_sheet_count": raw_probe[
        ...
        "issue_method_interaction_check_count": browser["issue_method_interaction_check_count"],
    ...
    historical_summary = historical.get("invoice_tax_summary", {})
    historical_quarantine = {
        "legacy_manifest_validated_as_historical_fixture": True,
        "legacy_s14_p2_dynamic_state_is_authoritative": False,
        "legacy_pending_twelve_quarantined": historical_summary.get("pending_reconciliation_count") == 12,
        "legacy_three_issue_candidates_quarantined": historical_summary.get("issue_candidate_count") == 3,
        "legacy_three_cash_summaries_quarantined": historical_summary.get("cash_summary_count") == 3,
        "current_identified_issue_candidate_count": 0,
        "current_materialized_cash_summary_count": 0,
    }
    validation_summary = {
    ...
    manifest = {
        "schema_version": "kmfa.v014.s14_p2_post_remediation_manifest.v1",
        "project_id": "KMFA",
        "stage_id": "S14",
        "phase_id": PHASE_ID,
        "roadmap_phase_id": ROADMAP_PHASE_ID,
        ...
        "reviewed_head": p1._git_output(["rev-parse", "HEAD"]),
        "branch": p1._git_output(["branch", "--show-current"]),
        "remote": p1._git_output(["remote", "get-url", "origin"]),
        "s14_p1_post_remediation_dependency_validated": True,
        "historical_s14_p2_dynamic_state_is_authoritative": False,
        "summary": summary,
        "source_lanes": lanes,
        "issue_review_methods": issue_methods,
        "cash_summary_methods": cash_methods,
        ...
        "reviewed_dependencies": {
            "current_s14_p1": p1.MANIFEST_PATH.as_posix(),
            ...
            "historical_s14_p2_policy_fixture": HISTORICAL_MANIFEST_PATH.as_posix(),
        },
        "raw_boundary": {
        ...
        "next_phase": "S14-P3",
        "next_required_step": (
            "Execute S14-P3 only as a separate run; do not perform Stage 14 review, invoice issuance, tax filing, "
            "upload, reinstall, formal release, difference closure, persistent write, or business execution."
        ),
    }
    go_no_go = {
        "schema_version": "kmfa.v014.s14_p2_go_no_go.v1",
        "phase_id": PHASE_ID,
        "decision": "NO_GO",
        "data_quality_grade": "Q4",
        "report_grade": "D",
        "reason": "authoritative invoice tax period entity row and value bindings remain unproven",
        "s14_p3_allowed_in_this_run": False,
        "invoice_issuance_allowed": False,
        "tax_filing_allowed": False,
        "github_upload_allowed": False,
        "business_execution_allowed": False,
    ...
    public_writes = (
        ...
        (LANES_PATH, {"schema_version": "kmfa.v014.s14_p2_source_lanes.v1", "lanes": lanes}),
        (
            ISSUE_METHODS_PATH,
            {"schema_version": "kmfa.v014.s14_p2_issue_methods.v1", "methods": issue_methods},
        ),
        (
            CASH_METHODS_PATH,
            {"schema_version": "kmfa.v014.s14_p2_cash_methods.v1", "methods": cash_methods},
        ),
        (MATRIX_PATH, matrix),
        (GO_NO_GO_PATH, go_no_go),
        (METADATA_SUMMARY_PATH, summary),
        (METADATA_MANIFEST_PATH, manifest),
        (METADATA_LANES_PATH, {"schema_version": "kmfa.v014.s14_p2_source_lanes.v1", "lanes": lanes}),
        (
            METADATA_ISSUE_METHODS_PATH,
            {"schema_version": "kmfa.v014.s14_p2_issue_methods.v1", "methods": issue_methods},
        ),
    ...
    p1._write_text(REPORT_PATH, _render_report(summary))
    p1._write_text(
        TEST_RESULTS_PATH,
        f"""# S14-P2 测试结果
...
- 三类问题方法交互：{browser['issue_method_interaction_check_count']}/6 PASS
- 上游链接 HTTP / 真实导航：{browser['dependency_link_http_check_count']}/4 / {browser['dependency_navigation_check_count']}/4 PASS
- private raw probe：612 unique invoice/tax candidate sheets / 0 roundtrip mismatch
- raw 前后/跨 S14-P1/current：exact match
""",
    )
    p1._write_text(
        RISK_REGISTER_PATH,
...
""",
    ...
    p1._write_json(PRIVATE_RAW_BEFORE_PATH, raw_before)
    p1._write_json(PRIVATE_RAW_AFTER_PATH, raw_after)
    p1._write_json(PRIVATE_PROBE_PATH, raw_probe)
    p1._write_text(PRIVATE_REPORT_PATH, _render_private_report(summary))
    if write_governance:
