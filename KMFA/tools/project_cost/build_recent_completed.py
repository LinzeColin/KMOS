    ...
    for path in glob.glob(f"{data_root}/**/{STATUS_SHEET}", recursive=True):
        ...
        for sheet_name in workbook.sheetnames:
            ...
            for row in sheet.iter_rows(min_row=header_row + 2, values_only=True):
                ...
                #
                # 合成回归：同一合同号挂到两个不同甲方。原来
                # `found[合同号] = record` 加「保留完工日期较晚的」会静默吃掉一条。
                #
...
#:
#: 为什么必须统计它：参考回归曾证明，只报项目已归集额会系统性漏掉
#: `不分项目`／`KMX999`／`KMX9999` 占位桶中的金额。
#:
...
def collect_ledger_cost(data_root: str, targets: set[str], account_to_row: dict,
    ...
    并进主号。合成回归包含 `KMX20991119-979-Z` 与
    `KMX2099116-903--Z` 两种后缀，用来证明原始合同号构成会被保留。
...
#: 合成参考回放中的工数冲突；只用于证明旧产线会显式披露，不包含真实合同。
LABOUR_HOURS_CONFLICT = {
    "KMX20991119-979": {"报表工数": 119, "红圈工数": 214},
    "KMX2099120-904": {"报表工数": 31, "红圈工数": 32},
}
...
def read_master_contracts(data_root: str) -> dict[str, dict]:
    ...
    它同时解掉「一个合同号挂两个甲方」的合成冲突：权威主合同唯一匹配时以权威表为准。
    """
    ...
    for key, record in projects.items():
        ...
        if authority:
            ...
            if same_party:
                ...
                # 不能把它改写成权威表的甲方，否则会把甲方 A 的成本
                # 挂到甲方 B 名下，凭空造出一个不存在的成本。
                # 保留原样并标出来，让人去改源头。
                record["身份来源"] = (
...
def build(data_root: str, account_map: dict, limit: int = 0) -> dict:
    ...
    return {
        ...
        "锁定的算法": [
            ...
            "绝不静默丢弃（由合成冲突 fixture 持续验证）",
            "合同号后缀（-Z／--Z／-XF／-Z1）按主号归并；后缀标的是科目记账线，"
            "不是项目身份。每个项目的"
            "`金蝶合同号构成` 保留原始号，可逐笔回溯",
        ],
        "单价标定": {
