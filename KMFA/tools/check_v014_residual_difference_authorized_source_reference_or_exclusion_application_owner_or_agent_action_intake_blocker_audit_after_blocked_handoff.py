#!/usr/bin/env python3
"""Validate action-intake blocker audit after blocked handoff."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_action_intake_blocker_audit_after_blocked_handoff import (  # noqa: E402
    ACCEPTANCE_ID,
    AUDIT_CONCLUSION,
    DECISION,
    ...
    SOURCE_PRIVATE_ACTION_INTAKE_REPORT_PATH,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_action_intake_public_artifacts_read_by_this_phase",
        "source_action_intake_manifest_read_by_this_phase",
        "source_action_intake_go_no_go_read_by_this_phase",
        "source_action_intake_matrix_read_by_this_phase",
        "source_private_action_intake_diagnostic_read_by_this_phase",
        "source_private_action_intake_blocker_records_read_by_this_phase",
        "source_private_action_intake_report_existence_checked_by_this_phase",
        "private_action_intake_blocker_audit_diagnostic_written_by_this_phase",
        "private_action_intake_blocker_audit_queue_written_by_this_phase",
        "private_action_intake_blocker_audit_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_action_intake_diagnostic_mutated_by_this_phase",
        "source_private_action_intake_blocker_records_mutated_by_this_phase",
        "source_private_action_intake_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal(
        "summary.private intake rows",
        summary.get("source_private_action_intake_blocker_records_item_count"),
        48,
    )
    ...
    _require_equal("summary.prior observation", summary.get("prior_action_intake_blocker_observation_count"), 0)
    _require_equal("summary.observation", summary.get("action_intake_blocker_observation_count"), 1)
    ...
    _require_equal(
        "summary.source ref audit blockers",
        summary.get("source_reference_or_owner_exclusion_audit_blocker_count"),
        40,
    )
    _require_equal("summary.formula audit blockers", summary.get("formula_or_non_numeric_mapping_audit_blocker_count"), 8)
    _require_equal("summary.binding ready", summary.get("binding_ready_after_action_intake_blocker_audit_count"), 0)
    _require_equal("summary.retry ready", summary.get("comparison_retry_ready_after_action_intake_blocker_audit_count"), 0)
    _require_equal("summary.unresolved differences", summary.get("unresolved_difference_count"), 72)
    for key in (
        "private_action_intake_blocker_audit_diagnostic_written",
        "private_action_intake_blocker_audit_queue_written",
        "private_action_intake_blocker_audit_report_written",
        "private_action_intake_blocker_audit_diagnostic_gitignored",
        "private_action_intake_blocker_audit_queue_gitignored",
        "private_action_intake_blocker_audit_report_gitignored",
        "source_private_action_intake_diagnostic_gitignored",
        "source_private_action_intake_blocker_records_gitignored",
        "source_private_action_intake_report_gitignored",
        "action_intake_blocker_audit_checked_by_this_phase",
    ):
    ...
    for code in (
        "source_action_intake_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_action_intake_blockers_complete",
        "source_owner_action_intake_ready_absent",
        "source_private_action_intake_records_read",
        "first_action_intake_blocker_observation_recorded",
        "action_intake_blocker_threshold_not_met",
        "private_audit_queue_locked",
    ...
    _require_equal("go_no_go.prior observation", go_no_go.get("prior_action_intake_blocker_observation_count"), 0)
    _require_equal("go_no_go.observation", go_no_go.get("action_intake_blocker_observation_count"), 1)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_ACTION_INTAKE_BLOCKER_AUDIT_DIAGNOSTIC_PATH)
    audit_queue = _read_jsonl(PRIVATE_ACTION_INTAKE_BLOCKER_AUDIT_QUEUE_PATH)
    report_text = PRIVATE_ACTION_INTAKE_BLOCKER_AUDIT_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal("private diagnostic rows", len(diagnostic.get("audit_queue") or []), 48)
    _require_equal("private audit queue row count", len(audit_queue), 48)
    if "Action intake blocker observation 1 recorded" not in report_text:
        raise ValidationError("private audit report missing observation conclusion")
    counts = Counter(row.get("diagnostic_kind") for row in audit_queue)
    _require_equal(
    ...
    for row in audit_queue:
        _require_equal("private audit status", row.get("action_intake_blocker_audit_status"), AUDIT_CONCLUSION)
        _require_false("private action-intake blocker audit ready", row.get("action_intake_blocker_audit_ready"))
        _require_true("private action-intake blocker audit blocker", row.get("action_intake_blocker_audit_blocker"))
        _require_equal("private observation count", row.get("action_intake_blocker_observation_count"), 1)
        ...
        _require_false("private binding", row.get("binding_ready_after_action_intake_blocker_audit"))
        _require_false("private retry", row.get("comparison_retry_ready_after_action_intake_blocker_audit"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "PASS: validated V014 owner/authorized-agent action-intake blocker audit after blocked handoff "
