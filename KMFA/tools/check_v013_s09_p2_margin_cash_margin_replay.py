#!/usr/bin/env python3
"""Validate KMFA v0.1.3 S09-P2 margin and cash margin replay evidence."""
...
from KMFA.tools.v013_s09_p2_margin_cash_margin_replay import (
    MANIFEST_PATH,
    NEXT_REQUIRED_STEP,
    PHASE_SCOPE,
    RAW_DIR,
...
def validate_v013_s09_p2_margin_cash_margin_replay(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    ...
    require(
        manifest.get("status") == "completed_validated_local_only_no_go_upload_deferred_margin_cash_margin_replayed",
        "status mismatch",
    )
    require(manifest.get("completed_task_ids") == ["S9PBT01", "S9PBT02", "S9PBT03"], "completed tasks mismatch")
    require(
        manifest.get("acceptance_ids") == ["ACC-V013-S09-P2-MARGIN-CASH-MARGIN-REPLAY"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.3 S09-P2 margin and cash margin replay.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH)
    args = parser.parse_args(argv)

    result = validate_v013_s09_p2_margin_cash_margin_replay(args.manifest)
    print(
        "PASS: KMFA v0.1.3 S09-P2 margin and cash margin replay validated "
