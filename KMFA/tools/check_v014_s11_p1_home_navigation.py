#!/usr/bin/env python3
"""Validate KMFA v0.1.4 S11-P1 public-safe home navigation evidence."""
...
from KMFA.tools.v014_s11_p1_home_navigation import (
    ACCEPTANCE_ID,
    HTML_OUTPUT_PATH,
    MANIFEST_PATH,
    NEXT_REQUIRED_STEP,
    ...
    validate_legacy_s11_p1_artifacts,
    validate_s10_stage_review_dependency,
)
...
def validate_v014_s11_p1_home_navigation(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    errors: list[str] = []
    ...
    require(manifest.get("phase_id") == "S11-P1", "phase_id must be S11-P1", errors)
    ...
    require(
        manifest.get("status") == "completed_validated_local_only_no_go_upload_deferred_home_navigation_locked",
        "status mismatch",
        errors,
    )
    require(manifest.get("completed_task_ids") == ["S11P1T01", "S11P1T02", "S11P1T03"], "tasks mismatch", errors)
    require(manifest.get("acceptance_ids") == [ACCEPTANCE_ID], "acceptance id mismatch", errors)
    require(manifest.get("s10_stage_review_dependency_validated") is True, "S10 dependency flag mismatch", errors)
    require(s10_review.get("next_phase") == "S11-P1", "S10 review dependency did not route to S11-P1", errors)
    require(manifest.get("legacy_s11_p1_dependency_validated") is True, "legacy S11-P1 flag mismatch", errors)
    ...
    require(progress.get("s11_p1_performed") is True, "S11-P1 must be performed", errors)
    require(progress.get("s11_p2_performed") is False, "S11-P2 must be false", errors)
    ...
    summary = manifest.get("home_navigation_summary", {})
    require(summary.get("navigation_module_count") == 8, "navigation module count mismatch", errors)
    require(summary.get("required_navigation_labels") == list(REQUIRED_NAVIGATION_LABELS), "required labels mismatch", errors)
    require(summary.get("html_export_count") == 1, "HTML export count mismatch", errors)
    require(summary.get("nav_button_count") == 8, "nav button count mismatch", errors)
    require(summary.get("module_action_button_count") == 8, "module action button count mismatch", errors)
    require(summary.get("visible_feedback_panel_count") == 1, "feedback panel count mismatch", errors)
    require(summary.get("aria_live_count") >= 1, "aria live count mismatch", errors)
    require(summary.get("click_event_listener_count") >= 2, "click listener count mismatch", errors)
    require(summary.get("existing_navigation_target_count") == 8, "navigation target existence mismatch", errors)
    require(summary.get("target_href_count") == 8, "target href count mismatch", errors)
    ...
    require(summary.get("blue_business_style") is True, "blue business style mismatch", errors)
    require(summary.get("all_chinese_visible_copy") is True, "Chinese visible copy mismatch", errors)
    ...
    for key, expected in baseline.items():
        if key in {
            "implementation_reflects_clickable_navigation",
            "implementation_reflects_visible_feedback",
            "implementation_reflects_report_center_entry",
        }:
            continue
    ...
    require(
        v14.get("implementation_reflects_clickable_navigation") is True,
        "clickable navigation reflection mismatch",
        errors,
    )
    require(v14.get("implementation_reflects_visible_feedback") is True, "visible feedback reflection mismatch", errors)
    require(v14.get("implementation_reflects_report_center_entry") is True, "report center reflection mismatch", errors)

    phase = manifest.get("phase_boundaries", {})
    require(phase.get("s11_p1_home_navigation_scope_included") is True, "S11-P1 scope flag mismatch", errors)
    ...
    require(quality.get("home_navigation_export_allowed") is True, "home navigation export flag mismatch", errors)
    ...
    require(records == legacy["records"], "v014 copied records must match legacy S11-P1 records", errors)
    require(html_text == legacy["html_text"], "v014 copied HTML must match legacy S11-P1 HTML", errors)
    require("报告中心" in html_text, "HTML must contain report center", errors)
    require("function selectModule" in html_text, "HTML must include selectModule feedback function", errors)
    require("function openModulePage" in html_text, "HTML must include openModulePage navigation function", errors)
    require('aria-live="polite"' in html_text, "HTML must include live feedback panel", errors)

    artifact_refs = manifest.get("artifact_refs", {})
    for key in (
        "s10_stage_review_manifest",
        "legacy_manifest",
        "legacy_records",
        "legacy_html",
    ...
    for evidence in (MANIFEST_PATH, RECORDS_PATH, HTML_OUTPUT_PATH, REPORT_PATH, TEST_RESULTS_PATH, RISK_REGISTER_PATH, ROLLBACK_PATH):
    ...
    require("check_v014_s11_p1_home_navigation.py" in test_text, "S11-P1 validator evidence missing", errors)
    require("test_v014_s11_p1_home_navigation" in test_text, "S11-P1 unit test evidence missing", errors)
    require("S11-P2, S11-P3, Stage 11 overall review" in test_text, "phase boundary evidence missing", errors)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate KMFA v0.1.4 S11-P1 home navigation evidence.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH)
    args = parser.parse_args(argv)
    try:
        manifest = validate_v014_s11_p1_home_navigation(args.manifest)
    except (OSError, json.JSONDecodeError, ValidationError, RuntimeError) as exc:
        print(f"FAIL: KMFA v0.1.4 S11-P1 home navigation validation failed ({exc})")
        return 1

    summary = manifest["home_navigation_summary"]
    baseline = manifest["v14_html_uiux_baseline"]
    print(
        "PASS: KMFA v0.1.4 S11-P1 home navigation validated "
