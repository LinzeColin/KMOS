#!/usr/bin/env python3
"""Validate resolution-intake blocker final recheck after final-check closure."""
...
from KMFA.tools.v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_resolution_intake_blocker_final_recheck_after_final_check_closure import (  # noqa: E402
    ACCEPTANCE_ID,
    FINAL_RECHECK_CONCLUSION,
    DECISION,
...
def _check_raw_boundary(raw_boundary: dict[str, Any]) -> None:
    true_keys = (
        ...
        "source_resolution_intake_blocker_recheck_public_artifacts_read_by_this_phase",
        "source_resolution_intake_blocker_recheck_manifest_read_by_this_phase",
        "source_resolution_intake_blocker_recheck_go_no_go_read_by_this_phase",
        "source_resolution_intake_blocker_recheck_matrix_read_by_this_phase",
        "source_private_resolution_intake_blocker_recheck_diagnostic_read_by_this_phase",
        "source_private_resolution_intake_blocker_recheck_queue_read_by_this_phase",
        "source_private_resolution_intake_blocker_recheck_report_existence_checked_by_this_phase",
        "private_resolution_intake_blocker_final_recheck_diagnostic_written_by_this_phase",
        "private_resolution_intake_blocker_final_recheck_queue_written_by_this_phase",
        "private_resolution_intake_blocker_final_recheck_report_written_by_this_phase",
    )
    false_keys = (
        "source_private_resolution_intake_blocker_recheck_diagnostic_mutated_by_this_phase",
        "source_private_resolution_intake_blocker_recheck_queue_mutated_by_this_phase",
        "source_private_resolution_intake_blocker_recheck_report_mutated_by_this_phase",
...
def _check_summary(
    ...
    _require_equal("summary.threshold", summary.get("final_recheck_conclusion"), FINAL_RECHECK_CONCLUSION)
    _require_equal("summary.source phase", summary.get("source_phase_id"), source_recheck.PHASE_ID)
    _require_equal("summary.source manifest", summary.get("source_manifest_phase_id"), source_recheck.PHASE_ID)
    ...
    _require_equal("summary.source owner intake blockers", summary.get("source_resolution_intake_blocker_recheck_blocker_count"), 48)
    _require_equal("summary.source owner intake ready", summary.get("source_resolution_intake_blocker_recheck_ready_count"), 0)
    _require_equal("summary.source audit items", summary.get("source_resolution_intake_blocker_recheck_item_count"), 48)
    _require_equal(
        "summary.private audit queue rows",
        summary.get("source_private_resolution_intake_blocker_recheck_queue_item_count"),
        48,
    )
    ...
    _require_equal("summary.prior observation", summary.get("prior_resolution_intake_blocker_observation_count"), 2)
    ...
    _require_equal("summary.owner intake ready", summary.get("resolution_intake_blocker_final_recheck_ready_count"), 0)
    _require_equal("summary.owner intake blocker", summary.get("resolution_intake_blocker_final_recheck_blocker_count"), 48)
    _require_equal("summary.recheck items", summary.get("resolution_intake_blocker_final_recheck_item_count"), 48)
    _require_equal("summary.private final recheck queue", summary.get("private_final_recheck_queue_item_count"), 48)
    ...
    _require_equal(
        ...
        summary.get("source_reference_or_owner_exclusion_final_recheck_blocker_count"),
        40,
    )
    _require_equal(
        "summary.formula threshold blockers",
        summary.get("formula_or_non_numeric_mapping_final_recheck_blocker_count"),
        8,
    )
    _require_equal(
        "summary.binding ready",
        summary.get("binding_ready_after_resolution_intake_blocker_final_recheck_count"),
        0,
    )
    _require_equal(
        "summary.retry ready",
        summary.get("comparison_retry_ready_after_resolution_intake_blocker_final_recheck_count"),
        0,
    )
    for key in (
    ...
    for key in (
        "private_resolution_intake_blocker_final_recheck_diagnostic_written",
        "private_resolution_intake_blocker_final_recheck_queue_written",
        "private_resolution_intake_blocker_final_recheck_report_written",
        "private_resolution_intake_blocker_final_recheck_diagnostic_gitignored",
        "private_resolution_intake_blocker_final_recheck_queue_gitignored",
        "private_resolution_intake_blocker_final_recheck_report_gitignored",
        "source_private_resolution_intake_blocker_recheck_diagnostic_gitignored",
        "source_private_resolution_intake_blocker_recheck_queue_gitignored",
        "source_private_resolution_intake_blocker_recheck_report_gitignored",
        "resolution_intake_blocker_final_recheck_checked_by_this_phase",
    ):
    ...
    for code in (
        "source_resolution_intake_blocker_recheck_phase_loaded",
        "source_go_no_go_preserved",
        "source_matrix_clean",
        "source_resolution_intake_blocker_recheck_complete",
        "source_resolution_intake_blocker_recheck_ready_absent",
        "source_private_resolution_intake_blocker_recheck_queue_read",
        "final_resolution_intake_blocker_observation_recorded",
        "resolution_intake_blocker_audit_threshold_met",
    ...
    _require_equal("go_no_go.threshold", go_no_go.get("final_recheck_conclusion"), FINAL_RECHECK_CONCLUSION)
    _require_equal("go_no_go.source blockers", go_no_go.get("source_resolution_intake_blocker_recheck_blocker_count"), 48)
    _require_equal("go_no_go.source ready", go_no_go.get("source_resolution_intake_blocker_recheck_ready_count"), 0)
    _require_equal("go_no_go.goal", go_no_go.get("goal_status_recommendation"), "blocked")
    _require_equal("go_no_go.prior observation", go_no_go.get("prior_resolution_intake_blocker_observation_count"), 2)
    _require_equal("go_no_go.observation", go_no_go.get("resolution_intake_blocker_observation_count"), 3)
    _require_true("go_no_go.threshold", go_no_go.get("resolution_intake_blocker_audit_threshold_met"))
    _require_equal("go_no_go.owner intake ready", go_no_go.get("resolution_intake_blocker_final_recheck_ready_count"), 0)
    _require_equal("go_no_go.owner intake blocker", go_no_go.get("resolution_intake_blocker_final_recheck_blocker_count"), 48)
...
def _check_private_artifacts() -> None:
    ...
    diagnostic = _read_json(PRIVATE_RESOLUTION_INTAKE_BLOCKER_FINAL_RECHECK_DIAGNOSTIC_PATH)
    final_recheck_rows = _read_jsonl(PRIVATE_RESOLUTION_INTAKE_BLOCKER_FINAL_RECHECK_QUEUE_PATH)
    report_text = PRIVATE_RESOLUTION_INTAKE_BLOCKER_FINAL_RECHECK_REPORT_PATH.read_text(encoding="utf-8")
    _require_equal("private diagnostic phase", diagnostic.get("phase_id"), PHASE_ID)
    _require_equal("private diagnostic rows", len(diagnostic.get("final_recheck_rows") or []), 48)
    _require_equal("private final recheck queue row count", len(final_recheck_rows), 48)
    if "Resolution intake blocker observation 3 recorded" not in report_text:
        raise ValidationError("private final recheck report missing observation conclusion")
    counts = Counter(row.get("diagnostic_kind") for row in final_recheck_rows)
    _require_equal(
    ...
    for row in final_recheck_rows:
        _require_equal("private final recheck status", row.get("resolution_intake_blocker_final_recheck_status"), FINAL_RECHECK_CONCLUSION)
        _require_false("private resolution-intake blocker final recheck ready", row.get("resolution_intake_blocker_final_recheck_ready"))
        _require_true("private resolution-intake blocker final recheck blocker", row.get("resolution_intake_blocker_final_recheck_blocker"))
        _require_equal("private prior observation count", row.get("prior_resolution_intake_blocker_observation_count"), 2)
        _require_equal("private observation count", row.get("resolution_intake_blocker_observation_count"), 3)
        _require_true("private final recheck threshold", row.get("resolution_intake_blocker_audit_threshold_met"))
        _require_equal("private goal recommendation", row.get("goal_status_recommendation"), "blocked")
        ...
        _require_false("private binding", row.get("binding_ready_after_resolution_intake_blocker_final_recheck"))
        _require_false("private retry", row.get("comparison_retry_ready_after_resolution_intake_blocker_final_recheck"))
...
def validate(*, require_private_final_recheck: bool = False) -> dict[str, Any]:
    ...
    if require_private_final_recheck:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-private-final-recheck", action="store_true")
    args = parser.parse_args(argv)
    try:
        manifest = validate(require_private_final_recheck=args.require_private_final_recheck)
    except ValidationError as exc:
        print(f"FAIL: {exc}", file=sys.stderr)
        return 1
    summary = manifest["summary"]
    print(
        "PASS: validated V014 owner/authorized-agent resolution-intake blocker final recheck after final-check closure "
