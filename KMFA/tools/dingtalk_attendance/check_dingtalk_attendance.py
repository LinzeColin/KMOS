#!/usr/bin/env python3
"""Validate the public-safe file contract for the KMFA 钉钉考勤 skill."""
...
def validate_dingtalk_attendance_files(root: Path) -> dict[str, Any]:
    ...
    if manifest.get("skill_id") != SKILL_ID or "stage_id" in manifest:
        errors.append("skill identity drift")
    ...
    prompt_mirrors_match = all(
        ...
        == metadata_path.read_text(encoding="utf-8").rstrip("\n") + "\n"
        for package_path, metadata_path in prompt_mirror_pairs
    )
    if not prompt_mirrors_match:
        errors.append("package and metadata prompt mirrors differ")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the KMFA 钉钉考勤 skill file contract.")
    parser.add_argument("--root", type=Path, default=Path(__file__).resolve().parents[2])
    args = parser.parse_args(argv)

    result = validate_dingtalk_attendance_files(args.root)
