...
当前状态：`RELEASED_0_2_0_FAIL_CLOSED`。产品工作流已通过 R12 发布门禁，但当前公司输入仍是 `BLOCKED_SOURCE`，不得把发布状态当作真实成本计算成功。全局安装尚未执行；必须在独立下一轮验证远端/main 同步后安装。
...
- R12 不执行全局安装。全局可发现安装必须在独立下一轮先确认远端、main 与发布提交一致，再安装并验证发现性与行为 parity。
