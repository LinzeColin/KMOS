...
- 当前 Phase: `v0.1.4 S08-P3 实体匹配质量已完成本地验证；S08-P2 dependency PASS、legacy S08-P3 validator PASS，4 类匹配质量场景、4 条 quality cases、3 条 manual review queue、1 份 entity matching quality report、risk summary high=2/medium=1/low=1 已锁定；下一步只能另起 run work 执行 Stage 8 整体复审，不执行 GitHub upload、raw value matching、lineage full check 或正式报告`
...
- v0.1.4 S08-P3 实体匹配质量已完成；下一轮只能在用户明确开启后执行 `Stage 8 整体复审`。不得直接执行 GitHub upload、S09-P1、raw value matching、lineage full check、正式报告、live connector、OpMe 深度耦合或任何业务动作。
