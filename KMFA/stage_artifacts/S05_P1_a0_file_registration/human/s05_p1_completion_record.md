...
- 新增 `KMFA/metadata/baseline/a0_file_manifest.json`，登记 `销售绩效考核.zip` 包 SHA256、8 个 PDF、1 个 Excel、legacy inventory 指纹和成员 hash 状态。
...
- 本机未找到私有 `销售绩效考核.zip`，所以 9 个成员文件的 `member_sha256` 均显式记录为 `pending_private_zip_unavailable`；没有把 legacy CRC/指纹伪装成 SHA256。
...
- 若后续需要真正完成成员级 SHA256，必须在本地私有路径提供 `销售绩效考核.zip`，运行 `KMFA/tools/a0_file_register.py --source-zip <private-path>` 后复验；不得提交 zip 或解包文件。
