...
- 当前 40 条 PDF 字段候选已记录 hash/source anchor，5 条 Excel 字段候选仍为 `pending_private_source_unavailable`。
...
- 当前回填不是正式 A0 golden fixture 锁定；仍有 5 条 Excel 字段 pending，且未做 Q4 人工确认。
- 后续必须补齐 Excel 候选字段或形成明确人工豁免/不适用决策，再进入 S05-P3；公开仓库仍只允许保存 hash/ref/status，不允许保存真实字段值。
