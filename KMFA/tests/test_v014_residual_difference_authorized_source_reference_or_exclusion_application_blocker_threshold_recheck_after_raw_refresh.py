...
from KMFA.tools.check_v014_residual_difference_authorized_source_reference_or_exclusion_application_blocker_threshold_recheck_after_raw_refresh import (
    validate,
)
...
SOURCE_INPUT_PATHS = [
    generator.SOURCE_APPLICATION_BLOCKER_AUDIT_SUMMARY_PATH,
    generator.SOURCE_APPLICATION_BLOCKER_AUDIT_MANIFEST_PATH,
    generator.SOURCE_APPLICATION_BLOCKER_AUDIT_GO_NO_GO_PATH,
    generator.SOURCE_APPLICATION_BLOCKER_AUDIT_MATRIX_PATH,
    generator.SOURCE_PRIVATE_APPLICATION_BLOCKER_AUDIT_DIAGNOSTIC_PATH,
    generator.SOURCE_PRIVATE_APPLICATION_BLOCKER_AUDIT_RECORDS_PATH,
    generator.SOURCE_PRIVATE_APPLICATION_BLOCKER_AUDIT_REPORT_PATH,
]


class AuthorizedSourceReferenceOrExclusionApplicationBlockerThresholdRecheckAfterRawRefreshTest(
    unittest.TestCase
):
    @classmethod
    ...
    def test_records_second_application_blocker_observation(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["source_application_blocker_audit_item_count"], 48)
        self.assertEqual(summary["source_application_blocker_audit_ready_item_count"], 0)
        self.assertEqual(summary["source_private_application_blocker_audit_record_count"], 48)
        self.assertEqual(summary["prior_application_blocker_observation_count"], 1)
        self.assertEqual(summary["application_blocker_observation_count"], 2)
        self.assertFalse(summary["application_blocked_audit_threshold_met"])
        self.assertEqual(
        ...
        self.assertEqual(summary["application_blocker_threshold_recheck_item_count"], 48)
        ...
        self.assertEqual(summary["binding_ready_after_threshold_recheck_count"], 0)
    ...
    def test_private_threshold_records_match_source_audit_records(self) -> None:
        source_rows = self._read_jsonl(generator.SOURCE_PRIVATE_APPLICATION_BLOCKER_AUDIT_RECORDS_PATH)
        ...
        tracks = Counter(row["intake_track"] for row in threshold_rows)
        ...
        self.assertTrue(
            all(
                row["threshold_recheck_status"]
                == "application_blocker_observation_2_missing_authorized_resolution_application"
                for row in threshold_rows
            )
        )
        self.assertTrue(all(row["application_blocked_audit_threshold_met"] is False for row in threshold_rows))
        self.assertTrue(all(row["binding_ready_after_threshold_recheck"] is False for row in threshold_rows))
    ...
    def test_preserves_source_audit_inputs_and_raw_boundary(self) -> None:
        ...
        self.assertTrue(boundary["source_private_application_blocker_audit_records_read_by_this_phase"])
        self.assertFalse(boundary["source_private_application_blocker_audit_records_mutated_by_this_phase"])
    ...
    def test_matrix_all_pass_and_validator_accepts_private_threshold(self) -> None:
        ...
        self.assertEqual(manifest["summary"]["application_blocker_threshold_recheck_item_count"], 48)
        self.assertEqual(manifest["summary"]["application_blocker_observation_count"], 2)
        self.assertFalse(manifest["summary"]["application_blocked_audit_threshold_met"])
