...
from KMFA.tools import (
    v014_residual_difference_owner_authorized_anchor_confirmation_authorization_intake as generator,
)
from KMFA.tools.check_v014_residual_difference_owner_authorized_anchor_confirmation_authorization_intake import (
    validate,
)
...
class ResidualDifferenceOwnerAnchorAuthorizationIntakeTest(unittest.TestCase):
    @classmethod
    ...
    def test_records_authorization_without_confirming_anchors(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["authorization_item_count"], 72)
        self.assertTrue(summary["owner_authorization_intaken"])
        self.assertTrue(summary["owner_authorized_anchor_confirmation_preparation_allowed_next_phase"])
    ...
    def test_private_authorization_queue_matches_source_queue(self) -> None:
        ...
        self.assertTrue(
            all(
                row["owner_authorization_decision_code"]
                == "AUTHORIZE_CODEX_PREPARE_PRIVATE_ANCHOR_CONFIRMATION"
                for row in auth_rows
            )
        )
        self.assertTrue(
            all(row["owner_authorized_anchor_confirmation_preparation_allowed_next_phase"] is True for row in auth_rows)
        )
        self.assertTrue(all(row["anchor_confirmation_allowed_by_this_phase"] is False for row in auth_rows))
