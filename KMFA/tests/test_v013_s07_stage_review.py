...
class TestV013S07StageReview(unittest.TestCase):
    def test_stage_review_closes_s07_without_upload_raw_read_or_stage8(self) -> None:
        result = validate_v013_s07_stage_review()

        self.assertEqual(result["stage_id"], "S07")
        self.assertEqual(result["review_scope"], "v013_s07_stage_review_only")
        self.assertEqual(result["phase_count"], 3)
        self.assertEqual(result["phase_results"]["S07-P1"], "PASS")
        self.assertEqual(result["phase_results"]["S07-P2"], "PASS")
        self.assertEqual(result["phase_results"]["S07-P3"], "PASS")
        self.assertTrue(result["s07_p1_dependency_validated"])
        self.assertTrue(result["s07_p2_dependency_validated"])
        self.assertTrue(result["s07_p3_dependency_validated"])
        ...
        self.assertIn("S08-P1", result["next_required_step"])
