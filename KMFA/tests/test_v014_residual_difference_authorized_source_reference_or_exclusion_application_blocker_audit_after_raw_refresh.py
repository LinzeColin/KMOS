...
from KMFA.tools.check_v014_residual_difference_authorized_source_reference_or_exclusion_application_blocker_audit_after_raw_refresh import (
    validate,
)
...
ARTIFACT_PATHS = [
    ...
    generator.PRIVATE_AUDIT_RECORDS_PATH,
...
class AuthorizedSourceReferenceOrExclusionApplicationBlockerAuditAfterRawRefreshTest(unittest.TestCase):
    @classmethod
    ...
    def test_audit_locks_all_application_blockers_without_resolution(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(
            summary["version"],
            "0.1.4-residual-difference-authorized-source-reference-or-exclusion-application-blocker-audit-after-raw-refresh",
        )
        self.assertEqual(
            summary["phase_id"],
            "V014_RESIDUAL_DIFFERENCE_AUTHORIZED_SOURCE_REFERENCE_OR_EXCLUSION_APPLICATION_BLOCKER_AUDIT_AFTER_RAW_REFRESH",
        ...
        self.assertEqual(summary["source_application_readiness_item_count"], 48)
        self.assertEqual(summary["source_application_ready_item_count"], 0)
        self.assertEqual(summary["source_application_blocker_item_count"], 48)
        self.assertEqual(summary["application_blocker_audit_item_count"], 48)
        self.assertEqual(summary["application_blocker_audit_ready_item_count"], 0)
        ...
        self.assertEqual(summary["binding_ready_after_application_blocker_audit_count"], 0)
        self.assertEqual(summary["comparison_retry_ready_after_application_blocker_audit_count"], 0)
        self.assertEqual(summary["unresolved_difference_count"], 72)

    def test_private_audit_records_preserve_blocker_distribution(self) -> None:
        audit_rows = self._read_jsonl(generator.PRIVATE_AUDIT_RECORDS_PATH)
        self.assertEqual(len(audit_rows), 48)
        tracks = Counter(row["intake_track"] for row in audit_rows)
        self.assertEqual(tracks["source_reference_or_owner_exclusion"], 40)
        self.assertEqual(tracks["formula_or_non_numeric_mapping"], 8)
        blocker_codes = Counter(row["application_blocker_code"] for row in audit_rows)
        self.assertEqual(blocker_codes["missing_authoritative_source_reference_or_owner_exclusion_application"], 40)
        self.assertEqual(blocker_codes["missing_formula_or_non_numeric_mapping_application"], 8)
        self.assertTrue(all(row["application_blocker_audit_performed_by_this_phase"] is True for row in audit_rows))
        self.assertTrue(all(row["binding_ready_after_application_blocker_audit"] is False for row in audit_rows))
        self.assertTrue(all(row["comparison_retry_ready_after_application_blocker_audit"] is False for row in audit_rows))
        self.assertTrue(all(row["public_commit_allowed"] is False for row in audit_rows))

    def test_preserves_source_readiness_artifacts_and_does_not_read_raw_inbox(self) -> None:
    ...
    def test_matrix_and_validator_accept_private_audit(self) -> None:
        ...
        self.assertEqual(manifest["summary"]["application_blocker_audit_item_count"], 48)
        self.assertEqual(manifest["summary"]["comparison_retry_ready_after_application_blocker_audit_count"], 0)
