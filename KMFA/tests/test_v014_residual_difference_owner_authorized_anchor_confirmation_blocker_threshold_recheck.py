...
from KMFA.tools import (
    v014_residual_difference_owner_authorized_anchor_confirmation_blocker_threshold_recheck as generator,
)
from KMFA.tools.check_v014_residual_difference_owner_authorized_anchor_confirmation_blocker_threshold_recheck import (
    validate,
)
...
class ResidualDifferenceOwnerAuthorizedAnchorBlockerThresholdRecheckTest(unittest.TestCase):
    @classmethod
    ...
    def test_records_second_owner_authorized_anchor_blocker_observation(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["source_owner_authorized_anchor_blocker_observation_count"], 1)
        self.assertEqual(summary["prior_owner_authorized_anchor_blocker_observation_count"], 1)
        self.assertEqual(summary["owner_authorized_anchor_blocker_observation_count"], 2)
    ...
    def test_preserves_source_blocker_audit_inputs(self) -> None:
        ...
        self.assertTrue(boundary["source_private_anchor_blocker_audit_diagnostic_read_by_this_phase"])
        self.assertTrue(boundary["source_private_anchor_blocker_audit_queue_read_by_this_phase"])
        self.assertFalse(boundary["source_private_anchor_blocker_audit_diagnostic_mutated_by_this_phase"])
        self.assertFalse(boundary["source_private_anchor_blocker_audit_queue_mutated_by_this_phase"])
    ...
    def test_private_threshold_queue_matches_source_audit_queue(self) -> None:
        ...
        self.assertTrue(all(row["anchor_confirmation_ready_after_audit"] is False for row in source_rows))
        self.assertTrue(all(row["anchor_confirmation_ready_after_threshold_recheck"] is False for row in threshold_rows))
        self.assertTrue(all(row["raw_to_processed_value_comparison_ready_after_threshold_recheck"] is False for row in threshold_rows))
