...
from KMFA.tools import v014_outside_scope_candidate_review_intake_blocker_audit as generator
from KMFA.tools.check_v014_outside_scope_candidate_review_intake_blocker_audit import validate
...
class OutsideScopeCandidateReviewIntakeBlockerAuditTest(unittest.TestCase):
    @classmethod
    ...
    def test_audit_preserves_keep_pending_counts(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["prior_phase_id"], "V014_OUTSIDE_SCOPE_CANDIDATE_REVIEW_INTAKE_READINESS_RECHECK")
    ...
    def test_records_second_observation_without_blocked_threshold(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["prior_review_intake_blocker_observation_count"], 1)
        self.assertEqual(summary["review_intake_blocker_observation_count"], 2)
    ...
    def test_does_not_mutate_prior_readiness_inputs(self) -> None:
        ...
        self.assertTrue(boundary["prior_public_readiness_summary_read_by_this_phase"])
        self.assertTrue(boundary["prior_private_readiness_diagnostic_read_by_this_phase"])
        self.assertFalse(boundary["prior_private_readiness_diagnostic_mutated_by_this_phase"])
