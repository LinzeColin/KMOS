...
from KMFA.tools import v014_residual_difference_raw_to_processed_comparison_precheck_after_alignment as generator
from KMFA.tools.check_v014_residual_difference_raw_to_processed_comparison_precheck_after_alignment import validate
...
SOURCE_PATHS = [
    ...
    generator.SOURCE_ALIGNMENT_MATRIX_PATH,
...
class ResidualDifferenceRawComparisonPrecheckAfterAlignmentTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_alignment_after_precheck_keeps_formal_comparison_blocked_without_owner_anchors(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["source_alignment_item_count"], 72)
        self.assertEqual(summary["source_raw_candidate_anchor_draft_item_count"], 72)
        self.assertEqual(summary["owner_authorized_comparison_anchor_count"], 0)
        self.assertEqual(summary["alignment_authorization_anchor_required_count"], 72)
    ...
    def test_track_counts_are_preserved_from_alignment(self) -> None:
        summary = self.result["summary"]
    ...
    def test_raw_boundary_excludes_raw_inbox_access_after_alignment(self) -> None:
        boundary = self.result["summary"]["raw_boundary"]
        self.assertTrue(boundary["source_public_alignment_summary_read_by_this_phase"])
        self.assertTrue(boundary["source_private_alignment_read_by_this_phase"])
        self.assertTrue(boundary["source_private_anchor_draft_read_by_this_phase"])
        self.assertTrue(boundary["private_precheck_written_by_this_phase"])
        self.assertFalse(boundary["source_private_alignment_outputs_mutated_by_this_phase"])
    ...
    def test_source_private_alignment_inputs_are_not_mutated(self) -> None:
    ...
    def test_validator_accepts_private_after_alignment_precheck(self) -> None:
