...
class TestV013S08StageReview(unittest.TestCase):
    def test_stage_review_closes_s08_without_upload_raw_read_or_s09(self) -> None:
        result = validate_v013_s08_stage_review()

        self.assertEqual(result["stage_id"], "S08")
        self.assertEqual(result["review_scope"], "v013_s08_stage_review_only")
        self.assertEqual(result["phase_count"], 3)
        self.assertEqual(result["phase_results"]["S08-P1"], "PASS")
        self.assertEqual(result["phase_results"]["S08-P2"], "PASS")
        self.assertEqual(result["phase_results"]["S08-P3"], "PASS")
        ...
        self.assertIn("S09-P1", result["next_required_step"])
