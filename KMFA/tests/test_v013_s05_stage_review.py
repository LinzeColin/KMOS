...
class TestV013S05StageReview(unittest.TestCase):
    def test_stage_review_closes_s05_without_upload_or_raw_mutation(self) -> None:
        result = validate_v013_s05_stage_review()

        self.assertEqual(result["stage_id"], "S05")
        self.assertEqual(result["review_scope"], "v013_s05_stage_review_only")
        self.assertEqual(result["phase_count"], 3)
        self.assertEqual(result["phase_results"]["S05-P1"], "PASS")
        self.assertEqual(result["phase_results"]["S05-P2"], "PASS")
        self.assertEqual(result["phase_results"]["S05-P3"], "PASS")
        self.assertTrue(result["s05_p1_dependency_validated"])
        self.assertTrue(result["s05_p2_dependency_validated"])
        self.assertTrue(result["s05_p3_dependency_validated"])
        ...
        self.assertTrue(result["s05_p1_prior_raw_read_recorded"])
        ...
        self.assertIn("S06-P1", result["next_required_step"])
