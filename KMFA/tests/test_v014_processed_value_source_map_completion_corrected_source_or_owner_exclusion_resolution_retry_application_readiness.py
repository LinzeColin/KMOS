...
from KMFA.tools import (
    v014_processed_value_source_map_completion_corrected_source_or_owner_exclusion_resolution_retry_application_readiness as generator,
)
from KMFA.tools.check_v014_processed_value_source_map_completion_corrected_source_or_owner_exclusion_resolution_retry_application_readiness import (
    validate,
)
...
class CorrectedSourceOrOwnerExclusionRetryApplicationReadinessTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_retry_readiness_passes_for_36_owner_exclusions(self) -> None:
        ...
        self.assertEqual(summary["retry_input_valid_count"], 36)
        self.assertEqual(summary["retry_input_missing_count"], 0)
        self.assertEqual(summary["retry_application_ready_item_count"], 36)
        self.assertEqual(summary["retry_application_blocker_queue_count"], 0)

    def test_readiness_allows_next_application_but_not_this_phase(self) -> None:
        summary = self.result["summary"]
        self.assertTrue(summary["all_36_retry_inputs_valid"])
        self.assertTrue(summary["all_36_unlinked_blockers_resolved"])
        self.assertTrue(summary["resolution_application_ready"])
        self.assertTrue(summary["resolution_application_allowed_next_phase"])
    ...
    def test_public_matrix_is_aggregate_only(self) -> None:
        ...
        self.assertEqual(matrix["application_readiness_check_count"], 8)
        self.assertEqual(matrix["application_readiness_pass_count"], 8)
        self.assertEqual(matrix["application_readiness_fail_count"], 0)
        self.assertEqual(matrix["ready_item_count"], 36)
        self.assertEqual(matrix["application_blocker_queue_count"], 0)
        self.assertTrue(matrix["resolution_application_ready"])
    ...
    def test_raw_boundary_excludes_raw_inbox_access(self) -> None:
        ...
        self.assertTrue(boundary["private_retry_template_read_by_this_phase"])

    def test_validator_accepts_private_readiness(self) -> None:
        manifest = validate(require_private_readiness=True)
        self.assertEqual(manifest["summary"]["retry_application_ready_item_count"], 36)
        self.assertEqual(manifest["summary"]["retry_application_blocker_queue_count"], 0)
        self.assertTrue(manifest["summary"]["resolution_application_allowed_next_phase"])
