...
from KMFA.tools import (
    v014_residual_difference_owner_authorized_anchor_confirmation_authorization_readiness as generator,
)
from KMFA.tools.check_v014_residual_difference_owner_authorized_anchor_confirmation_authorization_readiness import (
    validate,
)
...
ARTIFACT_PATHS = [
    ...
    generator.PRIVATE_READINESS_QUEUE_PATH,
...
SOURCE_INPUT_PATHS = [
    ...
    generator.SOURCE_PRIVATE_AUTHORIZATION_QUEUE_PATH,
    generator.SOURCE_PRIVATE_DIAGNOSTIC_PATH,
    generator.SOURCE_PRIVATE_REPORT_PATH,
]


class ResidualDifferenceOwnerAnchorAuthorizationReadinessTest(unittest.TestCase):
    @classmethod
    ...
    def test_confirms_authorization_readiness_without_confirming_anchors(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["authorization_item_count"], 72)
        self.assertEqual(summary["readiness_ready_item_count"], 72)
        self.assertEqual(summary["readiness_blocker_item_count"], 0)
        self.assertTrue(summary["owner_authorization_readiness_confirmed"])
        self.assertTrue(summary["owner_authorized_anchor_confirmation_preparation_ready_next_phase"])
    ...
    def test_preserves_authorization_intake_inputs(self) -> None:
    ...
    def test_private_readiness_queue_matches_authorization_queue(self) -> None:
        source_rows = self._read_jsonl(generator.SOURCE_PRIVATE_AUTHORIZATION_QUEUE_PATH)
        readiness_rows = self._read_jsonl(generator.PRIVATE_READINESS_QUEUE_PATH)
        self.assertEqual(len(source_rows), 72)
        self.assertEqual(len(readiness_rows), 72)
        self.assertEqual(
            {row[generator.PRIVATE_SLOT_KEY] for row in source_rows},
            {row[generator.PRIVATE_SLOT_KEY] for row in readiness_rows},
        )
        self.assertTrue(all(row["authorization_readiness_status"] == "ready_for_private_preparation" for row in readiness_rows))
        self.assertTrue(
            all(row["owner_authorized_anchor_confirmation_preparation_ready_next_phase"] is True for row in readiness_rows)
        )
        self.assertTrue(all(row["anchor_confirmation_allowed_by_this_phase"] is False for row in readiness_rows))
        self.assertTrue(all(row["public_commit_allowed"] is False for row in readiness_rows))
