...
from KMFA.tools import v014_linked_scope_raw_to_processed_comparison_precheck as generator
from KMFA.tools.check_v014_linked_scope_raw_to_processed_comparison_precheck import validate
...
class LinkedScopeRawToProcessedComparisonPrecheckTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_linked_scope_precheck_counts(self) -> None:
        ...
        self.assertEqual(summary["linked_scope_candidate_record_match_count"], 77)
        self.assertEqual(summary["linked_scope_private_fingerprint_precheck_pair_count"], 77)
        self.assertEqual(summary["linked_scope_exact_fingerprint_match_count"], 77)
        self.assertEqual(summary["linked_scope_fingerprint_mismatch_count"], 0)
        self.assertEqual(summary["linked_scope_missing_candidate_count"], 0)
        self.assertEqual(summary["linked_scope_invalid_materialized_record_count"], 0)
    ...
    def test_precheck_passes_but_full_comparison_stays_closed(self) -> None:
        summary = self.result["summary"]
        self.assertTrue(summary["linked_scope_raw_to_processed_value_comparison_precheck_performed_by_this_phase"])
        self.assertTrue(summary["linked_scope_raw_to_processed_value_comparison_precheck_passed"])
        self.assertTrue(summary["linked_scope_raw_to_processed_value_consistency_prechecked"])
    ...
    def test_public_matrix_is_aggregate_only(self) -> None:
        ...
        self.assertEqual(matrix["linked_scope_precheck_count"], 7)
        self.assertEqual(matrix["linked_scope_precheck_pass_count"], 7)
        self.assertEqual(matrix["linked_scope_precheck_fail_count"], 0)
        self.assertEqual(matrix["linked_scope_exact_fingerprint_match_count"], 77)
        self.assertEqual(matrix["linked_scope_fingerprint_mismatch_count"], 0)
