...
from KMFA.tools import v014_processed_value_source_map_completion_owner_confirmation_response_intake as generator
from KMFA.tools.check_v014_processed_value_source_map_completion_owner_confirmation_response_intake import validate


class ProcessedValueSourceMapCompletionOwnerConfirmationResponseIntakeTest(unittest.TestCase):
    @classmethod
    ...
    def test_owner_confirmation_response_intake_counts(self) -> None:
        summary = self.manifest["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["source_response_packet_item_count"], 3)
        self.assertEqual(summary["source_response_packet_target_slot_count"], 12)
        self.assertEqual(summary["response_draft_item_count"], 3)
        self.assertEqual(summary["valid_owner_confirmation_response_count"], 0)
        self.assertEqual(summary["pending_owner_confirmation_response_count"], 3)
        self.assertEqual(summary["invalid_owner_confirmation_response_count"], 0)
        self.assertEqual(summary["filled_field_counts"], {})
        self.assertEqual(
        ...
        self.assertTrue(summary["owner_confirmation_response_intake_performed"])
    ...
    def test_validator_accepts_private_owner_confirmation_intake(self) -> None:
        manifest = validate(require_private_owner_confirmation_intake=True)
        self.assertEqual(manifest["summary"]["valid_owner_confirmation_response_count"], 0)
        self.assertEqual(manifest["summary"]["pending_owner_confirmation_response_count"], 3)
