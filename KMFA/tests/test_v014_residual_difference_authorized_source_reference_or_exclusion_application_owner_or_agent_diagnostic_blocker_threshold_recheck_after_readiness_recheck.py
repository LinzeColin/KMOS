...
from KMFA.tools.check_v014_residual_difference_authorized_source_reference_or_exclusion_application_owner_or_agent_diagnostic_blocker_threshold_recheck_after_readiness_recheck import (
    validate,
)
...
class AuthorizedSourceReferenceOrExclusionApplicationOwnerOrAgentDiagnosticBlockerThresholdRecheckAfterReadinessRecheckTest(
    unittest.TestCase
):
    @classmethod
    ...
    def test_threshold_recheck_records_third_observation(self) -> None:
    ...
    def test_threshold_recheck_counts_remain_public_safe_and_locked(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["source_private_audit_queue_item_count"], 48)
        ...
        self.assertEqual(summary["source_reference_or_owner_exclusion_blocker_count"], 40)
        self.assertEqual(summary["formula_or_non_numeric_mapping_blocker_count"], 8)
        self.assertEqual(summary["unresolved_difference_count"], 72)

    def test_private_threshold_records_contain_only_blocked_items(self) -> None:
        rows = [
            json.loads(line)
            for line in generator.PRIVATE_THRESHOLD_RECORDS_PATH.read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
        self.assertEqual(len(rows), 48)
        self.assertTrue(all(row["threshold_status"] == "blocked_threshold_met" for row in rows))
        self.assertTrue(all(row["prior_diagnostic_blocker_observation_count"] == 2 for row in rows))
        self.assertTrue(all(row["diagnostic_blocker_observation_count"] == 3 for row in rows))
    ...
    def test_does_not_mutate_source_private_inputs_or_raw_inbox(self) -> None:
        ...
        self.assertTrue(boundary["source_private_audit_diagnostic_read_by_this_phase"])
        self.assertTrue(boundary["source_private_audit_queue_read_by_this_phase"])
        self.assertFalse(boundary["source_private_audit_diagnostic_mutated_by_this_phase"])
        self.assertFalse(boundary["source_private_audit_queue_mutated_by_this_phase"])
    ...
    def test_matrix_and_private_outputs_are_valid(self) -> None:
        ...
        self.assertTrue(self.result["summary"]["private_threshold_diagnostic_gitignored"])
        self.assertTrue(self.result["summary"]["private_threshold_records_gitignored"])
        self.assertTrue(self.result["summary"]["private_threshold_report_gitignored"])

    def test_validator_accepts_private_threshold_recheck(self) -> None:
