...
class AuthorizedSourceReferenceOrExclusionApplicationOwnerOrAgentActionReadinessAfterBlockedHandoffTest(
    unittest.TestCase
):
    ...
    def _load_modules(self) -> tuple[ModuleType, ModuleType]:
        try:
            ...
            self.fail(f"missing owner/authorized agent action readiness generator: {exc.name}")
        try:
            validator = importlib.import_module(VALIDATOR_MODULE)
        except ModuleNotFoundError as exc:
            self.fail(f"missing owner/authorized agent action readiness validator: {exc.name}")
    ...
    def test_action_readiness_stays_blocked_without_new_owner_or_agent_inputs(self) -> None:
        ...
        artifact_paths = [
            ...
            generator.PRIVATE_ACTION_READINESS_DIAGNOSTIC_PATH,
            generator.PRIVATE_ACTION_READINESS_BLOCKER_RECORDS_PATH,
            generator.PRIVATE_ACTION_READINESS_QUESTION_LIST_PATH,
        ]
        ...
        self.assertTrue(summary["source_actionability_blocked_audit_threshold_met"])
        self.assertEqual(summary["owner_action_ready_count"], 0)
        self.assertEqual(summary["owner_action_blocker_count"], 48)
        self.assertEqual(summary["actionable_owner_resolution_count"], 0)
        self.assertEqual(summary["source_reference_or_owner_exclusion_owner_action_blocker_count"], 40)
        self.assertEqual(summary["formula_or_non_numeric_mapping_owner_action_blocker_count"], 8)
        self.assertEqual(summary["binding_ready_after_owner_action_readiness_count"], 0)
        self.assertEqual(summary["comparison_retry_ready_after_owner_action_readiness_count"], 0)
        self.assertEqual(summary["unresolved_difference_count"], 72)

        blocker_rows = self._read_jsonl(generator.PRIVATE_ACTION_READINESS_BLOCKER_RECORDS_PATH)
        self.assertEqual(len(blocker_rows), 48)
        self.assertTrue(
            all(
                row["owner_action_readiness_status"]
                == "blocked_waiting_for_owner_or_authorized_agent_action"
                for row in blocker_rows
            )
        )
        self.assertTrue(all(row["owner_action_ready"] is False for row in blocker_rows))
        self.assertTrue(all(row["public_commit_allowed"] is False for row in blocker_rows))
        self.assertIn(
            "owner/授权代理 action readiness 问题清单",
            generator.PRIVATE_ACTION_READINESS_QUESTION_LIST_PATH.read_text(encoding="utf-8"),
        )
        ...
        self.assertTrue(summary["private_action_readiness_diagnostic_gitignored"])
        self.assertTrue(summary["private_action_readiness_blocker_records_gitignored"])
        self.assertTrue(summary["private_action_readiness_question_list_gitignored"])

        manifest = validator.validate(require_private_action_readiness=True)
        self.assertEqual(manifest["summary"]["owner_action_blocker_count"], 48)
