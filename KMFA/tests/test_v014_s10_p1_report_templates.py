...
from KMFA.tools.v014_s10_p1_report_templates import generate


class TestV014S10P1ReportTemplates(unittest.TestCase):
    def test_locks_v14_report_templates_without_later_phase_or_upload(self) -> None:
        generate()
        result = validate_v014_s10_p1_report_templates()

        self.assertEqual(result["stage_id"], "S10")
        self.assertEqual(result["phase_id"], "S10-P1")
        self.assertEqual(result["phase_scope"], "v014_s10_p1_report_templates_only")
        ...
        self.assertEqual(result["v14_html_uiux_audit_fail_count"], 0)
        self.assertEqual(result["v14_html_uiux_audit_pass_count"], 54)
