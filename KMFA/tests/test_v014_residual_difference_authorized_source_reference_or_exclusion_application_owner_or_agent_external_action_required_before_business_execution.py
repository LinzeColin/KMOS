...
class OwnerOrAuthorizedAgentExternalActionRequiredBeforeBusinessExecutionTest(unittest.TestCase):
    ...
    def _load_modules(self) -> tuple[ModuleType, ModuleType]:
        try:
            ...
            self.fail(f"missing business execution requirement generator: {exc.name}")
        try:
            validator = importlib.import_module(VALIDATOR_MODULE)
        except ModuleNotFoundError as exc:
            self.fail(f"missing business execution requirement validator: {exc.name}")
    ...
    def test_external_action_required_before_business_execution_remains_blocked(self) -> None:
        generator, validator = self._load_modules()
        source_paths = [
            generator.SOURCE_APP_REINSTALL_SUMMARY_PATH,
            generator.SOURCE_APP_REINSTALL_MANIFEST_PATH,
            generator.SOURCE_APP_REINSTALL_GO_NO_GO_PATH,
            generator.SOURCE_APP_REINSTALL_MATRIX_PATH,
            generator.SOURCE_PRIVATE_APP_REINSTALL_REQUIREMENT_DIAGNOSTIC_PATH,
            generator.SOURCE_PRIVATE_APP_REINSTALL_REQUIREMENT_QUEUE_PATH,
            generator.SOURCE_PRIVATE_APP_REINSTALL_REQUIREMENT_REPORT_PATH,
        ]
        artifact_paths = [
            ...
            generator.PRIVATE_BUSINESS_EXECUTION_REQUIREMENT_DIAGNOSTIC_PATH,
            generator.PRIVATE_BUSINESS_EXECUTION_REQUIREMENT_QUEUE_PATH,
            generator.PRIVATE_BUSINESS_EXECUTION_REQUIREMENT_REPORT_PATH,
        ]
        ...
        self.assertEqual(summary["source_phase_id"], source_app_reinstall.PHASE_ID)
        self.assertEqual(summary["source_app_reinstall_requirement_ready_count"], 0)
        self.assertEqual(summary["source_app_reinstall_requirement_blocker_count"], 48)
        self.assertEqual(summary["source_app_reinstall_requirement_required_count"], 48)
        self.assertEqual(summary["source_private_app_reinstall_requirement_queue_item_count"], 48)
        self.assertEqual(summary["goal_status_recommendation"], "blocked")
        self.assertEqual(summary["business_execution_requirement_ready_count"], 0)
        self.assertEqual(summary["business_execution_requirement_blocker_count"], 48)
        self.assertEqual(summary["business_execution_requirement_required_count"], 48)
        self.assertEqual(summary["actionable_owner_resolution_count"], 0)
        self.assertEqual(summary["source_reference_or_owner_exclusion_business_execution_requirement_count"], 40)
        self.assertEqual(summary["formula_or_non_numeric_mapping_business_execution_requirement_count"], 8)
        ...
        rows = self._read_jsonl(generator.PRIVATE_BUSINESS_EXECUTION_REQUIREMENT_QUEUE_PATH)
        self.assertEqual(len(rows), 48)
        self.assertTrue(
            all(
                row["business_execution_requirement_status"]
                == "blocked_owner_or_authorized_agent_external_action_required_before_business_execution"
                for row in rows
            )
        )
        self.assertTrue(all(row["business_execution_requirement_required"] is True for row in rows))
        ...
        self.assertIn(
            ...
            generator.PRIVATE_BUSINESS_EXECUTION_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8"),
        )

        boundary = summary["raw_boundary"]
        self.assertTrue(boundary["source_app_reinstall_public_artifacts_read_by_this_phase"])
        self.assertTrue(boundary["source_private_app_reinstall_requirement_queue_read_by_this_phase"])
        self.assertTrue(boundary["private_business_execution_requirement_queue_written_by_this_phase"])
        ...
        self.assertTrue(summary["private_business_execution_requirement_diagnostic_gitignored"])
        self.assertTrue(summary["private_business_execution_requirement_queue_gitignored"])
        self.assertTrue(summary["private_business_execution_requirement_report_gitignored"])

        manifest = validator.validate(require_private_business_execution_requirement=True)
        self.assertEqual(manifest["summary"]["business_execution_requirement_blocker_count"], 48)
