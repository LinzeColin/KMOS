...
class FundWeeklyAnalysisSkillContractTest(unittest.TestCase):
    ...
    def test_daily_entrypoint_uses_vision_ocr_engine_by_default_when_ready(self) -> None:
        ...
        self.assertIn("--retry-max-rows 64", ocr_call)
    ...
    def test_runner_emits_no_hallucination_package_when_input_folder_exists(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(audit_by_id["c_level_visuals"]["audit_status"], "pass")
            self.assertIn("homepage_chart_size_status=PASS", audit_by_id["c_level_visuals"]["evidence"])
            self.assertIn("homepage_chart_semantics_status=PASS", audit_by_id["c_level_visuals"]["evidence"])
            self.assertEqual(audit_by_id["kmfa_metadata_transform"]["blocking"], "false")
            self.assertIn("metadata_signal_count=", audit_by_id["kmfa_metadata_transform"]["evidence"])
            self.assertEqual(audit_by_id["company_bank_matrix"]["audit_status"], "pending_review")
            self.assertEqual(audit_by_id["company_bank_matrix"]["blocking"], "true")
            self.assertEqual(audit_by_id["internal_transfer_netting"]["audit_status"], "pending_real_rows")
            self.assertEqual(audit_by_id["internal_transfer_netting"]["blocking"], "true")
            self.assertEqual(audit_by_id["operating_cashflow_validation"]["audit_status"], "pending_real_rows")
            self.assertEqual(audit_by_id["operating_cashflow_validation"]["blocking"], "true")
            self.assertEqual(audit_by_id["tax_deposit_loan_project_cost_forecast"]["audit_status"], "pending_known_due_dates")
            self.assertEqual(audit_by_id["tax_deposit_loan_project_cost_forecast"]["blocking"], "true")
            self.assertEqual(audit_by_id["raw_sensitive_runtime_boundary"]["audit_status"], "pass")
            self.assertIn("visible_sensitive_text_status=PASS", audit_by_id["raw_sensitive_runtime_boundary"]["evidence"])
            self.assertEqual(audit_by_id["github_main_runtime_contract"]["audit_status"], "pass")
    ...
    def test_runner_builds_pending_ocr_financial_fact_candidates_without_promoting(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            review_all_csv = run_dir / "ocr_fact_candidate_owner_decision_review_all.csv"
            review_all_xlsx = run_dir / "ocr_fact_candidate_owner_decision_review_all.xlsx"
            self.assertTrue(review_all_csv.exists())
            self.assertTrue(review_all_xlsx.exists())
            with review_all_csv.open(encoding="utf-8-sig", newline="") as f:
                review_all_rows = list(csv.DictReader(f))
            self.assertEqual(len(review_all_rows), len(owner_worklist_rows))
            self.assertEqual(
                [row["fact_candidate_id"] for row in review_all_rows],
                [row["fact_candidate_id"] for row in owner_worklist_rows],
            )
            self.assertTrue(
                all(
                    row["owner_review_completion_status"].startswith("blocked_")
                    for row in review_all_rows
                )
            )
            self.assertEqual(
                sum(
                    row["owner_review_completion_status"] == "ready_for_private_owner_decision_manifest_no_write"
                    for row in review_all_rows
                ),
                0,
            )
            self.assertTrue(all(row["fund_ledger_write_allowed"] == "false" for row in review_all_rows))
            self.assertTrue(all(row["financial_fact_promoted"] == "false" for row in review_all_rows))
            self.assertTrue(all(row["management_conclusion_allowed"] == "false" for row in review_all_rows))

            workbook_validation = subprocess.run(
                [
                    sys.executable,
            ...
            self.assertEqual(workbook_validation.returncode, 0, workbook_validation.stderr)
            workbook_validation_payload = json.loads(workbook_validation.stdout)
            self.assertEqual(workbook_validation_payload["status"], "OWNER_REVIEW_WORKBOOK_READY")
            self.assertEqual(workbook_validation_payload["owner_review_data_row_count"], len(owner_worklist_rows))
            self.assertTrue(workbook_validation_payload["write_flags_all_false"])
            self.assertFalse(workbook_validation_payload["fund_ledger_write_allowed"])
            self.assertFalse(workbook_validation_payload["financial_fact_promoted"])
            self.assertFalse(workbook_validation_payload["management_conclusion_allowed"])
            ...
            with (run_dir / "owner_decision_readiness_gate.csv").open(encoding="utf-8-sig", newline="") as f:
                owner_decision_gate_rows = list(csv.DictReader(f))
            self.assertEqual(len(owner_decision_gate_rows), 1)
            owner_decision_gate = owner_decision_gate_rows[0]
            self.assertEqual(owner_decision_gate["candidate_count"], "3")
            self.assertEqual(owner_decision_gate["ready_count"], "0")
            self.assertEqual(owner_decision_gate["blocking_count"], "3")
            self.assertEqual(owner_decision_gate["missing_owner_decision_manifest_count"], "3")
            self.assertEqual(owner_decision_gate["pending_owner_review_count"], "3")
            self.assertEqual(owner_decision_gate["readiness_status"], "blocked_missing_owner_decision_manifest")
            self.assertEqual(
                owner_decision_gate["owner_decision_manifest_relative_path"],
                f"KMFA/metadata/fund_weekly_analysis/private_runtime/ocr_fact_candidate_owner_decisions/{run_id}.json",
            )
            self.assertEqual(owner_decision_gate["owner_decision_manifest_write_allowed"], "false")
            self.assertEqual(owner_decision_gate["fund_ledger_write_allowed"], "false")
            self.assertEqual(owner_decision_gate["financial_fact_promoted"], "false")
            self.assertEqual(owner_decision_gate["management_conclusion_allowed"], "false")
            ...
            self.assertEqual(cross_review["ocr_fact_candidate_owner_decision_review_all_count"], 3)
            self.assertEqual(cross_review["ocr_fact_candidate_owner_decision_review_all_ready_count"], 0)
            self.assertEqual(cross_review["ocr_fact_candidate_owner_decision_review_all_blocking_count"], 3)
            self.assertTrue(cross_review["ocr_fact_candidate_owner_decision_review_all_xlsx_ready"])
            ...
            self.assertEqual(cross_review["owner_decision_readiness_gate_count"], 1)
            self.assertEqual(cross_review["owner_decision_readiness_gate_ready_count"], 0)
            self.assertEqual(cross_review["owner_decision_readiness_gate_blocking_count"], 1)
            self.assertEqual(cross_review["owner_decision_readiness_status"], "blocked_missing_owner_decision_manifest")
    ...
    def test_owner_decision_readiness_gate_summarizes_manifest_blockers_no_write(self) -> None:
        runner = load_fund_runner_module()
        manifest = {"run_id": "owner_decision_readiness_gate_test"}
        missing_manifest_rows = [
            {
                "decision_validation_status": "missing_decision_manifest",
                "decision_preview_status": "blocked_missing_owner_decision_manifest",
        ...
        missing_gate = runner.build_owner_decision_readiness_gate_rows(manifest, missing_manifest_rows)[0]

        self.assertEqual(missing_gate["readiness_status"], "blocked_missing_owner_decision_manifest")
        self.assertEqual(missing_gate["candidate_count"], "2")
        self.assertEqual(missing_gate["ready_count"], "0")
        self.assertEqual(missing_gate["blocking_count"], "2")
        self.assertEqual(missing_gate["missing_owner_decision_manifest_count"], "2")
        self.assertEqual(missing_gate["pending_owner_review_count"], "2")
        self.assertEqual(missing_gate["owner_decision_manifest_write_allowed"], "false")
        self.assertEqual(missing_gate["fund_ledger_write_allowed"], "false")
        self.assertEqual(missing_gate["financial_fact_promoted"], "false")
        self.assertEqual(missing_gate["management_conclusion_allowed"], "false")

        ready_rows = [
            {
                "decision_validation_status": "valid_owner_decision_validation_only",
                "decision_preview_status": "ready_for_private_ocr_fact_authorization_update_no_write",
                "owner_authorization_decision": "approve_for_review_authorization",
                "owner_corrected_company": "武汉开明",
                "owner_corrected_bank": "招商银行",
            },
        ]
        ready_gate = runner.build_owner_decision_readiness_gate_rows(manifest, ready_rows)[0]

        self.assertEqual(ready_gate["readiness_status"], "ready_for_private_ocr_fact_authorization_update_no_write")
        self.assertEqual(ready_gate["candidate_count"], "1")
        self.assertEqual(ready_gate["ready_count"], "1")
        self.assertEqual(ready_gate["blocking_count"], "0")
        self.assertEqual(ready_gate["approved_for_authorization_count"], "1")
        self.assertEqual(ready_gate["owner_decision_manifest_write_allowed"], "false")
        self.assertEqual(ready_gate["fund_ledger_write_allowed"], "false")
        self.assertEqual(ready_gate["financial_fact_promoted"], "false")
        self.assertEqual(ready_gate["management_conclusion_allowed"], "false")
