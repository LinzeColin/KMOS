# -*- coding: utf-8 -*-
"""项目成本是**下限**这件事，必须是数，不能是形容词。

2026-07-30 拿 Owner 的 8 份真实《竣工项目财务报表》对了一次线上产物：
**0/8 落在 ±10% 内，而且 8 个全部偏低**——区间 -10.4% ～ -79.3%，
新疆宜化 064 偏 -79.3%（报表 1,563,121.52，产物 323,932.08）。

偏低不是解析错。解析这边该做的都做了：按列名取数、按 CRC 去重账簿、
取借方发生额不取净额、合同号按主号归并。偏低是**账上就没把钱挂到项目**：
064 的承包费在竣工报表上是 930,152.75，在金蝶该项目名下是 0，
那笔钱记在 `不分项目` 桶里。
...
def test_the_payload_quantifies_the_understatement():
    ...
    #
    # 判据只盖**这一块**。仓里既有的 docstring 里有历史举证金额
    # （如「广安台泥 079 的金蝶成本 64,653.90」），那是另一件事——
    # Owner 已定「KMOS 长期 public，历史不动，洗白单独做」，这里不越界去改它。
