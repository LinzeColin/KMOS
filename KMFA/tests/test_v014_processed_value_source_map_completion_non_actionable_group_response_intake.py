...
from KMFA.tools import v014_processed_value_source_map_completion_non_actionable_group_response_intake as generator
from KMFA.tools.check_v014_processed_value_source_map_completion_non_actionable_group_response_intake import validate


class ProcessedValueSourceMapCompletionNonActionableGroupResponseIntakeTest(unittest.TestCase):
    @classmethod
    ...
    def test_response_intake_counts(self) -> None:
        ...
        self.assertEqual(summary["source_non_actionable_group_count"], 3)
        self.assertEqual(summary["source_non_actionable_target_slot_count"], 12)
        self.assertEqual(summary["response_group_count"], 3)
        self.assertEqual(summary["response_target_slot_count"], 12)
        self.assertEqual(summary["ready_for_intake_group_count"], 0)
        self.assertEqual(summary["ready_for_intake_target_slot_count"], 0)
        self.assertEqual(summary["pending_response_group_count"], 3)
        self.assertEqual(summary["pending_response_target_slot_count"], 12)
        self.assertEqual(summary["invalid_response_group_count"], 0)
        self.assertEqual(summary["invalid_response_target_slot_count"], 0)
        self.assertEqual(
            summary["resolution_decision_code_counts"],
            {"KEEP_PENDING_WITH_REASON": 2, "REQUEST_ADDITIONAL_SOURCE_EVIDENCE": 1},
        )
        self.assertFalse(summary["owner_or_authorized_delegate_resolution_supplied"])
    ...
    def test_validator_accepts_private_response_intake(self) -> None:
        manifest = validate(require_private_response_intake=True)
        self.assertEqual(manifest["summary"]["ready_for_intake_group_count"], 0)
        self.assertEqual(manifest["summary"]["pending_response_group_count"], 3)
