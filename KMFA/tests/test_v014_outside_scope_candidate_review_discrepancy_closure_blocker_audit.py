...
from KMFA.tools import v014_outside_scope_candidate_review_discrepancy_closure_blocker_audit as generator
from KMFA.tools.check_v014_outside_scope_candidate_review_discrepancy_closure_blocker_audit import validate
...
ARTIFACT_PATHS = [
    ...
    generator.PRIVATE_RESIDUAL_QUEUE_PATH,
    generator.PRIVATE_RESIDUAL_REPORT_PATH,
]

SOURCE_INPUT_PATHS = [
    generator.SOURCE_SUMMARY_PATH,
    generator.SOURCE_MANIFEST_PATH,
    generator.SOURCE_PRIVATE_BLOCKING_QUEUE_PATH,
    generator.SOURCE_PRIVATE_WORKPACK_PATH,
]


class OutsideScopeCandidateReviewDiscrepancyClosureBlockerAuditTest(unittest.TestCase):
    @classmethod
    ...
    def test_audit_preserves_source_closure_counts(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["decision"], "NO_GO")
        self.assertEqual(summary["source_phase_id"], "V014_OUTSIDE_SCOPE_CANDIDATE_REVIEW_DISCREPANCY_CLOSURE_READINESS")
        self.assertEqual(summary["source_closure_plan_item_count"], 72)
        self.assertEqual(summary["source_closure_ready_item_count"], 0)
        self.assertEqual(summary["source_closure_blocked_item_count"], 72)
        self.assertEqual(summary["source_safe_auto_closure_count"], 0)

    def test_writes_residual_blocker_queue_without_closing_discrepancies(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["source_private_blocking_queue_item_count"], 72)
        self.assertEqual(summary["residual_blocker_queue_item_count"], 72)
        self.assertEqual(summary["open_closure_blocker_count"], 72)
    ...
    def test_does_not_mutate_source_private_inputs(self) -> None:
        ...
        self.assertTrue(boundary["source_private_closure_blocker_queue_read_by_this_phase"])
        self.assertFalse(boundary["source_private_closure_blocker_queue_mutated_by_this_phase"])
        self.assertFalse(boundary["source_private_closure_workpack_mutated_by_this_phase"])
