...
from KMFA.tools import (
    v014_outside_scope_candidate_review_residual_difference_source_map_correction_blocker_audit as generator,
)
from KMFA.tools.check_v014_outside_scope_candidate_review_residual_difference_source_map_correction_blocker_audit import (
    validate,
)
...
class ResidualDifferenceSourceMapCorrectionBlockerAuditTest(unittest.TestCase):
    @classmethod
    ...
    def test_records_first_source_map_correction_blocker_observation(self) -> None:
        ...
        self.assertEqual(summary["source_map_correction_blocker_observation_count"], 1)
    ...
    def test_preserves_source_readiness_inputs(self) -> None:
        ...
        self.assertTrue(boundary["source_response_import_readiness_private_diagnostic_read_by_this_phase"])
        self.assertTrue(boundary["source_response_import_readiness_private_blocker_queue_read_by_this_phase"])
        self.assertFalse(boundary["source_response_import_readiness_private_diagnostic_mutated_by_this_phase"])
        self.assertFalse(boundary["source_response_import_readiness_private_blocker_queue_mutated_by_this_phase"])
    ...
    def test_private_audit_queue_matches_source_blocker_rows(self) -> None:
        source_rows = self._read_jsonl(generator.SOURCE_PRIVATE_BLOCKER_QUEUE_PATH)
        ...
        self.assertTrue(all(row["source_map_correction_ready_after_recheck"] is False for row in source_rows))
        self.assertTrue(all(row["source_map_correction_ready_after_audit"] is False for row in audit_rows))
        self.assertTrue(all(row["discrepancy_closed_after_audit"] is False for row in audit_rows))
