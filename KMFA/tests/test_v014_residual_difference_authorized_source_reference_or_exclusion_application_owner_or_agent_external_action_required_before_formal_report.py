...
class OwnerOrAuthorizedAgentExternalActionRequiredBeforeFormalReportTest(unittest.TestCase):
    ...
    def _load_modules(self) -> tuple[ModuleType, ModuleType]:
        try:
            ...
            self.fail(f"missing formal report requirement generator: {exc.name}")
        try:
            validator = importlib.import_module(VALIDATOR_MODULE)
        except ModuleNotFoundError as exc:
            self.fail(f"missing formal report requirement validator: {exc.name}")
    ...
    def test_external_action_required_before_formal_report_remains_blocked(self) -> None:
        generator, validator = self._load_modules()
        source_paths = [
            generator.SOURCE_LINEAGE_FULL_CHECK_SUMMARY_PATH,
            generator.SOURCE_LINEAGE_FULL_CHECK_MANIFEST_PATH,
            generator.SOURCE_LINEAGE_FULL_CHECK_GO_NO_GO_PATH,
            generator.SOURCE_LINEAGE_FULL_CHECK_MATRIX_PATH,
            generator.SOURCE_PRIVATE_LINEAGE_FULL_CHECK_REQUIREMENT_DIAGNOSTIC_PATH,
            generator.SOURCE_PRIVATE_LINEAGE_FULL_CHECK_REQUIREMENT_QUEUE_PATH,
            generator.SOURCE_PRIVATE_LINEAGE_FULL_CHECK_REQUIREMENT_REPORT_PATH,
        ]
        artifact_paths = [
            ...
            generator.PRIVATE_FORMAL_REPORT_REQUIREMENT_DIAGNOSTIC_PATH,
            generator.PRIVATE_FORMAL_REPORT_REQUIREMENT_QUEUE_PATH,
            generator.PRIVATE_FORMAL_REPORT_REQUIREMENT_REPORT_PATH,
        ]
        ...
        self.assertEqual(summary["source_phase_id"], source_lineage_full_check.PHASE_ID)
        self.assertEqual(summary["source_lineage_full_check_requirement_ready_count"], 0)
        self.assertEqual(summary["source_lineage_full_check_requirement_blocker_count"], 48)
        self.assertEqual(summary["source_lineage_full_check_requirement_required_count"], 48)
        self.assertEqual(summary["source_private_lineage_full_check_requirement_queue_item_count"], 48)
        self.assertEqual(summary["goal_status_recommendation"], "blocked")
        self.assertEqual(summary["formal_report_requirement_ready_count"], 0)
        self.assertEqual(summary["formal_report_requirement_blocker_count"], 48)
        self.assertEqual(summary["formal_report_requirement_required_count"], 48)
        self.assertEqual(summary["actionable_owner_resolution_count"], 0)
        self.assertEqual(summary["source_reference_or_owner_exclusion_formal_report_requirement_count"], 40)
        self.assertEqual(summary["formula_or_non_numeric_mapping_formal_report_requirement_count"], 8)
        ...
        self.assertEqual(summary["formal_report_ready_count"], 0)
        self.assertEqual(summary["unresolved_difference_count"], 72)

        rows = self._read_jsonl(generator.PRIVATE_FORMAL_REPORT_REQUIREMENT_QUEUE_PATH)
        self.assertEqual(len(rows), 48)
        self.assertTrue(
            all(
                row["formal_report_requirement_status"]
                == "blocked_owner_or_authorized_agent_external_action_required_before_formal_report"
                for row in rows
            )
        )
        self.assertTrue(all(row["formal_report_requirement_required"] is True for row in rows))
        self.assertTrue(all(row["formal_report_ready"] is False for row in rows))
        self.assertTrue(all(row["formal_report_allowed"] is False for row in rows))
        ...
        self.assertIn(
            ...
            generator.PRIVATE_FORMAL_REPORT_REQUIREMENT_REPORT_PATH.read_text(encoding="utf-8"),
        )

        boundary = summary["raw_boundary"]
        self.assertTrue(boundary["source_lineage_full_check_public_artifacts_read_by_this_phase"])
        self.assertTrue(boundary["source_private_lineage_full_check_requirement_queue_read_by_this_phase"])
        self.assertTrue(boundary["private_formal_report_requirement_queue_written_by_this_phase"])
        ...
        self.assertFalse(boundary["formal_report_generated_by_this_phase"])
        ...
        self.assertTrue(summary["private_formal_report_requirement_diagnostic_gitignored"])
        self.assertTrue(summary["private_formal_report_requirement_queue_gitignored"])
        self.assertTrue(summary["private_formal_report_requirement_report_gitignored"])

        manifest = validator.validate(require_private_formal_report_requirement=True)
        self.assertEqual(manifest["summary"]["formal_report_requirement_blocker_count"], 48)
