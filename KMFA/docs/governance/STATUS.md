...
- project_id: `KMFA`
- version: `0.1.0-s06-p2-cross-source-difference-queue`
- current_stage: `S06`
...
- S06-P3 metadata/quality 运行时证据输出、事实层、lineage 完整检查、报告、UI 和外部接口尚未完成；S05-P3 authority baseline lock、S06-P1 validator 和 S06-P2 difference queue 不代表正式报告可发布。

## 阻塞条件

- 不能把 Stage 1 治理基线当成业务 MVP。
- 不能上传原始敏感经营数据。
- S05-P3 已完成 40 条 public-safe hash/source-anchor 字段锁定并排除 5 条 Excel 字段，Stage 5 review/upload、S06-P1 和 S06-P2 已完成；下一步只能执行 S06-P3，不能跳到 UI、报告、事实层或外部接口。
