...
- status: `s05p2_owner_decision_application_preview_blocked_expected`
...
- S05-P2 已完成 Excel 候选机器复核记录、owner 决策包、owner decision validator、owner decision intake validator、三种 owner 决策模板、owner decision application preview 和 completion gate：当前证据只能证明交叉来源支持，不能安全机器合成为单一 A0 项目基准；application preview 与 completion gate 在 5 条 Excel 字段 pending 且无 active owner/授权决策时按预期阻断，仍需 owner 或授权私有映射决策。

## 未完成

- S05-P2 Excel 候选 5 条字段 hash/source anchor 回填或 owner/授权人工决策、S05-P3 权威锁定、zero-delta、事实层、报告、UI 和外部接口尚未完成；intake validator、模板、application preview 和 completion gate 只验证/辅助/阻断公开安全边界。

## 阻塞条件

- 不能把 Stage 1 治理基线当成业务 MVP。
- 不能上传原始敏感经营数据。
- S05-P2 已完成 40/45 hash-only 部分回填并记录 Excel 机器复核、owner 决策包、intake validator、决策模板、application preview 和 completion gate blocked-as-expected 证据，但 5 条 Excel 字段仍 pending；下一步仍只能执行 S05-P2 Excel 私有字段回填或 owner/授权人工豁免/不适用/降级决策，不能进入 S05-P3。
