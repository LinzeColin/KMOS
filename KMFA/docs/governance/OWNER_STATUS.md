...
KMFA 已在 v1.2 FULL_HTML_NO_OMISSION 基线上完成 `S04-P1｜金额工具`、`S04-P2｜字段标准化` 和 `S04-P3｜基础工具测试`：现在可以把金额标准化为整数分、检查 KMFA Python 代码中禁止的 float 金额用法，把通用中文字段映射到日期、期间、主体、项目、客户/对手方和合同编号 canonical fields，并用 22 个合成边界用例生成工具函数测试报告；项目仍不是可用业务系统。
...
下一步只执行 Stage 4 整体复审，修复复审问题后才允许整体上传 GitHub；不扩大到 S05、UI、报告、事实层、zero-delta 或自动接口。
