...
## Weekly Monday/Saturday 11:00 Australia/Sydney run

The local scheduler uses Sydney local time. Every Monday and Saturday at 11:00 Australia/Sydney is the operational schedule. Beijing time must not be used as the scheduler timezone.
...
18b. The runner automatically creates full private owner review files `ocr_fact_candidate_owner_decision_review_all.csv` and `ocr_fact_candidate_owner_decision_review_all.xlsx` whenever `ocr_fact_candidate_owner_worklist.csv` has rows. The all-packet covers every worklist row, uses the same XLSX protection and summary structure as the batch export, and must remain no-write/no-promote/no-conclusion. It replaces the old manual all-export step only; owner-edited intake still requires `validate_owner_review_workbook.py` and `install_owner_decision_manifest.py` dry-run before any acknowledged private manifest write.
19. After a successful scheduled runner invocation, first let the runner reuse any matching, non-empty private OCR sidecars already present in ignored private runtime. If OCR coverage is still missing, run `tools/generate_screenshot_ocr_sidecars.py --engine vision --apply --run-dir <run_dir>` to write `screenshot_ocr_sidecar_generation_plan.csv`, `screenshot_ocr_sidecar_generation_summary.json`, and non-empty local Vision OCR text sidecars under private runtime. Existing successful plan rows must be preserved and new OCR batches must append with the next generation id. If the summary reports newly generated sidecars, rerun `tools/run_fund_weekly_analysis.py` once with the same `run_id` so private OCR sidecars are indexed into `ocr_text_candidates.csv`, `ocr_value_candidates.csv`, and OCR fact review outputs as pending-review candidates. Manual use stays dry-run unless `--apply` is explicit. OCR text must remain private runtime only, empty OCR output must not be written, and generated or reused OCR must not be promoted into ledger amounts without review. Use bounded batches with `--vision-batch-size` and per-batch `--timeout-seconds` so a slow Vision batch cannot hang the whole run; scheduled runs additionally retry timed-out rows with `--retry-timeout-seconds 30 --retry-batch-size 1`.
19a. After runner/OCR/package validation, run `tools/check_delivery_acceptance.py --run-id <run_id>`. A passing engineering delivery with missing owner-reviewed values returns `DELIVERY_ACCEPTANCE_READY_WITH_OWNER_BLOCKERS`; this confirms native Excel/PDF, automation/source/zip/Git parity, OCR reuse, no hallucinated generated amounts, owner review all-packet readiness, and fail-closed owner blockers, but still does not write owner manifests, promote facts, write formal ledgers, or release management conclusions.
...
29. Build `goal_completion_audit.csv`: record final-objective requirement status and evidence, including C-level chart quality, KMFA metadata transform, private runtime boundary, and main-only runtime contract, without granting formal promotion or conclusion authority. Keep company-bank matrix, internal-transfer netting, cashflow validation, and known due-date forecasting rows `blocking=true` when the current run lacks the real reviewed rows needed to evidence those objective requirements.
...
39. Build `fact_promotion_execution_authorization_template.json` and `fact_promotion_execution_authorization_preview.csv`: create a private execution authorization draft under `fact_promotion_execution_authorizations/<run_id>.json` and validate coverage only. Every `ready_for_owner_execution_authorization_no_write` execution plan row must be explicitly authorized in the private manifest before any row may reach `ready_for_controlled_execution_run_no_write`; incomplete coverage must emit `blocked_incomplete_execution_authorization_coverage`. Every row must keep `source_mutation_allowed=false`, `fact_promotion_execution_allowed=false`, `fund_ledger_write_allowed=false`, `financial_fact_promoted=false`, and `management_conclusion_allowed=false`.
...
```bash
python3 KMFA/skills/资金周报/tools/materialize_fund_source.py \
