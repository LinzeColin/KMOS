...
* `tools/check_codex_app_automation.py` must also validate the tracked contract itself. If the contract drifts from Monday/Saturday 11:00 `Australia/Sydney`, the weekly Monday/Saturday 11:00 prompt, the governed OneDrive input path, the source readiness gate, the canonical project worktree policy, or the deferred App parity phase, it must return `CODEX_AUTOMATION_CONTRACT_INVALID` even when the live local automation matches the bad contract. `--contract-only` validates the public contract before upload while live parity remains deferred to `V014_APP_REINSTALL_AND_PARITY`.
* The tracked branch/worktree policy token is `canonical_project_worktree_no_new_branch_no_pr_no_extra_worktree`.
...
The editable workbook template is a private local prerequisite:

      KMFA/metadata/fund_weekly_analysis/private_runtime/templates/fund_weekly_template.xlsx

`KMFA_FUND_TEMPLATE_PATH` may point to another ignored local copy. The validated private template keeps `01_首页总览` first, clears visible report-page row 2 explanatory text, shows the requested 4+4 homepage KPI card order, and contains exactly two homepage native line charts for the latest 15 and 30 daily snapshots. Missing templates fail closed with `PRIVATE_TEMPLATE_MISSING` before source reads. The template and generated production outputs belong under private runtime and must not be committed or packaged in a public task-pack ZIP.
