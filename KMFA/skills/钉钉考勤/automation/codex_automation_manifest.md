...
| 名称 | 考勤异常简报 每工作日发送（生产管理群） | 考勤看门狗 + 生产部周报（生产管理群） |
| rrule | `FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR;BYHOUR=19,20,21;BYMINUTE=15` | 同前缀，`BYHOUR=10,11,12;BYMINUTE=50` |
| 本机时间 | 每工作日 19:15 / 20:15 / 21:15 | 每工作日 10:50 |
| 调的命令 | `scripts/kmfa_brief_cron.sh` | `scripts/kmfa_weekly_cron.sh`（一条，内含看门狗 + 周报） |
...
## 生产部周报：连续在岗（2026-09-16 起）

跑在 `automation-2` 的**唯一**一条命令 `scripts/kmfa_weekly_cron.sh` 上 —— 不新建 automation。
工作日上午（本机 10:50 = 北京 08:50），看上一个自然日，发生产管理群。
它取代了 2026-09-15 上线的「出勤越线通知 + 周一月报」：那一版按自然月累计出勤天数判，
一个月只在第四周炸一次，一炸 17–21 人（六到七成部门），其余三周全哑 —— 没有区分度。

**一周一条，周一出。** 台账按 ISO 周去重（`考勤周报台账.json`）：
周一那趟没跑成（机器睡着 / 网关阵发 / Codex 没触发），周二到周五的同一个槽位
会接着出同一周那条；**发成功才记账**，所以补得上，也不会重发。

| 报文块 | 判据 |
| --- | --- |
| 连续在岗名单 | 连续到岗满 `KMFA_BRIEF_REST_THRESHOLD`（24）天。休一天即归零，自己从名单上消失 |
| 在岗待核实 | 在册却连续 `KMFA_BRIEF_ZERO_PUNCH_DAYS`（10）天一次卡没打，**且没走过请假流程** |
| 打卡规范 | 本月 1 日至业务日，迟到 / 缺卡 / 补卡累计 ≥ `KMFA_BRIEF_DISCIPLINE`（3）次 |

### 为什么 24 天不会误伤双休岗 —— 是算术，不是名单

每周只要休 1 天，连续到岗最长就是 12 天（上周休周一、这周休周日）。
要满 24 天，必须**连着三周一天没休**。所以这条线天然筛掉所有正常休息的人，
办公室 / 行政 / 管理 / 新人 / 调岗 / 转外派全都不用管，**不需要维护任何岗位白名单**。
180 天全量实测佐证：双休岗最大连续 7–19 天，一线 24–87 天，**交集 0 人**。

人员流动因此是零维护的：每轮重新拉通讯录和人员表，入职自动进、离职自动出、
调岗自动换项目、转双休岗自动不再出现。

### 唯一分组口径：人员表的「项目名称」列

**不用钉钉考勤组的地点，也不用钉钉部门（车工组 / 焊工组…）。**
考勤组的地点是滞后的 —— 实测 2026-09-14：有人钉钉考勤组挂在 A 现场，
人员表上人却在 B 现场，两地相隔数千公里。班组也解决不了跨项目的排休，项目经理才能。
混两个口径等于没有口径。

人员表项目为空 + 类别检修 = 人在厂内。姓名一律过 `roster.arbitrate`
（OCR 会把同音近形的姓名认错，实测有名字被整整认错两个字）。从业务日往回最多找 5 天 ——
周报跑在周一、业务日是周日、那天没有人员表，必须能回溯到周五；
名单里的人全有项目就停，常态只解析一张表（实测一张 18 秒，OCR 走 macOS 系统
Vision，离线、确定性、零 token）。一张都取不到就降级：名单照发，一人一行，
**不拿钉钉班组凑第二个口径**。

### 取数窗口只要 32 天

只判「够不够门槛」、不对外报天数，所以窗口比门槛长就够。32 天正好是
`query-data` 的单次跨度上限，于是永远只有一个分片、**2 次调用**。
实测 32 天窗口与 180 天窗口算出的名单完全一致，而调用次数是 2 对 12 ——
钉钉网关是阵发性失败，少一次调用就少一次暴露。整轮实测 84–98 秒，硬墙钟 900 秒。
...
看门狗跑在 `automation-2` 上，它守的是 `automation`。可 `automation-2` 整个停掉的时候，
看门狗和周报是**一起停**的 —— 那一侧没有任何人会说话。
2026-09-15 实测真发生过：`automation-2` 的 `last_run_at` 停在 09-14 晚上，
09-15 早上那趟根本没触发，而当天没有任何告警。

两处补上：

- **`runtime.peer_check()`**：日报（`automation`）成功出报之后顺手查一眼 `automation-2`
  还在不在、上一个工作日跑过没有，不对就私聊。放在发送之后 —— 出报是交付物，
  互查是附带的，绝不让它挡住简报。判据写成「上一个工作日之前」而不是「多少小时」：
  这条线每工作日一次，周五到周一隔着 72 小时，按小时判每个周一都会误报。
- **`automation-2` 从单槽位改成三槽位**（本机 10:50 / 11:50 / 12:50）。
  单点 + 自己守自己 = 静默失效。三个钟点在 AEST 下全部落在北京 08:00–12:00，
  夏令时切到 AEDT 之后仍有两个有效。自检里有一条守卫断言「窗口内至少两趟」。

## 判读下沉到脚本，调度侧只抄一行

调度侧跑的是 SCNet 那个小模型。让它对着两条命令、两张十几行的标记表做判读，
等于把业务判断交给一个判不了的东西。现在：

- `automation-2` 只调一条命令，内部跑完看门狗 + 周报，自己取更严重的那个结论；
- 两条线的 python 都在 `finally` 里打一行 `ACTION: NONE|ACT|ESCALATE`
  （`runtime.emit_action()`，由最后一个结论性标记推出来）；
- 包装脚本从本轮 stderr 里取那一行往外打，取不到就 `ESCALATE`；
- automation 的 prompt 只剩「跑这条命令 → 把最后那行 ACTION 抄到第一行」。

自检里有一条守卫查 `automation-2` 只调一条命令，另一条查两个 python 都打了 ACTION。

## 两条硬闸，两条线都挂

**① 报文里不许出现这 15 个词**（`runtime.BANNED`）：
加班、工时、小时、劳动法、仲裁、违法、超时、疲劳、连续工作、未休、旷工、
加班费、法定、赔偿、举报。
要求是「哪怕员工拿着这些记录去举报，也是对公司有利的」——
所以报文只下排班指令，不做任何事实认定，也不逐人印天数：门槛写在开头一句话里，
谁在名单上、为什么在，一看就懂；把天数贴在每个人名字后面，
就从「排班待办」变成了「用工强度记录」。
闸在**投递之前**，而且**干跑也过闸**（日报以前把它写在「干跑就 return」的后面，
于是干跑永远看不出报文会被拒）。

**② 干跑在调用图上就够不着投递。**
2026-09-15 栽过一次：当时的 `run_attendance_monthly.py` 里 `--dry-run` 只被拿去跳过台账，
