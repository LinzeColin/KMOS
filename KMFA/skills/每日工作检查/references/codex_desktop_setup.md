...
Use Codex Desktop locally, not Codex Cloud, to install and maintain the `每日工作检查`（id `daily_routine_check_skill`）under:

```text
/Users/linzezhang/Documents/Codex/KMOS/KMFA/skills/每日工作检查
```
...
6. Preserve the governance style used by `钉钉考勤`（id `kmfa-dingtalk-attendance-skill`）.
