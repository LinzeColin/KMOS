...
- 输入：固定 DWS `v1.0.58-beta.1` 先以 `dws chat message search-advanced --conversation-ids <唯一群ID> --conversation-type group --sender-ids <唯一发送人ID>` 按固定时间窗与 opaque `nextCursor` 分页读取历史消息。若且仅若该接口返回终页但缺失显式记录列表，才改用官方 `dws chat +chat-messages --group <同一唯一群> --start ... --end ... --order asc --page-all`：由 DWS 在进程内按 provider 毫秒 cursor 完成整个窗口，只有 `complete=true`、`hasMore=false`、无失败/截断且群/发送人身份齐全才交给后续流程。任一不完整或身份缺失均失败，绝不以时间戳自行续页。消息仍须逐条匹配唯一 `senderOpenDingTalkId` 与允许文档族（资金账户明细表、资金流水明细/资金明细）；显示名 `sender` 只能用于人工诊断，绝不能作为来源 ID。
