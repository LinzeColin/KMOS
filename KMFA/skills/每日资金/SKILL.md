...
- 输入：固定 DWS `v1.0.58-beta.1` 先以 `dws chat message search-advanced --conversation-ids <唯一群ID> --conversation-type group --sender-ids <静态发送人允许列表>` 按固定时间窗与 opaque `nextCursor` 分页读取历史消息。允许列表由 `DAILY_FUNDS_SENDER_IDS` 在部署时显式配置，最多 12 个，不从群成员动态发现；`DAILY_FUNDS_SENDER_ID` 是列表内的主发送人，保留以兼容原有单发送人部署。若且仅若该接口返回终页但缺失显式记录列表，才改用官方 `dws chat +chat-messages --group <同一唯一群> --start ... --end ... --order asc --page-all`：由 DWS 在进程内按 provider 毫秒 cursor 完成整个窗口，只有 `complete=true`、`hasMore=false`、无失败/截断且群/发送人身份齐全才交给后续流程。任一不完整或身份缺失均失败，绝不以时间戳自行续页。消息仍须逐条匹配配置中的 `senderOpenDingTalkId` 与允许文档族（资金账户明细表、资金流水明细/资金明细）；显示名 `sender` 只能用于人工诊断，绝不能作为来源 ID。
- DWS 身份：首次只在 daily-funds 的独立云端卷中显式完成一次 `bootstrap-dws-auth`。当 Coolify 的 Bearer API 没有容器执行能力时，已通过 Cloudflare Access 的 KMFA 私有页面可经既有控制卷发出**唯一固定的**开始/取消请求；同容器 broker 只运行 DWS 设备流、只回传短时 `verificationUri`/`userCode` 给该私有页面，不能执行任意命令，不能读取原始数据，且设备码不进入 cron、日志、status、Git 或公开面。`DAILY_FUNDS_DWS_AUTH_BUNDLE_B64` 仅是可选灾备恢复导入。未配置 `DAILY_FUNDS_DWS_CLIENT_ID` 时，受控 DWS 进程使用其官方默认客户端；如配置，该值只在本切片构造的环境中作为覆盖值使用，绝不继承宿主或其他 Skill。旧读取只按唯一群与唯一发送人的交集远程读取；兼容读取只按同一唯一群远程读取，但在内存中强制群/稳定发送人/文档族三重门禁，远端筛选不能替代本地验真。`auth status` 或任一门禁失败即关闭。运行时不得另行注入 AppSecret；AppKey/AppSecret 不能替代登录态，也不得读取宿主或其他服务的登录态。
- 受控历史验证：私有页面可发起**一次固定的**最近 24 小时、最多两页 `search-advanced` 探针。若该接口终页缺失显式记录列表，才调用官方 `+chat-messages` 的同一唯一群、同一窗口、两页上限验证；其 provider 毫秒 cursor 只在 DWS 内部延续，回执仅表明该有限控制事实。请求不含命令、群 ID、发送人、cursor、时间范围、消息、附件或金额；broker 仅返回 `REQUESTED/RUNNING/COMPLETED/FAILED/EXPIRED`、固定 continuation 枚举，以及 `NOT_OBSERVED/NO_DIRECT_LIST/UNRECOGNIZED_DIRECT_LIST` 三态的协议结构枚举。结构枚举不含字段名、数组长度、ID、消息、附件、cursor 或金额。它不下载附件、不写 raw、不改变 15 分钟主采集链，也绝不构成 DF-008/DF-022、整数分勾稽、金额或 publication PASS。
...
- 附件能力：`.csv`、`.txt`、`.xls`、`.xlsx`、`.xlsm` 仅为候选格式。只有目标群真实字节经私有 Git readback、SHA/lineage、MIME/magic、模板和 parser-open 全部成功后，运行时才以 SHA 写入受保护 `parser_evidence` 回执；合成测试不能把任何格式标为生产已支持。旧版 `.xls` 还必须通过完整 OLE、唯一普通工作表、无宏流、无 BIFF 公式及未加密门禁，缓存公式值绝不作为资金事实。图片与扫描 PDF 仅可走显式启用的离线确定性 Tesseract 回退：来源校验、关键字段至少 0.98 置信度、两个不同业务日同版式校准和后续同版式 readback 缺一不可；它不是模型/API，任何缺口均保持 `needs-review`，不得发布金额。若客户端给出未识别外层后缀，只有原始字节签名能唯一确定图片/PDF 类型且声明 MIME 同样通过时，才可按该签名进入同一 OCR 门；已识别但矛盾的后缀仍拒绝。
- 收支图：显式允许文档族中的`资金流水明细`和`资金明细`可进入**独立**的截图收支观察器；即使后者不能满足正式账户/流水事实表结构，也只能由该观察器逐行校验日期、银行、流入、流出及合计后形成图点。任一候选截图失败都会清空图点并保持 `needs-review`。该路径绝不构造可用资金、账户余额、正式勾稽或 publication。
- 待付款请示图：已登记财务群中的日表截图可进入**独立**的待付款请示观察器。固定标题、业务日期、总合计标签和金额须经三种离线 OCR 分割一致后才形成按日总额；同一业务日以更晚收到的完整日表覆盖。它只表达待付款请示，页面必须明确标注其不是账户余额、已完成付款、可用资金或风险阈值依据。
...
- 页面：Owner 的公开覆写生效，`/ops/app?tab=每日资金` 与对应受控查询 API 无需登录；仍只能显示已通过各自验证门的结果，绝不暴露原始附件、消息/账户标识或下载链接。未通过正式勾稽的金额继续不显示。
