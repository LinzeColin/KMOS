...
当真实 `资金明细` 图片具备完整的日期、转出、收入和银行表头，或能唯一确认日期与收支类别锚点、两列重复右对齐金额边界时，`raw-archive-audit` 才会以离线确定性 OCR 逐行重算转出/收入。两种路径都要求关键 OCR 单元至少 0.98 置信度、单一业务日，以及与图片中唯一“合计”行逐分一致。只有当前范围内**每一份**候选图片都通过、业务日不重复、且至少覆盖两个业务日，才写入只含日期、流入、流出、净流动和脱敏覆盖计数的 `cashflow_observation.json`。
