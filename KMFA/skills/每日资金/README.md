...
当真实 `资金明细` 图片具备明确的日期、转出、收入和银行列时，`raw-archive-audit` 还会以离线确定性 OCR 逐行重算转出/收入，并要求与图片中的唯一“合计”行逐分一致。只有当前范围内**每一份**候选图片都通过、业务日不重复、且至少覆盖两个业务日，才写入只含日期、流入、流出、净流动和脱敏覆盖计数的 `cashflow_observation.json`。
