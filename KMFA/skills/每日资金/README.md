...
受控恢复链的同名第一阶段只做完整的私库元数据审计：它验证全部来源信封、occurrence 和批次关联，但不把全历史图片 OCR 作为发布的等待条件。后续 `raw-fact-replay` 仍会从同一已覆盖提交重新打开每一份候选账户余额和资金流水的原始字节，执行确定性解析、同日配对和逐分勾稽；任何进入 D1/R2/OCI 的事实都保留该字节级证据门。
...
只有 `raw-coverage-repair` 留下与当前私库提交一致的 360 天覆盖回执后，受控 `raw-fact-replay` 才能运行。它重新从该私库提交打开每个候选的账户余额和资金流水事实；标题缺失的原生 `fileId` 附件还必须与当前私有 KMFile 登记中的同一财务群、同一消息和同一文件精确关联，登记只缩小候选范围，文档族和金额仍完全由重新打开的原始字节确定。每个业务日必须恰好一对、重新解析、逐分勾稽及 R2/D1/OCI 发布链全部通过。历史日按日期顺序写入，只有最后一个已验证业务日可以切换 `current.json`；缺失、歧义或无法解析的日期保持需处理并在 values-free 回执中计数，绝不补造金额。
...
财务群内的日度待付款请示截图通过独立的精确 DWS 实时读取进入观察器。它只读取已配置群和静态允许发送人，按 31 天滚动窗口刷新；历史原件审计运行时也不会阻塞该页面快照。固定标题、业务日期、总合计标签与总额必须由三种离线 OCR 分割一致识别；任何已确认标题但后续字段不一致的图片都会使本次观察保持 `NEEDS_REVIEW`，不显示金额。同一业务日存在多份完整日表时，只有更晚收到的那份可以覆盖；时间相同的重复日表会保持待复核。
