...
4. Use 红圈 in Chrome to obtain main-contract data:
   ...
   - Synthetic example: search `甲方A` then `零星维修`, yielding one row with contract name `合成零星维修项目施工合同`, contract number `KMX990806-965`, party A `合成甲方A有限公司`, and amount fields including `含税合同额 (元)`, `累计开票`, and `结算金额 (元)`.
