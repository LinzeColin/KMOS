#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""KMFile-Archive 白箱进度：百分比 + 已完成/未完成 + 阶段运行状态，一条命令看清。
...
DEFAULT_WORKDIR = pathlib.Path("/tmp/kmfile_work/pipeline_v2")
REG = DEFAULT_WORKDIR / "文件登记表.new.csv"
DESC = DEFAULT_WORKDIR / "desc.csv"
ACCEPT = DEFAULT_WORKDIR / "accept_report.json"

STAGES = [("scan 扫描", "kmfile_pipeline.py scan"),
          ("probe 探测", "kmfile_pipeline.py probe"),
          ("extract 抽取", "kmfile_pipeline.py extract"),
          ("dedup 去重", "kmfile_pipeline.py dedup"),
          ("label 标注", "kmfile_pipeline.py label"),
          ("rename 改名", "kmfile_pipeline.py rename"),
          ("registry 登记", "kmfile_pipeline.py registry"),
          ("accept 自验收", "kmfile_pipeline.py accept"),
          ("report 汇总", "kmfile_pipeline.py report")]
...
def read_reg(reg: pathlib.Path):
    ...
    return {"total": total, "renamed": renamed, "sens": sens, "desc": desc}
...
def snapshot(workdir: pathlib.Path) -> str:
    ...
    L = [f"═══ KMFile 进度 {time.strftime('%m-%d %H:%M:%S')} ═══", ""]
    ...
    L += ["", "【素材库】"]
    if r:
        L.append(f"  文件 {r['total']} · 脱敏非无 {r['sens']} · 有描述 {r['desc']}")
