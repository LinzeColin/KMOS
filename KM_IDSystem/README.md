...
- 失败去向：若出现 `No module named app`，先确认 `PYTHONPATH=backend`；若 `.venv` 或 `pytest` 缺失，先按 `backend/requirements.txt` 恢复依赖；若 API/PDF 断言失败，再查 `KM_IDSystem/docs/OpMe_structure_report.md` 和 `backend/tests/`。
