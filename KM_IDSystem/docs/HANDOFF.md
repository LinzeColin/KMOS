...
- Current local state: `STAGE-038 · Worker 队列基线` has Phase 1 through Phase 3 complete; STAGE-037 remains `completed_reviewed_local`.
- Current task: `IDS-V0_1-STAGE038-P3`; acceptance: `ACC-STAGE-038`.
...
- Only `IDS-V0_1-STAGE038-P4` may run next, in a separate run. Phase 4 and whole-stage review have not started.
...
- Current Phase 3 evidence: `docs/pursuing_goal/ids_v0_1/STAGE038_PHASE3_WORKER_QUEUE_SCENARIOS.md`; this is not the later whole-stage review.
...
- STAGE-038 Phase 3 remains an isolated in-process validation baseline, not production readiness. Persistent queue/claim state, retry/dead-letter policy, measured backpressure/fairness, production lock/lease/fencing, automatic lifecycle, process crash recovery, cleanup execution, PostgreSQL actions, raw source reads, and IDS business job execution remain absent.
