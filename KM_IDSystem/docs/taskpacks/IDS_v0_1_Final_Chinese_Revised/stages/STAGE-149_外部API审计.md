...
记录 sent_to_external_api、provider、model、chunk_id、token_count、policy reason。
...
### Phase 3：外部 API 审计 专项验证与异常场景
1. 验证禁止外发、路径穿越阻断、危险操作确认、secret 不入库不入 Git。
2. 验证审计日志不可被普通操作清理。
3. 验证安全失败有明确中文提示。

### Phase 4：外部 API 审计 交付证据、回滚与中文反馈
