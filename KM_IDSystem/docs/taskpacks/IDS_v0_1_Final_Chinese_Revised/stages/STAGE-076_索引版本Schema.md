# STAGE-076 · 索引版本 Schema

- 本地编号：D13-S001
- 当前版本：v0.1
- 能力域：D13 · 索引版本与检索不中断
- 入口：IDS 系统运营入口
- Acceptance ID：ACC-STAGE-076
- 建议并行度：否
...
建立 fulltext、vector、hybrid 的 index_versions、active_index_pointer、building_index_version。
...
1. 定义索引版本、active pointer、building version、影子索引、冒烟测试和回滚。
2. 确认每次批量导入后构建新索引，旧索引继续服务。
3. 定义新索引未通过测试不得切换。

### Phase 2：实现、接入与最小可运行切片
1. 实现索引版本 schema、后台构建、冒烟测试、原子切换或回滚切片。
2. 记录 index_version、document scope、chunk_count、embedding model。
3. 保留旧索引可回滚窗口。

### Phase 3：索引版本 Schema 专项验证与异常场景
1. 验证索引构建失败、冒烟测试失败、切换失败、回滚和旧索引继续服务。
2. 验证并发检索不受后台索引构建影响。
3. 验证索引版本显示在 Operations 和报告快照中。

### Phase 4：索引版本 Schema 交付证据、回滚与中文反馈
1. 交付 index manifest、冒烟测试日志、切换记录和回滚证明。
2. 记录旧索引保留策略和空间影响。
3. 提供索引重建、暂停和恢复说明。
