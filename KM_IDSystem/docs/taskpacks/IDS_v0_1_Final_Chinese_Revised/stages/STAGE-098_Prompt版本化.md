# STAGE-098 · Prompt 版本化

- 本地编号：D16-S002
- 当前版本：v0.1
- 能力域：D16 · RAG 回答层与安全
- 入口：IDS 系统运营入口
- Acceptance ID：ACC-STAGE-098
- 建议并行度：否
...
记录 prompt_version、model_provider、model_version、temperature、retrieval_context。
...
### Phase 3：Prompt 版本化 专项验证与异常场景
1. 验证文档内提示词无法覆盖 IDS 规则。
2. 验证无内部证据时不会伪装成内部经验。
3. 验证高风险工程建议、合同承诺、生产写回不会自动进入最终结论。

### Phase 4：Prompt 版本化 交付证据、回滚与中文反馈
