...
区分 safe_summary、draft_recommendation、high_risk_advice、contractual_commitment 等输出等级。
...
### Phase 3：模型输出权限门禁 专项验证与异常场景
1. 验证文档内提示词无法覆盖 IDS 规则。
2. 验证无内部证据时不会伪装成内部经验。
3. 验证高风险工程建议、合同承诺、生产写回不会自动进入最终结论。

### Phase 4：模型输出权限门禁 交付证据、回滚与中文反馈
