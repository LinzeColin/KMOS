# STAGE-121 · Chunk 质量测试

- 本地编号：D19-S003
- 当前版本：v0.1
- 能力域：D19 · 数据质量门禁
- 入口：IDS 系统运营入口
- Acceptance ID：ACC-STAGE-121
- 建议并行度：是
...
验证 chunk 可追溯、section_path 完整、页码合法、重复 chunk 可识别。
...
### Phase 3：Chunk 质量测试 专项验证与异常场景
1. 验证缺字段、重复 ID、低解析率、低 OCR、无页码、证据缺口和引用不完整。
2. 验证质量失败会降级而不是静默通过。
3. 验证质量门禁不会误删或修改原始资料。

### Phase 4：Chunk 质量测试 交付证据、回滚与中文反馈
