# STAGE-099 · 内部依据与外部增强分离

- 本地编号：D16-S003
- 当前版本：v0.1
- 能力域：D16 · RAG 回答层与安全
- 入口：IDS 系统运营入口
- Acceptance ID：ACC-STAGE-099
- 建议并行度：否
...
UI 可合并展示外部公开参考和模型推理为外部增强意见，但底层 source_type 必须分开。
...
### Phase 3：内部依据与外部增强分离 专项验证与异常场景
1. 验证文档内提示词无法覆盖 IDS 规则。
2. 验证无内部证据时不会伪装成内部经验。
3. 验证高风险工程建议、合同承诺、生产写回不会自动进入最终结论。

### Phase 4：内部依据与外部增强分离 交付证据、回滚与中文反馈
