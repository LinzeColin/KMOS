"""Stage078 P3 的纯内存索引冒烟测试专项场景重放。

模块只重放 Stage078 P2 的固定控制输入与控制投影。它观察候选构建未完成、
...
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_INDEX_SMOKE_TEST_SCENARIOS"
PASS_RESULT = "PASS_INDEX_SMOKE_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_INDEX_SMOKE_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
SCENARIOS = (
    {
        ...
        "phase2_control_scenario": "vector_build_incomplete_keeps_active",
...
def build_index_smoke_test_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2，并在内存中评估固定的 Stage078 P3 控制场景。"""
...
def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_index_smoke_test_control_slice
...
def _load_phase2_module() -> Any:
    path = Path(__file__).with_name("stage078_index_smoke_test_slice.py")
    spec = importlib.util.spec_from_file_location("stage078_phase2_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage078 P2 index smoke test slice")
...
def _phase2_shape_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    if (
        ...
        or result.get("all_shadow_candidates_are_isolated_from_active_service")
        is not True
        or result.get("all_old_active_versions_continue_serving") is not True
        or result.get("all_nonpassed_smoke_tests_block_switch") is not True
...
def _evaluate_scenario(
    ...
    concurrent_retrieval_isolated = (
        ...
        and result.get("actual_concurrent_retrieval_performed") is False
    )
    ...
    expectation_met = (
        phase2_side_effect_free
        and index_record.get("index_kind") == candidate_build.get("index_kind")
...
def _scenario_references_are_control_only(result: Mapping[str, Any]) -> bool:
    ...
    return all(_reference_value_is_control_only(result.get(field)) for field in reference_fields)
...
def _reference_value_is_control_only(value: object) -> bool:
    return (
        ...
        and (":control:stage078-p2:" in value or value.startswith("CONTROL_"))
    )
