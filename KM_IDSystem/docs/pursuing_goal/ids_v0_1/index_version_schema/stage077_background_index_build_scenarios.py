"""Stage077 P3 的纯内存后台索引构建专项场景重放。

本模块只重放 Stage077 P2 的五条固定、非业务、reference-only 控制投影，
观察构建未完成、冒烟失败、切换失败、回退候选、并发检索隔离及控制展示可见性。
它不读取业务资料，不构建或查询实际索引，不写活动指针、Operations 或报告快照。
"""
...
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_BACKGROUND_INDEX_BUILD_SCENARIOS"
PASS_RESULT = "PASS_BACKGROUND_INDEX_BUILD_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_BACKGROUND_INDEX_BUILD_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
PHASE2_RECORD_SPECS = (
    ...
    ("background_build_output_control_projections", "BACKGROUND_BUILD_OUTPUT_FIELDS"),
    ("smoke_test_control_projections", "SMOKE_TEST_FIELDS"),
...
SCENARIOS = (
    {
        ...
        "phase2_control_scenario": "vector_background_building_keeps_active",
        "expected_build_state": "CONTROL_BACKGROUND_BUILD_NOT_STARTED",
        "expected_smoke_test_status": "PENDING",
    ...
    },
    {
        ...
        "expected_build_state": "CONTROL_BACKGROUND_BUILD_COMPLETE_NOT_STARTED",
...
def build_background_index_build_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2，并在内存中评估六条固定 Stage077 P3 控制场景。"""
    ...
    valid = (
        ...
        and all(required_scenario_results.values())
    ...
    return {
        ...
        "build_not_complete_preserved": required_scenario_results[
            "build_not_complete_old_active_continues"
        ],
        "smoke_test_failure_preserved": required_scenario_results[
            "smoke_test_failure_blocks_switch"
        ],
        "switch_failure_preserved": required_scenario_results[
            "switch_failure_preserves_active"
        ],
        "rollback_preserved": required_scenario_results[
            "rollback_retains_previous_active"
        ],
        "concurrent_retrieval_isolation_preserved": required_scenario_results[
            "background_build_concurrent_retrieval_isolated"
        ],
        "operations_and_report_snapshot_visibility_preserved": required_scenario_results[
            "operations_and_report_snapshot_version_visibility"
        ],
        ...
        "actual_active_pointer_switch_count": 0,
...
def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_background_index_build_control_slice
...
def _load_phase2_module() -> Any:
    path = Path(__file__).with_name("stage077_background_index_build_slice.py")
    spec = importlib.util.spec_from_file_location("stage077_phase2_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage077 P2 background index build slice")
...
def _phase2_shape_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    if (
        ...
        or result.get("all_candidate_versions_differ_from_active_versions") is not True
        or result.get("all_shadow_candidates_are_isolated_from_active_service") is not True
        or result.get("all_active_versions_continue_serving_during_control_build")
        is not True
        or result.get("all_failed_or_pending_smoke_tests_block_switch") is not True
        or result.get("all_switch_projections_keep_active_pointer_unchanged")
        is not True
        ...
        projections.append(
            {
                ...
                ref_field: f"{ref_field.removesuffix('_ref')}{marker}",
...
def _evaluate_scenario(
    ...
    build_output = _record_at(
        result.get("background_build_output_control_projections"), index
    )
    smoke = _record_at(result.get("smoke_test_control_projections"), index)
    ...
    concurrent_retrieval_isolated = (
        ...
        and building.get("build_state") == "CONTROL_BACKGROUND_BUILD_NOT_STARTED"
    ...
    expectation_met = (
        ...
        and build_output.get("build_state") == scenario["expected_build_state"]
        and smoke.get("smoke_test_status") == scenario["expected_smoke_test_status"]
    ...
    return {
        ...
        "referenced_smoke_test_ref": smoke.get("smoke_test_ref"),
        ...
        "observed_build_state": build_output.get("build_state"),
        "observed_smoke_test_status": smoke.get("smoke_test_status"),
...
def _view_visible(
    ...
    return (
        ...
        and ":control:stage077-p2:" in view[ref_field]
...
def _scenario_references_are_control_only(result: Mapping[str, Any]) -> bool:
    ...
    return all(
        ...
        and ":control:stage077-p2:" in result[field]
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    ...
    return all(
        ...
        and ":control:stage077-p2:" in record[field]
        for field in reference_fields
    )
