"""Stage078 P4 的纯内存索引冒烟测试交付证据。

模块只复用 Stage078 P2 的五组固定、非业务控制投影及 P3 的六条专项场景，
...
RECORD_KIND = "INDEX_SMOKE_TEST_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_INDEX_SMOKE_TEST_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_INDEX_SMOKE_TEST_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE078-P4-GATE"
...
def build_index_smoke_test_phase4_delivery_report(
    ...
    return {
        ...
        "stage077_review_evidence_declared": True,
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_index_smoke_test_phase3_report()


def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_phase2_module()
    return module.execute_index_smoke_test_control_slice(module.build_control_input())
...
def _load_phase3_module() -> Any:
    return _load_module("stage078_phase3_scenarios", "stage078_index_smoke_test_scenarios.py")


def _load_phase2_module() -> Any:
    return _load_module("stage078_phase2_slice", "stage078_index_smoke_test_slice.py")
...
def _phase2_report_is_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("all_nonpassed_smoke_tests_block_switch") is True
    ...
    return [
        {
            ...
            "index_manifest_ref": f"index-manifest:control:stage078-p2:{scenario}",
    ...
    return [
        {
            ...
            "smoke_test_log_ref": (
                f"smoke-test-log:control:stage078-p2:{scenario['phase2_control_scenario']}"
    ...
    return [
        {
            "control_scenario": record["control_scenario"],
            "rollback_proof_ref": (
                f"rollback-proof:control:stage078-p2:{record['control_scenario']}"
            ),
...
def _old_index_retention_projection() -> dict[str, Any]:
    return {
        "retention_policy_ref": "index-retention-policy:control:stage078-p2:previous-active",
...
def _operational_instruction_projections() -> list[dict[str, Any]]:
    marker = ":control:stage078-p2:operations"
    return [
        {
            "instruction_id": "INDEX_SMOKE_TEST_REBUILD_CONTROL_INSTRUCTION",
            "action": "REBUILD",
        ...
        {
            "instruction_id": "INDEX_SMOKE_TEST_PAUSE_CONTROL_INSTRUCTION",
            "action": "PAUSE",
        ...
        {
            "instruction_id": "INDEX_SMOKE_TEST_RECOVERY_CONTROL_INSTRUCTION",
            "action": "RECOVERY",
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    ...
    return bool(reference_values) and all(
        ":control:stage078-p2:" in str(value) for value in reference_values
    )
