"""Stage080 P3 的纯内存索引回滚专项场景重放。

模块只重放 Stage080 P2 的固定控制输入与控制投影。它观察候选构建未完成、
冒烟失败、切换失败、回滚资格、构建期间检索隔离及版本可见性；不读取业务
...
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_INDEX_ROLLBACK_SCENARIOS"
PASS_RESULT = "PASS_INDEX_ROLLBACK_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_INDEX_ROLLBACK_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
SCENARIOS = (
    {
        ...
        "expected_build_state": "CONTROL_BUILD_INCOMPLETE_REFERENCE_ONLY",
    ...
    },
    {
        ...
        "expected_build_state": "CONTROL_BUILD_COMPLETE_REFERENCE_ONLY",
...
def build_index_rollback_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2，并在内存中评估固定的 Stage080 P3 控制场景。"""
...
def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_index_rollback_control_slice
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage080_index_rollback_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage080_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage080 P2 index rollback slice")
...
def _evaluate_scenario(
    ...
    concurrent_retrieval_isolated = (
        ...
        and build.get("build_state") == "CONTROL_BUILD_INCOMPLETE_REFERENCE_ONLY"
...
def _reference_value_is_control_only(value: object) -> bool:
    return (
        ...
        and (":control:stage080-p2:" in value or value.startswith("CONTROL_"))
    )
