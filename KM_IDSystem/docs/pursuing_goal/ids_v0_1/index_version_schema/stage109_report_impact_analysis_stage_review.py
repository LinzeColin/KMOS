"""Stage109 报告影响分析的纯内存整阶段机械复审。

模块只复审冻结任务包与 Stage109 P1--P4 已提交的控制合同和纯内存报告。
...
SCHEMA_VERSION = "ids.stage109.report_impact_analysis.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_IMPACT_ANALYSIS_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_REPORT_IMPACT_ANALYSIS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_REPORT_IMPACT_ANALYSIS_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE109-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE110-P1-GATE"

P1_SCHEMA_VERSION = "ids.stage109.report_impact_analysis.phase1.v1"
P1_CONTRACT_STATE = "REPORT_IMPACT_ANALYSIS_CONTRACT_RUNTIME_DISABLED"
...
P2_PASS_RESULT = "PASS_IN_MEMORY_REPORT_IMPACT_ANALYSIS_CONTROL_SLICE_RUNTIME_DISABLED"
P3_SCHEMA_VERSION = "ids.stage109.report_impact_analysis.phase3.v1"
P3_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_IMPACT_ANALYSIS_SCENARIOS"
P3_PASS_RESULT = "PASS_REPORT_IMPACT_ANALYSIS_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_SCHEMA_VERSION = "ids.stage109.report_impact_analysis.phase4.delivery.v1"
P4_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_IMPACT_ANALYSIS_DELIVERY_EVIDENCE"
P4_PASS_RESULT = "PASS_REPORT_IMPACT_ANALYSIS_DELIVERY_EVIDENCE_RUNTIME_DISABLED"

P1_CONTRACT_PATH = Path(__file__).with_name("stage109_report_impact_analysis_contract.json")
P2_MODULE_PATH = Path(__file__).with_name("stage109_report_impact_analysis_control_slice.py")
P3_MODULE_PATH = Path(__file__).with_name("stage109_report_impact_analysis_controlled_scenarios.py")
P4_MODULE_PATH = Path(__file__).with_name("stage109_report_impact_analysis_delivery.py")

REVIEWED_CONTROL_SHAPE = {
    "phase1_reference_field_count": 33,
    "phase1_snapshot_component_count": 5,
    "phase1_failure_state_count": 35,
    "phase1_chinese_feedback_count": 4,
    "phase2_control_request_count": 5,
    "phase2_input_field_count": 35,
    "phase2_phase1_reference_field_count": 33,
    "phase2_projection_group_count": 4,
    "phase2_projection_field_count_per_request": 101,
    "phase2_control_field_check_count": 505,
    "phase2_failure_state_count": 35,
    "phase2_chinese_feedback_count": 4,
    "phase3_scenario_count": 5,
    "phase3_scenario_field_count": 42,
    "phase3_scenario_field_check_count": 210,
    "phase3_control_view_count": 5,
    "phase3_human_handling_count": 5,
    "phase3_whitebox_confirmation_required_count": 2,
    "phase3_failure_state_count": 20,
...
REVIEW_RUNTIME_FALSE_FIELDS = (
    ...
    "push_performed",
    "stage109_review_runtime_executed",
    "stage110_runtime_started",
)
...
P2_PROJECTION_GROUPS = (
    ...
    ("report_impact_analysis_and_lifecycle_control_projections", 5, 44),
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module(P2_MODULE_PATH, "stage109_phase2_for_stage_review")
    return module.execute_report_impact_analysis_control_slice(module.build_control_input())


def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module(P3_MODULE_PATH, "stage109_phase3_for_stage_review")
    return module.build_report_impact_analysis_phase3_report()


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module(P4_MODULE_PATH, "stage109_phase4_for_stage_review")
    return module.build_report_impact_analysis_phase4_delivery_report()
...
def _phase1_valid(contract: Mapping[str, Any]) -> bool:
    ...
    impact = contract.get("report_impact_analysis_contract")
    ...
    return all(
        (
            ...
            contract.get("phase") == "IDS-STAGE109-P1",
            contract.get("task_id") == "IDS-V0_1-STAGE109-P1",
            contract.get("contract_state") == P1_CONTRACT_STATE,
            contract.get("entry_gate") == "IDS-STAGE109-P1-GATE",
            contract.get("next_gate") == "IDS-STAGE109-P2-GATE",
            ...
            impact.get("future_control_reference_field_count") == 33,
            impact.get(
                "critical_conclusion_requires_evidence_id_or_evidence_gap_independently"
            )
            is True,
            impact.get("citation_source_and_page_required_in_future_pdf_report") is True,
            impact.get("external_augmentation_retains_underlying_source_types") is True,
            impact.get(
                "external_augmentation_may_not_be_presented_as_internal_project_evidence"
            )
            is True,
            impact.get("external_augmentation_may_not_close_evidence_gap") is True,
            isinstance(failure, Mapping),
            failure.get("failure_state_count") == 35,
            ...
            boundary.get("stage110_started") is False,
...
def _phase2_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            report.get("control_projection_field_total_per_request") == 101,
            report.get("control_projection_field_total") == 505,
...
def _scenario_semantics_valid(scenario: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            scenario.get("affected_report_control_state")
            in {
            ...
            scenario.get("actual_report_impact_analysis_performed") is False,
...
def _phase3_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            report.get("failure_state") is None,
            report.get("current_gate") == "IDS-STAGE109-P3-GATE",
            report.get("next_gate") == "IDS-STAGE109-P4-GATE",
            report.get("phase2_control_request_count") == 5,
            report.get("phase2_input_field_count") == 35,
            report.get("phase2_projection_group_count") == 4,
            report.get("phase2_projection_field_count_per_request") == 101,
            report.get("phase2_projection_field_count_total") == 505,
            report.get("scenario_count") == 5,
            report.get("scenario_field_count") == 42,
            report.get("scenario_field_check_count") == 210,
            report.get("control_view_count") == 5,
            report.get("human_handling_count") == 5,
            isinstance(scenarios, list),
            len(scenarios) == 5,
            all(isinstance(item, Mapping) and len(item) == 42 for item in scenarios),
...
def _report_impact_semantics_valid(scenarios: list[Mapping[str, Any]]) -> bool:
    return all(
        (
            "CONTROL_SOURCE_WITHDRAWAL_FUTURE_REPORT_STATUS_REVIEW_REQUIRED"
            ...
            "CONTROL_AFFECTED_REPORT_AND_CRITICAL_CONCLUSION_FUTURE_REVIEW_REQUIRED"
            in {item.get("affected_report_control_state") for item in scenarios},
        )
    )
...
def _phase4_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            report.get("failure_state") is None,
            report.get("current_gate") == "IDS-STAGE109-P4-GATE",
            report.get("next_gate") == REVIEW_GATE,
            report.get("phase2_control_request_count") == 5,
            report.get("phase2_input_field_count") == 35,
            report.get("phase2_phase1_reference_field_count") == 33,
            report.get("phase2_projection_group_count") == 4,
            report.get("phase2_projection_field_count_per_request") == 101,
            report.get("phase2_projection_field_count_total") == 505,
            report.get("scenario_count") == 5,
...
def build_report_impact_analysis_stage_review(
    ...
    """机械复审 Stage109 P1--P4 控制工件，漂移时保持失败关闭。"""
    ...
    if not _report_impact_semantics_valid(scenarios):
        return _base_report(False, "REPORT_IMPACT_SEMANTICS_MISMATCH")
