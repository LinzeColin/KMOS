"""Stage094 P4 的纯内存证据撤回交付控制证据。

模块只从 Stage094 P3 已验收的固定、非业务、reference-only 异常场景派生
...
SCHEMA_VERSION = "ids.stage094.evidence_revocation.phase4.delivery.v1"
RECORD_KIND = "EVIDENCE_REVOCATION_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_EVIDENCE_REVOCATION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_REVOCATION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE094-P4-GATE"
NEXT_GATE = "IDS-STAGE094-REVIEW-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_REVOCATION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage094-p2:"
DELIVERY_PREFIX = ":control:stage094-p4:"
...
DEGRADATION_INSTRUCTION_DEFINITIONS = (
    {
        "instruction_id": "low_ocr_evidence_revocation_degradation_instruction",
    ...
    {
        "instruction_id": "old_version_evidence_revocation_degradation_instruction",
    ...
    {
        "instruction_id": "conflict_evidence_revocation_degradation_instruction",
    ...
    {
        "instruction_id": "revoked_evidence_revocation_degradation_instruction",
...
def build_evidence_revocation_phase4_delivery_report(
    phase3_report_provider: Phase3ReportProvider | None = None,
) -> dict[str, Any]:
    """从固定 P3 场景派生未持久化的 Stage094 P4 交付控制记录。"""
    ...
    return {
        ...
        "chinese_feedback": [
            ...
            "Stage094 P4 只开放整阶段机械复审门，Review、Stage095、OVH、生产与正式上传保持后续阶段。",
        ],
    }


def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_evidence_revocation_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage094_evidence_revocation_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage094_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage094 P3 evidence revocation scenarios")
...
def _phase3_reference_is_allowed(
    ...
    return _inherited_control_ref(value) or (
        ...
        and scenario.get("scenario_id") == "no_internal_evidence_revocation_control"
        and value is None
    )
...
def _phase3_semantic_failure(report: Mapping[str, Any]) -> str | None:
    ...
    no_internal = scenarios.get("no_internal_evidence_revocation_control", {})
    if (
    ...
    degradation_states = {
        "low_ocr_evidence_revocation_degradation_control": (
            "CONTROL_LOW_OCR_DEGRADED_NOT_ACCEPTED"
        ),
        "old_version_evidence_revocation_degradation_control": (
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED"
        ),
        "conflict_evidence_revocation_degradation_control": (
    ...
    malicious = scenarios.get("malicious_evidence_revocation_quarantine_control", {})
    if (
...
def _delivery_reference_is_allowed(
    ...
    return _control_ref(value) or (
        field == "evidence_id_ref"
        and record.get("scenario_id") == "no_internal_evidence_revocation_control"
        and value is None
    )
