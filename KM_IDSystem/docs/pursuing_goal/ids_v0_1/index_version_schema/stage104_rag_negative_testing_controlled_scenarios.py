"""Stage104 P3：RAG 负向测试的纯内存专项异常场景验证。

模块只重放 Stage104 P2 的固定、非业务、reference-only 控制投影。它验证
...
SCHEMA_VERSION = "ids.stage104.rag_negative_testing.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_RAG_NEGATIVE_TEST_SCENARIOS"
CURRENT_GATE = "IDS-STAGE104-P3-GATE"
NEXT_GATE = "IDS-STAGE104-P4-GATE"
PASS_RESULT = "PASS_RAG_NEGATIVE_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_RAG_NEGATIVE_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
P2_CONTROL_PREFIX = ":control:stage104-p2:"
P2_CONTROL_REQUEST_COUNT = 5
P2_INPUT_FIELD_COUNT = 29
P2_PROJECTION_GROUP_COUNT = 4
P2_PROJECTION_FIELD_COUNT_PER_REQUEST = 57
P2_PROJECTION_FIELD_COUNT_TOTAL = 285
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name(
        "stage104_rag_negative_testing_control_slice.py"
    )
    spec = importlib.util.spec_from_file_location("stage104_phase2_slice", module_path)
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    if (
        ...
        or tuple(getattr(phase2_module, "CONTROL_FIELDS", ())) != ("control_requests",)
        ...
        for field, value in request.items():
            ...
            if value is None:
                if field not in {"internal_evidence_ref", "evidence_gap_ref"}:
                    return False
                continue
            if field == "external_augmentation_display_label":
                if value != "external_augmentation_opinion":
                    return False
                continue
            if field == "negative_test_case_id":
                if value not in P2_NEGATIVE_TEST_CASES:
                    return False
                continue
            if not (
...
def _failure_for_projection(
    ...
    if defense.get("document_instruction_cannot_override_ids_rule_state") != (
        "CONTROL_DOCUMENT_INSTRUCTION_CANNOT_OVERRIDE_IDS_RULE"
    ):
        return "DOCUMENT_INSTRUCTION_CAN_OVERRIDE_IDS_RULE"
    if defense.get("document_instruction_cannot_relax_output_permission_state") != (
        "CONTROL_DOCUMENT_INSTRUCTION_CANNOT_RELAX_OUTPUT_PERMISSION"
    ):
        return "UNTRUSTED_DOCUMENT_INSTRUCTION_ACCEPTED"
    if defense.get("document_instruction_cannot_bypass_human_confirmation_state") != (
        "CONTROL_DOCUMENT_INSTRUCTION_CANNOT_BYPASS_HUMAN_CONFIRMATION"
    ):
        return "UNTRUSTED_DOCUMENT_INSTRUCTION_ACCEPTED"
    if defense.get("actual_document_content_read_performed") is not False or defense.get(
    ...
    if source.get("external_augmentation_display_composition_state") != (
        "CONTROL_EXTERNAL_PUBLIC_REFERENCE_AND_MODEL_REASONING_RETAINED"
    ):
        return "EXTERNAL_AUGMENTATION_CLOSED_EVIDENCE_GAP"
    if source.get("external_augmentation_may_not_replace_internal_evidence") is not True:
        return "EXTERNAL_AUGMENTATION_CLOSED_EVIDENCE_GAP"
    if source.get("actual_source_type_bound") is not False or source.get(
    ...
    if evidence_gap_present:
        if source.get("evidence_gap_presentation_state") != (
            "CONTROL_EVIDENCE_GAP_NOT_INTERNAL_EXPERIENCE"
        ):
            return "EVIDENCE_GAP_PRESENTED_AS_INTERNAL_EXPERIENCE"
        if not _is_control_reference(source.get("evidence_gap_ref")):
            return "NON_OPAQUE_EVIDENCE_GAP_REFERENCE"
    elif source.get("evidence_gap_presentation_state") != (
        "CONTROL_INTERNAL_EVIDENCE_REFERENCE_PRESENT"
    ):
        return "EVIDENCE_GAP_SEMANTIC_DRIFT"
    ...
    if not all(
        _is_control_reference(source.get(field))
        for field in (
            "external_public_reference_ref",
            "model_reasoning_ref",
            "external_augmentation_ref",
    ...
    output_category = str(permission.get("output_category", ""))
    if output_category != definition["expected_output_category"]:
        return "OUTPUT_CATEGORY_DRIFT"
    high_risk = output_category in HUMAN_CONFIRMATION_OUTPUT_CATEGORIES
    expected_permission_state = (
        "CONTROL_HUMAN_WHITEBOX_CONFIRMATION_REQUIRED_NO_AUTO_FINALIZATION"
        if high_risk
        else "CONTROL_NO_AUTO_PUBLICATION_NO_AUTO_FINALIZATION"
    )
    if permission.get("output_permission_state") != expected_permission_state:
        return (
            "HIGH_RISK_OUTPUT_PERMISSION_BYPASS"
            if high_risk
            else "BUSINESS_WHITEBOX_BOUNDARY_BREACH"
        )
    if permission.get("business_line_whitebox_human_confirmation_required") is not high_risk:
        return (
            "HIGH_RISK_HUMAN_CONFIRMATION_BYPASS"
            if high_risk
            else "NON_HIGH_RISK_HUMAN_CONFIRMATION_STATE_DRIFT"
        )
    if permission.get("automatic_final_conclusion_allowed") is not False or permission.get(
        "actual_final_conclusion_published"
    ) is not False:
        return definition.get(
            "auto_finalization_failure_state", "AUTOMATIC_FINAL_CONCLUSION_ALLOWED"
        )
    if permission.get("automatic_answer_publication_allowed") is not False:
        return "AUTOMATIC_PUBLICATION_ENABLED"
    if permission.get("automatic_production_writeback_allowed") is not False or permission.get(
        "actual_production_writeback_performed"
    ) is not False:
        return (
            "PRODUCTION_WRITEBACK_AUTO_FINALIZED"
            if output_category == "production_writeback"
            else "AUTOMATIC_FINAL_CONCLUSION_ALLOWED"
        )
    if permission.get("actual_output_classified") is not False or permission.get(
        "actual_human_confirmation_recorded"
    ) is not False:
        return "PHASE2_SIDE_EFFECT_BOUNDARY_BREACH"
    if permission.get("negative_test_execution_state") != (
...
def _scenario_record(
    ...
    output_category = str(permission["output_category"])
    human_confirmation_state = (
...
def build_rag_negative_testing_phase3_report(
    ...
    try:
        ...
        executor = (
            ...
            else phase2_module.execute_rag_negative_testing_control_slice
        )
    ...
    for index, definition in enumerate(SCENARIO_DEFINITIONS):
        ...
        defense = _projection_record(
            phase2_result, "document_evidence_and_rule_defense", index
        )
        source = _projection_record(
            phase2_result, "source_semantics_and_external_augmentation", index
        )
