"""Stage085 P4 的纯内存元数据过滤交付证据。

模块只重放 Stage085 P3 的八个受控场景，派生 metadata-only 的检索样例、
...
SCHEMA_VERSION = "ids.stage085.metadata_filter.phase4.delivery.v1"
RECORD_KIND = "METADATA_FILTER_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_METADATA_FILTER_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_METADATA_FILTER_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE085-P4-GATE"
...
RUNTIME_CLOSED_FIELDS = P3_RUNTIME_CLOSED_FIELDS + (
...
def build_metadata_filter_phase4_delivery_report(
    ...
    scenario_records = (
        _as_records(phase3_report.get("scenario_results")) if phase3_valid else []
    )
    samples = _retrieval_samples(scenario_records)
    trace_logs = _trace_logs(scenario_records)
    filter_results = _filter_results(scenario_records)
    validity_reports = _validity_reports(scenario_records)
    evidence_gaps = _evidence_gaps(scenario_records)
    ...
    return {
        ...
        "stage084_review_evidence_declared": True,
        ...
        **runtime_flags,
        "chinese_feedback": [
            ...
            "模型、版本、维度与相似度度量只保留不透明控制引用，未生成 embedding 或计算相似度。",
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_metadata_filter_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage085_metadata_filter_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage085_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage085 P3 metadata filter scenarios")
...
def _phase3_report_is_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("stage084_review_evidence_declared") is True
    ...
    return [
        {
            ...
            "rollback_target_ref": (
                f"stage085-p3-control-boundary{CONTROL_PREFIX}{scenario_id}"
            ),
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    for field, value in record.items():
        if field.endswith("_ref"):
            if not isinstance(value, str) or CONTROL_PREFIX not in value:
                return False
        if field == "metadata_filter_refs":
            refs = _refs(value)
            if not refs or any(CONTROL_PREFIX not in item for item in refs):
                return False
    return True
