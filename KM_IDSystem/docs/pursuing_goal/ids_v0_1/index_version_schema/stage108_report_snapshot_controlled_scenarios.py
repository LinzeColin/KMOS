"""Stage108 P3：报告快照的纯内存专项异常场景验证。

模块只重放 Stage108 P2 的固定、非业务、reference-only 控制投影。它验证关键
结论始终关联 evidence_id 或 evidence_gap，资料撤回、证据降级和索引版本变化
保持未来报告状态影响控制形状，外部增强保持外部来源语义并受业务线白箱确认门禁
约束。模块不读取真实资料、报告、PDF 或证据账本，也不调用模型、Agent、OVH 或
生产服务，不写入数据库、审计或持久化状态。
"""
...
SCHEMA_VERSION = "ids.stage108.report_snapshot.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_SNAPSHOT_SCENARIOS"
CURRENT_GATE = "IDS-STAGE108-P3-GATE"
NEXT_GATE = "IDS-STAGE108-P4-GATE"
PASS_RESULT = "PASS_REPORT_SNAPSHOT_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REPORT_SNAPSHOT_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"

P2_SCHEMA_VERSION = "ids.stage108.report_snapshot.phase2.v1"
P2_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_SNAPSHOT"
P2_EXECUTION_STATE = "PASS_IN_MEMORY_REPORT_SNAPSHOT_CONTROL_SLICE_RUNTIME_DISABLED"
P2_CONTROL_PREFIX = ":control:stage108-p2:"
P2_CONTROL_FIELDS = ("report_snapshot_control_requests",)
P2_CONTROL_REQUEST_COUNT = 5
P2_INPUT_FIELD_COUNT = 26
P2_PHASE1_REFERENCE_FIELD_COUNT = 24
P2_PROJECTION_GROUP_COUNT = 4
P2_PROJECTION_FIELD_COUNT_PER_REQUEST = 82
P2_PROJECTION_FIELD_COUNT_TOTAL = 410
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage108_phase3_runtime_executed",
)
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage108_report_snapshot_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage108_phase2_slice", module_path)
...
def build_report_snapshot_phase3_report(
    ...
    try:
        ...
        executor = (
            ...
            else phase2_module.execute_report_snapshot_control_slice
        )
