"""Stage080 的纯内存整阶段机械复审，不读取真实资料或启动 Stage081。"""
...
P1_CONTRACT = BASE / "stage080_index_rollback_contract.json"
P2_CONTRACT = BASE / "stage080_index_rollback_slice_contract.json"
P3_CONTRACT = BASE / "stage080_index_rollback_scenarios_contract.json"
P4_CONTRACT = BASE / "stage080_index_rollback_delivery_contract.json"

SCHEMA_VERSION = "ids.stage080.index_rollback.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE080-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-080"
PASS_RESULT = "PASS_REVIEWED_INDEX_ROLLBACK_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_INDEX_ROLLBACK_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE080-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE081-P1-GATE"
RETURN_STATE = "PASS_INDEX_ROLLBACK_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
P2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_INDEX_ROLLBACK_CONTROL_SLICE"
P3_PASS_RESULT = "PASS_INDEX_ROLLBACK_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
def build_index_rollback_stage080_review_report(
    ...
    """机械复审 Stage080 P1--P4，仅输出零运行时结论及后续门禁。"""

    try:
        phase2_module = _load_module("stage080_index_rollback_control_slice.py")
        phase3_module = _load_module("stage080_index_rollback_scenarios.py")
        phase4_module = _load_module("stage080_index_rollback_delivery.py")
    ...
    phase2_report = _provider_value(
        ...
        or (
            lambda: phase2_module.execute_index_rollback_control_slice(
                phase2_module.build_control_input()
            )
        )
    )
    phase3_report = _provider_value(
        phase3_report_provider or phase3_module.build_index_rollback_phase3_report
    )
    phase4_report = _provider_value(
        phase4_report_provider
        or phase4_module.build_index_rollback_phase4_delivery_report
    )
    ...
    invariants = {
        ...
        "stage081_gate_only_opens_after_review": _future_stage_boundary(
            phase1, phase2, phase3, phase4, phase3_report, phase4_report
        ),
...
def _review_report(
    ...
    return {
        ...
        "source_authority": "FROZEN_STAGE080_TASKPACK_AND_STAGE080_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        ...
        "stage079_review_evidence_read": True,
        "stage080_started": True,
        "phase1_started": True,
        "phase2_started": True,
        "phase3_started": True,
        "phase4_started": True,
        ...
        **_runtime_closed_flags(),
        "rollback": {
            ...
            "scope": "STAGE080_REVIEW_ARTIFACTS_AND_LOCAL_GOVERNANCE_ONLY",
...
def _failed_report() -> dict[str, Any]:
    return _review_report(
        ...
        {},
        {
            ...
            "stage081_gate_only_opens_after_review": False,
...
def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="IDS-STAGE080-P1",
            task_id="IDS-V0_1-STAGE080-P1",
            schema_version="ids.stage080.index_rollback.phase1.v1",
            contract_state="PHASE1_INDEX_ROLLBACK_CONTRACT_RUNTIME_DISABLED",
            next_gate="IDS-STAGE080-P2-GATE",
        )
        and _expected(
            versions,
            {
                ...
                "reuses_stage079_control_field_shapes_only": True,
        ...
        and failures.get("failure_state_count") == 9
        and _expected(
            local_code,
            {
                ...
                "atomic_switch_runner_created": False,
                "rollback_runner_created": False,
            },
        )
        and _expected(
            boundary,
            {
                "stage079_review_evidence_declared": True,
                "stage080_started": True,
                "phase1_started": True,
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="IDS-STAGE080-P2",
            task_id="IDS-V0_1-STAGE080-P2",
            schema_version="ids.stage080.index_rollback.phase2.v1",
            contract_state="PHASE2_INDEX_ROLLBACK_CONTROL_SLICE_RUNTIME_DISABLED",
            next_gate="IDS-STAGE080-P3-GATE",
        )
        and _expected(
            input_contract,
            {
        ...
        and _expected(
            local_code,
            {
                "runtime_module_created": True,
                "runtime_module_is_pure_in_memory_control_slice": True,
                "database_schema_created": False,
                "background_worker_created": False,
                "index_artifact_created": False,
                "smoke_test_runner_created": False,
        ...
        and _expected(
            ...
            {
                "stage079_review_evidence_declared": True,
                "stage080_phase1_completed": True,
                "stage080_phase2_started": True,
                "whole_stage_review_performed": False,
                "stage081_started": False,
                "github_upload_allowed": False,
                "push_allowed": False,
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="IDS-STAGE080-P3",
            task_id="IDS-V0_1-STAGE080-P3",
            schema_version="ids.stage080.index_rollback.phase3.v1",
            contract_state="PHASE3_INDEX_ROLLBACK_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
            next_gate="IDS-STAGE080-P4-GATE",
        )
        and _expected(
            replay,
            {
                "required_execution_state": P2_EXECUTION_STATE,
                "control_request_count": 5,
                "index_version_record_count": 5,
                "index_version_record_field_count": 7,
                "building_and_shadow_projection_count": 5,
                "building_and_shadow_projection_field_count": 5,
                "active_pointer_projection_count": 5,
                "active_pointer_projection_field_count": 5,
                "smoke_test_input_projection_count": 5,
                "smoke_test_input_projection_field_count": 6,
                "smoke_test_output_projection_count": 5,
                "smoke_test_output_projection_field_count": 5,
                "switch_projection_count": 5,
                "switch_projection_field_count": 9,
                "rollback_request_projection_count": 5,
                "rollback_request_projection_field_count": 8,
                "phase2_control_field_check_count": 225,
        ...
        and failures.get("failure_state_count") == 13
        and feedback.get("feedback_count") == 4
        and _expected(
            boundary,
            {
                "stage079_review_evidence_declared": True,
                "stage080_phase1_completed": True,
                "stage080_phase2_completed": True,
                "stage080_phase3_started": True,
                "whole_stage_review_performed": False,
                "stage081_started": False,
                "github_upload_allowed": False,
                "push_allowed": False,
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="P4",
            task_id="IDS-V0_1-STAGE080-P4",
            schema_version="ids.stage080.index_rollback.phase4.delivery.v1",
            contract_state="PHASE4_INDEX_ROLLBACK_DELIVERY_EVIDENCE_RUNTIME_DISABLED",
        ...
        and _expected(
            phase3_replay,
            {
                ...
                "scenario_field_check_count": 168,
        ...
        and _expected(
            boundary,
            {
                ...
                "stage080_phase1_completed": True,
                "stage080_phase2_completed": True,
                "stage080_phase3_completed": True,
                "phase4_started": True,
                "whole_stage_review_performed": False,
                "stage081_started": False,
                "github_upload_allowed": False,
    ...
    return (
        _expected(
            report,
            {
                "schema_version": "ids.stage080.index_rollback.phase2.v1",
                "input_accepted": True,
                "execution_state": P2_EXECUTION_STATE,
        ...
        and _phase2_field_check_count(report, groups) == 225
...
def _phase3_report_valid(report: Mapping[str, Any], module: Any) -> bool:
    return (
        ...
        and _records_have_exact_shape(report.get("scenario_results"), 6, module.SCENARIO_RESULT_FIELDS)
        and _records_have_exact_shape(
            report.get("operations_version_control_views"), 5, module.OPERATIONS_VIEW_FIELDS
        )
        and _records_have_exact_shape(
            report.get("report_snapshot_version_control_views"), 5, module.REPORT_SNAPSHOT_FIELDS
        )
        and _report_runtime_closed(report, module)
    )
...
def _controlled_replay(
    ...
    return {
        "phase1_index_version_field_count": _as_int(p1_versions.get("index_version_field_count")),
        "phase1_active_pointer_field_count": _as_int(p1_versions.get("active_pointer_field_count")),
        "phase1_building_and_shadow_field_count": _as_int(p1_building.get("building_and_shadow_field_count")),
        "phase1_smoke_input_field_count": _as_int(p1_building.get("smoke_test_input_field_count")),
        "phase1_smoke_output_field_count": _as_int(p1_building.get("smoke_test_output_field_count")),
        "phase1_rollback_request_field_count": _as_int(p1_rollback.get("rollback_request_field_count")),
        "phase1_rollback_proof_field_count": _as_int(p1_rollback.get("rollback_proof_field_count")),
        "phase1_rollback_condition_count": _as_int(p1_rollback.get("condition_count")),
        "phase1_failure_state_count": _as_int(p1_failures.get("failure_state_count")),
        "phase2_control_request_count": _as_int(phase2_report.get("received_control_request_count")),
        "phase2_index_version_record_count": _as_int(phase2_report.get("index_version_control_record_count")),
        "phase2_building_and_shadow_projection_count": _as_int(phase2_report.get("building_and_shadow_control_projection_count")),
        "phase2_active_pointer_projection_count": _as_int(phase2_report.get("active_pointer_control_projection_count")),
        "phase2_smoke_input_projection_count": _as_int(phase2_report.get("smoke_test_input_control_projection_count")),
        "phase2_smoke_output_projection_count": _as_int(phase2_report.get("smoke_test_output_control_projection_count")),
        "phase2_switch_projection_count": _as_int(phase2_report.get("switch_control_projection_count")),
        "phase2_rollback_request_projection_count": _as_int(phase2_report.get("rollback_request_control_projection_count")),
        "phase2_control_field_check_count": _phase2_field_check_count_from_report(phase2_report),
        ...
        "phase3_scenario_field_check_count": _as_int(phase3_report.get("scenario_field_check_count")),
        "phase3_operations_view_count": _as_int(phase3_report.get("operations_version_control_view_count")),
        "phase3_report_snapshot_view_count": _as_int(phase3_report.get("report_snapshot_version_control_view_count")),
        "phase3_human_handling_required_count": _as_int(phase3_report.get("human_handling_required_count")),
        "phase3_failure_state_count": _as_int(p3_failures.get("failure_state_count")),
        "phase4_index_manifest_sample_count": _as_int(phase4_report.get("index_manifest_control_sample_count")),
        "phase4_smoke_log_sample_count": _as_int(phase4_report.get("smoke_test_log_control_sample_count")),
        "phase4_switch_record_sample_count": _as_int(phase4_report.get("switch_record_control_sample_count")),
        "phase4_rollback_proof_sample_count": _as_int(phase4_report.get("rollback_proof_control_sample_count")),
        "phase4_old_index_retention_count": _as_int(phase4_report.get("old_index_retention_projection_count")),
        "phase4_operational_instruction_count": _as_int(phase4_report.get("operational_instruction_projection_count")),
        "phase4_chinese_feedback_count": len(_as_sequence(phase4_report.get("chinese_feedback"))),
...
def _failure_stop_and_rollback_boundary(
    ...
    p1 = _mapping(phase1.get("rollback_eligibility_and_service_continuity_contract"))
    p2 = _mapping(phase2.get("failure_and_continuity_contract"))
    p3 = _mapping(phase3.get("failure_and_continuity_contract"))
    p4 = _mapping(phase4.get("delivery_evidence_contract"))
    return (
        _expected(
            p1,
            {
        ...
        and _mapping(phase4.get("rollback_contract")).get("return_to") == P3_PASS_RESULT
...
def _future_stage_boundary(
    ...
    return (
        ...
        and all(
            _mapping(contract.get("stage_and_phase_boundary")).get("stage081_started")
            is False
            for contract in (phase1, phase2, phase3, phase4)
        )
        and phase4.get("next_gate") == REVIEW_GATE
        and phase3_report.get("stage081_started") is False
        and phase4_report.get("stage081_started") is False
        and phase3_report.get("next_gate") == "IDS-STAGE080-P4-GATE"
...
def _authority_closed(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and authority["authority"].startswith("FROZEN_STAGE080_TASKPACK")
        and _expected(
            authority,
            {
                "second_authoritative_source_created": False,
...
def _closed_rollback(contract: Mapping[str, Any]) -> bool:
...
def _phase2_field_check_count_from_report(report: Mapping[str, Any]) -> int:
    return sum(
        len(_as_records(report.get(key))) * field_count
        for key, field_count in (
            ("index_version_control_records", 7),
            ("building_and_shadow_control_projections", 5),
            ("active_pointer_control_projections", 5),
            ("smoke_test_input_control_projections", 6),
            ("smoke_test_output_control_projections", 5),
            ("switch_control_projections", 9),
            ("rollback_request_control_projections", 8),
        )
    )
...
def _chinese_feedback(review_valid: bool) -> list[str]:
    if review_valid:
        return [
            "Stage080 索引回滚控制合同已机械复审，未执行实际索引操作。",
            "候选隔离、失败保持旧活动版本与回滚保留边界保持不变。",
            "交付证据仅为内存控制投影，业务线白箱人工复核仍是唯一权威。",
            "仅开放 Stage081 P1 门，未启动后续阶段、生产或上传。",
        ]
    return [
        "Stage080 整阶段复审未通过，保持在 Review 门。",
