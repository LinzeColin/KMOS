"""Stage081 P4 的纯内存影子索引交付证据。

模块只复用 Stage081 P2 的五组固定、非业务控制投影及 P3 的六条受控场景，
派生控制版索引清单、冒烟测试日志、切换记录、回滚证明、保留／空间影响投影
...
SCHEMA_VERSION = "ids.stage081.shadow_index.phase4.delivery.v1"
RECORD_KIND = "SHADOW_INDEX_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_SHADOW_INDEX_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_SHADOW_INDEX_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE081-P4-GATE"
...
def build_shadow_index_phase4_delivery_report(
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_shadow_index_phase3_report()


def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_phase2_module()
    return module.execute_shadow_index_control_slice(module.build_control_input())
...
def _load_phase3_module() -> Any:
    return _load_module("stage081_phase3_scenarios", "stage081_shadow_index_scenarios.py")


def _load_phase2_module() -> Any:
    return _load_module("stage081_phase2_slice", "stage081_shadow_index_control_slice.py")
    ...
    return [
        {
            ...
            "index_manifest_ref": f"index-manifest:control:stage081-p2:{scenario}",
    ...
    return [
        {
            ...
            "smoke_test_log_ref": (
                "smoke-test-log:control:stage081-p2:"
...
def _rollback_proof_samples(report: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "control_scenario": scenario,
            "rollback_proof_ref": f"rollback-proof:control:stage081-p2:{scenario}",
            "rollback_record_ref": record["rollback_request_ref"],
            "index_kind": record["index_kind"],
        ...
        for scenario, record in zip(
            P2_SCENARIOS, _as_records(report.get("rollback_request_control_projections"))
        )
    ]
...
def _old_index_retention_projection() -> dict[str, Any]:
    return {
        "retention_policy_ref": "index-retention-policy:control:stage081-p2:previous-active",
...
def _operational_instruction_projections() -> list[dict[str, Any]]:
    marker = ":control:stage081-p2:operations"
    return [
        {
            "instruction_id": "SHADOW_INDEX_REBUILD_CONTROL_INSTRUCTION",
            "action": "REBUILD",
            "target_ref": f"shadow-index-rebuild{marker}",
        ...
        {
            "instruction_id": "SHADOW_INDEX_PAUSE_CONTROL_INSTRUCTION",
            "action": "PAUSE",
            "target_ref": f"shadow-index-pause{marker}",
        ...
        {
            "instruction_id": "SHADOW_INDEX_RECOVERY_CONTROL_INSTRUCTION",
            "action": "RECOVERY",
            "target_ref": f"shadow-index-recovery{marker}",
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    ...
    return bool(reference_values) and all(
        ":control:stage081-p2:" in str(value) for value in reference_values
    )
