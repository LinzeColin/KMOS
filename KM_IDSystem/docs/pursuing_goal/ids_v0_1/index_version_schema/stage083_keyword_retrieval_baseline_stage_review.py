"""Stage083 的纯内存整阶段机械复审，不读取真实资料或启动 Stage084。"""
...
P1_CONTRACT = BASE / "stage083_keyword_retrieval_baseline_contract.json"
P2_CONTRACT = BASE / "stage083_keyword_retrieval_baseline_slice_contract.json"
P3_CONTRACT = BASE / "stage083_keyword_retrieval_baseline_scenarios_contract.json"
P4_CONTRACT = BASE / "stage083_keyword_retrieval_baseline_delivery_contract.json"

SCHEMA_VERSION = "ids.stage083.keyword_retrieval_baseline.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE083-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-083"
PASS_RESULT = "PASS_REVIEWED_KEYWORD_RETRIEVAL_BASELINE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_KEYWORD_RETRIEVAL_BASELINE_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE083-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE084-P1-GATE"
RETURN_STATE = "PASS_KEYWORD_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
...
P4_PASS_RESULT = "PASS_KEYWORD_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage083-p2:"

EXPECTED_CONTROLLED_REPLAY = {
    "phase1_query_field_count": 7,
    "phase1_metadata_filter_field_count": 6,
    "phase1_candidate_field_count": 8,
    "phase1_selected_result_field_count": 7,
    "phase1_hybrid_score_field_count": 7,
    "phase1_retrieval_trace_field_count": 8,
    ...
    "phase2_failure_state_count": 14,
    "phase3_scenario_count": 8,
    "phase3_scenario_field_count": 26,
    "phase3_scenario_field_check_count": 208,
    ...
    "phase4_parameter_rollback_instruction_count": 3,
    "phase4_delivery_field_check_count": 432,
...
def build_keyword_retrieval_stage083_review_report(
    *,
    ...
    phase2_module = _load_module("stage083_review_phase2", "stage083_keyword_retrieval_baseline_control_slice.py")
    phase3_module = _load_module("stage083_review_phase3", "stage083_keyword_retrieval_baseline_scenarios.py")
    phase4_module = _load_module("stage083_review_phase4", "stage083_keyword_retrieval_baseline_delivery.py")
    ...
    controlled_replay = _controlled_replay(phase1, phase2_contract, phase2, phase3, phase4_contract, phase4)
    ...
    next_stage_available_but_not_started = (
        ...
        and _mapping(phase1.get("stage_and_phase_boundary")).get("stage084_started") is False
        and phase3.get("stage084_started") is False
        and phase4.get("stage084_started") is False
    )
    ...
    review_invariants = {
        ...
        "stage084_gate_only_opens_after_review": review_valid and next_gate == NEXT_GATE,
    }
    return {
        "schema_version": SCHEMA_VERSION,
        "task_id": TASK_ID,
        "acceptance_id": ACCEPTANCE_ID,
        "source_authority": "FROZEN_STAGE083_TASKPACK_AND_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        "reviewed_phase_ids": [
            "IDS-STAGE083-P1",
            "IDS-STAGE083-P2",
            "IDS-STAGE083-P3",
        ...
        "rollback": {
            "scope": "STAGE083_REVIEW_ARTIFACTS_AND_LOCAL_GOVERNANCE_ONLY",
...
def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_module("stage083_review_phase2_provider", "stage083_keyword_retrieval_baseline_control_slice.py")
    if module is None:
        return {}
    return module.execute_keyword_retrieval_control_slice(module.build_control_input())


def _default_phase3_report_provider() -> Mapping[str, Any]:
    module = _load_module("stage083_review_phase3_provider", "stage083_keyword_retrieval_baseline_scenarios.py")
    if module is None:
        return {}
    return module.build_keyword_retrieval_phase3_report()


def _default_phase4_report_provider() -> Mapping[str, Any]:
    module = _load_module("stage083_review_phase4_provider", "stage083_keyword_retrieval_baseline_delivery.py")
    if module is None:
        return {}
    return module.build_keyword_retrieval_phase4_delivery_report()
...
def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    expected_shapes = (
        ("query_field_count", "future_query_fields", 7),
        ("metadata_filter_field_count", "future_metadata_filter_fields", 6),
        ("candidate_field_count", "future_candidate_fields", 8),
        ("selected_result_field_count", "future_selected_result_fields", 7),
        ("hybrid_score_field_count", "future_hybrid_score_fields", 7),
        ("retrieval_trace_field_count", "future_retrieval_trace_fields", 8),
    )
    return (
        contract.get("schema_version") == "ids.stage083.keyword_retrieval_baseline_contract.phase1.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE083-P1"
        and contract.get("stage") == "STAGE-083"
        and contract.get("phase") == "IDS-STAGE083-P1"
        and contract.get("next_gate") == "IDS-STAGE083-P2-GATE"
        and all(
        ...
        and boundary.get("stage083_started") is True
        ...
        and boundary.get("stage084_started") is False
        and rollback.get("return_to") == "PASS_REVIEWED_OLD_INDEX_RETENTION_RUNTIME_DISABLED"
        and _mapping(contract.get("failure_and_stop_contract")).get("failure_state_count") == 13
    )
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    boundary = _mapping(contract.get("stage_and_phase_boundary"))
    return (
        contract.get("schema_version") == "ids.stage083.keyword_retrieval_baseline.phase2.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE083-P2"
        and contract.get("stage") == "STAGE-083"
        and contract.get("phase") == "IDS-STAGE083-P2"
        and contract.get("next_gate") == "IDS-STAGE083-P3-GATE"
        and inputs.get("control_request_count") == 5
        and inputs.get("control_prefix") == CONTROL_PREFIX
        and inputs.get("input_field_count") == 15
        and inputs.get("all_control_values_are_opaque_labels") is True
        ...
        and boundary.get("whole_stage_review_performed") is False
        and boundary.get("stage084_started") is False
        and _mapping(contract.get("failure_and_stop_contract")).get("failure_state_count") == 14
    )
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        contract.get("schema_version") == "ids.stage083.keyword_retrieval_baseline.phase3.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE083-P3"
        and contract.get("stage") == "STAGE-083"
        and contract.get("phase") == "IDS-STAGE083-P3"
        and contract.get("next_gate") == "IDS-STAGE083-P4-GATE"
        and replay.get("required_control_request_count") == 5
        and replay.get("control_prefix") == CONTROL_PREFIX
        and replay.get("expected_phase2_field_check_count") == 250
        and scenarios.get("scenario_count") == 8
        and scenarios.get("scenario_field_count") == 26
        and scenarios.get("expected_scenario_field_check_count") == 208
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    rollback = _mapping(contract.get("rollback_contract"))
    return (
        contract.get("schema_version") == "ids.stage083.keyword_retrieval_baseline.phase4.delivery.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE083-P4"
        and contract.get("stage") == "STAGE-083"
        and contract.get("phase") == "IDS-STAGE083-P4"
        and contract.get("next_gate") == REVIEW_GATE
        and replay.get("required_control_request_count") == 5
        and replay.get("phase2_control_field_check_count") == 250
        and replay.get("scenario_count") == 8
        and replay.get("scenario_field_count") == 26
        and replay.get("scenario_field_check_count") == 208
        ...
        and delivery.get("parameter_rollback_instruction_count") == 3
        and delivery.get("delivery_field_check_count") == 432
        ...
        and _mapping(contract.get("failure_and_stop_contract")).get("failure_state_count") == 16
    )
...
def _phase2_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        report.get("schema_version") == "ids.stage083.keyword_retrieval_baseline.phase2.v1"
...
def _phase3_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("next_gate") == "IDS-STAGE083-P4-GATE"
        ...
        and report.get("phase2_control_record_field_check_count") == 250
        and report.get("scenario_count") == 8
        and report.get("scenario_field_count") == 26
        and report.get("scenario_field_check_count") == 208
...
def _phase4_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    shape_specs = (
        ...
        ("parameter_rollback_instruction_control_records", "PARAMETER_ROLLBACK_INSTRUCTION_FIELDS", 3),
    )
    return (
        report.get("valid") is True
        ...
        and report.get("parameter_rollback_instruction_count") == 3
        and report.get("delivery_field_check_count") == 432
...
def _controlled_replay(
    ...
    p2_projection = _mapping(phase2_contract.get("control_projection_contract"))
    p4_delivery = _mapping(phase4_contract.get("delivery_evidence_contract"))
    return {
...
def _failure_and_rollback_boundary(
    ...
    return (
        _mapping(phase1.get("failure_and_stop_contract")).get("failure_state_count") == 13
        and _mapping(phase2_contract.get("failure_and_stop_contract")).get("failure_state_count") == 14
        and _mapping(phase3_contract.get("scenario_result_contract")).get("silent_drop_allowed") is False
        and _mapping(phase4_contract.get("failure_and_stop_contract")).get("failure_state_count") == 16
        ...
        and _mapping(phase4_contract.get("rollback_contract")).get("return_to") == P3_PASS_RESULT
...
def _nested_runtime_closed(*items: Mapping[str, Any]) -> bool:
    for item in items:
        for field, value in item.items():
            if field.endswith("_performed") or field.endswith("_written") or field.endswith("_published"):
