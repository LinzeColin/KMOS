"""Stage115 P3 复核 UI 专项异常场景的纯内存控制投影。

模块机械投影 Stage115 P2 的固定 reference-only 控制请求，形成五条专项
...
SCHEMA_VERSION = "ids.stage115.review_ui.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REVIEW_UI_SCENARIOS"
CONTROL_ADAPTER_VERSION = "stage115-p3-controlled-scenarios-v1"
PASS_RESULT = "PASS_IN_MEMORY_REVIEW_UI_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
REJECTED_RESULT = "REJECTED_IN_MEMORY_REVIEW_UI_CONTROLLED_SCENARIOS"
CONTROL_PREFIX = ":control:stage115-p3:"
CONTROL_FIELDS = ("review_ui_controlled_scenario_requests",)
...
CONTROLLED_SCENARIO_DEFINITIONS = (
    {
        ...
        "expected_binding_mode": "CONTROL_BINDING_REVIEW_QUEUE_ROUTE",
        "expected_review_status": "pending_review",
        "expected_review_action": "submit_for_review",
        "chinese_title": "低质量 OCR 复核操作控制",
        "chinese_reason": "低 OCR 置信度进入待复核控制路径，业务线白箱确认后承接处理。",
    ...
    },
    {
        ...
        "expected_review_action": "confirm",
    ...
    },
    {
        ...
        "expected_review_action": "request_more_material",
    ...
    },
    {
        ...
        "expected_binding_mode": "CONTROL_BINDING_EXTERNAL_AUGMENTATION_ROUTE",
        "expected_review_status": "archived",
        "expected_review_action": "archive",
        "chinese_title": "外部增强替代内部证据控制",
        "chinese_reason": "外部增强保持外部来源身份，归档控制由业务线白箱确认承接。",
        "whitebox_handling_code": (
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "stage115_phase3_runtime_executed",
)
...
CHINESE_FEEDBACK = (
    "复核 UI 专项场景已验证，低质量 OCR、资料冲突和撤回资料保持 reference-only 控制记录。",
    "复核操作保留 actor、time、reason、old value、new value、复核结果与复核审计的未来控制引用。",
    "复核结果对 evidence trust level 与报告质量分的影响保持未来写回控制，业务线白箱确认继续作为前置。",
    "外部增强保持外部来源身份，内部证据绑定和证据缺口继续由业务线白箱确认承接。",
    "P4 交付样例、复核日志和 UI 流程说明继续受 IDS-STAGE115-P4-GATE 门禁控制。",
)
...
def _controlled_scenario_request(
    ...
    phase2_request: Mapping[str, str],
) -> dict[str, str]:
    """将一条 P2 控制请求扩展为固定 P3 场景记录。"""

    scenario_id = definition["scenario_id"]
    request = dict(phase2_request)
    request.update(
        {
...
def build_controlled_scenario_input() -> dict[str, list[dict[str, str]]]:
    """返回唯一允许的五条 Stage115 P3 专项场景控制输入。"""
...
def _phase2_result_is_closed(phase2_result: Mapping[str, Any]) -> bool:
    field_total_per_request = sum(
        len(fields) for _prefix, fields in phase2.PROJECTION_FIELDS
    )
    return all(
        (
            ...
            phase2_result.get("control_projection_field_total_per_request")
            == field_total_per_request,
            phase2_result.get("control_projection_field_total")
            == len(PHASE2_CONTROL_SCENARIOS) * field_total_per_request,
...
def _scenario_binding_is_valid(
    ...
    for definition, scenario in zip(CONTROLLED_SCENARIO_DEFINITIONS, scenarios):
        ...
        if scenario["control_scenario"] != definition["phase2_control_scenario"]:
            return False
        if scenario["control_binding_mode"] != definition["expected_binding_mode"]:
            return False
        if (
            ...
            return False
        if (
            scenario["fixed_review_action_control_value"]
            != definition["expected_review_action"]
        ):
            return False
        if scenario["controlled_scenario_chinese_title"] != definition["chinese_title"]:
            return False
        if scenario["controlled_scenario_chinese_reason"] != definition["chinese_reason"]:
            return False
...
def _result_base(
    *,
    ...
    phase2_field_total_per_request = sum(
        len(fields) for _prefix, fields in phase2.PROJECTION_FIELDS
    )
    return {
        ...
        "phase2_projection_field_total_per_request": phase2_field_total_per_request,
        "phase2_projection_field_check_count": (
            len(PHASE2_CONTROL_SCENARIOS) * phase2_field_total_per_request
        ),
...
def project_review_ui_controlled_scenarios(
    controlled_scenario_input: Mapping[str, Any],
) -> dict[str, Any]:
    """机械投影固定 P3 输入；漂移输入形成零投影拒绝结果。"""
    ...
    phase2_result = phase2.project_review_ui_control_slice(phase2_input)
