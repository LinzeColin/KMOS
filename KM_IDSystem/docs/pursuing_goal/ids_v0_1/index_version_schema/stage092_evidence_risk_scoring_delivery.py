"""Stage092 P4 的纯内存证据风险评分交付控制证据。

模块只从 Stage092 P3 的固定、非业务、reference-only 异常场景派生交付
控制记录。所有结果只存在于当前 Python 进程；来源文档、真实证据账本、报告
与业务事实仍由既有权威和业务线白箱人工复核裁定。
"""
...
SCHEMA_VERSION = "ids.stage092.evidence_risk_scoring.phase4.delivery.v1"
RECORD_KIND = "EVIDENCE_RISK_SCORING_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_EVIDENCE_RISK_SCORING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_RISK_SCORING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE092-P4-GATE"
NEXT_GATE = "IDS-STAGE092-REVIEW-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_RISK_SCORING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage092-p2:"
DELIVERY_PREFIX = ":control:stage092-p4:"
...
def build_evidence_risk_scoring_phase4_delivery_report(
    phase3_report_provider: Phase3ReportProvider | None = None,
) -> dict[str, Any]:
    """从固定 P3 场景派生未持久化的 Stage092 P4 交付控制记录。"""
    ...
    return {
        ...
        "chinese_feedback": [
            ...
            "Stage092 P4 只开放整阶段机械复审门，Review、OVH、生产与正式上传保持后续阶段。",
        ],
    }


def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_evidence_risk_scoring_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage092_evidence_risk_scoring_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage092_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage092 P3 evidence risk scoring scenarios")
...
def _phase3_shape_is_preserved(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("phase2_projection_group_count") == 11
...
def _phase3_reference_is_allowed(
    ...
    return _inherited_control_ref(value) or (
        ...
        and scenario.get("scenario_id") == "no_internal_evidence_risk_control"
        and value is None
    )
...
def _phase3_semantic_failure(report: Mapping[str, Any]) -> str | None:
    ...
    no_internal = scenarios.get("no_internal_evidence_risk_control", {})
    if (
        ...
        return "NO_INTERNAL_EVIDENCE_RISK_ROUTE_MISSING"
    degradation_states = {
        "low_ocr_evidence_risk_degradation_control": (
            "CONTROL_LOW_OCR_DEGRADED_NOT_ACCEPTED"
        ),
        "old_version_evidence_risk_degradation_control": (
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED"
        ),
        "conflict_evidence_risk_degradation_control": (
    ...
    revoked = scenarios.get("revoked_evidence_risk_report_impact_control", {})
    if (
    ...
    malicious = scenarios.get("malicious_evidence_risk_quarantine_control", {})
    if (
    ...
    masquerade = scenarios.get("low_grade_high_trust_masquerade_risk_control", {})
    if (
...
def _delivery_reference_is_allowed(
    ...
    return _control_ref(value) or (
        field == "evidence_id_ref"
        and record.get("scenario_id") == "no_internal_evidence_risk_control"
        and value is None
    )
