"""Stage096 P3 的纯内存知识库投毒防护异常场景验证。

模块只重放 Stage096 P2 的固定、非业务、reference-only 控制投影。它不读取
来源资料、检索结果或真实证据账本，不连接数据库，不计算真实风险，不改变证据
等级，不执行撤回、降级、隔离、投毒处置或报告状态更新，也不写入持久化记录。
"""
...
SCHEMA_VERSION = "ids.stage096.knowledge_base_poisoning_defense.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_KNOWLEDGE_BASE_POISONING_DEFENSE_EXCEPTION_SCENARIOS"
PASS_RESULT = "PASS_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CURRENT_GATE = "IDS-STAGE096-P3-GATE"
NEXT_GATE = "IDS-STAGE096-P4-GATE"
P2_EXECUTION_STATE = "CONTROL_KNOWLEDGE_BASE_POISONING_DEFENSE_PROJECTIONS_DECLARED"
...
SCENARIO_DEFINITIONS = (
    {
        "scenario_id": "no_internal_evidence_poisoning_defense_control",
...
def _load_phase2_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage096_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage096 P2 control slice")
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    ...
    if (
        ...
        != ("knowledge_base_poisoning_defense_control_requests",)
...
def _phase2_semantic_failure(result: Mapping[str, Any]) -> str | None:
    ...
    for records in records_by_scenario.values():
        ...
        relation = records["knowledge_base_poisoning_defense_relation"]
    ...
    no_internal_schema = no_internal["knowledge_base_poisoning_defense_schema_binding"]
    if (
...
def _scenario_expectation_met(
    ...
    schema = records["knowledge_base_poisoning_defense_schema_binding"]
    relation = records["knowledge_base_poisoning_defense_relation"]
...
def build_knowledge_base_poisoning_defense_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制投影并验证 Stage096 P3 固定异常场景。"""

    try:
        phase2_module = _load_phase2_module()
        executor = (
            phase2_executor
            or phase2_module.execute_knowledge_base_poisoning_defense_control_slice
        )
