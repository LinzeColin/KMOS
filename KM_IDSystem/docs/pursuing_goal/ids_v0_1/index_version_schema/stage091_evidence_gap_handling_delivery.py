"""Stage091 P4 的纯内存证据缺口处理交付控制证据。

模块重放 Stage091 P3 的七个固定异常场景，派生 reference-only 的证据账本
样例、等级报告、撤回影响、回归记录、结论依据限制与恢复说明。输出仅存在于
当前 Python 进程，来源文档、真实证据账本、报告和业务事实仍由各自权威裁定。
"""
...
SCHEMA_VERSION = "ids.stage091.evidence_gap_handling.phase4.delivery.v1"
RECORD_KIND = "EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE091-P4-GATE"
NEXT_GATE = "IDS-STAGE091-REVIEW-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage091-p2:"
DELIVERY_PREFIX = ":control:stage091-p4:"

P2_CONTROL_REQUEST_COUNT = 6
P2_CONTROL_FIELD_CHECK_COUNT = 468
P3_SCENARIO_COUNT = 7
...
DEGRADATION_INSTRUCTION_DEFINITIONS = (
    ...
    },
    {
        ...
        "condition_field": "freshness_indicator_ref",
...
def build_evidence_gap_handling_phase4_delivery_report(
    phase3_report_provider: Phase3ReportProvider | None = None,
) -> dict[str, Any]:
    """从固定 P3 场景派生未持久化的 Stage091 P4 交付控制记录。"""
    ...
    return {
        ...
        "stage090_review_evidence_declared": True,
        ...
        "chinese_feedback": [
            ...
            "Stage091 P4 只开放整阶段机械复审门，Review、OVH、生产与正式上传保持后续阶段。",
        ],
    }


def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_evidence_gap_handling_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage091_evidence_gap_handling_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage091_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage091 P3 evidence gap handling scenarios")
...
def _phase3_reference_is_allowed(
    ...
    return _inherited_control_ref(value) or (
        ...
        and scenario.get("scenario_id") == "no_internal_evidence_gap_control"
        and value is None
    )
    ...
    return [
        {
            ...
            "freshness_indicator_ref": scenario["freshness_indicator_ref"],
...
def _delivery_reference_is_allowed(
    ...
    return _control_ref(value) or (
        field == "evidence_id_ref"
        and record.get("scenario_id") == "no_internal_evidence_gap_control"
        and value is None
    )
