"""Stage096 P2 的纯内存知识库投毒防护控制切片。

模块只投影自身定义的固定、非业务、reference-only 控制输入。它承接
Stage096 P1 静态合同和 Stage095 已复审证据回归控制工件，不读取真实资料、
...
SCHEMA_VERSION = "ids.stage096.knowledge_base_poisoning_defense.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_KNOWLEDGE_BASE_POISONING_DEFENSE"
CONTROL_ADAPTER_VERSION = (
    "ids.knowledge_base_poisoning_defense.control_adapter.v0_1.stage096.p2"
)
CONTROL_PREFIX = ":control:stage096-p2:"
CONTROL_FIELDS = ("knowledge_base_poisoning_defense_control_requests",)
PHASE1_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTRACT_CONTROL_REF = (
    ":control:stage096-p2:stage096-phase1-knowledge-base-poisoning-defense-contract:reference-only"
)
STAGE095_REVIEW_CONTROL_REF = (
    ":control:stage096-p2:stage095-reviewed-evidence-regression:reference-only"
)

KNOWLEDGE_BASE_POISONING_DEFENSE_SCHEMA_BINDING_FIELDS = (
    "phase1_knowledge_base_poisoning_defense_contract_ref",
    "stage095_review_control_ref",
    "evidence_ledger_ref",
    "evidence_id_ref",
    "evidence_gap_ref",
...
KNOWLEDGE_BASE_POISONING_DEFENSE_RELATION_FIELDS = (
    "evidence_ledger_ref",
    "evidence_id_ref",
    "evidence_gap_ref",
    "critical_conclusion_ref",
...
PROJECTION_FIELDS = (
    (
        "knowledge_base_poisoning_defense_schema_binding",
        KNOWLEDGE_BASE_POISONING_DEFENSE_SCHEMA_BINDING_FIELDS,
    ),
    (
        "knowledge_base_poisoning_defense_relation",
        KNOWLEDGE_BASE_POISONING_DEFENSE_RELATION_FIELDS,
    ),
    (
...
def _zero_actual_counts() -> dict[str, int]:
    return {
        ...
        "actual_knowledge_base_poisoning_defense_execution_count": 0,
...
def _rejected_result() -> dict[str, Any]:
    ...
    return {
        ...
        "execution_state": "REJECTED_IN_MEMORY_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTROL_SLICE",
...
def _project(
    ...
    configuration = CONTROL_SCENARIO_CONFIGURATION[str(request["control_scenario"])]
    schema_binding = {
        "phase1_knowledge_base_poisoning_defense_contract_ref": (
            PHASE1_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTRACT_CONTROL_REF
        ),
        "stage095_review_control_ref": STAGE095_REVIEW_CONTROL_REF,
        ...
        "schema_binding_state": "CONTROL_PHASE1_KNOWLEDGE_BASE_POISONING_DEFENSE_SHAPE_BOUND",
        "control_slice_state": "CONTROL_REFERENCE_ONLY_IN_MEMORY",
    }
    relation = {
        field: request[field]
        for field in KNOWLEDGE_BASE_POISONING_DEFENSE_RELATION_FIELDS
    }
    retrieval_capture = {
        field: request[field]
    ...
    return {
        "knowledge_base_poisoning_defense_schema_binding": schema_binding,
        "knowledge_base_poisoning_defense_relation": relation,
...
def execute_knowledge_base_poisoning_defense_control_slice(
    ...
    return {
        ...
        "control_adapter_version": CONTROL_ADAPTER_VERSION,
        "input_accepted": True,
        "execution_state": "CONTROL_KNOWLEDGE_BASE_POISONING_DEFENSE_PROJECTIONS_DECLARED",
        "failure_state": None,
