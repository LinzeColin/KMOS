"""Stage084 的纯内存整阶段机械复审，不读取真实资料或启动 Stage085。"""
...
P1_CONTRACT = BASE / "stage084_vector_retrieval_baseline_contract.json"
P2_CONTRACT = BASE / "stage084_vector_retrieval_baseline_slice_contract.json"
P3_CONTRACT = BASE / "stage084_vector_retrieval_baseline_scenarios_contract.json"
P4_CONTRACT = BASE / "stage084_vector_retrieval_baseline_delivery_contract.json"

SCHEMA_VERSION = "ids.stage084.vector_retrieval_baseline.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE084-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-084"
PASS_RESULT = "PASS_REVIEWED_VECTOR_RETRIEVAL_BASELINE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_VECTOR_RETRIEVAL_BASELINE_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE084-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE085-P1-GATE"
RETURN_STATE = "PASS_VECTOR_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
P2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_VECTOR_RETRIEVAL_CONTROL_SLICE"
P3_PASS_RESULT = "PASS_VECTOR_RETRIEVAL_BASELINE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_PASS_RESULT = "PASS_VECTOR_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage084-p2:"

EXPECTED_CONTROLLED_REPLAY = {
    "phase1_query_field_count": 11,
    "phase1_metadata_filter_field_count": 6,
    "phase1_candidate_field_count": 10,
    "phase1_selected_result_field_count": 7,
    ...
    "phase2_control_field_check_count": 290,
    "phase2_failure_state_count": 18,
...
def build_vector_retrieval_stage084_review_report(
    *,
    ...
    phase2_module = _load_module("stage084_review_phase2", "stage084_vector_retrieval_baseline_control_slice.py")
    phase3_module = _load_module("stage084_review_phase3", "stage084_vector_retrieval_baseline_scenarios.py")
    phase4_module = _load_module("stage084_review_phase4", "stage084_vector_retrieval_baseline_delivery.py")
    ...
    next_stage_available_but_not_started = (
        ...
        and _mapping(phase1.get("stage_and_phase_boundary")).get("stage085_started") is False
        and _mapping(phase2_contract.get("stage_and_phase_boundary")).get("stage085_started") is False
        and _mapping(phase3_contract.get("stage_boundary")).get("stage085_started") is False
        and _mapping(phase4_contract.get("stage_and_phase_boundary")).get("stage085_started") is False
        and phase3.get("stage085_started") is False
        and phase4.get("stage085_started") is False
    )
    ...
    review_invariants = {
        ...
        "stage085_gate_only_opens_after_review": review_valid and next_gate == NEXT_GATE,
    }
    return {
        "schema_version": SCHEMA_VERSION,
        "task_id": TASK_ID,
        "acceptance_id": ACCEPTANCE_ID,
        "source_authority": "FROZEN_STAGE084_TASKPACK_AND_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        "reviewed_phase_ids": [
            "IDS-STAGE084-P1",
            "IDS-STAGE084-P2",
            "IDS-STAGE084-P3",
        ...
        "phase3_started": True,
        "phase4_started": True,
        "stage084_started": True,
        "stage085_started": False,
        ...
        "rollback": {
            "scope": "STAGE084_REVIEW_ARTIFACTS_AND_LOCAL_GOVERNANCE_ONLY",
...
def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_module(
        "stage084_review_phase2_provider", "stage084_vector_retrieval_baseline_control_slice.py"
    )
    if module is None:
        return {}
    return module.execute_vector_retrieval_control_slice(module.build_control_input())


def _default_phase3_report_provider() -> Mapping[str, Any]:
    module = _load_module(
        "stage084_review_phase3_provider", "stage084_vector_retrieval_baseline_scenarios.py"
    )
    if module is None:
        return {}
    return module.build_vector_retrieval_phase3_report()


def _default_phase4_report_provider() -> Mapping[str, Any]:
    module = _load_module(
        "stage084_review_phase4_provider", "stage084_vector_retrieval_baseline_delivery.py"
    )
    if module is None:
        return {}
    return module.build_vector_retrieval_phase4_delivery_report()
...
def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    expected_shapes = (
        ...
        ("metadata_filter_field_count", "future_metadata_filter_fields", 6),
        ("candidate_field_count", "future_candidate_fields", 10),
        ("selected_result_field_count", "future_selected_result_fields", 7),
        ("hybrid_score_field_count", "future_hybrid_score_fields", 7),
        ("retrieval_trace_field_count", "future_retrieval_trace_fields", 10),
    )
    return (
        contract.get("schema_version") == "ids.stage084.vector_retrieval_baseline_contract.phase1.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE084-P1"
        and contract.get("stage") == "STAGE-084"
        and contract.get("phase") == "IDS-STAGE084-P1"
        and contract.get("next_gate") == "IDS-STAGE084-P2-GATE"
        and all(
            shape.get(count_field) == expected_count
            and _sequence_length(shape.get(fields_field)) == expected_count
            for count_field, fields_field, expected_count in expected_shapes
        )
        and shape.get("keyword_retrieval_baseline_required") is True
        and shape.get("vector_retrieval_baseline_required") is True
        and shape.get("vector_similarity_only_prohibited") is True
        and shape.get("embedding_model_version_contract_required") is True
        and shape.get("similarity_metric_contract_required") is True
        and shape.get("metadata_filter_contract_required") is True
        and shape.get("hybrid_ranking_contract_required") is True
        and shape.get("retrieval_trace_contract_required") is True
        ...
        and boundary.get("stage084_started") is True
        ...
        and boundary.get("stage085_started") is False
        and rollback.get("return_to") == "PASS_REVIEWED_KEYWORD_RETRIEVAL_BASELINE_RUNTIME_DISABLED"
        and _mapping(contract.get("failure_and_stop_contract")).get("failure_state_count") == 17
    )
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    boundary = _mapping(contract.get("stage_and_phase_boundary"))
    return (
        contract.get("schema_version") == "ids.stage084.vector_retrieval_baseline.phase2.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE084-P2"
        and contract.get("stage") == "STAGE-084"
        and contract.get("phase") == "IDS-STAGE084-P2"
        and contract.get("next_gate") == "IDS-STAGE084-P3-GATE"
        and inputs.get("control_request_count") == 5
        and inputs.get("control_prefix") == CONTROL_PREFIX
        and inputs.get("input_field_count") == 19
        and inputs.get("all_values_are_opaque_control_labels_only") is True
        and projection.get("each_projection_count") == 5
        and projection.get("control_projection_field_total_per_request") == 58
        and projection.get("control_projection_field_total") == 290
        and projection.get("keyword_retrieval_baseline_required") is True
        and projection.get("vector_retrieval_baseline_required") is True
        and projection.get("vector_similarity_only_prohibited") is True
        and projection.get("five_metadata_filter_dimensions_required") is True
        and projection.get("embedding_model_version_dimension_and_similarity_metric_required") is True
        and projection.get("candidate_selected_and_trace_active_index_version_chain_required") is True
        and projection.get("candidate_and_trace_vector_contract_chain_required") is True
        and projection.get("score_explanation_required") is True
        and projection.get("selected_result_and_trace_evidence_ledger_binding_required") is True
        and authority.get("second_authoritative_source_created") is False
        and authority.get("source_body_or_path_allowed") is False
        and _runtime_boundary_closed(_mapping(contract.get("runtime_boundary")))
        ...
        and boundary.get("whole_stage_review_performed") is False
        and boundary.get("stage085_started") is False
        and _mapping(contract.get("failure_and_stop_contract")).get("failure_state_count") == 18
    )
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        contract.get("schema_version") == "ids.stage084.vector_retrieval_baseline.phase3.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE084-P3"
        and contract.get("stage") == "STAGE-084"
        and contract.get("phase") == "IDS-STAGE084-P3"
        and contract.get("next_gate") == "IDS-STAGE084-P4-GATE"
        and replay.get("required_control_request_count") == 5
        and replay.get("control_prefix") == CONTROL_PREFIX
        and replay.get("expected_phase2_field_check_count") == 290
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    rollback = _mapping(contract.get("rollback_contract"))
    return (
        contract.get("schema_version") == "ids.stage084.vector_retrieval_baseline.phase4.delivery.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE084-P4"
        and contract.get("stage") == "STAGE-084"
        and contract.get("phase") == "IDS-STAGE084-P4"
        and contract.get("next_gate") == REVIEW_GATE
        and replay.get("required_control_request_count") == 5
        and replay.get("phase2_control_field_check_count") == 290
        ...
        and delivery.get("retrieval_sample_control_record_count") == 8
        and delivery.get("trace_log_control_record_count") == 8
        and delivery.get("filter_result_control_record_count") == 8
        and delivery.get("validity_test_report_control_record_count") == 8
        and delivery.get("evidence_gap_control_record_count") == 8
        and delivery.get("parameter_rollback_instruction_count") == 4
...
def _phase2_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    projection_shapes_valid = all(
        _records_have_exact_shape(report.get(records_key), 5, getattr(module, field_name, ()))
        for records_key, field_name in P2_PROJECTION_FIELDS
    )
    counts_valid = all(
        report.get(f"{records_key.removesuffix('s')}_count") == 5
        for records_key, _field_name in P2_PROJECTION_FIELDS
    )
    return (
        report.get("schema_version") == "ids.stage084.vector_retrieval_baseline.phase2.v1"
        and report.get("input_accepted") is True
        and report.get("execution_state") == P2_EXECUTION_STATE
        and report.get("control_input_request_count") == 5
        and report.get("actual_input_request_count") == 0
        and projection_shapes_valid
        and counts_valid
        and report.get("all_keyword_baselines_declared") is True
        and report.get("all_vector_baselines_declared") is True
        and report.get("all_vector_similarity_only_routes_rejected") is True
        and report.get("all_metadata_filter_dimensions_covered") is True
        and report.get("all_candidate_active_index_versions_match") is True
        and report.get("all_candidate_vector_contracts_match") is True
        and report.get("all_selected_results_match_candidates") is True
        and report.get("all_score_explanations_declared") is True
        and report.get("all_trace_active_index_versions_match") is True
        and report.get("all_trace_vector_contracts_match") is True
        and report.get("all_evidence_ledger_bindings_declared") is True
...
def _phase3_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("next_gate") == "IDS-STAGE084-P4-GATE"
        ...
        and report.get("phase2_control_record_field_check_count") == 290
...
def _phase4_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    shape_specs = (
        ...
        ("parameter_rollback_instruction_control_records", "PARAMETER_ROLLBACK_INSTRUCTION_FIELDS", 4),
    )
    return (
        report.get("valid") is True
        ...
        and all(
            _records_have_exact_shape(report.get(records_key), count, getattr(module, field_name, ()))
...
def _controlled_replay(
    ...
    return {
        ...
        "phase1_metadata_filter_field_count": _integer(p1_shape.get("metadata_filter_field_count")),
        "phase1_candidate_field_count": _integer(p1_shape.get("candidate_field_count")),
        "phase1_selected_result_field_count": _integer(p1_shape.get("selected_result_field_count")),
        "phase1_hybrid_score_field_count": _integer(p1_shape.get("hybrid_score_field_count")),
        "phase1_retrieval_trace_field_count": _integer(p1_shape.get("retrieval_trace_field_count")),
        ...
        "phase2_failure_state_count": _integer(
            _mapping(phase2_contract.get("failure_and_stop_contract")).get("failure_state_count")
        ),
        ...
        "phase4_failure_state_count": _integer(
            _mapping(phase4_contract.get("failure_and_stop_contract")).get("failure_state_count")
        ),
    }
...
def _single_authority_boundary(
    ...
    return (
        ...
        and p3_authority.get("control_result_can_become_business_fact_authority") is False
        and p4_authority.get("source_document_remains_authoritative") is True
        and p4_authority.get("delivery_control_metadata_can_replace_source_document") is False
        and p4_authority.get("delivery_control_metadata_can_become_business_fact_authority") is False
...
def _failure_and_rollback_boundary(
    ...
    return (
        _mapping(phase1.get("failure_and_stop_contract")).get("failure_state_count") == 17
        and _mapping(phase2_contract.get("failure_and_stop_contract")).get("failure_state_count") == 18
        ...
        and _mapping(phase4_contract.get("failure_and_stop_contract")).get("failure_state_count") == 17
