"""Stage109 P4：报告影响分析的 metadata-only 交付证据。

模块只从 Stage109 P3 固定、非业务、reference-only 场景派生报告样例、快照、
质量评分、影响分析、模板限制、人工确认与生命周期说明的控制记录。模块不读取
真实资料、报告、PDF 或证据账本，不调用模型、Agent、OVH 或生产服务，也不写入
数据库、审计或持久化状态。
"""
...
SCHEMA_VERSION = "ids.stage109.report_impact_analysis.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_IMPACT_ANALYSIS_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_REPORT_IMPACT_ANALYSIS_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REPORT_IMPACT_ANALYSIS_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE109-P4-GATE"
NEXT_GATE = "IDS-STAGE109-REVIEW-GATE"
...
P3_CURRENT_GATE = "IDS-STAGE109-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE109-P4-GATE"
P2_CONTROL_PREFIX = ":control:stage109-p2:"
DELIVERY_PREFIX = ":control:stage109-p4:"
...
P3_SCENARIO_IDS = (
    ...
    "affected_report_lifecycle_external_augmentation_whitebox_control",
)
P3_SCENARIO_FIELD_COUNT = 42
...
P3_PHASE2_INPUT_FIELD_COUNT = 35
P3_PHASE2_PHASE1_REFERENCE_FIELD_COUNT = 33
...
P3_SCENARIO_FIELD_CHECK_COUNT = 210
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage109_phase4_runtime_executed",
)
ZERO_COUNTER_FIELDS = (
...
def _load_phase3_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        "stage109_phase3_report_impact_analysis_scenarios", module_path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage109 P3 报告影响分析专项场景")
...
def _phase3_shape_is_preserved(phase3_module: Any, report: Mapping[str, Any]) -> bool:
    if (
        ...
        or report.get("phase2_control_request_count") != P3_PHASE2_CONTROL_REQUEST_COUNT
        or report.get("phase2_input_field_count") != P3_PHASE2_INPUT_FIELD_COUNT
        or report.get("phase2_projection_group_count") != P3_PHASE2_PROJECTION_GROUP_COUNT
    ...
    for scenario in scenarios:
        ...
        if (
            ...
            is not True
            or scenario.get("external_augmentation_may_not_close_evidence_gap")
            is not True
...
def _report_snapshot_record(scenario: Mapping[str, Any], index: int) -> dict[str, Any]:
    return {
        ...
        "impact_trigger_ref": scenario["impact_trigger_ref"],
        ...
        "snapshot_consistency_state": "CONTROL_SNAPSHOT_REQUIRES_REPORT_STATUS_AND_TRIGGER_BINDING",
...
def _report_impact_analysis_record(scenario: Mapping[str, Any], index: int) -> dict[str, Any]:
    return {
        ...
        "affected_report_control_state": scenario["affected_report_control_state"],
...
def _template_and_whitebox_record(scenario: Mapping[str, Any], index: int) -> dict[str, Any]:
    return {
        ...
        "human_confirmation_gate_ref": _delivery_ref(f"whitebox-confirmation-gate-{index}"),
    ...
    return [
        {
            ...
            "rollback_target_ref": _delivery_ref("stage109-p3-controlled-scenarios"),
            "rollback_target_result": P3_PASS_RESULT,
            "predecessor_phase_ref": _delivery_ref("stage109-p3"),
...
def _validate_delivery_groups(report: Mapping[str, Any]) -> Optional[str]:
    ...
    for name, _fields in DELIVERY_GROUPS[:-1]:
        for record in report[name]:
            if (
                ...
                or any(value is True for key, value in record.items() if key.startswith("actual_"))
            ):
    ...
    for record in report["report_snapshot_control_records"]:
        if (
            ...
            or record["actual_report_status_impact_updated"] is not False
        ):
    ...
    for record in report["report_quality_score_control_records"]:
        if (
            ...
            or record["actual_report_quality_score_persisted"] is not False
        ):
    ...
    if {
        record["control_domain"] for record in lifecycle_records
...
def build_report_impact_analysis_phase4_delivery_report(
    ...
    try:
        ...
        executor = (
            ...
            else phase3_module.build_report_impact_analysis_phase3_report
        )
