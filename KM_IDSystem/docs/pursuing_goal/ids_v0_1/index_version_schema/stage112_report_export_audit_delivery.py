"""Stage112 P4：报告导出审计的 metadata-only 交付控制证据。

模块只从 Stage112 P3 固定、非业务、reference-only 场景派生报告样例、快照、
质量评分、影响分析、模板限制、人工确认与生命周期说明的控制记录。它不读取真实
资料、报告、PDF、证据账本或既有审计日志，不调用模型、Agent、OVH 或生产服务，
也不写入数据库、审计或持久化状态。
"""
...
SCHEMA_VERSION = "ids.stage112.report_export_audit.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_EXPORT_AUDIT_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_REPORT_EXPORT_AUDIT_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REPORT_EXPORT_AUDIT_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE112-P4-GATE"
NEXT_GATE = "IDS-STAGE112-REVIEW-GATE"
...
P3_CURRENT_GATE = "IDS-STAGE112-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE112-P4-GATE"
P2_CONTROL_PREFIX = ":control:stage112-p2:"
DELIVERY_PREFIX = ":control:stage112-p4:"
...
P3_PHASE2_INPUT_FIELD_COUNT = 34
P3_PHASE2_PHASE1_REFERENCE_FIELD_COUNT = 32
P3_PHASE2_PROJECTION_GROUP_COUNT = 4
P3_PHASE2_PROJECTION_FIELD_COUNT_PER_REQUEST = 100
P3_PHASE2_PROJECTION_FIELD_COUNT_TOTAL = 500
P3_SCENARIO_FIELD_CHECK_COUNT = 265
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage112_phase4_runtime_executed",
)
ZERO_COUNTER_FIELDS = (
...
FAILURE_STATES = (
    ...
    "REPORT_EXPORT_AUDIT_IDENTITY_CONTROL_MISSING",
    "REPORT_SNAPSHOT_CONTROL_MISSING",
    "QUALITY_SCORE_BOUNDARY_MISSING",
    "REPORT_STATUS_OR_EXPORT_AUDIT_CONTROL_MISSING",
...
OPERATOR_FEEDBACK = (
    ...
    "资料撤回、证据降级和索引版本变化保持未来报告状态、质量、快照和导出审计复核要求。",
...
def _phase3_shape_is_preserved(phase3_module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and all(
            ...
            and _is_control_reference(handling.get("report_id_ref"))
...
def _scenario_references_are_opaque(scenario: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and (evidence_id is None or _is_control_reference(evidence_id))
...
def _report_sample_record(scenario: Mapping[str, Any], index: int) -> dict[str, Any]:
    return {
        ...
        "report_export_audit_record_ref": scenario["report_export_audit_record_ref"],
        "actor_ref": scenario["actor_ref"],
        "export_time_ref": scenario["export_time_ref"],
        "report_id_ref": scenario["report_id_ref"],
        "evidence_snapshot_ref": scenario["evidence_snapshot_ref"],
...
def _report_impact_analysis_record(
    ...
    return {
        ...
        "report_status_impact_ref": scenario["report_status_impact_ref"],
        "report_export_audit_record_ref": scenario["report_export_audit_record_ref"],
        "report_export_audit_state_ref": scenario["report_export_audit_state_ref"],
    ...
    return [
        {
            ...
            "rollback_target_ref": _delivery_ref(
                "stage112-p3-controlled-scenarios"
            ),
            "rollback_target_result": P3_PASS_RESULT,
            "predecessor_phase_ref": _delivery_ref("stage112-p3"),
            "report_export_audit_record_ref": _delivery_ref(
                "report-export-audit-record"
            ),
...
def _validate_delivery_groups(report: Mapping[str, Any]) -> Optional[str]:
    ...
    for record in report["report_sample_control_records"]:
        ...
        if (
            ...
            or record["actual_report_sample_rendered"] is not False
        ):
    ...
    expected_impact_states = {
        "source_withdrawal_report_status_impact_state": {
            ...
            "CONTROL_SOURCE_WITHDRAWAL_NOT_APPLICABLE",
        },
        "evidence_downgrade_report_status_impact_state": {
            "CONTROL_EVIDENCE_DOWNGRADE_FUTURE_REPORT_STATUS_QUALITY_AND_AUDIT_REVIEW_REQUIRED",
            "CONTROL_EVIDENCE_DOWNGRADE_NOT_APPLICABLE",
        },
        "index_version_change_report_status_impact_state": {
    ...
    for record in report["report_impact_analysis_control_records"]:
        if (
            ...
            return "REPORT_STATUS_OR_EXPORT_AUDIT_CONTROL_MISSING"
    ...
    if any(
        any(
            (
                ...
                not _is_delivery_reference(record["report_export_audit_record_ref"]),
...
def build_report_export_audit_phase4_delivery_report(
    ...
    try:
        ...
        canonical_phase3_report = (
            phase3_module.build_report_export_audit_phase3_report()
        )
        executor = (
            phase3_executor
