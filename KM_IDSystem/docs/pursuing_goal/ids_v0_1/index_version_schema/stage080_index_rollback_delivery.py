"""Stage080 P4 的纯内存索引回滚交付证据。

模块只复用 Stage080 P2 的五组固定、非业务控制投影及 P3 的六条专项场景，
...
RECORD_KIND = "INDEX_ROLLBACK_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_INDEX_ROLLBACK_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_INDEX_ROLLBACK_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE080-P4-GATE"
...
def build_index_rollback_phase4_delivery_report(
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_index_rollback_phase3_report()


def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_phase2_module()
    return module.execute_index_rollback_control_slice(module.build_control_input())
...
def _load_phase3_module() -> Any:
    return _load_module(
        "stage080_phase3_scenarios", "stage080_index_rollback_scenarios.py"
    )


def _load_phase2_module() -> Any:
    return _load_module(
        "stage080_phase2_slice", "stage080_index_rollback_control_slice.py"
    )
    ...
    return [
        {
            ...
            "index_manifest_ref": f"index-manifest:control:stage080-p2:{scenario}",
    ...
    return [
        {
            ...
            "smoke_test_log_ref": (
                "smoke-test-log:control:stage080-p2:"
...
def _rollback_proof_samples(report: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "control_scenario": scenario,
            "rollback_proof_ref": f"rollback-proof:control:stage080-p2:{scenario}",
...
def _old_index_retention_projection() -> dict[str, Any]:
    return {
        "retention_policy_ref": "index-retention-policy:control:stage080-p2:previous-active",
...
def _operational_instruction_projections() -> list[dict[str, Any]]:
    marker = ":control:stage080-p2:operations"
    return [
        {
            "instruction_id": "INDEX_ROLLBACK_REBUILD_CONTROL_INSTRUCTION",
            "action": "REBUILD",
        ...
        {
            "instruction_id": "INDEX_ROLLBACK_PAUSE_CONTROL_INSTRUCTION",
            "action": "PAUSE",
        ...
        {
            "instruction_id": "INDEX_ROLLBACK_RECOVERY_CONTROL_INSTRUCTION",
            "action": "RECOVERY",
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    ...
    return bool(reference_values) and all(
        ":control:stage080-p2:" in str(value) for value in reference_values
    )
