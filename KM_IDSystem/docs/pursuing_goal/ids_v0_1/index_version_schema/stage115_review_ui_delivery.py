"""Stage115 P4 复核 UI metadata-only 交付控制证据。

模块只从 Stage115 P3 固定、非业务、reference-only 专项场景派生复核队列
...
from KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema import (
    stage115_review_ui_control_slice as phase2,
)
from KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema import (
...
SCHEMA_VERSION = "ids.stage115.review_ui.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REVIEW_UI_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_REVIEW_UI_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEW_UI_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE115-P4-GATE"
NEXT_GATE = "IDS-STAGE115-REVIEW-GATE"
...
P2_CONTROL_PREFIX = phase2.CONTROL_PREFIX
P3_CONTROL_PREFIX = phase3.CONTROL_PREFIX
DELIVERY_PREFIX = ":control:stage115-p4:"

P3_CONTROLLED_SCENARIO_IDS = (
...
P3_PHASE2_INPUT_FIELD_COUNT = 23
P3_PHASE2_PHASE1_REFERENCE_FIELD_COUNT = 19
P3_PHASE2_PROJECTION_GROUP_COUNT = 4
P3_PHASE2_PROJECTION_FIELD_COUNT_PER_REQUEST = 117
P3_PHASE2_PROJECTION_FIELD_COUNT_TOTAL = 585
P3_SCENARIO_FIELD_COUNT = 47
P3_SCENARIO_FIELD_CHECK_COUNT = 235
...
BUSINESS_LINE_WHITEBOX_CONFIRMATION_FIELDS = (
    ...
    "actual_final_business_conclusion_published",
...
ROLLBACK_AND_RE_REVIEW_INSTRUCTION_FIELDS = (
    ...
    "actual_review_ui_rollback_performed",
...
DELIVERY_FIELD_CHECK_COUNT = sum(
    len(fields) * (2 if name == "rollback_and_re_review_instruction_control_records" else 5)
    for name, fields in DELIVERY_GROUPS
)

RUNTIME_CLOSED_FIELDS = (
    *phase3.RUNTIME_CLOSED_FIELDS,
    ...
    "review_ui_rollback_performed",
    "re_review_performed",
    "stage115_phase4_runtime_executed",
)
ZERO_COUNTER_FIELDS = (
    *phase3.ZERO_COUNTER_FIELDS,
...
FAILURE_STATES = (
    ...
    "REVIEW_UI_ROLLBACK_AND_RE_REVIEW_CONTROL_MISSING",
    "AUTOMATIC_DELIVERY_OR_AUDIT_BOUNDARY_BREACH",
    "ACTUAL_REVIEW_UI_OR_AUDIT_WRITE_SIGNAL_DETECTED",
...
OPERATOR_FEEDBACK = (
    ...
    "回滚与重新复核说明固定指向可验证的 P3 回退目标，未执行复核 UI 回退、重新复核或归档动作。",
)
...
def _predecessor_reference(value: object) -> bool:
    return (
...
def _delivery_reference(value: object) -> bool:
    return (
        isinstance(value, str)
...
def _phase3_shape_is_preserved(report: Mapping[str, Any]) -> bool:
    expected_projection_field_total = sum(
        len(fields) for _group, fields in phase2.PROJECTION_FIELDS
    )
    ...
    if not all(
        (
            ...
            expected_projection_field_total
            ...
            report.get("controlled_scenario_count") == len(P3_CONTROLLED_SCENARIO_IDS),
            report.get("controlled_scenario_field_count") == P3_SCENARIO_FIELD_COUNT,
    ...
    return (
        ...
        and all(
            ...
            and set(scenario) == set(phase3.SCENARIO_FIELDS)
            for scenario in scenarios
        )
        and tuple(
        ...
        and all(
            ...
            and isinstance(
                handling.get("business_line_whitebox_handling_code"), str
            )
...
def _phase3_runtime_is_closed(report: Mapping[str, Any]) -> bool:
    boundary = report.get("runtime_boundary")
    return (
        isinstance(boundary, Mapping)
        and tuple(boundary) == tuple(phase3.RUNTIME_CLOSED_FIELDS)
...
def _review_queue_sample_record(
    ...
    return {
        ...
        "review_queue_entry_ref": scenario["review_queue_entry_ref"],
        ...
        "scenario_trigger_control_label": scenario["scenario_trigger_control_label"],
        "scenario_route_control_label": scenario["scenario_route_control_label"],
        "scenario_status_control_label": scenario["scenario_status_control_label"],
        "scenario_action_control_label": scenario["scenario_action_control_label"],
        "evidence_risk_before_control_ref": scenario[
            "evidence_risk_before_control_ref"
        ],
        "evidence_risk_after_control_ref": scenario[
            "evidence_risk_after_control_ref"
        ],
        "fixed_review_status_control_value": scenario[
            "fixed_review_status_control_value"
        ],
        "fixed_review_action_control_value": scenario[
            "fixed_review_action_control_value"
        ],
        "review_queue_sample_state": (
...
def _review_audit_log_sample_record(
    ...
    return {
        ...
        "controlled_scenario_id": scenario["controlled_scenario_id"],
        "review_audit_control_ref": scenario["review_audit_control_ref"],
        "actor_control_ref": scenario["actor_control_ref"],
        "time_control_ref": scenario["time_control_ref"],
        "reason_control_ref": scenario["reason_control_ref"],
        "old_value_control_ref": scenario["old_value_control_ref"],
        "new_value_control_ref": scenario["new_value_control_ref"],
        "review_result_control_ref": scenario["review_result_control_ref"],
        "re_review_control_ref": scenario["re_review_control_ref"],
        "archive_control_ref": scenario["archive_control_ref"],
        "review_audit_sample_state": (
...
def _business_line_whitebox_confirmation_record(
    ...
    return {
        ...
        "external_augmentation_and_whitebox_control_ref": scenario[
            "external_augmentation_and_whitebox_control_ref"
        ],
        "external_augmentation_source_separation_state": (
        ...
        "actual_final_business_conclusion_published": False,
...
def _rollback_and_re_review_instruction_records() -> list[dict[str, Any]]:
    return [
        {
            "instruction_id": _delivery_ref("review-ui-rollback-instruction"),
            "control_domain": "REVIEW_UI_ROLLBACK",
            "trigger_state_ref": _delivery_ref("review-ui-rollback-trigger"),
            "rollback_target_ref": _delivery_ref("stage115-p3-controlled-scenarios"),
            "rollback_target_result": P3_PASS_RESULT,
            "predecessor_phase_ref": _delivery_ref("stage115-p3"),
            "review_ui_entry_control_ref": _delivery_ref("review-ui-entry"),
            "review_audit_control_ref": _delivery_ref("review-audit-record"),
            "business_line_whitebox_confirmation_required": True,
            "human_confirmation_required": True,
            "versioned_basis_required": True,
            "verifiable_rollback_target_required": True,
            "actual_review_ui_rollback_performed": False,
...
def _delivery_actual_signals_are_closed(record: Mapping[str, Any]) -> bool:
    return all(
        value is False
...
def _validate_delivery_groups(report: Mapping[str, Any]) -> Optional[str]:
    expected_counts = {
        "review_queue_sample_control_records": len(P3_CONTROLLED_SCENARIO_IDS),
        "review_audit_log_sample_control_records": len(P3_CONTROLLED_SCENARIO_IDS),
        "review_ui_flow_explanation_control_records": len(P3_CONTROLLED_SCENARIO_IDS),
        "human_judgment_boundary_control_records": len(P3_CONTROLLED_SCENARIO_IDS),
        "business_line_whitebox_confirmation_control_records": len(
            P3_CONTROLLED_SCENARIO_IDS
        ),
        "rollback_and_re_review_instruction_control_records": 2,
    }
    for name, fields in DELIVERY_GROUPS:
        if not _records_match_shape(report, name, fields, expected_counts[name]):
            return "DELIVERY_RECORD_SHAPE_MISMATCH"

    for name, _fields in DELIVERY_GROUPS[:-1]:
        records = report[name]
        if tuple(record["controlled_scenario_id"] for record in records) != (
            P3_CONTROLLED_SCENARIO_IDS
        ):
            return "CONTROLLED_SCENARIO_BINDING_INVALID"
        for record in records:
            if not _delivery_reference(record["delivery_record_id"]):
                return "DELIVERY_REFERENCE_NOT_OPAQUE"
            if not _delivery_actual_signals_are_closed(record):
                return "ACTUAL_REVIEW_UI_OR_AUDIT_WRITE_SIGNAL_DETECTED"

    for record in report["review_queue_sample_control_records"]:
        if (
            not all(
                _predecessor_reference(record[field])
                for field in (
                    "review_queue_entry_ref",
                    "review_trigger_type_ref",
                    "review_status_ref",
    ...
    for record in report["review_ui_flow_explanation_control_records"]:
        if (
            not _predecessor_reference(record["review_queue_entry_ref"])
            or not _predecessor_reference(record["scenario_status_control_label"])
            or not isinstance(record["review_reason_chinese_control_message"], str)
            or not record["review_reason_chinese_control_message"]
            or not _delivery_reference(record["review_ui_flow_step_ref"])
            or not _delivery_reference(record["review_ui_entry_control_ref"])
    ...
    for record in report["business_line_whitebox_confirmation_control_records"]:
        if (
            not _predecessor_reference(
                record["business_line_whitebox_confirmation_gate_ref"]
            )
            or not _predecessor_reference(
                record["external_augmentation_and_whitebox_control_ref"]
            )
    ...
    for record in report["rollback_and_re_review_instruction_control_records"]:
        if (
            not _delivery_reference(record["instruction_id"])
            or record["control_domain"] not in {"REVIEW_UI_ROLLBACK", "RE_REVIEW"}
            or not _delivery_reference(record["trigger_state_ref"])
            or not _delivery_reference(record["rollback_target_ref"])
            or record["rollback_target_result"] != P3_PASS_RESULT
            or not _delivery_reference(record["predecessor_phase_ref"])
            or not _delivery_reference(record["review_ui_entry_control_ref"])
            or not _delivery_reference(record["review_audit_control_ref"])
            or record["business_line_whitebox_confirmation_required"] is not True
            ...
            or record["actual_review_ui_rollback_performed"] is not False
            or record["actual_re_review_performed"] is not False
        ):
            return "REVIEW_UI_ROLLBACK_AND_RE_REVIEW_CONTROL_MISSING"
...
def build_review_ui_phase4_delivery_report(
    phase3_executor: Optional[Phase3Executor] = None,
) -> dict[str, Any]:
    """从固定 P3 场景派生纯内存 P4 metadata-only 交付控制记录。"""

    try:
        canonical_phase3_report = phase3.project_review_ui_controlled_scenarios(
    ...
    if not _phase3_runtime_is_closed(phase3_report):
        return _base_report(False, "PHASE3_RUNTIME_BOUNDARY_BREACH")
    if phase3_report != canonical_phase3_report:
        return _base_report(False, "PHASE3_CONTROL_SHAPE_MISMATCH")
    ...
    if tuple(handlings_by_scenario) != P3_CONTROLLED_SCENARIO_IDS:
