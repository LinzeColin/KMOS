"""Stage079 的纯内存整阶段机械复审，不读取真实资料或启动 Stage080。"""
...
P1_CONTRACT = BASE / "stage079_atomic_index_switch_contract.json"
P2_CONTRACT = BASE / "stage079_atomic_index_switch_slice_contract.json"
P3_CONTRACT = BASE / "stage079_atomic_index_switch_scenarios_contract.json"
P4_CONTRACT = BASE / "stage079_atomic_index_switch_delivery_contract.json"

SCHEMA_VERSION = "ids.stage079.atomic_index_switch.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE079-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-079"
PASS_RESULT = "PASS_REVIEWED_ATOMIC_INDEX_SWITCH_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_ATOMIC_INDEX_SWITCH_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE079-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE080-P1-GATE"
RETURN_STATE = "PASS_ATOMIC_INDEX_SWITCH_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
...
def build_atomic_index_switch_stage079_review_report(
    ...
    """机械复审 Stage079 P1--P4，仅输出零运行时结论及后续门禁。"""

    try:
        phase2_module = _load_module("stage079_atomic_index_switch_control_slice.py")
        phase3_module = _load_module("stage079_atomic_index_switch_scenarios.py")
        phase4_module = _load_module("stage079_atomic_index_switch_delivery.py")
    except Exception:
    ...
    phase2_report = _provider_value(
        ...
        or (
            lambda: phase2_module.execute_atomic_index_switch_control_slice(
                phase2_module.build_control_input()
            )
        )
    )
    phase3_report = _provider_value(
        phase3_report_provider or phase3_module.build_atomic_index_switch_phase3_report
    )
    phase4_report = _provider_value(
        phase4_report_provider
        or phase4_module.build_atomic_index_switch_phase4_delivery_report
    )
    ...
    invariants = {
        ...
        "stage080_gate_only_opens_after_review": _future_stage_boundary(
            phase1, phase2, phase3, phase4, phase3_report, phase4_report
        ),
...
def _review_report(
    ...
    return {
        ...
        "source_authority": "FROZEN_STAGE079_TASKPACK_AND_STAGE079_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        ...
        "stage078_review_evidence_read": True,
        ...
        **_runtime_closed_flags(),
        "rollback": {
            ...
            "scope": "STAGE079_REVIEW_ARTIFACTS_AND_LOCAL_GOVERNANCE_ONLY",
...
def _failed_report() -> dict[str, Any]:
    return _review_report(
        ...
        {},
        {
            ...
            "stage080_gate_only_opens_after_review": False,
...
def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    switch = _mapping(contract.get("atomic_switch_and_rollback_contract"))
    ...
    return (
        _contract_common_valid(
            contract,
            phase="IDS-STAGE079-P1",
            task_id="IDS-V0_1-STAGE079-P1",
            schema_version="ids.stage079.atomic_index_switch.phase1.v1",
            contract_state="PHASE1_ATOMIC_INDEX_SWITCH_CONTRACT_RUNTIME_DISABLED",
        ...
        and _expected(
            versions,
            {
                ...
                "reuses_stage078_control_field_shapes_only": True,
                "all_values_are_control_labels_only": True,
                "active_pointer_read_allowed_in_phase1": False,
                "active_pointer_write_allowed_in_phase1": False,
        ...
        and _expected(
            building,
            {
                ...
                "old_active_index_must_continue_serving_during_build_smoke_and_switch": True,
        ...
        and _expected(
            switch,
            {
                ...
                "switch_record_write_allowed_in_phase1": False,
                "rollback_proof_write_allowed_in_phase1": False,
                "active_pointer_switch_allowed_in_phase1": False,
                "automatic_rollback_execution_allowed": False,
                "actual_atomic_switch_performed": False,
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="IDS-STAGE079-P2",
            task_id="IDS-V0_1-STAGE079-P2",
            schema_version="ids.stage079.atomic_index_switch.phase2.v1",
            contract_state="PHASE2_ATOMIC_INDEX_SWITCH_CONTROL_SLICE_RUNTIME_DISABLED",
            next_gate="IDS-STAGE079-P3-GATE",
        )
        and contract.get("slice_executable") is True
        and contract.get("execution_ready") == "CONTROL_ONLY_IN_MEMORY_ATOMIC_INDEX_SWITCH_SLICE"
        and _expected(
            reuse,
            {
                "index_version_field_count": 7,
                ...
                "all_values_are_control_labels_only": True,
                "phase1_static_contract_remains_authoritative": True,
            },
        )
        and _expected(
            inputs,
            {
                ...
                "chunk_count_must_equal_zero": True,
                "real_business_input_allowed": False,
                "actual_input_request_count": 0,
            },
        )
        and _expected(
            projections,
            {
                ...
                "candidate_build_field_count": 5,
                "active_pointer_field_count": 5,
                "smoke_test_field_count": 7,
                "switch_projection_field_count": 9,
                "rollback_projection_field_count": 8,
                "candidate_must_not_overwrite_active_version": True,
                "shadow_must_remain_isolated": True,
                "old_active_index_must_continue_serving": True,
                "passed_smoke_test_only_creates_unapplied_future_switch_candidate": True,
                "failed_or_missing_smoke_test_blocks_switch": True,
                "switch_failure_keeps_active_pointer_unchanged": True,
                "rollback_target_must_be_retained_previous_active_version": True,
                "actual_active_pointer_write_allowed": False,
                "actual_switch_or_rollback_allowed": False,
            },
        )
        and _expected(
            failures,
        ...
        and _expected(
            local_code,
            {
                "pure_memory_control_slice_module_created": True,
                "runtime_module_created": False,
                "database_schema_created": False,
                "background_worker_created": False,
                "index_artifact_created": False,
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="IDS-STAGE079-P3",
            task_id="IDS-V0_1-STAGE079-P3",
            schema_version="ids.stage079.atomic_index_switch.phase3.v1",
            contract_state="PHASE3_ATOMIC_INDEX_SWITCH_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        _contract_common_valid(
            contract,
            phase="P4",
            task_id="IDS-V0_1-STAGE079-P4",
            schema_version="ids.stage079.atomic_index_switch.phase4.delivery.v1",
            contract_state="PHASE4_ATOMIC_INDEX_SWITCH_DELIVERY_EVIDENCE_RUNTIME_DISABLED",
...
def _contract_common_valid(
    ...
    *,
    ...
    return (
        ...
        and boundary.get("stage078_review_evidence_declared") is True
        and boundary.get("whole_stage_review_performed") is False
        and boundary.get("stage080_started") is False
...
def _phase2_report_valid(report: Mapping[str, Any], module: Any) -> bool:
    ...
    return (
        report.get("schema_version") == "ids.stage079.atomic_index_switch.phase2.v1"
...
def _phase3_report_valid(report: Mapping[str, Any], module: Any) -> bool:
    ...
    return (
        report.get("schema_version") == "ids.stage079.atomic_index_switch.phase3.v1"
        and report.get("valid") is True
        and report.get("result") == P3_PASS_RESULT
        and report.get("next_gate") == "IDS-STAGE079-P4-GATE"
        and _expected(
            report,
            {
                ...
                "phase2_control_record_field_check_count": 205,
                "scenario_count": 6,
                "scenario_field_count": 26,
                "scenario_field_check_count": 156,
...
def _phase4_report_valid(report: Mapping[str, Any], module: Any) -> bool:
    return (
        report.get("schema_version") == "ids.stage079.atomic_index_switch.phase4.delivery.v1"
        and report.get("valid") is True
...
def _controlled_replay(
    ...
    building = _mapping(phase1.get("building_version_shadow_and_smoke_contract"))
    switch = _mapping(phase1.get("atomic_switch_and_rollback_contract"))
    ...
    return {
        ...
        "phase1_building_version_field_count": _as_int(
            building.get("building_and_shadow_field_count")
        ),
        "phase1_smoke_input_field_count": _as_int(
            building.get("smoke_test_input_field_count")
        ),
        "phase1_smoke_output_field_count": _as_int(
            building.get("smoke_test_output_field_count")
        ),
        "phase1_switch_record_field_count": _as_int(
            switch.get("switch_record_field_count")
        ),
        "phase1_rollback_proof_field_count": _as_int(
            switch.get("rollback_proof_field_count")
        ),
...
def _failure_stop_and_rollback_boundary(
    ...
    p1_switch = _mapping(phase1.get("atomic_switch_and_rollback_contract"))
...
def _future_stage_boundary(
    ...
    return (
        ...
        and all(
            _mapping(contract.get("stage_and_phase_boundary")).get("stage080_started")
            is False
            for contract in (phase1, phase2, phase3, phase4)
        )
        and phase4.get("next_gate") == REVIEW_GATE
        and phase3_report.get("stage080_started") is False
        and phase4_report.get("stage080_started") is False
        and phase3_report.get("next_gate") == "IDS-STAGE079-P4-GATE"
...
def _authority_closed(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and authority["authority"].startswith("FROZEN_STAGE079_TASKPACK")
        and _expected(
            authority,
            {
                "second_authoritative_source_created": False,
...
def _chinese_feedback(review_valid: bool) -> list[str]:
    if review_valid:
        return [
            "Stage079 索引原子切换控制合同已机械复审，未执行实际索引操作。",
            "候选隔离、失败保持旧活动版本与回退保留边界保持不变。",
            "交付证据仅为内存控制投影，业务线白箱人工复核仍是唯一权威。",
            "仅开放 Stage080 P1 门，未启动后续阶段、生产或上传。",
        ]
    return [
        "Stage079 整阶段复审未通过，保持在 Review 门。",
