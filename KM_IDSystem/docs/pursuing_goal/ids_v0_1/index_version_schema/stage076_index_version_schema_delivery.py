"""Stage076 P4 的纯内存索引版本交付证据。

本模块只复用已验证的 P2 五组索引版本控制记录与 P3 六条专项场景，派生
控制版索引清单、冒烟记录、切换／回退证明、保留／空间影响投影和操作说明。
所有结果仅存在于当前 Python 进程，不能替代来源文档、真实 manifest、日志、
审计或业务事实，也不会写入索引、调用模型或启动运行时。
"""
...
RECORD_KIND = "INDEX_VERSION_SCHEMA_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_INDEX_VERSION_SCHEMA_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_INDEX_VERSION_SCHEMA_DELIVERY_EVIDENCE"
ENTRY_GATE = "IDS-STAGE076-P4-GATE"
NEXT_GATE = "IDS-STAGE076-REVIEW-GATE"
...
def build_index_version_schema_phase4_delivery_report(
    ...
    return {
        ...
        "all_delivery_references_control_only": all(
            ...
            for item in group
        ) and _record_references_are_control_only(retention) and all(
            _record_references_are_control_only(item) for item in instructions
        ),
        ...
        **runtime_flags,
        "chinese_feedback": [
            "索引清单、冒烟记录、切换记录和回退证明仅为内存控制投影，未写入实际索引或日志。",
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_index_version_schema_phase3_report()


def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_phase2_module()
    return module.execute_index_version_schema_control_slice(module.build_control_input())
...
def _load_phase3_module() -> Any:
    return _load_module("stage076_phase3_scenarios", "stage076_index_version_schema_scenarios.py")


def _load_phase2_module() -> Any:
    return _load_module("stage076_phase2_slice", "stage076_index_version_schema_slice.py")
...
def _phase2_report_is_valid(module: Any, report: Mapping[str, Any]) -> bool:
    groups = (
        ...
        ("building_version_control_records", module.BUILDING_VERSION_FIELDS),
        ("active_pointer_control_projections", module.ACTIVE_POINTER_FIELDS),
        ("verification_control_projections", module.VERIFICATION_FIELDS),
    ...
    return (
        ...
        and report.get("all_active_versions_continue_serving_during_control_build") is True
        and report.get("all_rollback_targets_reference_retained_previous_active") is True
        and report.get("control_output_is_not_actual_index_database_or_retrieval") is True
        and all(
    ...
    return [
        {
            ...
            "index_manifest_ref": f"index-manifest:control:stage076-p2:{scenario}",
    ...
    return [
        {
            ...
            "smoke_test_log_ref": (
                f"smoke-test-log:control:stage076-p2:{scenario['phase2_control_scenario']}"
                f":{scenario['scenario_id']}"
            ),
            "index_version_ref": scenario["referenced_index_version_ref"],
            "verification_state": scenario["observed_verification_state"],
    ...
    return [
        {
            ...
            "resulting_active_index_version_ref": record["resulting_active_index_version_ref"],
            "switch_outcome": record["switch_outcome"],
            "active_service_continues": record["active_service_continues"],
    ...
    return [
        {
            "control_scenario": record["control_scenario"],
            "rollback_proof_ref": (
                f"rollback-proof:control:stage076-p2:{record['control_scenario']}"
            ),
            "rollback_record_ref": record["rollback_record_ref"],
            "index_kind": record["index_kind"],
            "rollback_target_index_version_ref": record["rollback_target_index_version_ref"],
...
def _old_index_retention_projection() -> dict[str, Any]:
    return {
        "retention_policy_ref": "index-retention-policy:control:stage076-p2:previous-active",
...
def _operational_instruction_projections() -> list[dict[str, Any]]:
    marker = ":control:stage076-p2:operations"
    return [
        {
            "instruction_id": "INDEX_REBUILD_CONTROL_INSTRUCTION",
            "action": "REBUILD",
            "target_ref": f"index-rebuild{marker}",
            "entry_precondition": "AUTHORIZED_INPUT_AND_NEW_CANDIDATE_REQUIRED",
            "active_service_requirement": "OLD_ACTIVE_VERSION_CONTINUES_UNTIL_VERIFIED_SWITCH",
            "control_outcome": "CONTROL_REBUILD_NOT_STARTED",
            "actual_operation_performed": False,
            "human_handling_required": True,
        },
        {
            "instruction_id": "INDEX_PAUSE_CONTROL_INSTRUCTION",
            "action": "PAUSE",
        ...
        {
            "instruction_id": "INDEX_RECOVERY_CONTROL_INSTRUCTION",
            "action": "RECOVERY",
...
def _retention_is_control_only(record: Mapping[str, Any]) -> bool:
    return (
        ...
        and record.get("retention_window_state") == "CONTROL_PREVIOUS_ACTIVE_RETAINED"
        and record.get("space_impact_state") == "CONTROL_SPACE_IMPACT_NOT_MEASURED_RUNTIME_DISABLED"
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    ...
    return bool(reference_values) and all(
        ":control:stage076-p2:" in str(value) for value in reference_values
    )
