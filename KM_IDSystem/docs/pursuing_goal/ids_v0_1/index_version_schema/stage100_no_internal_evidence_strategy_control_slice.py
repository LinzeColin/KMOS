"""Stage100 P2 的纯内存无内部依据策略受控切片。

模块只投影自身定义的固定、非业务、reference-only 控制输入。它承接
Stage100 P1 静态合同与 Stage099 Review 已复审内部依据与外部增强分离控制
工件，不读取业务资料、检索结果、提示词、模型配置或 evidence，不连接数据库，
...
SCHEMA_VERSION = "ids.stage100.no_internal_evidence_strategy.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_NO_INTERNAL_EVIDENCE_STRATEGY"
CONTROL_ADAPTER_VERSION = (
    "ids.no_internal_evidence_strategy.control_adapter.v0_1.stage100.p2"
)
CONTROL_PREFIX = ":control:stage100-p2:"
CONTROL_FIELDS = ("no_internal_evidence_strategy_control_requests",)
PHASE1_NO_INTERNAL_EVIDENCE_STRATEGY_CONTROL_REF = (
    ":control:stage100-p2:stage100-phase1-no-internal-evidence-strategy-contract:reference-only"
)
STAGE099_REVIEW_INTERNAL_EVIDENCE_EXTERNAL_AUGMENTATION_CONTROL_REF = (
    ":control:stage100-p2:stage099-reviewed-internal-evidence-external-augmentation-control:reference-only"
)

ANSWER_CONTRACT_AND_POLICY_BINDING_FIELDS = (
    ...
    "internal_evidence_insufficiency_state",
...
PROJECTION_FIELDS = (
    ("answer_contract_and_policy_binding", ANSWER_CONTRACT_AND_POLICY_BINDING_FIELDS),
    (
...
def _control_request(scenario: str) -> dict[str, Optional[str]]:
    ...
    return {
        ...
        "no_internal_evidence_policy_ref": _control_ref(
            "no-internal-evidence-policy", scenario
        ),
        ...
        "internal_evidence_insufficiency_state": configuration[
            "internal_evidence_insufficiency_state"
        ],
    }
...
def _zero_actual_counts() -> dict[str, int]:
    return {
        ...
        "actual_no_internal_evidence_policy_evaluation_count": 0,
...
def _project(
    ...
    answer_contract_and_policy_binding = {
        "stage100_phase1_no_internal_evidence_strategy_contract_ref": (
            PHASE1_NO_INTERNAL_EVIDENCE_STRATEGY_CONTROL_REF
        ),
        "stage099_review_internal_evidence_external_augmentation_control_ref": (
        ...
        "no_internal_evidence_policy_ref": request["no_internal_evidence_policy_ref"],
        "internal_evidence_insufficiency_state": request[
            "internal_evidence_insufficiency_state"
        ],
    ...
    return {
        "answer_contract_and_policy_binding": answer_contract_and_policy_binding,
        "query_index_version_and_selected_evidence_record": (
            query_index_version_and_selected_evidence_record
        ),
        "source_type_and_external_augmentation_opinion_display": (
...
def execute_no_internal_evidence_strategy_control_slice(
