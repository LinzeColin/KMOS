"""Stage095 P4 的纯内存证据回归交付控制证据。

模块只从 Stage095 P3 已验收的固定、非业务、reference-only 异常场景派生
...
SCHEMA_VERSION = "ids.stage095.evidence_regression.phase4.delivery.v1"
RECORD_KIND = "EVIDENCE_REGRESSION_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_EVIDENCE_REGRESSION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_REGRESSION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE095-P4-GATE"
NEXT_GATE = "IDS-STAGE095-REVIEW-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_REGRESSION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage095-p2:"
DELIVERY_PREFIX = ":control:stage095-p4:"
...
DEGRADATION_INSTRUCTION_DEFINITIONS = (
    {
        "instruction_id": "low_ocr_evidence_regression_degradation_instruction",
...
def build_evidence_regression_phase4_delivery_report(
    phase3_report_provider: Phase3ReportProvider | None = None,
) -> dict[str, Any]:
    """从固定 P3 场景派生未持久化的 Stage095 P4 交付控制记录。"""
    ...
    return {
        ...
        "chinese_feedback": [
            ...
            "Stage095 P4 控制只开放整阶段机械复审门，Review、Stage096、OVH、生产与正式上传保持后续阶段。",
        ],
    }


def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_evidence_regression_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage095_evidence_regression_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage095_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage095 P3 evidence regression scenarios")
...
def _report_status_impact_ref(scenario: Mapping[str, Any]) -> str:
    return _delivery_ref(scenario["scenario_id"], "report-status-impact")
...
def _phase3_reference_is_allowed(
    ...
    return _inherited_control_ref(value) or (
        ...
        and scenario.get("scenario_id") == "no_internal_evidence_regression_control"
        and value is None
    ) or (
        field == "evidence_gap_ref"
        and scenario.get("scenario_id") != "no_internal_evidence_regression_control"
        and value is None
    )
...
def _phase3_semantic_failure(report: Mapping[str, Any]) -> str | None:
    ...
    no_internal = scenarios.get("no_internal_evidence_regression_control", {})
    if (
    ...
    degradation_states = {
        "low_ocr_evidence_regression_degradation_control": (
            "CONTROL_LOW_OCR_DEGRADED_NOT_ACCEPTED"
        ),
        "old_version_evidence_regression_degradation_control": (
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED"
        ),
        "conflict_evidence_regression_degradation_control": (
    ...
    if (
        ...
        or not _inherited_control_ref(revoked.get("report_id_ref"))
    ):
        return "REVOKED_EVIDENCE_REPORT_STATUS_IMPACT_MISSING"
    malicious = scenarios.get("malicious_evidence_regression_quarantine_control", {})
    if (
    ...
    return [
        {
            ...
            or _delivery_ref(scenario["scenario_id"], "evidence-gap-route"),
            "ocr_confidence_indicator_ref": _delivery_ref(
                scenario["scenario_id"], "ocr-confidence-indicator"
            ),
            "version_status_indicator_ref": _delivery_ref(
                scenario["scenario_id"], "version-status-indicator"
            ),
            "conflict_status_indicator_ref": _delivery_ref(
                scenario["scenario_id"], "conflict-status-indicator"
            ),
            "revocation_ref": scenario["revocation_status_ref"],
    ...
    return [
        {
            ...
            "report_status_impact_ref": _report_status_impact_ref(by_id[definition["scenario_id"]]),
...
def _delivery_reference_is_allowed(
    ...
    return _control_ref(value) or (
        field == "evidence_id_ref"
        and record.get("scenario_id") == "no_internal_evidence_regression_control"
        and value is None
    )
