"""Stage084 P2 的纯内存向量检索基线控制切片。

模块只接受五条固定、非业务、reference-only 控制请求，并在内存中投影关键词／
向量基线、五类元数据过滤、候选、混合评分、选择结果、检索轨迹、向量模型版本、
...
SCHEMA_VERSION = "ids.stage084.vector_retrieval_baseline.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_VECTOR_RETRIEVAL_BASELINE"
CONTROL_ADAPTER_VERSION = "ids.vector_retrieval.control_adapter.v0_1.stage084.p2"
CONTROL_FIELDS = ("vector_retrieval_control_requests",)
...
def _marker(scenario: str) -> str:
    return f":control:stage084-p2:{scenario}"
...
def build_control_request(scenario: str) -> dict[str, Any]:
    """构造一条固定、无业务含义的 Stage084 P2 控制请求。"""
...
def _rejected_result() -> dict[str, Any]:
    return {
        ...
        "execution_state": "CONTROL_INPUT_REJECTED_VECTOR_RETRIEVAL_RUNTIME_DISABLED",
...
def execute_vector_retrieval_control_slice(
    control_input: Mapping[str, object] | object,
) -> dict[str, Any]:
    """在内存中投影固定 Stage084 P2 控制记录，并拒绝任何其他输入。"""
    ...
    return {
        ...
        "control_adapter_version": CONTROL_ADAPTER_VERSION,
        "input_accepted": True,
        "execution_state": "COMPLETED_IN_MEMORY_VECTOR_RETRIEVAL_CONTROL_SLICE",
        "failure_state": "NONE",
