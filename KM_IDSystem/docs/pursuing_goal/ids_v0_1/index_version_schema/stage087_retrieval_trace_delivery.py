"""Stage087 P4 的纯内存检索轨迹交付证据。

模块只重放 Stage087 P3 的八个受控场景，派生 metadata-only 的检索样例、
...
SCHEMA_VERSION = "ids.stage087.retrieval_trace.phase4.delivery.v1"
RECORD_KIND = "RETRIEVAL_TRACE_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_RETRIEVAL_TRACE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_RETRIEVAL_TRACE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE087-P4-GATE"
NEXT_GATE = "IDS-STAGE087-REVIEW-GATE"
P3_PASS_RESULT = "PASS_RETRIEVAL_TRACE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage087-p2:"

P2_CONTROL_FIELD_CHECK_COUNT = 426
P3_SCENARIO_COUNT = 8
...
def build_retrieval_trace_phase4_delivery_report(
    phase3_report_provider: Phase3ReportProvider | None = None,
) -> dict[str, Any]:
    """从固定 P3 报告派生未持久化的 Stage087 P4 控制交付记录。"""
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_retrieval_trace_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage087_retrieval_trace_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage087_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage087 P3 retrieval trace scenarios")
    ...
    return [
        {
            ...
            "rollback_target_ref": (
                f"stage087-p3-control-boundary{CONTROL_PREFIX}{scenario_id}"
            ),
