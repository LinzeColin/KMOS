"""Stage089 P3 的纯内存证据账本异常场景控制验证。

模块只重放 Stage089 P2 的六条固定、非业务、reference-only 控制投影。
无内部证据、低 OCR、旧版本、冲突、撤回、恶意资料与低等级伪装都只是
受控场景标签；它不读取业务资料，不创建或更新真实证据账本、报告或数据库。
"""
...
SCHEMA_VERSION = "ids.stage089.evidence_ledger.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_LEDGER_EXCEPTION_SCENARIOS"
PASS_RESULT = "PASS_EVIDENCE_LEDGER_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_LEDGER_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CURRENT_GATE = "IDS-STAGE089-P3-GATE"
NEXT_GATE = "IDS-STAGE089-P4-GATE"
P2_EXECUTION_STATE = "CONTROL_EVIDENCE_LEDGER_PROJECTIONS_DECLARED_NOT_EXECUTED"
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage089_evidence_ledger_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage089_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage089 P2 evidence ledger control slice")
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    if (
        ...
        != ("evidence_ledger_control_requests",)
        or len(getattr(phase2_module, "INPUT_FIELDS", ())) != 24
...
def _phase2_records(
    ...
    for prefix, _field_constant in P2_PROJECTION_SPECS:
        ...
        record = projections[index]
        if not isinstance(record, Mapping):
            raise ValueError(f"invalid Stage089 P2 {prefix} projection")
...
def _phase2_semantic_failure(result: Mapping[str, Any]) -> str | None:
    ...
    if (
        not _control_ref(grade_a["evidence_gap"]["evidence_gap_ref"])
        or grade_a["evidence_gap"].get("gap_state")
        != "CONTROL_GAP_REFERENCE_DECLARED_NOT_EVALUATED"
    ):
    ...
    if (
        ...
        or not _control_ref(expired["evidence_schema"]["source_version_ref"])
    ):
    ...
    if (
        low_grade["degradation"].get("evidence_grade_label") != "D"
        or "evidence-grade-D" not in str(low_grade["evidence_schema"]["evidence_grade_ref"])
    ):
...
def _scenario_expectation_met(
    ...
    schema = records["evidence_schema"]
    relation = records["evidence_relation"]
    gap = records["evidence_gap"]
    ...
    scenario_specific = (
        gap.get("gap_state") == "CONTROL_GAP_REFERENCE_DECLARED_NOT_EVALUATED"
        if scenario_id == "no_internal_evidence_gap_control"
        else True
    )
    scenario_specific = scenario_specific and (
        revocation.get("revocation_state") == "CONTROL_REVOCATION_REFERENCE_NOT_EXECUTED"
    ...
    scenario_specific = scenario_specific and (
        "evidence-grade-D" in str(schema.get("evidence_grade_ref"))
...
def _build_scenario(
    ...
    return {
        ...
        "evidence_gap_ref": gap["evidence_gap_ref"],
        "evidence_grade_ref": schema["evidence_grade_ref"],
        "evidence_grade_label": degradation["evidence_grade_label"],
        "source_version_ref": schema["source_version_ref"],
        ...
        "poisoning_defense_ref": defense["poisoning_defense_ref"],
...
def build_evidence_ledger_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制投影并验证 P3 固定异常场景。"""

    phase2_module = _load_phase2_module()
    executor = phase2_executor or phase2_module.execute_evidence_ledger_control_slice
    try:
    ...
    return {
        ...
        "phase2_field_check_count": phase2_field_check_count,
