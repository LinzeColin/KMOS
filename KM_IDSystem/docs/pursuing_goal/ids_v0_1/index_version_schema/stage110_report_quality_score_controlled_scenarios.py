"""Stage110 P3：报告质量评分的纯内存专项场景验证。

模块只重放 Stage110 P2 的固定、非业务、reference-only 控制投影。它验证关键
...
SCHEMA_VERSION = "ids.stage110.report_quality_score.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_QUALITY_SCORE_SCENARIOS"
CURRENT_GATE = "IDS-STAGE110-P3-GATE"
NEXT_GATE = "IDS-STAGE110-P4-GATE"
PASS_RESULT = "PASS_REPORT_QUALITY_SCORE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REPORT_QUALITY_SCORE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"

P2_SCHEMA_VERSION = "ids.stage110.report_quality_score.phase2.v1"
P2_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_QUALITY_SCORE"
P2_EXECUTION_STATE = "PASS_IN_MEMORY_REPORT_QUALITY_SCORE_CONTROL_SLICE_RUNTIME_DISABLED"
P2_CONTROL_PREFIX = ":control:stage110-p2:"
P2_CONTROL_FIELDS = ("report_quality_score_control_requests",)
P2_CONTROL_REQUEST_COUNT = 5
P2_INPUT_FIELD_COUNT = 42
P2_PHASE1_REFERENCE_FIELD_COUNT = 40
P2_PROJECTION_GROUP_COUNT = 4
P2_PROJECTION_FIELD_COUNT_PER_REQUEST = 126
P2_PROJECTION_FIELD_COUNT_TOTAL = 630
P2_CONTROL_SCENARIOS = (
    ...
    "report_quality_score_and_export_reference_only",
)
P2_PROJECTION_PREFIXES = (
    "report_evidence_binding_and_section",
    "generation_snapshot",
    "report_quality_score_and_lifecycle",
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage110_phase3_runtime_executed",
)
...
ZERO_COUNTER_FIELDS = (
    ...
    "actual_quality_formula_weight_threshold_definition_count",
...
CONTROL_VIEW_FIELDS = {
    ...
    "quality_score_boundary_control_view": (
        "scenario_id",
        "internal_evidence_coverage_rate_ref",
        "citation_completeness_rate_ref",
        "external_augmentation_ratio_ref",
...
SCENARIO_DEFINITIONS = (
    {
        ...
        "quality_whitebox_confirmation_required": False,
    ...
    {
        "scenario_id": "quality_score_export_external_augmentation_whitebox_control",
        "scenario_category": "QUALITY_SCORE_EXPORT_EXTERNAL_AUGMENTATION_WHITEBOX_CONTROL",
        "phase2_control_scenario": P2_CONTROL_SCENARIOS[4],
        "expected_binding_mode": "evidence_id",
        "source_withdrawal_impact_required": False,
        "evidence_downgrade_impact_required": False,
        "index_version_change_impact_required": False,
        "quality_whitebox_confirmation_required": True,
        "human_confirmation_required": True,
        "business_line_whitebox_handling_code": (
            "BUSINESS_LINE_WHITEBOX_REVIEW_QUALITY_SCORE_AND_EXTERNAL_AUGMENTATION"
        ),
    },
)
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage110_report_quality_score_control_slice.py")
    spec = importlib.util.spec_from_file_location(
        "stage110_phase2_report_quality_score_slice", module_path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage110 P2 报告质量评分受控最小切片")
...
def _failure_for_projection(
    ...
    if (
        section.get("report_evidence_binding_control_state")
        != "CONTROL_EVIDENCE_BINDING_REFERENCE_ONLY_NOT_WRITTEN"
        or section.get("report_section_output_control_state")
    ...
    if lifecycle.get("source_withdrawal_control_state") != lifecycle_states[
        "source_withdrawal_control_state"
    ]:
        return "SOURCE_WITHDRAWAL_IMPACT_CONTROL_MISSING"
    if lifecycle.get("evidence_downgrade_control_state") != lifecycle_states[
        "evidence_downgrade_control_state"
    ]:
        return "EVIDENCE_DOWNGRADE_IMPACT_CONTROL_MISSING"
    if lifecycle.get("index_version_change_control_state") != lifecycle_states[
        "index_version_change_control_state"
    ]:
        return "INDEX_VERSION_CHANGE_IMPACT_CONTROL_MISSING"
    ...
    if any(
        ...
        != value
        for field, value in {
            "internal_evidence_coverage_rate_control_state": (
                "CONTROL_INTERNAL_EVIDENCE_COVERAGE_REFERENCE_ONLY_NOT_CALCULATED"
            ),
            "citation_completeness_rate_control_state": (
                "CONTROL_CITATION_COMPLETENESS_REFERENCE_ONLY_NOT_CALCULATED"
            ),
            "external_augmentation_ratio_control_state": (
                "CONTROL_EXTERNAL_AUGMENTATION_RATIO_REFERENCE_ONLY_NOT_CALCULATED"
            ),
            "evidence_gap_count_control_state": (
                "CONTROL_EVIDENCE_GAP_COUNT_REFERENCE_ONLY_NOT_CALCULATED"
            ),
            "quality_metric_definition_control_state": (
                "CONTROL_QUALITY_METRIC_DEFINITION_REFERENCE_ONLY_NOT_RECORDED"
            ),
        }.items()
    ):
        return "QUALITY_METRIC_CONTROL_MISSING"
    if (
        lifecycle.get("quality_formula_weight_threshold_control_state")
        != "CONTROL_QUALITY_FORMULA_WEIGHT_THRESHOLD_REFERENCE_ONLY_NOT_DEFINED"
        or lifecycle.get("report_quality_score_control_state")
        != "CONTROL_REPORT_QUALITY_SCORE_REFERENCE_ONLY_NOT_CALCULATED"
        or lifecycle.get("quality_score_explanation_control_state")
        != "CONTROL_QUALITY_SCORE_EXPLANATION_REFERENCE_ONLY_NOT_PERSISTED"
        or lifecycle.get("report_export_audit_control_state")
        != "CONTROL_REPORT_EXPORT_AUDIT_REFERENCE_ONLY_NOT_WRITTEN"
        or lifecycle.get("quality_formula_weight_threshold_business_line_whitebox_required")
        is not True
    ):
        return "QUALITY_SCORE_CONTROL_MISSING"
    if any(
        lifecycle.get(field) is not False
        for field in (
            "automatic_report_status_update_allowed",
            "automatic_quality_metric_calculation_allowed",
            "automatic_quality_formula_definition_allowed",
            ...
            "actual_report_status_impact_updated",
            "actual_internal_evidence_coverage_rate_calculated",
            "actual_citation_completeness_rate_calculated",
            "actual_external_augmentation_ratio_calculated",
            "actual_evidence_gap_count_calculated",
            "actual_quality_metric_definition_recorded",
            "actual_quality_formula_defined",
            "actual_quality_weight_defined",
            "actual_quality_threshold_defined",
            "actual_report_quality_scored",
            "actual_quality_score_explanation_persisted",
        ...
        return "QUALITY_SCORING_BOUNDARY_BREACH"
...
def _scenario_record(
    ...
    quality_score_boundary_state = (
        ...
        if definition["quality_whitebox_confirmation_required"]
        else "CONTROL_QUALITY_SCORE_REFERENCE_ONLY_NOT_CALCULATED"
    )
    return {
        ...
        "internal_evidence_coverage_rate_ref": request[
            "internal_evidence_coverage_rate_ref"
        ],
        "citation_completeness_rate_ref": request["citation_completeness_rate_ref"],
        "external_augmentation_ratio_ref": request[
            "external_augmentation_ratio_ref"
        ],
        "evidence_gap_count_ref": request["evidence_gap_count_ref"],
        "quality_metric_definition_ref": request["quality_metric_definition_ref"],
        "quality_formula_ref": request["quality_formula_ref"],
        "quality_weight_ref": request["quality_weight_ref"],
        "quality_threshold_ref": request["quality_threshold_ref"],
        "report_quality_score_ref": request["report_quality_score_ref"],
        "quality_score_explanation_ref": request["quality_score_explanation_ref"],
        ...
        "quality_score_boundary_state": quality_score_boundary_state,
        "external_augmentation_source_separation_state": (
            "CONTROL_EXTERNAL_AUGMENTATION_RETAINS_UNDERLYING_SOURCE_TYPE_"
            "SEPARATE_FROM_INTERNAL_EVIDENCE"
        ),
        ...
        "actual_report_quality_scored": lifecycle["actual_report_quality_scored"],
...
def _human_handling(
    ...
    return {
        ...
        "quality_whitebox_confirmation_required": definition[
...
def build_report_quality_score_phase3_report(
    ...
    try:
        ...
        executor = (
            ...
            else phase2_module.execute_report_quality_score_control_slice
        )
    ...
    for index, definition in enumerate(SCENARIO_DEFINITIONS):
        ...
        lifecycle = _projection_record(
            phase2_result, "report_quality_score_and_lifecycle", index
        )
