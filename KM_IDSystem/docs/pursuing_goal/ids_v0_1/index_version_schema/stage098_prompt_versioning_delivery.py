"""Stage098 P4 的纯内存 Prompt 版本化交付证据。

模块只从 P3 的固定、非业务、reference-only 异常场景派生交付控制记录。
它不读取真实资料、提示词、模型配置、证据、回答或日志，不执行检索、RAG 或模型，
不消耗模型 Token，也不连接、写入或修改任何持久化系统。
"""
...
SCHEMA_VERSION = "ids.stage098.prompt_versioning.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_PROMPT_VERSIONING_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_PROMPT_VERSIONING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_PROMPT_VERSIONING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE098-P4-GATE"
NEXT_GATE = "IDS-STAGE098-REVIEW-GATE"
P3_PASS_RESULT = "PASS_PROMPT_VERSIONING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P3_CURRENT_GATE = "IDS-STAGE098-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE098-P4-GATE"
CONTROL_PREFIX = ":control:stage098-p2:"
...
def _load_phase3_module() -> Any:
    module_path = Path(__file__).with_name("stage098_prompt_versioning_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage098_phase3_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage098 P3 controlled scenarios")
...
def _phase3_shape_failure(phase3_module: Any, report: Mapping[str, Any]) -> str | None:
    if (
        ...
        != "ids.stage098.prompt_versioning.phase3.v1"
        ...
        or report.get("schema_version") != "ids.stage098.prompt_versioning.phase3.v1"
    ...
    return [
        {
            ...
            "predecessor_phase_ref": _delivery_ref("stage098-phase3"),
...
def _delivery_records_have_expected_shape(report: Mapping[str, Any]) -> bool:
    expected_counts = {
        "answer_sample_control_records": len(P3_SCENARIO_IDS),
        "negative_test_result_control_records": len(P3_SCENARIO_IDS),
        "prompt_version_control_records": len(P3_SCENARIO_IDS),
        "reproducible_log_control_records": len(P3_SCENARIO_IDS),
        "output_permission_boundary_control_records": len(P3_SCENARIO_IDS),
        "rollback_and_fallback_control_records": 2,
    }
    return all(
        isinstance(report.get(name), list)
        and len(report[name]) == expected_counts[name]
...
def _rollback_and_fallback_is_valid(records: list[Mapping[str, Any]]) -> bool:
    return (
...
def _delivery_runtime_is_closed(report: Mapping[str, Any]) -> bool:
    for group_name, _fields in DELIVERY_GROUPS:
        for record in report[group_name]:
            for field, value in record.items():
                if field.startswith("actual_") and value is not False:
                    return False
                if field in {
                    "automatic_final_conclusion_allowed",
                    "persistent_state_write_performed",
...
def build_prompt_versioning_phase4_delivery_report(
    phase3_executor: Phase3Executor | None = None,
) -> dict[str, Any]:
    """派生 Stage098 P4 交付控制记录，不执行任何业务或运行时动作。"""

    try:
        phase3_module = _load_phase3_module()
        executor = phase3_executor or phase3_module.build_prompt_versioning_phase3_report
    ...
    if not _delivery_records_have_expected_shape(report):
    ...
    if not _rollback_and_fallback_is_valid(report["rollback_and_fallback_control_records"]):
        return _failure_report("PROMPT_ROLLBACK_OR_MODEL_FALLBACK_MISSING")
    if not _delivery_runtime_is_closed(report):
