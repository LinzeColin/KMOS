"""Stage103 模型输出权限门禁的纯内存整阶段机械复审。

模块只复审冻结任务包和 Stage103 P1--P4 已存在的控制工件。它不读取业务资料、
...
SCHEMA_VERSION = "ids.stage103.model_output_permission_gate.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_MODEL_OUTPUT_PERMISSION_GATE_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_MODEL_OUTPUT_PERMISSION_GATE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_MODEL_OUTPUT_PERMISSION_GATE_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE103-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE104-P1-GATE"

P1_SCHEMA_VERSION = "ids.stage103.model_output_permission_gate.phase1.v1"
P1_CONTRACT_STATE = "PHASE1_MODEL_OUTPUT_PERMISSION_GATE_RUNTIME_DISABLED"
...
P2_PASS_RESULT = "PASS_IN_MEMORY_MODEL_OUTPUT_PERMISSION_GATE_CONTROL_SLICE_RUNTIME_DISABLED"
...
P4_SCHEMA_VERSION = "ids.stage103.model_output_permission_gate.phase4.delivery.v1"
P4_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_MODEL_OUTPUT_PERMISSION_GATE_DELIVERY_EVIDENCE"
P4_PASS_RESULT = "PASS_MODEL_OUTPUT_PERMISSION_GATE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
...
REVIEWED_CONTROL_SHAPE = {
    "phase1_static_shape": "13/5/4/5/24/4",
    ...
    "phase2_projection_field_count_per_request": 46,
    "phase2_control_field_check_count": 230,
    "phase2_failure_state_count": 33,
...
P1_CONTRACT_PATH = BASE / "stage103_model_output_permission_gate_contract.json"
P2_MODULE_PATH = BASE / "stage103_model_output_permission_gate_control_slice.py"
P3_MODULE_PATH = BASE / "stage103_model_output_permission_gate_controlled_scenarios.py"
P4_MODULE_PATH = BASE / "stage103_model_output_permission_gate_delivery.py"
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module("stage103_review_phase2", P2_MODULE_PATH)
    return module.execute_model_output_permission_gate_control_slice(
        module.build_control_input()
    )


def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module("stage103_review_phase3", P3_MODULE_PATH)
    return module.build_model_output_permission_gate_phase3_report()


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module("stage103_review_phase4", P4_MODULE_PATH)
    return module.build_model_output_permission_gate_phase4_delivery_report()
...
def _phase1_valid(contract: Mapping[str, Any]) -> bool:
    ...
    gate = _mapping(contract.get("model_output_permission_gate_contract"))
    answer = _mapping(gate.get("answer_structure_contract"))
    ...
    return all(
        (
            ...
            contract.get("phase") == "IDS-STAGE103-P1",
            contract.get("task_id") == "IDS-V0_1-STAGE103-P1",
            contract.get("contract_state") == P1_CONTRACT_STATE,
            contract.get("entry_gate") == "IDS-STAGE103-P1-GATE",
            contract.get("next_gate") == "IDS-STAGE103-P2-GATE",
            ...
            gate.get("future_control_reference_field_count") == 13,
            len(gate.get("future_control_reference_fields", [])) == 13,
            gate.get("control_references_are_labels_only") is True,
            ...
            semantics.get("document_evidence_state")
            ...
            semantics.get("no_internal_evidence_strategy", {}).get(
            ...
            failure.get("failure_state_count") == 24,
            len(failure.get("declared_failure_states", [])) == 24,
...
def _phase2_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            len(getattr(module, "INPUT_FIELDS", ())) == 26,
            len(getattr(module, "PROJECTION_FIELDS", ())) == 4,
            ...
            report.get("control_projection_field_total_per_request") == 46,
            report.get("control_projection_field_total") == 230,
            report.get("persistent_record_created") is False,
            _actual_counts_closed(report),
            _runtime_closed(runtime),
            all(
                isinstance(report.get(f"{prefix}_control_projections"), list)
                and len(report[f"{prefix}_control_projections"]) == 5
                for prefix, _fields in getattr(module, "PROJECTION_FIELDS", ())
            ),
        )
    )
...
def _phase3_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    expected_fields = tuple(getattr(module, "SCENARIO_FIELDS", ()))
    if not all(
        (
            ...
            report.get("failure_state") is None,
            report.get("current_gate") == "IDS-STAGE103-P3-GATE",
            report.get("next_gate") == "IDS-STAGE103-P4-GATE",
            ...
            report.get("phase2_projection_field_count_per_request") == 46,
            report.get("phase2_projection_field_count_total") == 230,
            ...
            _actual_counts_closed(report),
            _runtime_closed(runtime),
            _records_have_shape(scenarios, expected_fields, 5),
        )
    ):
        return False
...
def _phase4_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    if not all(
        (
            ...
            report.get("failure_state") is None,
            report.get("current_gate") == "IDS-STAGE103-P4-GATE",
...
def _base_report(valid: bool, failure_state: str | None) -> dict[str, Any]:
    return {
        ...
        "failure_and_stop_contract": {
            ...
            "stage104_must_remain_not_started": True,
        },
        "stage_and_phase_boundary": {
            "stage102_review_evidence_declared": True,
            "stage103_started": True,
            "phase1_completed": True,
            "phase2_completed": True,
            "phase3_completed": True,
            "phase4_completed": True,
            "stage103_review_started": True,
            "whole_stage_review_completed_in_memory_only": valid,
            "stage104_started": False,
        ...
        "chinese_feedback": [
            "Stage103 P1--P4 控制工件已完成纯内存机械复审，来源文档与业务线白箱人工复核继续承担业务事实权威。",
            "文档内潜在指令保持不可信 evidence，IDS 规则保持优先；内部依据不足保持 evidence_gap，不作为内部经验呈现。",
            "高风险工程建议、合同承诺和生产写回保持业务线白箱人工处理，人工确认未记录，最终结论未发布。",
            "P4 交付证据保持 P4→P3 的版本化、白箱批准和可验证回退目标；Stage104 保持下一独立 run 门禁。",
        ],
...
def build_model_output_permission_gate_stage103_review_report(
    ...
    """机械复审 Stage103 P1--P4；控制工件漂移时保持失败关闭。"""
    ...
    try:
        phase2_module = _load_module("stage103_review_phase2_validation", P2_MODULE_PATH)
    ...
    try:
        phase3_module = _load_module("stage103_review_phase3_validation", P3_MODULE_PATH)
    ...
    try:
        phase4_module = _load_module("stage103_review_phase4_validation", P4_MODULE_PATH)
