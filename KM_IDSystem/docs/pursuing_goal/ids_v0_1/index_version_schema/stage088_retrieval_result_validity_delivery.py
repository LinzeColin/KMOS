"""Stage088 P4 的纯内存检索结果有效性门禁交付证据。

模块只重放 Stage088 P3 的八个受控场景，派生 metadata-only 的检索样例、
...
SCHEMA_VERSION = "ids.stage088.retrieval_result_validity.phase4.delivery.v1"
RECORD_KIND = "RETRIEVAL_RESULT_VALIDITY_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_RETRIEVAL_RESULT_VALIDITY_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_RETRIEVAL_RESULT_VALIDITY_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE088-P4-GATE"
NEXT_GATE = "IDS-STAGE088-REVIEW-GATE"
P3_PASS_RESULT = "PASS_RETRIEVAL_RESULT_VALIDITY_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage088-p2:"

P2_CONTROL_FIELD_CHECK_COUNT = 528
P3_SCENARIO_COUNT = 8
P3_SCENARIO_FIELD_COUNT = 33
P3_SCENARIO_FIELD_CHECK_COUNT = 264
...
def build_retrieval_result_validity_phase4_delivery_report(
    phase3_report_provider: Phase3ReportProvider | None = None,
) -> dict[str, Any]:
    """从固定 P3 报告派生未持久化的 Stage088 P4 控制交付记录。"""
    ...
    return {
        ...
        "phase3_controlled_scenarios_replayed_in_memory_only": phase3_valid,
        ...
        **runtime_flags,
        "chinese_feedback": [
            ...
            "关键词／向量基线、候选、选择、评分、活动索引版本与全部引用只保留不透明控制标签，未执行检索、过滤或排序。",
            "检索不足与证据缺口已逐场景显式保留，仍需业务线白箱人工处理，自动补全和自动采纳保持关闭。",
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_retrieval_result_validity_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage088_retrieval_result_validity_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage088_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage088 P3 retrieval result validity scenarios")
...
def _phase3_report_is_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("result_validity_gate_chain_preserved") is True
        and report.get("six_dimension_filter_combination_preserved") is True
        and report.get("active_index_version_chain_preserved") is True
        and report.get("top_k_ranking_and_validity_preserved") is True
        and report.get("old_index_trace_version_preserved") is True
        and report.get("all_result_validity_states_not_evaluated") is True
        and report.get("all_validity_gates_pending_human_whitebox") is True
        and report.get("all_business_line_handling_required") is True
        ...
        and report.get("stage087_review_evidence_declared") is True
        and report.get("stage088_started") is True
...
def _failure_state(
    ...
    if not phase3_report:
        return "PHASE3_CONTROL_OUTPUT_INVALID"
    if any(
        phase3_report.get(field) is True
        for field in _load_phase3_module().RUNTIME_CLOSED_FIELDS
    ):
        return "PHASE3_RUNTIME_SIGNAL_DETECTED"
    if (
        phase3_report.get("phase2_control_record_field_check_count")
        != P2_CONTROL_FIELD_CHECK_COUNT
        or phase3_report.get("scenario_count") != P3_SCENARIO_COUNT
        or phase3_report.get("scenario_field_count") != P3_SCENARIO_FIELD_COUNT
        or phase3_report.get("scenario_field_check_count")
        != P3_SCENARIO_FIELD_CHECK_COUNT
    ):
        return "PHASE3_CONTROL_SHAPE_MISMATCH"
    if phase3_report.get("valid") is not True:
        return "PHASE3_CONTROL_SHAPE_MISMATCH"
    if phase3_report.get("all_control_references_opaque") is not True:
        return "CONTROL_REFERENCE_NOT_OPAQUE"
    if phase3_report.get("keyword_and_domain_coverage_preserved") is not True:
        return "KEYWORD_BASELINE_MISSING"
    scenarios = _as_records(phase3_report.get("scenario_results"))
    if not all(
        str(item.get("observed_vector_baseline_state", "")).startswith(
            "CONTROL_VECTOR_BASELINE_"
        )
        for item in scenarios
    ):
        return "VECTOR_BASELINE_MISSING"
    if not all(
        item.get("observed_vector_only_rejected") is True for item in scenarios
    ):
        return "VECTOR_ONLY_ROUTE_DETECTED"
    if phase3_report.get("six_dimension_filter_combination_preserved") is not True:
        return "SIX_DIMENSION_FILTER_COMBINATION_INCOMPLETE"
    if phase3_report.get("active_index_version_chain_preserved") is not True:
        return "ACTIVE_INDEX_VERSION_CONTRACT_MISMATCH"
    if phase3_report.get("top_k_ranking_and_validity_preserved") is not True:
        return "TOP_K_OR_RANKING_EXPLANATION_MISSING"
    if phase3_report.get("result_validity_gate_chain_preserved") is not True:
        return "RESULT_VALIDITY_CHAIN_MISSING"
    if (
        phase3_report.get("all_validity_gates_pending_human_whitebox")
        is not True
    ):
        return "VALIDITY_GATE_NOT_PENDING_HUMAN_WHITEBOX"
    if phase3_report.get("old_index_trace_version_preserved") is not True:
        return "OLD_INDEX_TRACE_VERSION_MISMATCH"
    if phase3_report.get("automatic_business_recommendation_allowed") is True:
        return "AUTOMATIC_BUSINESS_RECOMMENDATION_ATTEMPTED"
    if phase3_report.get("automatic_index_switch_allowed") is True:
        return "AUTOMATIC_INDEX_SWITCH_ATTEMPTED"
    if any(
        item.get("actual_retrieval_sample_written") is True
        or item.get("actual_trace_log_written") is True
        or item.get("actual_filter_result_written") is True
        or item.get("actual_validity_test_executed") is True
        or item.get("actual_evidence_gap_record_written") is True
        for group in (
            samples,
            trace_logs,
            filter_results,
        ...
        return "DELIVERY_METADATA_WRITE_SIGNAL_DETECTED"
    if any(
        item.get("entry_precondition")
        != "VERSIONED_PARAMETER_CHANGE_AND_BUSINESS_LINE_WHITEBOX_APPROVAL_REQUIRED"
        or item.get("actual_retrieval_parameter_rollback_performed") is True
        for item in rollback_instructions
    ):
        return "PARAMETER_ROLLBACK_PRECONDITION_MISSING"
    if any(item.get("automatic_resolution_allowed") is True for item in evidence_gaps):
        return "AUTOMATIC_GAP_RESOLUTION_ATTEMPTED"
    if not phase3_valid or not delivery_integrity:
    ...
    return [
        {
            ...
            "candidate_chunk_ref": _text(item, "candidate_chunk_ref"),
            "selected_chunk_ref": _text(item, "selected_chunk_ref"),
            "keyword_retrieval_baseline_ref": _text(
                item, "keyword_retrieval_baseline_ref"
            ),
            "vector_retrieval_baseline_ref": _text(
                item, "vector_retrieval_baseline_ref"
            ),
    ...
    return [
        {
            ...
            "requested_top_k_ref": _text(item, "requested_top_k_ref"),
            "hybrid_score_ref": _text(item, "hybrid_score_ref"),
            "ranking_policy_ref": _text(item, "ranking_policy_ref"),
            ...
            "validity_gate_ref": _text(item, "validity_gate_ref"),
...
def _parameter_rollback_instructions() -> list[dict[str, Any]]:
    controls = (
        ...
        (
            "VECTOR_AND_SEMANTIC_PARAMETER_ROLLBACK_CONTROL",
            "semantic_similarity_hybrid_control",
            "VECTOR_BASELINE_AND_SEMANTIC_SIMILARITY",
        ),
        (
            "METADATA_FILTER_PARAMETER_ROLLBACK_CONTROL",
            "six_dimension_filter_combination_control",
    ...
    return [
        {
            ...
            "rollback_target_ref": (
                f"stage088-p3-control-boundary{CONTROL_PREFIX}{scenario_id}"
            ),
...
def _trace_log_is_control_only(record: Mapping[str, Any]) -> bool:
    return (
        str(record.get("trace_version_state", "")).startswith("CONTROL_")
...
def _validity_report_is_control_only(record: Mapping[str, Any]) -> bool:
    return (
        str(record.get("observed_result_validity_state", "")).startswith(
            "CONTROL_RESULT_VALIDITY_"
        )
