"""Stage091 P3 的纯内存证据缺口处理异常场景控制验证。

模块只重放 Stage091 P2 的固定、非业务、reference-only 控制投影。它不读取
来源资料或真实证据账本，不连接数据库，不计算 OCR、版本、风险或可信等级，
不执行撤回、投毒处置或报告状态更新，也不写入持久化记录。
"""
...
SCHEMA_VERSION = "ids.stage091.evidence_gap_handling.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_GAP_HANDLING_EXCEPTION_SCENARIOS"
PASS_RESULT = "PASS_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CURRENT_GATE = "IDS-STAGE091-P3-GATE"
NEXT_GATE = "IDS-STAGE091-P4-GATE"
P2_EXECUTION_STATE = "CONTROL_EVIDENCE_GAP_HANDLING_PROJECTIONS_DECLARED_NOT_EXECUTED"
...
SCENARIO_DEFINITIONS = (
    ...
    },
    {
        ...
        "phase2_control_scenario": "low_grade_evidence_gap_degraded_reference_only",
    ...
    },
    {
        ...
        "phase2_control_scenario": "expired_evidence_gap_degraded_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "C",
    ...
    },
    {
        ...
        "phase2_control_scenario": "conflict_evidence_gap_degraded_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "B",
    ...
    },
    {
        ...
        "phase2_control_scenario": "revoked_evidence_gap_degraded_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "A",
    ...
    },
    {
        ...
        "phase2_control_scenario": "suspected_poisoning_gap_quarantined_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "E",
        "expected_degradation_state": (
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name(
        "stage091_evidence_gap_handling_control_slice.py"
    )
    spec = importlib.util.spec_from_file_location("stage091_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage091 P2 evidence gap handling slice")
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    if (
        ...
        != ("evidence_gap_handling_control_requests",)
...
def _phase2_records(
    ...
    for prefix, _field_constant in P2_PROJECTION_SPECS:
        ...
        if not isinstance(record, Mapping):
            raise ValueError(f"invalid Stage091 P2 {prefix} projection")
...
def _phase2_semantic_failure(result: Mapping[str, Any]) -> str | None:
    ...
    low_grade = _phase2_records(
        result, "low_grade_evidence_gap_degraded_reference_only"
    )
    conflict = _phase2_records(
        result, "conflict_evidence_gap_degraded_reference_only"
    )
    expired = _phase2_records(
        result, "expired_evidence_gap_degraded_reference_only"
    )
    revoked = _phase2_records(
        result, "revoked_evidence_gap_degraded_reference_only"
    )
    malicious = _phase2_records(
        result, "suspected_poisoning_gap_quarantined_reference_only"
    )
    if (
        no_internal["evidence_gap_schema_binding"].get("evidence_id_ref") is not None
        or not _control_ref(
            no_internal["evidence_gap_schema_binding"].get("evidence_gap_ref")
        )
        or not _control_ref(
    ...
    if (
        ...
        or not _control_ref(low_grade["risk_score"].get("ocr_quality_indicator_ref"))
    ):
    ...
    if (
        ...
        or not _control_ref(expired["risk_score"].get("freshness_indicator_ref"))
    ):
    ...
    if (
        ...
        or not _control_ref(conflict["risk_score"].get("conflict_indicator_ref"))
    ):
    ...
    if (
        ...
        != "CONTROL_REPORT_STATUS_REFERENCE_DEGRADED_REVOKED"
        or not _control_ref(
            revoked["future_integration"].get("report_status_update_route_ref")
        )
    ):
    ...
    if (
        ...
        or not _control_ref(
            malicious["poisoning_defense"].get("poisoning_defense_state_ref")
        )
    ):
    ...
    if (
        ...
        or "evidence-grade-D"
        not in str(
            low_grade["critical_conclusion_binding"].get("evidence_grade_ref")
        )
    ):
...
def _scenario_expectation_met(
    ...
    schema = records["evidence_gap_schema_binding"]
    relation = records["evidence_gap_relation"]
...
def build_evidence_gap_handling_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制投影并验证 Stage091 P3 固定异常场景。"""

    phase2_module = _load_phase2_module()
    executor = phase2_executor or phase2_module.execute_evidence_gap_handling_control_slice
    try:
