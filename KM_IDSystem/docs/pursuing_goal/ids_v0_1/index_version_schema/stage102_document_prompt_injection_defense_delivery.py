"""Stage102 P4 的纯内存文档内提示注入防护交付证据。

本模块只从 Stage102 P3 已固定的非业务、reference-only 场景派生控制记录。
...
SCHEMA_VERSION = "ids.stage102.document_prompt_injection_defense.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_DOCUMENT_PROMPT_INJECTION_DEFENSE_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_DOCUMENT_PROMPT_INJECTION_DEFENSE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_DOCUMENT_PROMPT_INJECTION_DEFENSE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE102-P4-GATE"
NEXT_GATE = "IDS-STAGE102-REVIEW-GATE"
...
P3_CURRENT_GATE = "IDS-STAGE102-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE102-P4-GATE"
P3_CONTROL_PREFIX = ":control:stage102-p2:"
DELIVERY_PREFIX = ":control:stage102-p4:"
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage102_phase4_runtime_executed",
)
...
FAILURE_STATES = (
    ...
    "DOCUMENT_PROMPT_INJECTION_PROTECTION_MISSING",
...
def _load_phase3_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage102_phase3_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage102 P3 文档内提示注入防护场景")
...
def _phase3_shape_failure(phase3_module: Any, report: Mapping[str, Any]) -> str | None:
    ...
    if (
        ...
        or report.get("phase2_control_request_count") != 7
        or report.get("phase2_input_field_count") != 28
        or report.get("phase2_projection_group_count") != 4
        or report.get("phase2_projection_field_count_per_request") != 50
        or report.get("phase2_projection_field_count_total") != 350
    ...
    if (
        ...
        or any(set(item) != set(P3_SCENARIO_FIELDS) for item in scenarios)
    ):
        return "PHASE3_CONTROL_SHAPE_MISMATCH"
...
def _phase3_semantic_failure(report: Mapping[str, Any]) -> str | None:
    ...
    for scenario in scenarios.values():
        ...
        if (
            ...
            return "DOCUMENT_PROMPT_INJECTION_PROTECTION_MISSING"
        if (
    ...
    return [
        {
            ...
            **{field: item[field] for field in REPRODUCIBILITY_TUPLE_FIELDS},
            ...
            "answer_sample_state": (
                "CONTROL_DOCUMENT_PROMPT_INJECTION_ANSWER_SAMPLE_REFERENCE_ONLY_NOT_EXECUTED"
            ),
    ...
    return [
        {
            ...
            "negative_test_result_state": (
                "CONTROL_DOCUMENT_PROMPT_INJECTION_NEGATIVE_RESULT_REFERENCE_ONLY"
            ),
    ...
    return [
        {
            ...
            "version_record_state": (
                "CONTROL_PROMPT_MODEL_AND_DOCUMENT_INSTRUCTION_REFERENCE_ONLY"
            ),
    ...
    return [
        {
            ...
            **{field: item[field] for field in REPRODUCIBILITY_TUPLE_FIELDS},
            ...
            "reproducible_log_state": (
                "CONTROL_DOCUMENT_PROMPT_INJECTION_TUPLE_REFERENCE_ONLY"
            ),
    ...
    return [
        {
            ...
            "predecessor_phase_ref": _delivery_ref("stage102-phase3"),
...
def _delivery_shape_is_valid(report: Mapping[str, Any]) -> bool:
    for group_name, fields in DELIVERY_GROUPS:
        ...
        if (
            ...
            return False
    return report.get("delivery_field_check_count") == 528
...
def build_document_prompt_injection_defense_phase4_delivery_report(
    phase3_executor: Phase3Executor | None = None,
) -> dict[str, Any]:
    """派生 P4 交付控制记录；任何 P3 漂移均失败关闭。"""

    try:
        phase3_module = _load_phase3_module()
        executor = (
            phase3_executor
            or phase3_module.build_document_prompt_injection_defense_phase3_report
        )
