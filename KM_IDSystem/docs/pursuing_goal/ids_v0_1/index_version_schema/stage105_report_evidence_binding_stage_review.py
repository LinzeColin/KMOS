"""Stage105 报告证据绑定的纯内存整阶段机械复审。

本模块只复审冻结任务包和 Stage105 P1--P4 已提交的控制合同与纯内存报告。
...
SCHEMA_VERSION = "ids.stage105.report_evidence_binding.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_EVIDENCE_BINDING_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_REPORT_EVIDENCE_BINDING_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_REPORT_EVIDENCE_BINDING_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE105-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE106-P1-GATE"

P1_SCHEMA_VERSION = "ids.stage105.report_evidence_binding.phase1.v1"
P1_CONTRACT_STATE = "REPORT_EVIDENCE_BINDING_CONTRACT_RUNTIME_DISABLED"
...
P2_PASS_RESULT = "PASS_IN_MEMORY_REPORT_EVIDENCE_BINDING_CONTROL_SLICE_RUNTIME_DISABLED"
...
P4_SCHEMA_VERSION = "ids.stage105.report_evidence_binding.phase4.delivery.v1"
P4_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_EVIDENCE_BINDING_DELIVERY_EVIDENCE"
P4_PASS_RESULT = "PASS_REPORT_EVIDENCE_BINDING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"

P2_CONTROL_PREFIX = ":control:stage105-p2:"
P4_DELIVERY_PREFIX = ":control:stage105-p4:"
...
REVIEWED_CONTROL_SHAPE = {
    ...
    "phase2_projection_field_count_per_request": 66,
    "phase2_control_field_check_count": 330,
    "phase2_failure_state_count": 19,
...
REVIEW_RUNTIME_FALSE_FIELDS = (
    ...
    "push_performed",
    "stage105_review_runtime_executed",
)
...
FAILURE_STATES = (
    ...
    "RUNTIME_SIGNAL_OR_STAGE106_ENTRY_DETECTED",
)

Provider = Callable[[], Mapping[str, Any]]
BASE = Path(__file__).resolve().parent
P1_CONTRACT_PATH = BASE / "stage105_report_evidence_binding_contract.json"
P2_CONTRACT_PATH = BASE / "stage105_report_evidence_binding_control_slice_contract.json"
P3_CONTRACT_PATH = BASE / "stage105_report_evidence_binding_controlled_scenarios_contract.json"
P4_CONTRACT_PATH = BASE / "stage105_report_evidence_binding_delivery_contract.json"
P2_MODULE_PATH = BASE / "stage105_report_evidence_binding_control_slice.py"
P3_MODULE_PATH = BASE / "stage105_report_evidence_binding_controlled_scenarios.py"
P4_MODULE_PATH = BASE / "stage105_report_evidence_binding_delivery.py"
...
def _p1_shape_valid(contract: Mapping[str, Any]) -> bool:
    report = _mapping(contract.get("report_evidence_binding_contract"))
    snapshot = _mapping(contract.get("generation_snapshot_contract"))
    external = _mapping(contract.get("external_augmentation_contract"))
    human = _mapping(contract.get("human_confirmation_contract"))
    lifecycle = _mapping(contract.get("report_lifecycle_control_contract"))
    boundary = _mapping(contract.get("stage_and_phase_boundary"))
    return (
        contract.get("schema_version") == P1_SCHEMA_VERSION
        and contract.get("stage") == "STAGE-105"
        and contract.get("phase") == "IDS-STAGE105-P1"
        and contract.get("task_id") == "IDS-V0_1-STAGE105-P1"
        and contract.get("contract_state") == P1_CONTRACT_STATE
        and contract.get("entry_gate") == "IDS-STAGE105-P1-GATE"
        and contract.get("next_gate") == "IDS-STAGE105-P2-GATE"
        and tuple(report.get("future_control_reference_fields", ())) == P1_REFERENCE_FIELDS
        and report.get("future_control_reference_field_count") == 24
        and report.get("critical_conclusion_requires_evidence_id_or_evidence_gap") is True
        and report.get("citation_source_must_be_displayed_in_future_pdf_report") is True
        and tuple(snapshot.get("required_future_snapshot_components", ()))
        == P1_SNAPSHOT_COMPONENTS
        and snapshot.get("required_future_snapshot_component_count") == 5
        and external.get("external_augmentation_may_not_be_written_as_internal_project_evidence")
        is True
        and external.get("external_augmentation_may_not_close_evidence_gap") is True
        and human.get("business_line_whitebox_human_confirmation_required_before_final_conclusion")
        is True
        and human.get("final_conclusion_may_not_be_automated") is True
        and lifecycle.get("evidence_revocation_requires_future_report_impact_review") is True
        and lifecycle.get("evidence_grade_change_requires_future_report_impact_review")
        is True
        and lifecycle.get("index_version_change_requires_future_report_impact_review")
        is True
        and boundary.get("phase1_completed") is True
        and boundary.get("stage106_started") is False
...
def _p2_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and contract.get("phase") == "IDS-STAGE105-P2"
        and contract.get("task_id") == "IDS-V0_1-STAGE105-P2"
        and contract.get("contract_state")
        == "REPORT_EVIDENCE_BINDING_CONTROL_SLICE_RUNTIME_DISABLED"
        and contract.get("entry_gate") == "IDS-STAGE105-P2-GATE"
        and contract.get("next_gate") == "IDS-STAGE105-P3-GATE"
        and slice_contract.get("control_request_count") == 5
        and slice_contract.get("control_input_field_count") == 26
        and slice_contract.get("projection_group_count") == 4
        and slice_contract.get("projection_field_total_per_request") == 66
        and slice_contract.get("projection_field_total") == 330
...
def _p3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and contract.get("phase") == "IDS-STAGE105-P3"
        and contract.get("task_id") == "IDS-V0_1-STAGE105-P3"
        and contract.get("contract_state")
        == "REPORT_EVIDENCE_BINDING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
        and contract.get("entry_gate") == "IDS-STAGE105-P3-GATE"
        and contract.get("next_gate") == "IDS-STAGE105-P4-GATE"
...
def _p4_contract_valid(contract: Mapping[str, Any]) -> bool:
    delivery = _mapping(contract.get("taskpack_delivery_contract"))
    shape = _mapping(contract.get("delivery_shape_contract"))
    rollback = _mapping(contract.get("rollback_and_withdrawal_contract"))
    boundary = _mapping(contract.get("stage_boundary"))
    return (
        contract.get("schema_version") == P4_SCHEMA_VERSION
        and contract.get("phase") == "IDS-STAGE105-P4"
        and contract.get("task_id") == "IDS-V0_1-STAGE105-P4"
        and contract.get("contract_state")
        == "REPORT_EVIDENCE_BINDING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
        and contract.get("entry_gate") == "IDS-STAGE105-P4-GATE"
        ...
        and shape.get("evidence_id_or_evidence_gap_is_exclusive") is True
        and shape.get("external_augmentation_remains_external_source_type") is True
        and shape.get("final_conclusion_remains_unpublished") is True
        and rollback.get("rollback_target_result") == P3_PASS_RESULT
        and rollback.get("business_line_whitebox_confirmation_required") is True
        and rollback.get("versioned_basis_required") is True
        and boundary.get("phase4_completed") is True
        and boundary.get("stage105_review_started") is False
        and boundary.get("stage106_started") is False
        and _contract_boundary_closed(contract)
    )
...
def _p2_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    if (
        ...
        or report.get("control_projection_field_total_per_request") != 66
        or report.get("control_projection_field_total") != 330
...
def _p3_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    if (
        ...
        or getattr(module, "CURRENT_GATE", None) != "IDS-STAGE105-P3-GATE"
        or getattr(module, "NEXT_GATE", None) != "IDS-STAGE105-P4-GATE"
        ...
        or report.get("failure_state") is not None
        or report.get("current_gate") != "IDS-STAGE105-P3-GATE"
        or report.get("next_gate") != "IDS-STAGE105-P4-GATE"
...
def _p4_report_valid(module: Any, report: Mapping[str, Any]) -> bool:
    if (
        ...
        or getattr(module, "ENTRY_GATE", None) != "IDS-STAGE105-P4-GATE"
        ...
        or report.get("valid") is not True
        or report.get("failure_state") is not None
        or report.get("current_gate") != "IDS-STAGE105-P4-GATE"
        or report.get("next_gate") != REVIEW_GATE
        or report.get("delivery_field_check_count") != 388
        or report.get("control_references_opaque") is not True
        or report.get("second_authoritative_source_created") is not False
        or report.get("persistent_record_created") is not False
        or report.get("stage105_review_started") is not False
...
def _evidence_and_source_semantics_valid(
    ...
    if not isinstance(external, Mapping) or (
        ...
        or external["actual_external_augmentation_displayed"] is not False
    ):
        return False
    return all(
        (item["evidence_id_ref"] is None) != (item["evidence_gap_ref"] is None)
...
def build_report_evidence_binding_stage_review(
    ...
    try:
        phase2_module = _load_module("stage105_review_phase2", P2_MODULE_PATH)
        phase2_report = (
            phase2_provider()
            if phase2_provider
    ...
    try:
        phase3_module = _load_module("stage105_review_phase3", P3_MODULE_PATH)
        phase3_report = (
            phase3_provider()
            if phase3_provider
            else phase3_module.build_report_evidence_binding_phase3_report()
        )
    except Exception:
    ...
    try:
        phase4_module = _load_module("stage105_review_phase4", P4_MODULE_PATH)
        phase4_report = (
            phase4_provider()
            if phase4_provider
            else phase4_module.build_report_evidence_binding_phase4_delivery_report()
        )
    except Exception:
    ...
    if not all(
        ...
        return _failure_report("RUNTIME_SIGNAL_OR_STAGE106_ENTRY_DETECTED")
    ...
    report: dict[str, Any] = {
        ...
        "second_authoritative_source_created": False,
        "persistent_record_created": False,
        "stage105_review_started": True,
        "whole_stage_review_completed_in_memory_only": True,
        "stage106_started": False,
        **_zero_actual_counts(),
        "runtime_boundary": _runtime_boundary(),
        "chinese_feedback": [
            "报告证据绑定 P1--P4 已完成纯内存机械复审，来源文档、真实证据账本与业务线白箱人工复核继续承担业务事实权威。",
