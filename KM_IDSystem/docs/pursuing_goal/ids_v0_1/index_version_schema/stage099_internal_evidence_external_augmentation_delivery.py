"""Stage099 P4 的纯内存内部依据与外部增强交付证据。

模块只从 P3 固定、非业务、reference-only 控制场景派生交付记录。它不读取
...
FAIL_RESULT = (
    "FAIL_INTERNAL_EVIDENCE_EXTERNAL_AUGMENTATION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
)
ENTRY_GATE = "IDS-STAGE099-P4-GATE"
NEXT_GATE = "IDS-STAGE099-REVIEW-GATE"
P3_SCHEMA_VERSION = (
...
P3_CURRENT_GATE = "IDS-STAGE099-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE099-P4-GATE"
...
def _load_phase3_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage099_phase3_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage099 P3 controlled scenarios")
...
def _phase3_shape_failure(phase3_module: Any, report: Mapping[str, Any]) -> str | None:
    if (
        ...
        or report.get("phase2_field_check_count") != 210
...
def _phase3_semantic_failure(report: Mapping[str, Any]) -> str | None:
    ...
    if (
        ...
        or external.get("display_label_is_not_source_type_state")
        != "CONTROL_EXTERNAL_AUGMENTATION_OPINION_IS_DISPLAY_LABEL_ONLY"
        or external.get("display_preserves_underlying_source_types_state")
    ...
    return [
        {
            ...
            "predecessor_phase_ref": _delivery_ref("stage099-phase3"),
...
def build_internal_evidence_external_augmentation_phase4_delivery_report(
    phase3_executor: Phase3Executor | None = None,
) -> dict[str, Any]:
    """派生 Stage099 P4 受控交付证据，不执行任何业务或运行时动作。"""

    try:
        phase3_module = _load_phase3_module()
        executor = (
            phase3_executor
            or phase3_module.build_internal_evidence_external_augmentation_phase3_report
        )
