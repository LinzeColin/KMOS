"""Stage091 证据缺口处理的纯内存整阶段机械复审。

复审只读取冻结任务包和 P1--P4 控制工件，确认固定控制形状、单一权威、
业务线白箱人工处理、失败关闭与回退边界。它不读取真实资料，不执行检索、
证据账本写入、风险评分、撤回、恢复、模型调用、Agent、OVH 或生产运行。
"""
...
P1_CONTRACT = BASE / "stage091_evidence_gap_handling_contract.json"
P2_CONTRACT = BASE / "stage091_evidence_gap_handling_control_slice_contract.json"
P3_CONTRACT = BASE / "stage091_evidence_gap_handling_controlled_scenarios_contract.json"
P4_CONTRACT = BASE / "stage091_evidence_gap_handling_delivery_contract.json"
P2_MODULE = BASE / "stage091_evidence_gap_handling_control_slice.py"
P3_MODULE = BASE / "stage091_evidence_gap_handling_controlled_scenarios.py"
P4_MODULE = BASE / "stage091_evidence_gap_handling_delivery.py"

SCHEMA_VERSION = "ids.stage091.evidence_gap_handling.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE091-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-091"
PASS_RESULT = "PASS_REVIEWED_EVIDENCE_GAP_HANDLING_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_EVIDENCE_GAP_HANDLING_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE091-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE092-P1-GATE"
RETURN_STATE = "PASS_EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
P4_RETURN_STATE = "PASS_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage091-p2:"
DELIVERY_PREFIX = ":control:stage091-p4:"

P1_SHAPES = (
    ("future_evidence_gap_fields", "evidence_gap_field_count", 12),
    ("future_evidence_gap_relation_fields", "evidence_gap_relation_field_count", 7),
)
P2_GROUPS = (
    ("evidence_gap_schema_binding", 6),
    ("evidence_gap_relation", 7),
    ("retrieval_evidence_capture_binding", 10),
    ("risk_score", 9),
    ("revocation", 8),
    ("poisoning_defense", 8),
    ("critical_conclusion_binding", 7),
    ("degradation", 10),
    ("report_status_impact", 6),
...
EXPECTED_CONTROLLED_REPLAY = {
    "phase1_static_shape": "12/7/5",
    "phase1_failure_state_count": 12,
    "phase2_control_request_count": 6,
    "phase2_control_input_field_count": 27,
    "phase2_projection_group_count": 10,
    "phase2_fields_per_request": 78,
    "phase2_control_field_check_count": 468,
...
def build_evidence_gap_handling_stage091_review_report(
    *,
    ...
    """复审冻结 P1--P4 控制工件；漂移时停留在 Stage091 Review gate。"""
    ...
    invariants = {
        ...
        "stage092_gate_only_opens_after_review": review_valid and next_gate == NEXT_GATE,
    }
    return {
        "schema_version": SCHEMA_VERSION,
        "task_id": TASK_ID,
        "acceptance_id": ACCEPTANCE_ID,
        "source_authority": "FROZEN_STAGE091_TASKPACK_AND_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        "reviewed_phase_ids": [
            "IDS-STAGE091-P1",
            "IDS-STAGE091-P2",
            "IDS-STAGE091-P3",
        ...
        "stage091_started": True,
        "stage091_review_started": True,
        "review_control_completed": review_valid,
        "whole_stage_review_performed": False,
        "stage092_started": False,
        ...
        **{field: 0 for field in ACTUAL_COUNT_FIELDS},
        "rollback": {
            "scope": "STAGE091_REVIEW_ARTIFACTS_AND_LOCAL_GOVERNANCE_ONLY",
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module("stage091_review_phase2", P2_MODULE)
    return (
        {}
        if module is None
        else _mapping(
            module.execute_evidence_gap_handling_control_slice(
...
def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module("stage091_review_phase3", P3_MODULE)
    return (
        {}
        if module is None
        else _mapping(module.build_evidence_gap_handling_phase3_report())
    )


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module("stage091_review_phase4", P4_MODULE)
    return (
        {}
        if module is None
        else _mapping(module.build_evidence_gap_handling_phase4_delivery_report())
    )


def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    shape = _mapping(contract.get("evidence_gap_contract"))
    ...
    return (
        ...
        and contract.get("stage") == "STAGE-091"
        and contract.get("phase") == "IDS-STAGE091-P1"
        and contract.get("task_id") == "IDS-V0_1-STAGE091-P1"
        and contract.get("contract_state")
        == "PHASE1_EVIDENCE_GAP_HANDLING_CONTRACT_RUNTIME_DISABLED"
        and contract.get("next_gate") == "IDS-STAGE091-P2-GATE"
        ...
        and shape.get("critical_conclusion_requires_evidence_id_or_evidence_gap")
        is True
        and shape.get("internal_material_insufficiency_requires_evidence_gap") is True
        and shape.get("evidence_gap_may_not_be_presented_as_internal_experience")
        is True
        ...
        and boundary.get("stage091_started") is True
        ...
        and boundary.get("stage092_started") is False
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    boundary = _mapping(contract.get("stage_and_phase_boundary"))
    return (
        contract.get("schema_version") == "ids.stage091.evidence_gap_handling.phase2.v1"
        and contract.get("stage") == "STAGE-091"
        and contract.get("phase") == "IDS-STAGE091-P2"
        and contract.get("task_id") == "IDS-V0_1-STAGE091-P2"
        and contract.get("contract_state")
        == "PHASE2_EVIDENCE_GAP_HANDLING_CONTROL_SLICE_RUNTIME_DISABLED"
        and contract.get("next_gate") == "IDS-STAGE091-P3-GATE"
        and _source_authority_closed(_mapping(contract.get("source_authority")))
        and control_input.get("control_prefix") == CONTROL_PREFIX
        and _integer(control_input.get("control_request_count")) == 6
        and _integer(control_input.get("input_field_count")) == 27
        and _sequence_length(control_input.get("fixed_control_scenarios")) == 6
        and _integer(projections.get("each_projection_count")) == 6
        and _integer(projections.get("control_projection_group_count")) == 10
        and all(
            _sequence_length(projections.get(f"{prefix}_projection_fields"))
            == field_count
            for prefix, field_count in P2_GROUPS
        )
        and _integer(projections.get("control_projection_field_total_per_request")) == 78
        and _integer(projections.get("control_projection_field_total")) == 468
        and _integer(_mapping(contract.get("failure_and_stop_contract")).get("failure_state_count"))
        == 25
        and _runtime_mapping_closed(_mapping(contract.get("runtime_boundary")))
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        contract.get("schema_version") == "ids.stage091.evidence_gap_handling.phase3.v1"
        and contract.get("stage") == "STAGE-091"
        and contract.get("phase") == "IDS-STAGE091-P3"
        and contract.get("task_id") == "IDS-V0_1-STAGE091-P3"
        and contract.get("contract_state")
        == "PHASE3_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
        and contract.get("next_gate") == "IDS-STAGE091-P4-GATE"
        and _source_authority_closed(_mapping(contract.get("source_authority")))
        and replay.get("control_prefix") == CONTROL_PREFIX
        and _integer(replay.get("required_control_request_count")) == 6
        and _integer(replay.get("required_input_field_count")) == 27
        and _integer(replay.get("required_projection_group_count")) == 10
        and _integer(replay.get("expected_projection_fields_per_request")) == 78
        and _integer(replay.get("expected_phase2_field_check_count")) == 468
        and _integer(scenarios.get("scenario_count")) == 7
        and tuple(scenarios.get("scenario_fields", ())) == P3_SCENARIO_FIELDS
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return (
        contract.get("schema_version")
        == "ids.stage091.evidence_gap_handling.phase4.delivery.v1"
        and contract.get("stage") == "STAGE-091"
        and contract.get("phase") == "IDS-STAGE091-P4"
        and contract.get("task_id") == "IDS-V0_1-STAGE091-P4"
        and contract.get("contract_state")
        == "PHASE4_EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
        ...
        is True
        and source.get("delivery_control_metadata_can_replace_source_document") is False
        and source.get("delivery_control_metadata_can_become_business_fact_authority")
        is False
        and _integer(replay.get("required_control_request_count")) == 6
        and replay.get("control_prefix") == CONTROL_PREFIX
        and _integer(replay.get("phase2_control_field_check_count")) == 468
        and _integer(replay.get("scenario_count")) == 7
        and _integer(replay.get("scenario_field_count")) == 32
        and _integer(replay.get("scenario_field_check_count")) == 224
        ...
        and boundary.get("whole_stage_review_performed") is False
        and boundary.get("stage091_review_started") is False
        and boundary.get("stage092_started") is False
        and boundary.get("github_upload_allowed") is False
        and boundary.get("push_allowed") is False
    )
...
def _phase2_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        report.get("schema_version") == "ids.stage091.evidence_gap_handling.phase2.v1"
        and report.get("record_kind") == "CONTROL_ONLY_IN_MEMORY_EVIDENCE_GAP_HANDLING"
...
def _phase3_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        report.get("schema_version") == "ids.stage091.evidence_gap_handling.phase3.v1"
        and report.get("record_kind")
        == "CONTROL_ONLY_IN_MEMORY_EVIDENCE_GAP_HANDLING_EXCEPTION_SCENARIOS"
        and report.get("valid") is True
        and report.get("result")
        == "PASS_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE091-P3-GATE"
        and report.get("next_gate") == "IDS-STAGE091-P4-GATE"
        ...
        and _integer(report.get("phase2_field_check_count")) == 468
        ...
        and all(
            tuple(record) == P3_SCENARIO_FIELDS
            and len(record) == 32
            and _record_references_are_control_only(record, (CONTROL_PREFIX,))
        ...
        and all(
            ...
            and record.get("high_trust_conclusion_allowed") is False
            and record.get("silent_drop") is False
            for record in scenarios
        )
        and by_id.get("no_internal_evidence_gap_control", {}).get("evidence_id_ref")
        is None
        and _control_reference(
            by_id.get("no_internal_evidence_gap_control", {}).get("evidence_gap_ref")
        )
        and by_id.get("no_internal_evidence_gap_control", {}).get(
            "conclusion_acceptance_state"
        )
        ...
        and by_id.get("revoked_evidence_report_impact_control", {}).get(
            "report_status_impact_state"
        )
        ...
        and by_id.get("malicious_evidence_quarantine_control", {}).get(
            "evidence_disposition_state"
        )
        == "CONTROL_MALICIOUS_EVIDENCE_QUARANTINED_NOT_ACCEPTED"
        and by_id.get("low_grade_high_trust_masquerade_control", {}).get(
            "evidence_grade_label"
        )
        == "D"
        and by_id.get("low_grade_high_trust_masquerade_control", {}).get(
...
def _phase4_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        == "ids.stage091.evidence_gap_handling.phase4.delivery.v1"
        and report.get("record_kind") == "EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_REPORT"
        and report.get("valid") is True
        and report.get("result")
        == "PASS_EVIDENCE_GAP_HANDLING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE091-P4-GATE"
        ...
        and _integer(report.get("phase2_control_field_check_count")) == 468
        ...
        and all(
            ...
            and item.get("instruction_state")
            == "CONTROL_REVOCATION_AND_RECOVERY_DECLARED_NOT_EXECUTED"
        ...
        and _actual_counts_zero(report)
        and _runtime_mapping_closed(_mapping(report.get("runtime_boundary")))
        and report.get("stage092_started") is False
...
def _controlled_replay(
    ...
    p1 = _mapping(phase1.get("evidence_gap_contract"))
    ...
    return {
        ...
        "phase2_projection_group_count": _integer(
            p2_projection.get("control_projection_group_count")
        ),
...
def _single_authority_boundary(
    ...
    phase4_authority = _mapping(phase4.get("source_authority"))
    return (
        all(
            _source_authority_closed(_mapping(artifact.get("source_authority")))
            for artifact in (phase1, phase2, phase3, phase4)
        )
        and phase4_authority.get("source_document_remains_authoritative") is True
        and phase4_authority.get("business_line_whitebox_human_review_remains_authoritative")
        is True
        and phase4_authority.get("delivery_control_metadata_can_replace_source_document")
        is False
        and phase4_authority.get(
...
def _next_stage_available_but_not_started(
    ...
    return (
        ...
        and all(boundary.get("stage092_started") is False for boundary in boundaries)
        and phase4_report.get("stage092_started") is False
...
def _source_authority_closed(source: Mapping[str, Any]) -> bool:
    ...
    report_access_closed = source.get(
        "answer_or_report_access_performed",
        source.get("report_or_audit_log_access_performed"),
    )
    return (
        bool(source)
        and all(source.get(field) is False for field in required_false)
        and report_access_closed is False
    )
...
def _record_references_are_control_only(
    ...
    return all(
        (key == "evidence_id_ref" and value is None)
        or (
            isinstance(value, str)
            and any(prefix in value for prefix in prefixes)
        )
