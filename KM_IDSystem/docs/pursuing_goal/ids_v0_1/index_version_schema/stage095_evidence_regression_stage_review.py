...
SCHEMA_VERSION = "ids.stage095.evidence_regression.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_REGRESSION_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_EVIDENCE_REGRESSION_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_EVIDENCE_REGRESSION_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE095-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE096-P1-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_REGRESSION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_PASS_RESULT = "PASS_EVIDENCE_REGRESSION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:stage095-"

P1_STATIC_SHAPE = "14/14/6/5"
...
REVIEW_RUNTIME_FALSE_FIELDS = (
    ...
    "push_performed",
    "stage095_review_runtime_executed",
)
...
P1_CONTRACT_PATH = BASE / "stage095_evidence_regression_contract.json"
P2_MODULE_PATH = BASE / "stage095_evidence_regression_control_slice.py"
P3_MODULE_PATH = BASE / "stage095_evidence_regression_controlled_scenarios.py"
P4_MODULE_PATH = BASE / "stage095_evidence_regression_delivery.py"
NEXT_TASKPACK_PATH = (
    BASE.parents[2]
    / "taskpacks"
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module("stage095_review_phase2", P2_MODULE_PATH)
    return module.execute_evidence_regression_control_slice(module.build_control_input())


def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module("stage095_review_phase3", P3_MODULE_PATH)
    return module.build_evidence_regression_phase3_report()


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module("stage095_review_phase4", P4_MODULE_PATH)
    return module.build_evidence_regression_phase4_delivery_report()
...
def _opaque_control_reference(value: object) -> bool:
    return value is None or (
        isinstance(value, str) and CONTROL_PREFIX in value
    )
...
def _phase1_valid(contract: Mapping[str, Any]) -> bool:
    ...
    regression = _mapping(contract.get("evidence_regression_contract"))
    grade_reference = _mapping(regression.get("evidence_grade_definition_reference"))
    failures = _mapping(contract.get("failure_and_stop_contract"))
    runtime = _mapping(contract.get("runtime_boundary"))
    return (
        contract.get("phase") == "IDS-STAGE095-P1"
        and contract.get("task_id") == "IDS-V0_1-STAGE095-P1"
        and contract.get("next_gate") == "IDS-STAGE095-P2-GATE"
        ...
        and len(_mapping(regression.get("control_definitions"))) == 6
        and regression.get("evidence_regression_field_count") == 14
        and regression.get("evidence_regression_relation_field_count") == 14
        and grade_reference.get("grade_labels") == ["A", "B", "C", "D", "E"]
        and grade_reference.get("grade_assignment_defined") is False
        and grade_reference.get("grade_threshold_defined") is False
        and regression.get("actual_evidence_regression_executed") is False
        and regression.get("actual_evidence_grade_assigned_or_changed") is False
        and regression.get("actual_evidence_revocation_processed") is False
        and regression.get("actual_poisoning_defense_evaluated") is False
        and regression.get("actual_report_status_updated") is False
        and regression.get("critical_conclusion_requires_evidence_id_or_evidence_gap")
        is True
        and regression.get("evidence_regression_may_not_replace_evidence_id_or_evidence_gap")
        is True
        and regression.get(
            "business_line_whitebox_human_review_required_before_business_use"
        )
        is True
        and regression.get("all_values_are_control_labels_only") is True
...
def _phase2_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage095_review_phase2_shape", P2_MODULE_PATH)
...
def _phase3_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage095_review_phase3_shape", P3_MODULE_PATH)
    except Exception:
        return False
    ...
    return (
        ...
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE095-P3-GATE"
        and report.get("next_gate") == "IDS-STAGE095-P4-GATE"
        ...
        and by_id["no_internal_evidence_regression_control"].get("evidence_id_ref")
        is None
        and _opaque_control_reference(
            by_id["no_internal_evidence_regression_control"].get("evidence_gap_ref")
        )
        and by_id["revoked_evidence_report_impact_control"].get(
            "actual_report_status_updated"
        )
        is False
        and str(
            by_id["revoked_evidence_report_impact_control"].get(
                "report_status_impact_state", ""
            )
        ).endswith("DECLARED_NOT_APPLIED")
...
def _phase4_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage095_review_phase4_shape", P4_MODULE_PATH)
    except Exception:
        return False
    specs = (
        (
    ...
    for records_key, count_key, fields_key, fields, count in specs:
        ...
        if (
            ...
            return False
        if any(
            field.endswith("_ref") and not _opaque_control_reference(value)
            for record in records
            for field, value in record.items()
        ):
            return False
    revocation_impacts = report.get("revocation_impact_control_records", [])
    revoked = next(
        (
            record
    ...
    return (
        ...
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE095-P4-GATE"
        ...
        and revoked.get("report_status_impact_state", "").endswith("NOT_APPLIED")
...
def build_evidence_regression_stage095_review_report(
    ...
    """机械聚合 Stage095 P1--P4 控制工件，漂移保持失败关闭。"""
    ...
    phase1_regression = _mapping(phase1.get("evidence_regression_contract"))
    phase1_grade_reference = _mapping(
        phase1_regression.get("evidence_grade_definition_reference")
    )
    source_boundary = all(
        (
    ...
    owner_rule_boundary = (
        phase1_grade_reference.get("grade_assignment_defined") is False
        and phase1_grade_reference.get("grade_threshold_defined") is False
        and phase1_regression.get("actual_evidence_grade_assigned_or_changed") is False
        and phase1_regression.get(
            "business_line_whitebox_human_review_required_before_business_use"
        )
        is True
    )
    ...
    runtime_actions_disabled = (
        _runtime_boundary_closed(phase1)
        and _runtime_boundary_closed(phase2)
    ...
    rollback_boundary = (
        ...
        and phase4.get("stage095_review_started") is False
        and phase4.get("stage096_started") is False
    )
    ...
    review_invariants = {
        ...
        "owner_evidence_regression_rule_boundary_preserved": owner_rule_boundary,
        ...
        "stage096_gate_only_opens_after_review": False,
    }
    review_valid = all(phase_results.values()) and all(
        value
        for key, value in review_invariants.items()
        if key != "stage096_gate_only_opens_after_review"
    )
    review_invariants["stage096_gate_only_opens_after_review"] = review_valid
    ...
    if not owner_rule_boundary:
        failure_reasons.append("OWNER_EVIDENCE_REGRESSION_RULE_BOUNDARY_MISMATCH")
    ...
    return {
        ...
        "stage094_review_evidence_declared": True,
        "stage095_started": True,
        "phase1_completed": True,
        "phase2_completed": True,
        "phase3_completed": True,
        "phase4_completed": True,
        "stage095_review_started": True,
        "whole_stage_review_performed": False,
        "stage096_started": False,
        ...
        **{field: False for field in REVIEW_RUNTIME_FALSE_FIELDS},
        "runtime_boundary": {
            ...
            if field != "stage095_review_runtime_executed"
        },
        "rollback": {
            "return_to": P4_PASS_RESULT,
            "preserve_stage095_phase1_to_phase4_evidence": True,
            "preserve_stage094_review_evidence": True,
