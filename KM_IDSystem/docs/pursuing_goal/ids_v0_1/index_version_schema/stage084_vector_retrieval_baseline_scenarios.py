"""Stage084 P3 的纯内存向量检索基线受控场景重放。

模块只重放 Stage084 P2 的五条固定控制投影，验证八类检索、过滤、Top-K、
排序解释、结果有效性与旧索引版本轨迹的控制边界。材料牌号、设备型号、
标准号和语义相似仅是场景类别；模型、版本、维度与相似度度量均为控制标签，
模块不读取或计算任何业务内容。
"""
...
SCHEMA_VERSION = "ids.stage084.vector_retrieval_baseline.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_VECTOR_RETRIEVAL_BASELINE_SCENARIOS"
PASS_RESULT = "PASS_VECTOR_RETRIEVAL_BASELINE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_VECTOR_RETRIEVAL_BASELINE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
SCENARIOS = (
    ...
    },
    {
        ...
        "phase2_control_scenarios": ("vector_year_filter_reference_only",),
        "expected_query_kind": "keyword",
        "expected_filter_reference_count": 1,
        "expected_semantic_similarity_state": (
            "CONTROL_SEMANTIC_SIMILARITY_NOT_REQUESTED"
...
def build_vector_retrieval_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2，并在内存中验证固定的 Stage084 P3 控制场景。"""
    ...
    return {
        ...
        "next_gate": NEXT_GATE if valid else "IDS-STAGE084-P3-GATE",
        ...
        **runtime_closed_flags,
        "chinese_feedback": [
            ...
            "材料牌号、设备型号、标准号与语义相似只作为场景类别，模型、版本、维度与相似度度量只作为控制标签。",
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name(
        "stage084_vector_retrieval_baseline_control_slice.py"
    )
    spec = importlib.util.spec_from_file_location("stage084_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage084 P2 vector retrieval slice")
...
def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_vector_retrieval_control_slice
...
def _evaluate_scenario(
    ...
    vector_baseline_declared = (
        ...
        and bool(embedding_model_ref)
        and bool(embedding_model_version_ref)
        and bool(vector_dimension_ref)
        and bool(similarity_metric_ref)
    ...
    top_k_declared = (
        bool(requested_top_k_ref) and CONTROL_PREFIX in requested_top_k_ref
    )
