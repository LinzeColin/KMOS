"""Stage108 P4：报告快照的 metadata-only 交付证据。

该模块仅从 Stage108 P3 的固定、非业务、reference-only 场景派生交付控制
记录。它描述报告样例、报告快照、质量评分、影响分析、模板限制、人工确认、
重新生成与撤回的未来控制条件；它不读取真实资料、报告、PDF 或证据账本，
...
SCHEMA_VERSION = "ids.stage108.report_snapshot.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_SNAPSHOT_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_REPORT_SNAPSHOT_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REPORT_SNAPSHOT_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE108-P4-GATE"
NEXT_GATE = "IDS-STAGE108-REVIEW-GATE"
...
P3_CURRENT_GATE = "IDS-STAGE108-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE108-P4-GATE"
P2_CONTROL_PREFIX = ":control:stage108-p2:"
DELIVERY_PREFIX = ":control:stage108-p4:"
...
P3_PHASE2_INPUT_FIELD_COUNT = 26
P3_PHASE2_PHASE1_REFERENCE_FIELD_COUNT = 24
P3_PHASE2_PROJECTION_GROUP_COUNT = 4
P3_PHASE2_PROJECTION_FIELD_COUNT_PER_REQUEST = 82
P3_PHASE2_PROJECTION_FIELD_COUNT_TOTAL = 410
P3_SCENARIO_FIELD_CHECK_COUNT = 195
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage108_phase4_runtime_executed",
)
ZERO_COUNTER_FIELDS = (
...
def _load_phase3_module() -> Any:
    module_path = Path(__file__).with_name("stage108_report_snapshot_controlled_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage108_phase3_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage108 P3 报告快照专项场景")
...
def _template_and_whitebox_record(scenario: Mapping[str, Any], index: int) -> dict[str, Any]:
    return {
        ...
        "scenario_id": scenario["scenario_id"],
        "report_id_ref": scenario["report_id_ref"],
        "report_template_limit_ref": scenario["report_template_limit_ref"],
    ...
    return [
        {
            ...
            "rollback_target_ref": _delivery_ref("stage108-p3-controlled-scenarios"),
            "rollback_target_result": P3_PASS_RESULT,
            "predecessor_phase_ref": _delivery_ref("stage108-p3"),
...
def _validate_delivery_groups(report: Mapping[str, Any]) -> Optional[str]:
    ...
    for record in report["report_quality_score_control_records"]:
        if (
            ...
            or not _is_control_reference(record["report_quality_score_ref"])
    ...
    for record in report["report_impact_analysis_control_records"]:
        if (
            ...
            or record["actual_report_status_update_performed"] is not False
        ):
    ...
    for record in report["report_template_and_whitebox_confirmation_control_records"]:
        if (
            not _is_control_reference(record["report_id_ref"])
            or not _is_control_reference(record["report_template_limit_ref"])
...
def build_report_snapshot_phase4_delivery_report(
    ...
    try:
        ...
        executor = (
            ...
            else phase3_module.build_report_snapshot_phase3_report
        )
