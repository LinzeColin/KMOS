"""Stage081 P2 的纯内存影子索引控制切片。

模块只接受五条固定、非业务、reference-only 控制请求，并在内存中投影
索引版本、候选后台构建、影子隔离、冒烟门、活动指针、未应用切换和未应用
回滚资格。它不读取业务资料，不连接数据库，不构建或查询真实索引，也不
选择或调用模型。
"""
...
SCHEMA_VERSION = "ids.stage081.shadow_index.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_SHADOW_INDEX"
CONTROL_ADAPTER_VERSION = "ids.shadow_index.control_adapter.v0_1.stage081.p2"
CONTROL_FIELDS = ("shadow_index_control_requests",)
...
CONTROL_SCENARIO_CONFIGURATION = {
    ...
    "hybrid_shadow_rollback_candidate_retains_previous": {
        "index_kind": "hybrid",
        "lifecycle_state": "SHADOW_ROLLBACK_CANDIDATE",
        "build_state": "CONTROL_BACKGROUND_BUILD_COMPLETE_REFERENCE_ONLY",
        "smoke_test_status": "PASSED",
...
def build_control_request(scenario: str) -> dict[str, Any]:
    """构造一条固定、无业务含义的 Stage081 P2 控制请求。"""
...
def execute_shadow_index_control_slice(
    control_input: Mapping[str, object] | object,
) -> dict[str, Any]:
    """在内存中投影固定影子索引控制记录，并拒绝任何其他输入。"""
    ...
    for scenario, request in zip(CONTROL_SCENARIOS, requests):
        ...
        rollback_eligible = (
            all(_rollback_conditions(request).values())
    ...
    result = {
        ...
        "input_accepted": True,
        "execution_state": "COMPLETED_IN_MEMORY_SHADOW_INDEX_CONTROL_SLICE",
