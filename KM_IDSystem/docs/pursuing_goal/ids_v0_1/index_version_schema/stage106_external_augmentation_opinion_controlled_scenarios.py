"""Stage106 P3：外部增强意见章节的纯内存专项异常场景验证。

模块只重放 Stage106 P2 的固定、非业务、reference-only 控制投影。它验证
...
SCHEMA_VERSION = "ids.stage106.external_augmentation_opinion.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EXTERNAL_AUGMENTATION_OPINION_SCENARIOS"
CURRENT_GATE = "IDS-STAGE106-P3-GATE"
NEXT_GATE = "IDS-STAGE106-P4-GATE"
PASS_RESULT = "PASS_EXTERNAL_AUGMENTATION_OPINION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EXTERNAL_AUGMENTATION_OPINION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
P2_EXECUTION_STATE = "PASS_IN_MEMORY_EXTERNAL_AUGMENTATION_OPINION_CONTROL_SLICE_RUNTIME_DISABLED"
P2_CONTROL_PREFIX = ":control:stage106-p2:"
P2_CONTROL_FIELDS = ("external_augmentation_opinion_control_requests",)
P2_CONTROL_REQUEST_COUNT = 5
P2_INPUT_FIELD_COUNT = 30
P2_PHASE1_REFERENCE_FIELD_COUNT = 27
P2_PHASE2_ADDED_REFERENCE_FIELD_COUNT = 1
P2_PROJECTION_GROUP_COUNT = 4
P2_PROJECTION_FIELD_COUNT_PER_REQUEST = 74
P2_PROJECTION_FIELD_COUNT_TOTAL = 370
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage106_phase3_runtime_executed",
)
...
def _load_phase2_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage106_phase2_slice", module_path)
...
def _failure_for_projection(
    ...
    if (
        ...
        return "CRITICAL_CONCLUSION_EVIDENCE_BINDING_DRIFT"
    if (
        section.get("external_augmentation_opinion_chapter_output_state")
        != "CONTROL_EXTERNAL_AUGMENTATION_OPINION_CHAPTER_REFERENCE_ONLY_NOT_RENDERED"
        or section.get("pdf_citation_source_and_page_display_state")
        != "CONTROL_FUTURE_PDF_CITATION_SOURCE_AND_PAGE_REQUIRED_NOT_RENDERED"
        or section.get("actual_report_evidence_binding_performed") is not False
        or section.get("actual_chapter_output_written") is not False
    ):
    ...
    if (
        lifecycle.get("report_lifecycle_control_state")
        != "CONTROL_REPORT_LIFECYCLE_REFERENCE_ONLY_NOT_EXECUTED"
        or lifecycle.get("report_status_impact_control_state")
...
def _scenario_record(
    ...
    return {
        ...
        "actual_external_augmentation_opinion_generated": external[
            "actual_external_augmentation_opinion_generated"
        ],
...
def build_external_augmentation_opinion_phase3_report(
    ...
    for index, definition in enumerate(SCENARIO_DEFINITIONS):
        ...
        section = _projection_record(
            phase2_result, "report_evidence_binding_and_chapter", index
        )
        snapshot = _projection_record(phase2_result, "generation_snapshot", index)
        lifecycle = _projection_record(
            phase2_result, "report_lifecycle_and_export_audit", index
        )
