"""Stage081 P3 的纯内存影子索引受控场景重放。

模块仅重放 Stage081 P2 的固定控制输入与控制投影，用于验证构建/冒烟/切换
失败、回滚候选、后台构建时的检索隔离及版本可见性。它不读取业务资料，不
创建或查询真实索引，不执行并发检索，也不写入 Operations 或报告快照。
"""
...
SCHEMA_VERSION = "ids.stage081.shadow_index.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_SHADOW_INDEX_SCENARIOS"
PASS_RESULT = "PASS_SHADOW_INDEX_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_SHADOW_INDEX_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
NEXT_GATE = "IDS-STAGE081-P4-GATE"
P2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_SHADOW_INDEX_CONTROL_SLICE"
P2_SCENARIOS = (
    "fulltext_shadow_smoke_passed_switch_candidate",
    ...
    "hybrid_shadow_rollback_candidate_retains_previous",
)
...
SCENARIOS = (
    {
        ...
        "expected_rollback_eligibility": "CONTROL_NOT_ELIGIBLE_OR_NOT_REQUESTED",
    ...
    },
    {
        ...
        "expected_rollback_eligibility": "CONTROL_ELIGIBLE_REFERENCE_ONLY",
        "expected_concurrent_retrieval_isolated": False,
    ...
    },
    {
        ...
        "phase2_control_scenario": "hybrid_shadow_rollback_candidate_retains_previous",
        "expected_build_state": "CONTROL_BACKGROUND_BUILD_COMPLETE_REFERENCE_ONLY",
        "expected_smoke_test_status": "PASSED",
    ...
    },
    {
        ...
        "phase2_control_scenario": "fulltext_shadow_smoke_passed_switch_candidate",
        "expected_build_state": "CONTROL_BACKGROUND_BUILD_COMPLETE_REFERENCE_ONLY",
        "expected_smoke_test_status": "PASSED",
...
def build_shadow_index_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2，并在内存中评估固定的 Stage081 P3 控制场景。"""
    ...
    return {
        ...
        * len(SCENARIO_RESULT_FIELDS),
        ...
        "stage080_review_evidence_declared": True,
...
def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_shadow_index_control_slice
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage081_shadow_index_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage081_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage081 P2 shadow index slice")
...
def _phase2_shape_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    switch_records = _as_records(result.get("switch_control_projections"))
    if (
        ...
        is not True
        or [record.get("control_scenario") for record in switch_records]
        != list(P2_SCENARIOS)
    ):
        return False
...
def _build_control_views(
    ...
    *,
    ...
    for scenario, record, pointer in zip(P2_SCENARIOS, records, pointers):
        marker = f":control:stage081-p2:{scenario}"
        projections.append(
            {
...
def _evaluate_scenario(
    ...
    build = _record_at(result.get("building_and_shadow_control_projections"), index)
    pointer = _record_at(result.get("active_pointer_control_projections"), index)
    smoke_input = _record_at(result.get("smoke_test_input_control_projections"), index)
    smoke_output = _record_at(result.get("smoke_test_output_control_projections"), index)
    switch = _record_at(result.get("switch_control_projections"), index)
    rollback = _record_at(result.get("rollback_request_control_projections"), index)
    ...
    old_active_continues = (
        ...
        and build.get("candidate_index_version_ref") != active_before
        and build.get("shadow_index_ref") != active_before
        and switch.get("switch_applied") is False
        and active_after == active_before
        and result.get("all_candidate_versions_are_isolated") is True
    ...
    rollback_target_is_retained_previous_active = (
        ...
        == pointer.get("previous_active_index_version_ref")
        and rollback.get("current_active_index_version_ref") == active_before
        and rollback.get("previous_active_index_version_ref") != active_before
        and rollback.get("retention_window_state") == "CONTROL_RETENTION_WINDOW_VALID"
        and rollback.get("rollback_applied") is False
    )
    smoke_input_matches_candidate = (
        smoke_input.get("candidate_index_version_ref")
        == index_record.get("index_version")
        and smoke_input.get("active_index_version_ref") == active_before
        and smoke_input.get("shadow_index_ref") == build.get("shadow_index_ref")
        and smoke_input.get("chunk_count") == 0
    )
    expectation_met = (
        phase2_shape_preserved
        and phase2_side_effect_free
        and index_record.get("index_version")
        == build.get("candidate_index_version_ref")
        and index_record.get("index_kind") == pointer.get("index_kind")
        and smoke_input_matches_candidate
        and build.get("build_state") == scenario["expected_build_state"]
        and smoke_output.get("smoke_test_status")
        == scenario["expected_smoke_test_status"]
        and switch.get("switch_outcome") == scenario["expected_switch_outcome"]
        and rollback.get("rollback_eligibility")
        == scenario["expected_rollback_eligibility"]
        and old_active_continues
        ...
        and rollback_target_is_retained_previous_active
    ...
    return {
        ...
        "referenced_candidate_index_version_ref": build.get(
            "candidate_index_version_ref"
        ),
        "referenced_shadow_index_ref": build.get("shadow_index_ref"),
        "referenced_smoke_test_ref": smoke_output.get("smoke_test_ref"),
        "referenced_switch_ref": switch.get("switch_record_ref"),
        "referenced_rollback_request_ref": rollback.get("rollback_request_ref"),
        ...
        "observed_build_state": build.get("build_state"),
        "observed_smoke_test_status": smoke_output.get("smoke_test_status"),
        "observed_switch_outcome": switch.get("switch_outcome"),
        "observed_rollback_eligibility": rollback.get("rollback_eligibility"),
...
def _reference_value_is_control_only(value: object) -> bool:
    return (
        ...
        and ":control:stage081-p2:" in value
