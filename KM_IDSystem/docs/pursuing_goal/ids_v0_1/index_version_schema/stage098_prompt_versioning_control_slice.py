"""Stage098 P2 的纯内存 Prompt 版本化受控切片。

模块只投影自身定义的固定、非业务、reference-only 控制输入。它承接
Stage098 P1 静态 Prompt 版本化合同与 Stage097 Review 已复审回答合同，
不读取真实资料、检索结果、提示词、模型配置或 evidence，不连接数据库，
...
SCHEMA_VERSION = "ids.stage098.prompt_versioning.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_PROMPT_VERSIONING"
CONTROL_ADAPTER_VERSION = "ids.prompt_versioning.control_adapter.v0_1.stage098.p2"
CONTROL_PREFIX = ":control:stage098-p2:"
CONTROL_FIELDS = ("prompt_versioning_control_requests",)
PHASE1_PROMPT_VERSIONING_CONTROL_REF = (
    ":control:stage098-p2:stage098-phase1-prompt-versioning-contract:reference-only"
)
STAGE097_REVIEW_ANSWER_CONTRACT_CONTROL_REF = (
    ":control:stage098-p2:stage097-reviewed-answer-contract:reference-only"
)
...
def _rejected_result() -> dict[str, Any]:
    ...
    return {
        ...
        "execution_state": "REJECTED_IN_MEMORY_PROMPT_VERSIONING_CONTROL_SLICE",
...
def execute_prompt_versioning_control_slice(
    ...
    return {
        ...
        "control_adapter_version": CONTROL_ADAPTER_VERSION,
        "input_accepted": True,
        "execution_state": "PASS_IN_MEMORY_PROMPT_VERSIONING_CONTROL_SLICE_RUNTIME_DISABLED",
        "failure_state": None,
