"""Stage098 P3 Prompt 版本化的纯内存异常场景验证。

本模块只重放 Stage098 P2 自身生成的六条固定、非业务、reference-only
控制投影。它验证文档内指令无法覆盖 IDS 规则、无内部依据不伪装为内部经验，
以及三类高风险输出不自动进入最终结论；不读取真实资料、提示词、回答或检索
结果，不执行模型、Agent、OVH 或生产运行，也不创建持久化记录。
"""
...
SCHEMA_VERSION = "ids.stage098.prompt_versioning.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_PROMPT_VERSIONING_SCENARIOS"
CURRENT_GATE = "IDS-STAGE098-P3-GATE"
NEXT_GATE = "IDS-STAGE098-P4-GATE"
PASS_RESULT = "PASS_PROMPT_VERSIONING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAILED_PROMPT_VERSIONING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P2_SCHEMA_VERSION = "ids.stage098.prompt_versioning.phase2.v1"
P2_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_PROMPT_VERSIONING"
P2_EXECUTION_STATE = "PASS_IN_MEMORY_PROMPT_VERSIONING_CONTROL_SLICE_RUNTIME_DISABLED"
P2_CONTROL_PREFIX = ":control:stage098-p2:"
...
P2_PROJECTION_SPECS = (
    (
        "prompt_versioning_and_answer_contract_binding",
        "PROMPT_VERSIONING_AND_ANSWER_CONTRACT_BINDING_FIELDS",
    ),
    (
...
ZERO_COUNTER_FIELDS = (
    "actual_scenario_business_use_count",
    "actual_query_or_index_access_count",
    "actual_prompt_or_answer_access_count",
    "actual_model_call_count",
    "actual_model_token_count",
    "actual_agent_execution_count",
    "actual_ovh_deployment_count",
    "actual_production_activation_count",
    "actual_persistent_write_count",
    "actual_github_upload_count",
    "actual_push_count",
)
SCENARIO_FIELDS = (
    "scenario_id",
...
CONTROL_VIEW_FIELDS = {
    "prompt_versioning_binding_control_view": (
        "scenario_id",
        "phase2_control_scenario",
        "prompt_version_ref",
        "model_provider_ref",
...
SCENARIO_DEFINITIONS = (
    {
        "scenario_id": "external_augmentation_preserves_source_type_prompt_version_control",
        ...
        "expected_output_permission_state": "CONTROL_INFORMATIONAL_OUTPUT_REMAINS_REFERENCE_ONLY",
        "human_handling_code": "BUSINESS_LINE_WHITEBOX_REVIEW_EXTERNAL_AUGMENTATION_SOURCE_TYPE",
    },
    {
        "scenario_id": "evidence_gap_cannot_masquerade_as_internal_experience_prompt_version_control",
        ...
        "expected_prompt_injection_defense_state": "CONTROL_PROMPT_INJECTION_DEFENSE_DECLARED",
        "expected_output_permission_state": "CONTROL_EVIDENCE_GAP_REQUIRES_HUMAN_WHITEBOX_CONFIRMATION",
        "human_handling_code": "BUSINESS_LINE_WHITEBOX_REVIEW_EVIDENCE_GAP_BEFORE_BUSINESS_USE",
    },
    {
        "scenario_id": "retrieval_document_cannot_override_ids_rule_prompt_version_control",
    ...
    {
        "scenario_id": "high_risk_engineering_advice_requires_whitebox_confirmation_prompt_version_control",
    ...
    {
        "scenario_id": "contract_commitment_requires_whitebox_confirmation_prompt_version_control",
    ...
    {
        "scenario_id": "production_writeback_requires_whitebox_confirmation_prompt_version_control",
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage098_prompt_versioning_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage098_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage098 P2 control slice")
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    if (
        ...
        != ("prompt_versioning_control_requests",)
        or len(getattr(phase2_module, "INPUT_FIELDS", ())) != 23
        ...
        or result.get("control_projection_field_total_per_request") != 41
        or result.get("control_projection_field_total") != 246
    ):
        return False
...
def _phase2_control_references_are_opaque(result: Mapping[str, Any]) -> bool:
    for scenario in P2_CONTROL_SCENARIOS:
        ...
        if records is None:
            return False
        binding = records["prompt_versioning_and_answer_contract_binding"]
        ...
        if not all(
            ...
            for field in (
                "stage098_phase1_prompt_versioning_contract_ref",
                "stage097_review_answer_contract_ref",
                "query_ref",
                "index_version_ref",
                "prompt_version_ref",
                "model_provider_ref",
...
def _scenario_from_phase2(
    ...
    scenario = {
        ...
        "external_augmentation_display_ref": source["external_augmentation_display_ref"],
...
def _scenario_failure_state(
    ...
    if (
        ...
        or scenario["display_does_not_replace_source_type_state"]
...
def build_prompt_versioning_phase3_report(
    phase2_executor: Optional[Phase2Executor] = None,
) -> dict[str, Any]:
    """重放 P2 的固定控制投影，返回未持久化的 P3 验证报告。"""
    ...
    phase2_result = (
        ...
        else phase2_module.execute_prompt_versioning_control_slice(control_input)
    )
    ...
    for definition in SCENARIO_DEFINITIONS:
        ...
        if scenario is None:
            return _base_report(False, "PHASE3_PROMPT_VERSIONING_CONTROLLED_SCENARIOS_REJECTED")
