...
SCHEMA_VERSION = "ids.stage114.review_workflow.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REVIEW_WORKFLOW"
CONTROL_ADAPTER_VERSION = "stage114-p2-control-slice-v1"
PASS_RESULT = "PASS_IN_MEMORY_REVIEW_WORKFLOW_CONTROL_SLICE_RUNTIME_DISABLED"
REJECTED_RESULT = "REJECTED_IN_MEMORY_REVIEW_WORKFLOW_CONTROL_SLICE"
CONTROL_PREFIX = ":control:stage114-p2:"
CONTROL_FIELDS = ("review_workflow_control_requests",)
...
CONTROL_SCENARIO_CONFIGURATION = {
    "low_ocr_pending_review_submit_reference_only": {
        ...
        "fixed_workflow_action": "submit_for_review",
    },
    "source_conflict_confirm_reference_only": {
        "binding_mode": "evidence_gap",
        "fixed_review_status": "confirmed",
        "fixed_workflow_action": "confirm",
    },
    "parsing_failure_needs_more_material_reference_only": {
        "binding_mode": "evidence_gap",
        "fixed_review_status": "needs_more_material",
        "fixed_workflow_action": "request_more_material",
    },
    "evidence_risk_reject_reference_only": {
        "binding_mode": "evidence_id",
        "fixed_review_status": "rejected",
        "fixed_workflow_action": "reject",
    },
    "external_augmentation_archive_reference_only": {
        "binding_mode": "evidence_id",
        "fixed_review_status": "archived",
        "fixed_workflow_action": "archive",
    },
}
...
HUMAN_REASON_MESSAGES = {
    ...
    "evidence_risk_reject_reference_only": (
        "证据风险：已形成拒绝动作控制投影，证据等级和报告影响由业务线白箱确认。"
    ),
    "external_augmentation_archive_reference_only": (
...
WORKFLOW_QUEUE_UI_DYNAMIC_FIELDS = (
    "review_status_chinese_control_label",
    "workflow_action_chinese_control_label",
    "review_reason_chinese_control_message",
    "review_transition_control_label",
)

WORKFLOW_QUEUE_UI_STATES = {
    "review_workflow_control_state": (
        "CONTROL_REVIEW_WORKFLOW_REFERENCE_ONLY_NOT_EXECUTED"
    ),
    "review_queue_control_state": (
        "CONTROL_REVIEW_QUEUE_REFERENCE_ONLY_NOT_CREATED"
    ),
    "review_ui_control_state": (
        "CONTROL_CHINESE_REVIEW_UI_REFERENCE_ONLY_NOT_RENDERED"
    ),
    "fixed_review_status_catalog": FIXED_REVIEW_STATUSES,
    "fixed_workflow_action_catalog": FIXED_WORKFLOW_ACTIONS,
...
EVIDENCE_RISK_AND_REPORT_STATUS_WRITEBACK_DYNAMIC_FIELDS = (
    "review_workflow_writeback_control_label",
)
...
def _control_request(scenario: str) -> dict[str, Optional[str]]:
    ...
    request: dict[str, Optional[str]] = {
        ...
        "fixed_workflow_action_control_value": configuration[
            "fixed_workflow_action"
        ],
    }
...
def _project(request: Mapping[str, Optional[str]]) -> dict[str, dict[str, Any]]:
    ...
    status = str(request["fixed_review_status_control_value"])
    action = str(request["fixed_workflow_action_control_value"])
    ...
    for prefix, input_fields, state_values in PROJECTION_SPECS:
        ...
        if prefix == "review_workflow_queue_and_ui":
            record.update(
                {
                    "review_status_chinese_control_label": STATUS_CHINESE_LABELS[
                        status
                    ],
                    "workflow_action_chinese_control_label": ACTION_CHINESE_LABELS[
                        action
                    ],
                    "review_reason_chinese_control_message": HUMAN_REASON_MESSAGES[
                        scenario
                    ],
                    "review_transition_control_label": _control_ref(
                        "review-transition", scenario
                    ),
                }
            )
        if prefix == "evidence_risk_and_report_status_writeback":
            record["review_workflow_writeback_control_label"] = _control_ref(
                "review-workflow-writeback", scenario
            )
...
def project_review_workflow_control_slice(
