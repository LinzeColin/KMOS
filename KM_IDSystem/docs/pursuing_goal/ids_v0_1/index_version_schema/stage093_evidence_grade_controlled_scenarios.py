"""Stage093 P3 的纯内存证据可信等级异常场景控制验证。

模块只重放 Stage093 P2 的固定、非业务、reference-only 控制投影。它不读取
来源资料、检索结果或真实证据账本，不连接数据库，不分配真实可信等级，不执行
...
SCHEMA_VERSION = "ids.stage093.evidence_grade.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_GRADE_EXCEPTION_SCENARIOS"
PASS_RESULT = "PASS_EVIDENCE_GRADE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_GRADE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CURRENT_GATE = "IDS-STAGE093-P3-GATE"
NEXT_GATE = "IDS-STAGE093-P4-GATE"
P2_EXECUTION_STATE = "CONTROL_EVIDENCE_GRADE_PROJECTIONS_DECLARED"
...
SCENARIO_DEFINITIONS = (
    {
        "scenario_id": "no_internal_evidence_grade_control",
    ...
    },
    {
        ...
        "phase2_control_scenario": "low_trust_evidence_grade_degraded_reference_only",
    ...
    },
    {
        ...
        "phase2_control_scenario": "expired_evidence_grade_degraded_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "C",
    ...
    },
    {
        ...
        "phase2_control_scenario": "conflict_evidence_grade_degraded_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "B",
    ...
    },
    {
        ...
        "phase2_control_scenario": "revoked_evidence_grade_degraded_reference_only",
        "expected_evidence_id_present": True,
        "expected_grade": "A",
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage093_evidence_grade_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage093_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage093 P2 evidence grade slice")
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    ...
    if (
        ...
        != ("evidence_grade_control_requests",)
...
def _phase2_records(
    ...
    for prefix, _field_constant in P2_PROJECTION_SPECS:
        ...
        if not isinstance(record, Mapping):
            raise ValueError(f"invalid Stage093 P2 {prefix} projection")
...
def _phase2_semantic_failure(result: Mapping[str, Any]) -> str | None:
    ...
    low_trust = _phase2_records(
        result, "low_trust_evidence_grade_degraded_reference_only"
    )
    conflict = _phase2_records(
        result, "conflict_evidence_grade_degraded_reference_only"
    )
    expired = _phase2_records(
        result, "expired_evidence_grade_degraded_reference_only"
    )
    revoked = _phase2_records(
        result, "revoked_evidence_grade_degraded_reference_only"
    )
    malicious = _phase2_records(
        result, "suspected_poisoning_grade_quarantined_reference_only"
    )
    if (
        no_internal["evidence_grade_schema_binding"].get("evidence_id_ref") is not None
        or not _control_ref(
            no_internal["evidence_grade_schema_binding"].get("evidence_gap_ref")
        )
        or not _control_ref(
    ...
    if (
        ...
        != "CONTROL_REVOCATION_ROUTE_DECLARED"
    ...
    for scenario in P2_CONTROL_SCENARIOS:
        ...
        assignment = records["evidence_grade_assignment"]
        expected_evidence_id = scenario != P2_CONTROL_SCENARIOS[0]
        assignment_state = assignment.get("grade_assignment_state")
        if (
        ...
        if (
            not isinstance(assignment_state, str)
            or not assignment_state.startswith("CONTROL_GRADE_ASSIGNMENT_ROUTE_")
            or not assignment_state.endswith("OWNER_RULE_REQUIRED")
        ):
            return "EVIDENCE_GRADE_ASSIGNMENT_RULE_UNAUTHORIZED"
        if (
...
def _scenario_expectation_met(
    ...
    schema = records["evidence_grade_schema_binding"]
    relation = records["evidence_grade_relation"]
    ...
    scenario_specific = scenario_specific and (
        revocation.get("revocation_state") == "CONTROL_REVOCATION_ROUTE_DECLARED"
        and _control_ref(revocation.get("affected_report_ref"))
        and report_impact.get("report_status_impact_state")
        == "CONTROL_REPORT_STATUS_REFERENCE_DEGRADED_REVOKED"
        if scenario_id == "revoked_evidence_grade_report_impact_control"
        else True
    )
    scenario_specific = scenario_specific and (
        defense.get("defense_state") == "CONTROL_POISONING_DEFENSE_ROUTE_DECLARED"
        if scenario_id == "malicious_evidence_grade_quarantine_control"
        else True
    )
    scenario_specific = scenario_specific and (
    ...
    return (
        ...
        and isinstance(assignment_state, str)
        and assignment_state.endswith("OWNER_RULE_REQUIRED")
...
def build_evidence_grade_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制投影并验证 Stage093 P3 固定异常场景。"""

    try:
        phase2_module = _load_phase2_module()
        executor = phase2_executor or phase2_module.execute_evidence_grade_control_slice
