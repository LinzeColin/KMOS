"""Stage085 P3 的纯内存元数据过滤受控场景重放。

模块只重放 Stage085 P2 的六条固定控制投影，验证关键词、材料牌号、设备型号、
...
SCHEMA_VERSION = "ids.stage085.metadata_filter.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_METADATA_FILTER_SCENARIOS"
PASS_RESULT = "PASS_METADATA_FILTER_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_METADATA_FILTER_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
def build_metadata_filter_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2，并在内存中验证固定的 Stage085 P3 控制场景。"""
    ...
    return {
        ...
        "next_gate": NEXT_GATE if valid else "IDS-STAGE085-P3-GATE",
        ...
        **runtime_closed_flags,
        "chinese_feedback": [
            ...
            "六维过滤、Top-K、排序解释、有效性和旧索引版本轨迹只核验控制引用形状，未过滤、排序、选择或写入 trace。",
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage085_metadata_filter_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage085_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage085 P2 metadata filter slice")
...
def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_metadata_filter_control_slice
