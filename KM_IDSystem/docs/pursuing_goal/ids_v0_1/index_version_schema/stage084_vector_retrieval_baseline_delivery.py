"""Stage084 P4 的纯内存向量检索基线交付证据。

模块只复用 Stage084 P3 的八个受控场景，派生 metadata-only 的检索样例、
...
SCHEMA_VERSION = "ids.stage084.vector_retrieval_baseline.phase4.delivery.v1"
RECORD_KIND = "VECTOR_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_VECTOR_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_VECTOR_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE084-P4-GATE"
...
def build_vector_retrieval_phase4_delivery_report(
    ...
    return {
        ...
        "stage083_review_evidence_declared": True,
        ...
        **runtime_flags,
        "chinese_feedback": [
            ...
            "向量模型、版本、维度与相似度度量只保留不透明控制引用，未生成 embedding 或计算相似度。",
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_vector_retrieval_phase3_report()
...
def _load_phase3_module() -> Any:
    path = Path(__file__).with_name("stage084_vector_retrieval_baseline_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage084_phase3_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage084 P3 vector retrieval scenarios")
...
def _phase3_report_is_valid(module: Any, report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("stage083_review_evidence_declared") is True
        and report.get("stage084_started") is True
        ...
        and report.get("stage085_started") is False
    ...
    return [
        {
            ...
            "embedding_model_version_ref": _text(item, "embedding_model_version_ref"),
    ...
    return [
        {
            ...
            "rollback_target_ref": f"stage084-p3-control-boundary{CONTROL_PREFIX}{scenario}",
        ...
        for instruction_id, scenario, parameter_component in controls
    ]
