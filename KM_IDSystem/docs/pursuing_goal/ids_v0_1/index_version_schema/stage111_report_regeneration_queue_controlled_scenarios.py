"""Stage111 P3：报告重新生成队列的纯内存专项异常场景验证。

模块只重放 Stage111 P2 的固定、非业务、reference-only 控制投影。它验证关键
结论始终关联 evidence_id_ref 或 evidence_gap_ref，资料撤回、证据降级、证据冲突
和索引版本变化保持未来报告状态与队列复核形状，外部增强保持来源分离并受业务线
白箱确认门禁约束。模块不读取真实资料、报告、PDF 或证据账本，不调用模型、Agent、
OVH 或生产服务，也不写入数据库、审计或持久化状态。
"""
...
SCHEMA_VERSION = "ids.stage111.report_regeneration_queue.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_REPORT_REGENERATION_QUEUE_SCENARIOS"
CURRENT_GATE = "IDS-STAGE111-P3-GATE"
NEXT_GATE = "IDS-STAGE111-P4-GATE"
PASS_RESULT = "PASS_REPORT_REGENERATION_QUEUE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REPORT_REGENERATION_QUEUE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
P2_EXECUTION_STATE = "PASS_IN_MEMORY_REPORT_REGENERATION_QUEUE_CONTROL_SLICE_RUNTIME_DISABLED"
P2_CONTROL_PREFIX = ":control:stage111-p2:"
P2_CONTROL_FIELDS = ("report_regeneration_queue_control_requests",)
P2_CONTROL_REQUEST_COUNT = 5
P2_INPUT_FIELD_COUNT = 32
P2_PHASE1_REFERENCE_FIELD_COUNT = 30
P2_PROJECTION_GROUP_COUNT = 4
P2_PROJECTION_FIELD_COUNT_PER_REQUEST = 88
P2_PROJECTION_FIELD_COUNT_TOTAL = 440
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage111_phase3_runtime_executed",
)
...
ZERO_COUNTER_FIELDS = (
    ...
    "actual_queue_entry_creation_count",
...
SCENARIO_DEFINITIONS = (
    {
        "scenario_id": "cited_material_update_evidence_id_binding_control",
        "scenario_category": "CITED_MATERIAL_UPDATE_EVIDENCE_BINDING_CONTROL",
        ...
        "evidence_conflict_impact_required": False,
    ...
    {
        "scenario_id": "source_withdrawal_evidence_gap_report_status_control",
        "scenario_category": "SOURCE_WITHDRAWAL_REPORT_STATUS_AND_QUEUE_CONTROL",
    ...
    {
        "scenario_id": "evidence_downgrade_evidence_id_report_status_control",
        "scenario_category": "EVIDENCE_DOWNGRADE_REPORT_STATUS_AND_QUEUE_CONTROL",
    ...
    {
        "scenario_id": "evidence_conflict_evidence_gap_report_status_control",
        "scenario_category": "EVIDENCE_CONFLICT_REPORT_STATUS_AND_QUEUE_CONTROL",
        "phase2_control_scenario": P2_CONTROL_SCENARIOS[3],
        "expected_binding_mode": "evidence_gap",
        "source_withdrawal_impact_required": False,
        "evidence_downgrade_impact_required": False,
        "evidence_conflict_impact_required": True,
        "index_version_change_impact_required": False,
        "business_line_whitebox_handling_code": (
            "BUSINESS_LINE_WHITEBOX_REVIEW_EVIDENCE_CONFLICT_REPORT_STATUS"
        ),
    },
    {
        "scenario_id": "index_version_change_evidence_id_report_status_control",
        "scenario_category": "INDEX_VERSION_CHANGE_REPORT_STATUS_AND_QUEUE_CONTROL",
...
FAILURE_STATES = (
    "PHASE2_CONTROL_REPLAY_UNAVAILABLE",
    "PHASE2_CONTROL_SHAPE_MISMATCH",
    "PHASE2_SIDE_EFFECT_BOUNDARY_BREACH",
    "PHASE2_CONTROL_INPUT_NOT_OPAQUE",
    ...
    "EXTERNAL_AUGMENTATION_SOURCE_TYPE_DRIFT",
    "EXTERNAL_AUGMENTATION_REPRESENTED_AS_INTERNAL_EVIDENCE",
    "EXTERNAL_AUGMENTATION_REPLACES_EVIDENCE_BINDING",
    "EXTERNAL_AUGMENTATION_CLOSES_EVIDENCE_GAP",
    "BUSINESS_WHITEBOX_CONFIRMATION_GATE_MISSING",
    "HUMAN_CONFIRMATION_OR_FINAL_CONCLUSION_BOUNDARY_BREACH",
)

CHINESE_FEEDBACK = (
    "关键结论控制场景保留 evidence_id 或 evidence_gap 的严格二选一关联。",
    "资料撤回、证据降级、证据冲突和索引版本变化保持未来报告状态与队列复核控制。",
    "外部增强保留 external_public_reference 与 model_reasoning 的底层来源，不能写成内部项目依据。",
    "业务线白箱人工确认和最终结论保持未记录、未发布，等待后续授权阶段。",
)
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name(
        "stage111_report_regeneration_queue_control_slice.py"
    )
    spec = importlib.util.spec_from_file_location(
        "stage111_phase2_report_regeneration_queue_slice", module_path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage111 P2 报告重新生成队列受控最小切片")
...
def _control_input_is_opaque(phase2_module: Any, control_input: Mapping[str, Any]) -> bool:
    ...
    requests = control_input.get(fields[0]) if fields else None
    if (
...
def _failure_for_projection(
    ...
    if (
        binding.get("report_evidence_binding_control_state")
        != "CONTROL_EVIDENCE_BINDING_REFERENCE_ONLY_NOT_WRITTEN"
        or binding.get("report_section_output_control_state")
        != "CONTROL_REPORT_SECTION_REFERENCE_ONLY_NOT_RENDERED"
        or binding.get("report_write_field_control_state")
        != "CONTROL_REPORT_FIELDS_REFERENCE_ONLY_NOT_WRITTEN"
        or binding.get("future_pdf_citation_control_state")
        != "CONTROL_FUTURE_PDF_CITATION_SOURCE_AND_PAGE_REQUIRED_NOT_RENDERED"
        or binding.get("actual_report_evidence_binding_performed") is not False
        or binding.get("actual_report_section_output_performed") is not False
        or binding.get("actual_report_write_performed") is not False
        or binding.get("actual_pdf_citation_rendered") is not False
        or binding.get("evidence_id_ref") != request.get("evidence_id_ref")
        or binding.get("evidence_gap_ref") != request.get("evidence_gap_ref")
    ):
        return "CRITICAL_CONCLUSION_EVIDENCE_BINDING_DRIFT"
    if binding_mode == "evidence_id" and request.get("binding_mode") != "CONTROL_BINDING_EVIDENCE_ID":
        return "CRITICAL_CONCLUSION_EVIDENCE_BINDING_DRIFT"
    if binding_mode == "evidence_gap" and request.get("binding_mode") != "CONTROL_BINDING_EVIDENCE_GAP":
        return "CRITICAL_CONCLUSION_EVIDENCE_BINDING_DRIFT"
    if not _all_control_references(
        binding,
        (
    ...
    if any(
        impact_queue.get(field) != expected
...
def _impact_state(required: bool, name: str) -> str:
    return (
        f"CONTROL_FUTURE_{name}_REPORT_STATUS_AND_QUEUE_REVIEW_REQUIRED"
        if required
        else f"CONTROL_{name}_REPORT_STATUS_AND_QUEUE_REVIEW_NOT_REQUIRED"
    )
...
def _scenario_record(
    ...
    return {
        ...
        "evidence_conflict_ref": request["evidence_conflict_ref"],
        ...
        "report_regeneration_queue_entry_ref": request[
            "report_regeneration_queue_entry_ref"
        ],
        "report_regeneration_reason_ref": request["report_regeneration_reason_ref"],
        "report_regeneration_queue_state_ref": request[
            "report_regeneration_queue_state_ref"
        ],
        ...
        "source_withdrawal_report_status_impact_state": _impact_state(
            definition["source_withdrawal_impact_required"], "SOURCE_WITHDRAWAL"
        ),
        "evidence_downgrade_report_status_impact_state": _impact_state(
            definition["evidence_downgrade_impact_required"], "EVIDENCE_DOWNGRADE"
        ),
        "evidence_conflict_report_status_impact_state": _impact_state(
            definition["evidence_conflict_impact_required"], "EVIDENCE_CONFLICT"
        ),
        "index_version_change_report_status_impact_state": _impact_state(
            definition["index_version_change_impact_required"], "INDEX_VERSION_CHANGE"
        ),
        "external_augmentation_source_separation_state": (
        ...
        "automatic_final_conclusion_allowed": external[
            "automatic_final_conclusion_allowed"
        ],
        "actual_report_status_impact_analysis_performed": impact_queue[
            "actual_report_impact_analysis_performed"
        ],
        "actual_report_status_updated": impact_queue["actual_report_status_updated"],
        "actual_queue_entry_created": impact_queue["actual_queue_entry_created"],
...
def build_report_regeneration_queue_phase3_report(
    ...
    try:
        ...
        executor = (
            ...
            else phase2_module.execute_report_regeneration_queue_control_slice
        )
    ...
    for index, definition in enumerate(SCENARIO_DEFINITIONS):
        ...
        impact_queue = _projection_record(
            phase2_result, "report_impact_queue_and_audit", index
        )
        external = _projection_record(
            phase2_result, "external_augmentation_and_whitebox_gate", index
        )
        if not all(
            isinstance(record, Mapping) for record in (binding, snapshot, impact_queue, external)
        ):
            return _base_report(False, "PHASE2_CONTROL_SHAPE_MISMATCH")
        failure_state = _failure_for_projection(
            definition, request, binding, snapshot, impact_queue, external
        )
        if failure_state is not None:
            return _base_report(False, failure_state)
        scenarios.append(_scenario_record(definition, request, impact_queue, external))
    ...
    report.update(
        {
            ...
            "whitebox_confirmation_required_scenario_count": sum(
                item["whitebox_confirmation_required"] for item in handlings
            ),
