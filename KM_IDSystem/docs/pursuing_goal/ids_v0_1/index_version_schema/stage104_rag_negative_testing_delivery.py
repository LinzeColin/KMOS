"""Stage104 P4 RAG 负向测试的纯内存交付证据。

本模块只从 Stage104 P3 已固定的非业务、reference-only 场景派生控制记录。
...
SCHEMA_VERSION = "ids.stage104.rag_negative_testing.phase4.delivery.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_RAG_NEGATIVE_TEST_DELIVERY_EVIDENCE"
PASS_RESULT = "PASS_RAG_NEGATIVE_TEST_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_RAG_NEGATIVE_TEST_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE104-P4-GATE"
NEXT_GATE = "IDS-STAGE104-REVIEW-GATE"
...
P3_CURRENT_GATE = "IDS-STAGE104-P3-GATE"
P3_NEXT_GATE = "IDS-STAGE104-P4-GATE"
P3_CONTROL_PREFIX = ":control:stage104-p2:"
DELIVERY_PREFIX = ":control:stage104-p4:"
...
RUNTIME_CLOSED_FIELDS = (
    ...
    "push_performed",
    "stage104_phase4_runtime_executed",
)
...
def _load_phase3_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage104_phase3_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage104 P3 RAG 负向测试场景模块")
...
def _phase3_shape_failure(phase3_module: Any, report: Mapping[str, Any]) -> str | None:
    ...
    if (
        ...
        or report.get("phase2_input_field_count") != 29
        or report.get("phase2_projection_group_count") != 4
        or report.get("phase2_projection_field_count_per_request") != 57
        or report.get("phase2_projection_field_count_total") != 285
    ...
    return [
        {
            ...
            **{field: item[field] for field in REPRODUCIBILITY_TUPLE_FIELDS},
            ...
            "answer_sample_state": (
                "CONTROL_RAG_NEGATIVE_TEST_ANSWER_SAMPLE_REFERENCE_ONLY_NOT_EXECUTED"
            ),
    ...
    return [
        {
            ...
            "negative_test_result_state": (
                "CONTROL_RAG_NEGATIVE_TEST_NEGATIVE_RESULT_REFERENCE_ONLY"
            ),
    ...
    return [
        {
            ...
            **{field: item[field] for field in REPRODUCIBILITY_TUPLE_FIELDS},
            ...
            "reproducible_log_state": (
                "CONTROL_RAG_NEGATIVE_TEST_TUPLE_REFERENCE_ONLY"
            ),
    ...
    return [
        {
            ...
            "predecessor_phase_ref": _delivery_ref("stage104-phase3"),
...
def build_rag_negative_testing_phase4_delivery_report(
    ...
    try:
        ...
        executor = phase3_executor or phase3_module.build_rag_negative_testing_phase3_report
