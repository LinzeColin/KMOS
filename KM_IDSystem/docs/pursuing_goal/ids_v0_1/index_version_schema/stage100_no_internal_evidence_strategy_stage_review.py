...
SCHEMA_VERSION = "ids.stage100.no_internal_evidence_strategy.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_NO_INTERNAL_EVIDENCE_STRATEGY_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_NO_INTERNAL_EVIDENCE_STRATEGY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_NO_INTERNAL_EVIDENCE_STRATEGY_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE100-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE101-P1-GATE"

P1_SCHEMA_VERSION = "ids.stage100.no_internal_evidence_strategy.phase1.v1"
P1_CONTRACT_STATE = "PHASE1_NO_INTERNAL_EVIDENCE_STRATEGY_RUNTIME_DISABLED"
P2_SCHEMA_VERSION = "ids.stage100.no_internal_evidence_strategy.phase2.v1"
P2_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_NO_INTERNAL_EVIDENCE_STRATEGY"
P2_EXECUTION_STATE = (
    "PASS_IN_MEMORY_NO_INTERNAL_EVIDENCE_STRATEGY_CONTROL_SLICE_RUNTIME_DISABLED"
)
P3_SCHEMA_VERSION = "ids.stage100.no_internal_evidence_strategy.phase3.v1"
P3_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_NO_INTERNAL_EVIDENCE_STRATEGY_SCENARIOS"
P3_PASS_RESULT = "PASS_NO_INTERNAL_EVIDENCE_STRATEGY_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_SCHEMA_VERSION = "ids.stage100.no_internal_evidence_strategy.phase4.delivery.v1"
P4_RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_NO_INTERNAL_EVIDENCE_STRATEGY_DELIVERY_EVIDENCE"
P4_PASS_RESULT = "PASS_NO_INTERNAL_EVIDENCE_STRATEGY_DELIVERY_EVIDENCE_RUNTIME_DISABLED"

P1_REFERENCE_FIELD_COUNT = 9
P1_SOURCE_TYPE_COUNT = 4
P1_HIGH_RISK_OUTPUT_COUNT = 3
P1_FAILURE_STATE_COUNT = 17
P1_CHINESE_FEEDBACK_COUNT = 4
P2_CONTROL_REQUEST_COUNT = 6
P2_CONTROL_INPUT_FIELD_COUNT = 21
P2_PROJECTION_GROUP_COUNT = 4
P2_FIELDS_PER_REQUEST = 38
P2_CONTROL_FIELD_CHECK_COUNT = 228
P3_SCENARIO_COUNT = 6
P3_SCENARIO_FIELD_COUNT = 29
P3_SCENARIO_FIELD_CHECK_COUNT = 174
...
REVIEW_RUNTIME_FALSE_FIELDS = (
    ...
    "push_performed",
    "stage100_review_runtime_executed",
)
...
CONTROL_PREFIX = ":control:stage100-p2:"
DELIVERY_PREFIX = ":control:stage100-p4:"

Provider = Callable[[], Mapping[str, Any]]
BASE = Path(__file__).resolve().parent
P1_CONTRACT_PATH = BASE / "stage100_no_internal_evidence_strategy_contract.json"
P2_MODULE_PATH = BASE / "stage100_no_internal_evidence_strategy_control_slice.py"
P3_MODULE_PATH = BASE / "stage100_no_internal_evidence_strategy_controlled_scenarios.py"
P4_MODULE_PATH = BASE / "stage100_no_internal_evidence_strategy_delivery.py"
NEXT_TASKPACK_PATH = (
    BASE.parents[2]
    / "taskpacks"
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module("stage100_review_phase2", P2_MODULE_PATH)
    return module.execute_no_internal_evidence_strategy_control_slice(
        module.build_control_input()
    )


def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module("stage100_review_phase3", P3_MODULE_PATH)
    return module.build_no_internal_evidence_strategy_phase3_report()


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module("stage100_review_phase4", P4_MODULE_PATH)
    return module.build_no_internal_evidence_strategy_phase4_delivery_report()
...
def _phase1_valid(contract: Mapping[str, Any]) -> bool:
    ...
    answer = _mapping(contract.get("answer_no_internal_evidence_contract"))
    source_types = _mapping(answer.get("source_type_contract"))
    display = _mapping(source_types.get("external_augmentation_display_composition"))
    retrieval = _mapping(answer.get("retrieval_document_boundary"))
    gap = _mapping(answer.get("no_internal_evidence_strategy"))
    ...
    return all(
        (
            ...
            contract.get("stage") == "STAGE-100",
            contract.get("phase") == "IDS-STAGE100-P1",
            contract.get("task_id") == "IDS-V0_1-STAGE100-P1",
            contract.get("contract_state") == P1_CONTRACT_STATE,
            contract.get("entry_gate") == "IDS-STAGE100-P1-GATE",
            contract.get("next_gate") == "IDS-STAGE100-P2-GATE",
            ...
            all(
                ...
                for field in (
                    "stage100_contract_can_replace_source_document",
            ...
            answer.get("answer_policy_reference_field_count")
            == P1_REFERENCE_FIELD_COUNT,
            tuple(answer.get("future_answer_policy_reference_fields", ()))
            == expected_fields,
            ...
            all(
                source_types.get(field) is True
                for field in (
            ...
            all(
                source_types.get(field) is False
                for field in (
                    "source_type_assignment_defined",
            ...
            set(_mapping(permission.get("classified_output_types")))
            == {
                "high_risk_engineering_advice",
                "contract_commitment",
                "production_writeback",
            },
            permission.get("output_classification_count") == P1_HIGH_RISK_OUTPUT_COUNT,
            permission.get(
            ...
            boundary.get("stage099_review_evidence_declared") is True,
            boundary.get("stage100_started") is True,
...
def _phase2_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage100_review_phase2_shape", P2_MODULE_PATH)
    except Exception:
        return False
    ...
    if not all(
        (
            ...
            == ("no_internal_evidence_strategy_control_requests",),
    ...
    source_ok = all(
        ...
        and record.get("external_augmentation_does_not_close_evidence_gap_state")
    ...
    permission_ok = all(
        ...
        and record.get("final_conclusion_state") == "CONTROL_FINAL_CONCLUSION_NOT_PUBLISHED"
...
def _phase3_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage100_review_phase3_shape", P3_MODULE_PATH)
    except Exception:
        return False
    ...
    if not all(
        (
            ...
            report.get("failure_state") is None,
            report.get("current_gate") == "IDS-STAGE100-P3-GATE",
            report.get("next_gate") == "IDS-STAGE100-P4-GATE",
            ...
            report.get("phase2_input_field_count") == P2_CONTROL_INPUT_FIELD_COUNT,
    ...
    if tuple(item.get("scenario_id") for item in scenarios) != scenario_ids:
        return False
    if set(views) != set(module.CONTROL_VIEW_NAMES):
        return False
    if any(
        not isinstance(views.get(name), list)
        or len(views.get(name, [])) != P3_SCENARIO_COUNT
        for name in module.CONTROL_VIEW_NAMES
    ):
        return False
    ...
    semantics_ok = all(
        (
            ...
            == "external_augmentation_opinion",
            external.get("source_type_separation_state")
            == "CONTROL_INTERNAL_EXTERNAL_PUBLIC_MODEL_REASONING_AND_GAP_SEPARATED",
            gap.get("internal_evidence_ref") is None,
    ...
    return scenarios_ok and semantics_ok


def _phase4_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage100_review_phase4_shape", P4_MODULE_PATH)
    except Exception:
        return False
    ...
    if not all(
        (
            ...
            report.get("failure_state") is None,
            report.get("current_gate") == "IDS-STAGE100-P4-GATE",
    ...
    injection_id = "retrieval_document_cannot_override_ids_rule_control"
    high_risk_ids = (
...
def build_no_internal_evidence_strategy_stage100_review_report(
    ...
    """机械聚合 Stage100 P1--P4 控制工件，任何漂移保持失败关闭。"""
    ...
    review_invariants = {
        ...
        "stage101_gate_only_opens_after_review": False,
    }
    review_valid = all(
        value
        for key, value in review_invariants.items()
        if key != "stage101_gate_only_opens_after_review"
    )
    review_invariants["stage101_gate_only_opens_after_review"] = review_valid
    ...
    return {
        ...
        "stage099_review_evidence_declared": True,
        "stage100_started": True,
        "phase1_completed": True,
        "phase2_completed": True,
        "phase3_completed": True,
        "phase4_completed": True,
        "stage100_review_control_report_generated_in_memory_only": True,
        "stage101_started": False,
        "github_upload_allowed": False,
        "push_allowed": False,
        "chinese_feedback": [
            "Stage100 P1 至 P4 的控制形状、无内部依据声明、来源类型、提示注入防护、输出权限和回退前置保持一致，复审结果只承载控制结论。",
            "来源文档与业务线白箱人工复核继续承担业务事实权威；内部依据不足保持 evidence_gap，外部增强意见不能替代或伪装内部依据。",
            "高风险工程建议、合同承诺和生产写回保持业务线白箱人工处理，人工确认未记录，最终结论未发布。",
            "Stage101 只开放下一门禁；真实资料、检索、提示词、模型、模型 Token、Agent、OVH、生产与上传保持后续授权边界。",
        ],
        ...
        **{field: False for field in REVIEW_RUNTIME_FALSE_FIELDS},
        "runtime_boundary": {
            ...
            if field != "stage100_review_runtime_executed"
        },
        "rollback": {
            "return_to": P4_PASS_RESULT,
            "preserve_stage100_phase1_to_phase4_evidence": True,
            "preserve_stage099_review_evidence": True,
