"""Stage077 P4 的纯内存后台索引构建交付证据。

本模块只复用已验证的 P2 五组后台索引构建控制记录与 P3 六条专项场景，派生
控制版索引清单、冒烟测试日志、切换／回退证明、保留／空间影响投影和操作说明。
...
RECORD_KIND = "BACKGROUND_INDEX_BUILD_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_BACKGROUND_INDEX_BUILD_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_BACKGROUND_INDEX_BUILD_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
ENTRY_GATE = "IDS-STAGE077-P4-GATE"
NEXT_GATE = "IDS-STAGE077-REVIEW-GATE"
...
def build_background_index_build_phase4_delivery_report(
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    return _load_phase3_module().build_background_index_build_phase3_report()


def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_phase2_module()
    return module.execute_background_index_build_control_slice(module.build_control_input())
...
def _load_phase3_module() -> Any:
    return _load_module(
        "stage077_phase3_scenarios", "stage077_background_index_build_scenarios.py"
    )


def _load_phase2_module() -> Any:
    return _load_module("stage077_phase2_slice", "stage077_background_index_build_slice.py")
...
def _phase2_report_is_valid(module: Any, report: Mapping[str, Any]) -> bool:
    groups = (
        ...
        (
            ...
            module.BACKGROUND_BUILD_OUTPUT_FIELDS,
        ),
        ("smoke_test_control_projections", module.SMOKE_TEST_FIELDS),
    ...
    return (
        ...
        and report.get("all_shadow_candidates_are_isolated_from_active_service")
        is True
        ...
        and report.get("all_switch_projections_keep_active_pointer_unchanged")
        is True
    ...
    return [
        {
            ...
            "index_manifest_ref": f"index-manifest:control:stage077-p2:{scenario}",
    ...
    return [
        {
            ...
            "smoke_test_log_ref": (
                f"smoke-test-log:control:stage077-p2:{scenario['phase2_control_scenario']}"
    ...
    return [
        {
            "control_scenario": record["control_scenario"],
            "rollback_proof_ref": (
                f"rollback-proof:control:stage077-p2:{record['control_scenario']}"
            ),
...
def _old_index_retention_projection() -> dict[str, Any]:
    return {
        "retention_policy_ref": "index-retention-policy:control:stage077-p2:previous-active",
...
def _operational_instruction_projections() -> list[dict[str, Any]]:
    marker = ":control:stage077-p2:operations"
    return [
        {
            "instruction_id": "BACKGROUND_INDEX_REBUILD_CONTROL_INSTRUCTION",
            "action": "REBUILD",
            "target_ref": f"index-rebuild{marker}",
            "entry_precondition": "AUTHORIZED_INPUT_AND_NEW_CANDIDATE_REQUIRED",
            "active_service_requirement": "OLD_ACTIVE_VERSION_CONTINUES_UNTIL_SMOKE_TEST_PASSES_AND_SWITCH_IS_AUTHORIZED",
            "control_outcome": "CONTROL_REBUILD_NOT_STARTED",
            "actual_operation_performed": False,
            "human_handling_required": True,
        },
        {
            "instruction_id": "BACKGROUND_INDEX_PAUSE_CONTROL_INSTRUCTION",
            "action": "PAUSE",
        ...
        {
            "instruction_id": "BACKGROUND_INDEX_RECOVERY_CONTROL_INSTRUCTION",
            "action": "RECOVERY",
...
def _record_references_are_control_only(record: Mapping[str, Any]) -> bool:
    ...
    return bool(reference_values) and all(
        ":control:stage077-p2:" in str(value) for value in reference_values
    )
