...
SCHEMA_VERSION = "ids.stage092.evidence_risk_scoring.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_RISK_SCORING_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_EVIDENCE_RISK_SCORING_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_EVIDENCE_RISK_SCORING_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE092-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE093-P1-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_RISK_SCORING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_PASS_RESULT = "PASS_EVIDENCE_RISK_SCORING_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:"

P1_STATIC_SHAPE = "20/10/5"
P1_FAILURE_STATE_COUNT = 14
P2_CONTROL_REQUEST_COUNT = 6
P2_CONTROL_INPUT_FIELD_COUNT = 27
P2_PROJECTION_GROUP_COUNT = 11
P2_FIELDS_PER_REQUEST = 97
P2_CONTROL_FIELD_CHECK_COUNT = 582
P2_FAILURE_STATE_COUNT = 28
P3_SCENARIO_COUNT = 7
...
P1_CONTRACT_PATH = BASE / "stage092_evidence_risk_scoring_contract.json"
P2_MODULE_PATH = BASE / "stage092_evidence_risk_scoring_control_slice.py"
P3_MODULE_PATH = BASE / "stage092_evidence_risk_scoring_controlled_scenarios.py"
P4_MODULE_PATH = BASE / "stage092_evidence_risk_scoring_delivery.py"
NEXT_TASKPACK_PATH = (
    BASE.parents[2]
    / "taskpacks"
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module("stage092_review_phase2", P2_MODULE_PATH)
    return module.execute_evidence_risk_scoring_control_slice(module.build_control_input())


def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module("stage092_review_phase3", P3_MODULE_PATH)
    return module.build_evidence_risk_scoring_phase3_report()


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module("stage092_review_phase4", P4_MODULE_PATH)
    return module.build_evidence_risk_scoring_phase4_delivery_report()
...
def _phase1_valid(contract: Mapping[str, Any]) -> bool:
    ...
    risk = contract.get("evidence_risk_scoring_contract")
    failures = contract.get("failure_and_stop_contract")
    runtime = contract.get("runtime_boundary")
    return (
        contract.get("phase") == "IDS-STAGE092-P1"
        and contract.get("task_id") == "IDS-V0_1-STAGE092-P1"
        and contract.get("next_gate") == "IDS-STAGE092-P2-GATE"
        ...
        and risk.get("evidence_risk_field_count") == 20
        and risk.get("evidence_risk_relation_field_count") == 10
        and risk.get("risk_factor_input_field_count") == 5
        and risk.get("evidence_grade_count") == 5
        and risk.get("risk_score_formula_defined") is False
        and risk.get("risk_score_weight_defined") is False
        and risk.get("risk_score_threshold_defined") is False
        and risk.get("critical_conclusion_requires_evidence_id_or_evidence_gap") is True
        and risk.get("business_line_whitebox_human_review_required_before_business_use")
        is True
...
def _phase2_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage092_review_phase2_shape", P2_MODULE_PATH)
    ...
    return (
        ...
        == "CONTROL_EVIDENCE_RISK_SCORING_PROJECTIONS_DECLARED_NOT_CALCULATED"
...
def _phase3_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage092_review_phase3_shape", P3_MODULE_PATH)
    except Exception:
        return False
    ...
    return (
        ...
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE092-P3-GATE"
        and report.get("next_gate") == "IDS-STAGE092-P4-GATE"
        ...
        and by_id["no_internal_evidence_risk_control"].get("evidence_id_ref") is None
        and _opaque_control_reference(
            by_id["no_internal_evidence_risk_control"].get("evidence_gap_ref")
        )
        and by_id["revoked_evidence_risk_report_impact_control"].get(
            "actual_report_status_updated"
        )
        is False
        and by_id["low_grade_high_trust_masquerade_risk_control"].get(
...
def _phase4_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage092_review_phase4_shape", P4_MODULE_PATH)
    except Exception:
        return False
    specs = (
        (
    ...
    return (
        ...
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE092-P4-GATE"
...
def build_evidence_risk_scoring_stage092_review_report(
    ...
    """机械聚合 Stage092 P1--P4 控制工件，任何漂移保持失败关闭。"""
    ...
    phase1_risk = _mapping(phase1.get("evidence_risk_scoring_contract"))
    source_boundary = all(
        (
    ...
    owner_formula_boundary = (
        phase1_risk.get("risk_score_formula_defined") is False
        and phase1_risk.get("risk_score_weight_defined") is False
        and phase1_risk.get("risk_score_threshold_defined") is False
    )
    delivery_boundary = (
    ...
    rollback_boundary = (
        ...
        and phase4.get("stage092_review_started") is False
        and phase4.get("stage093_started") is False
    )
    ...
    review_invariants = {
        ...
        "owner_formula_boundary_preserved": owner_formula_boundary,
        ...
        "stage093_gate_only_opens_after_review": False,
    }
    review_valid = all(phase_results.values()) and all(
        value
        for key, value in review_invariants.items()
        if key != "stage093_gate_only_opens_after_review"
    )
    review_invariants["stage093_gate_only_opens_after_review"] = review_valid
    ...
    if not owner_formula_boundary:
        failure_reasons.append("OWNER_FORMULA_BOUNDARY_MISMATCH")
    ...
    return {
        ...
        "stage091_review_evidence_declared": True,
        "stage092_started": True,
        "phase1_completed": True,
        "phase2_completed": True,
        "phase3_completed": True,
        "phase4_completed": True,
        "stage092_review_started": True,
        "whole_stage_review_performed": False,
        "stage093_started": False,
        ...
        **{field: False for field in REVIEW_RUNTIME_FALSE_FIELDS},
        ...
        "rollback": {
            ...
            "preserve_stage092_phase1_to_phase4_evidence": True,
            "preserve_stage091_review_evidence": True,
