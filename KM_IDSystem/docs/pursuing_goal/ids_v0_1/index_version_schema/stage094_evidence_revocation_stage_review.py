...
SCHEMA_VERSION = "ids.stage094.evidence_revocation.stage_review.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_REVOCATION_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_EVIDENCE_REVOCATION_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_EVIDENCE_REVOCATION_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE094-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE095-P1-GATE"
P3_PASS_RESULT = "PASS_EVIDENCE_REVOCATION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_PASS_RESULT = "PASS_EVIDENCE_REVOCATION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
CONTROL_PREFIX = ":control:"

P1_STATIC_SHAPE = "18/14/6/5"
P1_FAILURE_STATE_COUNT = 16
P2_CONTROL_REQUEST_COUNT = 6
P2_CONTROL_INPUT_FIELD_COUNT = 29
P2_PROJECTION_GROUP_COUNT = 11
P2_FIELDS_PER_REQUEST = 105
P2_CONTROL_FIELD_CHECK_COUNT = 630
P2_FAILURE_STATE_COUNT = 30
P3_SCENARIO_COUNT = 7
...
P1_CONTRACT_PATH = BASE / "stage094_evidence_revocation_contract.json"
P2_MODULE_PATH = BASE / "stage094_evidence_revocation_control_slice.py"
P3_MODULE_PATH = BASE / "stage094_evidence_revocation_controlled_scenarios.py"
P4_MODULE_PATH = BASE / "stage094_evidence_revocation_delivery.py"
NEXT_TASKPACK_PATH = (
    BASE.parents[2]
    / "taskpacks"
...
def _default_phase2_report() -> Mapping[str, Any]:
    module = _load_module("stage094_review_phase2", P2_MODULE_PATH)
    return module.execute_evidence_revocation_control_slice(module.build_control_input())


def _default_phase3_report() -> Mapping[str, Any]:
    module = _load_module("stage094_review_phase3", P3_MODULE_PATH)
    return module.build_evidence_revocation_phase3_report()


def _default_phase4_report() -> Mapping[str, Any]:
    module = _load_module("stage094_review_phase4", P4_MODULE_PATH)
    return module.build_evidence_revocation_phase4_delivery_report()
...
def _phase1_valid(contract: Mapping[str, Any]) -> bool:
    ...
    revocation = _mapping(contract.get("evidence_revocation_contract"))
    grade_reference = _mapping(revocation.get("evidence_grade_definition_reference"))
    definitions = _mapping(revocation.get("control_definitions"))
    failures = _mapping(contract.get("failure_and_stop_contract"))
    runtime = _mapping(contract.get("runtime_boundary"))
    return (
        contract.get("phase") == "IDS-STAGE094-P1"
        and contract.get("task_id") == "IDS-V0_1-STAGE094-P1"
        and contract.get("next_gate") == "IDS-STAGE094-P2-GATE"
        ...
        and len(definitions) == 6
        and revocation.get("evidence_revocation_field_count") == 18
        and revocation.get("evidence_revocation_relation_field_count") == 14
        ...
        and revocation.get("actual_evidence_grade_assigned_or_changed") is False
        and revocation.get("actual_evidence_revocation_processed") is False
        and revocation.get("actual_evidence_degradation_processed") is False
        and revocation.get("actual_evidence_recovery_processed") is False
        and revocation.get("actual_report_status_updated") is False
        and revocation.get("critical_conclusion_requires_evidence_id_or_evidence_gap")
        is True
        and revocation.get("evidence_revocation_may_not_replace_evidence_id_or_evidence_gap")
        is True
        and revocation.get(
        ...
        is True
        and revocation.get("report_status_impact_is_future_reference_only") is True
        and revocation.get(
            "business_line_whitebox_human_review_required_before_business_use"
        )
        is True
        and revocation.get("all_values_are_control_labels_only") is True
...
def _phase2_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage094_review_phase2_shape", P2_MODULE_PATH)
...
def _phase3_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage094_review_phase3_shape", P3_MODULE_PATH)
    except Exception:
        return False
    ...
    return (
        ...
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE094-P3-GATE"
        and report.get("next_gate") == "IDS-STAGE094-P4-GATE"
        ...
        and by_id["no_internal_evidence_revocation_control"].get("evidence_id_ref")
        is None
        and _opaque_control_reference(
            by_id["no_internal_evidence_revocation_control"].get("evidence_gap_ref")
        )
...
def _phase4_valid(report: Mapping[str, Any]) -> bool:
    try:
        module = _load_module("stage094_review_phase4_shape", P4_MODULE_PATH)
    except Exception:
        return False
    specs = (
        (
    ...
    return (
        ...
        and report.get("failure_state") is None
        and report.get("current_gate") == "IDS-STAGE094-P4-GATE"
...
def build_evidence_revocation_stage094_review_report(
    ...
    """机械聚合 Stage094 P1--P4 控制工件，漂移保持失败关闭。"""
    ...
    phase1_revocation = _mapping(phase1.get("evidence_revocation_contract"))
    phase1_grade_reference = _mapping(
        phase1_revocation.get("evidence_grade_definition_reference")
    )
    source_boundary = all(
        (
    ...
    owner_rule_boundary = (
        ...
        and phase1_revocation.get("actual_evidence_grade_assigned_or_changed") is False
        and phase1_revocation.get(
            "business_line_whitebox_human_review_required_before_business_use"
        )
        is True
    )
    ...
    rollback_boundary = (
        ...
        and phase4.get("stage094_review_started") is False
        and phase4.get("stage095_started") is False
    )
    ...
    review_invariants = {
        ...
        "owner_revocation_rule_boundary_preserved": owner_rule_boundary,
        ...
        "stage095_gate_only_opens_after_review": False,
    }
    review_valid = all(phase_results.values()) and all(
        value
        for key, value in review_invariants.items()
        if key != "stage095_gate_only_opens_after_review"
    )
    review_invariants["stage095_gate_only_opens_after_review"] = review_valid
    ...
    if not owner_rule_boundary:
        failure_reasons.append("OWNER_REVOCATION_RULE_BOUNDARY_MISMATCH")
    ...
    return {
        ...
        "stage093_review_evidence_declared": True,
        "stage094_started": True,
        "phase1_completed": True,
        "phase2_completed": True,
        "phase3_completed": True,
        "phase4_completed": True,
        "stage094_review_started": True,
        "whole_stage_review_performed": False,
        "stage095_started": False,
        ...
        **{field: False for field in REVIEW_RUNTIME_FALSE_FIELDS},
        ...
        "rollback": {
            ...
            "preserve_stage094_phase1_to_phase4_evidence": True,
            "preserve_stage093_review_evidence": True,
