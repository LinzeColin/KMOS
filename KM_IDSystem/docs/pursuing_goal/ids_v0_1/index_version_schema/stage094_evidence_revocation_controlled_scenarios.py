"""Stage094 P3 的纯内存证据撤回异常场景控制验证。

模块只重放 Stage094 P2 的固定、非业务、reference-only 控制投影。它不读取
来源资料、检索结果或真实证据账本，不连接数据库，不计算真实风险，不执行撤回、
降级、恢复、投毒处置或报告状态更新，也不写入持久化记录。
"""
...
SCHEMA_VERSION = "ids.stage094.evidence_revocation.phase3.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EVIDENCE_REVOCATION_EXCEPTION_SCENARIOS"
PASS_RESULT = "PASS_EVIDENCE_REVOCATION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EVIDENCE_REVOCATION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
CURRENT_GATE = "IDS-STAGE094-P3-GATE"
NEXT_GATE = "IDS-STAGE094-P4-GATE"
P2_EXECUTION_STATE = "CONTROL_EVIDENCE_REVOCATION_PROJECTIONS_DECLARED"
CONTROL_PREFIX = ":control:stage094-p2:"
...
P2_PROJECTION_SPECS = (
    ("evidence_revocation_schema_binding", "EVIDENCE_REVOCATION_SCHEMA_BINDING_FIELDS"),
    ("evidence_revocation_relation", "EVIDENCE_REVOCATION_RELATION_FIELDS"),
    (
    ...
    ("evidence_grade_binding", "EVIDENCE_GRADE_BINDING_FIELDS"),
    ("revocation_control", "REVOCATION_CONTROL_FIELDS"),
...
SCENARIO_DEFINITIONS = (
    {
        "scenario_id": "no_internal_evidence_revocation_control",
    ...
    {
        "scenario_id": "low_ocr_evidence_revocation_degradation_control",
    ...
    {
        "scenario_id": "old_version_evidence_revocation_degradation_control",
    ...
    {
        "scenario_id": "conflict_evidence_revocation_degradation_control",
    ...
    {
        "scenario_id": "malicious_evidence_revocation_quarantine_control",
...
def _load_phase2_module() -> Any:
    module_path = Path(__file__).with_name("stage094_evidence_revocation_control_slice.py")
    spec = importlib.util.spec_from_file_location("stage094_phase2_slice", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage094 P2 evidence revocation slice")
...
def _phase2_shape_is_preserved(phase2_module: Any, result: Mapping[str, Any]) -> bool:
    ...
    if (
        ...
        != ("evidence_revocation_control_requests",)
        or len(getattr(phase2_module, "INPUT_FIELDS", ())) != 29
...
def _reference_is_allowed(field: str, value: object, scenario: str) -> bool:
    return (
        ...
        and scenario == P2_CONTROL_SCENARIOS[0]
        and value is None
...
def _phase2_records(
    ...
    for prefix, _field_constant in P2_PROJECTION_SPECS:
        ...
        if not isinstance(record, Mapping):
            raise ValueError(f"invalid Stage094 P2 {prefix} projection")
        records[prefix] = record
    return records


def _phase2_semantic_failure(result: Mapping[str, Any]) -> str | None:
    no_internal = _phase2_records(result, P2_CONTROL_SCENARIOS[0])
    low_trust = _phase2_records(result, P2_CONTROL_SCENARIOS[1])
    conflict = _phase2_records(result, P2_CONTROL_SCENARIOS[2])
    expired = _phase2_records(result, P2_CONTROL_SCENARIOS[3])
    revoked = _phase2_records(result, P2_CONTROL_SCENARIOS[4])
    malicious = _phase2_records(result, P2_CONTROL_SCENARIOS[5])
    if (
        no_internal["evidence_revocation_schema_binding"].get("evidence_id_ref")
        is not None
        or not _control_ref(
            no_internal["evidence_revocation_schema_binding"].get("evidence_gap_ref")
        )
        or not _control_ref(
            no_internal["critical_conclusion_binding"].get("evidence_gap_ref")
        )
        or no_internal["critical_conclusion_binding"].get(
    ...
    if (
        low_trust["evidence_grade_binding"].get("evidence_grade_label") != "D"
        or low_trust["degradation"].get("degradation_state")
        != "CONTROL_DEGRADED_LOW_TRUST_NOT_ACCEPTED"
        or not _control_ref(
            low_trust["risk_reference_binding"].get("ocr_confidence_indicator_ref")
        )
    ):
        return "LOW_OCR_EVIDENCE_NOT_DEGRADED"
    if (
        expired["evidence_grade_binding"].get("evidence_grade_label") != "C"
        or expired["degradation"].get("degradation_state")
        != "CONTROL_DEGRADED_EXPIRED_NOT_ACCEPTED"
        or not _control_ref(
            expired["risk_reference_binding"].get("version_status_indicator_ref")
        )
    ):
        return "OLD_VERSION_EVIDENCE_NOT_DEGRADED"
    if (
        conflict["evidence_grade_binding"].get("evidence_grade_label") != "B"
        or conflict["degradation"].get("degradation_state")
        != "CONTROL_DEGRADED_CONFLICT_NOT_ACCEPTED"
        or not _control_ref(
            conflict["risk_reference_binding"].get("conflict_status_indicator_ref")
        )
    ):
    ...
    if (
        ...
        or revoked["revocation_control"].get("revocation_state")
        != "CONTROL_REVOCATION_REFERENCE_REVOKED_NOT_EXECUTED"
        or not _control_ref(revoked["revocation_control"].get("affected_report_ref"))
    ...
    if (
        low_trust["evidence_grade_binding"].get("evidence_grade_label") != "D"
        or "evidence-grade-D"
        not in str(low_trust["critical_conclusion_binding"].get("evidence_grade_ref"))
    ):
    ...
    for scenario in P2_CONTROL_SCENARIOS:
        ...
        grade = records["evidence_grade_binding"]
        ...
        if (
            grade.get("human_whitebox_review_state")
            != "CONTROL_HUMAN_WHITEBOX_REVIEW_REQUIRED"
            or defense.get("human_whitebox_review_state")
            != "CONTROL_HUMAN_WHITEBOX_REVIEW_REQUIRED"
        ):
...
def _scenario_expectation_met(
    ...
    schema = records["evidence_revocation_schema_binding"]
    relation = records["evidence_revocation_relation"]
    capture = records["retrieval_evidence_capture_binding"]
    risk = records["risk_reference_binding"]
    grade = records["evidence_grade_binding"]
    revocation = records["revocation_control"]
    defense = records["poisoning_defense"]
    binding = records["critical_conclusion_binding"]
    ...
    scenario_specific = (
        ...
        if scenario_id == "no_internal_evidence_revocation_control"
        else True
    )
    scenario_specific = scenario_specific and (
        revocation.get("revocation_state")
        == "CONTROL_REVOCATION_REFERENCE_REVOKED_NOT_EXECUTED"
    ...
    scenario_specific = scenario_specific and (
        ...
        == "CONTROL_POISONING_DEFENSE_ROUTE_DECLARED_NOT_EXECUTED"
        if scenario_id == "malicious_evidence_revocation_quarantine_control"
        else True
    )
    scenario_specific = scenario_specific and (
    ...
    return (
        ...
        and grade.get("evidence_grade_label") == definition["expected_grade"]
        and grade.get("human_whitebox_review_state")
        == "CONTROL_HUMAN_WHITEBOX_REVIEW_REQUIRED"
...
def _build_scenario(
    ...
    return {
        ...
        "evidence_grade_label": grade["evidence_grade_label"],
...
def build_evidence_revocation_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制投影并验证 Stage094 P3 固定异常场景。"""

    try:
        phase2_module = _load_phase2_module()
        executor = phase2_executor or phase2_module.execute_evidence_revocation_control_slice
