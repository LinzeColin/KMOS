# STAGE-026 Phase 3 · Archive Manifest 场景验证

- Task ID: `IDS-V0_1-STAGE026-P3`
- Acceptance ID: `ACC-STAGE-026`
- Schema: `ids.stage026.archive_manifest.scenario_validation.v1`
- Helper: `build_stage026_scenario_report`
- Archive manifest schema: `ids.stage026.archive_manifest.v1`
- 入口：`IDS 系统运营入口`
- Recorded at UTC: `2026-07-03T03:19:56Z`

## 验证目标

Phase 3 只验证 archive manifest 的异常场景、post-extract re-ingest 计划和 cleanup allowlist，不进入 Phase 4 closeout，不执行 GitHub upload、PR、merge 或 app reinstall。

本轮验证覆盖：

- 路径穿越：期望 `ARCHIVE_MANIFEST_PATH_TRAVERSAL_BLOCKED`
- 绝对路径：期望 `ARCHIVE_MANIFEST_ABSOLUTE_PATH_BLOCKED`
- 压缩炸弹：期望 `ARCHIVE_MANIFEST_TOTAL_SIZE_LIMIT_EXCEEDED`
- 嵌套包：期望 `ARCHIVE_MANIFEST_NESTED_DEPTH_LIMIT_EXCEEDED`
- 乱码文件名：期望 `ARCHIVE_MANIFEST_GARBLED_FILENAME_OWNER_REVIEW_REQUIRED`
- 超大文件数：期望 `ARCHIVE_MANIFEST_FILE_COUNT_LIMIT_EXCEEDED`

## Re-ingest 验证

`build_stage026_scenario_report` 聚合各场景的 in-memory archive manifest payload。安全条目必须进入 `POST_EXTRACT_REINGEST_VALIDATED`，并声明后续仍需经过：
...
cleanup validation 的通过状态为 `ARCHIVE_MANIFEST_CLEANUP_ALLOWLIST_VALIDATED`。
