"""Stage073 P2 的纯内存 Embedding 审计测试控制切片。

模块只接受五条固定、非业务、reference-only 控制请求，并在内存中机械投影
策略继承、未来队列/缓存/失败重试、零值成本、模型版本及完整外部 API 审计
...
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EMBEDDING_AUDIT_TEST"
CONTROL_ADAPTER_VERSION = "ids.embedding_audit_test.control_adapter.v0_1.stage073.p2"
CONTROL_FIELDS = ("embedding_audit_test_requests",)
...
REFERENCE_INPUT_FIELDS = (
    "embedding_audit_test_request_ref",
...
def build_control_request(scenario: str) -> dict[str, Any]:
    ...
    if scenario not in CONTROL_SCENARIO_CONFIGURATION:
        raise ValueError("unknown embedding audit test control scenario")
    ...
    marker = f":control:stage073-p2:{scenario}"
    return {
        "embedding_audit_test_request_ref": f"audit-test-request{marker}",
...
def build_control_input() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        "embedding_audit_test_requests": [
            build_control_request(scenario) for scenario in CONTROL_SCENARIOS
        ]
    }


def execute_embedding_audit_test_control_slice(
    ...
    for scenario, request in zip(CONTROL_SCENARIOS, requests):
        ...
        disposition = _audit_disposition(resolution)
    ...
    return {
        ...
        "input_accepted": True,
        "execution_state": "COMPLETED_IN_MEMORY_EMBEDDING_AUDIT_TEST_CONTROL_SLICE",
        ...
        **_runtime_closed_flags(),
        "chinese_feedback": [
            "当前只在内存中投影五条固定审计控制记录，未读取或保留任何真实资料、摘要正文、文本块、路径、页面或业务结论。",
            "有效外部 API 策略从 data source 经 document 自动继承到 chunk；默认 denied，文档不能放宽来源策略，也不能手工标记单个 chunk。",
            "未授权 chunk 不会外发；provider、模型、版本、Token、成本和审计字段只是控制引用与零值投影，未创建运行时记录。",
            "任何控制候选都不会被持久化、调度或发送；未来 provider 调用前仍需要授权、预算、完整审计字段及业务线白箱人工复核。",
        ],
    }
...
def _accepted_control_requests(
    ...
    if not isinstance(control_input, Mapping) or set(control_input) != set(CONTROL_FIELDS):
        return None
    requests = control_input.get("embedding_audit_test_requests")
