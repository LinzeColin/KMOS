"""Stage073 P3 的 Embedding 审计测试受控专项场景。

模块只重放 Stage073 P2 的五条固定、非业务、reference-only ``:control:``
记录。它验证默认拒绝、摘要引用边界、文档收紧、全文引用候选、预算暂停及每个
未来调用候选的完整审计控制投影。所有输入与输出均为控制引用，不会读取、保留
或外发正文、摘要或文本块。
...
SCHEMA_VERSION = "ids.stage073.embedding_audit_test.phase3.controlled_scenarios.v1"
RECORD_KIND = "CONTROLLED_EMBEDDING_AUDIT_TEST_SCENARIO_REPORT"
PASS_RESULT = "PASS_PHASE3_EMBEDDING_AUDIT_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EMBEDDING_AUDIT_TEST_CONTROLLED_SCENARIOS"
NEXT_GATE = "IDS-STAGE073-P4-GATE"
PHASE2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_EMBEDDING_AUDIT_TEST_CONTROL_SLICE"
...
SCENARIOS = (
    {
        "scenario_id": "denied-policy-blocks-embedding-audit-test-egress-control",
...
def build_embedding_audit_test_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制切片并验证策略、预算、审计前置与零运行时。"""
    ...
    return {
        ...
        "embedding_audit_test_scenario_can_replace_source_document": False,
        "embedding_audit_test_scenario_can_become_business_fact_authority": False,
        ...
        **runtime_closed_flags,
        "chinese_feedback": [
            ...
            "本步骤可回退到 Stage073 P2，不影响来源资料、业务事实、OVH 或生产状态。",
        ],
    }


def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_embedding_audit_test_control_slice
...
def _load_phase2_module() -> Any:
    path = Path(__file__).with_name("stage073_embedding_audit_test_slice.py")
    spec = importlib.util.spec_from_file_location("stage073_phase2_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage073 P2 embedding audit test slice")
...
def _audit_references_are_control_only(audit: Mapping[str, Any]) -> bool:
    ...
    return all(
        ...
        and ":control:stage073-p2:" in audit[field]
        for field in fields
    )
