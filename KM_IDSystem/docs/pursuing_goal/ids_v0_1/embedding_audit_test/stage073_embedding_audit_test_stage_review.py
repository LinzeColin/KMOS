"""Stage073 的纯内存整阶段机械复审，不读取真实资料或启动 Stage074。"""
...
P1_CONTRACT = BASE / "stage073_embedding_audit_test_contract.json"
P2_CONTRACT = BASE / "stage073_embedding_audit_test_slice_contract.json"
P3_CONTRACT = BASE / "stage073_embedding_audit_test_scenarios_contract.json"
P4_CONTRACT = BASE / "stage073_embedding_audit_test_delivery_contract.json"

SCHEMA_VERSION = "ids.stage073.embedding_audit_test.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE073-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-073"
PASS_RESULT = "PASS_REVIEWED_LOCAL_EMBEDDING_AUDIT_TEST_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_LOCAL_EMBEDDING_AUDIT_TEST_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE073-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE074-P1-GATE"
RETURN_STATE = "PHASE4_EMBEDDING_AUDIT_TEST_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
P2_CONTRACT_STATE = "PHASE2_EMBEDDING_AUDIT_TEST_CONTROL_SLICE_RUNTIME_DISABLED"
P2_STATE = "COMPLETED_IN_MEMORY_EMBEDDING_AUDIT_TEST_CONTROL_SLICE"
P3_PASS_RESULT = "PASS_PHASE3_EMBEDDING_AUDIT_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P4_PASS_RESULT = "PASS_PHASE4_EMBEDDING_AUDIT_TEST_DELIVERY_RUNTIME_DISABLED"
...
P4_RECORD_SHAPES = {
    "embedding_audit_test_policy_samples": 27,
    "control_audit_log_samples": 12,
    "cost_estimate_samples": 16,
    "failure_handling_results": 8,
    "non_externalized_data_records": 10,
}
...
def build_stage073_review_report(
    ...
    """机械复审 Stage073 P1--P4，仅输出零运行时控制结论与下一门禁。"""
    ...
    phase3_report = _mapping((phase3_report_provider or _report_provider(
        "stage073_embedding_audit_test_scenarios.py",
        "build_embedding_audit_test_phase3_report",
    ))())
    phase4_report = _mapping((phase4_report_provider or _report_provider(
        "stage073_embedding_audit_test_delivery.py",
        "build_embedding_audit_test_phase4_delivery_report",
    ))())
    ...
    invariants = {
        ...
        "stage074_gate_only_opens_after_review": _future_stage_boundary(
    ...
    report: dict[str, Any] = {
        ...
        "source_authority": "FROZEN_STAGE073_TASKPACK_AND_STAGE073_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        ...
        "rollback": {
            ...
            "revertable_artifacts": (
                "Stage073 review document",
                "Stage073 review module",
                "Stage073 review focused tests",
                "Stage073 review governance projection",
            ),
...
def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    return _checks(
        contract,
        (
            ("schema_version", "ids.stage073.embedding_audit_test.phase1.v1"),
            ...
            ("execution_ready.controlled_test_execution_ready", False),
            ...
            ("failure_and_stop_contract.failure_state_count", 7),
            ...
            ("stage_and_phase_boundary.stage074_started", False),
        ),
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    return _checks(
        contract,
        (
            ("schema_version", "ids.stage073.embedding_audit_test.phase2.v1"),
            ("contract_state", P2_CONTRACT_STATE),
            ("task_id", "IDS-V0_1-STAGE073-P2"),
            ("next_gate", "IDS-STAGE073-P3-GATE"),
            ("slice_executable", True),
            ("execution_ready", False),
            ("source_authority.second_authoritative_source_created", False),
            ("source_authority.source_body_or_path_allowed", False),
            ("reference_only_embedding_audit_test_input_control_contract.field_count", 20),
            ("reference_only_embedding_audit_test_input_control_contract.control_request_count", 5),
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    return _checks(
        contract,
        (
            ("schema_version", "ids.stage073.embedding_audit_test.phase3.v1"),
            ("contract_state", "PHASE3_EMBEDDING_AUDIT_TEST_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"),
            ("task_id", "IDS-V0_1-STAGE073-P3"),
            ("next_gate", "IDS-STAGE073-P4-GATE"),
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    return _checks(
        contract,
        (
            ("schema_version", "ids.stage073.embedding_audit_test.phase4.delivery.v1"),
            ("contract_state", RETURN_STATE),
            ("task_id", "IDS-V0_1-STAGE073-P4"),
            ("next_gate", REVIEW_GATE),
...
def _phase2_report_valid(report: Mapping[str, Any]) -> bool:
    return all(
        (
            report.get("schema_version") == "ids.stage073.embedding_audit_test.phase2.v1",
            report.get("record_kind") == "CONTROL_ONLY_IN_MEMORY_EMBEDDING_AUDIT_TEST",
...
def _phase3_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            == "ids.stage073.embedding_audit_test.phase3.controlled_scenarios.v1",
            report.get("record_kind") == "CONTROLLED_EMBEDDING_AUDIT_TEST_SCENARIO_REPORT",
            report.get("valid") is True,
            report.get("result") == P3_PASS_RESULT,
            report.get("next_gate") == "IDS-STAGE073-P4-GATE",
...
def _phase4_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            report.get("schema_version") == "ids.stage073.embedding_audit_test.phase4.delivery.v1",
            report.get("record_kind") == "EMBEDDING_AUDIT_TEST_DELIVERY_EVIDENCE_REPORT",
            report.get("valid") is True,
            ...
            report.get("phase2_control_slice_report_valid") is True,
            report.get("source_document_remains_authoritative") is True,
            report.get("business_line_whitebox_human_review_remains_authoritative") is True,
            ...
            report.get("control_payload_content_retained") is False,
            ...
            query.get("persistent_audit_log_available") is False,
            query.get("real_externalization_history_available") is False,
            rollback.get("rollback_target_result") == P3_PASS_RESULT,
            rollback.get("rollback_target_gate") == "IDS-STAGE073-P4-GATE",
...
def _controlled_replay(
    ...
    return {
        ...
        "phase2_control_request_count": _int_at(
            phase2, "reference_only_embedding_audit_test_input_control_contract.control_request_count"
        ),
...
def _single_authority(*artifacts: Mapping[str, Any]) -> bool:
    contracts = artifacts[:4]
    phase3_report, phase4_report = artifacts[4:]
    return all(
        _get(contract, "source_authority.second_authoritative_source_created") is False
        and _get(contract, "source_authority.source_body_or_path_allowed") is False
        for contract in contracts
    ) and all(
        (
            phase3_report.get("source_document_remains_authoritative") is True,
            phase3_report.get("embedding_audit_test_scenario_can_replace_source_document")
            is False,
            phase3_report.get(
                "embedding_audit_test_scenario_can_become_business_fact_authority"
            ) is False,
...
def _policy_and_audit_boundary(
    ...
    return all(
        (
            _get(phase1, "policy_inheritance_contract.default_external_api_policy")
            == "denied",
            _get(phase1, "policy_inheritance_contract.document_may_widen_data_source_policy")
            is False,
            _get(
                phase1,
...
def _future_call_boundary(
    ...
    return all(
        (
            ...
            phase4_report.get("human_handling_required_count") == 4,
            phase4_report.get(
...
def _future_stage_boundary(*contracts: Mapping[str, Any]) -> bool:
    return all(
        (
            _get(contract, "stage_and_phase_boundary.stage074_started") is False,
...
def _phase2_report_provider() -> ReportProvider:
    module = _load_module("stage073_embedding_audit_test_slice.py")
    return lambda: _mapping(
        module.execute_embedding_audit_test_control_slice(module.build_control_input())
    )
...
def _load_module(filename: str) -> Any:
    path = BASE / filename
    spec = importlib.util.spec_from_file_location(f"stage073_review_{path.stem}", path)
...
def _record_field_count(report: Mapping[str, Any], name: str) -> int:
    ...
    return (
        len(records[0])
        if len(records) == 5 and all(len(item) == len(records[0]) for item in records)
        else -1
    )
