"""Stage063 P4 的章节感知切块 metadata-only 交付证据。

本模块仅从 P3 六类固定、非业务、reference-only 控制场景派生内存中的
JSONL 样例、控制覆盖率报告、低质量待人工清单、回归结论和回退说明。它不会
读取真实文档、创建真实 chunk，或执行版本、索引、数据库及任何运行时动作。
"""
...
SCHEMA_VERSION = "ids.stage063.chapter_aware_chunking.phase4.delivery.v1"
RECORD_KIND = "CHAPTER_AWARE_CHUNKING_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_PHASE4_CHAPTER_AWARE_CHUNKING_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_CHAPTER_AWARE_CHUNKING_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE063-REVIEW-GATE"

CHUNK_JSONL_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_CHUNK_JSONL_SAMPLE_NOT_REAL_CHUNK"
COVERAGE_REPORT_KIND = "CONTROLLED_CHUNK_COVERAGE_REPORT_NOT_REAL_COVERAGE"
LOW_QUALITY_LIST_KIND = "CONTROLLED_LOW_QUALITY_CHUNK_LIST_NOT_REAL_QUALITY_MEASUREMENT"
REGRESSION_RESULT_KIND = "CONTROLLED_CHUNKING_REGRESSION_RESULT_NOT_REAL_QUALITY_REGRESSION"
...
def build_chapter_aware_chunking_phase4_delivery_report(
    ...
    predecessor = predecessor_value if isinstance(predecessor_value, Mapping) else {}
    ...
    low_quality_list_valid = len(low_quality_list) == len(EXPECTED_SCENARIO_IDS) and all(
    ...
    valid = (
        ...
        and all(_jsonl_line_matches_sample(line, sample) for line, sample in zip(jsonl_lines, samples))
...
def _load_phase3_report_provider() -> ScenarioReportProvider:
    module_path = Path(__file__).with_name("stage063_chapter_aware_chunking_scenarios.py")
    spec = importlib.util.spec_from_file_location(
        "stage063_chapter_aware_chunking_scenarios", module_path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage063 P3 chapter-aware chunking scenarios are unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.build_chapter_aware_chunking_phase3_report
...
def _predecessor_is_valid(
    ...
    return (
        ...
        and predecessor.get("control_traceability_field_count") == len(TRACEABILITY_FIELDS)
        and predecessor.get("control_traceability_reference_check_count")
        == len(EXPECTED_SCENARIO_IDS) * len(TRACEABILITY_FIELDS)
        ...
        and all(_scenario_has_control_only_shape(scenario) for scenario in scenarios.values())
    )
...
def _scenario_has_control_only_shape(scenario: Mapping[str, Any]) -> bool:
    return (
        ...
        and isinstance(scenario.get("referenced_chunking_request_ref"), str)
        and ":control:" in scenario["referenced_chunking_request_ref"]
    )
...
def _build_chunk_jsonl_samples(
    ...
    for scenario_id in EXPECTED_SCENARIO_IDS:
        ...
        if scenario is None:
            continue
        sample = {
            "sample_id": f"chunk-jsonl-sample:{scenario_id}",
            ...
            "chunking_request_ref": scenario.get("referenced_chunking_request_ref"),
        ...
        sample.update({field: scenario.get(field) for field in TRACEABILITY_FIELDS})
...
def _sample_has_control_only_shape(sample: Mapping[str, Any]) -> bool:
    return (
        ...
        and sample.get("actual_chunk_persisted") is False
        ...
        and isinstance(sample.get("chunking_request_ref"), str)
        and ":control:" in sample["chunking_request_ref"]
        and all(
...
def _build_low_quality_chunk_list(
    ...
    for scenario_id in EXPECTED_SCENARIO_IDS:
        ...
        records.append(
            {
                "record_id": f"low-quality-chunk:control:{scenario_id}",
                ...
                "recommendation_zh": "请由业务线人工白箱复核章节边界、页码和来源位置；本记录不代表真实低质量 chunk。",
                "control_metadata_only": True,
                "actual_low_quality_chunk_observed": False,
                "actual_quality_measurement_performed": False,
...
def _build_regression_test_results(predecessor: Mapping[str, Any], samples: list[Mapping[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "actual_quality_regression_performed": False,
        "actual_chunking_quality_baseline_loaded": False,
...
def _build_strategy_applicability_boundary() -> dict[str, Any]:
    return {
        "record_kind": "CHAPTER_AWARE_CHUNKING_STRATEGY_APPLICABILITY_BOUNDARY_CONTROL_ONLY",
        ...
        "actual_chunking_strategy_applicability_validated": False,
...
def _build_regeneration_and_version_rollback_instructions() -> dict[str, Any]:
    return {
        ...
        "return_to": "PHASE3_CHAPTER_AWARE_CHUNKING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
        "in_memory_control_replay_only": True,
        "preserve_phase1_phase2_phase3_artifacts": True,
        "actual_chunk_regeneration_performed": False,
        "actual_chunk_version_rollback_performed": False,
        "actual_identity_or_version_implementation_performed": False,
        "actual_source_document_read_performed": False,
...
def _human_confirmation_prompts() -> list[dict[str, Any]]:
    return [
        {
            "prompt_id": "stage063-p4-chunk-jsonl-human-confirmation",
            "text": "请确认这六条 JSONL 样例仅为控制元数据，不代表真实 chunk 或来源内容。",
            "automatic_confirmation_performed": False,
        },
        {
            "prompt_id": "stage063-p4-coverage-human-confirmation",
            "text": "请确认覆盖率报告只覆盖六类固定控制场景，真实文档覆盖率仍需业务线白箱核验。",
            "automatic_confirmation_performed": False,
        },
        {
            "prompt_id": "stage063-p4-regeneration-human-confirmation",
            "text": "请确认重生成与版本回退说明只允许回到 P3 控制状态，不执行真实资料或索引回退。",
            "automatic_confirmation_performed": False,
        },
    ]
