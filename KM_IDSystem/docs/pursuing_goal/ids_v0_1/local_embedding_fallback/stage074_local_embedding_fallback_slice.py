"""Stage074 P2 的纯内存本地 Embedding 兜底控制切片。

模块只接受五条固定、非业务、reference-only 控制请求，并在内存中机械投影
策略继承、未来队列/缓存/失败重试、成本治理、零值成本、模型版本及完整外部
API 审计字段。它不会读取来源正文、摘要、文本块、物理路径或真实 URI；不会
创建持久记录；也不会选择或下载 provider/模型、执行本地 Embedding、调用外部
API 或消耗模型 Token。
"""
...
CONTROL_ADAPTER_VERSION = "ids.local_embedding_fallback.control_adapter.v0_1.stage074.p2"
CONTROL_FIELDS = ("local_embedding_fallback_requests",)
...
REFERENCE_INPUT_FIELDS = (
    "local_embedding_fallback_request_ref",
...
def build_control_request(scenario: str) -> dict[str, Any]:
    ...
    if scenario not in CONTROL_SCENARIO_CONFIGURATION:
        raise ValueError("unknown local embedding fallback control scenario")
    ...
    marker = f":control:stage074-p2:{scenario}"
    return {
        "local_embedding_fallback_request_ref": f"fallback-request{marker}",
...
def build_control_input() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        "local_embedding_fallback_requests": [
            build_control_request(scenario) for scenario in CONTROL_SCENARIOS
        ]
    }


def execute_local_embedding_fallback_control_slice(
        ...
        cost_projections.append(
            {
                ...
                "cost_estimation_reason": (
                    "CONTROL_NO_LOCAL_OR_EXTERNAL_PAYLOAD_OR_MODEL_CALL_WAS_CREATED"
                ),
            }
        )
    ...
    return {
        ...
        "input_accepted": True,
        "execution_state": "COMPLETED_IN_MEMORY_LOCAL_EMBEDDING_FALLBACK_CONTROL_SLICE",
        "control_request_count": len(requests),
        "actual_input_request_count": 0,
        "control_scenarios_covered": list(CONTROL_SCENARIOS),
        "local_fallback_route_state": (
        ...
        **_runtime_closed_flags(),
        "chinese_feedback": [
            "当前只在内存中投影五条固定本地兜底控制记录，未读取或保留任何真实资料、摘要正文、文本块、路径或业务结论。",
            "有效外部 API 策略从 data source 经 document 自动继承到 chunk；默认 denied，文档不能放宽来源策略，也不能手工标记单个 chunk。",
            "未授权 chunk 不会外发；provider、模型、版本、Token、成本和审计字段只是控制引用与零值投影，未选择模型或创建运行时记录。",
            "任何控制候选都不会被持久化、调度、发送或执行本地 Embedding；未来调用前仍需要授权、预算、完整审计字段及业务线白箱人工复核。",
        ],
    }
...
def _accepted_control_requests(
    ...
    if not isinstance(control_input, Mapping) or set(control_input) != set(CONTROL_FIELDS):
        return None
    requests = control_input.get("local_embedding_fallback_requests")
...
def _control_disposition(resolution: Mapping[str, Any]) -> tuple[str, str]:
    ...
    if resolution["budget_check_state"] == BUDGET_INSUFFICIENT:
        return (
            "CONTROL_QUEUE_PAUSED_BUDGET_INSUFFICIENT",
            "CONTROL_NO_LOCAL_OR_EXTERNAL_TASK_CREATED_WHEN_BUDGET_IS_INSUFFICIENT",
        )
    return (
        "CONTROL_QUEUE_ELIGIBLE_NOT_PERSISTED_RUNTIME_DISABLED",
...
def _runtime_closed_flags() -> dict[str, bool]:
    return {
        ...
        "actual_local_provider_selected": False,
        "actual_local_model_selected": False,
        "actual_local_embedding_execution_performed": False,
        "actual_local_embedding_or_index_written": False,
        ...
        "local_provider_or_model_selected": False,
        "local_embedding_execution_performed": False,
        "local_embedding_or_index_write_performed": False,
