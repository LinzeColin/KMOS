"""Stage065 P3 的工程语义资产分类受控专项场景。

本模块只重放 Stage065 P2 的七条固定、非业务、reference-only 控制记录。
长文档、跨页参数表、施工步骤、参数表、引用页码与重复 chunk 写入边界仅是
控制场景标签；不会读取来源、验证真实切块质量、检测真实重复项，或执行
...
RECORD_KIND = "CONTROLLED_ENGINEERING_SEMANTIC_ASSET_CLASSIFICATION_SCENARIO_REPORT"
PASS_RESULT = (
    "PASS_PHASE3_ENGINEERING_SEMANTIC_ASSET_CLASSIFICATION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
)
FAIL_RESULT = "FAIL_ENGINEERING_SEMANTIC_ASSET_CLASSIFICATION_CONTROLLED_SCENARIOS"
...
PROTECTED_SURFACE_BY_ASSET_TYPE = {
...
SCENARIOS = (
    {
        "scenario_id": "long-document-semantic-asset-control-human-review",
        "scenario_category": "LONG_DOCUMENT_CONTROL",
        "semantic_asset_type": "procedure",
        "protected_semantic_surface": "ENGINEERING_PROCEDURE_STEP",
        "explicit_disposition": (
            "LONG_DOCUMENT_CONTROL_REFERENCE_REQUIRES_HUMAN_BOUNDARY_REVIEW"
        ),
    },
    {
        "scenario_id": "cross-page-parameter-table-semantic-asset-control-human-handling",
        "scenario_category": "CROSS_PAGE_PARAMETER_TABLE_CONTROL",
        "semantic_asset_type": "equipment",
        "protected_semantic_surface": "PARAMETER_TABLE",
        "explicit_disposition": (
            "CROSS_PAGE_PARAMETER_TABLE_CONTROL_REFERENCE_REQUIRES_HUMAN_HANDLING"
        ),
    },
    {
        "scenario_id": "engineering-procedure-semantic-asset-control-human-review",
        "scenario_category": "ENGINEERING_PROCEDURE_CONTROL",
        "semantic_asset_type": "procedure",
        "protected_semantic_surface": "ENGINEERING_PROCEDURE_STEP",
        "explicit_disposition": (
            "ENGINEERING_PROCEDURE_CONTROL_REFERENCE_REQUIRES_HUMAN_BOUNDARY_REVIEW"
        ),
    },
    {
        "scenario_id": "parameter-table-semantic-asset-control-human-review",
        "scenario_category": "PARAMETER_TABLE_CONTROL",
        "semantic_asset_type": "equipment",
        "protected_semantic_surface": "PARAMETER_TABLE",
        "explicit_disposition": (
            "PARAMETER_TABLE_CONTROL_REFERENCE_REQUIRES_HUMAN_BOUNDARY_REVIEW"
        ),
    },
    {
        "scenario_id": "citation-page-semantic-asset-control-human-confirmation",
        "scenario_category": "CITATION_PAGE_TRACEABILITY_CONTROL",
        "semantic_asset_type": "acceptance",
        "protected_semantic_surface": "ACCEPTANCE_CLAUSE",
        "explicit_disposition": (
            "CITATION_PAGE_CONTROL_REFERENCE_REQUIRES_HUMAN_SOURCE_CONFIRMATION"
        ),
    },
    {
        "scenario_id": "duplicate-semantic-asset-embedding-index-control-human-review",
        "scenario_category": "DUPLICATE_CHUNK_EMBEDDING_INDEX_CONTROL",
        "semantic_asset_type": "case",
        "protected_semantic_surface": None,
        "explicit_disposition": (
            "DUPLICATE_CONTROL_REFERENCE_REQUIRES_LATER_IDENTITY_AND_HUMAN_REVIEW"
        ),
    },
)
...
def build_engineering_semantic_asset_classification_phase3_report(
    ...
    records = _semantic_asset_records(phase2_result)
    ...
    phase2_shape_preserved = (
        ...
        and phase2_result.get("control_semantic_asset_classification_request_count") == 7
        and phase2_result.get("actual_input_request_count") == 0
        and phase2_result.get("semantic_asset_classification_record_count") == 7
        and phase2_result.get("engineering_semantic_asset_types_covered")
        == list(PHASE2_ASSET_TYPES)
        and phase2_result.get("protected_semantic_asset_types_covered")
        ...
        and phase2_result.get("control_traceability_reference_count") == 42
        ...
        and phase2_result.get("all_control_records_low_confidence_requires_human_review")
        is True
        and len(records) == 7
        and [record.get("semantic_asset_type") for record in records]
        == list(PHASE2_ASSET_TYPES)
    ...
    return {
        ...
        "unique_control_semantic_asset_record_count": len(
            {
                item["referenced_semantic_asset_classification_record_ref"]
                for item in scenario_results
                if isinstance(
                    item["referenced_semantic_asset_classification_record_ref"], str
                )
            }
        ),
        ...
        "semantic_asset_control_record_can_replace_source_document": False,
        "semantic_asset_control_record_can_become_business_fact_authority": False,
        ...
        **_runtime_closed_flags(),
        ...
        "next_gate": NEXT_GATE,
        "chinese_feedback": [
            "已完成六类固定工程语义资产分类专项控制场景；结果只说明控制处置与引用形状，不代表真实切块质量、分类质量或业务结论。",
            "长文档、跨页参数表、施工步骤、参数表和引用页码均保留业务线白箱人工复核，系统没有读取、切分、分类或写入真实来源。",
...
def _load_phase2_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage065_p2", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage065 P2 semantic asset control slice is unavailable")
...
def _load_phase2_executor() -> Phase2Executor:
    executor = _load_phase2_module().execute_engineering_semantic_asset_classification_control_slice
    if not callable(executor):
        raise RuntimeError("Stage065 P2 semantic asset control executor is unavailable")
    return executor
...
def _semantic_asset_records(result: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    records = result.get("semantic_asset_classification_records")
...
def _record_has_expected_control_shape(record: Mapping[str, Any]) -> bool:
    return (
        set(record) == set(SEMANTIC_ASSET_RECORD_FIELDS)
        and record.get("classification_status")
        == "CONTROL_LABEL_PROJECTED_LOW_CONFIDENCE_REQUIRES_HUMAN_REVIEW"
        and record.get("human_review_state")
...
def _evaluate_scenario(
    ...
    asset_type = str(scenario["semantic_asset_type"])
    record = _record_for_asset_type(asset_type, records)
    traceability_preserved = _control_traceability_preserved(record)
    control_reference_only = _control_reference_only(record)
    expected_record = (
        record is not None
        and record.get("semantic_asset_type") == asset_type
    ...
    return {
        ...
        "referenced_semantic_asset_classification_record_ref": (
            record.get("semantic_asset_classification_record_ref") if record else None
        ),
        "referenced_semantic_asset_type": asset_type,
        ...
        "chunk_id": record.get("chunk_id") if record else None,
        "chunk_hash": record.get("chunk_hash") if record else None,
...
def _record_for_asset_type(
    asset_type: str, records: Sequence[Mapping[str, Any]]
) -> Mapping[str, Any] | None:
    return next(
        (record for record in records if record.get("semantic_asset_type") == asset_type),
        None,
    )
