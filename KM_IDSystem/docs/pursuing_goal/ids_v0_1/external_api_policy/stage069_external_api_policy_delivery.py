"""Stage069 P4 的外部 API 策略继承 metadata-only 交付证据。

本模块只从 P3 五条固定、非业务、reference-only 控制场景派生内存交付样例、
审计日志投影样例、零成本估算、失败处理、未外发原因、查询和回滚说明。它不读取
来源、不保留摘要或文本块、不写审计日志或队列，也不选择 provider/模型、调用外部
API、消耗 Token、连接数据库或启动运行时服务。
"""
...
SCHEMA_VERSION = "ids.stage069.external_api_policy.phase4.delivery.v1"
RECORD_KIND = "EXTERNAL_API_POLICY_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_PHASE4_EXTERNAL_API_POLICY_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EXTERNAL_API_POLICY_DELIVERY_EVIDENCE"
ENTRY_GATE = "IDS-STAGE069-P4-GATE"
NEXT_GATE = "IDS-STAGE069-REVIEW-GATE"
P3_PASS_RESULT = "PASS_PHASE3_EXTERNAL_API_POLICY_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"

POLICY_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_EXTERNAL_API_POLICY_SAMPLE_NOT_REAL_PAYLOAD"
AUDIT_LOG_SAMPLE_KIND = "CONTROL_AUDIT_LOG_SAMPLE_NOT_PERSISTED"
COST_ESTIMATE_SAMPLE_KIND = "CONTROL_ZERO_COST_ESTIMATE_NOT_PROVIDER_PRICE"
FAILURE_HANDLING_KIND = "CONTROL_FAILURE_HANDLING_RESULT_NOT_REAL_FAILURE_RECORD"
NON_EXTERNALIZED_DATA_KIND = "CONTROL_NON_EXTERNALIZED_REFERENCE_RECORD_NOT_REAL_DATA"
QUERY_INSTRUCTION_KIND = (
    "EXTERNALIZATION_RECORD_QUERY_INSTRUCTIONS_IN_MEMORY_CONTROL_ONLY"
)
ROLLBACK_INSTRUCTION_KIND = "EXTERNAL_API_POLICY_ROLLBACK_INSTRUCTIONS_CONTROL_REPLAY_ONLY"
...
def build_external_api_policy_phase4_delivery_report(
    ...
    non_externalized_data_records = [_non_externalized_data_record(item) for item in scenarios]
    ...
    valid = (
        ...
        and all(item["externalization_performed"] is False for item in non_externalized_data_records)
    ...
    return {
        ...
        "external_api_policy_samples": policy_samples,
        "external_api_policy_sample_lines": tuple(
        ...
        **_runtime_closed_flags(),
        ...
        "chinese_feedback": [
            "已从五条固定控制场景生成策略样例、审计日志投影样例、零成本估算和失败处理结果；它们不是实际外发、审计日志、成本记录或业务资料。",
            "五条控制引用均未外发：denied 被策略阻断，摘要和全文引用仍需业务线白箱复核且运行时关闭，预算不足项保持暂停；没有形成真实载荷。",
            "外发记录查询仅说明在本报告内按场景和控制引用核对投影；当前没有持久审计日志、真实外发记录或可供查询的生产历史。",
            "如需撤回本 phase，只撤回 P4 交付工件并回到 P3 控制场景，不改动真实资料、审计日志、成本、队列、数据库、OVH 或部署。",
        ],
    }


def _load_phase3_report_provider() -> Phase3ReportProvider:
    module_path = Path(__file__).with_name("stage069_external_api_policy_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage069_p3", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage069 P3 controlled-scenarios module is unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    provider = getattr(module, "build_external_api_policy_phase3_report", None)
    if not callable(provider):
        raise RuntimeError("Stage069 P3 report provider is unavailable")
    return provider
...
def _phase3_report_is_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("external_api_policy_scenario_can_replace_source_document") is False
        and report.get("external_api_policy_scenario_can_become_business_fact_authority")
        is False
...
def _policy_sample(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "sample_id": f"external-api-policy-delivery-sample:{item['scenario_id']}",
...
def _audit_log_sample(item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "audit_log_sample_id": f"external-api-audit-log-sample:{item['scenario_id']}",
...
def _control_audit_projection(item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        return {}
    marker = f":control:stage069-p2:{phase2_scenario}"
    return {
        "external_api_audit_ref": item["referenced_external_api_audit_ref"],
...
def _audit_log_sample_has_exact_projection(item: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and all(
            isinstance(projection.get(field), str) and ":control:" in projection[field]
            for field in (
                "external_api_audit_ref",
                "data_source_ref",
                "document_ref",
...
def _cost_estimate_sample(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "cost_estimate_id": f"external-api-cost-estimate-sample:{item['scenario_id']}",
...
def _failure_handling_result(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "failure_handling_id": f"external-api-failure-handling:{item['scenario_id']}",
...
def _failure_state(item: Mapping[str, Any]) -> str:
    if item["effective_external_api_policy"] == "denied":
        return "POLICY_DENIED_NO_EXTERNAL_PAYLOAD"
    if item["observed_queue_state"] == "CONTROL_QUEUE_PAUSED_BUDGET_INSUFFICIENT":
        return "BUDGET_INSUFFICIENT_PAUSED_NO_EXTERNALIZATION"
    if item["effective_external_api_policy"] == "summary_only":
        return "WHITEBOX_REVIEW_REQUIRED_SUMMARY_REFERENCE_NOT_EXTERNALIZED"
    return "WHITEBOX_REVIEW_REQUIRED_TEXT_REFERENCE_NOT_EXTERNALIZED"


def _non_externalized_data_record(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "record_id": f"external-api-non-externalized-reference:{item['scenario_id']}",
...
def _non_externalization_reason(item: Mapping[str, Any]) -> str:
    if item["effective_external_api_policy"] == "denied":
        return "默认 denied 策略阻断，未形成任何外发载荷。"
    if item["observed_queue_state"] == "CONTROL_QUEUE_PAUSED_BUDGET_INSUFFICIENT":
        return "预算不足控制状态暂停，未形成任何外发载荷。"
    if item["effective_external_api_policy"] == "summary_only":
...
def _query_instructions(predecessor_valid: bool) -> dict[str, Any]:
    return {
        ...
        "query_scope": "仅本 P4 内存控制交付报告，不是持久审计日志或生产外发历史。",
        "query_keys": [
            "scenario_id",
            "external_api_audit_ref",
        ...
        "query_instruction_zh": "按场景 ID 或 :control: 审计、策略、队列引用核对本报告的投影样例；不得将结果解释为真实外发记录。",
...
def _rollback_instructions() -> dict[str, Any]:
    return {
        ...
        "query_after_rollback_instruction": "回退后只可重放 P3 控制场景；P4 内存样例不形成持久外发或审计记录。",
...
def _human_confirmation_prompts() -> list[str]:
    return [
        "请确认：五条策略、审计和成本样例均是控制元数据，不是实际外发、审计日志、成本或模型调用。",
        "请确认：未外发原因仅覆盖固定控制引用；真实资料是否可外发仍须业务线白箱人工复核。",
        "请确认：回滚只回到 P3 控制场景，查询只核对内存投影，不触及真实审计、队列、成本、OVH 或生产。",
    ]
