"""Stage053 P3 的按页 OCR 输出质量受控场景。

模块只重放 P2 的固定四页纯内存 control 输出。五类场景是标量类别，
...
SCHEMA_VERSION = "ids.stage053.per_page_ocr_output.phase3.quality_scenarios.v1"
RECORD_KIND = "CONTROLLED_PER_PAGE_OCR_QUALITY_SCENARIO_REPORT"
PASS_RESULT = "PASS_PHASE3_PER_PAGE_CONTROLLED_OCR_QUALITY_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_PER_PAGE_CONTROLLED_OCR_QUALITY_SCENARIOS"
NEXT_GATE = "IDS-STAGE053-P4-GATE"
...
SCENARIOS = (
    ...
    },
    {
        ...
        "expected_language": "SIMPLIFIED_CHINESE_AND_ENGLISH",
...
OutputExecutor = Callable[[Mapping[str, object]], Mapping[str, Any]]


def build_per_page_phase3_quality_report(
    output_executor: OutputExecutor | None = None,
) -> dict[str, Any]:
    """重放 P2 control 输出，返回不含 OCR 文本或来源内容的 P3 质量处置报告。"""

    executor = output_executor or _load_phase2_executor()
    output_result = executor(_phase2_control_input())
    pages_by_number = _pages_by_number(output_result)
    side_effect_free = all(output_result.get(field) is False for field in SIDE_EFFECT_FIELDS)
    cache_boundary_preserved = (
        output_result.get("cache_policy") == CACHE_POLICY
        and output_result.get("cache_created") is False
        and output_result.get("cache_ref") is None
        and output_result.get("cache_write_performed") is False
    )
    results = [
    ...
    output_shape_preserved = (
        output_result.get("input_accepted") is True
        and output_result.get("job_state") == "COMPLETED"
        and output_result.get("page_output_count") == 4
    ...
    return {
        ...
        "control_output_reexecuted": True,
        "control_output_shape_preserved": output_shape_preserved,
...
def _load_phase2_executor() -> OutputExecutor:
    path = Path(__file__).with_name("stage053_per_page_ocr_output_slice.py")
    spec = importlib.util.spec_from_file_location("stage053_per_page_ocr_output_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage053 P2 per-page control output slice is unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.execute_per_page_controlled_ocr_output


def _phase2_control_input() -> dict[str, object]:
    """仅返回 P2 已固定的四页 control 输入，不表示文件、页面或 OCR 文本。"""
...
def _pages_by_number(output_result: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    pages = output_result.get("page_outputs")
...
def _evaluate_scenario(
    ...
    expected_page_ref = f"source-page:control:stage053-p2:{page_number}"
    review_required = quality_disposition.startswith("DEGRADED_EVIDENCE")
    expected_symbolic_output = page_state != "OCR_PAGE_FAILED_EXPLICIT"
    expectation_met = (
        ...
        and (
            page.get("text_output_kind") == SYMBOLIC_OUTPUT_KIND
            if expected_symbolic_output
            else page.get("text_output_kind") == "NO_OUTPUT"
        )
    ...
    return {
        ...
        "symbolic_output_retained": False,
