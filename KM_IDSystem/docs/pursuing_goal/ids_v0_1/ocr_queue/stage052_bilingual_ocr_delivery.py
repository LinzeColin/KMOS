"""Stage052 P4 的中英文 OCR metadata-only 交付证据。
...
SCHEMA_VERSION = "ids.stage052.bilingual_ocr.phase4.delivery.v1"
RECORD_KIND = "CONTROLLED_BILINGUAL_OCR_DELIVERY_REPORT"
OUTPUT_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_BILINGUAL_OCR_OUTPUT_SAMPLE_NOT_REAL_OCR"
FAILURE_RECORD_KIND = "CONTROLLED_BILINGUAL_OCR_FAILURE_LIST_ENTRY_NOT_RUNTIME"
REVIEW_ROUTE_PROOF_KIND = "DECLARED_BILINGUAL_REVIEW_ROUTE_PROOF_NOT_QUEUED"
PASS_RESULT = "PASS_PHASE4_BILINGUAL_OCR_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_BILINGUAL_OCR_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE052-REVIEW-GATE"
P3_PASS_RESULT = "PASS_PHASE3_BILINGUAL_CONTROLLED_OCR_QUALITY_SCENARIOS_RUNTIME_DISABLED"
...
def build_bilingual_phase4_delivery_report(
    quality_report_provider: QualityReportProvider | None = None,
) -> dict[str, Any]:
    """派生可复核的 P4 交付证据；所有样例只保留控制元数据。"""
    ...
    return {
        ...
        "human_confirmation_prompts_zh": [
            {
                "prompt_id": "confirm-bilingual-control-evidence-boundary",
                "text": "请业务线确认：本页仅为中英文 OCR control 交付元数据，不可替代真实 OCR 结果或高可信证据。",
                "automatic_confirmation_performed": False,
            },
            {
                "prompt_id": "confirm-bilingual-degraded-review-ownership",
                "text": "请业务线确认：低置信和中英文混合 control 仅保留降级提示，实际复核仍由 Stage054 独立处理。",
                "automatic_confirmation_performed": False,
            },
            {
                "prompt_id": "confirm-bilingual-cache-boundary",
        ...
        "rollback": {
            "return_to": "PHASE3_BILINGUAL_CONTROLLED_QUALITY_SCENARIOS_ENGINE_DISABLED",
            "revertable_artifacts": [
                "Stage052 Phase4 bilingual delivery report module",
                "Stage052 Phase4 bilingual delivery contract",
                "Stage052 Phase4 focused tests",
                "Stage052 Phase4 governance projection",
            ],
...
def _load_phase3_report_provider() -> QualityReportProvider:
    path = Path(__file__).with_name("stage052_bilingual_ocr_quality_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage052_p3_quality_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage052 P3 bilingual quality scenarios are unavailable")
...
def _build_confidence_report(
    ...
    return {
        "report_kind": "CONTROLLED_BILINGUAL_CONFIDENCE_SUMMARY_NOT_REAL_OCR_ACCURACY",
...
def _cache_rerun_instructions() -> dict[str, Any]:
    return {
        ...
        "rerun_instructions_zh": [
            "仅重放 P3 的五类固定非业务中英文 OCR control 报告。",
