"""Stage052 P2 的纯内存中英文 OCR 控制队列切片。

模块只将固定、非业务的控制页标记映射为逐页结构与可解释状态。
...
SCHEMA_VERSION = "ids.stage052.bilingual_ocr.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_BILINGUAL_OCR_PAGE_OUTPUT"
CONTROL_ADAPTER_VERSION = "ids.bilingual_ocr.control_adapter.v0_1.stage052.p2"
SOURCE_IDENTITY_REF = "source:control:stage052-p2"
JOB_REF = "ocr-job:control:stage052-p2"
...
SOURCE_REF_PATTERN = re.compile(r"^source:control:stage052-p2$")
CONTROL_OUTPUT_PROFILES = {
    "CONTROL_ZH_PAGE": "SIMPLIFIED_CHINESE",
    "CONTROL_EN_PAGE": "ENGLISH",
    "CONTROL_MIXED_ZH_EN_PAGE": "MIXED_ZH_EN",
}


def execute_bilingual_controlled_ocr_queue(
...
def _accepted_control(
    ...
    if normalized["input_kind_hint"] not in ALLOWED_INPUT_KINDS:
        return None
    if normalized["parser_output_status"] != "CONTROL_BILINGUAL_OCR_QUEUE_CANDIDATE":
        return None
    if normalized["source_page_count_ref"] != "page-count:control:4":
        return None
    if normalized["language_profile"] != "SIMPLIFIED_CHINESE_AND_ENGLISH":
        return None
    if normalized["ocr_request_reason"] != "CONTROL_BILINGUAL_OUTPUT_SHAPE":
        return None
    if normalized["cache_policy_ref"] != "cache-policy:stage052-p2:in-memory":
        return None
...
def _page_output(
    ...
    if page_outcome == "OCR_PAGE_FAILED":
        ...
        feedback = "控制页识别失败已明确记录，未丢弃或创建高可信证据。"
    ...
    return {
        "source_identity_ref": reference["source_identity_ref"],
        "source_page_ref": f"source-page:control:stage052-p2:{page_number}",
        "ocr_text": ocr_text,
        "language_profile": language_profile,
        "confidence_level": confidence_level,
