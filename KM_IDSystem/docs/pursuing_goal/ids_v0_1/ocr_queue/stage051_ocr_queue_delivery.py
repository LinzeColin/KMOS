"""Stage051 P4 的 OCR 队列交付证据投影。

本模块只从 P3 的固定非业务标量 control 报告派生交付元数据。它不读取 PDF、图片、
页面或 OCR 文本，不选择或调用 OCR 引擎，也不创建队列、缓存、复核记录或运行时。
"""
...
SCHEMA_VERSION = "ids.stage051.ocr_queue.phase4.delivery.v1"
RECORD_KIND = "CONTROLLED_OCR_QUEUE_DELIVERY_REPORT"
OUTPUT_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_OCR_OUTPUT_SAMPLE_NOT_REAL_OCR"
FAILURE_RECORD_KIND = "CONTROLLED_OCR_FAILURE_LIST_ENTRY_NOT_RUNTIME"
REVIEW_ROUTE_PROOF_KIND = "DECLARED_REVIEW_ROUTE_PROOF_NOT_QUEUED"
PASS_RESULT = "PASS_PHASE4_OCR_QUEUE_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_OCR_QUEUE_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE051-REVIEW-GATE"
P3_PASS_RESULT = "PASS_PHASE3_CONTROLLED_OCR_QUALITY_SCENARIOS_RUNTIME_DISABLED"
...
def build_phase4_delivery_report(
    quality_report_provider: QualityReportProvider | None = None,
) -> dict[str, Any]:
    """派生可复核的 P4 交付证据；所有样例仅保留控制元数据。"""
    ...
    return {
        ...
        "quality_limitations_zh": [
            "本交付只来自五类非业务标量 control，不能代表真实扫描件、真实 OCR 文本或识别准确率。",
            "表格图片 control 未执行表结构提取；候选、降级和失败处置不能直接进入高可信证据层。",
            "低置信和中英文混合情形只声明后续 Stage054 复核路由，不创建实际人工复核任务。",
        ],
        "human_confirmation_prompts_zh": [
            {
                "prompt_id": "confirm-control-evidence-boundary",
                "text": "请业务线确认：本页仅为 control 交付元数据，不可替代真实 OCR 结果或高可信证据。",
                "automatic_confirmation_performed": False,
            },
            {
                "prompt_id": "confirm-degraded-review-ownership",
                "text": "请业务线确认：低置信和中英文混合 control 仅保留降级提示，实际复核仍由 Stage054 独立处理。",
                "automatic_confirmation_performed": False,
            },
            {
                "prompt_id": "confirm-cache-boundary",
        ...
        "rollback": {
            "return_to": "PHASE3_CONTROLLED_OCR_QUALITY_SCENARIOS_ENGINE_DISABLED",
            "revertable_artifacts": [
                "Stage051 Phase4 delivery report module",
                "Stage051 Phase4 delivery contract",
                "Stage051 Phase4 focused tests",
                "Stage051 Phase4 governance projection",
            ],
...
def _load_phase3_report_provider() -> QualityReportProvider:
    path = Path(__file__).with_name("stage051_ocr_queue_quality_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage051_p3_quality_scenarios", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage051 P3 quality scenarios are unavailable")
...
def _build_confidence_report(
    ...
    return {
        "report_kind": "CONTROLLED_CONFIDENCE_SUMMARY_NOT_REAL_OCR_ACCURACY",
