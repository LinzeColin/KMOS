"""Stage051 P2 的纯内存 OCR 队列控制切片。

该模块只把固定、非业务的页控制记录映射为逐页 OCR 结构与可解释状态。
...
SCHEMA_VERSION = "ids.stage051.ocr_queue.phase2.v1"
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_OCR_QUEUE_PAGE_OUTPUT"
CONTROL_ADAPTER_VERSION = "ids.ocr_queue.control_adapter.v0_1.stage051.p2"
SOURCE_IDENTITY_REF = "source:control:stage051-p2"
JOB_REF = "ocr-job:control:stage051-p2"
...
ALLOWED_OUTCOMES = {"OCR_OUTPUT_READY", "OCR_PAGE_FAILED"}
SOURCE_REF_PATTERN = re.compile(r"^source:control:stage051-p2$")
CONTROL_TEXT_PROFILES = {
    "中文控制页": "SIMPLIFIED_CHINESE",
    "English control page": "ENGLISH",
    "中英 mixed control page": "MIXED_ZH_EN",
}


def execute_controlled_ocr_queue(
    control_input: Mapping[str, object] | object,
) -> dict[str, Any]:
    """执行受控内存切片，返回不含真实来源内容的逐页结构。"""
...
def _accepted_control(
    ...
    if normalized["input_kind_hint"] not in ALLOWED_INPUT_KINDS:
        return None
    if normalized["parser_output_status"] != "CONTROL_OCR_QUEUE_CANDIDATE":
        return None
    if normalized["source_page_count_ref"] != "page-count:control:4":
        return None
    if normalized["language_profile"] != "SIMPLIFIED_CHINESE_AND_ENGLISH":
        return None
    if normalized["ocr_request_reason"] != "CONTROL_LOW_TEXT_COVERAGE_BASELINE":
        return None
    if normalized["cache_policy_ref"] != "cache-policy:stage051-p2:in-memory":
        return None
...
def _accepted_page(page: object) -> dict[str, object] | None:
    ...
    control_text = page.get("control_text")
    ...
    if page_outcome == "OCR_PAGE_FAILED":
        if (
            control_text is not None
            or language_profile != "UNKNOWN"
            or confidence_level != "UNKNOWN"
        ):
            return None
    elif (
        not isinstance(control_text, str)
        or CONTROL_TEXT_PROFILES.get(control_text) != language_profile
        or confidence_level == "UNKNOWN"
    ):
        return None
    return {
...
def _page_output(
    ...
    if page_outcome == "OCR_PAGE_FAILED":
        ...
        feedback = "当前控制页已形成候选逐页输出，尚未进行质量评估。"
        eligibility = CANDIDATE_ONLY
        review_route = NO_REVIEW_ROUTE
        ocr_text = page["control_text"]
    return {
        "source_identity_ref": reference["source_identity_ref"],
        "source_page_ref": f"source-page:control:stage051-p2:{page_number}",
        "ocr_text": ocr_text,
        "language_profile": language_profile,
        "confidence_level": confidence_level,
