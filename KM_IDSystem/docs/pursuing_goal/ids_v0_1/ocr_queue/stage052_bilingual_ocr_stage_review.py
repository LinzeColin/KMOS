"""Stage052 中英文 OCR 合同的独立整阶段本地复审。

本模块只读取已提交的 P1--P4 合同，并重放 P3/P4 的固定非业务控制报告。
输出只保留计数、状态和边界结论；不返回 OCR 文本、控制文本、来源正文、
真实路径或业务资料，也不启动 OCR、人工复核、缓存或外部运行时。
"""
...
SCHEMA_VERSION = "ids.stage052.bilingual_ocr.stage_review.v1"
RECORD_KIND = "STAGE052_CONTROLLED_BILINGUAL_OCR_STAGE_REVIEW"
PASS_RESULT = "PASS_REVIEWED_LOCAL_BILINGUAL_OCR_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_CLOSED_STAGE052_BILINGUAL_OCR_REVIEW"
REVIEW_GATE = "IDS-STAGE052-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE053-P1-GATE"
...
SOURCE_AUTHORITIES = {
    "phase1": "FROZEN_TASKPACK_TEXT_AND_STAGE051_REVIEW_ARTIFACTS",
    "phase2": "FROZEN_TASKPACK_TEXT_AND_STAGE051_REVIEW_ARTIFACTS",
    "phase3": "FROZEN_TASKPACK_TEXT_AND_STAGE052_PHASE1_PHASE2_ARTIFACTS",
    "phase4": "FROZEN_TASKPACK_TEXT_AND_STAGE052_PHASE1_PHASE3_ARTIFACTS",
}
REFERENCE_FIELDS = (
    "source_identity_ref",
    "input_kind_hint",
...
_CONTRACT_PATHS = {
    "phase1": _BASE / "stage052_bilingual_ocr_contract.json",
    "phase2": _BASE / "stage052_bilingual_ocr_slice_contract.json",
    "phase3": _BASE / "stage052_bilingual_ocr_quality_scenarios_contract.json",
    "phase4": _BASE / "stage052_bilingual_ocr_delivery_contract.json",
}
_MODULE_PATHS = {
    "phase3": _BASE / "stage052_bilingual_ocr_quality_scenarios.py",
    "phase4": _BASE / "stage052_bilingual_ocr_delivery.py",
}
...
def build_stage052_review_report(
    ...
    review_invariants = {
        ...
        "explicit_disposition_and_no_silent_drop_preserved": (
            ...
            and _mapping_or_empty(delivery_report.get("confidence_report")).get(
                "scenario_count"
            )
            == 5
        ),
    ...
    findings = [] if review_valid else ["STAGE052_REVIEW_INVARIANT_NOT_MET"]

    return {
        "schema_version": SCHEMA_VERSION,
        "record_kind": RECORD_KIND,
        "stage": "STAGE-052",
        "task_id": "IDS-V0_1-STAGE052-REVIEW",
        "acceptance_id": "ACC-STAGE-052",
        "source_authority": (
            "FROZEN_TASKPACK_TEXT_STAGE052_P1_TO_P4_AND_STAGE051_REVIEW_ARTIFACTS"
        ),
        ...
        "controlled_replay": {
            ...
            "phase1_per_page_output_field_count": _mapping_or_empty(
                contracts["phase1"].get("per_page_output_reference")
            ).get("field_count"),
        ...
        "stage053_started": False,
        "stage053_entry_allowed": False,
...
def _load_quality_report_provider() -> ReportProvider:
    module = _load_module("stage052_review_phase3", _MODULE_PATHS["phase3"])
    return module.build_bilingual_phase3_quality_report


def _load_delivery_report_provider() -> ReportProvider:
    module = _load_module("stage052_review_phase4", _MODULE_PATHS["phase4"])
    return module.build_bilingual_phase4_delivery_report


def _load_module(name: str, path: Path):
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Stage052 review dependency is unavailable: {path.name}")
...
def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    page_output = _mapping_or_empty(contract.get("per_page_output_reference"))
    ...
    return (
        contract.get("schema_version") == "ids.stage052.bilingual_ocr.phase1.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE052-P1"
...
def _phase2_slice_valid(contract: Mapping[str, Any]) -> bool:
    ...
    cache = _mapping_or_empty(contract.get("cache_boundary"))
    return (
        contract.get("schema_version") == "ids.stage052.bilingual_ocr.phase2.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE052-P2"
...
def _phase3_scenarios_valid(
    ...
    return (
        ...
        == "ids.stage052.bilingual_ocr.phase3.quality_scenarios.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE052-P3"
        ...
        == "PASS_PHASE3_BILINGUAL_CONTROLLED_OCR_QUALITY_SCENARIOS_RUNTIME_DISABLED"
...
def _phase4_delivery_valid(
    ...
    return (
        contract.get("schema_version")
        == "ids.stage052.bilingual_ocr.phase4.delivery.v1"
        and contract.get("task_id") == "IDS-V0_1-STAGE052-P4"
        and contract.get("valid_result")
        == "PASS_PHASE4_BILINGUAL_OCR_DELIVERY_RUNTIME_DISABLED"
        and contract.get("execution_ready") is False
...
def _metadata_only_delivery_boundary(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and all(
            ...
            == "DELIVERY_METADATA_ONLY_BILINGUAL_OCR_OUTPUT_SAMPLE_NOT_REAL_OCR"
        ...
        and all(
            ...
            == "CONTROLLED_BILINGUAL_OCR_FAILURE_LIST_ENTRY_NOT_RUNTIME"
        ...
        and all(
            item.get("record_kind")
            == "DECLARED_BILINGUAL_REVIEW_ROUTE_PROOF_NOT_QUEUED"
...
def _rollback_chain_preserved(
    ...
    return (
        ...
        == "PHASE3_BILINGUAL_CONTROLLED_QUALITY_SCENARIOS_ENGINE_DISABLED"
        ...
        == "PHASE2_BILINGUAL_CONTROLLED_QUEUE_SLICE_ENGINE_DISABLED"
        and _mapping_or_empty(contracts["phase2"].get("rollback_contract")).get(
            "return_to"
        )
        == "PHASE1_BILINGUAL_OCR_BOUNDARY_RUNTIME_DISABLED"
