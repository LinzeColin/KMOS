"""Stage052 P3 的中英文 OCR 质量受控场景。

模块只重放 P2 的固定四页纯内存 control 队列。五类场景是标量类别，
...
SCHEMA_VERSION = "ids.stage052.bilingual_ocr.phase3.quality_scenarios.v1"
RECORD_KIND = "CONTROLLED_BILINGUAL_OCR_QUALITY_SCENARIO_REPORT"
PASS_RESULT = "PASS_PHASE3_BILINGUAL_CONTROLLED_OCR_QUALITY_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_BILINGUAL_CONTROLLED_OCR_QUALITY_SCENARIOS"
NEXT_GATE = "IDS-STAGE052-P4-GATE"
...
def build_bilingual_phase3_quality_report(
...
def _load_phase2_executor() -> QueueExecutor:
    path = Path(__file__).with_name("stage052_bilingual_ocr_slice.py")
    spec = importlib.util.spec_from_file_location("stage052_bilingual_ocr_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage052 P2 bilingual control queue slice is unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.execute_bilingual_controlled_ocr_queue
...
def _evaluate_scenario(
    ...
    expected_page_ref = f"source-page:control:stage052-p2:{page_number}"
