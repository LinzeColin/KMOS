# Stage095 Phase 2 · 证据回归测试受控最小切片

## 本轮目标

只将冻结 Stage095 P1 的证据回归合同投影为可执行的纯内存控制切片。切片覆盖证据 schema 绑定、检索证据捕获引用、风险与可信等级引用、撤回与知识库投毒防护引用、关键结论与报告影响关联；它保持 evidence 与 document、chunk、fact、query、answer、report 的关联形状，并以 reference-only 标签表达六类后续验证场景。

## 唯一权威与前序

- 冻结 Stage095 任务包是本 P2 范围和验收的唯一来源。
- Stage095 P1 静态合同与 Stage094 Review 已复审证据撤回控制工件提供控制边界和既有 A/B/C/D/E 标签；来源文档、真实证据账本与业务线白箱人工复核继续承担业务事实权威。
- 本切片不建立第二权威事实源。输入与输出都使用 :control:stage095-p2: 前缀的 reference-only 标签，不包含正文、路径、业务事实、实际风险分值、实际等级、撤回理由或报告状态。

## 固定控制输入与投影

1. 输入固定为 6 条非业务、reference-only 控制请求，每条 21 个字段。场景依次覆盖内部资料不足、低 OCR、旧版本、冲突、撤回与疑似恶意资料。
2. 输入完整保留 Stage095 P1 的 14 个证据回归关联字段，并增加静态可信等级、捕获、风险、降级、报告影响和白箱复核控制状态。
...
本 P2 验收固定控制形状、关联完整性、六类降级或隔离控制状态、失败关闭、全零运行计数、机器事实投影和中文生成视图的一致性。回退只撤回本 P2 的说明、纯内存控制切片、合同、聚焦用例、machine run、机器事实投影、治理路线、生成中文视图与本交接，恢复到 PHASE1_EVIDENCE_REGRESSION_CONTRACT_RUNTIME_DISABLED。Stage095 P1、Stage094 Review、冻结任务包、真实资料、manifest、检索、evidence ledger、audit log、报告、数据库、GitHub、OVH 和应用状态保持原状。下一步仅可在新的独立 run 进入 IDS-STAGE095-P3-GATE。
