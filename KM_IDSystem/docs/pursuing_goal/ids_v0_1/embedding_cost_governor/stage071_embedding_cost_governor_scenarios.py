"""Stage071 P3 的 Embedding 成本治理器受控专项场景。

模块只重放 Stage071 P2 的七条固定、非业务、reference-only ``:control:``
记录。它机械验证默认 denied 不形成外发载荷、summary_only 只保留摘要引用类别、
full_text_allowed 才保留文本块引用类别、三类预算关闭都会暂停，以及每个未来外部
API 调用候选均已有完整审计投影。

所有“载荷”和“成本”仍只是控制引用或零值：不读取或保留正文、摘要正文、文本块、
物理路径、真实 URI、金额或预算；不创建真实队列、缓存、重试、成本或审计记录；
不调用外部 API、不选择模型、不消耗模型 Token，也不产生业务决策。
"""
...
SCHEMA_VERSION = "ids.stage071.embedding_cost_governor.phase3.controlled_scenarios.v1"
RECORD_KIND = "CONTROLLED_EMBEDDING_COST_GOVERNOR_SCENARIO_REPORT"
PASS_RESULT = "PASS_PHASE3_EMBEDDING_COST_GOVERNOR_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EMBEDDING_COST_GOVERNOR_CONTROLLED_SCENARIOS"
NEXT_GATE = "IDS-STAGE071-P4-GATE"
PHASE2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_EMBEDDING_COST_GOVERNOR_CONTROL_SLICE"
...
SCENARIOS = (
    {
        "scenario_id": "denied-policy-blocks-cost-governor-queue-cache-retry-and-externalization-control",
        ...
        "expected_cost_governor_state": "CONTROL_COST_GOVERNOR_BLOCKED_POLICY_DENIED",
        "expected_budget_failure_scope": None,
        ...
        "explicit_disposition": "DENIED_POLICY_BLOCKS_COST_GOVERNOR_QUEUE_CACHE_RETRY_AND_EXTERNALIZATION_WITHOUT_PAYLOAD",
    },
    {
        "scenario_id": "summary-only-policy-limits-control-payload-after-all-budget-gates-pass",
        "scenario_category": "SUMMARY_ONLY_PAYLOAD_BOUNDARY_CONTROL",
        "phase2_control_scenario": "summary_only_inherited_all_budget_gates_pass",
        ...
        "expected_cost_governor_state": "CONTROL_COST_GOVERNOR_ELIGIBLE_NOT_PERSISTED_RUNTIME_DISABLED",
        "expected_budget_failure_scope": None,
    ...
    {
        "scenario_id": "document-restriction-limits-full-text-to-summary-control-after-all-budget-gates-pass",
        "scenario_category": "DOCUMENT_RESTRICTION_PAYLOAD_BOUNDARY_CONTROL",
        "phase2_control_scenario": "document_restricts_full_text_to_summary_only_all_budget_gates_pass",
    ...
    {
        "scenario_id": "full-text-policy-keeps-only-control-text-reference-after-all-budget-gates-pass",
        "scenario_category": "FULL_TEXT_PAYLOAD_BOUNDARY_CONTROL",
        "phase2_control_scenario": "full_text_allowed_all_budget_gates_pass",
        "expected_source_policy": "full_text_allowed",
        "expected_document_policy": None,
    ...
    {
        "scenario_id": "current-batch-budget-insufficient-pauses-full-text-control",
        "scenario_category": "CURRENT_BATCH_BUDGET_PAUSE_CONTROL",
        "phase2_control_scenario": "current_batch_budget_insufficient_pauses_full_text",
        "expected_source_policy": "full_text_allowed",
        "expected_document_policy": None,
        "expected_effective_policy": "full_text_allowed",
        "expected_payload_mode": "FUTURE_AUTHORIZED_CHUNK_TEXT_REFERENCE_ONLY",
        "expected_payload_scope": "CONTROL_CHUNK_TEXT_REFERENCE_ONLY",
        "expected_cost_governor_state": "CONTROL_COST_GOVERNOR_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_budget_failure_scope": "current_batch",
        "expected_queue_state": "CONTROL_QUEUE_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_cache_disposition": "CONTROL_CACHE_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_retry_state": "CONTROL_RETRY_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_audit_disposition": "PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "future_external_api_call_candidate": False,
        "human_handling_required": True,
        "explicit_disposition": "CURRENT_BATCH_BUDGET_INSUFFICIENT_PAUSES_EXTERNAL_API_TASK_WITHOUT_EXTERNALIZATION",
    },
    {
        "scenario_id": "monthly-budget-insufficient-pauses-full-text-control",
        "scenario_category": "MONTHLY_BUDGET_PAUSE_CONTROL",
        "phase2_control_scenario": "monthly_budget_insufficient_pauses_full_text",
        "expected_source_policy": "full_text_allowed",
        "expected_document_policy": None,
        "expected_effective_policy": "full_text_allowed",
        "expected_payload_mode": "FUTURE_AUTHORIZED_CHUNK_TEXT_REFERENCE_ONLY",
        "expected_payload_scope": "CONTROL_CHUNK_TEXT_REFERENCE_ONLY",
        "expected_cost_governor_state": "CONTROL_COST_GOVERNOR_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_budget_failure_scope": "calendar_month",
        "expected_queue_state": "CONTROL_QUEUE_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_cache_disposition": "CONTROL_CACHE_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_retry_state": "CONTROL_RETRY_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_audit_disposition": "PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "future_external_api_call_candidate": False,
        "human_handling_required": True,
        "explicit_disposition": "MONTHLY_BUDGET_INSUFFICIENT_PAUSES_EXTERNAL_API_TASK_WITHOUT_EXTERNALIZATION",
    },
    {
        "scenario_id": "single-task-cap-exceeded-pauses-full-text-control",
        "scenario_category": "SINGLE_TASK_CAP_PAUSE_CONTROL",
        "phase2_control_scenario": "single_task_cap_exceeded_pauses_full_text",
        "expected_source_policy": "full_text_allowed",
        "expected_document_policy": None,
        "expected_effective_policy": "full_text_allowed",
        "expected_payload_mode": "FUTURE_AUTHORIZED_CHUNK_TEXT_REFERENCE_ONLY",
        "expected_payload_scope": "CONTROL_CHUNK_TEXT_REFERENCE_ONLY",
        "expected_cost_governor_state": "CONTROL_COST_GOVERNOR_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_budget_failure_scope": "single_task",
        "expected_queue_state": "CONTROL_QUEUE_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_cache_disposition": "CONTROL_CACHE_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_retry_state": "CONTROL_RETRY_PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "expected_audit_disposition": "PAUSED_THREE_BUDGET_GATES_NOT_ALL_PASSED",
        "future_external_api_call_candidate": False,
        "human_handling_required": True,
        "explicit_disposition": "SINGLE_TASK_CAP_EXCEEDED_PAUSES_EXTERNAL_API_TASK_WITHOUT_EXTERNALIZATION",
    },
)
...
def build_embedding_cost_governor_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 控制切片并验证策略、成本治理、预算、审计与运行时关闭边界。"""
    ...
    cost_queue_cache_retry_boundaries_preserved = all(
        result["observed_cost_governor_state"]
        == result["expected_cost_governor_state"]
        and result["observed_queue_state"] == result["expected_queue_state"]
    ...
    budget_scope_failure_coverage_preserved = tuple(
        result["observed_budget_failure_scope"]
        for result in scenario_results
        if result["expected_budget_failure_scope"] is not None
    ...
    valid = (
        ...
        and cost_queue_cache_retry_boundaries_preserved
        and audit_projection_invariant_preserved
        and future_call_audit_invariant_preserved
        and budget_scope_failure_coverage_preserved
    ...
    return {
        ...
        "control_cost_governor_record_count": len(
            _mapping_sequence(phase2_result.get("cost_governor_records"))
        ),
        ...
        "cost_queue_cache_retry_boundaries_preserved": (
            cost_queue_cache_retry_boundaries_preserved
        ),
        "budget_scope_failure_coverage_preserved": (
            budget_scope_failure_coverage_preserved
        ),
        ...
        "three_budget_scope_paused_count": sum(
            result["observed_queue_state"]
        ...
        "embedding_cost_governor_scenario_can_replace_source_document": False,
        "embedding_cost_governor_scenario_can_become_business_fact_authority": False,
        ...
        **runtime_closed_flags,
        ...
        "next_gate": NEXT_GATE,
        "chinese_feedback": [
            "已重放七条固定 Embedding 成本治理控制场景；结果只验证策略、成本治理、预算暂停和审计投影，不代表真实资料、金额、预算、摘要正文、文本块或外部调用。",
            "denied 固定不形成外发载荷并关闭成本治理、队列、缓存和重试；summary_only 只保留受控摘要引用类别；full_text_allowed 只保留受控文本块引用类别，均没有创建实际载荷。",
            "本批次、自然月和单任务任一预算关闭均固定暂停成本治理、队列、缓存和重试，未创建 provider 客户端或外部任务；策略例外仍需业务线白箱人工复核。",
            "每条控制场景均有十八字段审计投影；未来可能调用的三个控制情形必须先有审计投影，本轮没有持久化审计记录、外部 API 调用或模型 Token。",
        ],
    }
...
def _load_phase2_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        "stage071_embedding_cost_governor_p2", module_path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage071 P2 control slice")
...
def _phase2_executor(module: Any) -> Phase2Executor:
    executor = getattr(module, "execute_embedding_cost_governor_control_slice", None)
    if not callable(executor):
        raise RuntimeError("Stage071 P2 executor is unavailable")
    return executor
...
def _phase2_control_input(module: Any) -> dict[str, object]:
    ...
    if not callable(builder):
        raise RuntimeError("Stage071 P2 control input is unavailable")
    control_input = builder()
    if not isinstance(control_input, Mapping):
        raise RuntimeError("Stage071 P2 control input is invalid")
...
def _phase2_shape_preserved(
    ...
    governor_fields = _field_tuple(phase2_module, "COST_GOVERNOR_FIELDS")
    ...
    governors = _mapping_sequence(phase2_result.get("cost_governor_records"))
    ...
    return (
        ...
        and phase2_result.get("embedding_queue_record_count") == 7
        and len(queue_records) == 7
        and all(
            set(item) == set(queue_fields) | {"control_queue_state", "control_queue_reason"}
            for item in queue_records
        )
        and phase2_result.get("cache_record_count") == 7
        and len(cache_records) == 7
        and all(set(item) == set(cache_fields) for item in cache_records)
        and phase2_result.get("failed_retry_record_count") == 7
        and len(retry_records) == 7
        and all(set(item) == set(retry_fields) for item in retry_records)
        and phase2_result.get("external_api_audit_projection_count") == 7
        and len(audit_projections) == 7
        ...
        and phase2_result.get("all_three_budget_scope_failure_closures_covered")
        is True
        and phase2_result.get("control_cost_governor_blocked_policy_denied_count") == 1
        and phase2_result.get("control_cost_governor_paused_three_budget_gates_count")
        == 3
        and phase2_result.get("control_cost_governor_eligible_not_persisted_count")
        == 3
        and phase2_result.get("control_queue_blocked_policy_denied_count") == 1
        and phase2_result.get("control_queue_paused_three_budget_gates_count") == 3
        ...
        and phase2_result.get("control_cache_paused_three_budget_gates_count") == 3
        ...
        and phase2_result.get("control_retry_paused_three_budget_gates_count") == 3
...
def _phase2_side_effect_free(phase2_result: Mapping[str, Any]) -> bool:
    return (
        phase2_result.get("source_body_summary_body_or_chunk_text_retained") is False
        and all(
            phase2_result.get(field, False) is False
...
def _evaluate_scenario(
    ...
    governor = _cost_governor_for_scenario(phase2_scenario, phase2_result)
    ...
    audit_projection_present = _audit_projection_preserved(
        audit_projection, audit_fields, resolution, governor, queue_record
    )
    ...
    cost_state = (
        governor.get("control_cost_governor_state") if governor is not None else None
    )
    budget_failure_scope = _budget_failure_scope(governor)
    ...
    expectation_met = (
        ...
        and cost_state == scenario["expected_cost_governor_state"]
        and budget_failure_scope == scenario["expected_budget_failure_scope"]
        ...
        and audit_disposition == scenario["expected_audit_disposition"]
        and phase2_side_effect_free
    )
    explicit_disposition = (
    ...
    return {
        ...
        "referenced_cost_governor_request_ref": (
            governor.get("cost_governor_request_ref") if governor else None
        ),
        "referenced_policy_resolution_ref": (
        ...
        "expected_cost_governor_state": scenario["expected_cost_governor_state"],
        "observed_cost_governor_state": cost_state,
        "expected_budget_failure_scope": scenario["expected_budget_failure_scope"],
        "observed_budget_failure_scope": budget_failure_scope,
        ...
        "observed_audit_disposition": audit_disposition,
...
def _resolution_for_scenario(
    ...
    expected_ref = f"policy-resolution:control:stage071-p2:{scenario}"
...
def _cost_governor_for_scenario(
    scenario: str, phase2_result: Mapping[str, Any]
) -> Mapping[str, Any] | None:
    expected_ref = f"cost-governor-request:control:stage071-p2:{scenario}"
    for record in _mapping_sequence(phase2_result.get("cost_governor_records")):
        if record.get("cost_governor_request_ref") == expected_ref:
            return record
    return None
...
def _audit_projection_preserved(
    ...
    return (
        ...
        == governor.get("external_api_audit_ref")
        ...
        == governor.get("provider_ref")
        and audit_projection.get("model_ref") == queue_record.get("model_ref")
        == governor.get("model_ref")
        and audit_projection.get("model_version") == queue_record.get("model_version")
        == governor.get("model_version")
        and audit_projection.get("budget_check_state")
        == resolution.get("budget_check_state")
        and audit_projection.get("token_count") == 0
        and audit_projection.get("cost_estimate") == 0
        and governor.get("estimated_token_count") == 0
        and governor.get("estimated_cost") == 0
    )


def _budget_failure_scope(governor: Mapping[str, Any] | None) -> str | None:
    if governor is None:
        return "INVALID_CONTROL_BUDGET_SCOPE_FAIL_CLOSED"
    failures = []
    if governor.get("batch_budget_check_state") == "CONTROL_BUDGET_INSUFFICIENT":
        failures.append("current_batch")
    if governor.get("monthly_budget_check_state") == "CONTROL_BUDGET_INSUFFICIENT":
        failures.append("calendar_month")
    if governor.get("task_budget_cap_check_state") == "CONTROL_TASK_CAP_EXCEEDED":
        failures.append("single_task")
    if len(failures) == 1:
        return failures[0]
    if failures:
        return "INVALID_CONTROL_BUDGET_SCOPE_FAIL_CLOSED"
    allowed = {
        ...
        "CONTROL_BUDGET_NOT_APPLICABLE_POLICY_DENIED",
    }
    if all(
        governor.get(field) in allowed
        for field in (
