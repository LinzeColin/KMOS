"""Stage066 P4 的 Chunk 覆盖率指标 metadata-only 交付证据。

本模块只由 P3 固定控制场景派生内存 JSONL 样例、控制覆盖率报告、低质量待人工
清单、回归控制结果和回退说明。它不读取来源、不生成真实 chunk、不计算真实覆盖率，
也不写入文件、索引、数据库或运行时服务。
"""
...
SCHEMA_VERSION = "ids.stage066.chunk_coverage_metrics.phase4.delivery.v1"
RECORD_KIND = "CHUNK_COVERAGE_METRICS_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_PHASE4_CHUNK_COVERAGE_METRICS_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_CHUNK_COVERAGE_METRICS_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE066-REVIEW-GATE"
ENTRY_GATE = "IDS-STAGE066-P4-GATE"
...
COVERAGE_REPORT_KIND = (
    "CONTROLLED_CHUNK_COVERAGE_METRICS_COVERAGE_REPORT_NOT_REAL_COVERAGE"
)
LOW_QUALITY_LIST_KIND = (
    "CONTROLLED_LOW_QUALITY_CHUNK_COVERAGE_METRICS_LIST_NOT_REAL_QUALITY_MEASUREMENT"
)
...
REGENERATION_ROLLBACK_KIND = (
    "CHUNK_COVERAGE_METRICS_REGENERATION_AND_VERSION_ROLLBACK_INSTRUCTIONS_CONTROL_REPLAY_ONLY"
)
...
def build_chunk_coverage_metrics_phase4_delivery_report(
    ...
    return {
        ...
        "actual_coverage_metrics_implementation_performed": False,
        ...
        **_runtime_closed_flags(),
        ...
        "chinese_feedback": [
            "已生成六条仅含控制引用的交付样例；它们不是实际 chunk，也不包含来源正文。",
            "覆盖率报告只核对固定控制场景和引用形状；真实页面、分母与覆盖率仍须业务线白箱人工确认。",
            "低质量清单全部是控制边界待人工复核，不能解读为观察到真实质量问题。",
...
def _load_phase3_report_provider() -> Phase3ReportProvider:
    module_path = Path(__file__).with_name("stage066_chunk_coverage_metrics_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage066_p3", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage066 P3 controlled-scenarios module is unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    provider = getattr(module, "build_chunk_coverage_metrics_phase3_report", None)
    if not callable(provider):
        raise RuntimeError("Stage066 P3 report provider is unavailable")
    return provider
...
def _phase3_report_is_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("unique_control_metric_record_count") == 4
        ...
        and report.get("coverage_metric_control_record_can_replace_source_document") is False
        and report.get("coverage_metric_control_record_can_become_business_fact_authority")
        is False
...
def _jsonl_sample(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "sample_id": f"chunk-coverage-metrics-jsonl-sample:{item['scenario_id']}",
        ...
        "chunk_coverage_request_ref": item["referenced_chunk_coverage_request_ref"],
        ...
        "actual_chunk_coverage_metrics_created": False,
...
def _coverage_report(
    ...
    return {
        ...
        "unique_control_metric_record_count": (
            phase3_report.get("unique_control_metric_record_count") if complete else 0
        ),
        ...
        "control_coverage_complete": complete,
...
def _low_quality_list(scenarios: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            "record_id": f"chunk-coverage-metrics-low-quality-control:{item['scenario_id']}",
            ...
            "recommendation_zh": "请业务线白箱人工复核切块边界、页面追溯和覆盖率分母；当前控制记录不能自动降级或写入。",
            "control_metadata_only": True,
...
def _regression_results(
    ...
    return {
        ...
        "actual_coverage_baseline_compared": False,
...
def _human_confirmation_prompts() -> list[str]:
    return [
        "请确认：六条 JSONL 样例仅为控制元数据，不代表真实 chunk 或来源内容。",
        "请确认：覆盖率与低质量清单仅覆盖固定控制场景，真实页面、分母和质量需业务线白箱人工确认。",
