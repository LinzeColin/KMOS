"""Stage075 P4 的 metadata-only 外部 API 覆盖授权审计交付证据。

本模块只复用已验证的 P3 五条固定、非业务控制场景并重放 P2 控制切片，派生
策略样例、十九字段审计投影、零值成本、失败处理、未外发原因、查询和回滚说明。
所有结果仅存在于当前 Python 进程，不能替代来源文档、持久审计或业务事实，也不会
创建载荷、发起外部调用或消耗模型 Token。
"""
...
RECORD_KIND = "EXTERNAL_API_COVERAGE_AUDIT_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_PHASE4_EXTERNAL_API_COVERAGE_AUDIT_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_EXTERNAL_API_COVERAGE_AUDIT_DELIVERY_EVIDENCE"
ENTRY_GATE = "IDS-STAGE075-P4-GATE"
NEXT_GATE = "IDS-STAGE075-REVIEW-GATE"
P3_PASS_RESULT = "PASS_PHASE3_EXTERNAL_API_COVERAGE_AUDIT_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
P2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_EXTERNAL_API_COVERAGE_AUDIT_CONTROL_SLICE"

POLICY_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_EXTERNAL_API_COVERAGE_AUDIT_POLICY_SAMPLE_NOT_REAL_PAYLOAD"
AUDIT_LOG_SAMPLE_KIND = "CONTROL_EXTERNAL_API_COVERAGE_AUDIT_LOG_SAMPLE_NOT_PERSISTED"
COST_ESTIMATE_SAMPLE_KIND = "CONTROL_ZERO_COST_ESTIMATE_NOT_PROVIDER_PRICE"
FAILURE_HANDLING_KIND = "CONTROL_EXTERNAL_API_COVERAGE_AUDIT_FAILURE_HANDLING_NOT_REAL_FAILURE_RECORD"
NON_EXTERNALIZED_DATA_KIND = "CONTROL_NON_EXTERNALIZED_EXTERNAL_API_COVERAGE_AUDIT_REFERENCE_NOT_REAL_DATA"
OWNER_OVERRIDE_SAMPLE_KIND = "CONTROL_OWNER_FORCED_EGRESS_OVERRIDE_PRECONDITION_NOT_REAL_AUDIT_RECORD"
QUERY_INSTRUCTION_KIND = "EXTERNAL_API_COVERAGE_AUDIT_RECORD_QUERY_INSTRUCTIONS_IN_MEMORY_CONTROL_ONLY"
ROLLBACK_INSTRUCTION_KIND = "EXTERNAL_API_COVERAGE_AUDIT_POLICY_ROLLBACK_INSTRUCTIONS_CONTROL_REPLAY_ONLY"
...
OWNER_FORCED_EGRESS_OVERRIDE_FIELDS = (
    "actor",
    "reason",
    "old_value",
    "new_value",
...
P2_RUNTIME_FALSE_FIELDS = (
    ...
    "actual_owner_forced_egress_override_audit_record_created",
...
def build_external_api_coverage_audit_phase4_delivery_report(
    ...
    audits = _mapping_sequence(phase2_report.get("external_api_coverage_audit_projections"))
    costs = _mapping_sequence(phase2_report.get("cost_control_projections"))
    owner_records = _mapping_sequence(
        phase2_report.get("owner_forced_egress_override_control_projections")
    )

    policy_samples = [_policy_sample(item) for item in scenarios] if predecessors_valid else []
    audit_samples = [
    ...
    failure_results = [
        _failure_handling_result(scenario) for scenario in scenarios
    ] if predecessors_valid else []
    non_externalized_records = [
        _non_externalized_data_record(scenario) for scenario in scenarios
    ] if predecessors_valid else []
    owner_override_sample = (
        _owner_forced_egress_override_sample(owner_records[0])
        if predecessors_valid and owner_records
        else {}
    )
    ...
    delivery_integrity = (
        ...
        and len(non_externalized_records) == len(EXPECTED_SCENARIO_IDS)
        ...
        and all(not item["externalization_performed"] for item in non_externalized_records)
        and _owner_forced_egress_override_sample_is_control_only(owner_override_sample)
        and len(query_instructions["supported_query_keys"]) == 8
    ...
    report: dict[str, Any] = {
        ...
        "non_externalized_data_record_count": len(non_externalized_records),
        "externalization_record_query_key_count": len(
            query_instructions["supported_query_keys"]
        ),
        "owner_forced_egress_override_precondition_sample_count": (
            1 if owner_override_sample else 0
        ),
        "owner_forced_egress_override_field_count": len(
            owner_override_sample.get("precondition_projection", {})
            if isinstance(owner_override_sample, Mapping)
            else {}
        ),
        "owner_forced_egress_override_field_check_count": len(
            owner_override_sample.get("precondition_projection", {})
            if isinstance(owner_override_sample, Mapping)
            else {}
        ),
        ...
        "external_api_coverage_audit_policy_samples": policy_samples,
        "control_audit_log_samples": audit_samples,
        "cost_estimate_samples": cost_samples,
        "failure_handling_results": failure_results,
        "non_externalized_data_records": non_externalized_records,
        "owner_forced_egress_override_precondition_sample": owner_override_sample,
        ...
        "owner_override_control_metadata_can_become_business_fact_authority": False,
        ...
        "stage075_entry_authorized": True,
        "stage075_phase4_entry_authorized": True,
        "phase1_started": True,
        "phase2_started": True,
        "phase3_started": True,
        ...
        "actual_owner_forced_egress_override_audit_record_count": 0,
...
def _default_phase3_report_provider() -> Mapping[str, Any]:
    module = _load_module("stage075_external_api_coverage_audit_scenarios.py")
    return module.build_external_api_coverage_audit_phase3_report()


def _default_phase2_report_provider() -> Mapping[str, Any]:
    module = _load_module("stage075_external_api_coverage_audit_slice.py")
    return module.execute_external_api_coverage_audit_control_slice(
...
def _phase3_report_is_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("owner_forced_egress_override_control_projection_count") == 1
        and report.get("owner_forced_egress_override_field_count")
        == len(OWNER_FORCED_EGRESS_OVERRIDE_FIELDS)
        and report.get("owner_forced_egress_override_field_check_count")
        == len(OWNER_FORCED_EGRESS_OVERRIDE_FIELDS)
        ...
        and report.get("stage074_review_evidence_read") is True
        and report.get("stage075_started") is True
        ...
        and all(
            ...
            and all(
                ...
                and ":control:stage075-p2:" in item[field]
...
def _phase2_report_is_valid(report: Mapping[str, Any]) -> bool:
    audits = _mapping_sequence(report.get("external_api_coverage_audit_projections"))
    costs = _mapping_sequence(report.get("cost_control_projections"))
    owners = _mapping_sequence(
        report.get("owner_forced_egress_override_control_projections")
    )
    return (
        ...
        and set(owners[0]) == set(OWNER_FORCED_EGRESS_OVERRIDE_FIELDS)
        and all(
            item.get("estimated_token_count") == 0
            and item.get("estimated_cost") == 0
            for item in costs
        )
        and report.get("complete_owner_forced_egress_override_audit_required_before_future_policy_change")
        is True
        and report.get("owner_forced_egress_override_business_line_whitebox_human_review_required")
        is True
...
def _audit_references_are_control_only(projection: Mapping[str, Any]) -> bool:
    reference_fields = (
        "external_api_audit_ref",
        "data_source_ref",
        "document_ref",
    ...
    return all(
        ...
        and ":control:stage075-p2:" in projection[field]
        for field in reference_fields
    )
...
def _failure_handling_result(scenario: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "failure_state": failure_states[category],
        "failure_closed": True,
...
def _non_externalized_data_record(scenario: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "scenario_id": scenario["scenario_id"],
        "scenario_category": category,
        "non_externalization_reason": reasons[category],
...
def _owner_forced_egress_override_sample(
    projection: Mapping[str, Any]
) -> dict[str, Any]:
    return {
        "record_kind": OWNER_OVERRIDE_SAMPLE_KIND,
        "precondition_projection": {
            field: projection[field] for field in OWNER_FORCED_EGRESS_OVERRIDE_FIELDS
        },
        "precondition_complete_before_future_policy_change": True,
        "business_line_whitebox_human_review_required": True,
        "actual_override_audit_record_created": False,
        "actual_policy_override_applied": False,
    }


def _owner_forced_egress_override_sample_is_control_only(
    item: Mapping[str, Any]
) -> bool:
    projection = item.get("precondition_projection")
    return (
        item.get("record_kind") == OWNER_OVERRIDE_SAMPLE_KIND
        and isinstance(projection, Mapping)
        and set(projection) == set(OWNER_FORCED_EGRESS_OVERRIDE_FIELDS)
        and all(
            isinstance(value, str) and ":control:stage075-p2:" in value
            for value in projection.values()
        )
        and item.get("precondition_complete_before_future_policy_change") is True
        and item.get("business_line_whitebox_human_review_required") is True
        and item.get("actual_override_audit_record_created") is False
        and item.get("actual_policy_override_applied") is False
    )
...
def _human_confirmation_prompts() -> list[str]:
    return [
        ...
        "确认预算不足样例保持暂停，owner 强制允许外发仍须四字段前置，并可只回退本 P4 到 P3。",
    ]
...
def _chinese_feedback() -> list[str]:
    return [
        ...
        "预算不足时外部 API 候选保持暂停；owner 强制允许外发仍须 actor、reason、old_value、new_value 前置，本步骤可回退到 Stage075 P3。",
    ]
