"""Stage075 P3 的外部 API 覆盖授权审计专项验证与异常场景。

本模块只重放 P2 的五条固定、非业务、reference-only 控制投影，验证策略边界、
预算暂停和审计前置。它不会读取来源正文、摘要或文本块；不会选择 provider/模型、
执行外部 API 覆盖授权审计、创建队列/缓存/审计记录，或调用外部 API。
"""
...
RECORD_KIND = "CONTROL_ONLY_IN_MEMORY_EXTERNAL_API_COVERAGE_AUDIT_SCENARIO_REPORT"
PASS_RESULT = "PASS_PHASE3_EXTERNAL_API_COVERAGE_AUDIT_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_PHASE3_EXTERNAL_API_COVERAGE_AUDIT_CONTROLLED_SCENARIOS"
NEXT_GATE = "IDS-STAGE075-P4-GATE"
PHASE2_EXECUTION_STATE = "COMPLETED_IN_MEMORY_EXTERNAL_API_COVERAGE_AUDIT_CONTROL_SLICE"
...
SCENARIOS = (
    {
        ...
        "expected_payload_scope": "NO_EXTERNAL_PAYLOAD_CREATED",
        ...
        "explicit_disposition": "默认 denied：不形成外发载荷引用，队列、缓存和重试均被阻断。",
    },
    {
        "scenario_id": "S075-P3-002",
        ...
        "expected_payload_scope": "FUTURE_SUMMARY_REFERENCE_ONLY_NO_BODY",
        ...
        "explicit_disposition": "summary_only：只保留未来授权摘要引用，不创建摘要正文或外发载荷。",
    },
    {
        "scenario_id": "S075-P3-003",
        ...
        "future_external_api_call_candidate": True,
        "human_handling_required": True,
        "explicit_disposition": "document 收紧全文来源为摘要：不得升级为文本块引用或外发。",
    },
    {
        "scenario_id": "S075-P3-004",
        ...
        "expected_payload_scope": "FUTURE_CHUNK_TEXT_REFERENCE_ONLY_NO_BODY",
        "expected_queue_state": "CONTROL_QUEUE_ELIGIBLE_NOT_PERSISTED_RUNTIME_DISABLED",
        "expected_cache_disposition": "CONTROL_CACHE_ELIGIBLE_NOT_PERSISTED_RUNTIME_DISABLED",
        "expected_retry_state": "CONTROL_RETRY_NOT_SCHEDULED_RUNTIME_DISABLED",
        "expected_budget_check_state": "CONTROL_BUDGET_AVAILABLE_REFERENCE_ONLY",
        ...
        "future_external_api_call_candidate": True,
        "human_handling_required": True,
        "explicit_disposition": "full_text_allowed：只保留未来授权文本块引用，仍未创建正文载荷或调用。",
    },
    {
        "scenario_id": "S075-P3-005",
        ...
        "future_external_api_call_candidate": False,
        "human_handling_required": True,
        "explicit_disposition": "预算不足：队列、缓存和重试同步暂停，不能形成外部 API 候选。",
    },
)

PAYLOAD_SCOPE_BY_MODE = {
    "NO_EXTERNAL_PAYLOAD_POLICY_DENIED": "NO_EXTERNAL_PAYLOAD_CREATED",
    "FUTURE_AUTHORIZED_SUMMARY_REFERENCE_ONLY": "FUTURE_SUMMARY_REFERENCE_ONLY_NO_BODY",
    "FUTURE_AUTHORIZED_CHUNK_TEXT_REFERENCE_ONLY": "FUTURE_CHUNK_TEXT_REFERENCE_ONLY_NO_BODY",
}
...
RUNTIME_CLOSED_FIELDS = (*P2_RUNTIME_CLOSED_FIELDS, "actual_control_scenario_record_persisted", "actual_external_payload_created", "control_payload_content_retained", "audit_test_execution_performed")

Phase2Executor = Callable[[Mapping[str, object]], object]


def build_external_api_coverage_audit_phase3_report(
    phase2_executor: Phase2Executor | None = None,
) -> dict[str, Any]:
    """重放 P2 固定控制输入并验证外发边界、预算与审计前置。"""

    phase2_module = _load_phase2_module()
    executor = phase2_executor or _phase2_executor(phase2_module)
    raw_result = executor(_phase2_control_input(phase2_module))
    phase2_result = raw_result if isinstance(raw_result, Mapping) else {}
    audit_fields = _field_tuple(phase2_module, "AUDIT_FIELDS")
    owner_override_fields = _field_tuple(
        phase2_module, "OWNER_FORCED_EGRESS_OVERRIDE_AUDIT_FIELDS"
    )
    phase2_shape_preserved = _phase2_shape_preserved(
        phase2_module, phase2_result, audit_fields, owner_override_fields
    )
    phase2_side_effect_free = _phase2_side_effect_free(phase2_result)
    owner_forced_egress_override_precondition_preserved = (
        _owner_forced_egress_override_precondition_preserved(
            phase2_result, owner_override_fields
        )
    )
    scenario_results = [
        _evaluate_scenario(
            scenario, index, phase2_result, audit_fields, phase2_side_effect_free
        )
    ...
    valid = (
        ...
        and owner_forced_egress_override_precondition_preserved
    ...
    return {
        ...
        "control_cache_record_count": _count_records(phase2_result.get("cache_records")),
        "control_failed_retry_record_count": _count_records(
            phase2_result.get("failed_retry_records")
        ),
        "control_cost_governor_projection_count": _count_records(
            phase2_result.get("cost_governor_control_projections")
        ),
        ...
        "control_external_api_audit_projection_count": _count_records(
            phase2_result.get("external_api_coverage_audit_projections")
        ),
        "control_audit_field_count": len(audit_fields),
        "control_audit_field_check_count": len(scenario_results) * len(audit_fields),
        "owner_forced_egress_override_control_projection_count": _count_records(
            phase2_result.get("owner_forced_egress_override_control_projections")
        ),
        "owner_forced_egress_override_field_count": len(owner_override_fields),
        "owner_forced_egress_override_field_check_count": len(owner_override_fields),
        ...
        **runtime_closed_flags,
        "chinese_feedback": [
            ...
            "默认 denied 阻断外发；summary_only 只保留摘要引用，document 收紧不能升级为文本块引用。",
            "full_text_allowed 也只保留未来文本块引用；预算不足时队列、缓存和重试保持暂停。",
            "每个未来调用候选均先具备完整审计控制投影和业务线白箱人工复核前置；本步骤可回退到 Stage075 P2。",
        ],
    }


def _phase2_executor(phase2_module: Any) -> Phase2Executor:
    return phase2_module.execute_external_api_coverage_audit_control_slice
...
def _load_phase2_module() -> Any:
    path = Path(__file__).with_name("stage075_external_api_coverage_audit_slice.py")
    spec = importlib.util.spec_from_file_location("stage075_phase2_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage075 P2 external API coverage audit slice")
...
def _phase2_shape_preserved(
    ...
    owner_override_fields: tuple[str, ...],
) -> bool:
    ...
    cost_governor_fields = _field_tuple(phase2_module, "COST_GOVERNOR_FIELDS")
    ...
    return (
        ...
        and _records_have_exact_shape(
            phase2_result.get("policy_resolutions"), len(PHASE2_SCENARIOS), resolution_fields
        )
        and _records_have_exact_shape(
            phase2_result.get("embedding_queue_records"),
        ...
        and _records_have_exact_shape(
            phase2_result.get("cache_records"), len(PHASE2_SCENARIOS), cache_fields
        )
        and _records_have_exact_shape(
            phase2_result.get("failed_retry_records"), len(PHASE2_SCENARIOS), retry_fields
        )
        and _records_have_exact_shape(
            phase2_result.get("cost_governor_control_projections"),
            len(PHASE2_SCENARIOS),
        ...
        and _records_have_exact_shape(
            phase2_result.get("cost_control_projections"), len(PHASE2_SCENARIOS), cost_fields
        )
        and _records_have_exact_shape(
            phase2_result.get("external_api_coverage_audit_projections"),
            len(PHASE2_SCENARIOS),
        ...
        is True
        and phase2_result.get("owner_forced_egress_override_policy_change_applied")
        is False
        and phase2_result.get(
            "control_output_is_not_actual_queue_cache_cost_model_version_or_audit"
        )
        is True
    )
...
def _phase2_side_effect_free(phase2_result: Mapping[str, Any]) -> bool:
    return (
        all(phase2_result.get(field) is False for field in P2_RUNTIME_CLOSED_FIELDS)
        ...
        is False
        and phase2_result.get("actual_policy_override_applied") is False
        and phase2_result.get("owner_forced_egress_override_policy_change_applied")
        is False
    )
...
def _evaluate_scenario(
    ...
    audit = _record_at(
        phase2_result.get("external_api_coverage_audit_projections"), index
    )
    ...
    audit_required_fields_present = set(audit_fields).issubset(set(audit))
    audit_reference_fields_are_control_only = _audit_references_are_control_only(audit)
    audit_projection_present = (
        set(audit) == set(audit_fields) and audit_required_fields_present
    )
    ...
    model_version_sent_to_external_api = model_version.get("sent_to_external_api") is True
    ...
    expectation_met = (
        ...
        and observed_human_handling_required == scenario["human_handling_required"]
    ...
    return {
        ...
        "referenced_policy_resolution_ref": resolution.get("policy_resolution_ref"),
        ...
        "audit_reference_fields_are_control_only": audit_reference_fields_are_control_only,
        ...
        "actual_model_token_consumption_performed": actual_model_token_consumption_performed,
...
def _audit_references_are_control_only(audit: Mapping[str, Any]) -> bool:
    ...
    return all(
        ...
        and ":control:stage075-p2:" in audit[field]
        for field in fields
    )


def _owner_forced_egress_override_precondition_preserved(
    phase2_result: Mapping[str, Any], owner_override_fields: tuple[str, ...]
) -> bool:
    records = phase2_result.get("owner_forced_egress_override_control_projections")
    return (
        _records_have_exact_shape(records, 1, owner_override_fields)
