"""Stage058 P4 的表格 Schema 推断 metadata-only 交付证据。

模块只从 P3 的六类固定、非业务 control 场景派生交付样例、字段推断引用报告、
质量结果、人工处理建议及重解析/事实回滚说明。它不读取、打开、解析、规范化、
统计或保存任何真实 XLSX、CSV、工作表、单元格、事实或证据内容。
"""
...
SCHEMA_VERSION = "ids.stage058.table_schema_inference.phase4.delivery.v1"
RECORD_KIND = "CONTROLLED_TABLE_SCHEMA_INFERENCE_DELIVERY_REPORT"
OUTPUT_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_SCHEMA_PROFILE_SAMPLE_NOT_REAL_TABLE_FACT"
...
HANDLING_RECORD_KIND = (
    "CONTROLLED_TABLE_EXCEPTION_AND_HUMAN_HANDLING_NOT_REAL_TABLE_OBSERVATION"
)
REPARSE_AND_ROLLBACK_KIND = (
    "REPARSE_AND_FACT_ROLLBACK_INSTRUCTIONS_CONTROL_REPLAY_ONLY"
)
PASS_RESULT = "PASS_PHASE4_TABLE_SCHEMA_INFERENCE_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_TABLE_SCHEMA_INFERENCE_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE058-REVIEW-GATE"
P3_PASS_RESULT = (
...
SCENARIO_FIELD_REFERENCE_LABELS = {
    "empty-table-control-explicit-closed": "candidate_quality_result_ref",
    "merged-cells-control-human-handling": "candidate_equipment_ref",
    "unit-confusion-control-human-handling": "candidate_unit_ref",
    "date-format-variation-control-human-handling": "candidate_date_format_ref",
    "outlier-control-numeric-block": "candidate_field_type",
    "duplicate-row-control-human-handling": "candidate_fact_type",
}
...
def build_table_schema_inference_phase4_delivery_report(
    ...
    return {
        ...
        "quality_limitations_zh": [
            ...
            "字段推断报告只保留 P3 control 的候选引用标签，不构成真实 schema 推断、字段识别、事实抽取、来源追溯或数值统计。",
        ...
        "rollback": {
            ...
            "revertable_artifacts": [
                "Stage058 Phase4 table Schema delivery module",
                "Stage058 Phase4 delivery contract",
                "Stage058 Phase4 focused tests",
                "Stage058 Phase4 governance projection",
            ],
...
def _load_phase3_report_provider() -> QualityReportProvider:
    ...
    spec = importlib.util.spec_from_file_location("stage058_p3_quality_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage058 P3 table Schema quality scenarios are unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.build_table_schema_inference_phase3_report
        ...
        samples.append(
            {
                ...
                "referenced_schema_profile_id": scenario.get(
                    "referenced_schema_profile_id"
                ),
                "referenced_candidate_column_handle": scenario.get(
                    "referenced_candidate_column_handle"
                ),
...
def _build_field_inference_report(
    ...
    for scenario_id in EXPECTED_SCENARIO_IDS:
        if scenario_id not in scenarios:
            continue
        label = SCENARIO_FIELD_REFERENCE_LABELS[scenario_id]
        if label not in labels:
            labels.append(label)
    return {
        "report_kind": FIELD_INFERENCE_REPORT_KIND,
        "control_reference_only": True,
        "schema_profile_candidate_pool_count": _nonnegative_int(
            predecessor.get("unique_schema_profile_candidate_count")
        ),
...
def _human_confirmation_prompts() -> list[dict[str, Any]]:
    return [
        {
            "prompt_id": "confirm-table-schema-delivery-boundary",
            "text": "请业务线确认：本交付只包含表格 Schema control 元数据样例，不可替代真实表格事实、来源证据或数值统计。",
            "automatic_confirmation_performed": False,
        },
        {
            "prompt_id": "confirm-table-schema-human-handling-boundary",
            "text": "请业务线确认：空表、合并单元格、单位、日期、异常值和重复行均保留人工处理建议，当前没有自动解析或写入。",
            "automatic_confirmation_performed": False,
        },
        {
            "prompt_id": "confirm-table-schema-reparse-rollback-boundary",
...
def _is_valid(
    ...
    predecessor_shape_valid = (
        ...
        and predecessor.get("next_gate") == "IDS-STAGE058-P4-GATE"
        ...
        and predecessor.get("unique_schema_profile_candidate_count") == 11
        ...
        and predecessor.get("actual_schema_profile_created") is False
        and predecessor.get("actual_field_mapping_created") is False
    ...
    delivery_shape_valid = len(delivery_samples) == len(EXPECTED_SCENARIO_IDS) and all(
        ...
        and isinstance(item.get("referenced_schema_profile_id"), str)
        and isinstance(item.get("referenced_candidate_column_handle"), str)
    ...
    inference_valid = (
        ...
        and field_inference_report.get("schema_profile_candidate_pool_count") == 11
        ...
        and field_inference_report.get("actual_table_schema_created") is False
