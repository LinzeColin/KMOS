"""Stage059 P3 的事实抽取受控异常场景验证。

模块只重放 Stage059 P2 的两条固定、非业务、reference-only 控制记录和三条
...
SCHEMA_VERSION = "ids.stage059.fact_extraction.phase3.quality_scenarios.v1"
RECORD_KIND = "CONTROLLED_FACT_EXTRACTION_QUALITY_SCENARIO_REPORT"
PASS_RESULT = "PASS_PHASE3_FACT_EXTRACTION_CONTROLLED_QUALITY_SCENARIOS_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_FACT_EXTRACTION_CONTROLLED_QUALITY_SCENARIOS"
NEXT_GATE = "IDS-STAGE059-P4-GATE"
...
SCENARIOS = (
    {
        ...
        "candidate_selector_value": "QUALITY_FACT",
    ...
    },
    {
        ...
        "candidate_selector_field": "fact_type",
        "candidate_selector_value": "PRODUCTION_FACT",
    ...
    },
    {
        ...
        "candidate_selector_field": "unit_ref",
        "candidate_selector_value": "unit-ref:control:stage059-p2:production",
    ...
    },
    {
        ...
        "candidate_selector_field": "record_date_ref",
        "candidate_selector_value": "record-date-ref:control:stage059-p2:production",
    ...
    },
    {
        ...
        "candidate_selector_field": "field_type",
    ...
    },
    {
        ...
        "candidate_selector_field": "fact_type",
        "candidate_selector_value": "INSPECTION_FACT",
...
def build_fact_extraction_phase3_report(
    ...
    phase2_shape_preserved = (
        ...
        and phase2_result.get("fact_extraction_input_record_count") == 2
        and phase2_result.get("structured_fact_candidate_count") == 3
        and phase2_result.get("fact_category_count") == 3
        and set(phase2_result.get("fact_categories", []))
        == {"PRODUCTION_FACT", "QUALITY_FACT", "INSPECTION_FACT"}
        and phase2_result.get("typed_semantic_category_count") == 7
        and phase2_result.get("candidate_field_type_count") == 3
        and phase2_result.get("numeric_field_candidate_count") == 1
        and phase2_result.get("all_control_typed_values_unset") is True
        and phase2_result.get("source_location_binding_candidate_count") == 3
        and phase2_result.get("source_location_references_preserved") is True
        and phase2_result.get("rag_summary_candidate_count") == 0
        and phase2_result.get("rag_summary_deferred_to_stage060") is True
        and len(candidates) == 3
        and all(
            len(candidate) == 25
            and candidate.get("typed_value") is None
            and candidate.get("extraction_state")
...
def _load_phase2_executor() -> Phase2Executor:
    path = Path(__file__).with_name("stage059_fact_extraction_slice.py")
    spec = importlib.util.spec_from_file_location("stage059_fact_extraction_slice", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage059 P2 fact extraction control slice is unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.execute_fact_extraction_control_slice


def _phase2_control_input() -> dict[str, object]:
    """返回 P2 冻结控制引用；它们不表示真实表格、字段或业务事实。"""
...
def _evaluate_scenario(
    ...
    typed_value_unset = candidate is not None and candidate.get("typed_value") is None
    ...
    return {
        ...
        "referenced_fact_id": _value(candidate, "fact_id"),
        "referenced_fact_type": _value(candidate, "fact_type"),
        "referenced_record_type": _value(candidate, "record_type"),
        ...
        "typed_value_unset": typed_value_unset,
