"""Stage057 P4 的 XLSX/CSV 接入 metadata-only 交付证据。

模块只从 P3 的六类固定、非业务 control 场景派生交付样例、字段引用报告、
质量结果、人工处置建议及重解析/事实回滚说明。它不读取、打开、解析、规范化、
...
SCHEMA_VERSION = "ids.stage057.xlsx_csv_ingestion.phase4.delivery.v1"
RECORD_KIND = "CONTROLLED_XLSX_CSV_INGESTION_DELIVERY_REPORT"
OUTPUT_SAMPLE_KIND = "DELIVERY_METADATA_ONLY_TABLE_FACT_SAMPLE_NOT_REAL_TABLE_FACT"
...
PASS_RESULT = "PASS_PHASE4_XLSX_CSV_INGESTION_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_XLSX_CSV_INGESTION_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE057-REVIEW-GATE"
P3_PASS_RESULT = (
...
SCENARIO_FIELD_REFERENCE_LABELS = {
    "empty-table-control-explicit-closed": "quality_result",
    "merged-cells-control-human-handling": "equipment_ref",
    "unit-confusion-control-human-handling": "unit_ref",
    "date-format-variation-control-human-handling": "record_date",
    "outlier-control-numeric-block": "measurement_value",
    "duplicate-row-control-human-handling": "quality_result",
}
...
def build_xlsx_csv_ingestion_phase4_delivery_report(
    ...
    return {
        ...
        "quality_limitations_zh": [
            "本交付只来自六类固定非业务 control，不能代表真实 XLSX/CSV、生产记录、质检记录、工作表、单元格、数值、日期或业务资料质量。",
        ...
        "rollback": {
            ...
            "revertable_artifacts": [
                "Stage057 Phase4 XLSX/CSV delivery module",
                "Stage057 Phase4 delivery contract",
                "Stage057 Phase4 focused tests",
                "Stage057 Phase4 governance projection",
            ],
...
def _load_phase3_report_provider() -> QualityReportProvider:
    ...
    spec = importlib.util.spec_from_file_location("stage057_p3_quality_scenarios", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage057 P3 XLSX/CSV quality scenarios are unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.build_xlsx_csv_ingestion_phase3_report
        ...
        samples.append(
            {
                ...
                "referenced_fact_candidate_ids": scenario.get(
                    "referenced_fact_candidate_ids"
                ),
                "source_identity_ref": scenario.get("source_identity_ref"),
        ...
        records.append(
            {
                ...
                "scenario_category": scenario.get("scenario_category"),
                "quality_disposition": scenario.get("quality_disposition"),
                "human_handling_required": scenario.get("human_handling_required") is True,
...
def _human_confirmation_prompts() -> list[dict[str, Any]]:
    return [
        {
            "prompt_id": "confirm-xlsx-csv-delivery-boundary",
            "text": "请业务线确认：本交付只包含 XLSX/CSV control 元数据样例，不可替代真实表格事实、来源证据或数值统计。",
            "automatic_confirmation_performed": False,
        },
        {
            "prompt_id": "confirm-xlsx-csv-human-handling-boundary",
            "text": "请业务线确认：空表、合并单元格、单位、日期、异常值和重复行均保留人工处理建议，当前没有自动解析或写入。",
            "automatic_confirmation_performed": False,
        },
        {
            "prompt_id": "confirm-xlsx-csv-reparse-rollback-boundary",
...
def _is_valid(
    ...
    predecessor_shape_valid = (
        ...
        and predecessor.get("next_gate") == "IDS-STAGE057-P4-GATE"
        ...
        and predecessor.get("unique_fact_candidate_count") == 10
    ...
    delivery_shape_valid = len(delivery_samples) == len(EXPECTED_SCENARIO_IDS) and all(
        ...
        and isinstance(item.get("referenced_fact_candidate_ids"), list)
        and bool(item.get("referenced_fact_candidate_ids"))
    ...
    inference_valid = (
        ...
        and field_inference_report.get("fact_candidate_pool_count") == 10
        and field_inference_report.get("scenario_reference_count") == 6
        and field_inference_report.get("referenced_field_candidate_count") == 5
