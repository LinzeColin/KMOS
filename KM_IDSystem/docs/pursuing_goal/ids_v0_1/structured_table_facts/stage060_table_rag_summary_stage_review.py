"""Stage060 的只读整阶段复审，不读取真实表格或启动批次复审。"""
...
P1_CONTRACT = BASE / "stage060_table_rag_summary_contract.json"
P2_CONTRACT = BASE / "stage060_table_rag_summary_slice_contract.json"
P3_CONTRACT = BASE / "stage060_table_rag_summary_quality_scenarios_contract.json"
P4_CONTRACT = BASE / "stage060_table_rag_summary_delivery_contract.json"

SCHEMA_VERSION = "ids.stage060.table_rag_summary.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE060-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-060"
PASS_RESULT = "PASS_REVIEWED_LOCAL_TABLE_RAG_SUMMARY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_LOCAL_TABLE_RAG_SUMMARY_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE060-REVIEW-GATE"
NEXT_GATE = "IDS-V0_1-BATCH-051-060-REVIEW-GATE"
RETURN_STATE = "PHASE4_TABLE_RAG_SUMMARY_DELIVERY_EVIDENCE_RUNTIME_DISABLED"

ContractProvider = Callable[[], Mapping[str, Any]]
ReportProvider = Callable[[], Mapping[str, Any]]


def build_stage060_review_report(
    ...
    """机械复审 Stage060 P1--P4，只返回 control 计数、边界与回滚结论。"""
    ...
    invariants = {
        ...
        "summary_and_numeric_authority_boundary_preserved": (
            _summary_and_numeric_authority_boundary_preserved(
                phase1, phase2, phase3, phase4
            )
        ),
        ...
        "fact_and_source_location_boundary_preserved": (
            _fact_and_source_location_boundary_preserved(phase1, phase2, phase3)
        ),
    ...
    report: dict[str, Any] = {
        ...
        "rollback": {
            ...
            "revertable_artifacts": [
                "Stage060 review document",
                "Stage060 review module",
                "Stage060 review focused tests",
                "Stage060 review governance projection",
            ],
...
def _load_phase3_report_provider() -> ReportProvider:
    return _module_callable_provider(
        ...
        "build_table_rag_summary_phase3_report",
    )


def _load_phase4_report_provider() -> ReportProvider:
    return _module_callable_provider(
        "stage060_table_rag_summary_delivery.py",
        "build_table_rag_summary_phase4_delivery_report",
    )


def _module_callable_provider(filename: str, callable_name: str) -> ReportProvider:
    path = BASE / filename
    spec = importlib.util.spec_from_file_location(f"stage060_review_{path.stem}", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Stage060 review dependency is unavailable: {filename}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    candidate = getattr(module, callable_name, None)
    if not callable(candidate):
        raise RuntimeError(f"Stage060 review dependency is invalid: {filename}")
    return candidate


def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    input_contract = _as_mapping(contract.get("reference_only_summary_input_contract"))
    output_contract = _as_mapping(contract.get("future_rag_summary_output_contract"))
    semantics = _as_mapping(contract.get("table_semantic_reference_contract"))
    authority = _as_mapping(contract.get("structured_fact_and_numeric_authority_boundary"))
    ...
    return all(
        (
            contract.get("schema_version") == "ids.stage060.table_rag_summary.phase1.v1",
            contract.get("contract_state")
            == "PHASE1_TABLE_RAG_SUMMARY_CONTRACT_RUNTIME_DISABLED",
            contract.get("task_id") == "IDS-V0_1-STAGE060-P1",
            contract.get("next_gate") == "IDS-STAGE060-P2-GATE",
            _single_authority_contract(_as_mapping(contract.get("source_authority"))),
            input_contract.get("field_count") == 13,
            input_contract.get("actual_input_record_count") == 0,
            output_contract.get("field_count") == 10,
            output_contract.get("actual_rag_summary_created") is False,
            output_contract.get("actual_summary_text_retained") is False,
            semantics.get("typed_semantic_category_count") == 7,
            len(_as_mapping(semantics.get("field_types"))) == 7,
            authority.get("source_document_remains_authoritative") is True,
            authority.get("summary_can_replace_structured_fact") is False,
            authority.get("summary_can_become_numeric_statistical_evidence") is False,
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    input_contract = _as_mapping(contract.get("reference_only_summary_input_contract"))
    candidates = _as_mapping(contract.get("rag_summary_candidate_contract"))
    references = _as_mapping(contract.get("fact_and_source_reference_contract"))
    authority = _as_mapping(contract.get("structured_fact_and_numeric_authority_boundary"))
    return all(
        (
            contract.get("schema_version") == "ids.stage060.table_rag_summary.phase2.v1",
            contract.get("contract_state")
            == "PHASE2_TABLE_RAG_SUMMARY_CONTROL_SLICE_RUNTIME_DISABLED",
            contract.get("task_id") == "IDS-V0_1-STAGE060-P2",
            contract.get("next_gate") == "IDS-STAGE060-P3-GATE",
            contract.get("slice_executable") is True,
            _single_authority_contract(_as_mapping(contract.get("source_authority"))),
            input_contract.get("field_count") == 13,
            input_contract.get("control_record_count") == 2,
            input_contract.get("actual_input_record_count") == 0,
            candidates.get("field_count") == 10,
            candidates.get("control_rag_summary_candidate_count") == 2,
            candidates.get("all_summary_text_unset") is True,
            candidates.get("actual_rag_summary_created") is False,
            references.get("fact_reference_count") == 2,
            references.get("location_field_count") == 6,
            references.get("candidate_binding_count") == 2,
            references.get("actual_structured_fact_created") is False,
            references.get("actual_source_location_binding_created") is False,
            references.get("actual_evidence_record_created") is False,
            authority.get("source_document_remains_authoritative") is True,
            authority.get("summary_can_replace_structured_fact") is False,
            authority.get("summary_can_become_numeric_statistical_evidence") is False,
            _contract_runtime_disabled(contract),
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            contract.get("task_id") == "IDS-V0_1-STAGE060-P3",
            contract.get("next_gate") == "IDS-STAGE060-P4-GATE",
            ...
            input_boundary.get("phase2_rag_summary_candidate_count") == 2,
            input_boundary.get("phase2_fact_reference_count") == 2,
            input_boundary.get("phase2_source_location_field_count") == 6,
            ...
            numeric.get("actual_structured_fact_created") is False,
            numeric.get("actual_rag_summary_created") is False,
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            contract.get("task_id") == "IDS-V0_1-STAGE060-P4",
...
def _quality_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            report.get("next_gate") == "IDS-STAGE060-P4-GATE",
            ...
            report.get("unique_rag_summary_candidate_count") == 2,
            report.get("source_location_reference_check_count") == 6,
            report.get("all_summary_text_unset") is True,
            len(results) == 6,
            outlier_count == 1,
            all(
    ...
    return all(
        (
            ...
            all(
                ...
                == (
                    "DELIVERY_METADATA_ONLY_TABLE_FACT_REFERENCE_SAMPLE_"
                    "NOT_REAL_STRUCTURED_FACT"
                )
            ...
            inference.get("rag_summary_candidate_pool_count") == 2,
...
def _summary_and_numeric_authority_boundary_preserved(
    phase1: Mapping[str, Any],
    phase2: Mapping[str, Any],
    phase3: Mapping[str, Any],
    phase4: Mapping[str, Any],
) -> bool:
    p1 = _as_mapping(phase1.get("structured_fact_and_numeric_authority_boundary"))
    p2 = _as_mapping(phase2.get("structured_fact_and_numeric_authority_boundary"))
    ...
    return all(
        (
            ...
            p1.get("summary_can_replace_structured_fact") is False,
            p1.get("summary_can_become_numeric_statistical_evidence") is False,
...
def _fact_and_source_location_boundary_preserved(
    phase1: Mapping[str, Any], phase2: Mapping[str, Any], phase3: Mapping[str, Any]
) -> bool:
    p1_input = _as_mapping(phase1.get("reference_only_summary_input_contract"))
    p1_location = _as_mapping(phase1.get("source_location_and_evidence_contract"))
    p2_references = _as_mapping(phase2.get("fact_and_source_reference_contract"))
    p3_traceability = _as_mapping(phase3.get("traceability_boundary"))
    return all(
        (
            p1_input.get("actual_structured_fact_read_performed") is False,
            p1_location.get("actual_source_location_binding_count") == 0,
            p1_location.get("actual_evidence_record_created") is False,
            p2_references.get("fact_reference_count") == 2,
            p2_references.get("location_field_count") == 6,
            p2_references.get("candidate_binding_count") == 2,
            p2_references.get("actual_structured_fact_created") is False,
            p2_references.get("actual_source_location_binding_created") is False,
            p2_references.get("actual_evidence_record_created") is False,
            p3_traceability.get("control_source_location_traceability_preserved")
            is True,
            p3_traceability.get("actual_source_file_traceability_validated")
            is False,
...
def _controlled_replay(
    ...
    p1_input = _as_mapping(phase1.get("reference_only_summary_input_contract"))
    p1_output = _as_mapping(phase1.get("future_rag_summary_output_contract"))
    p1_semantics = _as_mapping(phase1.get("table_semantic_reference_contract"))
    p1_location = _as_mapping(phase1.get("source_location_and_evidence_contract"))
    p1_failures = _as_mapping(phase1.get("failure_and_stop_contract"))
    p2_input = _as_mapping(phase2.get("reference_only_summary_input_contract"))
    p2_candidates = _as_mapping(phase2.get("rag_summary_candidate_contract"))
    p2_references = _as_mapping(phase2.get("fact_and_source_reference_contract"))
    ...
    return {
        ...
        "phase1_reference_input_field_count": _nonnegative_int(
            p1_input.get("field_count")
        ),
        "phase1_future_rag_summary_output_field_count": _nonnegative_int(
            p1_output.get("field_count")
        ),
        ...
        "phase2_control_record_count": _nonnegative_int(
            p2_input.get("control_record_count")
        ),
        "phase2_rag_summary_candidate_count": _nonnegative_int(
            p2_candidates.get("control_rag_summary_candidate_count")
        ),
        "phase2_fact_reference_count": _nonnegative_int(
            p2_references.get("fact_reference_count")
        ),
        "phase2_source_location_field_count": _nonnegative_int(
            p2_references.get("location_field_count")
        ),
        "phase2_source_location_binding_candidate_count": _nonnegative_int(
            p2_references.get("candidate_binding_count")
        ),
