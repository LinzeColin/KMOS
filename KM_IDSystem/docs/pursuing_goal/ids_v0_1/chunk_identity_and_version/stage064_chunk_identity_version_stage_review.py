"""Stage064 的只读整阶段机械复审，不读取真实资料或启动 Stage065。"""
...
P1_CONTRACT = BASE / "stage064_chunk_identity_version_contract.json"
P2_CONTRACT = BASE / "stage064_chunk_identity_version_slice_contract.json"
P3_CONTRACT = BASE / "stage064_chunk_identity_version_scenarios_contract.json"
P4_CONTRACT = BASE / "stage064_chunk_identity_version_delivery_contract.json"

SCHEMA_VERSION = "ids.stage064.chunk_identity_and_version.stage_review.v1"
TASK_ID = "IDS-V0_1-STAGE064-REVIEW"
ACCEPTANCE_ID = "ACC-STAGE-064"
PASS_RESULT = "PASS_REVIEWED_LOCAL_CHUNK_IDENTITY_AND_VERSION_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_REVIEWED_LOCAL_CHUNK_IDENTITY_AND_VERSION_RUNTIME_DISABLED"
REVIEW_GATE = "IDS-STAGE064-REVIEW-GATE"
NEXT_GATE = "IDS-STAGE065-P1-GATE"
RETURN_STATE = "PHASE4_CHUNK_IDENTITY_AND_VERSION_DELIVERY_EVIDENCE_RUNTIME_DISABLED"
P3_RETURN_STATE = "PHASE3_CHUNK_IDENTITY_AND_VERSION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED"
...
def build_stage064_review_report(
    ...
    """机械复审 Stage064 P1--P4，返回控制事实、门禁与回退结论。"""
    ...
    review_invariants = {
        ...
        "identity_version_shape_preserved": _identity_version_shape_preserved(
        ...
        "stage065_not_started": True,
    }
    report: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "task_id": TASK_ID,
        "acceptance_id": ACCEPTANCE_ID,
        "source_authority": "FROZEN_STAGE064_TASKPACK_AND_STAGE064_P1_TO_P4_CONTROL_ARTIFACTS_ONLY",
        ...
        "rollback": {
            ...
            "revertable_artifacts": [
                "Stage064 review document",
                "Stage064 review module",
                "Stage064 review focused tests",
                "Stage064 review governance projection",
            ],
        ...
        "stage065_started": False,
        "stage065_entry_allowed": False,
...
def _load_report_provider(filename: str, callable_name: str) -> ReportProvider:
    path = BASE / filename
    spec = importlib.util.spec_from_file_location(f"stage064_review_{path.stem}", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Stage064 review dependency is unavailable: {filename}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    candidate = getattr(module, callable_name, None)
    if not callable(candidate):
        raise RuntimeError(f"Stage064 review dependency is invalid: {filename}")
    return candidate


def _phase1_contract_valid(contract: Mapping[str, Any]) -> bool:
    inputs = _as_mapping(contract.get("reference_only_identity_and_version_input_contract"))
    outputs = _as_mapping(contract.get("future_chunk_identity_and_version_output_contract"))
    ...
    return all(
        (
            contract.get("schema_version") == "ids.stage064.chunk_identity_and_version.phase1.v1",
            contract.get("contract_state")
            == "PHASE1_CHUNK_IDENTITY_AND_VERSION_CONTRACT_RUNTIME_DISABLED",
            contract.get("task_id") == "IDS-V0_1-STAGE064-P1",
            contract.get("next_gate") == "IDS-STAGE064-P2-GATE",
            ...
            outputs.get("actual_chunk_id_created") is False,
            outputs.get("actual_chunk_hash_computed") is False,
            outputs.get("actual_chunk_version_created") is False,
            ...
            failures.get("failure_state_count") == 9,
...
def _phase2_contract_valid(contract: Mapping[str, Any]) -> bool:
    inputs = _as_mapping(contract.get("reference_only_identity_and_version_input_control_contract"))
    records = _as_mapping(contract.get("control_chunk_identity_and_version_record_contract"))
    ...
    return all(
        (
            contract.get("schema_version") == "ids.stage064.chunk_identity_and_version.phase2.v1",
            contract.get("contract_state")
            == "PHASE2_CHUNK_IDENTITY_AND_VERSION_CONTROL_SLICE_RUNTIME_DISABLED",
            contract.get("task_id") == "IDS-V0_1-STAGE064-P2",
            contract.get("next_gate") == "IDS-STAGE064-P3-GATE",
            ...
            inputs.get("actual_input_request_count") == 0,
            records.get("field_count") == 14,
            records.get("control_record_count") == 3,
            records.get("actual_chunk_created") is False,
            records.get("actual_chunk_persisted") is False,
            protected.get("protected_semantic_asset_type_count") == 3,
            protected.get("one_control_record_per_protected_semantic_asset_type") is True,
            protected.get("protected_surface_split_allowed") is False,
            traceability.get("traceability_field_count") == 6,
...
def _phase3_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            ...
            contract.get("task_id") == "IDS-V0_1-STAGE064-P3",
            contract.get("next_gate") == "IDS-STAGE064-P4-GATE",
            ...
            inputs.get("control_identity_version_request_count") == 3,
            inputs.get("control_chunk_identity_version_record_count") == 3,
...
def _phase4_contract_valid(contract: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            contract.get("schema_version") == "ids.stage064.chunk_identity_and_version.phase4.delivery.v1",
            contract.get("contract_state") == RETURN_STATE,
            contract.get("task_id") == "IDS-V0_1-STAGE064-P4",
            ...
            delivery.get("unique_control_identity_version_record_count") == 3,
...
def _phase3_report_valid(report: Mapping[str, Any]) -> bool:
    return all(
        (
            ...
            == "ids.stage064.chunk_identity_and_version.phase3.controlled_scenarios.v1",
            report.get("valid") is True,
            report.get("result")
            == "PASS_PHASE3_CHUNK_IDENTITY_AND_VERSION_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
            report.get("next_gate") == "IDS-STAGE064-P4-GATE",
            ...
            report.get("unique_chunk_identity_version_record_count") == 3,
...
def _phase4_report_valid(report: Mapping[str, Any]) -> bool:
    ...
    return all(
        (
            report.get("schema_version") == "ids.stage064.chunk_identity_and_version.phase4.delivery.v1",
            report.get("valid") is True,
            report.get("result")
            == "PASS_PHASE4_CHUNK_IDENTITY_AND_VERSION_DELIVERY_RUNTIME_DISABLED",
...
def _controlled_replay(
    ...
    phase1_inputs = _as_mapping(phase1.get("reference_only_identity_and_version_input_contract"))
    phase1_outputs = _as_mapping(phase1.get("future_chunk_identity_and_version_output_contract"))
    phase2_inputs = _as_mapping(phase2.get("reference_only_identity_and_version_input_control_contract"))
    phase2_records = _as_mapping(phase2.get("control_chunk_identity_and_version_record_contract"))
    ...
    return {
        ...
        "protected_semantic_asset_type_count": phase2_inputs.get("protected_semantic_asset_type_count"),
...
def _single_authority_boundary_preserved(
    ...
    return all(
        (
            ...
            scenario_report.get("chunk_identity_version_record_can_become_business_fact_authority") is False,
            delivery_report.get("source_document_remains_authoritative") is True,
            delivery_report.get("delivery_evidence_can_become_business_fact_authority") is False,
        )
    )


def _identity_version_shape_preserved(
    phase1: Mapping[str, Any],
    phase2: Mapping[str, Any],
    phase3: Mapping[str, Any],
    scenario_report: Mapping[str, Any],
    ...
    return all(
        (
            phase1_inputs.get("field_count") == 10,
            phase1_outputs.get("field_count") == 14,
            phase2_inputs.get("control_request_count") == 3,
            phase2_records.get("control_record_count") == 3,
            phase3_inputs.get("scenario_count") == 6,
            scenario_report.get("unique_chunk_identity_version_record_count") == 3,
...
def _human_handling_boundary_preserved(
    ...
    phase2_boundary = _as_mapping(phase2.get("authority_and_decision_boundary"))
    phase3_ownership = _as_mapping(phase3.get("ownership_boundary"))
    low_quality = delivery_report.get("low_quality_chunk_list")
    return all(
        (
            phase2_boundary.get("all_control_records_require_human_review") is True,
            phase3_ownership.get("business_line_whitebox_human_handling_required") is True,
...
def _future_stage_ownership_preserved(
    ...
    return all(
        (
            phase1_future.get("engineering_semantic_asset_type_implementation_owner") == "STAGE-065",
            ...
            phase4_ownership.get("semantic_asset_classification_owner") == "STAGE-065",
