"""Stage067 P4 的切块质量回归 metadata-only 交付证据。

本模块只由 P3 固定控制场景派生内存 JSONL 样例、控制交付覆盖报告、低质量待人工
清单、回归控制结果和回退说明。它不读取来源、不生成真实 chunk、不验证真实质量，
也不写入文件、索引、数据库或运行时服务。
"""
...
SCHEMA_VERSION = "ids.stage067.chunk_quality_regression.phase4.delivery.v1"
RECORD_KIND = "CHUNK_QUALITY_REGRESSION_DELIVERY_EVIDENCE_REPORT"
PASS_RESULT = "PASS_PHASE4_CHUNK_QUALITY_REGRESSION_DELIVERY_RUNTIME_DISABLED"
FAIL_RESULT = "FAIL_CHUNK_QUALITY_REGRESSION_DELIVERY_EVIDENCE"
NEXT_GATE = "IDS-STAGE067-REVIEW-GATE"
ENTRY_GATE = "IDS-STAGE067-P4-GATE"
...
COVERAGE_REPORT_KIND = (
    "CONTROLLED_CHUNK_QUALITY_REGRESSION_DELIVERY_COVERAGE_REPORT_NOT_REAL_COVERAGE"
)
LOW_QUALITY_LIST_KIND = (
    "CONTROLLED_LOW_QUALITY_CHUNK_QUALITY_REGRESSION_LIST_NOT_REAL_QUALITY_MEASUREMENT"
)
...
REGENERATION_ROLLBACK_KIND = (
    "CHUNK_QUALITY_REGRESSION_REGENERATION_AND_VERSION_ROLLBACK_INSTRUCTIONS_CONTROL_REPLAY_ONLY"
)
...
def build_chunk_quality_regression_phase4_delivery_report(
    ...
    return {
        ...
        "actual_quality_regression_delivery_implementation_performed": False,
        ...
        **_runtime_closed_flags(),
        ...
        "chinese_feedback": [
            ...
            "交付覆盖报告只核对固定控制场景和引用形状；真实页面、质量基线与质量结论仍须业务线白箱人工确认。",
...
def _load_phase3_report_provider() -> Phase3ReportProvider:
    module_path = Path(__file__).with_name("stage067_chunk_quality_regression_scenarios.py")
    spec = importlib.util.spec_from_file_location("stage067_p3", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Stage067 P3 controlled-scenarios module is unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    provider = getattr(module, "build_chunk_quality_regression_phase3_report", None)
    if not callable(provider):
        raise RuntimeError("Stage067 P3 report provider is unavailable")
    return provider
...
def _phase3_report_is_valid(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and report.get("unique_control_quality_regression_record_count") == 4
        ...
        and report.get("quality_regression_control_record_can_replace_source_document")
        is False
        and report.get(
...
def _jsonl_sample(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "sample_id": f"chunk-quality-regression-jsonl-sample:{item['scenario_id']}",
        ...
        "chunk_quality_regression_request_ref": item[
            "referenced_chunk_quality_regression_request_ref"
        ],
        ...
        "actual_chunk_quality_regression_created": False,
...
def _coverage_report(
    ...
    return {
        ...
        "unique_control_quality_regression_record_count": (
            phase3_report.get("unique_control_quality_regression_record_count")
            if complete
            else 0
        ),
        ...
        "actual_chunk_quality_regression_performed": False,
...
def _low_quality_list(scenarios: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            "record_id": (
                f"chunk-quality-regression-low-quality-control:{item['scenario_id']}"
            ),
...
def _human_confirmation_prompts() -> list[str]:
    return [
        ...
        "请确认：交付覆盖报告与低质量清单仅覆盖固定控制场景，真实页面和质量需业务线白箱人工确认。",
