...
SCOPE = BASE / "STAGE086_PHASE3_HYBRID_RANKING_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE / "index_version_schema" / "stage086_hybrid_ranking_scenarios_contract.json"
)
MODULE = (
    BASE / "index_version_schema" / "stage086_hybrid_ranking_controlled_scenarios.py"
)
TASKPACK = (
    ROOT
    / "docs"
...
P2_SCOPE = BASE / "STAGE086_PHASE2_HYBRID_RANKING_CONTROL_SLICE.md"
P2_CONTRACT = BASE / "index_version_schema" / "stage086_hybrid_ranking_slice_contract.json"
P2_SLICE = BASE / "index_version_schema" / "stage086_hybrid_ranking_control_slice.py"
STAGE085_REVIEW = BASE / "STAGE085_STAGE_REVIEW.md"


def _load_module():
    spec = importlib.util.spec_from_file_location("stage086_p3", MODULE)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage086 P3 controlled scenarios")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage086HybridRankingPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _report(self):
        return self.module.build_hybrid_ranking_phase3_report()
    ...
    def test_control_artifacts_phase_identity_and_gate_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage086.hybrid_ranking.phase3.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("STAGE-086", self.contract["stage"])
        self.assertEqual("IDS-STAGE086-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE086-P3", self.contract["task_id"])
        self.assertEqual(
            "PHASE3_HYBRID_RANKING_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
            self.contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE086-P4-GATE", self.contract["next_gate"])
    ...
    def test_contract_preserves_replay_shapes_and_runtime_boundaries(self):
        ...
        self.assertEqual(480, replay["expected_phase2_field_check_count"])
        self.assertEqual(
            [item[0] for item in self.module.P2_RECORD_SPECS],
            replay["required_projection_sets"],
        )
        for field in (
        ...
        self.assertTrue(failures["failure_closed"])
    ...
    def test_report_replays_p2_and_preserves_all_control_shapes(self):
        ...
        self.assertEqual(480, report["phase2_control_record_field_check_count"])
    ...
    def test_each_required_scenario_is_control_only_and_whitebox_gated(self):
        report = self._report()
        self.assertEqual(
    ...
    def test_taskpack_scenario_categories_hold_without_business_values(self):
        report = self._report()
        scenarios = {
    ...
    def test_invalid_or_side_effecting_phase2_replay_is_fail_closed(self):
        def missing_active_index_contract(control_input):
            result = self._phase2().execute_hybrid_ranking_control_slice(control_input)
            altered = copy.deepcopy(result)
            altered["all_active_index_version_contracts_match"] = False
            return altered

        failed = self.module.build_hybrid_ranking_phase3_report(
            missing_active_index_contract
        )
        self.assertFalse(failed["valid"])
        self.assertEqual(self.module.FAIL_RESULT, failed["result"])
        self.assertEqual("IDS-STAGE086-P3-GATE", failed["next_gate"])

        def malformed_active_index_record(control_input):
            result = self._phase2().execute_hybrid_ranking_control_slice(control_input)
            altered = copy.deepcopy(result)
            altered["active_index_version_control_projections"][0][
                "activation_state"
            ] = "CONTROL_ACTIVE_INDEX_VERSION_UNDECLARED"
            return altered

        failed = self.module.build_hybrid_ranking_phase3_report(
            malformed_active_index_record
        )
        self.assertFalse(failed["valid"])
        self.assertTrue(failed["phase2_shape_preserved"])
        self.assertFalse(failed["active_index_version_contract_preserved"])
        ...
        failed = self.module.build_hybrid_ranking_phase3_report(runtime_signal)
        self.assertFalse(failed["valid"])
    ...
    def test_report_is_zero_runtime_and_stops_before_phase4(self):
        ...
        for field in (
            ...
            "actual_material_quality_scoring_count",
            "actual_freshness_scoring_count",
            "actual_business_module_scoring_count",
        ...
        self.assertTrue(report["stage085_review_evidence_declared"])
        self.assertTrue(report["stage086_started"])
