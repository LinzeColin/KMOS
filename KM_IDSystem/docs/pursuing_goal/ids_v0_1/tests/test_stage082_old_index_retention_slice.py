...
SCOPE = BASE / "STAGE082_PHASE2_OLD_INDEX_RETENTION_CONTROL_SLICE.md"
CONTRACT = BASE / "index_version_schema" / "stage082_old_index_retention_slice_contract.json"
MODULE = BASE / "index_version_schema" / "stage082_old_index_retention_control_slice.py"
...
def load_module():
    spec = importlib.util.spec_from_file_location("stage082_slice", MODULE)
...
class Stage082OldIndexRetentionPhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.control_input = slice_module.build_control_input()
        cls.result = slice_module.execute_old_index_retention_control_slice(
            cls.control_input
        )
    ...
    def test_frozen_sources_and_phase_identity_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage082.old_index_retention.phase2.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("STAGE-082", self.contract["stage"])
        self.assertEqual("IDS-STAGE082-P2", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE082-P2", self.contract["task_id"])
        self.assertEqual("ACC-STAGE-082", self.contract["acceptance_id"])
        self.assertEqual(
            "PHASE2_OLD_INDEX_RETENTION_CONTROL_SLICE_RUNTIME_DISABLED",
            self.contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE082-P3-GATE", self.contract["next_gate"])
        for field in (
    ...
    def test_fixed_non_business_control_input_has_exact_shape(self):
        ...
        for request in requests:
            with self.subTest(scenario=request["control_scenario"]):
                ...
                self.assertEqual(
                    1, request["minimum_retained_previous_active_version_count"]
                )
                for field in (
                    "source_import_ref",
                    "document_scope_ref",
    ...
    def test_control_output_fields_and_counts_are_fixed(self):
        ...
        self.assertEqual(
            "COMPLETED_IN_MEMORY_OLD_INDEX_RETENTION_CONTROL_SLICE",
        ...
        for output_key, count_key, field_key, field_names in (
            ...
            (
                ...
                "retention_policy_control_projection_count",
                "retention_policy_fields",
                slice_module.RETENTION_POLICY_FIELDS,
            ),
            (
                "cleanup_eligibility_control_projections",
                "cleanup_eligibility_control_projection_count",
                "cleanup_eligibility_fields",
                slice_module.CLEANUP_ELIGIBILITY_FIELDS,
            ),
        ):
    ...
    def test_retention_rollback_and_cleanup_remain_fail_closed(self):
        rollbacks = dict(
            zip(
                slice_module.CONTROL_SCENARIOS,
        ...
        rollback = rollbacks["hybrid_retained_previous_rollback_window_unconfigured"]
        self.assertEqual(
            request["previous_active_index_version_ref"],
        ...
        self.assertEqual(
            ...
            rollback["retention_window_state"],
        )
        self.assertEqual(
        ...
        self.assertTrue(self.result["all_minimum_previous_active_versions_retained"])
        self.assertTrue(
            self.result["all_rollback_targets_reference_retained_previous_active"]
        )
        self.assertTrue(self.result["all_cleanup_projections_fail_closed"])
        for policy, cleanup in zip(
            self.result["retention_policy_control_projections"],
            self.result["cleanup_eligibility_control_projections"],
        ):
            with self.subTest(policy=policy["retention_policy_ref"]):
                self.assertEqual(
                    1, policy["minimum_retained_previous_active_version_count"]
                )
                self.assertEqual(
                    "CONTROL_ADDITIONAL_RETENTION_QUANTITY_UNCONFIGURED",
                    policy["additional_retained_version_count_requirement_ref"],
                )
                self.assertEqual(
                    "CONTROL_ROLLBACK_WINDOW_UNCONFIGURED",
                    cleanup["rollback_window_state"],
                )
                self.assertEqual(
                    "CONTROL_CLEANUP_TIMING_UNCONFIGURED",
                    cleanup["cleanup_timing_state"],
                )
                self.assertEqual(
                    "CONTROL_WHITEBOX_APPROVAL_NOT_RECORDED",
                    cleanup["business_line_whitebox_approval_state"],
                )
                self.assertEqual(
                    "CONTROL_CLEANUP_BLOCKED_UNCONFIGURED_POLICY",
                    cleanup["cleanup_eligibility"],
                )
        self.assertFalse(self.result["business_line_whitebox_human_approval_recorded"])

    def test_rejected_input_and_runtime_boundaries_remain_closed(self):
        rejected = slice_module.execute_old_index_retention_control_slice({})
        ...
        for field in (
            ...
            "stage082_phase1_completed",
            "stage082_phase2_started",
        ):
            with self.subTest(field=field):
