...
SCOPE = BASE / "STAGE092_PHASE4_EVIDENCE_RISK_SCORING_DELIVERY_EVIDENCE.md"
CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage092_evidence_risk_scoring_delivery_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage092_evidence_risk_scoring_delivery.py"
P3_SCOPE = BASE / "STAGE092_PHASE3_EVIDENCE_RISK_SCORING_CONTROLLED_SCENARIOS.md"
P3_CONTRACT = (
    BASE
    / "index_version_schema"
...
class Stage092EvidenceRiskScoringPhase4DeliveryTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage092_phase4_delivery", MODULE)
        cls.phase3 = load_module("stage092_phase3_scenarios", P3_MODULE)
        cls.report = cls.module.build_evidence_risk_scoring_phase4_delivery_report()

    def _phase3_report(self):
        return self.phase3.build_evidence_risk_scoring_phase3_report()
    ...
    def test_artifacts_identity_and_frozen_taskpack_exist(self):
        ...
        self.assertEqual("IDS-STAGE092-P4", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE092-P4", self.contract["task_id"])
        self.assertEqual("IDS-STAGE092-P4-GATE", self.contract["entry_gate"])
        self.assertEqual("IDS-STAGE092-REVIEW-GATE", self.contract["next_gate"])
    ...
    def test_report_replays_phase3_exact_shape_without_runtime(self):
        ...
        self.assertEqual(582, report["phase2_control_field_check_count"])
    ...
    def test_invalid_predecessor_fails_closed(self):
        failed = self.module.build_evidence_risk_scoring_phase4_delivery_report(
            lambda: {}
        )
    ...
    def test_semantic_drift_for_gap_revocation_and_masquerade_fails_closed(self):
        cases = (
            (
                ...
                "no_internal_evidence_risk_control",
                "evidence_id_ref",
                "evidence:control:stage092-p2:unexpected",
                "NO_INTERNAL_EVIDENCE_RISK_ROUTE_MISSING",
            ),
            (
                "degradation",
    ...
    def test_chinese_feedback_and_stage_boundary_remain_control_only(self):
        ...
        self.assertFalse(self.report["stage092_review_started"])
