...
SCOPE = BASE / "STAGE080_PHASE3_INDEX_ROLLBACK_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage080_index_rollback_scenarios_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage080_index_rollback_scenarios.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
P2_SCOPE = BASE / "STAGE080_PHASE2_INDEX_ROLLBACK_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE / "index_version_schema" / "stage080_index_rollback_slice_contract.json"
)
P2_SLICE = BASE / "index_version_schema" / "stage080_index_rollback_control_slice.py"
...
def _load_module():
    spec = importlib.util.spec_from_file_location("stage080_p3", MODULE)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage080 P3 controlled scenarios")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage080IndexRollbackPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _report(self):
        return self.module.build_index_rollback_phase3_report()
    ...
    def test_contract_declares_fixed_control_only_phase3_boundary(self):
        self.assertEqual(
            "ids.stage080.index_rollback.phase3.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("IDS-V0_1-STAGE080-P3", self.contract["task_id"])
        self.assertEqual(
            "PHASE3_INDEX_ROLLBACK_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
        ...
        self.assertEqual("IDS-STAGE080-P4-GATE", self.contract["next_gate"])
        ...
        self.assertEqual(225, replay["phase2_control_field_check_count"])
        scenarios = self.contract["controlled_scenario_contract"]
        self.assertEqual(6, scenarios["scenario_count"])
        self.assertEqual(28, scenarios["field_count"])
        views = self.contract["control_view_projection_contract"]
        self.assertEqual(5, views["operations_view_projection_count"])
        self.assertEqual(5, views["report_snapshot_projection_count"])
        ...
        for scenario in report["scenario_results"]:
            with self.subTest(scenario=scenario["scenario_id"]):
                ...
                for field in (
                    ...
                    self.assertIn(":control:stage080-p2:", scenario[field])
    ...
    def test_operations_and_report_snapshot_views_remain_control_only(self):
        ...
        for view in report["operations_version_control_views"]:
            with self.subTest(view=view["control_scenario"]):
                ...
                self.assertIn(":control:stage080-p2:", view["operations_view_ref"])
        for view in report["report_snapshot_version_control_views"]:
            with self.subTest(view=view["control_scenario"]):
                ...
                self.assertIn(":control:stage080-p2:", view["report_snapshot_ref"])
    ...
    def test_invalid_or_malformed_phase2_fails_closed(self):
        invalid = self.module.build_index_rollback_phase3_report(
        ...
        def malformed(_control):
            result = copy.deepcopy(
                phase2.execute_index_rollback_control_slice(
        ...
        malformed_report = self.module.build_index_rollback_phase3_report(
    ...
    def test_phase2_runtime_signal_fails_closed(self):
        ...
        report = self.module.build_index_rollback_phase3_report(
    ...
    def test_phase3_history_remains_valid_when_successor_phase_is_current(self):
        ...
        expected_states = {
            ...
            "IDS-V0_1-STAGE079-P4": {
                "stage": "IDS-STAGE079",
                "phase": "IDS-V0_1-STAGE079-P4",
                "task": "IDS-V0_1-STAGE079-P4",
                "next_gate": "IDS-STAGE079-REVIEW-GATE",
            },
            "IDS-STAGE079-REVIEW": {
                "stage": "IDS-STAGE079",
                "phase": "IDS-STAGE079-REVIEW",
                "task": "IDS-V0_1-STAGE079-REVIEW",
                "next_gate": "IDS-STAGE080-P1-GATE",
            },
            "IDS-V0_1-STAGE080-P1": {
                "stage": "IDS-STAGE080",
                "phase": "IDS-V0_1-STAGE080-P1",
                "task": "IDS-V0_1-STAGE080-P1",
                "next_gate": "IDS-STAGE080-P2-GATE",
            },
            "IDS-V0_1-STAGE080-P2": {
                "stage": "IDS-STAGE080",
                "phase": "IDS-V0_1-STAGE080-P2",
                "task": "IDS-V0_1-STAGE080-P2",
                "next_gate": "IDS-STAGE080-P3-GATE",
            },
            "IDS-V0_1-STAGE080-P3": {
                "stage": "IDS-STAGE080",
                "phase": "IDS-V0_1-STAGE080-P3",
                "task": "IDS-V0_1-STAGE080-P3",
                "next_gate": "IDS-STAGE080-P4-GATE",
            },
            "IDS-V0_1-STAGE080-P4": {
                "stage": "IDS-STAGE080",
                "phase": "IDS-V0_1-STAGE080-P4",
                "task": "IDS-V0_1-STAGE080-P4",
                "next_gate": "IDS-STAGE080-REVIEW-GATE",
            },
            "IDS-STAGE080-REVIEW": {
                "stage": "IDS-STAGE080",
                "phase": "IDS-STAGE080-REVIEW",
                "task": "IDS-V0_1-STAGE080-REVIEW",
                "next_gate": "IDS-STAGE081-P1-GATE",
            },
            "IDS-STAGE081-P1": {
                "stage": "IDS-STAGE081",
                "phase": "IDS-STAGE081-P1",
                "task": "IDS-V0_1-STAGE081-P1",
                "next_gate": "IDS-STAGE081-P2-GATE",
            },
            "IDS-STAGE081-P2": {
                "stage": "IDS-STAGE081",
                "phase": "IDS-STAGE081-P2",
                "task": "IDS-V0_1-STAGE081-P2",
                "next_gate": "IDS-STAGE081-P3-GATE",
            },
            "IDS-STAGE081-P3": {
                "stage": "IDS-STAGE081",
                "phase": "IDS-STAGE081-P3",
                "task": "IDS-V0_1-STAGE081-P3",
                "next_gate": "IDS-STAGE081-P4-GATE",
            },
            "IDS-STAGE081-P4": {
                "stage": "IDS-STAGE081",
                "phase": "IDS-STAGE081-P4",
            ...
            "IDS-STAGE082-P1": {
                "stage": "IDS-STAGE082",
                "phase": "IDS-STAGE082-P1",
                "task": "IDS-V0_1-STAGE082-P1",
                "next_gate": "IDS-STAGE082-P2-GATE",
            },

                              "IDS-STAGE082-P2": {
                "stage": "IDS-STAGE082",
                "phase": "IDS-STAGE082-P2",
                "task": "IDS-V0_1-STAGE082-P2",
                "next_gate": "IDS-STAGE082-P3-GATE",
            },
                              "IDS-STAGE082-P3": {
                "stage": "IDS-STAGE082",
                "phase": "IDS-STAGE082-P3",
            ...
            "IDS-STAGE083-P4": {
                "stage": "IDS-STAGE083",
                "phase": "IDS-STAGE083-P4",
                "task": "IDS-V0_1-STAGE083-P4",
                "next_gate": "IDS-STAGE083-REVIEW-GATE",
            },
            "IDS-STAGE083-REVIEW": {
                "stage": "IDS-STAGE083",
                "phase": "IDS-STAGE083-REVIEW",
                "task": "IDS-V0_1-STAGE083-REVIEW",
                "next_gate": "IDS-STAGE084-P1-GATE",
            },
            "IDS-STAGE084-P1": {
                "stage": "IDS-STAGE084",
                "phase": "IDS-STAGE084-P1",
            ...
            "IDS-STAGE085-P3": {
                "stage": "IDS-STAGE085",
                "phase": "IDS-STAGE085-P3",
                "task": "IDS-V0_1-STAGE085-P3",
                "next_gate": "IDS-STAGE085-P4-GATE",
            },
            "IDS-STAGE085-P4": {
                "stage": "IDS-STAGE085",
                "phase": "IDS-STAGE085-P4",
                "task": "IDS-V0_1-STAGE085-P4",
                "next_gate": "IDS-STAGE085-REVIEW-GATE",
            },
            "IDS-STAGE085-REVIEW": {
                "stage": "IDS-STAGE085",
                "phase": "IDS-STAGE085-REVIEW",
                "task": "IDS-V0_1-STAGE085-REVIEW",
                "next_gate": "IDS-STAGE086-P1-GATE",
            },
            "IDS-STAGE086-P1": {
                "stage": "IDS-STAGE086",
                "phase": "IDS-STAGE086-P1",
                "task": "IDS-V0_1-STAGE086-P1",
                "next_gate": "IDS-STAGE086-P2-GATE",
            }, 'IDS-STAGE086-P2': {
                "stage": "IDS-STAGE086",
                "phase": "IDS-STAGE086-P2",
                "task": "IDS-V0_1-STAGE086-P2",
                "next_gate": "IDS-STAGE086-P3-GATE",
            }, 'IDS-STAGE086-P3': {
                "stage": "IDS-STAGE086",
        ...
        self.assertEqual(
            ...
            {key: status[key] for key in ("stage", "phase", "task", "next_gate")},
        )
        self.assertFalse(status["runtime_enabled"])
        self.assertFalse(status["push_allowed"])
        self.assertIn(
            plan["task"], {state["task"] for state in expected_states.values()}
        )
        self.assertTrue(
            any(
                ...
                for gate in (
                    ...
                    "IDS-STAGE081-P1-GATE", "IDS-STAGE081-P2-GATE", "IDS-STAGE081-P3-GATE", "IDS-STAGE081-P4-GATE", "IDS-STAGE081-REVIEW-GATE", "IDS-STAGE082-P1-GATE", "IDS-STAGE082-P2-GATE",
