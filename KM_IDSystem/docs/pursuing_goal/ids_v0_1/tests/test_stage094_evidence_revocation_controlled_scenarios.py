...
SCOPE = BASE / "STAGE094_PHASE3_EVIDENCE_REVOCATION_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
P2_MODULE = BASE / "index_version_schema" / "stage094_evidence_revocation_control_slice.py"
...
P2_RECEIPT = ROOT / "machine" / "runs" / "2026-08-24-stage094-p2-local.json"
RECEIPT = ROOT / "machine" / "runs" / "2026-08-24-stage094-p3-local.json"
...
class Stage094EvidenceRevocationPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage094_phase3_scenarios", MODULE)
        cls.phase2 = load_module("stage094_phase2_slice", P2_MODULE)
        cls.report = cls.module.build_evidence_revocation_phase3_report()

    def _phase2_report(self):
        return self.phase2.execute_evidence_revocation_control_slice(
    ...
    def test_artifacts_phase_identity_and_frozen_taskpack_exist(self):
        ...
        self.assertEqual("ids.stage094.evidence_revocation.phase3.v1", self.contract["schema_version"])
        self.assertEqual("STAGE-094", self.contract["stage"])
        self.assertEqual("IDS-STAGE094-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE094-P3", self.contract["task_id"])
        self.assertEqual("IDS-STAGE094-P4-GATE", self.contract["next_gate"])

    def test_single_authority_replay_shape_and_phase_boundary_are_explicit(self):
        source = self.contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE094_TASKPACK_AND_STAGE093_REVIEWED_STAGE094_PHASE1_PHASE2_CONTROL_CONTRACTS_ONLY",
            source["authority"],
        )
        for field, value in source.items():
            if field not in {
                ...
                "stage094_phase2_scope_ref",
                "stage094_phase2_contract_ref",
                "stage094_phase2_receipt_ref",
            }:
                with self.subTest(field=field):
                    self.assertFalse(value)
        replay = self.contract["phase2_replay_contract"]
        self.assertEqual(self.module.CONTROL_PREFIX, replay["control_prefix"])
        self.assertEqual(self.module.P2_CONTROL_SCENARIOS, tuple(replay["required_control_scenarios"]))
        self.assertEqual(6, replay["required_control_request_count"])
        self.assertEqual(29, replay["required_input_field_count"])
        self.assertEqual(11, replay["required_projection_group_count"])
        self.assertEqual(105, replay["expected_projection_fields_per_request"])
    ...
    def test_phase2_replay_and_scenario_shapes_are_exact(self):
        ...
        self.assertEqual(630, report["phase2_field_check_count"])
    ...
    def test_exception_semantics_and_taskpack_coverage_are_exact(self):
        ...
        no_internal = scenarios["no_internal_evidence_revocation_control"]
        ...
        self.assertEqual(
            ...
            scenarios["low_ocr_evidence_revocation_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        self.assertEqual(
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED",
            scenarios["old_version_evidence_revocation_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        ...
        self.assertEqual(
            ...
            scenarios["malicious_evidence_revocation_quarantine_control"][
                "evidence_disposition_state"
            ],
        )
    ...
    def test_contract_failure_and_protected_boundaries_are_explicit(self):
        ...
        self.assertTrue(rollback["preserve_stage094_phase1_and_phase2"])
    ...
    def test_invalid_phase2_output_returns_controlled_failure(self):
        failed = self.module.build_evidence_revocation_phase3_report(lambda _: {})
    ...
    def test_phase2_runtime_signal_returns_controlled_failure(self):
        ...
        failed = self.module.build_evidence_revocation_phase3_report(runtime_signal)
        self.assertFalse(failed["valid"])
    ...
    def test_nonopaque_reference_and_semantic_drift_return_specific_failures(self):
        def nonopaque_reference(_control_input):
            altered = copy.deepcopy(self._phase2_report())
            altered["evidence_revocation_schema_binding_control_projections"][0][
                "evidence_gap_ref"
            ] = "unscoped-reference"
            return altered

        failed = self.module.build_evidence_revocation_phase3_report(nonopaque_reference)
        self.assertFalse(failed["valid"])
        ...
        failed = self.module.build_evidence_revocation_phase3_report(low_ocr_drift)
        self.assertFalse(failed["valid"])
        ...
        def masquerade_drift(_control_input):
            altered = copy.deepcopy(self._phase2_report())
            altered["critical_conclusion_binding_control_projections"][1][
                "evidence_grade_ref"
            ] = ":control:stage094-p2:evidence-grade-A:reference-only"
            return altered

        failed = self.module.build_evidence_revocation_phase3_report(masquerade_drift)
        self.assertFalse(failed["valid"])
        self.assertEqual(
            "LOW_GRADE_EVIDENCE_MASQUERADING_AS_HIGH_TRUST",
    ...
    def test_predecessor_p2_record_remains_immutable_before_or_after_p3(self):
        ...
        if current == p3_current:
            ...
            self.assertEqual("P3 受控场景已完成", acceptance_by_id["ACC-STAGE-094"])
            self.assertEqual("已通过", acceptance_by_id["ACC-STAGE094-P3-01"])
            self.assertEqual("已通过", acceptance_by_id["ACC-STAGE094-P3-02"])
            self.assertEqual("已通过", acceptance_by_id["ACC-STAGE094-P3-03"])
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE094-P3-04"])
            self.assertIn("EVT-IDS-V0_1-STAGE094-P3-20260824-001", event_ids)
            self.assertEqual("IDS-STAGE094-P4-GATE", receipt["next_gate"])
            self.assertEqual(
            ...
            self.assertIn(current, (p2_current, p4_current, review_current))
