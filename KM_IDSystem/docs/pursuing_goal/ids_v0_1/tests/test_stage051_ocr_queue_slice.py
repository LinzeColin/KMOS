...
PHASE1 = BASE / "STAGE051_PHASE1_OCR_QUEUE_SCOPE_BOUNDARY.md"
PHASE2 = BASE / "STAGE051_PHASE2_OCR_QUEUE_SLICE.md"
PHASE1_CONTRACT = BASE / "ocr_queue" / "stage051_ocr_queue_contract.json"
CONTRACT = BASE / "ocr_queue" / "stage051_ocr_queue_slice_contract.json"
SLICE = BASE / "ocr_queue" / "stage051_ocr_queue_slice.py"
BATCH = BASE / "BATCH051_060_UPLOAD_LOCK.yaml"
ROADMAP = ROOT / "docs" / "governance" / "roadmap.yaml"
EVENTS = ROOT / "docs" / "governance" / "events.jsonl"
STATUS = ROOT / "machine" / "facts" / "status.json"
RUN = ROOT / "machine" / "runs" / "2026-08-12-stage051-p2-local.json"


class Stage051OcrQueuePhase2Tests(unittest.TestCase):
    def _slice(self):
        spec = importlib.util.spec_from_file_location("stage051_ocr_queue_slice", SLICE)
    ...
    def test_contract_is_executable_but_engine_and_production_remain_disabled(self):
        contract = self._contract()
        self.assertEqual("ids.stage051.ocr_queue.phase2.v1", contract["schema_version"])
        self.assertEqual("IDS-V0_1-STAGE051-P2", contract["task_id"])
        self.assertTrue(contract["slice_executable"])
        self.assertFalse(contract["execution_ready"])
        self.assertEqual("IDS-STAGE051-P3-GATE", contract["next_gate"])
        self.assertFalse(
            contract["source_authority"]["second_authoritative_source_created"]
        )
        self.assertTrue(
            contract["implementation"]["in_memory_ocr_queue_slice_implemented"]
        )
        self.assertFalse(contract["implementation"]["ocr_engine_selected"])
    ...
    def test_high_confidence_control_page_creates_candidate_page_reference(self):
        result = self._slice().execute_controlled_ocr_queue(self._control())
        ...
        self.assertEqual("source:control:stage051-p2", page["source_identity_ref"])
        self.assertEqual("source-page:control:stage051-p2:1", page["source_page_ref"])
        self.assertEqual("中文控制页", page["ocr_text"])
    ...
    def test_failed_page_is_explicit_and_retains_no_text(self):
        result = self._slice().execute_controlled_ocr_queue(self._control())
    ...
    def test_invalid_control_rejects_without_returning_reference_or_text(self):
        control = self._control()
        control["unexpected"] = "not accepted"
        result = self._slice().execute_controlled_ocr_queue(control)
    ...
    def test_phase2_governance_projection_and_evidence_are_current(self):
        ...
        for text, expected in (
            (batch, 'status: "stage051_phase2_completed"'),
            (batch, 'current_task_id: "IDS-V0_1-STAGE051-P2"'),
            (batch, 'next_gate: "IDS-STAGE051-P3-GATE"'),
            (batch, 'in_memory_ocr_queue_slice_implemented: true'),
            ...
            (roadmap, 'phase_id: "IDS-STAGE051-P2"'),
            (roadmap, 'gate_id: "IDS-STAGE051-P2-GATE"'),
        ):
            with self.subTest(expected=expected):
                self.assertIn(expected, text)

        status = json.loads(STATUS.read_text(encoding="utf-8"))
        self.assertIn(status["stage"], ("IDS-STAGE051", "IDS-STAGE052", "IDS-STAGE053", "IDS-STAGE054", "IDS-STAGE055", "IDS-STAGE056", "IDS-STAGE057", "IDS-STAGE058", "IDS-STAGE059", "IDS-STAGE060"))
        self.assertIn(
            status["phase"],
            (
        ...
        self.assertEqual(
            "PASS_PHASE2_CONTROLLED_OCR_QUEUE_SLICE_ENGINE_DISABLED",
            run["result"].strip(chr(96)),
        )
        self.assertTrue(run["observed_work"]["in_memory_ocr_queue_slice_implemented"])
        ...
        event = next(
            item
            for item in events
            if item.get("event_id") == "EVT-IDS-V0_1-STAGE051-P2-20260812-001"
        )
        self.assertEqual("IDS-V0_1-STAGE051-P2", event["task_id"])
        self.assertEqual(["ACC-STAGE-051"], event["acceptance_ids"])
        self.assertIn("next_gate=IDS-STAGE051-P3-GATE", event["notes"])
