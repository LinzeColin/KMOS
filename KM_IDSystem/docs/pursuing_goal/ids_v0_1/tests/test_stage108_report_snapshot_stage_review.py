...
SCOPE = BASE / "STAGE108_STAGE_REVIEW.md"
CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage108_report_snapshot_stage_review_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage108_report_snapshot_stage_review.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
P1_CONTRACT = BASE / "index_version_schema" / "stage108_report_snapshot_contract.json"
P2_CONTRACT = (
...
P4_CONTRACT = (
    BASE / "index_version_schema" / "stage108_report_snapshot_delivery_contract.json"
)
P4_MODULE = BASE / "index_version_schema" / "stage108_report_snapshot_delivery.py"
P4_RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage108-p4-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage108-review-local.json"
...
class Stage108ReportSnapshotStageReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_module(MODULE, "stage108_review_for_test")
        cls.phase4 = _load_module(P4_MODULE, "stage108_phase4_for_review_test")
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.report = cls.module.build_report_snapshot_stage_review()
        cls.phase4_report = cls.phase4.build_report_snapshot_phase4_delivery_report()
    ...
    def test_identity_reviewed_shape_failure_contract_and_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("IDS-STAGE108-REVIEW", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE108-REVIEW", contract["task_id"])
        self.assertEqual(self.module.REVIEW_GATE, contract["entry_gate"])
        self.assertEqual(self.module.NEXT_GATE, contract["next_gate"])
        self.assertEqual(
            "STAGE108_REPORT_SNAPSHOT_REVIEW_RUNTIME_DISABLED",
        ...
        self.assertTrue(failure["stage109_must_remain_not_started"])
        ...
        for field in (
            ...
            "stage108_phase3_completed",
            "stage108_phase4_completed",
            "stage108_review_started",
            "whole_stage_review_completed_in_memory_only",
        ):
            with self.subTest(field=field):
                self.assertTrue(boundary[field])
        for field in ("stage109_started", "github_upload_allowed", "push_allowed"):
            with self.subTest(field=field):
    ...
    def test_review_passes_with_exact_phase_results_shapes_and_zero_runtime(self) -> None:
        ...
        self.assertFalse(report["stage109_started"])
        self.assertEqual(
            self.module.REVIEWED_CONTROL_SHAPE,
    ...
    def test_critical_binding_source_semantics_lifecycle_and_whitebox_gate_are_preserved(
        ...
        phase3 = _load_module(
            BASE
            / "index_version_schema"
            / "stage108_report_snapshot_controlled_scenarios.py",
            "stage108_phase3_for_review_semantics",
        )
        scenarios = phase3.build_report_snapshot_phase3_report()["scenario_results"]
    ...
    def test_tampered_phase_outputs_fail_closed_with_zero_runtime(self) -> None:
        ...
        malformed_p1["report_snapshot_contract"][
        ...
        malformed_p3 = _load_module(
            ...
            / "index_version_schema"
            / "stage108_report_snapshot_controlled_scenarios.py",
            "stage108_phase3_for_review_failure",
        ).build_report_snapshot_phase3_report()
        malformed_p3 = copy.deepcopy(malformed_p3)
        malformed_p3["scenario_results"][0][
            "evidence_gap_ref"
        ] = ":control:stage108-review:tampered-evidence-gap:reference-only"
        malformed_p4 = copy.deepcopy(self.phase4_report)
        malformed_p4["runtime_boundary"]["model_call_performed"] = True

        failures = (
            (
                self.module.build_report_snapshot_stage_review(
    ...
    def test_current_governance_receipt_and_event_are_exact(self) -> None:
        ...
        review_current = (
            "IDS-STAGE108",
            "IDS-STAGE108-REVIEW",
            "IDS-V0_1-STAGE108-REVIEW",
            "IDS-STAGE109-P1-GATE",
        )
        self.assertTrue(
            assert_legacy_or_current_projection(
        ...
        self.assertEqual(
            38,
            validation["stage108_phase1_to_review_compatibility_test_count"],
        )
        self.assertEqual(
            967,
            validation["stage088_to_stage108_precise_chain_test_count"],
        )
        ...
        self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-108"])
        for acceptance_id in (
            "ACC-STAGE108-REVIEW-01",
            "ACC-STAGE108-REVIEW-02",
            "ACC-STAGE108-REVIEW-03",
        ):
            with self.subTest(acceptance_id=acceptance_id):
                self.assertEqual("已通过", acceptance_by_id[acceptance_id])
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE108-REVIEW-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE108-REVIEW-20260826-001", event_ids)

    def test_scope_contract_and_review_output_keep_every_runtime_surface_closed(
        self,
    ) -> None:
