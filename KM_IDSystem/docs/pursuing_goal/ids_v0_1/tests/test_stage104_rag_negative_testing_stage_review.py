...
REVIEW_SOURCE = BASE / "stage104_rag_negative_testing_stage_review.py"
REVIEW_CONTRACT = BASE / "stage104_rag_negative_testing_stage_review_contract.json"
P1_CONTRACT = BASE / "stage104_rag_negative_testing_contract.json"
P2_SOURCE = BASE / "stage104_rag_negative_testing_control_slice.py"
P3_SOURCE = BASE / "stage104_rag_negative_testing_controlled_scenarios.py"
P4_SOURCE = BASE / "stage104_rag_negative_testing_delivery.py"
SCOPE_DOCUMENT = BASE.parent / "STAGE104_STAGE_REVIEW.md"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
REVIEW_RUN = ROOT / "machine" / "runs" / "2026-08-26-stage104-review-local.json"
...
class Stage104RagNegativeTestingReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.review = _load_module("stage104_review_test_module", REVIEW_SOURCE)
    ...
    def test_required_artifacts_and_identity_exist(self) -> None:
        ...
        self.assertEqual(
            ...
            "ids.stage104.rag_negative_testing.stage_review.v1",
        )
        self.assertEqual(
            self.review.RECORD_KIND,
            "CONTROL_ONLY_IN_MEMORY_RAG_NEGATIVE_TEST_STAGE_REVIEW",
        )
        self.assertEqual(self.review.REVIEW_GATE, "IDS-STAGE104-REVIEW-GATE")
        self.assertEqual(self.review.NEXT_GATE, "IDS-STAGE105-P1-GATE")
    ...
    def test_contract_keeps_authority_shape_and_runtime_closed(self) -> None:
        ...
        self.assertEqual(contract["phase"], "IDS-STAGE104-REVIEW")
        self.assertEqual(contract["task_id"], "IDS-V0_1-STAGE104-REVIEW")
        ...
        self.assertEqual(reviewed["phase1_static_shape"], "13/5/4/5/19/4")
        self.assertEqual(reviewed["phase2_control_field_check_count"], 285)
        self.assertEqual(reviewed["phase3_scenario_field_check_count"], 170)
        self.assertEqual(reviewed["phase4_delivery_field_check_count"], 384)
        self.assertEqual(reviewed["reproducibility_tuple_field_count"], 8)
        self.assertTrue(
            all(value == 0 for key, value in runtime.items() if key.endswith("_count"))
        )
        self.assertTrue(
            all(value is False for key, value in runtime.items() if not key.endswith("_count"))
        )
        self.assertTrue(boundary["stage104_review_started"])
        self.assertTrue(boundary["whole_stage_review_completed_in_memory_only"])
        self.assertFalse(boundary["stage105_started"])
    ...
    def test_default_report_is_valid_and_replays_fixed_phases(self) -> None:
        report = self.review.build_rag_negative_testing_stage104_review_report()
    ...
    def test_fixed_review_shape_and_p4_to_p3_rollback_are_preserved(self) -> None:
        ...
        self.assertEqual(shape["phase1_reference_field_count"], 13)
        self.assertEqual(shape["phase1_negative_test_case_count"], 5)
        self.assertEqual(shape["phase2_projection_field_count_per_request"], 57)
        self.assertEqual(shape["phase3_scenario_field_count"], 34)
        self.assertEqual(shape["phase3_human_handling_required_count"], 5)
        self.assertEqual(shape["phase4_delivery_shape"], "5/5/5/5/5/2")
    ...
    def test_authority_semantic_and_whitebox_controls_are_preserved(self) -> None:
        ...
        self.assertTrue(
            semantic["high_risk_engineering_advice_requires_business_line_whitebox_confirmation"]
        )
        self.assertTrue(
            semantic["contractual_commitment_requires_business_line_whitebox_confirmation"]
        )
        self.assertTrue(
            semantic["production_writeback_requires_business_line_whitebox_confirmation"]
        )
    ...
    def test_runtime_and_next_stage_boundaries_are_closed(self) -> None:
        ...
        self.assertFalse(stage["stage105_started"])
        ...
        self.assertTrue(failure["stage105_must_remain_not_started"])
        self.assertTrue(
    ...
    def test_phase1_or_phase2_drift_fails_closed(self) -> None:
        ...
        phase1["future_rag_control_contract"]["future_control_reference_field_count"] = 12
        phase1_report = self.review.build_rag_negative_testing_stage104_review_report(
            phase1_provider=lambda: phase1
        )
        self.assertFalse(phase1_report["valid"])
        self.assertEqual(
            phase1_report["failure_state"], "P1_CONTRACT_OR_CONTROL_OUTPUT_INVALID"
        )

        phase2 = copy.deepcopy(self.review._default_phase2_report())
        phase2["control_projection_field_total"] = 284
        phase2_report = self.review.build_rag_negative_testing_stage104_review_report(
            phase2_provider=lambda: phase2
        )
        self.assertFalse(phase2_report["valid"])
        self.assertEqual(
            phase2_report["failure_state"], "P2_CONTRACT_OR_CONTROL_OUTPUT_INVALID"
        )
    ...
    def test_phase3_or_phase4_drift_fails_closed(self) -> None:
        ...
        phase3_report = self.review.build_rag_negative_testing_stage104_review_report(
            phase3_provider=lambda: phase3
        )
        self.assertFalse(phase3_report["valid"])
        self.assertEqual(
            phase3_report["failure_state"], "P3_CONTRACT_OR_CONTROL_OUTPUT_INVALID"
        )
        ...
        phase4_report = self.review.build_rag_negative_testing_stage104_review_report(
            phase4_provider=lambda: phase4
        )
        self.assertFalse(phase4_report["valid"])
        self.assertEqual(
            phase4_report["failure_state"], "P4_CONTRACT_OR_CONTROL_OUTPUT_INVALID"
        )
    ...
    def test_report_is_detached_from_future_mutation(self) -> None:
        ...
        fresh = self.review.build_rag_negative_testing_stage104_review_report()

        self.assertEqual(
            fresh["reviewed_control_shape"]["phase4_delivery_field_check_count"], 384
        )
    ...
    def test_scope_receipt_and_current_governance_projection_are_explicit(self) -> None:
        ...
        self.assertEqual(list(self.review.FAILURE_STATES), failures["declared_failure_states"])
        self.assertTrue(failures["stage105_must_remain_not_started"])
        ...
        if current == review_current:
            ...
            self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-104"])
            for acceptance_id in (
                "ACC-STAGE104-REVIEW-01",
                "ACC-STAGE104-REVIEW-02",
                "ACC-STAGE104-REVIEW-03",
            ):
                with self.subTest(acceptance_id=acceptance_id):
                    self.assertEqual("已通过", acceptance_by_id[acceptance_id])
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE104-REVIEW-04"])
            event_ids = {
                json.loads(line)["event_id"]
                for line in EVENTS.read_text(encoding="utf-8").splitlines()
                if line.strip()
            }
            self.assertIn("EVT-IDS-V0_1-STAGE104-REVIEW-20260826-001", event_ids)
