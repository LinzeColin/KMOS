...
from KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema import (
    stage115_review_ui_controlled_scenarios as controlled_scenarios,
)
...
SCOPE = BASE / "STAGE115_PHASE3_REVIEW_UI_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
P2_SCOPE = BASE / "STAGE115_PHASE2_REVIEW_UI_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage115_review_ui_control_slice_contract.json"
)
P2_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage115-p2-local.json"
P3_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage115-p3-local.json"
...
EXPECTED_SCENARIOS = (
    (
        ...
        "CONTROL_BINDING_REVIEW_QUEUE_ROUTE",
        "pending_review",
        "submit_for_review",
    ),
    (
    ...
    (
        ...
        "CONTROL_BINDING_EXTERNAL_AUGMENTATION_ROUTE",
        "archived",
        "archive",
    ),
)
...
class Stage115ReviewUiPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.result = controlled_scenarios.project_review_ui_controlled_scenarios(
            cls.controlled_scenario_input
        )
    ...
    def test_identity_authority_predecessor_and_contract_shapes_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage115.review_ui.phase3.v1", contract["schema_version"]
        )
        self.assertEqual("STAGE-115", contract["stage"])
        self.assertEqual("IDS-STAGE115-P3", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE115-P3", contract["task_id"])
        self.assertEqual("ACC-STAGE-115", contract["acceptance_id"])
        self.assertEqual("IDS-STAGE115-P3-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE115-P4-GATE", contract["next_gate"])
        self.assertEqual(
        ...
        self.assertEqual(
            "FROZEN_STAGE115_TASKPACK_STAGE115_PHASE1_PHASE2_AND_STAGE114_REVIEW_CONTROL_ARTIFACTS_ONLY",
            authority["authority"],
        )
        for field in (
        ...
        self.assertTrue(predecessor["stage115_phase2_required"])
        self.assertEqual(
            control_slice.PASS_RESULT, predecessor["stage115_phase2_result"]
        )
        scenarios = contract["scenario_contract"]
        self.assertEqual(5, scenarios["phase2_control_request_count"])
        self.assertEqual(23, scenarios["phase2_control_input_field_count"])
        self.assertEqual(19, scenarios["phase2_phase1_reference_field_count"])
        self.assertEqual(4, scenarios["phase2_projection_group_count"])
        self.assertEqual(117, scenarios["phase2_projection_field_count_per_request"])
        self.assertEqual(585, scenarios["phase2_projection_field_check_count"])
        self.assertEqual(5, scenarios["scenario_count"])
        self.assertEqual(47, scenarios["scenario_field_count"])
        self.assertEqual(235, scenarios["scenario_field_check_count"])
    ...
    def test_canonical_input_strictly_preserves_p2_shape_and_scenario_binding(self) -> None:
        ...
        self.assertEqual(23, len(controlled_scenarios.PHASE2_INPUT_FIELDS))
        self.assertEqual(19, len(controlled_scenarios.PHASE2_REFERENCE_FIELDS))
        self.assertEqual(47, len(controlled_scenarios.SCENARIO_FIELDS))
        for scenario, expected in zip(scenarios, EXPECTED_SCENARIOS):
            ...
            with self.subTest(scenario=scenario_id):
                ...
                self.assertEqual(binding_mode, scenario["control_binding_mode"])
                self.assertEqual(
                    review_status, scenario["fixed_review_status_control_value"]
                )
                self.assertEqual(
                    review_action, scenario["fixed_review_action_control_value"]
                )
    ...
    def test_projection_shape_and_zero_runtime_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(23, result["phase2_control_input_field_count"])
        self.assertEqual(19, result["phase2_phase1_reference_field_count"])
        self.assertEqual(4, result["phase2_projection_group_count"])
        self.assertEqual(117, result["phase2_projection_field_total_per_request"])
        self.assertEqual(585, result["phase2_projection_field_check_count"])
        self.assertEqual(5, result["controlled_scenario_count"])
        self.assertEqual(47, result["controlled_scenario_field_count"])
        self.assertEqual(235, result["controlled_scenario_field_check_count"])
    ...
    def test_review_operation_fields_remain_future_control_references(self) -> None:
        ...
        for field in (
            ...
            "evidence_trust_level_before_after_controls_required",
            "report_quality_score_before_after_controls_required",
        ):
            with self.subTest(field=field):
    ...
    def test_impact_external_augmentation_and_whitebox_controls_are_exact(self) -> None:
        ...
        for handling in self.result["business_line_whitebox_handlings"]:
            with self.subTest(scenario=handling["controlled_scenario_id"]):
                ...
                self.assertFalse(handling["actual_human_confirmation_execution_performed"])
                self.assertFalse(handling["actual_final_business_conclusion_recorded"])
    ...
    def test_input_drift_fails_closed_without_scenarios_or_runtime(self) -> None:
        ...
        drifted[controlled_scenarios.CONTROL_FIELDS[0]][0]["review_status_ref"] = (
            "drifted"
        )
        rejected = controlled_scenarios.project_review_ui_controlled_scenarios(
            drifted
        )
        ...
        self.assertEqual("CONTROLLED_SCENARIO_INPUT_MISMATCH", rejected["failure_state"])
    ...
    def test_scope_contract_and_final_governance_follow_phase3(self) -> None:
        ...
        self.assertEqual("P3 专项异常场景已完成", acceptance_by_id["ACC-STAGE-115"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE115-P3-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE115-P3-20260827-001", event_ids)
