...
REVIEW_DOCUMENT = BASE / "STAGE100_STAGE_REVIEW.md"
CONTRACT = (
    BASE
...
MODULE = (
    BASE
    / "index_version_schema"
    / "stage100_no_internal_evidence_strategy_stage_review.py"
)
P1_CONTRACT = BASE / "index_version_schema" / "stage100_no_internal_evidence_strategy_contract.json"
P2_MODULE = BASE / "index_version_schema" / "stage100_no_internal_evidence_strategy_control_slice.py"
P3_MODULE = BASE / "index_version_schema" / "stage100_no_internal_evidence_strategy_controlled_scenarios.py"
P4_MODULE = BASE / "index_version_schema" / "stage100_no_internal_evidence_strategy_delivery.py"
P4_RUN = ROOT / "machine" / "runs" / "2026-08-25-stage100-p4-local.json"
REVIEW_RUN = ROOT / "machine" / "runs" / "2026-08-25-stage100-review-local.json"
...
class Stage100NoInternalEvidenceStrategyReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage100_no_internal_evidence_strategy_review", MODULE)
        cls.report = cls.module.build_no_internal_evidence_strategy_stage100_review_report()
    ...
    def test_required_phase_artifacts_exist(self) -> None:
        ...
        self.assertEqual("IDS-STAGE100-REVIEW", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE100-REVIEW", self.contract["task_id"])
        self.assertEqual("IDS-STAGE100-REVIEW-GATE", self.contract["entry_gate"])
        self.assertEqual("IDS-STAGE101-P1-GATE", self.contract["next_gate"])

    def test_contract_keeps_authority_runtime_and_stage101_closed(self) -> None:
        ...
        self.assertEqual("9/4/3/17/4", replay["phase1_static_shape"])
        self.assertEqual(228, replay["phase2_control_field_check_count"])
        self.assertEqual(174, replay["phase3_scenario_field_check_count"])
    ...
    def test_evidence_gap_injection_and_high_risk_controls_are_preserved(self) -> None:
    ...
    def test_runtime_and_rollback_boundaries_are_closed(self) -> None:
        ...
        self.assertFalse(report["stage101_started"])
        self.assertEqual(self.module.P4_PASS_RESULT, report["rollback"]["return_to"])
        self.assertTrue(report["rollback"]["preserve_stage100_phase1_to_phase4_evidence"])
        self.assertTrue(report["rollback"]["preserve_stage099_review_evidence"])
    ...
    def test_invalid_phase_contracts_fail_closed(self) -> None:
        for phase_name, kwargs in (
            ...
            with self.subTest(phase=phase_name):
                report = self.module.build_no_internal_evidence_strategy_stage100_review_report(
                    **kwargs
                )
    ...
    def test_review_document_contract_and_rollback_are_explicit(self) -> None:
        ...
        self.assertTrue(failures["stage101_must_remain_not_started"])
        rollback = self.contract["rollback_contract"]
        self.assertEqual(self.module.P4_PASS_RESULT, rollback["fallback_result"])
    ...
    def test_governance_projection_records_review_when_current(self) -> None:
        ...
        self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-100"])
        for acceptance_id in (
            "ACC-STAGE100-REVIEW-01",
            "ACC-STAGE100-REVIEW-02",
            "ACC-STAGE100-REVIEW-03",
        ):
            with self.subTest(acceptance_id=acceptance_id):
                self.assertEqual("已通过", acceptance_by_id[acceptance_id])
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE100-REVIEW-04"])
        self.assertIn("EVT-IDS-V0_1-STAGE100-REVIEW-20260825-001", event_ids)
        roadmap_text = ROADMAP.read_text(encoding="utf-8")
        for phrase in (
            "stage100_review_state:",
