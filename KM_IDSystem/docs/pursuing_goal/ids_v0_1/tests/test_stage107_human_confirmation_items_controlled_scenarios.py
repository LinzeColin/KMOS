"""Stage107 人工确认事项章节 Phase 3 纯内存专项场景的聚焦验证。"""
...
SCOPE = BASE / "STAGE107_PHASE3_HUMAN_CONFIRMATION_ITEMS_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
PHASE2_RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage107-p2-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage107-p3-local.json"
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "stage107_human_confirmation_items_controlled_scenarios", MODULE
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage107 P3 人工确认事项专项场景模块")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage107HumanConfirmationItemsPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_module()
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.report = cls.module.build_human_confirmation_items_phase3_report()

    def _mutated_phase2_executor(self, mutation):
        def executor(control_input):
            phase2_module = self.module._load_phase2_module()
            result = phase2_module.execute_human_confirmation_items_control_slice(
                control_input
            )
            mutation(result)
            return result
    ...
    def test_identity_authority_predecessors_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage107.human_confirmation_items.phase3.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-107", contract["stage"])
        self.assertEqual("IDS-STAGE107-P3", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE107-P3", contract["task_id"])
        self.assertEqual("ACC-STAGE-107", contract["acceptance_id"])
        self.assertEqual("IDS-STAGE107-P3-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE107-P4-GATE", contract["next_gate"])
        self.assertEqual(
        ...
        self.assertEqual(
            "FROZEN_STAGE107_TASKPACK_STAGE107_PHASE1_PHASE2_AND_STAGE106_REVIEWED_EXTERNAL_AUGMENTATION_CONTROL_ARTIFACTS_ONLY",
            source["authority"],
        )
        for field in (
        ...
        for field in (
            "stage106_review_required",
            "stage107_phase1_required",
            "stage107_phase2_required",
        ):
            with self.subTest(field=field):
        ...
        self.assertEqual(
            "PASS_IN_MEMORY_HUMAN_CONFIRMATION_ITEMS_CONTROL_SLICE_RUNTIME_DISABLED",
            predecessor["stage107_phase2_result"],
        )
        boundary = contract["stage_and_phase_boundary"]
        for field in (
            "stage106_review_preserved",
    ...
    def test_phase2_replay_and_scenario_contract_have_exact_shape(self) -> None:
        ...
        self.assertEqual(
            "CONTROL_ONLY_IN_MEMORY_HUMAN_CONFIRMATION_ITEMS",
        ...
        self.assertEqual(39, scenarios["scenario_field_count"])
        self.assertEqual(234, scenarios["scenario_field_check_count"])
        self.assertEqual(5, scenarios["control_view_count"])
        self.assertEqual(6, scenarios["human_handling_count"])
        self.assertEqual(6, scenarios["whitebox_confirmation_required_scenario_count"])
        self.assertEqual(list(self.module.SCENARIO_FIELDS), scenarios["scenario_fields"])
        self.assertEqual(
            EXPECTED_HUMAN_CONFIRMATION_CATEGORIES,
            scenarios["required_human_confirmation_categories"],
        )
        self.assertEqual(
    ...
    def test_report_projects_six_closed_control_scenarios(self) -> None:
        report = self.report
        self.assertTrue(report["valid"])
        self.assertEqual(self.module.PASS_RESULT, report["result"])
        self.assertIsNone(report["failure_state"])
        self.assertEqual("IDS-STAGE107-P3-GATE", report["current_gate"])
        self.assertEqual("IDS-STAGE107-P4-GATE", report["next_gate"])
        for field in (
        ...
        self.assertEqual(79, report["phase2_projection_field_count_per_request"])
        self.assertEqual(474, report["phase2_projection_field_count_total"])
        self.assertEqual(6, report["scenario_count"])
        self.assertEqual(39, report["scenario_field_count"])
        self.assertEqual(234, report["scenario_field_check_count"])
        ...
        self.assertEqual(6, len(report["scenario_results"]))
        for scenario in report["scenario_results"]:
            with self.subTest(scenario=scenario["scenario_id"]):
                ...
                for field in (
                    ...
                    self.assertTrue(
                        scenario[field].startswith(":control:stage107-p2:"), field
                    )
                for field in (
    ...
    def test_taskpack_special_cases_and_human_handlings_are_preserved(self) -> None:
        ...
        self.assertEqual(
            EXPECTED_HUMAN_CONFIRMATION_CATEGORIES,
            [item["human_confirmation_category"] for item in scenarios],
        )
        self.assertEqual(3, sum(item["evidence_id_ref"] is not None for item in scenarios))
        self.assertEqual(3, sum(item["evidence_gap_ref"] is not None for item in scenarios))
        for scenario in scenarios:
            with self.subTest(category=scenario["human_confirmation_category"]):
                self.assertEqual(
        ...
        self.assertEqual(6, len(handlings))
        self.assertEqual(6, sum(item["whitebox_confirmation_required"] for item in handlings))
        self.assertEqual(
            EXPECTED_HUMAN_CONFIRMATION_CATEGORIES,
            [item["human_confirmation_category"] for item in handlings],
        )
    ...
    def test_projection_drift_fails_closed(self) -> None:
        shape_mismatch = self.module.build_human_confirmation_items_phase3_report(
        ...
        def break_binding(result):
            result[
                ...
                ":control:stage107-p2:unexpected-evidence-id:reference-only"
            )

        binding_drift = self.module.build_human_confirmation_items_phase3_report(
        ...
        def allow_automatic_status_update(result):
            result["report_status_quality_and_export_audit_control_projections"][4][
                "automatic_report_status_impact_update_allowed"
            ] = True

        lifecycle_drift = self.module.build_human_confirmation_items_phase3_report(
        ...
        external_drift = self.module.build_human_confirmation_items_phase3_report(
        ...
        runtime_drift = self.module.build_human_confirmation_items_phase3_report(
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        if current == phase3_current:
            ...
            self.assertEqual("P3 专项控制场景已完成", acceptance_by_id["ACC-STAGE-107"])
            ...
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE107-P3-04"])
            event_ids = {
                json.loads(line)["event_id"]
                for line in EVENTS.read_text(encoding="utf-8").splitlines()
                if line.strip()
            }
            self.assertIn("EVT-IDS-V0_1-STAGE107-P3-20260826-001", event_ids)
