"""Stage106 外部增强意见章节 Phase 3 纯内存专项场景的聚焦验证。"""
...
SCOPE = BASE / "STAGE106_PHASE3_EXTERNAL_AUGMENTATION_OPINION_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage106-p3-local.json"
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "stage106_external_augmentation_opinion_controlled_scenarios", MODULE
    )
...
class Stage106ExternalAugmentationOpinionPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_module()
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.report = cls.module.build_external_augmentation_opinion_phase3_report()

    def _mutated_phase2_executor(self, mutation):
        def executor(control_input):
            phase2_module = self.module._load_phase2_module()
            result = phase2_module.execute_external_augmentation_opinion_control_slice(
                control_input
            )
            mutation(result)
            return result
    ...
    def test_identity_authority_predecessors_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage106.external_augmentation_opinion.phase3.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-106", contract["stage"])
        self.assertEqual("IDS-STAGE106-P3", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE106-P3", contract["task_id"])
        self.assertEqual("ACC-STAGE-106", contract["acceptance_id"])
        self.assertEqual("IDS-STAGE106-P3-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE106-P4-GATE", contract["next_gate"])
        self.assertEqual(
        ...
        self.assertEqual(
            "FROZEN_STAGE106_TASKPACK_STAGE106_PHASE1_PHASE2_AND_STAGE105_REVIEWED_REPORT_EVIDENCE_BINDING_CONTROL_ARTIFACTS_ONLY",
            source["authority"],
        )
        for field in (
        ...
        for field in (
            "stage105_review_required",
            "stage106_phase1_required",
            "stage106_phase2_required",
        ):
            with self.subTest(field=field):
        ...
        self.assertEqual(
            "PASS_IN_MEMORY_EXTERNAL_AUGMENTATION_OPINION_CONTROL_SLICE_RUNTIME_DISABLED",
            predecessor["stage106_phase2_result"],
        )
    ...
    def test_phase2_replay_and_scenario_contract_have_exact_shape(self) -> None:
        ...
        self.assertEqual(
            "CONTROL_ONLY_IN_MEMORY_EXTERNAL_AUGMENTATION_OPINION",
            replay["phase2_record_kind"],
        )
        self.assertEqual(5, replay["phase2_control_request_count"])
        self.assertEqual(30, replay["phase2_input_field_count"])
        self.assertEqual(27, replay["phase2_phase1_reference_field_count"])
        ...
        self.assertEqual(74, replay["phase2_projection_field_count_per_request"])
        self.assertEqual(370, replay["phase2_projection_field_count_total"])
    ...
    def test_report_projects_five_closed_control_scenarios(self) -> None:
        ...
        self.assertEqual("IDS-STAGE106-P3-GATE", report["current_gate"])
        self.assertEqual("IDS-STAGE106-P4-GATE", report["next_gate"])
        for field in (
        ...
        self.assertEqual(30, report["phase2_input_field_count"])
        self.assertEqual(4, report["phase2_projection_group_count"])
        self.assertEqual(74, report["phase2_projection_field_count_per_request"])
        self.assertEqual(370, report["phase2_projection_field_count_total"])
        ...
        for scenario in report["scenario_results"]:
            with self.subTest(scenario=scenario["scenario_id"]):
                ...
                for field in (
                    ...
                    self.assertTrue(
                        scenario[field].startswith(":control:stage106-p2:"), field
                    )
                self.assertFalse(scenario["automatic_final_conclusion_allowed"])
                self.assertFalse(
                    scenario["actual_report_status_impact_analysis_performed"]
                )
                self.assertFalse(
                    scenario["actual_external_augmentation_opinion_generated"]
                )
    ...
    def test_projection_drift_fails_closed(self) -> None:
        shape_mismatch = self.module.build_external_augmentation_opinion_phase3_report(
        ...
        def break_binding(result):
            result["report_evidence_binding_and_chapter_control_projections"][1][
                "evidence_id_ref"
            ] = ":control:stage106-p2:unexpected-evidence-id:reference-only"

        binding_drift = self.module.build_external_augmentation_opinion_phase3_report(
        ...
        def allow_automatic_status_update(result):
            result["report_lifecycle_and_export_audit_control_projections"][4][
                "automatic_report_status_impact_update_allowed"
            ] = True

        lifecycle_drift = self.module.build_external_augmentation_opinion_phase3_report(
        ...
        external_drift = self.module.build_external_augmentation_opinion_phase3_report(
        ...
        runtime_drift = self.module.build_external_augmentation_opinion_phase3_report(
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        if current == phase3_current:
            ...
            self.assertIn("IDS-STAGE106-P4-GATE", plan["stop_condition"])
        elif current in {phase4_current, review_current, stage107_phase1_current, stage107_phase2_current}:
            self.assertTrue(is_current_projection)
        else:
        ...
        if current == phase3_current:
            self.assertEqual("P3 专项异常场景已完成", acceptance_by_id["ACC-STAGE-106"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE106-P3-04"])

        receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
        self.assertEqual(self.module.PASS_RESULT, receipt["result"])
        self.assertEqual("IDS-STAGE106-P4-GATE", receipt["next_gate"])
        ...
        self.assertEqual(23, validation["focused_test_count"])
        self.assertTrue(validation["final_validation_recorded"])
        self.assertTrue(validation["all_local_validation_passed"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE106-P3-20260826-001", event_ids)
