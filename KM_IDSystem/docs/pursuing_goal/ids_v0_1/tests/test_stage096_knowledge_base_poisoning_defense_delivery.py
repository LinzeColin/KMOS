...
MODULE = (
    BASE
    / "index_version_schema"
    / "stage096_knowledge_base_poisoning_defense_delivery.py"
)
P3_MODULE = (
    BASE
    / "index_version_schema"
...
class Stage096KnowledgeBasePoisoningDefensePhase4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = load_module("stage096_phase4_delivery", MODULE)
        cls.phase3 = load_module("stage096_phase3_scenarios", P3_MODULE)
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.report = cls.module.build_knowledge_base_poisoning_defense_phase4_delivery_report()

    def test_contract_identity_authority_and_phase_boundary(self) -> None:
        self.assertEqual(
            "ids.stage096.knowledge_base_poisoning_defense.phase4.delivery.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("IDS-V0_1-STAGE096-P4", self.contract["task_id"])
        self.assertEqual("IDS-STAGE096-P4-GATE", self.contract["entry_gate"])
        self.assertEqual("IDS-STAGE096-REVIEW-GATE", self.contract["next_gate"])
                ...
                for field, value in record.items():
                    ...
                    with self.subTest(group=name, field=field, value=value):
                        if value is None:
                            ...
                            self.assertTrue(
                                value.startswith(self.module.CONTROL_PREFIX)
    ...
    def test_exception_dispositions_are_carried_forward(self) -> None:
        ...
        self.assertEqual(
            ...
            grades["low_ocr_poisoning_defense_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        self.assertEqual(
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED",
            grades["old_version_poisoning_defense_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        impacts = {
    ...
    def test_failure_stop_and_rollback_contracts_are_explicit(self) -> None:
        ...
        self.assertTrue(rollback["preserve_stage096_phase1_phase2_phase3"])
    ...
    def test_invalid_predecessor_output_returns_controlled_failure(self) -> None:
        failed = self.module.build_knowledge_base_poisoning_defense_phase4_delivery_report(
            lambda: {}
        )
    ...
    def test_predecessor_runtime_signal_returns_controlled_failure(self) -> None:
        altered = copy.deepcopy(
            self.phase3.build_knowledge_base_poisoning_defense_phase3_report()
        )
    ...
    def test_nonopaque_reference_and_semantic_drift_return_specific_failures(self) -> None:
        ...
        semantic_drift["scenario_results"][1]["degradation_state"] = (
    ...
    def test_current_governance_preserves_phase_history_and_p4_boundary(self) -> None:
        for path in (STATUS, PLAN, ACCEPTANCE, EVENTS, ROADMAP):
        ...
        if current == phase4_current:
            ...
            self.assertEqual("P4 交付证据已完成", acceptance_by_id["ACC-STAGE-096"])
            ...
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE096-P4-04"])
            self.assertIn("EVT-IDS-V0_1-STAGE096-P4-20260824-001", event_ids)
            self.assertEqual("IDS-STAGE096-REVIEW-GATE", receipt["next_gate"])
        ...
        elif current in (review_current, stage097_phase1_current):
