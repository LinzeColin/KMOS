"""Stage114 P4 复核工作流 metadata-only 交付控制验证。"""
...
SCOPE = BASE / "STAGE114_PHASE4_REVIEW_WORKFLOW_DELIVERY.md"
CONTRACT = BASE / "index_version_schema" / "stage114_review_workflow_delivery_contract.json"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage114-p4-local.json"
...
class Stage114ReviewWorkflowPhase4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = importlib.import_module(
            "KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema."
            "stage114_review_workflow_delivery"
        )
        ...
        cls.report = cls.module.build_review_workflow_phase4_delivery_report()

    def _mutated_phase3_executor(self, mutator):
        def executor():
            result = self.phase3.project_review_workflow_controlled_scenarios(
                self.phase3.build_controlled_scenario_input()
            )
    ...
    def test_identity_authority_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage114.review_workflow.phase4.delivery.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-114", contract["stage"])
        self.assertEqual("IDS-STAGE114-P4", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE114-P4", contract["task_id"])
        self.assertEqual("ACC-STAGE-114", contract["acceptance_id"])
        self.assertEqual(
            "REVIEW_WORKFLOW_DELIVERY_EVIDENCE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE114-P4-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE114-REVIEW-GATE", contract["next_gate"])
    ...
    def test_phase3_replay_and_delivery_contract_have_exact_shape(self) -> None:
        ...
        expected_replay = {
            ...
            "phase2_projection_field_count_total": 660,
            "scenario_count": 5,
            "scenario_field_count": 54,
            "scenario_field_check_count": 270,
        ...
        self.assertEqual(
            list(self.phase3.CONTROLLED_SCENARIO_IDS), replay["scenario_ids"]
        )
    ...
    def test_delivery_projects_closed_metadata_only_control_evidence(self) -> None:
        ...
        self.assertEqual("IDS-STAGE114-P4-GATE", report["current_gate"])
        self.assertEqual("IDS-STAGE114-REVIEW-GATE", report["next_gate"])
        ...
        self.assertEqual(54, report["scenario_field_count"])
        self.assertEqual(270, report["scenario_field_check_count"])
        self.assertEqual(660, report["phase2_projection_field_count_total"])
    ...
    def test_delivery_group_shapes_and_taskpack_controls_are_exact(self) -> None:
        ...
        for record in report["review_audit_log_sample_control_records"]:
            with self.subTest(scenario=record["controlled_scenario_id"]):
                ...
                self.assertTrue(
                    record["review_actor_ref"].startswith(self.module.P2_CONTROL_PREFIX)
                )
    ...
    def test_manual_boundary_whitebox_and_rollback_are_exact(self) -> None:
        ...
        for record in report["rollback_and_re_review_instruction_control_records"]:
            with self.subTest(domain=record["control_domain"]):
                ...
                self.assertFalse(record["actual_review_workflow_rollback_performed"])
    ...
    def test_phase3_drift_fails_closed_without_delivery_records_or_runtime(self) -> None:
        rejected = self.module.build_review_workflow_phase4_delivery_report(
        ...
        self.assertEqual(self.module.FAIL_RESULT, rejected["result"])
    ...
    def test_scope_contract_and_final_governance_follow_phase4(self) -> None:
        ...
        self.assertEqual(
            list(self.module.FAILURE_STATES), failure_contract["declared_failure_states"]
        )
        self.assertTrue(
        ...
        if not future_projection:
            ...
            self.assertIn("IDS-STAGE114-REVIEW-GATE", plan["stop_condition"])
            acceptance = json.loads(ACCEPTANCE.read_text(encoding="utf-8"))
            acceptance_by_id = {item["id"]: item["status"] for item in acceptance["items"]}
            self.assertEqual(
                "P4 复核工作流交付控制证据已完成",
                acceptance_by_id["ACC-STAGE-114"],
            )
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE114-P4-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE114-P4-20260827-001", event_ids)
