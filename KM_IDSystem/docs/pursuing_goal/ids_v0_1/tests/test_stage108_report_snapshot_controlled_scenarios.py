"""Stage108 报告快照 Phase 3 纯内存专项场景的聚焦验证。"""
...
SCOPE = BASE / "STAGE108_PHASE3_REPORT_SNAPSHOT_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
MODULE = (
    BASE / "index_version_schema" / "stage108_report_snapshot_controlled_scenarios.py"
)
TASKPACK = (
    ROOT
    / "docs"
...
PHASE1_RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage108-p1-local.json"
PHASE2_SCOPE = BASE / "STAGE108_PHASE2_REPORT_SNAPSHOT_CONTROL_SLICE.md"
PHASE2_CONTRACT = (
    BASE / "index_version_schema" / "stage108_report_snapshot_control_slice_contract.json"
)
PHASE2_MODULE = BASE / "index_version_schema" / "stage108_report_snapshot_control_slice.py"
PHASE2_RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage108-p2-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage108-p3-local.json"
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "stage108_report_snapshot_controlled_scenarios", MODULE
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage108 P3 报告快照专项场景模块")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage108ReportSnapshotPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_module()
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.report = cls.module.build_report_snapshot_phase3_report()

    def _mutated_phase2_executor(self, mutation):
        def executor(control_input):
            phase2_module = self.module._load_phase2_module()
            result = phase2_module.execute_report_snapshot_control_slice(control_input)
            mutation(result)
            return result
    ...
    def test_identity_authority_predecessors_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("ids.stage108.report_snapshot.phase3.v1", contract["schema_version"])
        self.assertEqual("STAGE-108", contract["stage"])
        self.assertEqual("IDS-STAGE108-P3", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE108-P3", contract["task_id"])
        self.assertEqual("ACC-STAGE-108", contract["acceptance_id"])
        self.assertEqual("IDS-STAGE108-P3-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE108-P4-GATE", contract["next_gate"])
        self.assertEqual(
        ...
        self.assertEqual(
            "FROZEN_STAGE108_TASKPACK_STAGE108_PHASE1_PHASE2_AND_STAGE107_"
            "REVIEWED_HUMAN_CONFIRMATION_ITEMS_CONTROL_ARTIFACTS_ONLY",
            authority["authority"],
        )
        for field in (
        ...
        for field in (
            "stage107_review_required",
            "stage108_phase1_required",
            "stage108_phase2_required",
        ):
            with self.subTest(field=field):
        ...
        self.assertEqual(
            "PASS_REPORT_SNAPSHOT_CONTRACT_RUNTIME_DISABLED",
            predecessor["stage108_phase1_result"],
        )
        self.assertEqual(
            "PASS_IN_MEMORY_REPORT_SNAPSHOT_CONTROL_SLICE_RUNTIME_DISABLED",
            predecessor["stage108_phase2_result"],
        )
        boundary = contract["stage_and_phase_boundary"]
        for field in (
            "stage107_review_preserved",
            "stage108_phase1_completed",
            "stage108_phase2_completed",
            "phase3_started",
            "phase3_completed",
        ):
            with self.subTest(field=field):
    ...
    def test_phase2_replay_and_scenario_contract_have_exact_shape(self) -> None:
        ...
        self.assertEqual("ids.stage108.report_snapshot.phase2.v1", replay["phase2_schema_version"])
        self.assertEqual("CONTROL_ONLY_IN_MEMORY_REPORT_SNAPSHOT", replay["phase2_record_kind"])
        ...
        self.assertEqual(195, scenarios["scenario_field_check_count"])
    ...
    def test_report_projects_five_closed_control_scenarios(self) -> None:
        ...
        self.assertEqual("IDS-STAGE108-P3-GATE", report["current_gate"])
        self.assertEqual("IDS-STAGE108-P4-GATE", report["next_gate"])
        for field in (
        ...
        self.assertEqual(82, report["phase2_projection_field_count_per_request"])
        self.assertEqual(410, report["phase2_projection_field_count_total"])
        self.assertEqual(5, report["scenario_count"])
        self.assertEqual(39, report["scenario_field_count"])
        self.assertEqual(195, report["scenario_field_check_count"])
        ...
        for scenario in report["scenario_results"]:
            with self.subTest(scenario=scenario["scenario_id"]):
                ...
                for field in (
                    ...
                    self.assertTrue(
                        scenario[field].startswith(":control:stage108-p2:"), field
                    )
                for field in (
    ...
    def test_taskpack_special_cases_and_human_handlings_are_preserved(self) -> None:
        ...
        for field in (
            ...
            with self.subTest(field=field):
                self.assertTrue(external[field])
    ...
    def test_projection_drift_fails_closed(self) -> None:
        shape_mismatch = self.module.build_report_snapshot_phase3_report(
        ...
        def break_binding(result):
            result["report_evidence_binding_and_section_control_projections"][0][
                "evidence_id_ref"
            ] = ":control:stage108-p2:unexpected-evidence-id:reference-only"

        binding_drift = self.module.build_report_snapshot_phase3_report(
        ...
        lifecycle_drift = self.module.build_report_snapshot_phase3_report(
        ...
        external_drift = self.module.build_report_snapshot_phase3_report(
        ...
        runtime_drift = self.module.build_report_snapshot_phase3_report(
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        if current != phase3_current:
            self.assertTrue(is_current_projection or current in {phase1_current, phase2_current})
            return
        ...
        self.assertIn("IDS-STAGE108-P4-GATE", plan["stop_condition"])
        acceptance = json.loads(ACCEPTANCE.read_text(encoding="utf-8"))
        acceptance_by_id = {item["id"]: item["status"] for item in acceptance["items"]}
        self.assertEqual("P3 报告快照专项异常场景已完成", acceptance_by_id["ACC-STAGE-108"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE108-P3-04"])

        receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
        self.assertEqual(self.module.PASS_RESULT, receipt["result"])
        self.assertEqual("IDS-STAGE108-P4-GATE", receipt["next_gate"])
        self.assertEqual(5, receipt["controlled_scenarios"]["scenario_count"])
        self.assertEqual(195, receipt["controlled_scenarios"]["scenario_field_check_count"])
        ...
        self.assertEqual(23, validation["stage108_phase1_to_phase3_compatibility_test_count"])
        self.assertEqual(952, validation["historical_whitebox_chain_test_count"])
        for field in (
        ...
        self.assertIn("EVT-IDS-V0_1-STAGE108-P3-20260826-001", event_ids)
