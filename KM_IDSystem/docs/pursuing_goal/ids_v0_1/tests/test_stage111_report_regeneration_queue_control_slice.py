"""Stage111 报告重新生成队列 Phase 2 纯内存控制切片的聚焦验证。"""
...
SCOPE = BASE / "STAGE111_PHASE2_REPORT_REGENERATION_QUEUE_CONTROL_SLICE.md"
CONTRACT = (
    BASE
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage111-p2-local.json"
...
class Stage111ReportRegenerationQueuePhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.result = cls.module.execute_report_regeneration_queue_control_slice(
            cls.control_input
        )
    ...
    def test_identity_authority_predecessors_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage111.report_regeneration_queue.phase2.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-111", contract["stage"])
        self.assertEqual("IDS-STAGE111-P2", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE111-P2", contract["task_id"])
        self.assertEqual("ACC-STAGE-111", contract["acceptance_id"])
        self.assertEqual(
            "REPORT_REGENERATION_QUEUE_CONTROL_SLICE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE111-P2-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE111-P3-GATE", contract["next_gate"])
        authority = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE111_TASKPACK_STAGE111_PHASE1_AND_STAGE110_REVIEW_CONTROL_"
            "ARTIFACTS_ONLY",
            authority["authority"],
        )
        for field in (
        ...
        self.assertTrue(predecessor["stage111_phase1_required"])
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            predecessor["stage111_phase1_result"],
        )
    ...
    def test_fixed_control_input_preserves_phase1_queue_shape(self) -> None:
        ...
        self.assertEqual(30, controls["phase1_control_reference_field_count"])
    ...
    def test_report_snapshot_queue_quality_and_audit_controls_remain_reference_only(self) -> None:
        ...
        queue_records = self.result["report_impact_queue_and_audit_control_projections"]
        for binding, snapshot, queue in zip(
            binding_records, snapshot_records, queue_records
        ):
            with self.subTest(scenario=binding["control_scenario"]):
                ...
                for field in (
                    ...
                    with self.subTest(field=field):
                        self.assertTrue(
                            binding[field].startswith(self.module.CONTROL_PREFIX)
                        )
                self.assertEqual(
                    "CONTROL_REPORT_FIELDS_REFERENCE_ONLY_NOT_WRITTEN",
                    binding["report_write_field_control_state"],
                )
                self.assertEqual(
                ...
                self.assertTrue(
                    queue["report_export_audit_control_label"].startswith(
                ...
                for field in (
                    ...
                    with self.subTest(field=field):
                        self.assertIn("REFERENCE_ONLY", queue[field])
                for record in (binding, snapshot, queue):
    ...
    def test_input_drift_fails_closed_without_projection_or_runtime(self) -> None:
        ...
        ] = ":control:stage111-p2:unexpected-queue-state:reference-only"
        rejected = self.module.execute_report_regeneration_queue_control_slice(
            invalid_input
        )
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        self.assertTrue(is_current_projection or current == phase2_current)
        ...
        self.assertEqual(
            "PASS_REPORT_REGENERATION_QUEUE_CONTROL_SLICE_RUNTIME_DISABLED",
            receipt["result"],
        )
        self.assertEqual("IDS-STAGE111-P2-GATE", receipt["entry_gate"])
        self.assertEqual("IDS-STAGE111-P3-GATE", receipt["next_gate"])
        self.assertEqual(32, receipt["control_shape"]["control_input_field_count"])
        self.assertEqual(
            88, receipt["control_shape"]["projection_field_count_per_request"]
        )
        self.assertEqual(440, receipt["control_shape"]["projection_field_count_total"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE111-P2-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE111-P2-20260827-001", event_ids)
