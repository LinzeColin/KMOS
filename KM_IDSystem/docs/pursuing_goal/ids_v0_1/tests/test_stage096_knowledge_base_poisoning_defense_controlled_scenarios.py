...
SCOPE = BASE / "STAGE096_PHASE3_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
P2_SCOPE = BASE / "STAGE096_PHASE2_KNOWLEDGE_BASE_POISONING_DEFENSE_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
...
class Stage096KnowledgeBasePoisoningDefensePhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage096_phase3_scenarios", MODULE)
        cls.phase2 = load_module("stage096_phase2_slice", P2_MODULE)
        cls.report = cls.module.build_knowledge_base_poisoning_defense_phase3_report()

    def _phase2_report(self):
        return self.phase2.execute_knowledge_base_poisoning_defense_control_slice(
    ...
    def test_artifacts_phase_identity_and_frozen_taskpack_exist(self):
        ...
        self.assertEqual(
            "ids.stage096.knowledge_base_poisoning_defense.phase3.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("STAGE-096", self.contract["stage"])
        self.assertEqual("IDS-STAGE096-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE096-P3", self.contract["task_id"])
        self.assertEqual("IDS-STAGE096-P4-GATE", self.contract["next_gate"])

    def test_single_authority_replay_shape_and_phase_boundary_are_explicit(self):
        source = self.contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE096_TASKPACK_AND_STAGE096_PHASE1_PHASE2_STAGE095_REVIEWED_CONTROL_ARTIFACTS_ONLY",
            source["authority"],
        )
        allowed = {
            "authority",
            "frozen_taskpack_ref",
            "stage095_review_ref",
            "stage095_review_contract_ref",
            "stage095_review_receipt_ref",
            "stage096_phase1_scope_ref",
            "stage096_phase1_contract_ref",
            "stage096_phase1_receipt_ref",
            "stage096_phase2_scope_ref",
            "stage096_phase2_contract_ref",
            "stage096_phase2_receipt_ref",
        }
    ...
    def test_exception_semantics_and_taskpack_coverage_are_exact(self):
        ...
        no_internal = scenarios["no_internal_evidence_poisoning_defense_control"]
        ...
        self.assertEqual(
            ...
            scenarios["low_ocr_poisoning_defense_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        self.assertEqual(
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED",
            scenarios["old_version_poisoning_defense_degradation_control"][
                "evidence_disposition_state"
            ],
        )
    ...
    def test_contract_failure_and_protected_boundaries_are_explicit(self):
        ...
        self.assertTrue(rollback["preserve_stage096_phase1_and_phase2"])
        self.assertTrue(rollback["preserve_stage095_reviewed_artifacts"])

    def test_invalid_phase2_output_returns_controlled_failure(self):
        failed = self.module.build_knowledge_base_poisoning_defense_phase3_report(
            lambda _: {}
        )
    ...
    def test_nonopaque_reference_and_semantic_drift_return_specific_failures(self):
        def nonopaque_reference(_control_input):
            altered = copy.deepcopy(self._phase2_report())
            altered["knowledge_base_poisoning_defense_schema_binding_control_projections"][0][
                "evidence_gap_ref"
            ] = "unscoped-reference"
            return altered
        ...
        def masquerade_drift(_control_input):
            ...
            altered["critical_conclusion_and_report_impact_control_projections"][1][
                "evidence_grade_ref"
            ] = ":control:stage096-p2:evidence-grade-A:reference-only"
            return altered
    ...
    def test_predecessor_p2_record_remains_immutable_before_or_after_p3(self):
        ...
        if current == phase3_current:
            ...
            self.assertEqual("P3 受控场景已完成", acceptance_by_id["ACC-STAGE-096"])
            ...
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE096-P3-04"])
            self.assertIn("EVT-IDS-V0_1-STAGE096-P3-20260824-001", event_ids)
            self.assertEqual("IDS-STAGE096-P4-GATE", receipt["next_gate"])
        ...
        if current in (review_current, stage097_phase1_current):
