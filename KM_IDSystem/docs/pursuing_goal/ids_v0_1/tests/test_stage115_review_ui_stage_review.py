...
SCOPE = BASE / "STAGE115_STAGE_REVIEW.md"
CONTRACT = (
    BASE / "index_version_schema" / "stage115_review_ui_stage_review_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage115_review_ui_stage_review.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
PHASE2_MODULE = BASE / "index_version_schema" / "stage115_review_ui_control_slice.py"
...
PHASE3_MODULE = (
    BASE / "index_version_schema" / "stage115_review_ui_controlled_scenarios.py"
)
PHASE3_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage115-p3-local.json"
PHASE4_SCOPE = BASE / "STAGE115_PHASE4_REVIEW_UI_DELIVERY.md"
PHASE4_CONTRACT = (
    BASE / "index_version_schema" / "stage115_review_ui_delivery_contract.json"
)
PHASE4_MODULE = BASE / "index_version_schema" / "stage115_review_ui_delivery.py"
PHASE4_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage115-p4-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage115-review-local.json"
...
class Stage115ReviewUiStageReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        prefix = "KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema."
        cls.module = importlib.import_module(f"{prefix}stage115_review_ui_stage_review")
        cls.phase2 = importlib.import_module(f"{prefix}stage115_review_ui_control_slice")
        cls.phase3 = importlib.import_module(
            f"{prefix}stage115_review_ui_controlled_scenarios"
        )
        cls.phase4 = importlib.import_module(f"{prefix}stage115_review_ui_delivery")
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.phase1_contract = json.loads(PHASE1_CONTRACT.read_text(encoding="utf-8"))
        cls.phase2_report = cls.phase2.project_review_ui_control_slice(
            cls.phase2.build_control_input()
        )
        cls.phase3_report = cls.phase3.project_review_ui_controlled_scenarios(
            cls.phase3.build_controlled_scenario_input()
        )
        cls.phase4_report = cls.phase4.build_review_ui_phase4_delivery_report()
        cls.report = cls.module.build_review_ui_stage_review()
    ...
    def test_identity_reviewed_shape_failure_contract_and_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("IDS-STAGE115-REVIEW", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE115-REVIEW", contract["task_id"])
        ...
        self.assertEqual(
            "STAGE115_REVIEW_UI_STAGE_REVIEW_RUNTIME_DISABLED",
        ...
        self.assertTrue(
            failure[
                "unassigned_successor_requires_new_frozen_taskpack_and_independent_authorization"
            ]
        )
    ...
    def test_review_passes_with_exact_phase_results_shapes_and_zero_runtime(self) -> None:
        ...
        self.assertFalse(report["unassigned_successor_started"])
        self.assertEqual(
            self.module.REVIEWED_CONTROL_SHAPE,
        ...
        for scenario in scenarios:
            with self.subTest(scenario=scenario["controlled_scenario_id"]):
                for field in (
                    ...
                    self.assertTrue(scenario[field].startswith(":control:"))
    ...
    def test_each_phase_or_semantic_drift_fails_closed(self) -> None:
        ...
        phase1["review_ui_control_contract"]["future_control_reference_field_count"] = 18
        p1_failed = self.module.build_review_ui_stage_review(
            phase1_contract_provider=lambda: phase1
        )
        phase2 = copy.deepcopy(self.phase2_report)
        phase2["control_projection_field_total"] = 1
        p2_failed = self.module.build_review_ui_stage_review(
        ...
        opaque_failed = self.module.build_review_ui_stage_review(
        ...
        lifecycle_failed = self.module.build_review_ui_stage_review(
    ...
    def test_scope_contract_and_review_output_keep_runtime_surfaces_closed(self) -> None:
        ...
        for phrase in (
            ...
            "业务线白箱",
            "P4→P3",
            "UNASSIGNED_SUCCESSOR_GATE",
        ):
            with self.subTest(phrase=phrase):
                self.assertIn(phrase, scope_text)
        self.assertEqual(
            tuple(self.module.REVIEW_RUNTIME_FALSE_FIELDS),
            tuple(self.contract["runtime_boundary"]),
        )
        self.assertTrue(
    ...
    def test_current_governance_receipt_and_event_are_exact(self) -> None:
        ...
        future_projection = assert_legacy_or_current_projection(
            self, current, {review_current, *historical}, status, plan, ROADMAP
        )
        if future_projection or current in historical:
            return
        self.assertEqual(
        ...
        self.assertIn("UNASSIGNED_SUCCESSOR_GATE", plan["stop_condition"])
        acceptance = json.loads(ACCEPTANCE.read_text(encoding="utf-8"))
        acceptance_by_id = {item["id"]: item["status"] for item in acceptance["items"]}
        self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-115"])
        for acceptance_id in (
            "ACC-STAGE115-REVIEW-01",
            "ACC-STAGE115-REVIEW-02",
            "ACC-STAGE115-REVIEW-03",
        ):
            with self.subTest(acceptance_id=acceptance_id):
                self.assertEqual("已通过", acceptance_by_id[acceptance_id])
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE115-REVIEW-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE115-REVIEW-20260827-001", event_ids)
