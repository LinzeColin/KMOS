...
SCOPE = BASE / "STAGE110_STAGE_REVIEW.md"
CONTRACT = (
    BASE
...
MODULE = (
    BASE / "index_version_schema" / "stage110_report_quality_score_stage_review.py"
)
P1_CONTRACT = BASE / "index_version_schema" / "stage110_report_quality_score_contract.json"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
...
P4_CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage110_report_quality_score_delivery_contract.json"
)
P4_MODULE = BASE / "index_version_schema" / "stage110_report_quality_score_delivery.py"
P4_RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage110-p4-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage110-review-local.json"
...
class Stage110ReportQualityScoreStageReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_module(MODULE, "stage110_review_for_test")
        cls.phase3 = _load_module(P3_MODULE, "stage110_phase3_for_review_test")
        cls.phase4 = _load_module(P4_MODULE, "stage110_phase4_for_review_test")
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.report = cls.module.build_report_quality_score_stage_review()
        cls.phase4_report = cls.phase4.build_report_quality_score_phase4_delivery_report()
    ...
    def test_identity_reviewed_shape_failure_contract_and_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("IDS-STAGE110-REVIEW", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE110-REVIEW", contract["task_id"])
        self.assertEqual(self.module.REVIEW_GATE, contract["entry_gate"])
        self.assertEqual(self.module.NEXT_GATE, contract["next_gate"])
        self.assertEqual(
            "STAGE110_REPORT_QUALITY_SCORE_REVIEW_RUNTIME_DISABLED",
        ...
        self.assertTrue(failure["stage111_must_remain_not_started"])
    ...
    def test_review_passes_with_exact_phase_results_shapes_and_zero_runtime(self) -> None:
        ...
        for field in (
            ...
            "report_quality_semantics_preserved",
        ...
        self.assertFalse(report["stage111_started"])
        self.assertEqual(
            self.module.REVIEWED_CONTROL_SHAPE,
    ...
    def test_quality_semantics_and_whitebox_lifecycle_controls_are_preserved(self) -> None:
        scenarios = self.phase3.build_report_quality_score_phase3_report()[
            "scenario_results"
        ]
        ...
        for scenario in scenarios:
            with self.subTest(scenario=scenario["scenario_id"]):
                ...
                self.assertFalse(scenario["actual_report_quality_scored"])
        ...
        self.assertEqual(
            2,
            sum(item["business_line_whitebox_confirmation_required"] for item in templates),
        )
        self.assertTrue(
            all(
        ...
        quality_records = self.phase4_report["report_quality_score_control_records"]
        self.assertEqual(
            1,
            sum(item["quality_whitebox_confirmation_required"] for item in quality_records),
        )
    ...
    def test_tampered_phase_outputs_fail_closed_with_zero_runtime(self) -> None:
        ...
        malformed_p1["quality_metric_definition_contract"][
            "required_quality_metric_control_field_count"
        ] = 9
        malformed_p3 = copy.deepcopy(
            self.phase3.build_report_quality_score_phase3_report()
        )
        malformed_p3["scenario_results"][0][
            "evidence_gap_ref"
        ] = ":control:stage110-review:tampered-evidence-gap:reference-only"
        malformed_p4 = copy.deepcopy(self.phase4_report)
        malformed_p4["runtime_boundary"]["model_call_performed"] = True

        failures = (
            (
                self.module.build_report_quality_score_stage_review(
    ...
    def test_current_governance_receipt_and_event_are_exact(self) -> None:
        ...
        self.assertIn("IDS-STAGE111-P1-GATE", plan["stop_condition"])
        acceptance = json.loads(ACCEPTANCE.read_text(encoding="utf-8"))
        acceptance_by_id = {item["id"]: item["status"] for item in acceptance["items"]}
        self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-110"])
        for acceptance_id in (
            "ACC-STAGE110-REVIEW-01",
            "ACC-STAGE110-REVIEW-02",
            "ACC-STAGE110-REVIEW-03",
        ):
            with self.subTest(acceptance_id=acceptance_id):
                self.assertEqual("已通过", acceptance_by_id[acceptance_id])
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE110-REVIEW-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE110-REVIEW-20260826-001", event_ids)
