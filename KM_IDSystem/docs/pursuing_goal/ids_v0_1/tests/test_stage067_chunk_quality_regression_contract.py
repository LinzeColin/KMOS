...
SCOPE = BASE / "STAGE067_PHASE1_CHUNK_QUALITY_REGRESSION_SCOPE_BOUNDARY.md"
CONTRACT = (
    BASE
...
RUN = ROOT / "machine" / "runs" / "2026-08-14-stage067-p1-local.json"


class Stage067ChunkQualityRegressionPhase1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_authority_and_stage_ownership_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage067.chunk_quality_regression.phase1.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-067", contract["stage"])
        self.assertEqual("IDS-V0_1-STAGE067-P1", contract["task_id"])
        self.assertEqual("ACC-STAGE-067", contract["acceptance_id"])
        self.assertEqual(
            "PHASE1_CHUNK_QUALITY_REGRESSION_CONTRACT_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE067-P2-GATE", contract["next_gate"])
        source = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE067_TASKPACK_AND_STAGE066_REVIEW_ARTIFACTS_ONLY",
            source["authority"],
        )
        for field in (
        ...
        self.assertTrue(owners["predecessor_stage066_review_reused_as_reference_only"])
        self.assertFalse(owners["actual_quality_regression_created"])
    ...
    def test_reference_only_shape_and_future_output_remain_static(self):
        inputs = self.contract["reference_only_quality_regression_input_contract"]
        ...
        output = self.contract["future_quality_regression_output_contract"]
    ...
    def test_quality_protected_semantics_traceability_and_duplicate_boundary_close(self):
        quality = self.contract["quality_regression_definition_contract"]
        self.assertEqual(
            ["ENGINEERING_PROCEDURE_STEP", "ACCEPTANCE_CLAUSE", "PARAMETER_TABLE"],
            quality["protected_semantic_asset_types"],
        )
        self.assertEqual(3, quality["protected_semantic_asset_type_count"])
        self.assertTrue(quality["quality_regression_is_future_control_only"])
        self.assertFalse(quality["quality_score_or_threshold_assigned"])
        self.assertFalse(quality["actual_quality_measurement_performed"])
        self.assertFalse(quality["actual_quality_regression_performed"])
        self.assertFalse(quality["actual_quality_degradation_performed"])
        self.assertTrue(quality["quality_failure_must_require_human_review"])
        self.assertEqual("STAGE-068", quality["quality_degradation_owner"])
        ...
        for field in (
            ...
            "quality_regression_can_override_source_document",
    ...
    def test_authority_failure_closure_and_rollback_are_declared(self):
        ...
        for field in (
            "quality_regression_can_replace_source_document",
            "quality_regression_can_become_business_fact_authority",
        ...
        self.assertEqual(15, failures["failure_state_count"])
        self.assertIn(
            "CHUNK_QUALITY_REGRESSION_REQUIRES_AUTHORIZED_EXECUTION",
            failures["declared_failure_states"],
        )
        self.assertIn(
            "DUPLICATE_CHUNK_CONTROL_REFERENCE_MISSING",
            failures["declared_failure_states"],
        )
        self.assertIn(
            "TRACEABILITY_UNVERIFIED_BLOCKS_QUALITY_REGRESSION",
            failures["declared_failure_states"],
        )
        ...
        self.assertEqual(
            "STAGE066_REVIEWED_LOCAL_CHUNK_COVERAGE_METRICS_RUNTIME_DISABLED",
            rollback["return_to"],
        )
        for field in (
    ...
    def test_chinese_feedback_and_runtime_boundary_are_explicit(self):
        ...
        self.assertTrue(runtime["predecessor_stage066_review_reused_as_reference_only"])
        self.assertTrue(runtime["stage067_started"])
        self.assertTrue(runtime["stage067_entry_authorized"])
...
("IDS-V0_1-STAGE071-P4", "IDS-V0_1-STAGE071-P4"),
                ...
                ("IDS-STAGE083-P1", "IDS-V0_1-STAGE083-P1"), ("IDS-STAGE083-P2", "IDS-V0_1-STAGE083-P2"), ("IDS-STAGE083-P3", "IDS-V0_1-STAGE083-P3"), ("IDS-STAGE083-P4", "IDS-V0_1-STAGE083-P4"), ("IDS-STAGE083-REVIEW", "IDS-V0_1-STAGE083-REVIEW"), ("IDS-STAGE084-P1", "IDS-V0_1-STAGE084-P1"), ('IDS-STAGE084-P2', 'IDS-V0_1-STAGE084-P2')),
                ("IDS-V0_1-STAGE073-P1", "IDS-V0_1-STAGE073-P1"),
        )
        self.assertTrue(
            (
("IDS-STAGE067-P2-GATE" in plan["stop_condition"]
            or "IDS-STAGE067-P3-GATE" in plan["stop_condition"]
        ...
        self.assertEqual("RUN-IDS-STAGE067-P1-LOCAL-20260814-001", run["run_id"])
        self.assertEqual("IDS-V0_1-STAGE067-P1", run["task_id"])
        self.assertEqual("IDS-STAGE067-P2-GATE", run["next_gate"])
        self.assertTrue(run["result"].startswith("PASS_LOCAL_"))
        self.assertFalse(run["observed_work"]["ovh_deployment_performed"])
        self.assertIn("stage067_phase1", BATCH.read_text(encoding="utf-8"))
        self.assertIn("IDS-V0_1-STAGE067-P1", ROADMAP.read_text(encoding="utf-8"))
        self.assertTrue(
            any(
                item.get("event_id") == "EVT-IDS-V0_1-STAGE067-P1-20260814-001"
                for item in events
            )
        )
