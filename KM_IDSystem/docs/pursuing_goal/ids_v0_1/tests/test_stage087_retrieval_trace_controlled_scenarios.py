...
SCOPE = BASE / "STAGE087_PHASE3_RETRIEVAL_TRACE_CONTROLLED_SCENARIOS.md"
CONTRACT = (
...
def _load_module():
    spec = importlib.util.spec_from_file_location("stage087_p3", MODULE)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage087 P3 controlled scenarios")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage087RetrievalTracePhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _report(self):
        return self.module.build_retrieval_trace_phase3_report()
    ...
    def test_control_artifacts_phase_identity_and_gate_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage087.retrieval_trace.phase3.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("STAGE-087", self.contract["stage"])
        self.assertEqual("IDS-STAGE087-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE087-P3", self.contract["task_id"])
        self.assertEqual(
            "PHASE3_RETRIEVAL_TRACE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
            self.contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE087-P4-GATE", self.contract["next_gate"])
    ...
    def test_contract_preserves_authority_replay_and_runtime_boundaries(self):
        ...
        self.assertEqual(426, replay["expected_phase2_field_check_count"])
        for field in (
    ...
    def test_report_replays_p2_and_preserves_all_control_shapes(self):
        ...
        self.assertEqual(426, report["phase2_control_record_field_check_count"])
    ...
    def test_invalid_or_side_effecting_phase2_replay_is_fail_closed(self):
        def missing_trace_chain(control_input):
            result = self._phase2().execute_retrieval_trace_control_slice(control_input)
            altered = copy.deepcopy(result)
            altered["all_trace_active_index_versions_match"] = False
            return altered

        failed = self.module.build_retrieval_trace_phase3_report(missing_trace_chain)
        ...
        failed = self.module.build_retrieval_trace_phase3_report(runtime_signal)
        self.assertFalse(failed["valid"])
    ...
    def test_report_is_zero_runtime_and_stops_before_phase4(self):
        ...
        self.assertTrue(report["stage086_review_evidence_declared"])
