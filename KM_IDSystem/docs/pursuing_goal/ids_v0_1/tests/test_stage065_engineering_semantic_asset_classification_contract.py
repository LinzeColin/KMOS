...
SCOPE = BASE / "STAGE065_PHASE1_ENGINEERING_SEMANTIC_ASSET_CLASSIFICATION_SCOPE_BOUNDARY.md"
CONTRACT = (
    BASE
...
RUN = ROOT / "machine" / "runs" / "2026-08-14-stage065-p1-local.json"


class Stage065EngineeringSemanticAssetClassificationPhase1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_authority_and_stage_ownership_are_explicit(self):
        ...
        self.assertEqual("STAGE-065", contract["stage"])
        self.assertEqual("IDS-V0_1-STAGE065-P1", contract["task_id"])
        self.assertEqual("ACC-STAGE-065", contract["acceptance_id"])
        self.assertEqual(
            "PHASE1_ENGINEERING_SEMANTIC_ASSET_CLASSIFICATION_CONTRACT_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE065-P2-GATE", contract["next_gate"])
        source = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE065_TASKPACK_AND_STAGE064_REVIEW_ARTIFACTS_ONLY",
            source["authority"],
        )
        for field in (
        ...
        self.assertFalse(owners["actual_semantic_asset_classification_created"])
        self.assertFalse(owners["cross_stage_owner_override_allowed"])

    def test_reference_only_shape_and_asset_catalog_remain_static(self):
        inputs = self.contract[
            "reference_only_semantic_asset_classification_input_contract"
        ]
        ...
        output = self.contract["future_semantic_asset_classification_output_contract"]
        self.assertEqual(16, output["field_count"])
        self.assertEqual(
            ["semantic_asset_type", "semantic_asset_subtype", "classification_status"],
            [
                field
                for field in output["required_fields"]
                if field
                in {"semantic_asset_type", "semantic_asset_subtype", "classification_status"}
            ],
        )
        ...
        for field in (
            ...
            "actual_semantic_asset_subtype_assigned",
            "actual_chunk_identity_or_version_bound",
        ):
            with self.subTest(field=field):
                self.assertFalse(output[field])

        catalog = self.contract["engineering_semantic_asset_type_catalog"]
        self.assertEqual(7, catalog["asset_type_count"])
        self.assertEqual(
            [
                "procedure",
                "risk",
            ...
            [item["asset_type"] for item in catalog["asset_types"]],
        )
        self.assertEqual(0, catalog["actual_asset_classification_count"])
        self.assertFalse(catalog["catalog_can_become_business_fact_authority"])
        self.assertTrue(
            all(not item["classification_performed"] for item in catalog["asset_types"])
        )
    ...
    def test_protected_semantics_traceability_and_future_interfaces_remain_closed(self):
        ...
        self.assertFalse(
            protected["semantic_asset_classification_can_override_protected_boundary"]
        )
    ...
    def test_authority_failure_closure_and_rollback_are_declared(self):
        ...
        for field in (
            "semantic_asset_classification_can_replace_source_document",
            "semantic_asset_classification_can_become_business_fact_authority",
        ...
        self.assertIn(
            "SEMANTIC_ASSET_CLASSIFICATION_REQUIRES_AUTHORIZED_EXECUTION",
        ...
        self.assertTrue(failures["unverified_traceability_blocks_semantic_asset_output"])
        self.assertFalse(failures["automatic_business_write_allowed"])

        rollback = self.contract["rollback_contract"]
        self.assertEqual(
            "STAGE064_REVIEWED_LOCAL_CHUNK_IDENTITY_AND_VERSION_RUNTIME_DISABLED",
            rollback["return_to"],
        )
        for field in (
    ...
    def test_chinese_feedback_and_runtime_boundary_are_explicit(self):
        ...
        self.assertTrue(runtime["predecessor_stage064_review_reused_as_reference_only"])
        self.assertTrue(runtime["stage065_started"])
        self.assertTrue(runtime["stage065_entry_authorized"])
    ...
    def test_governance_records_phase1_and_preserves_local_only_boundary(self):
        ...
        event = next(
            item
            for item in events
            if item.get("event_id") == "EVT-IDS-V0_1-STAGE065-P1-20260814-001"
        )
...
("IDS-V0_1-STAGE071-P3", "IDS-V0_1-STAGE071-P3", "IDS-STAGE071-P4-GATE"),
                ...
                ("IDS-STAGE082-REVIEW", "IDS-V0_1-STAGE082-REVIEW", "IDS-STAGE083-P1-GATE"), ("IDS-STAGE083-P1", "IDS-V0_1-STAGE083-P1", "IDS-STAGE083-P2-GATE"), ("IDS-STAGE083-P2", "IDS-V0_1-STAGE083-P2", "IDS-STAGE083-P3-GATE"), ("IDS-STAGE083-P3", "IDS-V0_1-STAGE083-P3", "IDS-STAGE083-P4-GATE"), ("IDS-STAGE083-P4", "IDS-V0_1-STAGE083-P4", "IDS-STAGE083-REVIEW-GATE"), ("IDS-STAGE083-REVIEW", "IDS-V0_1-STAGE083-REVIEW", "IDS-STAGE084-P1-GATE"), ("IDS-STAGE084-P1", "IDS-V0_1-STAGE084-P1", "IDS-STAGE084-P2-GATE"), ("IDS-STAGE084-P2", "IDS-V0_1-STAGE084-P2", "IDS-STAGE084-P3-GATE"), ('IDS-STAGE084-P3', 'IDS-V0_1-STAGE084-P3', 'IDS-STAGE084-P4-GATE')),
        )
...
("IDS-V0_1-STAGE071-P4", "IDS-V0_1-STAGE071-P4"),
                ...
                ("IDS-STAGE083-P1", "IDS-V0_1-STAGE083-P1"), ("IDS-STAGE083-P2", "IDS-V0_1-STAGE083-P2"), ("IDS-STAGE083-P3", "IDS-V0_1-STAGE083-P3"), ("IDS-STAGE083-P4", "IDS-V0_1-STAGE083-P4"), ("IDS-STAGE083-REVIEW", "IDS-V0_1-STAGE083-REVIEW"), ("IDS-STAGE084-P1", "IDS-V0_1-STAGE084-P1"), ('IDS-STAGE084-P2', 'IDS-V0_1-STAGE084-P2'), ('IDS-STAGE084-P3', 'IDS-V0_1-STAGE084-P3')),
                ("IDS-V0_1-STAGE073-P1", "IDS-V0_1-STAGE073-P1"),
        )
        self.assertTrue(
            (
("IDS-STAGE065-P2-GATE" in plan["stop_condition"]
        ...
        self.assertTrue(
            (
                'current_phase_id: "IDS-STAGE065-P1"' in roadmap_text
                and 'next_gate_id: "IDS-STAGE065-P2-GATE"' in roadmap_text
            )
            or (
        ...
        self.assertEqual("IDS-V0_1-STAGE065-P1", event["task_id"])
        self.assertEqual(["ACC-STAGE-065"], event["acceptance_ids"])
        self.assertEqual("IDS-V0_1-STAGE065-P1", run["task_id"])
        self.assertEqual("IDS-STAGE065-P2-GATE", run["next_gate"])
