...
class Stage063ChapterAwareChunkingStageReviewTests(unittest.TestCase):
    ...
    def test_governance_projection_records_stage_review(self):
        ...
        self.assertEqual("IDS-V0_1-STAGE063-REVIEW", status["phase"])
        self.assertEqual("IDS-V0_1-STAGE063-REVIEW", status["task"])
        self.assertEqual("IDS-STAGE064-P1-GATE", status["next_gate"])
        self.assertEqual("IDS-V0_1-STAGE063-REVIEW", plan["task"])
        self.assertIn("IDS-STAGE064-P1-GATE", plan["stop_condition"])
        self.assertTrue(
