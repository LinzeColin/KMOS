...
REVIEW_SOURCE = BASE / "stage103_model_output_permission_gate_stage_review.py"
REVIEW_CONTRACT = BASE / "stage103_model_output_permission_gate_stage_review_contract.json"
P1_CONTRACT = BASE / "stage103_model_output_permission_gate_contract.json"
P2_SOURCE = BASE / "stage103_model_output_permission_gate_control_slice.py"
P3_SOURCE = BASE / "stage103_model_output_permission_gate_controlled_scenarios.py"
P4_SOURCE = BASE / "stage103_model_output_permission_gate_delivery.py"
SCOPE_DOCUMENT = BASE.parent / "STAGE103_STAGE_REVIEW.md"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
REVIEW_RUN = ROOT / "machine" / "runs" / "2026-08-25-stage103-review-local.json"
...
class Stage103ModelOutputPermissionGateReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.review = _load_module("stage103_review_test_module", REVIEW_SOURCE)
    ...
    def test_required_artifacts_and_identity_exist(self) -> None:
        ...
        self.assertEqual(
            ...
            "ids.stage103.model_output_permission_gate.stage_review.v1",
        )
        self.assertEqual(
            self.review.RECORD_KIND,
            "CONTROL_ONLY_IN_MEMORY_MODEL_OUTPUT_PERMISSION_GATE_STAGE_REVIEW",
        )
        self.assertEqual(self.review.REVIEW_GATE, "IDS-STAGE103-REVIEW-GATE")
        self.assertEqual(self.review.NEXT_GATE, "IDS-STAGE104-P1-GATE")
    ...
    def test_contract_keeps_authority_shape_and_runtime_closed(self) -> None:
        ...
        self.assertEqual(contract["phase"], "IDS-STAGE103-REVIEW")
        self.assertEqual(contract["task_id"], "IDS-V0_1-STAGE103-REVIEW")
        ...
        self.assertEqual(reviewed["phase1_static_shape"], "13/5/4/5/24/4")
        self.assertEqual(reviewed["phase2_control_field_check_count"], 230)
        ...
        self.assertTrue(boundary["stage103_review_started"])
        self.assertFalse(boundary["stage104_started"])
    ...
    def test_default_report_is_valid_and_replays_fixed_phases(self) -> None:
        report = self.review.build_model_output_permission_gate_stage103_review_report()
    ...
    def test_fixed_review_shape_and_p4_to_p3_rollback_are_preserved(self) -> None:
        ...
        self.assertEqual(shape["phase2_control_request_count"], 5)
        self.assertEqual(shape["phase2_projection_field_count_per_request"], 46)
    ...
    def test_runtime_and_next_stage_boundaries_are_closed(self) -> None:
        ...
        self.assertFalse(stage["stage104_started"])
        ...
        self.assertTrue(failure["stage104_must_remain_not_started"])
    ...
    def test_phase1_or_phase2_drift_fails_closed(self) -> None:
        ...
        phase1["output_permission_contract"]["output_classification_count"] = 4
        phase1_report = self.review.build_model_output_permission_gate_stage103_review_report(
        ...
        phase2["control_projection_field_total"] = 229
        phase2_report = self.review.build_model_output_permission_gate_stage103_review_report(
    ...
    def test_phase3_or_phase4_drift_fails_closed(self) -> None:
        ...
        phase3_report = self.review.build_model_output_permission_gate_stage103_review_report(
        ...
        phase4_report = self.review.build_model_output_permission_gate_stage103_review_report(
    ...
    def test_report_is_detached_from_future_mutation(self) -> None:
        ...
        fresh = self.review.build_model_output_permission_gate_stage103_review_report()
    ...
    def test_scope_receipt_and_current_governance_projection_are_explicit(self) -> None:
        ...
        self.assertTrue(failures["stage104_must_remain_not_started"])
        ...
        if current == review_current:
            ...
            for acceptance_id in (
                "ACC-STAGE103-REVIEW-01",
                "ACC-STAGE103-REVIEW-02",
                "ACC-STAGE103-REVIEW-03",
            ):
                with self.subTest(acceptance_id=acceptance_id):
                    self.assertEqual("已通过", acceptance_by_id[acceptance_id])
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE103-REVIEW-04"])
            event_ids = {
                json.loads(line)["event_id"]
                for line in EVENTS.read_text(encoding="utf-8").splitlines()
                if line.strip()
            }
            self.assertIn("EVT-IDS-V0_1-STAGE103-REVIEW-20260825-001", event_ids)
