"""Stage108 报告快照 Phase 2 纯内存受控最小切片的聚焦验证。"""
...
SCOPE = BASE / "STAGE108_PHASE2_REPORT_SNAPSHOT_CONTROL_SLICE.md"
CONTRACT = BASE / "index_version_schema" / "stage108_report_snapshot_control_slice_contract.json"
MODULE = BASE / "index_version_schema" / "stage108_report_snapshot_control_slice.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage108-p2-local.json"
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "stage108_report_snapshot_control_slice", MODULE
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 Stage108 P2 报告快照受控最小切片")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage108ReportSnapshotPhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.result = cls.module.execute_report_snapshot_control_slice(cls.control_input)
    ...
    def test_identity_authority_predecessors_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("ids.stage108.report_snapshot.phase2.v1", contract["schema_version"])
        self.assertEqual("STAGE-108", contract["stage"])
        self.assertEqual("IDS-STAGE108-P2", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE108-P2", contract["task_id"])
        self.assertEqual("ACC-STAGE-108", contract["acceptance_id"])
        self.assertEqual(
            "REPORT_SNAPSHOT_CONTROL_SLICE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE108-P2-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE108-P3-GATE", contract["next_gate"])
        authority = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE108_TASKPACK_STAGE108_PHASE1_AND_STAGE107_REVIEWED_HUMAN_CONFIRMATION_ITEMS_CONTROL_ARTIFACTS_ONLY",
            authority["authority"],
        )
        for field in (
        ...
        self.assertTrue(predecessor["stage108_phase1_required"])
        self.assertEqual(
    ...
    def test_fixed_control_input_preserves_phase1_report_snapshot_shape(self) -> None:
        ...
        self.assertEqual(26, controls["control_input_field_count"])
        self.assertEqual(
            list(self.module.PHASE1_CONTROL_REFERENCE_FIELDS),
            controls["phase1_control_reference_fields"],
        )
        self.assertEqual(
            self.phase1_contract["report_snapshot_contract"][
            ...
            with self.subTest(scenario=request["control_scenario"]):
                ...
                for field in self.module.PHASE1_CONTROL_REFERENCE_FIELDS:
                    ...
                    if value is not None:
                        self.assertTrue(value.startswith(":control:stage108-p2:"), field)
    ...
    def test_accepted_control_slice_projects_exact_shape(self) -> None:
        ...
        self.assertEqual(82, result["control_projection_field_total_per_request"])
        self.assertEqual(410, result["control_projection_field_total"])
        self.assertEqual(5, controls["control_request_count"])
        self.assertEqual(4, controls["projection_group_count"])
        self.assertEqual(82, controls["projection_field_total_per_request"])
        self.assertEqual(410, controls["projection_field_total"])
    ...
    def test_report_binding_snapshot_and_lifecycle_controls_remain_reference_only(self) -> None:
    ...
    def test_input_drift_fails_closed_without_projection_or_runtime(self) -> None:
        ...
        invalid_input[self.module.CONTROL_FIELDS[0]][0]["evidence_gap_ref"] = (
            ":control:stage108-p2:unexpected-gap:reference-only"
        )
        rejected = self.module.execute_report_snapshot_control_slice(invalid_input)
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        if current != phase2_current:
            return
        self.assertEqual("REPORT_SNAPSHOT_CONTROL_SLICE_RUNTIME_DISABLED", status["evidence_status"])
        self.assertIn("IDS-STAGE108-P3-GATE", plan["stop_condition"])
        receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
        self.assertEqual(
            "PASS_IN_MEMORY_REPORT_SNAPSHOT_CONTROL_SLICE_RUNTIME_DISABLED",
            receipt["result"],
        )
        self.assertEqual("IDS-STAGE108-P3-GATE", receipt["next_gate"])
        self.assertTrue(all(value == 0 for value in receipt["runtime_counts"].values()))
        self.assertTrue(all(value is False for value in receipt["runtime_flags"].values()))
        final_validation = receipt["validation"]["final_validation"]
        self.assertEqual(8, final_validation["focused_test_count"])
        self.assertEqual(15, final_validation["stage108_phase1_to_phase2_compatibility_test_count"])
        self.assertEqual(944, final_validation["historical_whitebox_chain_test_count"])
        for field in (
        ...
        self.assertEqual("P2 报告快照受控最小切片已完成", acceptance_by_id["ACC-STAGE-108"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE108-P2-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE108-P2-20260826-001", event_ids)
