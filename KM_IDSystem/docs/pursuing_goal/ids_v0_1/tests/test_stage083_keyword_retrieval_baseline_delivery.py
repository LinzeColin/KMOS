...
SCOPE = BASE / "STAGE083_PHASE4_KEYWORD_RETRIEVAL_BASELINE_DELIVERY_EVIDENCE.md"
CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage083_keyword_retrieval_baseline_delivery_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage083_keyword_retrieval_baseline_delivery.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
PHASE1_SCOPE = BASE / "STAGE083_PHASE1_KEYWORD_RETRIEVAL_BASELINE_SCOPE_BOUNDARY.md"
PHASE1_CONTRACT = BASE / "index_version_schema" / "stage083_keyword_retrieval_baseline_contract.json"
PHASE2_SCOPE = BASE / "STAGE083_PHASE2_KEYWORD_RETRIEVAL_BASELINE_CONTROL_SLICE.md"
PHASE2_CONTRACT = (
    BASE
    / "index_version_schema"
...
PHASE3_SCOPE = BASE / "STAGE083_PHASE3_KEYWORD_RETRIEVAL_BASELINE_CONTROLLED_SCENARIOS.md"
PHASE3_CONTRACT = (
    BASE
    / "index_version_schema"
...
PREDECESSOR_REVIEW = BASE / "STAGE082_STAGE_REVIEW.md"
...
class Stage083KeywordRetrievalPhase4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = _load_module(MODULE, "stage083_p4")
        cls.phase3 = _load_module(PHASE3_MODULE, "stage083_p3_for_p4_test")

    def report(self):
        return self.module.build_keyword_retrieval_phase4_delivery_report()
    ...
    def test_control_artifacts_and_phase_identity_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage083.keyword_retrieval_baseline.phase4.delivery.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("IDS-STAGE083-P4", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE083-P4", self.contract["task_id"])
        self.assertEqual("IDS-STAGE083-P4-GATE", self.contract["entry_gate"])
        self.assertEqual("IDS-STAGE083-REVIEW-GATE", self.contract["next_gate"])
    ...
    def test_contract_declares_metadata_only_control_delivery(self):
        ...
        self.assertEqual((5, 250, 8, 26, 208), tuple(
            replay[field]
            for field in (
        ...
        self.assertEqual((8, 10, 8, 10, 8, 10, 8, 11, 8, 10, 3, 8, 432, 4), tuple(
            delivery[field]
            for field in (
    ...
    def test_predecessor_replay_preserves_exact_control_shapes(self):
        report = self.phase3.build_keyword_retrieval_phase3_report()
        self.assertTrue(report["valid"], report)
        self.assertEqual(
            (self.phase3.PASS_RESULT, self.module.ENTRY_GATE),
            (report["result"], report["next_gate"]),
        )
        self.assertEqual((250, 8, 26, 208, 8), tuple(
            report[field]
            for field in (
    ...
    def test_delivery_reuses_phase3_without_runtime_writes(self):
        ...
        self.assertEqual((8, 10, 8, 10, 8, 10, 8, 11, 8, 10, 3, 8, 432), tuple(
            report[field]
            for field in (
                "retrieval_sample_control_record_count",
                "retrieval_sample_field_count",
    ...
    def test_retrieval_samples_and_trace_logs_are_control_only(self):
        report = self.report()
        for item in report["retrieval_sample_control_records"]:
            with self.subTest(sample=item["scenario_id"]):
                ...
                self.assertIn(self.module.CONTROL_PREFIX, item["retrieval_sample_ref"])
        for item in report["trace_log_control_records"]:
            with self.subTest(trace=item["scenario_id"]):
                ...
                self.assertTrue(item["human_handling_required"])
                self.assertIn(self.module.CONTROL_PREFIX, item["trace_log_ref"])
    ...
    def test_filter_results_and_validity_reports_are_control_only(self):
        ...
        for item in report["filter_result_control_records"]:
            with self.subTest(filter_result=item["scenario_id"]):
                ...
                self.assertTrue(item["human_handling_required"])
                self.assertTrue(all(self.module.CONTROL_PREFIX in ref for ref in item["metadata_filter_refs"]))
    ...
    def test_evidence_gap_records_are_explicit_and_not_automatically_resolved(self):
        ...
        for item in report["evidence_gap_control_records"]:
            with self.subTest(gap=item["scenario_id"]):
                ...
                self.assertTrue(item["human_handling_required"])
                self.assertIn(self.module.CONTROL_PREFIX, item["evidence_gap_record_ref"])

    def test_parameter_rollback_instructions_do_not_change_live_parameters(self):
        report = self.report()
        self.assertEqual(3, len(report["parameter_rollback_instruction_control_records"]))
    ...
    def test_invalid_or_malformed_phase3_fails_closed(self):
        invalid = self.module.build_keyword_retrieval_phase4_delivery_report(
        ...
        def malformed_phase3():
            result = copy.deepcopy(self.phase3.build_keyword_retrieval_phase3_report())
            result["scenario_results"][0].pop("retrieval_trace_ref")
            return result

        malformed = self.module.build_keyword_retrieval_phase4_delivery_report(
    ...
    def test_predecessor_runtime_signal_fails_closed(self):
        def runtime_signal():
            result = copy.deepcopy(self.phase3.build_keyword_retrieval_phase3_report())
            result["model_token_consumption_performed"] = True
            return result

        report = self.module.build_keyword_retrieval_phase4_delivery_report(
