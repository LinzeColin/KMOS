...
SCOPE = BASE / "STAGE092_PHASE3_EVIDENCE_RISK_SCORING_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
P2_SCOPE = BASE / "STAGE092_PHASE2_EVIDENCE_RISK_SCORING_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
...
class Stage092EvidenceRiskScoringPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage092_phase3_scenarios", MODULE)
        cls.phase2 = load_module("stage092_phase2_slice", P2_MODULE)
        cls.report = cls.module.build_evidence_risk_scoring_phase3_report()

    def _phase2_report(self):
        return self.phase2.execute_evidence_risk_scoring_control_slice(
    ...
    def test_artifacts_phase_identity_and_frozen_taskpack_exist(self):
        ...
        self.assertEqual("IDS-STAGE092-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE092-P3", self.contract["task_id"])
        self.assertEqual("IDS-STAGE092-P4-GATE", self.contract["next_gate"])
    ...
    def test_contract_binds_single_authority_phase2_and_stage_boundary(self):
        ...
        self.assertEqual(97, replay["expected_projection_fields_per_request"])
        self.assertEqual(582, replay["expected_phase2_field_check_count"])
        self.assertTrue(replay["risk_formula_owner_approved_rule_required"])
    ...
    def test_report_replays_exact_phase2_shape_without_runtime(self):
        ...
        self.assertEqual(582, report["phase2_field_check_count"])
    ...
    def test_each_exception_scenario_is_control_only_and_whitebox_gated(self):
        ...
        expected = {
            "no_internal_evidence_risk_control",
            ...
            "revoked_evidence_risk_report_impact_control",
            "malicious_evidence_risk_quarantine_control",
            "low_grade_high_trust_masquerade_risk_control",
        }
        ...
        self.assertIsNone(scenarios["no_internal_evidence_risk_control"]["evidence_id_ref"])
        self.assertTrue(
            all(
    ...
    def test_taskpack_exception_dispositions_report_impact_and_masquerade_hold(self):
        ...
        self.assertEqual(
            ...
            scenarios["no_internal_evidence_risk_control"][
                "conclusion_acceptance_state"
            ],
        )
        self.assertEqual(
            "CONTROL_LOW_OCR_DEGRADED_NOT_ACCEPTED",
            scenarios["low_ocr_evidence_risk_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        self.assertEqual(
            "CONTROL_OLD_VERSION_DEGRADED_NOT_ACCEPTED",
            scenarios["old_version_evidence_risk_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        self.assertEqual(
            "CONTROL_CONFLICT_DEGRADED_NOT_ACCEPTED",
            scenarios["conflict_evidence_risk_degradation_control"][
                "evidence_disposition_state"
            ],
        )
        revoked = scenarios["revoked_evidence_risk_report_impact_control"]
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            scenarios["malicious_evidence_risk_quarantine_control"][
                "evidence_disposition_state"
            ],
        )
        masquerade = scenarios["low_grade_high_trust_masquerade_risk_control"]
    ...
    def test_contract_failure_and_protected_boundaries_are_explicit(self):
        ...
        self.assertTrue(rollback["preserve_stage092_phase1_and_phase2"])
        self.assertTrue(rollback["preserve_stage091_reviewed_artifacts"])

    def test_invalid_phase2_output_returns_controlled_failure(self):
        failed = self.module.build_evidence_risk_scoring_phase3_report(lambda _: {})
    ...
    def test_phase2_runtime_signal_returns_controlled_failure(self):
        ...
        failed = self.module.build_evidence_risk_scoring_phase3_report(runtime_signal)
        self.assertFalse(failed["valid"])
    ...
    def test_nonopaque_reference_and_semantic_drift_return_specific_failures(self):
        def nonopaque_reference(_control_input):
            altered = copy.deepcopy(self._phase2_report())
            altered["risk_schema_binding_control_projections"][0][
                "evidence_gap_ref"
            ] = "unscoped-reference"
            return altered

        failed = self.module.build_evidence_risk_scoring_phase3_report(
            nonopaque_reference
        )
        self.assertFalse(failed["valid"])
        ...
        failed = self.module.build_evidence_risk_scoring_phase3_report(low_ocr_drift)
        self.assertFalse(failed["valid"])
