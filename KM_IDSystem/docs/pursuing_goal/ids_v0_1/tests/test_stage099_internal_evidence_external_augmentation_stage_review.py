"""Stage099 内部依据与外部增强分离整阶段机械复审的聚焦测试。"""
...
REVIEW_DOCUMENT = BASE / "STAGE099_STAGE_REVIEW.md"
CONTRACT = (
    BASE
...
MODULE = (
    BASE
    / "index_version_schema"
    / "stage099_internal_evidence_external_augmentation_stage_review.py"
)
P1_CONTRACT = (
    BASE
    / "index_version_schema"
...
P4_RUN = ROOT / "machine" / "runs" / "2026-08-25-stage099-p4-local.json"
REVIEW_RUN = ROOT / "machine" / "runs" / "2026-08-25-stage099-review-local.json"
...
class Stage099InternalEvidenceExternalAugmentationReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage099_internal_evidence_external_augmentation_review", MODULE)
        cls.report = (
            cls.module.build_internal_evidence_external_augmentation_stage099_review_report()
        )
    ...
    def test_required_phase_artifacts_exist(self) -> None:
        ...
        self.assertEqual("IDS-STAGE099-REVIEW", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE099-REVIEW", self.contract["task_id"])
        self.assertEqual("IDS-STAGE099-REVIEW-GATE", self.contract["entry_gate"])
        self.assertEqual("IDS-STAGE100-P1-GATE", self.contract["next_gate"])

    def test_contract_keeps_authority_runtime_and_stage100_closed(self) -> None:
        ...
        self.assertEqual("8/4/3/15/4", replay["phase1_static_shape"])
    ...
    def test_runtime_and_rollback_boundaries_are_closed(self) -> None:
        ...
        self.assertFalse(report["stage100_started"])
        self.assertEqual(self.module.P4_PASS_RESULT, report["rollback"]["return_to"])
        self.assertTrue(report["rollback"]["preserve_stage099_phase1_to_phase4_evidence"])
        self.assertTrue(report["rollback"]["preserve_stage098_review_evidence"])
    ...
    def test_invalid_phase_contracts_fail_closed(self) -> None:
        for phase_name, kwargs in (
            ...
            with self.subTest(phase=phase_name):
                report = (
                    self.module.build_internal_evidence_external_augmentation_stage099_review_report(
                        **kwargs
                    )
                )
    ...
    def test_review_document_contract_and_rollback_are_explicit(self) -> None:
        ...
        self.assertTrue(failures["stage100_must_remain_not_started"])
    ...
    def test_governance_projection_records_review_when_current(self) -> None:
        ...
        for acceptance_id in (
            "ACC-STAGE099-REVIEW-01",
            "ACC-STAGE099-REVIEW-02",
            "ACC-STAGE099-REVIEW-03",
        ):
            with self.subTest(acceptance_id=acceptance_id):
                self.assertEqual("已通过", acceptance_by_id[acceptance_id])
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE099-REVIEW-04"])
