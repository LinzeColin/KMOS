...
SCOPE = BASE / "STAGE081_PHASE4_SHADOW_INDEX_DELIVERY_CLOSEOUT.md"
CONTRACT = BASE / "index_version_schema" / "stage081_shadow_index_delivery_contract.json"
MODULE = BASE / "index_version_schema" / "stage081_shadow_index_delivery.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
PHASE1_SCOPE = BASE / "STAGE081_PHASE1_SHADOW_INDEX_SCOPE_BOUNDARY.md"
PHASE1_CONTRACT = BASE / "index_version_schema" / "stage081_shadow_index_contract.json"
PHASE2_SCOPE = BASE / "STAGE081_PHASE2_SHADOW_INDEX_CONTROL_SLICE.md"
PHASE2_CONTRACT = (
    BASE / "index_version_schema" / "stage081_shadow_index_slice_contract.json"
)
PHASE2_MODULE = BASE / "index_version_schema" / "stage081_shadow_index_control_slice.py"
PHASE3_SCOPE = BASE / "STAGE081_PHASE3_SHADOW_INDEX_CONTROLLED_SCENARIOS.md"
PHASE3_CONTRACT = (
    BASE / "index_version_schema" / "stage081_shadow_index_scenarios_contract.json"
)
PHASE3_MODULE = BASE / "index_version_schema" / "stage081_shadow_index_scenarios.py"
...
class Stage081ShadowIndexPhase4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = _load_module(MODULE, "stage081_p4")
        cls.phase3 = _load_module(PHASE3_MODULE, "stage081_p3_for_p4_test")
        cls.phase2 = _load_module(PHASE2_MODULE, "stage081_p2_for_p4_test")

    def report(self):
        return self.module.build_shadow_index_phase4_delivery_report()
    ...
    def test_contract_declares_only_control_delivery_evidence(self):
        ...
        self.assertEqual("IDS-V0_1-STAGE081-P4", contract["task_id"])
        self.assertEqual(
        ...
        self.assertEqual("IDS-STAGE081-P4-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE081-REVIEW-GATE", contract["next_gate"])
        for field in (
    ...
    def test_predecessor_replay_preserves_exact_control_shapes(self):
        phase2 = self.phase2.execute_shadow_index_control_slice(
            self.phase2.build_control_input()
        )
        phase3 = self.phase3.build_shadow_index_phase3_report()
        self.assertEqual(
            (True, "COMPLETED_IN_MEMORY_SHADOW_INDEX_CONTROL_SLICE", 5, 5),
            (
    ...
    def test_manifest_smoke_switch_and_rollback_samples_are_control_only(self):
        ...
        for item in report["index_manifest_control_samples"]:
            with self.subTest(manifest=item["control_scenario"]):
                ...
                self.assertIn(":control:stage081-p2:", item["index_manifest_ref"])
        for item in report["smoke_test_log_control_samples"]:
            with self.subTest(smoke=item["scenario_id"]):
                ...
                self.assertIn(":control:stage081-p2:", item["smoke_test_log_ref"])
    ...
    def test_retention_space_and_operational_instructions_remain_control_only(self):
        ...
        for item in report["operational_instruction_projections"]:
            with self.subTest(instruction=item["instruction_id"]):
                ...
                self.assertIn(":control:stage081-p2:", item["target_ref"])
        self.assertEqual(4, len(report["chinese_feedback"]))

    def test_invalid_predecessor_or_runtime_signal_fails_closed(self):
        invalid = self.module.build_shadow_index_phase4_delivery_report(
        ...
        def malformed_phase2():
            result = copy.deepcopy(
                self.phase2.execute_shadow_index_control_slice(
                    self.phase2.build_control_input()
                )
            )
        ...
        malformed = self.module.build_shadow_index_phase4_delivery_report(
        ...
        def phase3_runtime_signal():
            result = copy.deepcopy(self.phase3.build_shadow_index_phase3_report())
            result["retrieval_query_performed"] = True
            return result

        runtime_signal = self.module.build_shadow_index_phase4_delivery_report(
