...
SCOPE = BASE / "STAGE086_PHASE4_HYBRID_RANKING_DELIVERY_EVIDENCE.md"
CONTRACT = (
    BASE / "index_version_schema" / "stage086_hybrid_ranking_delivery_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage086_hybrid_ranking_delivery.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
P3_SCOPE = BASE / "STAGE086_PHASE3_HYBRID_RANKING_CONTROLLED_SCENARIOS.md"
P3_CONTRACT = (
...
RUN = ROOT / "machine" / "runs" / "2026-08-22-stage086-p4-local.json"
...
class Stage086HybridRankingPhase4DeliveryTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = _load_module("stage086_p4", MODULE)
        cls.phase3 = _load_module("stage086_p3", P3_MODULE)

    def _report(self):
        return self.module.build_hybrid_ranking_phase4_delivery_report()

    def _phase3_report(self):
        return self.phase3.build_hybrid_ranking_phase3_report()
    ...
    def test_contract_identity_authority_and_review_gate_are_exact(self):
        ...
        self.assertEqual(
            "ids.stage086.hybrid_ranking.phase4.delivery.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-086", contract["stage"])
        self.assertEqual("IDS-STAGE086-P4", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE086-P4", contract["task_id"])
        self.assertEqual(
            "PHASE4_HYBRID_RANKING_DELIVERY_EVIDENCE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE086-P4-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE086-REVIEW-GATE", contract["next_gate"])
    ...
    def test_contract_replay_delivery_and_runtime_shapes_are_exact(self):
        ...
        self.assertEqual(480, replay["phase2_control_field_check_count"])
    ...
    def test_all_delivery_groups_keep_exact_shapes_and_scenario_order(self):
        ...
        groups = (
            ...
            ("validity_test_report_control_records", self.module.VALIDITY_TEST_REPORT_FIELDS),
    ...
    def test_parameter_rollback_instructions_are_future_only_and_whitebox_gated(self):
        ...
        self.assertTrue(
            all(set(item) == set(self.module.PARAMETER_ROLLBACK_INSTRUCTION_FIELDS) for item in instructions)
        )
    ...
    def test_malformed_phase3_report_fails_closed_without_delivery_records(self):
        ...
        altered["phase2_control_record_field_check_count"] = 479
        failed = self.module.build_hybrid_ranking_phase4_delivery_report(lambda: altered)
