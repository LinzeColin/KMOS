...
SCOPE = BASE / "STAGE083_PHASE1_KEYWORD_RETRIEVAL_BASELINE_SCOPE_BOUNDARY.md"
CONTRACT = BASE / "index_version_schema" / "stage083_keyword_retrieval_baseline_contract.json"
TASKPACK = ROOT / "docs" / "taskpacks" / "IDS_v0_1_Final_Chinese_Revised" / "stages" / "STAGE-083_关键词检索基线.md"
PREDECESSOR_REVIEW = BASE / "STAGE082_STAGE_REVIEW.md"
PREDECESSOR_CONTRACT = BASE / "index_version_schema" / "stage082_old_index_retention_contract.json"
PREDECESSOR_DELIVERY = BASE / "index_version_schema" / "stage082_old_index_retention_delivery_contract.json"


class Stage083KeywordRetrievalBaselinePhase1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_and_single_authority_boundary_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage083.keyword_retrieval_baseline_contract.phase1.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-083", contract["stage"])
        self.assertEqual("IDS-STAGE083-P1", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE083-P1", contract["task_id"])
        self.assertEqual("ACC-STAGE-083", contract["acceptance_id"])
        self.assertEqual(
            "PHASE1_KEYWORD_RETRIEVAL_BASELINE_CONTRACT_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE083-P2-GATE", contract["next_gate"])
        source = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE083_TASKPACK_AND_STAGE082_REVIEWED_OLD_INDEX_RETENTION_CONTRACTS_ONLY",
            source["authority"],
        )
        for field in (
    ...
    def test_future_runtime_prerequisites_are_defined_without_runtime(self):
        ...
        for field, value in prerequisite.items():
            if field.endswith("_created") or field.endswith("_performed") or field.endswith("_executed") or field.endswith("_persisted"):
                with self.subTest(field=field):
    ...
    def test_failure_and_runtime_boundaries_remain_zero(self):
        ...
        for state in (
            ...
            "PHASE1_KEYWORD_RETRIEVAL_NOT_AUTHORIZED",
        ):
    ...
    def test_rollback_preserves_predecessor_and_runtime_is_not_authorized(self):
        ...
        self.assertEqual(
            "PASS_REVIEWED_OLD_INDEX_RETENTION_RUNTIME_DISABLED",
            rollback["return_to"],
        )
