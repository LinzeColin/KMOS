"""Stage109 报告影响分析 Phase 1 静态合同的聚焦验证。"""
...
SCOPE = BASE / "STAGE109_PHASE1_REPORT_IMPACT_ANALYSIS_SCOPE_BOUNDARY.md"
CONTRACT = BASE / "index_version_schema" / "stage109_report_impact_analysis_contract.json"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage109-p1-local.json"
...
class Stage109ReportImpactAnalysisPhase1Tests(unittest.TestCase):
    @classmethod
    ...
    def test_identity_and_single_authority_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage109.report_impact_analysis.phase1.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-109", contract["stage"])
        self.assertEqual("IDS-STAGE109-P1", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE109-P1", contract["task_id"])
        self.assertEqual("ACC-STAGE-109", contract["acceptance_id"])
        self.assertEqual(
            "REPORT_IMPACT_ANALYSIS_CONTRACT_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE109-P1-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE109-P2-GATE", contract["next_gate"])
        authority = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE109_TASKPACK_AND_STAGE108_REVIEWED_REPORT_SNAPSHOT_CONTROL_ARTIFACTS_ONLY",
            authority["authority"],
        )
        for field in (
    ...
    def test_impact_control_shape_and_evidence_controls_are_exact(self) -> None:
        controls = self.contract["report_impact_analysis_contract"]
        self.assertEqual(EXPECTED_CONTROL_FIELDS, controls["future_control_reference_fields"])
        self.assertEqual(33, controls["future_control_reference_field_count"])
        ...
        for field in (
            ...
            "future_affected_report_and_critical_conclusion_identification_required",
    ...
    def test_snapshot_and_impact_delivery_controls_are_complete(self) -> None:
        ...
        for field, value in snapshot.items():
            if field.startswith("actual_"):
                with self.subTest(field=field):
                    self.assertFalse(value)
        delivery = self.contract["report_impact_delivery_control_contract"]
    ...
    def test_failure_feedback_rollback_and_stage_boundary_are_explicit(self) -> None:
        ...
        self.assertEqual(35, failures["failure_state_count"])
        self.assertEqual(
        ...
        for failure_state in (
            ...
            "CITED_MATERIAL_UPDATE_REFERENCE_MISSING",
            "EVIDENCE_DOWNGRADE_REFERENCE_MISSING",
            "AFFECTED_REPORT_REFERENCE_MISSING",
            ...
            "STAGE109_PHASE2_NOT_AUTHORIZED",
            "STAGE110_STARTED_WITHOUT_AUTHORIZATION",
        ):
    ...
    def test_current_governance_accepts_predecessor_or_current_projection(self) -> None:
        ...
        self.assertIn("IDS-STAGE109-P2-GATE", plan["stop_condition"])
        receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
        self.assertEqual(
            "PASS_REPORT_IMPACT_ANALYSIS_CONTRACT_RUNTIME_DISABLED",
            receipt["result"],
        )
        self.assertEqual("IDS-STAGE109-P2-GATE", receipt["next_gate"])
        ...
        self.assertTrue(receipt["final_validation"]["all_executed_validation_passed"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE109-P1-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE109-P1-20260826-001", event_ids)
