...
SCOPE = BASE / "STAGE085_PHASE1_METADATA_FILTER_SCOPE_BOUNDARY.md"
CONTRACT = BASE / "index_version_schema" / "stage085_metadata_filter_contract.json"
TASKPACK = ROOT / "docs" / "taskpacks" / "IDS_v0_1_Final_Chinese_Revised" / "stages" / "STAGE-085_元数据过滤.md"
PREDECESSOR_REVIEW = BASE / "STAGE084_STAGE_REVIEW.md"
PREDECESSOR_CONTRACT = BASE / "index_version_schema" / "stage084_vector_retrieval_baseline_contract.json"
PREDECESSOR_DELIVERY = BASE / "index_version_schema" / "stage084_vector_retrieval_baseline_delivery_contract.json"


class Stage085MetadataFilterPhase1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_and_single_authority_boundary_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage085.metadata_filter_contract.phase1.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-085", contract["stage"])
        self.assertEqual("IDS-STAGE085-P1", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE085-P1", contract["task_id"])
        self.assertEqual("ACC-STAGE-085", contract["acceptance_id"])
        self.assertEqual(
            "PHASE1_METADATA_FILTER_CONTRACT_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE085-P2-GATE", contract["next_gate"])
        source = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE085_TASKPACK_AND_STAGE084_REVIEWED_VECTOR_RETRIEVAL_BASELINE_CONTRACTS_ONLY",
            source["authority"],
        )
        for field in (
    ...
    def test_query_filter_candidate_selected_score_and_trace_shapes_are_fixed(self):
        ...
        self.assertEqual(
            ...
            shape["future_metadata_filter_fields"],
        )
        for field in (
    ...
    def test_future_runtime_prerequisites_are_defined_without_runtime(self):
        ...
        for field, value in prerequisite.items():
            if field.endswith("_created") or field.endswith("_performed") or field.endswith("_executed") or field.endswith("_persisted") or field.endswith("_selected") or field.endswith("_generated") or field.endswith("_evaluated"):
                with self.subTest(field=field):
    ...
    def test_failure_and_runtime_boundaries_remain_zero(self):
        ...
        for state in (
            ...
            "PHASE1_METADATA_FILTER_NOT_AUTHORIZED",
        ):
