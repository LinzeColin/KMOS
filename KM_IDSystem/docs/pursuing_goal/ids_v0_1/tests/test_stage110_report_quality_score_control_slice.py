"""Stage110 报告质量评分 Phase 2 纯内存控制切片的聚焦验证。"""
...
SCOPE = BASE / "STAGE110_PHASE2_REPORT_QUALITY_SCORE_CONTROL_SLICE.md"
CONTRACT = (
    BASE
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage110-p2-local.json"
...
class Stage110ReportQualityScorePhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = importlib.import_module(
            "KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema."
            "stage110_report_quality_score_control_slice"
        )
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.control_input = cls.module.build_control_input()
        cls.result = cls.module.execute_report_quality_score_control_slice(
            cls.control_input
        )
    ...
    def test_identity_authority_predecessors_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("IDS-STAGE110-P2", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE110-P2", contract["task_id"])
        self.assertEqual("ACC-STAGE-110", contract["acceptance_id"])
        self.assertEqual(
            "REPORT_QUALITY_SCORE_CONTROL_SLICE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE110-P2-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE110-P3-GATE", contract["next_gate"])
        authority = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE110_TASKPACK_STAGE110_PHASE1_AND_STAGE109_REVIEWED_"
            "REPORT_IMPACT_ANALYSIS_CONTROL_ARTIFACTS_ONLY",
            authority["authority"],
        )
        for field in (
        ...
        self.assertTrue(predecessor["stage110_phase1_required"])
        self.assertEqual(
    ...
    def test_fixed_control_input_preserves_phase1_quality_score_shape(self) -> None:
        ...
        self.assertEqual(40, controls["phase1_control_reference_field_count"])
        self.assertEqual(42, len(self.module.INPUT_FIELDS))
        self.assertEqual(
    ...
    def test_binding_snapshot_quality_and_export_controls_remain_reference_only(self) -> None:
        binding_records = self.result[
            "report_evidence_binding_and_section_control_projections"
        ]
        snapshot_records = self.result["generation_snapshot_control_projections"]
        quality_records = self.result[
            "report_quality_score_and_lifecycle_control_projections"
        ]
        self.assertEqual(5, len(binding_records))
        self.assertEqual(5, len(snapshot_records))
        self.assertEqual(5, len(quality_records))
        for binding, snapshot, quality in zip(
            binding_records, snapshot_records, quality_records
        ):
            with self.subTest(scenario=binding["control_scenario"]):
                self.assertTrue(
                    bool(binding["evidence_id_ref"])
                    ^ bool(binding["evidence_gap_ref"])
                )
                self.assertEqual(
                ...
                self.assertFalse(binding["actual_report_section_output_performed"])
                ...
                for field in (
                    ...
                    with self.subTest(field=field):
                        self.assertTrue(
                            quality[field].startswith(self.module.CONTROL_PREFIX)
                        )
                self.assertTrue(
                    quality[
                        "quality_formula_weight_threshold_business_line_whitebox_required"
                    ]
                )
    ...
    def test_input_drift_fails_closed_without_projection_or_runtime(self) -> None:
        ...
        invalid_input[self.module.CONTROL_FIELDS[0]][0]["quality_formula_ref"] = (
            ":control:stage110-p2:unexpected-quality-formula:reference-only"
        )
        rejected = self.module.execute_report_quality_score_control_slice(invalid_input)
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        for phrase in (
            ...
            "每条请求固定 42 个输入字段",
            "共 126 个字段、五条共 630 个检查点",
            "P3 才验证关键结论 evidence_id/evidence_gap",
        ...
        self.assertEqual(
            "PASS_REPORT_QUALITY_SCORE_CONTROL_SLICE_RUNTIME_DISABLED",
            receipt["result"],
        )
        self.assertEqual("IDS-STAGE110-P2-GATE", receipt["entry_gate"])
        self.assertEqual("IDS-STAGE110-P3-GATE", receipt["next_gate"])
        self.assertEqual(42, receipt["control_shape"]["control_input_field_count"])
        self.assertEqual(126, receipt["control_shape"]["projection_field_count_per_request"])
        self.assertEqual(630, receipt["control_shape"]["projection_field_count_total"])
        ...
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE110-P2-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE110-P2-20260826-001", event_ids)
