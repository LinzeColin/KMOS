"""Stage113 复核队列 Schema 整阶段机械复审的聚焦验证。"""
...
SCOPE = BASE / "STAGE113_STAGE_REVIEW.md"
CONTRACT = (
    BASE
...
MODULE = (
    BASE / "index_version_schema" / "stage113_review_queue_schema_stage_review.py"
)
P1_SCOPE = BASE / "STAGE113_PHASE1_REVIEW_QUEUE_SCHEMA_SCOPE_BOUNDARY.md"
P1_CONTRACT = BASE / "index_version_schema" / "stage113_review_queue_schema_contract.json"
P1_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage113-p1-local.json"
P2_SCOPE = BASE / "STAGE113_PHASE2_REVIEW_QUEUE_SCHEMA_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage113_review_queue_schema_control_slice_contract.json"
)
P2_MODULE = BASE / "index_version_schema" / "stage113_review_queue_schema_control_slice.py"
P2_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage113-p2-local.json"
P3_SCOPE = BASE / "STAGE113_PHASE3_REVIEW_QUEUE_SCHEMA_CONTROLLED_SCENARIOS.md"
P3_CONTRACT = (
    BASE
    / "index_version_schema"
...
P3_MODULE = (
    BASE / "index_version_schema" / "stage113_review_queue_schema_controlled_scenarios.py"
)
P3_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage113-p3-local.json"
P4_SCOPE = BASE / "STAGE113_PHASE4_REVIEW_QUEUE_SCHEMA_DELIVERY.md"
P4_CONTRACT = (
    BASE / "index_version_schema" / "stage113_review_queue_schema_delivery_contract.json"
)
P4_MODULE = BASE / "index_version_schema" / "stage113_review_queue_schema_delivery.py"
P4_RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage113-p4-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-27-stage113-review-local.json"
...
class Stage113ReviewQueueSchemaStageReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        prefix = "KM_IDSystem.docs.pursuing_goal.ids_v0_1.index_version_schema."
        cls.module = importlib.import_module(
            f"{prefix}stage113_review_queue_schema_stage_review"
        )
        cls.phase2 = importlib.import_module(
            f"{prefix}stage113_review_queue_schema_control_slice"
        )
        cls.phase3 = importlib.import_module(
            f"{prefix}stage113_review_queue_schema_controlled_scenarios"
        )
        cls.phase4 = importlib.import_module(
            f"{prefix}stage113_review_queue_schema_delivery"
        )
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.phase1_contract = json.loads(P1_CONTRACT.read_text(encoding="utf-8"))
        cls.phase2_report = cls.phase2.execute_review_queue_schema_control_slice(
            cls.phase2.build_control_input()
        )
        cls.phase3_report = cls.phase3.build_review_queue_schema_phase3_report()
        cls.phase4_report = cls.phase4.build_review_queue_schema_phase4_delivery_report()
        cls.report = cls.module.build_review_queue_schema_stage_review()
    ...
    def test_identity_reviewed_shape_failure_contract_and_boundary_are_exact(self) -> None:
        ...
        self.assertEqual("IDS-STAGE113-REVIEW", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE113-REVIEW", contract["task_id"])
        self.assertEqual(self.module.REVIEW_GATE, contract["entry_gate"])
        self.assertEqual(self.module.NEXT_GATE, contract["next_gate"])
        self.assertEqual(
            "STAGE113_REVIEW_QUEUE_SCHEMA_REVIEW_RUNTIME_DISABLED",
        ...
        self.assertTrue(failure["stage114_must_remain_not_started"])
        ...
        for field in (
            ...
            "stage113_phase4_completed",
            "stage113_review_started",
            "whole_stage_review_completed_in_memory_only",
        ):
            with self.subTest(field=field):
                self.assertTrue(boundary[field])
        for field in ("stage114_started", "github_upload_allowed", "push_allowed"):
            with self.subTest(field=field):
    ...
    def test_review_passes_with_exact_phase_results_shapes_and_zero_runtime(self) -> None:
        ...
        self.assertFalse(report["stage114_started"])
        self.assertEqual(
            self.module.REVIEWED_CONTROL_SHAPE,
        ...
        for scenario in scenarios:
            with self.subTest(scenario=scenario["scenario_id"]):
                ...
                for field in (
                    ...
                    self.assertTrue(scenario[field].startswith(":control:stage113-p2:"))
                self.assertEqual(
                ...
                self.assertFalse(
                    scenario["actual_evidence_or_report_writeback_execution_performed"]
                )
                self.assertFalse(scenario["actual_human_confirmation_execution_performed"])
        handlings = self.phase3_report["business_line_whitebox_handlings"]
        self.assertEqual(5, len(handlings))
        self.assertTrue(all(item["confirmation_required"] for item in handlings))
        self.assertTrue(
            all(
                not item["actual_human_confirmation_execution_performed"]
    ...
    def test_each_phase_or_semantic_drift_fails_closed(self) -> None:
        ...
        phase1["review_queue_schema_control_contract"][
            "future_control_reference_field_count"
        ] = 28
        p1_failed = self.module.build_review_queue_schema_stage_review(
        ...
        review_shape_failed = self.module.build_review_queue_schema_stage_review(
        ...
        lifecycle_failed = self.module.build_review_queue_schema_stage_review(
        ...
        expected = (
            ...
            (p3_failed, "CONTROL_REFERENCE_OPAQUENESS_MISMATCH"),
            (opaque_failed, "CONTROL_REFERENCE_OPAQUENESS_MISMATCH"),
            (whitebox_failed, "REVIEW_AUDIT_IMPACT_AND_WHITEBOX_SEMANTICS_MISMATCH"),
    ...
    def test_current_governance_receipt_and_event_are_exact(self) -> None:
        ...
        self.assertIn("IDS-STAGE114-P1-GATE", plan["stop_condition"])
        acceptance = json.loads(ACCEPTANCE.read_text(encoding="utf-8"))
        acceptance_by_id = {item["id"]: item["status"] for item in acceptance["items"]}
        self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-113"])
        for acceptance_id in (
            "ACC-STAGE113-REVIEW-01",
            "ACC-STAGE113-REVIEW-02",
            "ACC-STAGE113-REVIEW-03",
        ):
            with self.subTest(acceptance_id=acceptance_id):
                self.assertEqual("已通过", acceptance_by_id[acceptance_id])
        self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE113-REVIEW-04"])
        event_ids = {
            json.loads(line)["event_id"]
            for line in EVENTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
        self.assertIn("EVT-IDS-V0_1-STAGE113-REVIEW-20260827-001", event_ids)
