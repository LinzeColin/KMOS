...
SCOPE = BASE / "STAGE072_PHASE2_EMBEDDING_MODEL_VERSION_CONTROL_SLICE.md"
CONTRACT = (
    BASE
...
MODULE = (
    BASE
    / "embedding_model_version"
    / "stage072_embedding_model_version_slice.py"
)
TASKPACK = (
    ROOT
    / "docs"
...
def _load_module():
    spec = importlib.util.spec_from_file_location("stage072_slice", MODULE)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage072 P2 control slice")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage072EmbeddingModelVersionPhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_contract_preserves_single_authority_and_zero_runtime(self):
        ...
        self.assertEqual(
            "ids.stage072.embedding_model_version.phase2.v1",
            contract["schema_version"],
        )
        self.assertEqual("IDS-V0_1-STAGE072-P2", contract["task_id"])
        self.assertEqual(
            "PHASE2_EMBEDDING_MODEL_VERSION_CONTROL_SLICE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertTrue(contract["slice_executable"])
        self.assertFalse(contract["execution_ready"])
        self.assertEqual("IDS-STAGE072-P3-GATE", contract["next_gate"])
        for field in (
    ...
    def test_contract_shapes_cover_queue_cache_model_version_and_audit(self):
        input_contract = self.contract[
            "reference_only_embedding_model_version_input_control_contract"
        ]
        ...
        model_version = self.contract["model_version_control_contract"]
        self.assertEqual(
            [
                "provider_ref",
                "model_ref",
            ...
            model_version["required_fields"],
        )
        self.assertEqual(6, model_version["field_count"])
        self.assertEqual(0, model_version["actual_model_version_record_count"])
        self.assertFalse(model_version["provider_or_model_selection_allowed"])
    ...
    def test_fixed_input_is_non_business_and_projection_has_exact_shapes(self):
        ...
        requests = control_input["embedding_model_version_requests"]
        self.assertEqual(list(self.module.CONTROL_SCENARIOS), [
            scenario for scenario in self.contract[
                "reference_only_embedding_model_version_input_control_contract"
        ...
        for request in requests:
            ...
            self.assertTrue(
                all(
                    isinstance(value, str) and ":control:stage072-p2:" in value
        ...
        result = self.module.execute_embedding_model_version_control_slice(control_input)
    ...
    def test_policy_inheritance_blocks_unauthorized_egress_and_keeps_text_out(self):
        result = self.module.execute_embedding_model_version_control_slice(
    ...
    def test_model_and_audit_control_projections_keep_required_values_non_actual(self):
        result = self.module.execute_embedding_model_version_control_slice(
            self.module.build_control_input()
        )
    ...
    def test_tampered_or_reordered_input_fails_closed(self):
        ...
        tampered["embedding_model_version_requests"][0]["provider_ref"] = "provider:tampered"
        result = self.module.execute_embedding_model_version_control_slice(tampered)
        ...
        self.assertEqual(0, result["model_version_control_projection_count"])
        self.assertFalse(result["external_api_call_performed"])

        reordered = self.module.build_control_input()
        reordered["embedding_model_version_requests"].reverse()
        self.assertFalse(
            self.module.execute_embedding_model_version_control_slice(reordered)[
                "input_accepted"
            ]
        )
