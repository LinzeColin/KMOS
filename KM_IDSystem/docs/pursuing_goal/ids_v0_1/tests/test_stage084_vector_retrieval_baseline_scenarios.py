...
SCOPE = BASE / "STAGE084_PHASE3_VECTOR_RETRIEVAL_BASELINE_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
P1_SCOPE = BASE / "STAGE084_PHASE1_VECTOR_RETRIEVAL_BASELINE_SCOPE_BOUNDARY.md"
P1_CONTRACT = (
    BASE / "index_version_schema" / "stage084_vector_retrieval_baseline_contract.json"
)
P2_SCOPE = BASE / "STAGE084_PHASE2_VECTOR_RETRIEVAL_BASELINE_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
...
STAGE083_REVIEW = BASE / "STAGE083_STAGE_REVIEW.md"


def _load_module():
    spec = importlib.util.spec_from_file_location("stage084_p3", MODULE)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Stage084 P3 controlled scenarios")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage084VectorRetrievalPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _report(self):
        return self.module.build_vector_retrieval_phase3_report()
    ...
    def test_control_artifacts_phase_identity_and_gate_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage084.vector_retrieval_baseline.phase3.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("STAGE-084", self.contract["stage"])
        self.assertEqual("IDS-STAGE084-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE084-P3", self.contract["task_id"])
        self.assertEqual(
            "PHASE3_VECTOR_RETRIEVAL_BASELINE_CONTROLLED_SCENARIOS_RUNTIME_DISABLED",
            self.contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE084-P4-GATE", self.contract["next_gate"])
    ...
    def test_contract_preserves_authority_replay_and_runtime_boundaries(self):
        ...
        self.assertEqual(290, replay["expected_phase2_field_check_count"])
        for field in (
    ...
    def test_report_replays_p2_and_preserves_all_control_shapes(self):
        ...
        self.assertEqual(290, report["phase2_control_record_field_check_count"])
    ...
    def test_taskpack_scenario_categories_hold_without_business_values(self):
        ...
        self.assertEqual(
            ...
            scenarios["five_dimension_filter_combination_control"][
    ...
    def test_invalid_or_side_effecting_phase2_replay_is_fail_closed(self):
        def missing_vector_baseline(control_input):
            result = self._phase2().execute_vector_retrieval_control_slice(control_input)
            altered = copy.deepcopy(result)
            altered["all_vector_baselines_declared"] = False
            return altered

        failed = self.module.build_vector_retrieval_phase3_report(
            missing_vector_baseline
        )
        self.assertFalse(failed["valid"])
        self.assertEqual(self.module.FAIL_RESULT, failed["result"])
        self.assertEqual("IDS-STAGE084-P3-GATE", failed["next_gate"])
        ...
        failed = self.module.build_vector_retrieval_phase3_report(runtime_signal)
        self.assertFalse(failed["valid"])
