...
SCOPE = BASE / "STAGE090_PHASE2_RETRIEVAL_EVIDENCE_CAPTURE_CONTROL_SLICE.md"
CONTRACT = (
    BASE
...
def load_module():
    spec = importlib.util.spec_from_file_location(
        "stage090_retrieval_evidence_capture_control_slice", MODULE
    )
...
class Stage090RetrievalEvidenceCaptureControlSliceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.result = cls.module.execute_retrieval_evidence_capture_control_slice(
            cls.control_input
        )
    ...
    def test_identity_authority_and_phase_boundary_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage090.retrieval_evidence_capture.phase2.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-090", contract["stage"])
        self.assertEqual("IDS-STAGE090-P2", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE090-P2", contract["task_id"])
        self.assertEqual(
            "PHASE2_RETRIEVAL_EVIDENCE_CAPTURE_CONTROL_SLICE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE090-P3-GATE", contract["next_gate"])
        ...
        for field in (
            "stage089_review_required",
            "stage090_phase1_required",
            "predecessor_evidence_schema_binding_required",
            "stage090_phase2_may_not_replace_predecessor_contracts",
        ):
            with self.subTest(field=field):
    ...
    def test_fixed_control_input_is_exact_nonbusiness_and_reference_only(self):
        ...
        for request in requests:
            with self.subTest(scenario=request["control_scenario"]):
                ...
                self.assertEqual(
                    "CONTROL_EVIDENCE_CAPTURE_DECLARED_NOT_EXECUTED",
    ...
    def test_schema_binding_capture_relation_risk_revocation_and_binding_chain_is_exact(
        self,
    ):
        requests = self.control_input[self.module.CONTROL_FIELDS[0]]
        for index, request in enumerate(requests):
            with self.subTest(scenario=request["control_scenario"]):
                schema = self.result["evidence_schema_binding_control_projections"][index]
                retrieval = self.result["retrieval_capture_control_projections"][index]
                ledger = self.result[
                    "evidence_ledger_capture_control_projections"
                ][index]
                relation = self.result["capture_relation_control_projections"][index]
                ...
                self.assertEqual(
                    self.module.PREDECESSOR_SCHEMA_CONTRACT_CONTROL_REF,
                    schema["predecessor_evidence_schema_contract_ref"],
                )
                self.assertEqual(
                    "CONTROL_PREDECESSOR_SCHEMA_BOUND_NOT_REDEFINED",
                ...
                for field in (
                    ...
                    self.assertEqual(request[field], schema[field])
                for field in self.module.RETRIEVAL_CAPTURE_FIELDS:
                    self.assertEqual(request[field], retrieval[field])
                for field in self.module.EVIDENCE_LEDGER_CAPTURE_FIELDS:
                    self.assertEqual(request[field], ledger[field])
                for field in self.module.CAPTURE_RELATION_FIELDS:
    ...
    def test_low_conflict_expired_revoked_and_suspected_poisoning_are_not_accepted(
        self,
    ):
        expected = {
        ...
        for request, degradation, poison_defense in zip(
            requests, degradations, poison_defenses
        ):
            with self.subTest(scenario=request["control_scenario"]):
                self.assertEqual(
    ...
    def test_nonfixed_control_input_fails_closed_without_projections(self):
        ...
        rejected = self.module.execute_retrieval_evidence_capture_control_slice(
            invalid_input
        )
        self.assertFalse(rejected["input_accepted"])
        self.assertEqual(
            "REJECTED_IN_MEMORY_RETRIEVAL_EVIDENCE_CAPTURE_CONTROL_SLICE",
    ...
    def test_failure_runtime_and_protected_boundaries_remain_zero(self):
        ...
        for state in (
            ...
            "PREDECESSOR_EVIDENCE_SCHEMA_CONTRACT_REFERENCE_MISSING",
            "EVIDENCE_CAPTURE_REFERENCE_MISSING",
            ...
            "PHASE2_RETRIEVAL_EVIDENCE_CAPTURE_CONTROL_INPUT_REJECTED",
        ):
        ...
        allowed_local_code = {
            ...
            "pure_in_memory_only",
            "predecessor_evidence_schema_binding_control_slice_created",
            "retrieval_evidence_capture_control_slice_created",
            "evidence_ledger_capture_control_slice_created",
    ...
    def test_scope_and_rollback_keep_only_phase3_next(self):
        ...
        self.assertTrue(rollback["preserve_stage090_phase1_evidence"])
