"""Stage105 报告证据绑定 Phase 4 纯内存交付证据的聚焦验证。"""
...
SCOPE = BASE / "STAGE105_PHASE4_REPORT_EVIDENCE_BINDING_DELIVERY.md"
CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage105_report_evidence_binding_delivery_contract.json"
)
MODULE = (
...
P3_SCOPE = BASE / "STAGE105_PHASE3_REPORT_EVIDENCE_BINDING_CONTROLLED_SCENARIOS.md"
P3_CONTRACT = (
    BASE
    / "index_version_schema"
...
P3_RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage105-p3-local.json"
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-26-stage105-p4-local.json"
...
class Stage105ReportEvidenceBindingPhase4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_module(
            MODULE, "stage105_report_evidence_binding_delivery_for_test"
        )
        cls.phase3 = _load_module(
            P3_MODULE, "stage105_report_evidence_binding_phase3_for_delivery_test"
        )
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.phase3_report = cls.phase3.build_report_evidence_binding_phase3_report()
        cls.report = cls.module.build_report_evidence_binding_phase4_delivery_report()
    ...
    def test_identity_authority_predecessor_and_phase_boundary_are_exact(self) -> None:
        ...
        self.assertEqual(
            "ids.stage105.report_evidence_binding.phase4.delivery.v1",
            contract["schema_version"],
        )
        self.assertEqual("STAGE-105", contract["stage"])
        self.assertEqual("IDS-STAGE105-P4", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE105-P4", contract["task_id"])
        self.assertEqual("ACC-STAGE-105", contract["acceptance_id"])
        self.assertEqual("IDS-STAGE105-P4-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE105-REVIEW-GATE", contract["next_gate"])
        self.assertEqual(
        ...
        self.assertTrue(source["evidence_ledger_remains_authoritative"])
        ...
        self.assertEqual(26, predecessor["phase2_input_field_count"])
        self.assertEqual(4, predecessor["phase2_projection_group_count"])
        self.assertEqual(66, predecessor["phase2_projection_field_count_per_request"])
        self.assertEqual(330, predecessor["phase2_projection_field_count_total"])
    ...
    def test_delivery_shapes_control_references_and_counts_are_exact(self) -> None:
        ...
        self.assertEqual("IDS-STAGE105-P4-GATE", report["current_gate"])
        self.assertEqual("IDS-STAGE105-REVIEW-GATE", report["next_gate"])
                ...
                with self.subTest(group=group_name, record=record):
                    ...
                    for field, value in record.items():
                        ...
                        if field.endswith("_ref") or field in {
                            ...
                            self.assertTrue(
                                value.startswith(":control:stage105-p2:")
                                or value.startswith(":control:stage105-p4:")
                            )
    ...
    def test_invalid_or_tampered_phase3_output_fails_closed(self) -> None:
        invalid = self.module.build_report_evidence_binding_phase4_delivery_report(
            lambda: {}
        )
        self.assertFalse(invalid["valid"])
        self.assertEqual("PHASE3_CONTROL_OUTPUT_INVALID", invalid["failure_state"])
        self.assertEqual("IDS-STAGE105-P4-GATE", invalid["next_gate"])

        def replay_with(mutator):
            result = copy.deepcopy(self.phase3_report)
            mutator(result)
            return self.module.build_report_evidence_binding_phase4_delivery_report(
                lambda: result
            )
        ...
        for failed_report, failure_state in ((invalid, "PHASE3_CONTROL_OUTPUT_INVALID"), *expected):
    ...
    def test_delivery_boundary_validators_detect_write_review_and_template_drift(self) -> None:
        ...
        scenarios = template_drift["report_sample_control_records"]
        self.assertFalse(
        ...
        review_drift["stage105_review_started"] = True
        self.assertEqual(
            "STAGE105_REVIEW_STARTED",
    ...
    def test_runtime_receipt_and_current_governance_are_exact(self) -> None:
        ...
        self.assertFalse(self.report["stage105_review_started"])
        ...
        if current == phase4_current:
            ...
            self.assertEqual("IDS-STAGE105-REVIEW-GATE", receipt["next_gate"])
            ...
            self.assertEqual(
                23, validation["explicit_predecessor_focused_test_count"]
            )
            self.assertEqual(
                846, validation["historical_whitebox_chain_test_count"]
            )
            for field in (
            ...
            self.assertEqual(
                "已遵守", acceptance_by_id["ACC-STAGE105-P4-04"]
            )
            event_ids = {
        ...
        elif current in {review_current, stage106_phase1_current}:
            self.assertTrue(is_current_projection)
        else:
