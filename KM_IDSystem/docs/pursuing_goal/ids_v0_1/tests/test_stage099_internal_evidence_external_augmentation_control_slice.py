...
SCOPE = BASE / "STAGE099_PHASE2_INTERNAL_EVIDENCE_EXTERNAL_AUGMENTATION_CONTROL_SLICE.md"
CONTRACT = (
    BASE
...
RECEIPT = ROOT / "machine" / "runs" / "2026-08-25-stage099-p2-local.json"
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "stage099_internal_evidence_external_augmentation_control_slice", MODULE
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load Stage099 P2 control-slice module")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Stage099InternalEvidenceExternalAugmentationPhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.result = cls.module.execute_internal_evidence_external_augmentation_control_slice(
            cls.control_input
        )
    ...
    def test_identity_authority_and_phase_boundary_are_exact(self):
        ...
        self.assertEqual("IDS-STAGE099-P2", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE099-P2", contract["task_id"])
        self.assertEqual("ACC-STAGE-099", contract["acceptance_id"])
        self.assertEqual(
            "PHASE2_INTERNAL_EVIDENCE_EXTERNAL_AUGMENTATION_CONTROL_SLICE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE099-P2-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE099-P3-GATE", contract["next_gate"])
        source = contract["source_authority"]
        self.assertEqual(
            "FROZEN_STAGE099_TASKPACK_STAGE099_PHASE1_AND_STAGE098_REVIEWED_PROMPT_VERSIONING_CONTROL_ARTIFACTS_ONLY",
            source["authority"],
        )
        ...
        self.assertEqual(
            ...
            predecessor["stage099_phase1_result"],
        )
    ...
    def test_fixed_control_input_carries_required_reference_shapes(self):
        ...
        self.assertEqual(19, len(self.module.INPUT_FIELDS))
            ...
            with self.subTest(scenario=request["control_scenario"]):
                ...
                for field, value in request.items():
                    if field == "control_scenario" or value is None:
                        continue
                    self.assertTrue(
                        value.startswith(":control:stage099-p2:")
                ...
                for field in (
                    ...
                    self.assertTrue(request[field].startswith(":control:stage099-p2:"))
                self.assertTrue(
    ...
    def test_query_index_version_and_selected_evidence_records_remain_reference_only(self):
        requests = self.control_input[self.module.CONTROL_FIELDS[0]]
        bindings = self.result["answer_contract_and_prompt_binding_control_projections"]
        records = self.result[
        ...
        for request, binding, record in zip(requests, bindings, records):
            with self.subTest(scenario=request["control_scenario"]):
                self.assertEqual(
                    self.module.PHASE1_INTERNAL_EVIDENCE_EXTERNAL_AUGMENTATION_CONTROL_REF,
                    binding["stage099_phase1_contract_ref"],
                )
                self.assertEqual(
                    self.module.STAGE098_REVIEW_PROMPT_VERSIONING_CONTROL_REF,
                    binding["stage098_review_prompt_versioning_control_ref"],
                )
                for field in (
                    "query_ref",
                    "index_version_ref",
                ...
                self.assertEqual(request["prompt_version_ref"], binding["prompt_version_ref"])
                self.assertEqual(request["model_version_ref"], binding["model_version_ref"])
                self.assertEqual(
                    "CONTROL_QUERY_INDEX_PROMPT_MODEL_AND_SELECTED_EVIDENCE_RECORDS_REFERENCE_ONLY",
                    record["record_shape_state"],
                )

    def test_source_types_remain_separated_when_external_augmentation_opinion_is_displayed(self):
        requests = self.control_input[self.module.CONTROL_FIELDS[0]]
        ...
        for request, projection in zip(requests, projections):
            with self.subTest(scenario=request["control_scenario"]):
                ...
                self.assertEqual(
                    ...
                    projection["display_label_is_not_source_type_state"],
                )
                self.assertEqual(
                    "CONTROL_DISPLAY_PRESERVES_BOTTOM_SOURCE_TYPES",
                    projection["display_preserves_underlying_source_types_state"],
                )
    ...
    def test_nonfixed_control_input_produces_no_projection(self):
        ...
        invalid_input[self.module.CONTROL_FIELDS[0]][0]["external_augmentation_ref"] = (
            ":control:stage099-p2:altered:reference-only"
        )
        invalid_input["unexpected"] = []
        rejected = self.module.execute_internal_evidence_external_augmentation_control_slice(
            invalid_input
        )
    ...
    def test_runtime_boundary_failure_contract_and_local_code_remain_closed(self):
        ...
        for state in (
            ...
            "EXTERNAL_AUGMENTATION_OPINION_DISPLAY_SOURCE_TYPE_LOST",
    ...
    def test_scope_rollback_and_successor_governance_keep_phase3_gate_explicit(self):
        ...
        for field in (
            "preserve_stage099_phase1_evidence",
            "preserve_stage098_review_evidence",
            "preserve_stage098_phase1_to_phase4_evidence",
        ):
            with self.subTest(field=field):
        ...
        if current == phase2_current:
            ...
            self.assertEqual("P2 受控最小切片已完成", acceptance_by_id["ACC-STAGE-099"])
            ...
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE099-P2-04"])
            self.assertIn("EVT-IDS-V0_1-STAGE099-P2-20260825-001", event_ids)
            self.assertEqual("IDS-STAGE099-P3-GATE", receipt["next_gate"])
            self.assertEqual(
                "PASS_INTERNAL_EVIDENCE_EXTERNAL_AUGMENTATION_CONTROL_SLICE_RUNTIME_DISABLED",
                receipt["result"],
            )
