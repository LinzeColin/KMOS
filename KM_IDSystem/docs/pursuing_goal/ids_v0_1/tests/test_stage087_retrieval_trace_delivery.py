...
SCOPE = BASE / "STAGE087_PHASE4_RETRIEVAL_TRACE_DELIVERY_EVIDENCE.md"
CONTRACT = (
    BASE / "index_version_schema" / "stage087_retrieval_trace_delivery_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage087_retrieval_trace_delivery.py"
TASKPACK = (
    ROOT
    / "docs"
    / "taskpacks"
...
P2_SCOPE = BASE / "STAGE087_PHASE2_RETRIEVAL_TRACE_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE / "index_version_schema" / "stage087_retrieval_trace_slice_contract.json"
)
P2_SLICE = BASE / "index_version_schema" / "stage087_retrieval_trace_control_slice.py"
P3_SCOPE = BASE / "STAGE087_PHASE3_RETRIEVAL_TRACE_CONTROLLED_SCENARIOS.md"
P3_CONTRACT = (
    BASE / "index_version_schema" / "stage087_retrieval_trace_scenarios_contract.json"
)
P3_MODULE = (
    BASE / "index_version_schema" / "stage087_retrieval_trace_controlled_scenarios.py"
)
P3_RUN = ROOT / "machine" / "runs" / "2026-08-23-stage087-p3-local.json"
...
class Stage087RetrievalTracePhase4DeliveryTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = _load_module("stage087_p4", MODULE)
        cls.phase3 = _load_module("stage087_p3", P3_MODULE)

    def _report(self):
        return self.module.build_retrieval_trace_phase4_delivery_report()

    def _phase3_report(self):
        return self.phase3.build_retrieval_trace_phase3_report()
    ...
    def test_contract_identity_authority_and_review_gate_are_exact(self):
        ...
        self.assertEqual("IDS-STAGE087-P4", contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE087-P4", contract["task_id"])
        self.assertEqual(
            "PHASE4_RETRIEVAL_TRACE_DELIVERY_EVIDENCE_RUNTIME_DISABLED",
            contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE087-P4-GATE", contract["entry_gate"])
        self.assertEqual("IDS-STAGE087-REVIEW-GATE", contract["next_gate"])
    ...
    def test_contract_replay_delivery_and_runtime_shapes_are_exact(self):
        ...
        self.assertEqual(426, replay["phase2_control_field_check_count"])
    ...
    def test_malformed_phase3_report_fails_closed_without_delivery_records(self):
        ...
        altered["phase2_control_record_field_check_count"] = 425
        failed = self.module.build_retrieval_trace_phase4_delivery_report(lambda: altered)
    ...
    def test_zero_runtime_phase_boundaries_and_chinese_feedback_are_explicit(self):
        ...
        self.assertTrue(report["stage087_started"])
        ...
        self.assertFalse(report["stage087_review_started"])
