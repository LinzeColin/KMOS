...
REVIEW_DOCUMENT = BASE / "STAGE094_STAGE_REVIEW.md"
CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage094_evidence_revocation_stage_review_contract.json"
)
MODULE = BASE / "index_version_schema" / "stage094_evidence_revocation_stage_review.py"
...
P3_MODULE = (
    BASE / "index_version_schema" / "stage094_evidence_revocation_controlled_scenarios.py"
)
P4_MODULE = BASE / "index_version_schema" / "stage094_evidence_revocation_delivery.py"
ROADMAP = ROOT / "docs" / "governance" / "roadmap.yaml"
EVENTS = ROOT / "docs" / "governance" / "events.jsonl"
STATUS = ROOT / "machine" / "facts" / "status.json"
PLAN = ROOT / "machine" / "facts" / "plan.json"
ACCEPTANCE = ROOT / "machine" / "facts" / "acceptance.json"
REVIEW_RUN = ROOT / "machine" / "runs" / "2026-08-24-stage094-review-local.json"
...
class Stage094ReviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage094_review_test", MODULE)
        cls.report = cls.module.build_evidence_revocation_stage094_review_report()
    ...
    def test_required_phase_artifacts_exist(self):
        ...
        self.assertEqual("IDS-STAGE094-REVIEW", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE094-REVIEW", self.contract["task_id"])
        self.assertEqual("IDS-STAGE094-REVIEW-GATE", self.contract["entry_gate"])
        self.assertEqual("IDS-STAGE095-P1-GATE", self.contract["next_gate"])

    def test_contract_keeps_single_authority_runtime_and_stage095_closed(self):
        ...
        self.assertEqual("18/14/6/5", replay["phase1_static_shape"])
        self.assertEqual(630, replay["phase2_control_field_check_count"])
    ...
    def test_authority_rollback_and_runtime_are_closed(self):
        ...
        self.assertTrue(invariants["owner_revocation_rule_boundary_preserved"])
        ...
        self.assertEqual(
            "PASS_EVIDENCE_REVOCATION_DELIVERY_EVIDENCE_RUNTIME_DISABLED",
    ...
    def test_next_stage_stays_closed_except_for_gate(self):
        ...
        self.assertTrue(invariants["stage095_gate_only_opens_after_review"])
        self.assertTrue(self.report["stage094_started"])
        self.assertTrue(self.report["stage094_review_started"])
        self.assertFalse(self.report["whole_stage_review_performed"])
        self.assertFalse(self.report["stage095_started"])
    ...
    def test_invalid_phase4_report_fails_closed(self):
        report = self.module.build_evidence_revocation_stage094_review_report(
    ...
    def test_owner_revocation_rule_drift_fails_closed(self):
        baseline = self.module._default_phase1_contract()

        def rule_drift():
            altered = copy.deepcopy(baseline)
            altered["evidence_revocation_contract"]["evidence_grade_definition_reference"][
        ...
        self.assertFalse(report["review_valid"])
        self.assertFalse(report["phase_results"]["P1"])
        self.assertIn("OWNER_REVOCATION_RULE_BOUNDARY_MISMATCH", report["failure_reasons"])
    ...
    def test_governance_projection_records_stage_review_when_current(self):
        ...
        if current == stage094_review_current:
            self.assertTrue(REVIEW_RUN.is_file())
            run = json.loads(REVIEW_RUN.read_text(encoding="utf-8"))
            acceptance_by_id = {item["id"]: item["status"] for item in acceptance["items"]}
            self.assertEqual("整阶段已复审", acceptance_by_id["ACC-STAGE-094"])
            self.assertEqual("已通过", acceptance_by_id["ACC-STAGE094-REVIEW-01"])
            self.assertEqual("已通过", acceptance_by_id["ACC-STAGE094-REVIEW-02"])
            self.assertEqual("已通过", acceptance_by_id["ACC-STAGE094-REVIEW-03"])
            self.assertEqual("已遵守", acceptance_by_id["ACC-STAGE094-REVIEW-04"])
            self.assertIn("EVT-IDS-V0_1-STAGE094-REVIEW-20260824-001", event_ids)
            self.assertEqual("IDS-STAGE095-P1-GATE", run["next_gate"])
            self.assertEqual(
                "PASS_REVIEWED_EVIDENCE_REVOCATION_RUNTIME_DISABLED", run["result"]
            )
            self.assertTrue(all(value == 0 for value in run["runtime_counts"].values()))
            self.assertEqual("IDS-V0_1-STAGE094-REVIEW", plan["task"])
            roadmap_text = ROADMAP.read_text(encoding="utf-8")
            self.assertIn("stage094_review_state:", roadmap_text)
            self.assertIn('current_phase_id: "IDS-STAGE094-REVIEW"', roadmap_text)
            self.assertIn('next_gate_id: "IDS-STAGE095-P1-GATE"', roadmap_text)
        else:
            self.assertIn(
                current,
                (
