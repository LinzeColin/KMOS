...
SCOPE = BASE / "STAGE091_PHASE3_EVIDENCE_GAP_HANDLING_CONTROLLED_SCENARIOS.md"
CONTRACT = (
    BASE
...
P2_SCOPE = BASE / "STAGE091_PHASE2_EVIDENCE_GAP_HANDLING_CONTROL_SLICE.md"
P2_CONTRACT = (
    BASE
    / "index_version_schema"
...
class Stage091EvidenceGapHandlingPhase3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.module = load_module("stage091_phase3_scenarios", MODULE)
        cls.phase2 = load_module("stage091_phase2_slice", P2_MODULE)
        cls.report = cls.module.build_evidence_gap_handling_phase3_report()

    def _phase2_report(self):
        return self.phase2.execute_evidence_gap_handling_control_slice(
    ...
    def test_artifacts_phase_identity_and_frozen_taskpack_exist(self):
        ...
        self.assertEqual("IDS-STAGE091-P3", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE091-P3", self.contract["task_id"])
        self.assertEqual("IDS-STAGE091-P4-GATE", self.contract["next_gate"])
    ...
    def test_contract_binds_single_authority_phase2_and_stage_boundary(self):
        ...
        self.assertEqual(78, replay["expected_projection_fields_per_request"])
        self.assertEqual(468, replay["expected_phase2_field_check_count"])
    ...
    def test_report_replays_exact_phase2_shape_without_runtime(self):
        ...
        self.assertEqual(468, report["phase2_field_check_count"])
    ...
    def test_each_exception_scenario_is_control_only_and_whitebox_gated(self):
        ...
        self.assertIsNone(scenarios["no_internal_evidence_gap_control"]["evidence_id_ref"])
        self.assertTrue(
            all(
    ...
    def test_contract_failure_and_protected_boundaries_are_explicit(self):
        ...
        self.assertTrue(rollback["preserve_stage091_phase1_and_phase2"])
        self.assertTrue(rollback["preserve_stage090_reviewed_artifacts"])

    def test_invalid_phase2_output_returns_controlled_failure(self):
        failed = self.module.build_evidence_gap_handling_phase3_report(lambda _: {})
    ...
    def test_phase2_runtime_signal_returns_controlled_failure(self):
        ...
        failed = self.module.build_evidence_gap_handling_phase3_report(runtime_signal)
        self.assertFalse(failed["valid"])
    ...
    def test_nonopaque_reference_returns_controlled_failure(self):
        def nonopaque_reference(_control_input):
            altered = copy.deepcopy(self._phase2_report())
            altered["evidence_gap_relation_control_projections"][0][
                "evidence_gap_ref"
            ] = "unscoped-reference"
            return altered

        failed = self.module.build_evidence_gap_handling_phase3_report(
            nonopaque_reference
        )
        self.assertFalse(failed["valid"])
    ...
    def test_exception_control_drift_returns_specific_failure(self):
        ...
        failed = self.module.build_evidence_gap_handling_phase3_report(low_ocr_drift)
        self.assertFalse(failed["valid"])
    ...
    def test_revocation_and_masquerade_drift_return_specific_failures(self):
        def revoked_drift(_control_input):
            altered = copy.deepcopy(self._phase2_report())
            altered["report_status_impact_control_projections"][4][
                "report_status_impact_state"
            ] = "CONTROL_REPORT_STATUS_REFERENCE_PENDING_EVIDENCE_GAP_REVIEW"
            return altered

        revoked = self.module.build_evidence_gap_handling_phase3_report(revoked_drift)
        self.assertEqual(
            "REVOKED_EVIDENCE_REPORT_STATUS_IMPACT_MISSING",
        ...
        def masquerade_drift(_control_input):
            ...
            altered["critical_conclusion_binding_control_projections"][1][
                ...
                ":control:stage091-p2:evidence-grade-A:reference-only"
            )
            return altered

        masquerade = self.module.build_evidence_gap_handling_phase3_report(
            masquerade_drift
        )
        self.assertEqual(
            "LOW_GRADE_EVIDENCE_MASQUERADING_AS_HIGH_TRUST",
