...
SCOPE = BASE / "STAGE084_PHASE2_VECTOR_RETRIEVAL_BASELINE_CONTROL_SLICE.md"
CONTRACT = (
    BASE
...
P1_CONTRACT = (
    BASE
    / "index_version_schema"
    / "stage084_vector_retrieval_baseline_contract.json"
)
STAGE083_REVIEW = BASE / "STAGE083_STAGE_REVIEW.md"


def load_module():
    spec = importlib.util.spec_from_file_location("stage084_slice", MODULE)
...
class Stage084VectorRetrievalBaselinePhase2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
        cls.control_input = slice_module.build_control_input()
        cls.result = slice_module.execute_vector_retrieval_control_slice(
            cls.control_input
        )
    ...
    def test_frozen_sources_identity_and_single_authority_are_explicit(self):
        ...
        self.assertEqual(
            "ids.stage084.vector_retrieval_baseline.phase2.v1",
            self.contract["schema_version"],
        )
        self.assertEqual("STAGE-084", self.contract["stage"])
        self.assertEqual("IDS-STAGE084-P2", self.contract["phase"])
        self.assertEqual("IDS-V0_1-STAGE084-P2", self.contract["task_id"])
        self.assertEqual("ACC-STAGE-084", self.contract["acceptance_id"])
        self.assertEqual(
            "PHASE2_VECTOR_RETRIEVAL_BASELINE_CONTROL_SLICE_RUNTIME_DISABLED",
            self.contract["contract_state"],
        )
        self.assertEqual("IDS-STAGE084-P3-GATE", self.contract["next_gate"])
        for field in (
    ...
    def test_control_outputs_have_exact_contract_fields_and_counts(self):
        ...
        self.assertEqual(
            "COMPLETED_IN_MEMORY_VECTOR_RETRIEVAL_CONTROL_SLICE",
    ...
    def test_candidate_selected_trace_and_vector_contract_bindings_match(self):
    ...
    def test_non_fixed_input_and_vector_only_are_fail_closed(self):
        ...
        rejected = slice_module.execute_vector_retrieval_control_slice(tampered)
        ...
        for state in (
            ...
            "PHASE2_VECTOR_RETRIEVAL_CONTROL_INPUT_REJECTED",
        ):
    ...
    def test_stage_boundary_scope_and_rollback_stop_before_phase3(self):
        ...
        self.assertEqual(
            "PASS_VECTOR_RETRIEVAL_BASELINE_CONTRACT_RUNTIME_DISABLED",
            rollback["return_to"],
        )
        self.assertTrue(rollback["preserve_stage084_phase1_evidence"])
