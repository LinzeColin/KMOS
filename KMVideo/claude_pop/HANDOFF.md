...
| 成片、逐帧图、配乐分轨、字体/底板/依赖离线包、审片对比图 | GitHub Release **`KMVideo-claude-pop-v1`**（附件清单与 sha256 见 §7） |
| 参考片（Claude 官方 MV 原片） | GitHub Release `ClaudeVideo`，sha256 `141f4f2e14c7923821eda9f87ae824ebea32fadd7c56c3d56b523a0ef4a0b7f5` |

## 1. 三种接手方式（按省事程度排）

```bash
# A. 只要成片：下载 + 校验，四个文件落到 out/
KMVideo/claude_pop/fetch_release.sh video
...
# C. 从零重建（不下 Release）：build.sh 自己下字体、npm ci、画底板、合成配乐、渲全片
...
| p5 / p5.brush / puppeteer-core | 2.3.3 / 2.2.3 / 25.11.0 | `ClaudeAnimationBase/package-lock.json` 锁定；Release 里有 `node_modules.tar.gz`（纯 JS，跨平台） |
...
| 字体 | 4 款 SIL OFL / Apache | `fonts/SOURCES.txt` 有来源与 sha256；Release `build_env.zip` 里有现成文件 |
...
见同一 Release 的 `SHA256SUMS.txt`（`fetch_release.sh` 会逐个校验）。
...
| `frames.zip` | 1104 张 1080×1920 逐帧 JPG | 不重渲就能重新编码 / 换音乐 / 局部改 |
