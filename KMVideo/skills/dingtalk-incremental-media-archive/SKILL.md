...
8. 此 SMB 挂载禁止使用 macOS `shutil.copyfile` / `fcopyfile` 快速路径及 SMB `os.replace` 发布路径：前者会返回成功却写出等长全零文件，后者可能显示陈旧或截断大小。必须向最终路径显式分块读写并 `fsync`；目录创建、写入及源/SMB 大小和有限头尾字节比对属于同一次受控尝试。第一次不符时等 1 秒再复读一次，仍不符才删除未验证目标并重试一次；最终失败时必须立即在群 manifest 写入 `smb_status=failed`、素材 DWS 身份、相对路径和错误，停止该条且不得标记 `smb_status=complete`。`--repair-smb --apply` 只按此失败记录恢复该条，不重新枚举整个群历史。已标记完成但文件缺失、大小不符或头部全零时，同样只按现有 manifest 素材身份重下并替换该文件。
