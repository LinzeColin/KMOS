...
8. 此 SMB 挂载禁止使用 macOS `shutil.copyfile` / `fcopyfile` 快速路径：它会返回成功却写出等长全零文件。必须显式分块读写并 `fsync`；写入后比对源与 SMB 的大小及有限头尾字节，未通过先删除该次未验证目标并用新临时文件重试一次，仍失败才停止且不得标记 `smb_status=complete`。已标记完成但文件缺失、大小不符或头部全零时，只按现有 manifest 素材身份重下并替换该文件，不重新枚举整个群历史。
