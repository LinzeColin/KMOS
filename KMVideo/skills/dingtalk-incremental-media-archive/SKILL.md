...
8. 此 SMB 挂载禁止使用 macOS `shutil.copyfile` / `fcopyfile` 快速路径：它会返回成功却写出等长全零文件。必须显式分块读写并 `fsync`；目录创建、写入及源/SMB 大小和有限头尾字节比对属于同一次受控尝试，任一步骤未通过先删除该次未验证目标并用新临时文件重试一次，仍失败才停止且不得标记 `smb_status=complete`；SMB 若已报告临时文件不存在，则该临时文件视为已清理，不能掩盖真实媒体结果。已标记完成但文件缺失、大小不符或头部全零时，只按现有 manifest 素材身份重下并替换该文件，不重新枚举整个群历史。
